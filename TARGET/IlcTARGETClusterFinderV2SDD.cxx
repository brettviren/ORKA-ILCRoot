...
ClassImp(IlcTARGETClusterFinderV2SDD)
...
IlcTARGETClusterFinderV2SDD::IlcTARGETClusterFinderV2SDD(IlcTARGETDetTypeRec* dettyp):IlcTARGETClusterFinderV2(dettyp){
...
void IlcTARGETClusterFinderV2SDD::FindRawClusters(Int_t mod){
...
void IlcTARGETClusterFinderV2SDD::FindClustersSDD(TClonesArray *digits) {
  //------------------------------------------------------------
...
void IlcTARGETClusterFinderV2SDD::
  ...
  for (Int_t s=0; s<2; s++)
    for (Int_t i=0; i<nMaxBin; i++) {
      ...
      for (k=0; k<npeaks; k++) {
         ...
         IlcTARGETRecPoint c(fDetTypeRec->GetTARGETgeom());
	 ...
	 */
	 ...
	 if(clusters) new (cl[ncl]) IlcTARGETRecPoint(c); 
	 else {
...
void IlcTARGETClusterFinderV2SDD::RawdataToClusters(IlcRawReader* rawReader,TClonesArray** clusters){
    //------------------------------------------------------------
...
void IlcTARGETClusterFinderV2SDD::FindClustersSDD(IlcTARGETRawStream* input, 
...
void IlcTARGETClusterFinderV2SDD::CorrectPosition(Float_t &z, Float_t&y){

  //correction of coordinates using the maps stored in the DB

  IlcTARGETCalibrationSDD* cal = (IlcTARGETCalibrationSDD*)GetResp(fModule);
  static const Int_t nbint = cal->GetMapTimeNBin();
  static const Int_t nbina = cal->Chips()*cal->Channels();
  Float_t stepa = (GetSeg()->Dpz(0,0))/10000.; //anode pitch in cm
  Float_t stept = (GetSeg()->Dx(0)/cal->GetMapTimeNBin()/2.)/10.;
  Float_t xdet,zdet;
  fDetTypeRec->GetTARGETgeom()->TrackingV2ToDetL(fModule,y,z,xdet,zdet);

  Int_t bint = TMath::Abs((Int_t)(xdet/stept));
  if(xdet>=0) bint+=(Int_t)(nbint/2.);
  if(bint>nbint) IlcError("Wrong bin number!");

  Int_t bina = TMath::Abs((Int_t)(zdet/stepa));
  if(zdet>=0) bina+=(Int_t)(nbina/2.);
  if(bina>nbina) IlcError("Wrong bin number!");

  Float_t devz = cal->GetMapACell(bina,bint)/10000.;
  Float_t devx = cal->GetMapTCell(bina,bint)/10000.;
  zdet+=devz;
  xdet+=devx;
  fDetTypeRec->GetTARGETgeom()->DetLToTrackingV2(fModule,xdet,zdet,y,z);

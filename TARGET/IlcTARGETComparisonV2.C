 ...
 *  The ESD tracks must be in an appropriate state (kTARGETin or kTARGETrefit)    *
 *                                                                          *
...
Int_t GoodTracksTARGET(const Char_t *dir=".");
...
Int_t IlcTARGETComparisonV2
(Float_t ptcutl=0.2, Float_t ptcuth=10., const Char_t *dir=".", Float_t ratio=0.0) {
   gBenchmark->Start("IlcTARGETComparisonV2");

   ::Info("IlcTARGETComparisonV2.C","Doing comparison...");
   ...
   Char_t fname[100];
   sprintf(fname,"%s/GoodTracksTARGET.root",dir);

   TFile *refFile=TFile::Open(fname,"old");
   if (!refFile || !refFile->IsOpen()) {
   ::Info("IlcTARGETComparisonV2.C","Marking good tracks (will take a while)...");
     if (GoodTracksTARGET(dir)) {
        ::Error("IlcTARGETComparisonV2.C","Can't generate the reference file !");
        return 1;
     }
   }
   refFile=TFile::Open(fname,"old");
   if (!refFile || !refFile->IsOpen()) {
     ::Error("IlcTARGETComparisonV2.C","Can't open the reference file !");
     return 1;
   }   
  
   TTree *itsTree=(TTree*)refFile->Get("itsTree");
   if (!itsTree) {
     ::Error("IlcTARGETComparisonV2.C","Can't get the reference tree !");
     return 2;
   }
   TBranch *branch=itsTree->GetBranch("TARGET");
   if (!branch) {
     ::Error("IlcTARGETComparisonV2.C","Can't get the TARGET branch !");
     return 3;
   }
   TClonesArray dummy("IlcTrackReference",1000), *refs=&dummy;
   ...
   if ((!ef)||(!ef->IsOpen())) {
      sprintf(fname,"%s/IlcESDits.root",dir);
      ef=TFile::Open(fname);
      if ((!ef)||(!ef->IsOpen())) {
         ::Error("IlcTARGETComparisonV2.C","Can't open IlcESDits.root !");
         return 4;
      }
   }
   ...
   if (!esdTree) {
      ::Error("IlcTARGETComparison.C", "no ESD tree found");
      return 6;
   }
   ...
   while (esdTree->GetEvent(e)) {
     ...
     Int_t k;
     for (k=0; k<ngood; k++) {
	IlcTrackReference *ref=(IlcTrackReference*)refs->UncheckedAt(k); 
	   ...
	   UInt_t status=t->GetStatus();
           ...
           if ((status&IlcESDtrack::kTARGETrefit)==0) continue;
	...
	if (esd->GetTARGETFakeRatio()<ratio) tlab = TMath::Abs(tlab);
        ...
        Float_t dedx=esd->GetTARGETsignal();
        hep->Fill(mom,dedx,1.);
   ...
   TFile fc("IlcTARGETComparisonV2.root","RECREATE");
   c1->Write();
   c2->Write();
   fc.Close();

   gBenchmark->Stop("IlcTARGETComparisonV2");
   gBenchmark->Show("IlcTARGETComparisonV2");
...
Int_t GoodTracksTARGET(const Char_t *dir) {
   ...
   Char_t fname[100];
   sprintf(fname,"%s/gilc.root",dir);

   IlcRunLoader *rl = IlcRunLoader::Open(fname,"COMPARISON");
   if (!rl) {
      ::Error("GoodTracksTARGET","Can't start session !");
      return 1;
   }
   ...
   IlcTARGET *TARGET=(IlcTARGET*)rl->GetIlcRun()->GetDetector("TARGET");
   if (!TARGET) {
      ::Error("GoodTracksTARGET","Can't get the TARGET !");
      delete rl;
      return 2;
   }
   IlcTARGETgeom *geom=TARGET->GetTARGETgeom();
   if (!geom) {
      ::Error("GoodTracksTARGET","Can't get the TARGET geometry !"); 
      delete rl;
      return 3;
   }
   ...
   if (itsl == 0x0) {
       ::Error("GoodTracksTARGET","Can not find the TARGETLoader");
       delete rl;
       return 4;
   }
   itsl->LoadRecPoints();
   ...
   ::Info("GoodTracksTARGET","Number of events : %d\n",nev);  

   sprintf(fname,"%s/GoodTracksTPC.root",dir);
   TFile *tpcFile=TFile::Open(fname);
   if ((!tpcFile)||(!tpcFile->IsOpen())) {
       ::Error("GoodTracksTARGET","Can't open the GoodTracksTPC.root !");
       delete rl;
       return 5; 
   }
   TClonesArray dum("IlcTrackReference",1000), *tpcRefs=&dum;
   TTree *tpcTree=(TTree*)tpcFile->Get("tpcTree");
   if (!tpcTree) {
       ::Error("GoodTracksTARGET","Can't get the TPC reference tree !");
       delete rl;
       return 6;
   }
   TBranch *tpcBranch=tpcTree->GetBranch("TPC");
   if (!tpcBranch) {
      ::Error("GoodTracksTARGET","Can't get the TPC reference branch !");
      delete rl;
      return 7;
   }
   ...
   TClonesArray dummy2("IlcTrackReference",1000), *itsRefs=&dummy2;
   TTree itsTree("itsTree","Tree with info about the reconstructable TARGET tracks");
   itsTree.Branch("TARGET",&itsRefs);
   ...
   for (Int_t e=0; e<nev; e++) {
     ...
     if (!cTree) {
        ::Error("GoodTracksTARGET","Can't get the cluster tree !"); 
        delete rl;
        return 8;
     }
     TBranch *branch=cTree->GetBranch("TARGETRecPoints");
     if (!branch) {
        ::Error("GoodTracksTARGET","Can't get the clusters branch !"); 
        delete rl;
        return 9;
     }
     ...
     for (k=0; k<entr; k++) {
         ...
         if (lay<1 || lay>6) {
	    ::Error("GoodTracksTARGET","Wrong layer !"); 
            delete rl;
            return 10;
         }
         while (ncl--) {
            IlcTARGETRecPoint *pnt=(IlcTARGETRecPoint*)clusters->UncheckedAt(ncl);
...
// 		 cerr<<"Wrong label: "<<l2<<endl;
     ...
     Int_t nt=0;
     for (k=0; k<nk; k++) {
        IlcTrackReference *tpcRef=(IlcTrackReference *)tpcRefs->UncheckedAt(k);
	...
	IlcTrackReference *ref=new((*itsRefs)[nt]) IlcTrackReference(*tpcRef);

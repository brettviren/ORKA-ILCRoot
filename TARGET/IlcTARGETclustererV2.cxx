...
IlcTARGETclustererV2::IlcTARGETclustererV2(const IlcTARGETgeom *geom) {
  ...
  if (mmax>2200) {
     Fatal("IlcTARGETclustererV2","Too many TARGET subdetectors !"); 
  }
  Int_t m;
  for (m=0; m<mmax; m++) {
...
Int_t IlcTARGETclustererV2::Digits2Clusters(TTree *dTree, TTree *cTree) {
  //------------------------------------------------------------
  // This function creates TARGET clusters
  //------------------------------------------------------------
  Int_t ncl=0;
//  cout <<"IlcTARGETclustererV2::Digits2Clusters ...chiama questo metodo..."<<endl;
  ...
  TClonesArray *digitsSPD=new TClonesArray("IlcTARGETdigitSPD",3000);
  dTree->SetBranchAddress("TARGETDigitsSPD",&digitsSPD);
  TClonesArray *digitsSDD=new TClonesArray("IlcTARGETdigitSDD",3000);
  dTree->SetBranchAddress("TARGETDigitsSDD",&digitsSDD);
  TClonesArray *digitsSSD=new TClonesArray("IlcTARGETdigitSSD",3000);
  dTree->SetBranchAddress("TARGETDigitsSSD",&digitsSSD);

  TClonesArray *clusters=new TClonesArray("IlcTARGETclusterV2",1000);
...
void IlcTARGETclustererV2::Digits2Clusters(IlcRawReader* rawReader) {
  //------------------------------------------------------------
  ...
  IlcLoader* itsLoader = runLoader->GetLoader("TARGETLoader");
  if (!itsLoader) {
    Error("Digits2Clusters", "no loader for TARGET found");
    return;
  }
  if (!itsLoader->TreeR()) itsLoader->MakeTree("R");
  TTree* cTree = itsLoader->TreeR();

  TClonesArray *array=new TClonesArray("IlcTARGETclusterV2",1000);
  ...
  rawReader->Reset();
  IlcTARGETRawStreamSDD inputSDD(rawReader);
  ...
  for (Int_t iModule = 0; iModule < fNModules; iModule++) {
    ...
    if (!array) {
      ...
      array = new TClonesArray("IlcTARGETclusterV2");
    }
  ...
  Info("Digits2Clusters", "total number of found clusters in TARGET: %d\n", 
       nClusters);
}
  ...
  //------------------------------------------------------------
    ...
    if (label>=0) {
       ...
       while (part->P() < 0.005) {
          ...
          label=m;
          part=(TParticle*)gIlc->GetMCApp()->Particle(label);
        }
...
void IlcTARGETclustererV2::RecPoints2Clusters
...
FindClustersSPD(const TClonesArray *digits, TClonesArray *clusters) {
  ...
  IlcBin *bins=new IlcBin[kMAXBIN];
  ...
  for (k=0; k<kMAXBIN; k++) {
     ...
     d=(IlcTARGETdigitSPD*)digits->UncheckedAt(idx[0]);
     Int_t ymin=d->GetCoord2(),ymax=ymin;
     Int_t zmin=d->GetCoord1(),zmax=zmin;

     for (Int_t l=0; l<ni; l++) {
        d=(IlcTARGETdigitSPD*)digits->UncheckedAt(idx[l]);
	...
	if (milab[9]>0) CheckLabels2(milab);
	 ...
	 new (cl[n]) IlcTARGETclusterV2(milab,lp,info); n++; 	 
       }
  }
...
void IlcTARGETclustererV2::FindClustersSPD(IlcTARGETRawStream* input, 
					TClonesArray** clusters) 
{
  ...
  while (kTRUE) {
    ...
    if (!next || input->IsNewModule()) {
      ...
      if (bins) { 
	clusters[iModule] = new TClonesArray("IlcTARGETclusterV2");
	Int_t nClusters = 0;
...
//	  label[3] = iModule;
	  ...
	  for (Int_t iiz=zmin; iiz<=zmax;iiz+=idz)
		...
		IlcTARGETclusterV2(label, hit,info); 
	      nClusters++;
	    }
	}
...
Bool_t IlcTARGETclustererV2::IsMaximum(Int_t k,Int_t max,const IlcBin *bins) {
  //------------------------------------------------------------
...
void IlcTARGETclustererV2::
MakeCluster(Int_t k,Int_t max,IlcBin *bins,UInt_t m,IlcTARGETclusterV2 &c) {
  //------------------------------------------------------------
...
void IlcTARGETclustererV2::
FindClustersSDD(IlcBin* bins[2], Int_t nMaxBin, Int_t nzBins, 
	 ...
	 for (Int_t di=-2; di<=2;di++)
	   for (Int_t dj=-3;dj<=3;dj++){
		 ...
		 if (TMath::Abs(di)<2&&TMath::Abs(dj)<2){
		   IlcTARGETdigitSDD* d=(IlcTARGETdigitSDD*)digits->UncheckedAt(b->GetIndex());
	 ...
	 */
	 ...
	 if (digits) {	  
	   ...
	   //	   IlcTARGETdigitSDD* d=(IlcTARGETdigitSDD*)digits->UncheckedAt(b->GetIndex());
	   {
         ...
         new (cl[ncl]) IlcTARGETclusterV2(c); ncl++;
      }
    }
}
...
FindClustersSDD(const TClonesArray *digits, TClonesArray *clusters) {
  ...
  IlcBin *bins[2];
  bins[0]=new IlcBin[kMAXBIN];
  bins[1]=new IlcBin[kMAXBIN];

  IlcTARGETdigitSDD *d=0;
  Int_t i, ndigits=digits->GetEntriesFast();
  for (i=0; i<ndigits; i++) {
     d=(IlcTARGETdigitSDD*)digits->UncheckedAt(i);
...
void IlcTARGETclustererV2::FindClustersSDD(IlcTARGETRawStream* input, 
					TClonesArray** clusters) 
{
  //------------------------------------------------------------
  ...
  IlcBin *binsSDDInit = new IlcBin[kMaxBin];
  IlcBin *binsSDD1 = new IlcBin[kMaxBin];
  IlcBin *binsSDD2 = new IlcBin[kMaxBin];
  IlcBin *bins[2] = {NULL, NULL};
  ...
  while (kTRUE) {
	...
	bins[0] = bins[1] = NULL;
      ...
      bins[0]=binsSDD1;
      bins[1]=binsSDD2;
      memcpy(binsSDD1,binsSDDInit,sizeof(IlcBin)*kMaxBin);
      memcpy(binsSDD2,binsSDDInit,sizeof(IlcBin)*kMaxBin);
    }
  ...
  Info("FindClustersSDD", "found clusters in TARGET SDD: %d", nClustersSDD);
}



void IlcTARGETclustererV2::
FindClustersSSD(Ilc1Dcluster* neg, Int_t nn, 
...
//   }  
  ...
  for (Int_t ip=0;ip<np;ip++){
	...
	milab[ilab+3] = neg[j].GetLabel(ilab);
      ...
      IlcTARGETclusterV2 * cl2 = new (cl[ncl]) IlcTARGETclusterV2(milab,lp,info);
      ncl++;
...
void IlcTARGETclustererV2::FindClustersSSD(IlcTARGETRawStream* input, 
					TClonesArray** clusters) 
{
  //------------------------------------------------------------
...
FindClustersSPD(const TClonesArray *digits, TClonesArray *clusters) {
  //------------------------------------------------------------
  // Actual SPD cluster finding based on IlcTARGETClusterFinderSPD
  //------------------------------------------------------------
  static IlcTARGET *its=(IlcTARGET*)gIlc->GetModule("TARGET");
  static TClonesArray *points=its->RecPoints();
  static IlcTARGETsegmentationSPD *seg=
         (IlcTARGETsegmentationSPD *)its->DetType(0)->GetSegmentationModelSPD();
  static IlcTARGETClusterFinderSPD cf(seg, (TClonesArray*)digits, points);
...
FindClustersSDD(const TClonesArray *digits, TClonesArray *clusters) {
  //------------------------------------------------------------
  // Actual SDD cluster finding based on IlcTARGETClusterFinderSDD
  //------------------------------------------------------------
  static IlcTARGET *its=(IlcTARGET*)gIlc->GetModule("TARGET");
  static TClonesArray *points=its->RecPoints();
  static IlcTARGETresponseSDD *resp=
        (IlcTARGETresponseSDD *)its->DetType(1)->GetResponseModel();
  static IlcTARGETsegmentationSDD *seg=
         (IlcTARGETsegmentationSDD *)its->DetType(1)->GetSegmentationModel();
  static IlcTARGETClusterFinderSDD 
  ...
  while (nc--) { //To be consistent with the SSD cluster charges
     IlcTARGETRecPoint *p=(IlcTARGETRecPoint*)points->UncheckedAt(nc);
     p->SetQ(p->GetQ()/12.);
  }
  RecPoints2Clusters(points, fI, clusters);
  its->ResetClusters(1);
...
FindClustersSSD(const TClonesArray *digits, TClonesArray *clusters) {
  //------------------------------------------------------------
  // Actual SSD cluster finding based on IlcTARGETClusterFinderSSD
  //------------------------------------------------------------
  static IlcTARGET *its=(IlcTARGET*)gIlc->GetModule("TARGET");
  static TClonesArray *points=its->RecPoints();
  static IlcTARGETsegmentationSSD *seg=
         (IlcTARGETsegmentationSSD *)its->DetType(2)->GetSegmentationModel();
  static IlcTARGETClusterFinderSSD cf(seg,(TClonesArray*)digits);

...
/* $Id: IlcTARGETTrackerV1.cxx,v 1.2 2009/12/11 11:36:18 vitomeg Exp $ */

// The purpose of this class is to permorm the TARGET tracking. The 
// constructor has the task to iniziilcze some private members. The method 
...
// trough the TARGET, and the file where the recpoints are registered. The 
// method Recursivetracking is a recursive function that performs the 
// tracking trough the TARGET The method Intersection found the layer, ladder 
...
#include "IlcMagF.h"
#include "IlcTARGETTrackV1.h"
#include "IlcTARGETIOTrack.h"
#include "IlcTARGETRad.h"   
#include "IlcTPCtracker.h"
#include "IlcTARGETTrackerV1.h"
#include "IlcTARGETPid.h"
#include "IlcMC.h"

ClassImp(IlcTARGETTrackerV1)
 //______________________________________________________________________
IlcTARGETTrackerV1::IlcTARGETTrackerV1() {
  //Default constructor
  //fTARGET = 0;
  fresult = 0;
  fPtref=0.;
...
//______________________________________________________________________
IlcTARGETTrackerV1::IlcTARGETTrackerV1(Int_t evnumber, Bool_t flag) {
  ...
  frl       = new IlcTARGETRad(imax,jmax);
  ...
  if (!loader) {
    Error("IlcTARGETTrackerV1", "TARGET loader not found");
    return;
  }
  fDetTypeRec = new IlcTARGETDetTypeRec(loader);
  IlcTARGETgeom* g1 = loader->GetTARGETgeom();
  ...
  fDetx[0] = ((IlcTARGETgeomSPD*)(g1->GetShape(1, ll, dd)))->GetDx();
  fDetz[0] = ((IlcTARGETgeomSPD*)(g1->GetShape(1, ll, dd)))->GetDz();
  
  fDetx[1] = ((IlcTARGETgeomSPD*)(g1->GetShape(2, ll, dd)))->GetDx();
  fDetz[1] = ((IlcTARGETgeomSPD*)(g1->GetShape(2, ll, dd)))->GetDz();

    fDetx[2] = ((IlcTARGETgeomSDD*)(g1->GetShape(3, ll, dd)))->GetDx();
    fDetz[2] = ((IlcTARGETgeomSDD*)(g1->GetShape(3, ll, dd)))->GetDz();

    fDetx[3] = ((IlcTARGETgeomSDD*)(g1->GetShape(4, ll, dd)))->GetDx();
    fDetz[3] = ((IlcTARGETgeomSDD*)(g1->GetShape(4, ll, dd)))->GetDz();

    fDetx[4] = ((IlcTARGETgeomSSD*)(g1->GetShape(5, ll, dd)))->GetDx();
    fDetz[4] = ((IlcTARGETgeomSSD*)(g1->GetShape(5, ll, dd)))->GetDz();

    fDetx[5] = ((IlcTARGETgeomSSD*)(g1->GetShape(6, ll, dd)))->GetDx();
    fDetz[5] = ((IlcTARGETgeomSSD*)(g1->GetShape(6, ll, dd)))->GetDz();
    //cout<<"    Detx     Detz\n";
  ...
  fNRecPoints = new Int_t[numOfModules];
		 ...
		 for(Int_t module=0; module<numOfModules; module++) {				
		  ...
		  for(ind=0; ind<fNRecPoints[module]; ind++) {	  
		    recp=(IlcTARGETRecPoint*)frecPoints->UncheckedAt(ind);			 			 
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    IlcMagF * fieldPointer = gIlc->Field();
...
//______________________________________________________________________
IlcTARGETTrackerV1::IlcTARGETTrackerV1(const IlcTARGETTrackerV1 &cobj) : TObject(cobj) {
    // Origin  A. Badala' and G.S. Pappalardo:
    // e-mail Angela.Badala@ct.infn.it, Giuseppe.S.Pappalardo@ct.infn.it
    // copy constructor
	 
  //	 *fTARGET = *cobj.fTARGET;
    ...
    *frl = *cobj.frl;
    ...
    IlcRunLoader* rl = IlcRunLoader::Open("gilc.root");
    rl->CdGAFile();
    IlcTARGETgeom* g1 = (IlcTARGETgeom*)gDirectory->Get("IlcTARGETgeom");
...
/*
//______________________________________________________________________
void IlcTARGETTrackerV1::DelMatrix(Int_t numOfModules) {
...
//______________________________________________________________________
IlcTARGETTrackerV1 &IlcTARGETTrackerV1::operator=(const IlcTARGETTrackerV1 &obj) {
    // Origin  A. Badala' and G.S. Pappalardo:
    // e-mail Angela.Badala@ct.infn.it, Giuseppe.S.Pappalardo@ct.infn.it  
  ...
  //	 *fTARGET = *obj.fTARGET;
    ...
    *frl = *obj.frl;
    ...
    rl->CdGAFile();
    IlcTARGETgeom* g1 = (IlcTARGETgeom*)gDirectory->Get("IlcTARGETgeom");
    //	IlcTARGETgeom *g1 = fTARGET->GetTARGETgeom();  
   Int_t numOfModules = g1->GetIndexMax();
  fRecCylR = new Double_t *[numOfModules];
  fRecCylPhi = new Double_t *[numOfModules]; 
  fRecCylZ = new Double_t *[numOfModules];  
...
//______________________________________________________________________
void IlcTARGETTrackerV1::DoTracking(Int_t evNumber,Int_t minTr,Int_t maxTr,
				 TFile *file, Bool_t realmass) {
    // Origin   A. Badala' and G.S. Pappalardo:
    // e-mail Angela.Badala@ct.infn.it, Giuseppe.S.Pappalardo@ct.infn.it
    // The method needs the event number, the minimum and maximum order
    // number of TPC tracks that 
    // are to be tracked trough the TARGET, and the file where the recpoints
    // are registered.
    ...
    gIlc->GetEvent(evNumber);  //modificato per gestire hbt
 
    IlcKalmanTrack::SetFieldMap(gIlc->Field());
   // cout<<" field = "<<gIlc->Field()->SolenoidField()<<endl;


    TFile *cf=TFile::Open("IlcTPCclusters.root");  
    IlcTPCParam *digp= (IlcTPCParam*)cf->Get("75x40_100x60_150x60");
    ...
    //TString foldname(fTARGET->GetLoader()->GetEventFolder()->GetName());
    delete rl;   
    printf("This method is not converted to the NewIO !\n");  //I.B.
    return; //I.B.
    IlcTPCtracker *tracker = new IlcTPCtracker(digp);            //I.B.
    ...
    // Load tracks
    TFile *tf=TFile::Open("IlcTPCtracksSorted.root");  
    if (!tf->IsOpen()) {
	cerr<<"Can't open IlcTPCtracksSorted.root !\n";
	return ;
    } // end if
    ...
    IlcTARGETRecPoint *recp;    // oggi
	IlcTPCtrack *ioTrackTPC=0;    
    for (kk=0; kk<nentr; kk++) {
	ioTrackTPC=new IlcTPCtrack; 
    ...
    TTree tracktree1("TreeT","Tree with TARGET tracks");
    IlcTARGETIOTrack *ioTrack=0;
	IlcTARGETPid *pid=new IlcTARGETPid(1000);  // oggi

    tracktree1.Branch("TARGETtracks","IlcTARGETIOTrack",&ioTrack,32000,0);
  
    TDatabasePDG * db = new TDatabasePDG;   
  
    Int_t j;       
    for (j=minTr; j<=maxTr; j++) {     
	track=(IlcTPCtrack*)tracks.UncheckedAt(j);
	if (!track) continue;
...
//		 cout << "Mass = " << mass << endl;
   ...
   // track->PropagateTo(xk,0.,0.); //C02
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////  	
	//IlcTARGETTrackV1 trackTARGET(*track);
	IlcTARGETTrackV1 trackTARGET(*track, fFieldFactor);
	//cout<<" fFieldFactor = "<<fFieldFactor<<"\n";      
	trackTARGET.PutMass(mass);	  //new to add mass to track
	if(fresult){ delete fresult; fresult=0;}   	 
	fresult = new IlcTARGETTrackV1(trackTARGET);	 

	IlcTARGETTrackV1 primaryTrack(trackTARGET);
	...
	//cout<<" Dv e Zv = "<<dv<<" "<<zv<<"\n";
	trackTARGET.SetDv(dv);
	trackTARGET.SetZv(zv);
	trackTARGET.SetsigmaDv(sigmaDv);
	trackTARGET.SetsigmaZv(sigmaZv); 
	(*fresult).SetDv(dv);
	(*fresult).SetZv(zv);
	...
	trackTARGET.Setd2(d2); trackTARGET.Settgl2(tgl2);
	trackTARGET.Setdtgl(dtgl); 
	(*fresult).Setd2(d2); (*fresult).Settgl2(tgl2);
	(*fresult).Setdtgl(dtgl);
	/*
	  trackTARGET.SetVertex(vertex); trackTARGET.SetErrorVertex(ervertex);
	  (*result).SetVertex(vertex);   (*result).SetErrorVertex(ervertex);   
	*/
	TList *list= new TList();

	list->AddLast(&trackTARGET);
  
	fPtref=TMath::Abs( (trackTARGET).GetPt() );
	//cout<<" fPtref = " <<fPtref<<"\n";
	...
	for(lay=5; lay>=0; lay--) {
		...
		if(lpp>=0) {
		    TParticle *p=(TParticle*) gIlc->GetMCApp()->Particle(lpp);
	...
	Long_t labTARGET=(*fresult).GetLabel();
	//cout << " TARGET track label = " << labTARGET << "\n"; 	// stampa	    
	...
	if(numOfCluster >=5)  {            // cinque - sei
	    ...
	    */
		...
		////  for q definition
	          ...
	          if(idmodule>0.){			
	            fDetTypeRec->ResetRecPoints();
	            gIlc->TreeR()->GetEvent(idmodule);
	            recp=(IlcTARGETRecPoint*)frecPoints->UncheckedAt(idpoint);
 ...
 // cout<<" q prima = "<<q[0]<<" "<<q[1]<<" "<<q[2]<<" "<<q[3]<<"\n"; getchar(); 
  ...
  // cout<<" q  dopo if = "<<q[0]<<" "<<q[1]<<" "<<q[2]<<" "<<q[3]<<"\n"; getchar(); 
   ...
   //      else    globaldedx=(q[0]+q[1]+q[2])/3.; 
	...
	cout << "Number of saved TARGET tracks " << tracktree1.GetEntries() << endl;
    tracktree1.Write(hname);
  
    TTree *fIlc=gIlc->TreeK();
    TFile *fileIlc=0;
    if (fIlc) fileIlc =fIlc->GetCurrentFile();
    fileIlc->cd();
    ////////////////////////////////////////////////////////////////////
...
//______________________________________________________________________
void IlcTARGETTrackerV1::RecursiveTracking(TList *trackTARGETlist) {
    //   This function perform the recursive tracking in TARGET detectors
    ...
    IlcTARGETRecPoint *recp;        
    for(index =0; index<trackTARGETlist->GetSize(); index++) {
	IlcTARGETTrackV1 *trackTARGET = (IlcTARGETTrackV1 *) trackTARGETlist->At(index);
	if((*trackTARGET).GetLayer()==7) fresult->SetChi2(10.223e140);
	// cout <<" Layer inizio = "<<(*trackTARGET).GetLayer()<<"\n";
	//  cout<<"fvtrack =" <<"\n";
	//  cout << (*trackTARGET)(0) << " "<<(*trackTARGET)(1)<<" "
	//       <<(*trackTARGET)(2)<<" "<<(*trackTARGET)(3)<<" "
	//       <<(*trackTARGET)(4)<<"\n";
	//  cout<< " rtrack = "<<(*trackTARGET).Getrtrack()<<"\n";
	//  cout<< " Pt = "<<(*trackTARGET).GetPt()<<"\n";
	//  getchar();    
	Double_t chi2Now, chi2Ref;
	Float_t numClustRef = fresult->GetNumClust();		 
	if((*trackTARGET).GetLayer()==1 ) {
	    chi2Now = trackTARGET->GetChi2();
	    Float_t numClustNow = trackTARGET->GetNumClust();
	    if(trackTARGET->GetNumClust()) 
		chi2Now /= (Double_t)trackTARGET->GetNumClust();
	    ...
	    if( numClustNow > numClustRef ) {*fresult = *trackTARGET;} 
	...
	if(trackTARGET->Getfnoclust()>=2)  continue;      
	Float_t numClustNow = trackTARGET->GetNumClust();
	if(numClustNow) { 
	    chi2Now = trackTARGET->GetChi2();
	...
	Int_t layerInit = (*trackTARGET).GetLayer();
	...
	Int_t  outinters=Intersection(*trackTARGET,layerfin,ladinters,detinters);
	// cout<<" outinters = "<<outinters<<"\n";
	//  cout<<" Layer ladder detector intersection ="
	//      <<layerfin<<" "<<ladinters<<" "<<detinters<<"\n";
	//  cout << " phiinters zinters = "<<(*trackTARGET)(0) 
	//       << " "<<(*trackTARGET)(1)<<"\n"; getchar();
	if(outinters==-1) continue;
	Int_t flaghit=0;
	(*trackTARGET).SetLayer(layerfin);  // oggi
	(*trackTARGET).Setfcor();           // oggi
	if(outinters==0){
	    ...
	    */
		...
		rangephi=40.*epsphi*TMath::Sqrt(sigmaphil[layerfin-1]+
						(*trackTARGET).GetSigmaphi());
		rangez = 40.*epsz*TMath::Sqrt(sigmazl[layerfin-1]+
					      (*trackTARGET).GetSigmaZ());
	    } // end if
	    ...
	    phinters=(*trackTARGET).Getphi();
	    zinters=(*trackTARGET).GetZ();
	    Float_t distz = 0.0;
  ...
  // now vecclust is with cylindrical cohordinates
          ...
          cluster(1)=fRecCylPhi[indexmod][indnew];
		    ...
		    // Float_t epsphi=5.0, epsz=5.0;                 
		    //if(fPtref<0.2) {epsphi=3.; epsz=3.;}
		    Double_t rTrack=(*trackTARGET).Getrtrack();
		    ...
		    if(cluster(1)<6. && (*trackTARGET).Getphi()>6.) 
			cluster(1)=cluster(1)+(2.*TMath::Pi());
		    if(cluster(1)>6. && (*trackTARGET).Getphi()<6.) 
			cluster(1)=cluster(1)-(2.*TMath::Pi());
		    if(TMath::Abs(cluster(1)-(*trackTARGET).Getphi())>sigmatotphi)
			continue;
		    // cout<<" supero sigmaphi \n";      
		    IlcTARGETTrackV1 *newTrack = new IlcTARGETTrackV1((*trackTARGET));
		    //(*newTrack).SetLayer((*trackTARGET).GetLayer()-1);
...
//______________________________________________________________________
Int_t IlcTARGETTrackerV1::Intersection(IlcTARGETTrackV1 &track,Int_t layer,
...
//______________________________________________________________________
void IlcTARGETTrackerV1::KalmanFilter(IlcTARGETTrackV1 *newTrack,TVector &cluster,
				   Double_t sigma[2]){ 
    //Origin  A. Badala' and G.S. Pappalardo:
    // e-mail Angela.Badala@ct.infn.it, Giuseppe.S.Pappalardo@ct.infn.it
...
//----------------------------------------------------------------------
//void IlcTARGETTrackerV1::KalmanFilterVert(IlcTARGETTrackV1 *newTrack,

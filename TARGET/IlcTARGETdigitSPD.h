#ifndef ILCTARGETDIGTARGETPD_H
#define ILCTARGETDIGTARGETPD_H
...
class IlcTARGETdigitSPD: public IlcTARGETdigit {

 public:
    IlcTARGETdigitSPD(); //default creator
    IlcTARGETdigitSPD(const Int_t *digits);//standard creator digits only
    //standard creator with digits, tracks, and hits
    IlcTARGETdigitSPD(const Int_t *digits,const Int_t *tracks,const Int_t *hits);
    virtual ~IlcTARGETdigitSPD(){/*destructor*/}
 ...
 protected:
    ...
    ClassDef(IlcTARGETdigitSPD,2)   // Simulated digit object for SPD

};
// Input and output functions for standard C++ input/output.
ostream &operator<<(ostream &os,IlcTARGETdigitSPD &source);
istream &operator>>(istream &os,IlcTARGETdigitSPD &source);

...
// IlcTARGETtrackerANN
// ---------------------------------------------------------------------------------
// Neural Network-based algorithm for track reconstruction in the ILC TARGET.
// The class contains all that is necessary in order to perform the tracking
// using the TARGET clusters in the V2 format.
...
const Double_t IlcTARGETtrackerANN::fgkPi     = 3.141592653; // pi
const Double_t IlcTARGETtrackerANN::fgkHalfPi = 1.570796327; // pi / 2
const Double_t IlcTARGETtrackerANN::fgkTwoPi  = 6.283185307; // 2 * pi

ClassImp(IlcTARGETtrackerANN)


IlcTARGETtrackerANN::IlcTARGETtrackerANN() : IlcTARGETtrackerV2(), 
fVertexX(0.0),
fVertexY(0.0),
fVertexZ(0.0),
fSectorNum(0),
...
IlcTARGETtrackerANN::IlcTARGETtrackerANN(const IlcTARGETgeom *geom, Int_t msglev) 
: IlcTARGETtrackerV2(geom), 
fVertexX(0.0),
fVertexY(0.0),
fVertexZ(0.0),
fSectorNum(0),
...
/**************************************************************************
		...
		Arguments:
			1) the TARGET geometry used in the generated event 
			2) the flag for log-messages writing
			
		The IlcTARGETgeometry is used along the class, 
		in order to translate the local IlcTARGETRecPoint coordinates
		into the Global reference frame, which is necessary for the
		Neural Network algorithm to operate.
		In case of seriilczed use, the log messages should be excluded, 
...
IlcTARGETtrackerANN::IlcTARGETtrackerANN(const IlcTARGETtrackerANN &n) : IlcTARGETtrackerV2((IlcTARGETtrackerV2&)n),  
fVertexX(n.fVertexX),
fVertexY(n.fVertexY),
fVertexZ(n.fVertexZ),
...
IlcTARGETtrackerANN& IlcTARGETtrackerANN::operator=(const IlcTARGETtrackerANN& arg){
  //Assignment operator
  this->~IlcTARGETtrackerANN();
  new(this) IlcTARGETtrackerANN(arg);
  return *this;
}
	
//__________________________________________________________________________________
void IlcTARGETtrackerANN::SetCuts
...
//__________________________________________________________________________________ 
Bool_t IlcTARGETtrackerANN::GetGlobalXYZ
(Int_t refIndex, 
...
/**************************************************************************
	...
	Taking information from the TARGET geometry stored in the class,
	gets a stored IlcTARGETcluster and calculates it coordinates
	...
	Operations:
		essentially, determines the TARGET module index from the 
		detector index of the IlcTARGETRecPoint object, and extracts
		the roto-translation from the TARGET geometry, to convert
 ...
 **************************************************************************/ 
{
	...
	Int_t ilayer = (refIndex & kClusterLayerMask) >> kClusterLayerShift;
	...
	IlcTARGETRecPoint *refCluster = (IlcTARGETRecPoint*) GetCluster(refIndex);
	if (!refCluster) {
...
//__________________________________________________________________________________ 
IlcTARGETtrackerANN::IlcTARGETnode* IlcTARGETtrackerANN::AddNode(Int_t refIndex)

/**************************************************************************

	GENERATOR OF NEURAL NODES

	Fills the array of neural 'nodes', which are the TARGET clusters
	...
	Arguments:
		1) reference index of the correlated IlcTARGETRecPoint object
		
	Operations:
		- allocates the new IlcTARGETnode objects
	...
	REturn values:
		- the pointer of the creater IlcTARGETnode object
		- in case of errors, a waring is given and a NULL is returned

 **************************************************************************/
{
	// create object and set the reference
	IlcTARGETnode *node = new IlcTARGETnode;
	if (!node) {
		Warning("AddNode", "Error occurred when allocating IlcTARGETnode");
		return 0;
	}
...
//__________________________________________________________________________________
void IlcTARGETtrackerANN::CreateArrayStructure(Int_t nsectors)

/**************************************************************************

	ARRAY STRUCTURE CREATOR
	
	Creates a structure of nested TObjArray's where the IlcTARGETnode's
	have to be stored:
...
//__________________________________________________________________________________
Int_t IlcTARGETtrackerANN::ArrangePoints(char *exportFile)
...
/**************************************************************************
	...
	Operations:
		- for each IlcTARGETRecPoint in each IlcTARGETlayer, a ne IlcTARGETnode
 ...
 **************************************************************************/
{
	...
	for (ilayer = 0; ilayer < fNLayers; ilayer++) {
		...
		for (ientry = 0; ientry < nPtsLayer; ientry++) {
			...
			index = ilayer << 28;
			index += ientry;
			// add new IlcTARGETnode object
			IlcTARGETnode *n = AddNode(index);
...
//__________________________________________________________________________________
void IlcTARGETtrackerANN::StoreOverallMatches()
...
/**************************************************************************
	...
	are always the same.
	For thi purpose, each IlcTARGETnode has a data member, named 'fMatches'
	which contains references to all other IlcTARGETnodes in the successive layer
	...
	Operations:
		- for each IlcTARGETnode, matches are found according to the criteria
	...
	for (isector = 0; isector < fSectorNum; isector++) {
		...
		for (ilayer = 0; ilayer < fNLayers - 1; ilayer++) {
			for (itheta1 = 0; itheta1 < 180; itheta1++) {
				...
				while ( (node1 = (IlcTARGETnode*)iter1.Next()) ) {
					...
					for (itheta2 = imin; itheta2 <= imax; itheta2++) {
						...
						while ( (node2 = (IlcTARGETnode*)iter2.Next()) ) {
...
//__________________________________________________________________________________
Int_t IlcTARGETtrackerANN::PassAllCuts
(IlcTARGETnode *inner, IlcTARGETnode *outer, Int_t curvStep,                     
...
//__________________________________________________________________________________
Bool_t IlcTARGETtrackerANN::PassCurvCut
(IlcTARGETnode *inner, IlcTARGETnode *outer, 
 Int_t curvStep, 
 Double_t vx, Double_t vy, Double_t vz)
{
//***********************************************************************************
...
//__________________________________________________________________________________
void IlcTARGETtrackerANN::PrintMatches(Bool_t stop)
{
...
//	the matches for each one. In this case, a keypress is needed.
	...
	IlcTARGETnode *node1 = 0, *node2 = 0;
	//IlcTARGETRecPoint *cluster1 = 0, *cluster2 = 0;
	...
	for (ilayer = 0; ilayer < 6; ilayer++) {
		for (isector = 0; isector < fSectorNum; isector++) {
			for (itheta = 0; itheta < 180; itheta++) {
				...
				while ( (node1 = (IlcTARGETnode*)points.Next()) ) {
					...
					while ( (node2 = (IlcTARGETnode*)matches.Next()) ) {
...
//__________________________________________________________________________________
void IlcTARGETtrackerANN::ResetNodes(Int_t isector)
{
/***********************************************************************************
	...
	clears the arrays 'fInnerOf' and 'fOuterOf' of each IlcTARGETnode
 ...
 ***********************************************************************************/
	...
	Int_t ilayer, itheta;
	TObjArray *sector = 0;
	IlcTARGETnode *node = 0;
	for (ilayer = 0; ilayer < fNLayers; ilayer++) {
		for (itheta = 0; itheta < 180; itheta++) {
			sector = (TObjArray*)fNodes[ilayer][itheta]->At(isector);
			TObjArrayIter iter(sector);
			for (;;) {
				node = (IlcTARGETnode*)iter.Next();
				if (!node) break;
				node->InnerOf()->Clear();
				node->OuterOf()->Clear();
				/*
...
//__________________________________________________________________________________
Int_t IlcTARGETtrackerANN::CreateNeurons
(IlcTARGETnode *node, Int_t curvStep, Double_t vx, Double_t vy, Double_t vz)
{
	// This method is used to create alle suitable neurons starting from
	// a single IlcTARGETnode. Each unit is also stored in the fInnerOf array
	...
	Bool_t found = 0;       // flag
	IlcTARGETnode *match = 0;  // cursor for a IlcTARGETnode array
	IlcTARGETneuron *unit = 0; // cursor for a IlcTARGETneuron array
	...
	// This can happen in all TARGET layers, but it happens **for sure**
	...
	while ( (match = (IlcTARGETnode*)matches.Next()) ) {
		...
		if (!node->InnerOf()->IsEmpty()) {
			...
			while ( (unit = (IlcTARGETneuron*)presentUnits.Next()) ) {
		...
		if (found) continue;
		IlcTARGETneuron *iunit = new IlcTARGETneuron(node, match, fEdge2, fEdge1);
		fNeurons->AddLast(iunit);
                node->InnerOf()->AddLast(iunit);
                match->OuterOf()->AddLast(iunit);
...
//__________________________________________________________________________________
Int_t IlcTARGETtrackerANN::CreateNetwork(Int_t sector, Int_t curvStep)
{
	...
	// calls the ResetNodes() function to free the IlcTARGETnode arrays
	if (fMsgLevel >= 2) {
	...
	for (itheta = 0; itheta < 180; itheta++) {
		...
		while ( (p[0] = (IlcTARGETnode*)lay0.Next()) ) {
			...
			while ( (p[1] = (IlcTARGETnode*)lay1.Next()) ) {
				...
				while ( (p[2] = (IlcTARGETnode*)lay2.Next()) ) {
					...
					while ( (p[3] = (IlcTARGETnode*)lay3.Next()) ) {
						...
						while ( (p[4] = (IlcTARGETnode*)lay4.Next()) ) {
							...
							while ( (p[5] = (IlcTARGETnode*)lay5.Next()) ) {
	...
	for (ilayer = 0; ilayer < 6; ilayer++) {
		for (itheta = 0; itheta < 180; itheta++) {
			...
			while ( (inner = (IlcTARGETnode*)inners.Next()) ) {
				if (inner->GetUser() >= 0) continue;
				TObjArrayIter outers(inner->Matches());
				while ( (outer = (IlcTARGETnode*)outers.Next()) ) {
 ...
 //__________________________________________________________________________________
Int_t IlcTARGETtrackerANN::LinkNeurons()
{
/***********************************************************************************
	...
	For each of them, an IlcTARGETlink is created, which stores 
	the weight value, and will allow for a faster calculation
	of the total neural input for each updating cycle.
	
	Every neuron contains an object array which stores all IlcTARGETlink
 ...
 ***********************************************************************************/
	...
	IlcTARGETneuron *neuron = 0, *test = 0;
	
	// scan in the neuron array
	for (;;) {
		neuron = (IlcTARGETneuron*)neurons.Next();
		if (!neuron) break;
		// checks for neurons startin from its head ( -> )
 		iter = (TObjArrayIter*)neuron->Inner()->OuterOf()->MakeIterator();
		for (;;) {
			test = (IlcTARGETneuron*)iter->Next();
			if (!test) break;
			weight = Weight(test, neuron);
			if (weight > 0.0) neuron->Gain()->AddLast(new IlcTARGETlink(weight, test));
			total++;
		}
		delete iter;
...
//__________________________________________________________________________________
Bool_t IlcTARGETtrackerANN::Update()
{
/***********************************************************************************
 ...
 ***********************************************************************************/
	...
	IlcTARGETneuron *unit;
	for (;;) {
		unit = (IlcTARGETneuron*)iter.Next();
		if (!unit) break;
...
//__________________________________________________________________________________
void IlcTARGETtrackerANN::FollowChains(Int_t sector)
{
/***********************************************************************************
 ...
 ***********************************************************************************/
	...
	for (ilayer = 0; ilayer < fNLayers; ilayer++) {
		for (itheta = 0; itheta < 180; itheta++) {
			...
			while ( (p = (IlcTARGETnode*)nodes.Next()) ) {
				...
				while ( (n = (IlcTARGETneuron*)innerof.Next()) ) {
				...
				while ( (n = (IlcTARGETneuron*)outerof.Next()) ) {
					// if the examined neuron has not the largest activation
					// it is skipped and removed from array of all neurons
...
//__________________________________________________________________________________
Int_t IlcTARGETtrackerANN::SaveTracks(Int_t sector)
{
/********************************************************************************
...
***********************************************************************************/
	...
	TObjArray *lstSector = 0;
	IlcTARGETnode *p = 0, *q = 0, **node = new IlcTARGETnode*[fNLayers];
	for (l = 0; l < fNLayers; l++) node[l] = 0;
	
	/*
	array = new IlcTARGETnode*[fNLayers + 1];
	for (l = 0; l <= fNLayers; l++) array[l] = 0;
	array[0] = new IlcTARGETnode();
	array[0]->X() = fVertexX;
	array[0]->Y() = fVertexY;
	array[0]->Z() = fVertexZ;
	...
	for (ilayer = 0; ilayer < 1; ilayer++) {
		for (itheta = 0; itheta < 180; itheta++) {
			...
			while ( (p = (IlcTARGETnode*)nodes.Next()) ) {
				...
				IlcTARGETRecPoint *cluster = (IlcTARGETRecPoint*)GetCluster(node[0]->ClusterRef());
				Int_t mod = cluster->GetDetectorIndex();
				Int_t lay, lad, det;
				fGeom->GetModuleId(mod, lay, lad, det);
				Float_t y0 = cluster->GetY();
				Float_t z0 = cluster->GetZ();
				IlcTARGETtrackSA* trac = new IlcTARGETtrackSA(fGeom,lay, lad, det, 
				                                        y0, z0, 
																	 param[4], param[7], param[3], 1);
				for (l = 0; l < fNLayers; l++) {
					cluster = (IlcTARGETRecPoint*)GetCluster(node[l]->ClusterRef());
					if (cluster) trac->AddClusterV2(l, (node[l]->ClusterRef() & 0x03ffffff)>>0);
				}
            IlcTARGETtrackV2* ot = new IlcTARGETtrackV2(*trac);
				ot->ResetCovariance(10.);
				ot->ResetClusters();
				if (RefitAt(49.,ot,trac)) { //fit from layer 1 to layer 6
					IlcTARGETtrackV2 *otrack2 = new IlcTARGETtrackV2(*ot);
...
//__________________________________________________________________________________
void IlcTARGETtrackerANN::ExportTracks(const char *filename) const
{
// Exports found tracks into a TTree of IlcTARGETtrackV2 objects
	TFile *file = new TFile(filename, "RECREATE");
	TTree *tree = new TTree("TreeT-ANN", "Tracks found in TARGET stand-alone with Neural Tracking");
	IlcTARGETtrackV2 *track = 0;
	tree->Branch("Tracks", &track, "IlcTARGETtrackV2");
	TObjArrayIter tracks(fFoundTracks);
	while ( (track = (IlcTARGETtrackV2*)tracks.Next()) ) {
		tree->Fill();
	}
	file->cd();
	tree->Write();
...
//__________________________________________________________________________________
void IlcTARGETtrackerANN::CleanNetwork()
{
	// Removes deactivated units from the network

	IlcTARGETneuron *unit = 0;
	TObjArrayIter neurons(fNeurons);
	while ( (unit = (IlcTARGETneuron*)neurons.Next()) ) {
	...
	while ( (unit = (IlcTARGETneuron*)neurons.Next()) ) {
		...
		if (nIn > 1) {
			...
			while ( (enemy = (IlcTARGETneuron*)competing.Next()) ) {
...
//__________________________________________________________________________________
Int_t IlcTARGETtrackerANN::StoreTracks()
{
	...
	for (;;) {
		...
		while (node) {
			...
			fwd = 0;
			for (;;) {
				cursor = (IlcTARGETneuron*)fwdIter->Next();
				if (!cursor) break;
		...
		if (check >= 6) {
			...
			// initialization of Kalman Filter Tracking
			IlcTARGETRecPoint *cluster = (IlcTARGETRecPoint*)GetCluster(annTrack[0]->ClusterRef());
			Int_t mod = cluster->GetDetectorIndex();
			Int_t lay, lad, det;
			fGeom->GetModuleId(mod, lay, lad, det);
			Float_t y0 = cluster->GetY();
			Float_t z0 = cluster->GetZ();
			IlcTARGETtrackSA* trac = new IlcTARGETtrackSA(fGeom,lay, lad, det, y0, z0, 
																 annTrack.Phi(), annTrack.TanLambda(), 
																 annTrack.Curv(), 1);
			for (Int_t l = 0; l < fNLayers; l++) {
				if (!annTrack[l]) continue;
				cluster = (IlcTARGETRecPoint*)GetCluster(annTrack[l]->ClusterRef());
				if (cluster) trac->AddClusterV2(l, (annTrack[l]->ClusterRef() & 0x03ffffff)>>0);
			}
			IlcTARGETtrackV2* ot = new IlcTARGETtrackV2(*trac);
			ot->ResetCovariance(10.);
			ot->ResetClusters();
			...
			// END FIT
			for (i = 0; i < fNLayers; i++) {
				//node = (IlcTARGETnode*)removedPoints->At(i);
				//node->Use();
				annTrack[i]->Use();
			}
...
Double_t IlcTARGETtrackerANN::Weight(IlcTARGETneuron *nAB, IlcTARGETneuron *nBC)
{
/***********************************************************************************
 ...
 ***********************************************************************************/
	...
	IlcTARGETnode *pA = nAB->Inner();
	IlcTARGETnode *pB = nAB->Outer();
	IlcTARGETnode *pC = nBC->Outer();
...
/******************************************
 ******************************************
 *** IlcTARGETtrackerANN::IlcTARGETnode class ***
 ******************************************
 ******************************************/
 
//__________________________________________________________________________________ 
IlcTARGETtrackerANN::IlcTARGETnode::IlcTARGETnode(): 
fX(0.0),
fY(0.0),
fZ(0.0),
fEX2(0.0),
...
fPrev(NULL)
{
	// Constructor for the embedded 'IlcTARGETnode' class.
	// It initializes all pointer-like objects.
	
}

IlcTARGETtrackerANN::IlcTARGETnode::IlcTARGETnode(const IlcTARGETnode &n) : TObject((TObject&)n),
fX(n.fX),
fY(n.fY),
fZ(n.fZ),
fEX2(n.fEX2),
...
IlcTARGETtrackerANN::IlcTARGETnode::~IlcTARGETnode()
{
	// Destructor for the embedded 'IlcTARGETnode' class.
...
//__________________________________________________________________________________ 
Double_t IlcTARGETtrackerANN::IlcTARGETnode::GetPhi() const
{
...
//__________________________________________________________________________________ 
Double_t IlcTARGETtrackerANN::IlcTARGETnode::GetError(Option_t *option)
{
...
/********************************************
 ********************************************
 *** IlcTARGETtrackerANN::IlcTARGETneuron class ***
 ********************************************
 ********************************************/
 
//__________________________________________________________________________________
IlcTARGETtrackerANN::IlcTARGETneuron::IlcTARGETneuron
(IlcTARGETnode *inner, IlcTARGETnode *outer, Double_t minAct, Double_t maxAct): 
fUsed(0), 
fActivation(0),
fInner(inner), 
fOuter(outer),
...
IlcTARGETtrackerANN::IlcTARGETneuron::IlcTARGETneuron(const IlcTARGETneuron &n) : TObject((TObject&)n),
fUsed(n.fUsed),
...
//__________________________________________________________________________________
Double_t IlcTARGETtrackerANN::IlcTARGETneuron::Activate(Double_t temperature)
{
	...
	IlcTARGETneuron *linked = 0;    // cursor for scanning the neuron arrays (for link check)
	IlcTARGETlink *link;            // cursor for scanning the synapses arrays (for link check)
	...
	for(;;) {
		link = (IlcTARGETlink*)iterator->Next();
		if (!link) break;
	...
	for (;;) {
		linked = (IlcTARGETneuron*)iterator->Next();
		if (!linked) break;
...
/******************************************
 ******************************************
 *** IlcTARGETtrackerANN::IlcTARGETlink class ***
 ******************************************
 ******************************************/
 ...
 /**********************************************
  **********************************************
  *** IlcTARGETtrackerANN::IlcTARGETtrackANN class ***
  **********************************************
  **********************************************/
 
//__________________________________________________________________________________
IlcTARGETtrackerANN::IlcTARGETtrackANN::IlcTARGETtrackANN(Int_t dim) : 
fNPoints(dim),
fXCenter(0.0),
fYCenter(0.0),
fRadius(0.0),
...
fNode(0)
{
	// Default constructor for the IlcTARGETtrackANN class
	...
	if (! dim) {
		...
		Int_t i = 0;
		fNode = new IlcTARGETnode*[dim];
		for (i = 0; i < dim; i++) fNode[i] = 0;
	}
}

IlcTARGETtrackerANN::IlcTARGETtrackANN::IlcTARGETtrackANN(const IlcTARGETtrackANN &n) : TObject((TObject&)n),
fNPoints(n.fNPoints),
fXCenter(n.fXCenter),
fYCenter(n.fYCenter),
fRadius(n.fRadius),
...
//__________________________________________________________________________________
Int_t IlcTARGETtrackerANN::IlcTARGETtrackANN::CheckOccupation() const
{
...
//__________________________________________________________________________________
Bool_t IlcTARGETtrackerANN::IlcTARGETtrackANN::RiemannFit()
{ 
	...
	if (count != fNPoints) {
		Error ("IlcTARGETtrackANN::RiemannFit", "CheckOccupations returns %d, fNPoints = %d ==> MISMATCH", count, fNPoints);
		return kFALSE;
	}
	...
	IlcTARGETnode *p = fNode[0];
	phi1 = p->GetPhi();
	for (i = 1; i < dim; i++) {
		p = (IlcTARGETnode*)fNode[i];
		if (!p) break;
		phi2 = p->GetPhi();
		temp1 = phi1;
		temp2 = phi2;

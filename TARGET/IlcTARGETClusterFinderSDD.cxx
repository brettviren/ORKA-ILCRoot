 ...
 **************************************************************************/
/*
  $Id: IlcTARGETClusterFinderSDD.cxx,v 1.1.1.1 2008/03/11 14:52:50 vitomeg Exp $
 */
/////////////////////////////////////////////////////////////////////////// 
//  Cluster finder                                                       //
//  for Silicon                                                          //
...
ClassImp(IlcTARGETClusterFinderSDD)

//______________________________________________________________________
IlcTARGETClusterFinderSDD::IlcTARGETClusterFinderSDD():
...
//______________________________________________________________________
IlcTARGETClusterFinderSDD::IlcTARGETClusterFinderSDD(IlcTARGETDetTypeRec* dettyp,
                                               TClonesArray *digits,
                                               TClonesArray *recp):
...
fMaxNCells(0){
    ...
    SetDigits(digits);
    SetClusters(recp);
    SetCutAmplitude(fDetTypeRec->GetTARGETgeom()->GetStartSDD());
    SetDAnode();
    SetDTime();
    SetMinPeak((Int_t)((IlcTARGETCalibrationSDD*)GetResp(fDetTypeRec->GetTARGETgeom()->GetStartSDD()))->GetNoiseAfterElectronics(0)*5);
    SetMinNCells();
    SetMaxNCells();
    SetTimeCorr();
    SetMinCharge();
 Int_t nlayerstrip=4; //modaniele
    ...
    SetMap(lay,new IlcTARGETMapA1(lay,GetSeg(),Digits(),fCutAmplitude));
}
//______________________________________________________________________
void IlcTARGETClusterFinderSDD::SetCutAmplitude(Int_t mod,Double_t nsigma){
    ...
    for(Int_t ian=0;ian<nanodes;ian++){
      ...
      noiseAfterEl = ((IlcTARGETCalibrationSDD*)GetResp(mod))->GetNoiseAfterElectronics(ian);
      fCutAmplitude[ian] = (Int_t)((baseline + nsigma*noiseAfterEl));
    }
}
//______________________________________________________________________
void IlcTARGETClusterFinderSDD::Find1DClusters(){
    // find 1D clusters
  
    // retrieve the parameters 
    Int_t fNofMaps       = GetSeg()->Npz(1);
    Int_t fMaxNofSamples = GetSeg()->Npx(1);
    Int_t fNofAnodes     = fNofMaps/2;
    Int_t dummy          = 0;
    Double_t fTimeStep    = GetSeg()->Dpx(1,dummy);
    Double_t fSddLength   = GetSeg()->Dx(1);
    Double_t fDriftSpeed  = GetResp(fModule)->GetDriftSpeed();  
    Double_t anodePitch   = GetSeg()->Dpz(1,dummy);

    // map the signal
    Map(1)->ClearMap();
    Map(1)->SetThresholdArr(fCutAmplitude);
    Map(1)->FillMap2();
    ...
    for(j=0;j<2;j++) {
        for(k=0;k<fNofAnodes;k++) {
            ...
            for(l=0; l<fMaxNofSamples; l++) {
                fadc2=(Double_t)Map(1)->GetSignal(idx,l);
                if(l>0) fadc1=(Double_t)Map(1)->GetSignal(idx,l-1);
                if(l>0) dfadc[k][l-1] = fadc2-fadc1;
            } // samples
        } // anodes

        for(k=0;k<fNofAnodes;k++) {
            IlcDebug(5,Form("Anode: %d, Wing: %d",k+1,j+1));
            idx = j*fNofAnodes+k;
            Int_t imax  = 0;
            Int_t imaxd = 0;
            Int_t it    = 0;
            while(it <= fMaxNofSamples-3) {
                ...
                for(m=0;m<20;m++) {
                    ...
                    fadc=(float)Map(1)->GetSignal(idx,id);
                ...
                it = imaxd;
                if(Map(1)->TestHit(idx,imax) == kEmpty) {it++; continue;}
                // cluster charge
                Int_t tstart = it-2;
                ...
                if(ilcl) {
                    ...
                    for(ij=0; ij<20; ij++) {
                        ...
                        fadc=(float)Map(1)->GetSignal(idx,tstart+ij);
		    ...
		    for(its=tstart; its<=tstop; its++) {
                        fadc=(float)Map(1)->GetSignal(idx,its);
                        ...
                        if(fadc > clusterPeakAmplitude) {
                            ...
                            if(its>shift && its<(fMaxNofSamples-shift))
                                peakpos  = Map(1)->GetHitIndex(idx,its+shift);
                            else peakpos = Map(1)->GetHitIndex(idx,its);
                            if(peakpos<0) peakpos =Map(1)->GetHitIndex(idx,its);
                        } // end if
                    ...
                    IlcTARGETRawClusterSDD clust(j+1,//i
...
//______________________________________________________________________
void IlcTARGETClusterFinderSDD::Find1DClustersE(){
    // find 1D clusters
    // retrieve the parameters 
    Int_t fNofMaps = GetSeg()->Npz(1);
    Int_t fMaxNofSamples = GetSeg()->Npx(1);
    ...
    for( Int_t j=0; j<2; j++ ){
	    ...
	    Double_t baseline=GetResp(fModule)->GetBaseline(idx);
            ...
            for( Int_t l=0; l<fMaxNofSamples; l++ ){
                Double_t fadc = (Double_t)Map(1)->GetSignal( idx, l );
                if( fadc > 0.0 ){
                    if( on == kFALSE && l<fMaxNofSamples-4){
                        // star RawCluster (reset var.)
                        Double_t fadc1 = (Double_t)Map(1)->GetSignal( idx, l+1 );
                    ...
                    if( fadc > fmax ){ 
                        ...
                        if( l > shift && l < (fMaxNofSamples-shift) )  
                            peakpos = Map(1)->GetHitIndex( idx, l+shift );
                        else
                            peakpos = Map(1)->GetHitIndex( idx, l );
                        if( peakpos < 0) peakpos = Map(1)->GetHitIndex(idx,l);
                    } // end if fadc
                }else{ // end fadc>0
                    if( on == kTRUE ){        
                        if( nTsteps > 2 ){
                            ...
                            IlcTARGETRawClusterSDD clust(j+1,anode,time,charge,
                                                      fmax, peakpos,0.,0.,
                                                      driftPath,anodePath,
                                                      nTsteps,start,stop,
                                                      start, stop, 1, k, k );
			    fDetTypeRec->AddCluster( 1, &clust );
                            if(IlcDebugLevel()>=5) clust.PrintInfo();
                            nClu++;
                        } // end if nTsteps
                        on = kFALSE;
                    } // end if on==kTRUE
    ...
    IlcDebug(3,Form("# Rawclusters %d",nClu));         
    return; 
}
//_______________________________________________________________________
Int_t IlcTARGETClusterFinderSDD::SearchPeak(Double_t *spect,Int_t xdim,Int_t zdim,
...
//______________________________________________________________________
void IlcTARGETClusterFinderSDD::PeakFunc( Int_t xdim, Int_t zdim, Double_t *par,
...
Double_t IlcTARGETClusterFinderSDD::ChiSqr( Int_t xdim, Int_t zdim, Double_t *spe,
                                        Double_t *speFit ) const{
    // EVALUATES UNNORMILCZED CHI-SQUARED
    Double_t chi2 = 0.;
    for( Int_t z=0; z<zdim; z++ ){
...
//_______________________________________________________________________
void IlcTARGETClusterFinderSDD::Minim( Int_t xdim, Int_t zdim, Double_t *param,
...
Int_t IlcTARGETClusterFinderSDD::NoLinearFit( Int_t xdim, Int_t zdim, 
...
//______________________________________________________________________
void IlcTARGETClusterFinderSDD::ResolveClusters(){
    ...
    for( Int_t j=0; j<nofClusters; j++ ){ 
        ...
        IlcTARGETRawClusterSDD *clusterJ=(IlcTARGETRawClusterSDD*) Cluster(j);
        ...
        for( Int_t ianode=astart; ianode<=astop; ianode++ ){
            for( Int_t itime=tstart; itime<=tstop; itime++ ){
                Double_t fadc = Map(1)->GetSignal( ianode, itime );
        ...
        if( npeak >= 1 ){
            ...
            for( i=0; i<npeak; i++ ){
                IlcTARGETRawClusterSDD clusterI( *clusterJ );
            ...
            //    clusterI.SetPeakPos( peakpos );
                ...
                Int_t peakpos = Map(1)->GetHitIndex( newAnode, timeBin );
                if( peakpos < 0 ) { 
                    for( Int_t ii=0; ii<3; ii++ ) {
                        peakpos = Map(1)->GetHitIndex( newAnode, timeBin+ii );
                        if( peakpos > 0 ) break;
                        peakpos = Map(1)->GetHitIndex( newAnode, timeBin-ii );
                        if( peakpos > 0 ) break;
                    }
                ...
                if( peakpos < 0 ) { 
                    ...
                    //if(IlcDebugLevel()>=3) clusterI.PrintInfo(); 
                   continue;
                }
                clusterI.SetPeakPos( peakpos );    
...
void  IlcTARGETClusterFinderSDD::GroupClusters(){
    // group clusters
    Int_t dummy=0;
    Double_t fTimeStep = GetSeg()->Dpx(1,dummy);
    // get number of clusters for this module
    Int_t nofClusters = NClusters();
    nofClusters -= fNclusters;
    IlcTARGETRawClusterSDD *clusterI;
    IlcTARGETRawClusterSDD *clusterJ;
    ...
    for(i=0; i<nofClusters; i++) { 
        ...
        for(j=i+1; j<nofClusters; j++) { 
            ...
            clusterI = (IlcTARGETRawClusterSDD*) Cluster(i);
            clusterJ = (IlcTARGETRawClusterSDD*) Cluster(j);
            // 1.3 good
...
//________________________________________________________________________
void IlcTARGETClusterFinderSDD::SelectClusters(){
    // get number of clusters for this module
    Int_t nofClusters = NClusters();
    ...
    Int_t i;
    for(i=0; i<nofClusters; i++) { 
        IlcTARGETRawClusterSDD *clusterI =(IlcTARGETRawClusterSDD*) Cluster(i);
        Int_t rmflg = 0;
        Double_t wy = 0.;
...
//______________________________________________________________________
void IlcTARGETClusterFinderSDD::GetRecPoints(){
    // get rec points
    ...
    const Double_t kRMSx = 38.0*kconv; // microns->cm TARGET TDR Table 1.3
    const Double_t kRMSz = 28.0*kconv; // microns->cm TARGET TDR Table 1.3
    Int_t i;
    Int_t ix, iz, idx=-1;
    IlcTARGETdigitSDD *dig=0;
    ...
    for(i=0; i<nofClusters; i++) { 
        ...
        if(idx&&idx<= ndigits) dig =(IlcTARGETdigitSDD*)GetDigit(idx);
        if(!dig) {
            // try cog
            GetSeg()->GetPadIxz(1,clusterI->X(),clusterI->Z(),ix,iz);
            dig = (IlcTARGETdigitSDD*)Map(1)->GetHit(iz-1,ix-1);
            // if null try neighbours
            if (!dig) dig = (IlcTARGETdigitSDD*)Map(1)->GetHit(iz-1,ix); 
            if (!dig) dig = (IlcTARGETdigitSDD*)Map(1)->GetHit(iz-1,ix+1); 
...
//______________________________________________________________________
void IlcTARGETClusterFinderSDD::FindRawClusters(Int_t mod){
    // find raw clusters
    
    SetModule(mod);
    SetCutAmplitude(mod);
    Int_t nanodes=GetSeg()->Npz(1);
    Int_t noise=0;
    for(Int_t i=0;i<nanodes;i++){
      noise+=(Int_t)(((IlcTARGETCalibrationSDD*)GetResp(mod))->GetNoiseAfterElectronics(i));
    }    
...
//_______________________________________________________________________
void IlcTARGETClusterFinderSDD::PrintStatus() const{

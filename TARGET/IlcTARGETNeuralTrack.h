#ifndef ILCTARGETNEURALTRACK_H
#define ILCTARGETNEURALTRACK_H

#include <TMatrixD.h>

class TObjArray;
class IlcTARGETNeuralPoint;
//class IlcTARGETVertex;
class IlcTARGETIOTrack;

class IlcTARGETNeuralTrack : public TObject {

public:
	         IlcTARGETNeuralTrack();
				IlcTARGETNeuralTrack(IlcTARGETNeuralTrack &track);
	virtual ~IlcTARGETNeuralTrack();
	...
	void     Insert(IlcTARGETNeuralPoint *point);
	...
	Bool_t   Filter(IlcTARGETNeuralPoint *test);
	Bool_t   KalmanFit();
	Bool_t   RiemannFit();
	...
	*/
	IlcTARGETIOTrack* ExportIOtrack(Int_t min);
...
private:
	...
	IlcTARGETNeuralPoint  fVertex;   // vertex position data
	IlcTARGETNeuralPoint *fPoint[6]; // track points

	ClassDef(IlcTARGETNeuralTrack, 1)
};

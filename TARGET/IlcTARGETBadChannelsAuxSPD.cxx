...
/*
$Log: IlcTARGETBadChannelsAuxSPD.cxx,v $
...
Revision 1.2  2006/04/12 08:32:31  hristov
...
///////////////////////////////////////////////////////////////////////////
// IlcTARGETBadChannelsAuxSPD implementation by P. Nilsson 2005
...
// Examples - Converters
//
// root [0] IlcTARGETdigitSPD *d = new IlcTARGETdigitSPD();
// root [1] d->SetCoord1(1);
// root [2] d->SetCoord2(2);
// root [3] d->SetSignal(1);
// root [4] IlcTARGETBadChannelsAuxSPD *aux = new IlcTARGETBadChannelsAuxSPD();
// root [5] IlcTARGETChannelSPD *c = aux->CreateChannelFromDigit(d);
// root [6] cout << c->GetColumn() << endl;
// 1
// root [7] cout << c->GetRow() << endl;
// 2
// root [8] IlcTARGETdigitSPD *d2 = aux->CreateDigitFromChannel(c);
...
ClassImp(IlcTARGETBadChannelsAuxSPD)

//__________________________________________________________________________
IlcTARGETBadChannelsAuxSPD::IlcTARGETBadChannelsAuxSPD(void)
{
  // Default constructor
}

//__________________________________________________________________________
Bool_t IlcTARGETBadChannelsAuxSPD::Diff(TObjArray *&inputArray1, TObjArray *&inputArray2,
				     TObjArray *&outputArray1, TObjArray *&outputArray2) const
{
  // Make a diff between the input TObjArrays
  // 
  // Input: Two input TObjArrays of IlcTARGETChannelSPD objects to be tested, two output TObjArrays corresponding
  //        to 
  // Output: Two output TObjArrays where outputArray1 contains the IlcTARGETChannelSPD objects from inputArray1
  ...
  IlcTARGETChannelSPD *ch1 = 0;
  IlcTARGETChannelSPD *ch2 = 0;
  Bool_t found = kFALSE;
  ...
  for (i = 0; i < kInputArray1Size; i++)
    {
      ...
      ch1 = (IlcTARGETChannelSPD *) inputArray1->At(i);

      // Is ch1 also in array 2?
      for (j = lastFoundAtJ + 1; j < kInputArray2Size; j++)
        {
          ch2 = (IlcTARGETChannelSPD *) inputArray2->At(j);
          if (*ch1 == *ch2)
            {
              // Abort, go to next i
              found = kTRUE;
  ...
  for (i = 0; i < kInputArray2Size; i++)
    {
      ...
      ch2 = (IlcTARGETChannelSPD *) inputArray2->At(i);

      // Is ch2 also in array 1?
      for (j = lastFoundAtJ + 1; j < kInputArray1Size; j++)
        {
          ch1 = (IlcTARGETChannelSPD *) inputArray1->At(j);
          if (*ch1 == *ch2)
            {
              // Abort, go to next i
              found = kTRUE;
...
//__________________________________________________________________________
Bool_t IlcTARGETBadChannelsAuxSPD::Find(IlcTARGETChannelSPD *&channel, TObjArray *&array) const
{
  // Find the channel in the array
  //
  // Input: IlcTARGETChannelSPD channel object, TObjArray of IlcTARGETChannelSPD channel objects
  // Ouput: (none)
  ...
  while (channelNr < kN)
    {
      if (*channel == *(IlcTARGETChannelSPD *)array->At(channelNr))
	{
	  status = kTRUE;
	  break;
	}
...
//__________________________________________________________________________
Bool_t IlcTARGETBadChannelsAuxSPD::Find(IlcTARGETdigitSPD *&digit, TObjArray *&array) const
{
  // Find the digit in the array
  //
  // WARNING: Using IlcTARGETdigitSPD digits in this way is roughly 10% slower than to use IlcTARGETChannelSPD channels
  //
  // Input: IlcTARGETdigitSPD digit object, TObjArray of IlcTARGETChannelSPD channel objects
  // Ouput: (none)
  ...
  while (channelNr < kN)
    {
      channel = (IlcTARGETChannelSPD *)array->At(channelNr);
...
//__________________________________________________________________________
IlcTARGETdigitSPD* IlcTARGETBadChannelsAuxSPD::CreateDigitFromChannel(const IlcTARGETChannelSPD *&channel) const
{
  // Create a digit from a channel
  //
  // Input: IlcTARGETChannelSPD object
  // Ouput: (none)
  // Return: IlcTARGETdigitSPD object

  IlcTARGETdigitSPD *digit = new IlcTARGETdigitSPD();
...
//__________________________________________________________________________
IlcTARGETChannelSPD* IlcTARGETBadChannelsAuxSPD::CreateChannelFromDigit(const IlcTARGETdigitSPD *&digit) const
{
  // Create a channel from a digit
  //
  // Input: IlcTARGETdigitSPD object
  // Ouput: (none)
  // Return: IlcTARGETChannelSPD object

  IlcTARGETChannelSPD *channel = new IlcTARGETChannelSPD();
...
//__________________________________________________________________________
Int_t IlcTARGETBadChannelsAuxSPD::GetNumberOfBadChannels(Int_t* &badChannelsArray, Int_t* &indexArray, Int_t size) const
{
...
//__________________________________________________________________________
Bool_t IlcTARGETBadChannelsAuxSPD::CreateHTMLReport(char *name, Int_t* &badChannelsArray, Int_t* &indexArray,

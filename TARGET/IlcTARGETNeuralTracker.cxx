...
// *                                                                        *
// * AN TARGET STAND-ALONE "NEURAL" TRACK FINDER                               *
// * ----------------------------------------                               *
// * This class implements the Denby-Peterson algorithm for track finding   *
// * in the TARGET stand-alone, by means of a neural network simulation.       *
...
// * of the class "IlcTARGETNeuralPoint", into a single branch called       *
// * "Points".                                                              *
// **************************************************************************/
...
//using namespace std;
ClassImp(IlcTARGETNeuralTracker)

//--------------------------------------------------------------------------------------------

IlcTARGETNeuralTracker::IlcTARGETNeuralTracker()
{
	// CONSTRUCTOR
	//
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::Display(TCanvas*& canv) const
{
	...
	for (;;) {
		IlcTARGETneuron *unit = (IlcTARGETneuron*)iter.Next();
		if (!unit) break;
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::SetThetaCuts2D(Double_t *min, Double_t *max)
{
	// Cut setter
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::SetThetaCuts3D(Double_t *min, Double_t *max)
{
	// Cut setter
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::SetHelixMatchCuts(Double_t *min, Double_t *max)
{
	// Cut setter
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::SetCurvatureCuts(Int_t ncuts, Double_t *curv)
{
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::CreateArrayStructure(Int_t nsectors)
{
	// ARRAY CREATOR
	//
...
//--------------------------------------------------------------------------------------------
//
Int_t IlcTARGETNeuralTracker::ArrangePoints(TTree* ptstree)
{
	// POINTS STORAGE INTO ARRAY
	//
	// Reads points from the tree and creates IlcTARGETNode objects for each one,
	...
	TObjArray *sector = 0;
	IlcTARGETNode *created = 0;
	IlcTARGETNeuralPoint *cursor = 0;
	...
	for (ientry = 0; ientry < nentries; ientry++) {
		...
		// creates the object
		created = new IlcTARGETNode(cursor, kTRUE);
		created->SetUser(-1);
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::PrintPoints()
{
	// creates the TCollection structure
	TObjArray *sector = 0;
	Int_t ilayer, isector, itheta;
...
//--------------------------------------------------------------------------------------------
//
Bool_t IlcTARGETNeuralTracker::PassCurvCut
(IlcTARGETNode *p1, IlcTARGETNode *p2, Int_t curvindex, Double_t vx, Double_t vy, Double_t vz)
{
...
//--------------------------------------------------------------------------------------------
//
Int_t IlcTARGETNeuralTracker::PassAllCuts
(IlcTARGETNode *p1, IlcTARGETNode *p2, Int_t curvindex, Double_t vx, Double_t vy, Double_t vz)
{
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::StoreAbsoluteMatches()
{
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::PrintMatches(Bool_t stop)
{
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::ResetNodes(Int_t isector)
{
...
//--------------------------------------------------------------------------------------------
//
void IlcTARGETNeuralTracker::NeuralTracking(const char* filesave, TCanvas*& display)
{
	...
	for (sector = 0; sector < fSectorNum; sector++) {
		...
		for(curv = 0; curv < fCurvNum; curv++) {
			...
			for (i = 0;; i++) {
				...
				TObjArrayIter iter(fNeurons);
				for (;;) {
					IlcTARGETneuron *n = (IlcTARGETneuron*)iter.Next();
					if (!n) break;
				}
				if (isStable) break;
			}
...
//--------------------------------------------------------------------------------------------
//
Int_t IlcTARGETNeuralTracker::CreateNeurons(Int_t sectoridx, Int_t curvidx)
{
...
//
//
Int_t IlcTARGETNeuralTracker::LinkNeurons() const
{
...
// which share a point.
	...
	for (;;) {
		neuron = (IlcTARGETneuron*)iter.Next();
		if (!neuron) break;
		// gain contributors
		testiter = (TObjArrayIter*)neuron->Inner()->OuterOf()->MakeIterator();
		for (;;) {
			test = (IlcTARGETneuron*)testiter->Next();
			if (!test) break;
...
//
//
Bool_t IlcTARGETNeuralTracker::Update()
{
...
//
//
void IlcTARGETNeuralTracker::CleanNetwork()
{
...
//
//
Bool_t IlcTARGETNeuralTracker::CheckOccupation() const
{
...
//
//
Int_t IlcTARGETNeuralTracker::Save(Int_t sectorid)
{
...
/*
Int_t IlcTARGETNeuralTracker::Save(Int_t sectoridx)
// Creates chains of active neurons, in order to
// find the tracks obtained as the neural network output.
{
	...
	for (itheta = 0; itheta < 180; itheta++) {
		...
		for (i = 0; i < 2; i++) {
			...
			while ( (node = (IlcTARGETNode*)iter.Next()) ) {
				...
				IlcTARGETneuralChain *chain = new IlcTARGETneuralChain;
				for (temp = node; temp; temp = temp->fNext) {
					chain->Insert((IlcTARGETNeuralPoint*)temp);
				}
				ok = kFALSE;
...
//
//
Double_t IlcTARGETNeuralTracker::Activate(IlcTARGETneuron* &unit)
{
...
// returns the variation w.r.t the previous one
	...
	IlcTARGETneuron *linked = 0;
	for (;;) {
		linked = (IlcTARGETneuron*)testiter->Next();
		if (!linked) break;
...
//
//
void IlcTARGETNeuralTracker::IlcTARGETneuron::Add2Gain(IlcTARGETneuron *n, Double_t multconst, Double_t exponent)
{
// Adds a neuron to the collection of sequenced ones of another neuron			

	IlcTARGETlink *link = new IlcTARGETlink;
	link->Linked() = n;
...
//
//
Double_t IlcTARGETNeuralTracker::IlcTARGETneuron::Weight(IlcTARGETneuron *n)
{
...
//
//
IlcTARGETNeuralTracker::IlcTARGETNeuralTracker(const IlcTARGETNeuralTracker &t)
: TObject((TObject&)t)
{ 
	// NO copy constructor
	Fatal("IlcTARGETNeuralTracker", "No copy constructor allowed!");
	exit(0);
}
IlcTARGETNeuralTracker& IlcTARGETNeuralTracker::operator=(const IlcTARGETNeuralTracker&)
{ 
	// NO assignment operator
	Fatal("IlcTARGETNeuralTracker", "No assignment allowed!");
	return *this;
}
//
IlcTARGETNeuralTracker::IlcTARGETNode::IlcTARGETNode(const IlcTARGETNode &t)
: IlcTARGETNeuralPoint((IlcTARGETNeuralPoint&)t)
{ 
	// NO copy constructor
	Fatal("IlcTARGETNode", "No copy constructor allowed!");
	exit(0);
}
IlcTARGETNeuralTracker::IlcTARGETNode& IlcTARGETNeuralTracker::IlcTARGETNode::operator=(const IlcTARGETNode&)
{ 
	// NO assignment operator
	Fatal("IlcTARGETNode", "No assignment allowed!");
	return *this;
}
//
IlcTARGETNeuralTracker::IlcTARGETneuron::IlcTARGETneuron(const IlcTARGETneuron &t)
: TObject((TObject&)t)
{ 
	// NO copy constructor
	Fatal("IlcTARGETneuron", "No copy constructor allowed!");
	exit(0);
}
IlcTARGETNeuralTracker::IlcTARGETneuron& IlcTARGETNeuralTracker::IlcTARGETneuron::operator=(const IlcTARGETneuron&)
{ 
	// NO assignment operator
	Fatal("IlcTARGETneuron", "No assignment allowed!");
	return *this;
}
//
IlcTARGETNeuralTracker::IlcTARGETlink::IlcTARGETlink(const IlcTARGETlink &t)
: TObject((TObject&)t)
{ 
	// NO copy constructor
	Fatal("IlcTARGETlink", "No copy constructor allowed!");
	exit(0);
}
IlcTARGETNeuralTracker::IlcTARGETlink& IlcTARGETNeuralTracker::IlcTARGETlink::operator=(const IlcTARGETlink&)
{ 
	// NO assignment operator
	Fatal("IlcTARGETlink", "No assignment allowed!");
	return *this;
}

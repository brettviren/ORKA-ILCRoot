...
/* $Id: IlcTARGETsimulationSDD.cxx,v 1.2 2008/03/17 18:20:42 vitomeg Exp $ */
...
ClassImp(IlcTARGETsimulationSDD)
////////////////////////////////////////////////////////////////////////
// Version: 0                                                         //
// Written by Piergiorgio Cerello                                     //
// November 23 1999                                                   //
//                                                                    //
// IlcTARGETsimulationSDD is the simulation of SDDs.                     //
////////////////////////////////////////////////////////////////////////
...
//______________________________________________________________________
void FastFourierTransform(IlcTARGETetfSDD *ilcsddetf,Double_t *real,
                          Double_t *imag,Int_t direction) {
    // Do a Fast Fourier Transform

    Int_t samples = ilcsddetf->GetSamples();
    ...
    for(i=1; i<=l; i++) {
        for(j=0; j<samples; j += m1) {
            Int_t p = 0;
            for(k=j; k<= j+m-1; k++) {
                Double_t wsr = ilcsddetf->GetWeightReal(p);
                Double_t wsi = ilcsddetf->GetWeightImag(p);
...
//______________________________________________________________________
IlcTARGETsimulationSDD::IlcTARGETsimulationSDD():
IlcTARGETsimulation(),
fTARGET(0),
fHitMap2(0),
fHitSigMap2(0),
...
//______________________________________________________________________
IlcTARGETsimulationSDD::IlcTARGETsimulationSDD(IlcTARGETsimulationSDD &source) :
    IlcTARGETsimulation(source){
    // Copy constructor to satify Coding roules only.

    if(this==&source) return;
    Error("IlcTARGETsimulationSDD","Not allowed to make a copy of "
          "IlcTARGETsimulationSDD Using default creater instead");
    IlcTARGETsimulationSDD();
}
//______________________________________________________________________
IlcTARGETsimulationSDD& IlcTARGETsimulationSDD::operator=(const IlcTARGETsimulationSDD &src){
    // Assignment operator to satify Coding roules only.

    if(this==&src) return *this;
    Error("IlcTARGETsimulationSDD","Not allowed to make a = with "
          "IlcTARGETsimulationSDD Using default creater instead");
    return *this ;
}
//______________________________________________________________________
IlcTARGETsimulation& IlcTARGETsimulationSDD::operator=(const IlcTARGETsimulation &src){
    // Assignment operator to satify Coding roules only.
...
//______________________________________________________________________
IlcTARGETsimulationSDD::IlcTARGETsimulationSDD(IlcTARGETDetTypeSim* dettyp):
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::Init(){
    ...
    IlcTARGETsegmentationSDD* seg = (IlcTARGETsegmentationSDD*)GetSegmentationModel2(2);
    
    IlcTARGETresponseSDD* res = (IlcTARGETresponseSDD*)fDetType->GetResponse(1);
    fpList = new IlcTARGETpList( seg->Npz(1),
                              fScaleSize*seg->Npx(1) );
    fHitSigMap2 = new IlcTARGETMapA2(1,seg,fScaleSize,1);
    fHitNoiMap2 = new IlcTARGETMapA2(1,seg,fScaleSize,1);
    fHitMap2 = fHitSigMap2;

    fNofMaps = seg->Npz(1);
    fMaxNofSamples = seg->Npx(1);
    fAnodeFire = new Bool_t [fNofMaps];
    
    Float_t sddLength = seg->Dx(1);
    Float_t sddWidth  = seg->Dz(1);

    Int_t dummy        = 0;
    Float_t anodePitch = seg->Dpz(1,dummy);
    Double_t timeStep  = (Double_t)seg->Dpx(1,dummy);
    Float_t driftSpeed = res->DriftSpeed();

    if(anodePitch*(fNofMaps/2) > sddWidth) {
        Warning("IlcTARGETsimulationSDD",
                "Too many anodes %d or too big pitch %f \n",
                fNofMaps/2,anodePitch);
    } // end if

    if(timeStep*fMaxNofSamples < sddLength/driftSpeed) {
        Error("IlcTARGETsimulationSDD",
              "Time Interval > Allowed Time Interval: exit\n");
        return;
    } // end if

    fElectronics = new IlcTARGETetfSDD(timeStep/fScaleSize,
                                    res->Electronics());
    ...
    fTARGET       = (IlcTARGET*)gIlc->GetModule("TARGET");
    Int_t size = fNofMaps*fMaxNofSamples;
    fStream    = new IlcTARGETInStream(size);
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::InitSimulationModule( Int_t module, Int_t event ) {
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::ClearMaps() {
    // clear maps
    fpList->ClearMap();
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::SDigitiseModule(IlcTARGETmodule *mod,Int_t md,Int_t ev){
...
//______________________________________________________________________
Bool_t IlcTARGETsimulationSDD::AddSDigitsToModule(TClonesArray *pItemArray,
                                               Int_t mask ) {
   ...
   IlcTARGETresponseSDD* res = (IlcTARGETresponseSDD*)fDetType->GetResponse(1);
    ...
    for( Int_t i=0; i<nItems; i++ ) {
        IlcTARGETpListItem * pItem = (IlcTARGETpListItem *)(pItemArray->At( i ));
        if( pItem->GetModule() != fModule ) {
            Error( "IlcTARGETsimulationSDD","Error reading, SDigits module "
        ...
        IlcTARGETpListItem * pItem2 = fpList->GetpListItem( pItem->GetIndex() );
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::FinishSDigitiseModule() {
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::DigitiseModule(IlcTARGETmodule *mod,Int_t md,Int_t ev){
    ...
    for( Int_t i=0; i<fNofMaps; i++ ) {
        for( Int_t j=0; j<fMaxNofSamples; j++ ) {
            ...
            IlcTARGETpListItem pItemTmp2( fModule, index, 0. );
            // put the fScaleSize analog digits in only one
            for( Int_t ik=0; ik<fScaleSize; ik++ ) {
                IlcTARGETpListItem *pItemTmp = fpList->GetpListItem( i, jdx+ik );
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::FinishDigits() {
    ...
    IlcTARGETCalibrationSDD* res = (IlcTARGETCalibrationSDD*)GetCalibrationModel(fModule);
    const char *kopt = res->GetZeroSuppOption();
    ZeroSuppression( kopt );
}
//______________________________________________________________________
void IlcTARGETsimulationSDD::HitsToAnalogDigits( IlcTARGETmodule *mod ) {
    // create maps to build the lists of tracks for each digit
  ...
  TObjArray *hits     = mod->GetHits();
    ...
    Float_t  sddWidth   = seg->Dz(1);
    Float_t  anodePitch = seg->Dpz(1,dummy);
    Float_t  timeStep   = seg->Dpx(1,dummy);
    ...
    for(ii=0; ii<nhits; ii++) {
        ...
        if(drPath < 0) {
            IlcDebug(1, // this should be fixed at geometry level
...
/*
//______________________________________________________________________
void IlcTARGETsimulationSDD::ListOfFiredCells(Int_t *arg,Double_t timeAmplitude,
					   TObjArray *ilcst,TClonesArray *padr){
    ...
    IlcTARGETTransientDigit* pdigit;
    ...
    if (!fHitMap1->TestHit(index, it)) {
        ...
        ilcst->AddAtAndExpand(new IlcTARGETTransientDigit(phys,digits),counter);
        fHitMap1->SetHit(index, it, counter);
        counter++;
        pdigit=(IlcTARGETTransientDigit*)ilcst->At(ilcst->GetLast());
        // list of tracks
        TObjArray *trlist=(TObjArray*)pdigit->TrackList();
        trlist->Add(&trinfo);
    } else {
        pdigit = (IlcTARGETTransientDigit*) fHitMap1->GetHit(index, it);
...
void IlcTARGETsimulationSDD::AddDigit( Int_t i, Int_t j, Int_t signal ) {
    // Adds a Digit.
    Int_t size = IlcTARGETdigitSPD::GetNTracks();
    Int_t digits[3];
    ...
    IlcTARGETpListItem *pItem = fpList->GetpListItem( i, j );
    if( pItem == 0 ) {
        phys = 0.0;
    ...
    fTARGET->AddSimDigit( 1, phys, digits, tracks, hits, charges ); 
    delete [] tracks;
    delete [] hits;
    delete [] charges;
}
//______________________________________________________________________
void IlcTARGETsimulationSDD::ChargeToSignal(Int_t mod,Bool_t bAddNoise) {
    // add baseline, noise, electronics and ADC saturation effects

    char opt1[20], opt2[20];
    IlcTARGETCalibrationSDD* res = (IlcTARGETCalibrationSDD*)GetCalibrationModel(mod);
...
void IlcTARGETsimulationSDD::ApplyDeadChannels(Int_t mod) {    
    // Set dead channel signal to zero
    IlcTARGETCalibrationSDD * calibr = (IlcTARGETCalibrationSDD *)GetCalibrationModel(mod);
    IlcTARGETsegmentationSDD* seg = (IlcTARGETsegmentationSDD*)GetSegmentationModel2(2);
    // nothing to do
    if( calibr->IsDead() ||   
    ...
    // static IlcTARGET *iTS = (IlcTARGET*)gIlc->GetModule( "TARGET" );
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::ApplyCrosstalk(Int_t mod) {
    ...
    // create and iniziilc crosstalk map
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::CompressionParam(Int_t i,Int_t &db,Int_t &tl,
                                           Int_t &th) const{
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::CompressionParam(Int_t i,Int_t &db,Int_t &tl) const{
    // returns the compression alogirthm parameters
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::SetCompressParam(){ 
    // Sets the compression alogirthm parameters  
    IlcTARGETCalibrationSDD* calibr = (IlcTARGETCalibrationSDD*)GetCalibrationModel(fModule);
...
//______________________________________________________________________
Int_t IlcTARGETsimulationSDD::Convert10to8(Int_t signal) const {
...
/*
//______________________________________________________________________
IlcTARGETMap*   IlcTARGETsimulationSDD::HitMap(Int_t i){
...
*/
//______________________________________________________________________
void IlcTARGETsimulationSDD::ZeroSuppression(const char *option) {
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::Init2D(){
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::Compress2D(){
    // simple TARGET cluster finder -- online zero-suppression conditions
...
//______________________________________________________________________
void  IlcTARGETsimulationSDD::FindCluster(Int_t i,Int_t j,Int_t signal,
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::Init1D(){
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::Compress1D(){
    ...
    // back to gilc.root file
    if(savedir) savedir->cd();
}
//______________________________________________________________________
void IlcTARGETsimulationSDD::StoreAllDigits(){
    ...
    for (i=0; i<fNofMaps; i++) {
        for (j=0; j<fMaxNofSamples; j++) {
            ...
            digits[1] = j;
            digits[2] = signal;
            fTARGET->AddRealDigit(1,digits);
        } // end for j
    } // end for i
} 
//______________________________________________________________________
void IlcTARGETsimulationSDD::CreateHistograms(Int_t scale){
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::FillHistograms(){
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::ResetHistograms(){
...
//______________________________________________________________________
TH1F *IlcTARGETsimulationSDD::GetAnode(Int_t wing, Int_t anode) { 
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::WriteToFile(TFile *hfile) {
...
//______________________________________________________________________
Float_t IlcTARGETsimulationSDD::GetNoise() {  
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::WriteSDigits(){
    ...
    for( Int_t i=0; i<fNofMaps; i++ ) {
        ...
        for( Int_t j=0; j<fMaxNofSamples; j++ ) {
            ...
            if( sig > 0.2 ) {
                ...
                ilcTARGET->AddSumDigit( pItemTmp2 );
            } // end if (sig > 0.2)
        }
    }
    return;
...
//______________________________________________________________________
void IlcTARGETsimulationSDD::PrintStatus() const {

...
Int_t CalculateFOTrigger( IlcTARGETLoader *itsl, IlcTARGETgeom* geom, TH1F* hDigits, Float_t& nFO)
{
  ...
  treeD = itsl->TreeD();
  br = treeD->GetBranch("TARGETDigitsSPD");
  ...
  for (m=startSPD;m<lastSPD;m++) {
    ...
    for(Int_t dig=0; dig<ndig; dig++){
      dp = (IlcTARGETdigitSPD*) digits->At(dig);

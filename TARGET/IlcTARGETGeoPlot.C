...
Int_t GetRecCoor(TObject *ge, TClonesArray *TARGETrec, Int_t mod, TH2F *h2, TH1F *h1, Bool_t verb);
Int_t GetClusCoor(TObject *ge, TClonesArray *TARGETrec, Int_t mod, TH2F *h2, TH1F *h1, Bool_t verb);
void GetDigits(TObject *tmps,TObject *ge,TClonesArray *TARGETdigits, Int_t subd, Int_t mod, Bool_t verbose, TObjArray & histos);
...
Int_t GetClusCoor(TObject *ge, TClonesArray *TARGETrec, Int_t mod, TH2F *h2, TH1F *h1, Bool_t verb){
  ...
  IlcTARGETgeom *geom = (IlcTARGETgeom*)ge;
  Int_t nrecp = TARGETrec->GetEntries();
  if(nrecp>0){
    ...
    for(Int_t irec=0;irec<nrecp;irec++) {
      IlcTARGETRecPoint *recp = (IlcTARGETRecPoint*)TARGETrec->At(irec);
      Double_t rot[9];     
...
void GetDigits(TObject *tmps,TObject *ge,TClonesArray *TARGETdigits, Int_t subd, Int_t mod, Bool_t verbose, TObjArray & histos){
  IlcTARGETsegmentation *seg = (IlcTARGETsegmentation*)tmps;
  IlcTARGETgeom *geom = (IlcTARGETgeom*)ge;
  Int_t layer;
  Int_t ladder;
  Int_t detec;
  ...
  if(ndigits){ 
    if(verbose){
      cout<<"-------------------------------------------------------"<<endl;
      cout<<"Number of DIGTARGET for module "<<mod<<": "<<ndigits<<endl;
    }
        ...
        if(ssdone[digit]==0){
          ...
          for(Int_t digi2=0;digi2<ndigits;digi2++){
            if(ssdone[digi2]==0 && impaired){
              IlcTARGETdigitSSD *dig2=(IlcTARGETdigitSSD*)TARGETdigits->UncheckedAt(digi2);
        ...
        if(verbose){
          ...
          if(subd==2){
            if(pair[digit]==-1){
              ...
              IlcTARGETdigitSSD *dig2=(IlcTARGETdigitSSD*)TARGETdigits->UncheckedAt(dtmp);

 ...
 Int_t nev = 5)            // number of events to process
{
	...
	IlcKalmanTrack::SetConvConst
	(
		1000/0.299792458/rl->GetIlcRun()->Field()->SolenoidField()
	);
	
	// Get the TARGET data & geometry & recpoints
	IlcTARGET *dTARGET = (IlcTARGET*)rl->GetIlcRun()->GetDetector("TARGET");
	if (!dTARGET) {
		cerr<<"IlcTARGETFindClusters.C : Can not find the TARGET detector !"<<endl;
		return 6;
	}
	IlcTARGETgeom *geom = dTARGET->GetTARGETgeom();
	...
	*/
	...
	IlcTARGETtrackerANN tracker(geom, 2);
	...
	// Get ESD files
	TFile *itsf=TFile::Open("IlcESDits.root","RECREATE");
	if ((!itsf)||(!itsf->IsOpen())) {
		cerr << "Can't IlcESDits.root !\n"; 
		return 1;
	}
	TFile *tpcf=TFile::Open("IlcESDtpc.root");
	if ((!tpcf)||(!tpcf->IsOpen())) {
		cerr<<"Can't IlcESDtpc.root !\n";
		return 1;
	}
	...
	for (Int_t i = 0; i < nev; i++) {
		...
		if (!cTree) {
			cerr<<"IlcTARGETFindTracksANN.C : NO clusters tree!" << endl;
			return 4;
		}
...
//		return;
		...
		tracker.ExportTracks("TARGET.Neural.root");

...
ClassImp(IlcTARGETVertexer3D)

//______________________________________________________________________
IlcTARGETVertexer3D::IlcTARGETVertexer3D():IlcTARGETVertexer(),
fLines(),
fVert3D(),
fCoarseDiffPhiCut(0.),
fMaxRCut(0.),
...
//______________________________________________________________________
IlcTARGETVertexer3D::IlcTARGETVertexer3D(TString fn): IlcTARGETVertexer(fn),
fLines(),
fVert3D(),
fCoarseDiffPhiCut(0.),
fMaxRCut(0.),
...
fDiffPhiMax(0.)  {
  ...
  fLines = new TClonesArray("IlcStrLine",1000);
  SetCoarseDiffPhiCut();
  SetMaxRCut();
  SetZCutDiamond();
  SetDCAcut();
...
//______________________________________________________________________
IlcTARGETVertexer3D::~IlcTARGETVertexer3D() {
  // Destructor
...
//______________________________________________________________________
Int_t IlcTARGETVertexer3D::FindTracklets(Int_t evnumber, Int_t optCuts){
  ...
  if(IlcDebugLevelClass()) printf("RecPoints on Layer 1,2 = %d, %d\n",nrpL1,nrpL2);

  Double_t a[3]={xbeam,ybeam,0.}; 
  Double_t b[3]={xbeam,ybeam,10.};
  IlcStrLine zeta(a,b,kTRUE);
  ...
  for(Int_t modul1= irstL1; modul1<=lastL1;modul1++){   // Loop on modules of layer 1
    ...
    TClonesArray *prpl1 = new TClonesArray("IlcTARGETRecPoint",nrecp1);
    prpl1->SetOwner();
    TClonesArray &rpl1 = *prpl1;
    for(Int_t j=0;j<nrecp1;j++){
      IlcTARGETRecPoint *recp = (IlcTARGETRecPoint*)itsRec->At(j);
      new(rpl1[j])IlcTARGETRecPoint(*recp);
    }
    detTypeRec.ResetRecPoints();
    for(Int_t j=0;j<nrecp1;j++){
      IlcTARGETRecPoint *recp = (IlcTARGETRecPoint*)prpl1->At(j);
	...
	for(Int_t j2=0;j2<nrecp2;j2++){
	  recp = (IlcTARGETRecPoint*)itsRec->At(j2);
	  ...
	  if(phi2<0)phi2=2*TMath::Pi()+phi2;
          ...
          IlcStrLine line((Double_t*)gc,(Double_t*)gc2,kTRUE);
	  Double_t cp[3];
...
//______________________________________________________________________
void IlcTARGETVertexer3D::FindVertices(){
...
//______________________________________________________________________
void IlcTARGETVertexer3D::Find3DVertex(){
  // Determines the vertex position 
  // same algorithm as in IlcVertexerTracks::StrLinVertexFinderMinDist(0)
  // adapted to pure IlcStrLine objects
  ...
  for(Int_t i=0; i<knacc; i++){
    IlcStrLine *line1 = (IlcStrLine*)fLines->At(i); 
    ...
    Double_t matr[3][3];
    Double_t dknow[3];
    IlcVertexerTracks::GetStrLinDerivMatrix(p0,p1,matr,dknow);
  ...
  Double_t vett[3][3];
  Double_t det=IlcVertexerTracks::GetDeterminant3X3(sum);
  Double_t initPos[3];
  Double_t sigma = 0.;
  ...
  if(det!=0){
	...
	for(Int_t kk=0;kk<3;kk++) vett[ww][kk]=sum[ww][kk];
      ...
      initPos[zz]=IlcVertexerTracks::GetDeterminant3X3(vett)/det;
    }
	...
	sigma+=IlcVertexerTracks::GetStrLinMinDist(p0,p1,initPos);
      }
  ...
  if(fVert3D){
    ...
    fVert3D = new IlcVertex(initPos,sigma,knacc);
  }
  if(IlcDebugLevelClass()) fVert3D->Print();
}



//______________________________________________________________________
Int_t  IlcTARGETVertexer3D::Prepare3DVertex(){
  ...
  Int_t *vilcdate = new Int_t [fLines->GetEntriesFast()];
  for(Int_t i=0; i<fLines->GetEntriesFast();i++)vilcdate[i]=0;
  //  Int_t itot=fLines->GetEntriesFast()-1-1;
  for(Int_t i=0; i<fLines->GetEntriesFast()-1;i++){
    //if(vilcdate[i]==1)continue;
    IlcStrLine *l1 = (IlcStrLine*)fLines->At(i);
    for(Int_t j=i+1;j<fLines->GetEntriesFast();j++){
      IlcStrLine *l2 = (IlcStrLine*)fLines->At(j);
  ...
  for(Int_t i=0; i<fLines->GetEntriesFast()-1;i++){ // remove tracklets sharing one recpoint
    if(vilcdate[i]==0)continue;
    IlcStrLine *l1 = (IlcStrLine*)fLines->At(i);
    for(Int_t j=i+1;j<fLines->GetEntriesFast();j++){
    if(vilcdate[j]==0)continue;
      IlcStrLine *l2 = (IlcStrLine*)fLines->At(j);
  ...
  for(Int_t i=0; i<fLines->GetEntriesFast();i++)if(vilcdate[i]>=1)numbtracklets++;
  if(numbtracklets<2){delete [] vilcdate; delete h3d; return retcode; }

  for(Int_t i=0; i<fLines->GetEntriesFast();i++){
    if(vilcdate[i]<1)fLines->RemoveAt(i);
  }
  fLines->Compress();
  if (IlcDebugLevelClass()) cout<<"Number of tracklets (after compress) "<<fLines->GetEntriesFast()<<endl;
  delete [] vilcdate;
  ...
  fLines->Compress();
  if (IlcDebugLevelClass()) cout<<"Number of tracklets (after 2nd compression) "<<fLines->GetEntriesFast()<<endl;
  ...
  if(fLines->GetEntriesFast()>1){
    ...
    fVert3D->GetXYZ(peak);
    if (IlcDebugLevelClass()) cout<<"FIRST V candidate: "<<peak[0]<<"; "<<peak[1]<<"; "<<peak[2]<<endl;
    for(Int_t i=0; i<fLines->GetEntriesFast()-1;i++){
      IlcStrLine *l1 = (IlcStrLine*)fLines->At(i);
      if(l1->GetDistFromPoint(peak)> fDCAcut)fLines->RemoveAt(i);
    }
    fLines->Compress();
    if (IlcDebugLevelClass()) cout<<"Number of tracklets (after 3rd compression) "<<fLines->GetEntriesFast()<<endl;
    if(fLines->GetEntriesFast()>1){
 ...
 //______________________________________________________________________
void  IlcTARGETVertexer3D::MakeTracklet(Double_t *pA, Double_t *pB, Int_t &nolines) {
  // makes a tracklet
  ...
  new(lines[nolines++])IlcStrLine(pA,pB,kTRUE);
}

 //______________________________________________________________________
void  IlcTARGETVertexer3D::MakeTracklet(Float_t *pA, Float_t *pB, Int_t &nolines) {// Makes a tracklet
  //
  Double_t a[3],b[3];
  for(Int_t i=0;i<3;i++){
    a[i] = pA[i];
...
void IlcTARGETVertexer3D::PrintStatus() const {

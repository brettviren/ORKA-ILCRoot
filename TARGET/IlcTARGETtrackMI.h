#ifndef ILCTARGETTRACKMI_H
#define ILCTARGETTRACKMI_H
...
/*****************************************************************************
 *                          December 18, 2000                                *
 *  Internal view of the TARGET track parametrisation as well as the order of   *
 *           track parameters are subject for possible changes !             *
 *  Use GetExternalParameters() and GetExternalCovariance() to access TARGET    *
...
class IlcTARGETtrackMI : public IlcTARGETtrackV2 {
public:
  IlcTARGETtrackMI();
  IlcTARGETtrackMI(IlcESDtrack& t,Bool_t c=kFALSE) throw (const Char_t *);
  IlcTARGETtrackMI(const IlcTARGETtrackMI& t);
  ...
  Double_t GetD(Double_t x, Double_t y) const
    {return IlcTARGETtrackV2::GetD(x,y);}
...
protected:
  ...
  Float_t fChi2MIP[kMaxClusters];      // MIP chi squres 

  Float_t fDy[kMaxClusters];           //dy in layer
  Float_t fDz[kMaxClusters];           //dz in layer
  Float_t fSigmaY[kMaxClusters];       //sigma y 
  Float_t fSigmaZ[kMaxClusters];       //sigma z
  Float_t fNy[kMaxClusters];           //expected size of cluster
  Float_t fNz[kMaxClusters];           //expected size of cluster
  Float_t fD[2];                       //distance to the vertex
  Float_t fDnorm[2];                   // normilczed distance to the vertex
  Float_t fNormQ[kMaxClusters];        // normilczed Q
  Float_t fExpQ;                       // expected Q
  Float_t fNormChi2[kMaxClusters];     // normilczed chi2 
  Float_t fChi22;                      // chi22
  Float_t fdEdxMismatch;    
  Bool_t fConstrain;                   //indication of the vertex constrain
  Int_t  fClIndex[kMaxClusters];       //cluster Index
  Bool_t fGoldV0;                      //corresponding gold V0 found
  ClassDef(IlcTARGETtrackMI,1)   //TARGET reconstructed track
};

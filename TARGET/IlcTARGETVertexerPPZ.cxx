...
// IlcTARGETVertexerIons, which in turn is optimized for A-A collsions    //
// Origin: masera@to.infn.it      9/12/2002                            //
//                                                                     //
/////////////////////////////////////////////////////////////////////////
ClassImp(IlcTARGETVertexerPPZ)


 //______________________________________________________________________
#ifdef WIN32 
IlcESDVertex* IlcTARGETVertexerPPZ::FindVertexForCurrentEvent(TTree*)
{ 
...
//______________________________________________________________________
IlcTARGETVertexerPPZ::IlcTARGETVertexerPPZ():IlcTARGETVertexer() {
  // Default Constructor
...
IlcTARGETVertexerPPZ::IlcTARGETVertexerPPZ(TString fn, Float_t x0, Float_t y0):IlcTARGETVertexer(fn) {
...
//______________________________________________________________________
IlcTARGETVertexerPPZ::~IlcTARGETVertexerPPZ() {
  // Default Destructor
  //fTARGET = 0;
}

//________________________________________________________
void  IlcTARGETVertexerPPZ::EvalZ(TH1F *hist,Int_t sepa, Int_t ncoinc, TArrayF *zval) {
  ...
  if(nbinNotZero==1){
    fZFound = curz;
    fZsig=0;
    fCurrentVertex = new IlcESDVertex(fZFound,fZsig,nbinNotZero);
    return;
  }
  ...
  if(IlcDebugLevelClass()>1)cout<<"totst, totst2, cut: "<<totst<<", "<<totst2<<", "<<cut<<endl;
  ...
  if((val2-val1)>deltaVal){
    ...
    if(IlcDebugLevelClass()>0)cout<<"val1 and val2 recomputed\n";
  }
  if(IlcDebugLevelClass()>0)cout<<"Values for Z finding: "<<val1<<" "<<val2<<" "<<val2-val1<<endl;
  fZFound=0;
  fZsig=0;
  nN=0;
  ...
  if(nN==1){
    fZsig = 0;
    fCurrentVertex = new IlcESDVertex(fZFound,fZsig,nN);
    return;
  }
...
//______________________________________________________________________
IlcESDVertex* IlcTARGETVertexerPPZ::FindVertexForCurrentEvent(Int_t evnumber){
  ...
  if(!geom) {
    Error("FindVertexForCurrentEvent","TARGET geometry is not defined");
  ...
  if(!branch){ 
    branch = tR->GetBranch("TARGETRecPointsF");
  }
  //}
  if(!branch){
   Error("FindVertexForCurrentEvent","branch for TARGET rec points not found");
   return fCurrentVertex;
  }
  Float_t zave=0;
  Float_t rmszav=0;
  ...
  for(Int_t module= fFirstL1; module<=fLastL1;module++){
    ...
    for(Int_t i=0; i<nrecp1;i++){
      IlcTARGETRecPoint *current = (IlcTARGETRecPoint*)itsRec->At(i);
  ...
  Int_t ncoinc=0;
  for(Int_t module= fFirstL1; module<=fLastL1;module++){
    if(IlcDebugLevelClass()>0)cout<<"processing module   "<<module<<"                  \r";
    branch->GetEvent(module);
    Int_t nrecp1 = itsRec->GetEntries();
    ...
    detTypeRec.ResetRecPoints();
    for(Int_t i=0; i<nrecp1;i++){
      IlcTARGETRecPoint *current = (IlcTARGETRecPoint*)poiL1->At(i);
      lc[0]=current->GetDetLocalX();
      lc[2]=current->GetDetLocalZ();
      geom->LtoG(module,lc,gc);
      gc[0]-=fX0;
      ...
      if(IlcDebugLevelClass()>1)cout<<"module "<<module<<" "<<gc[0]<<" "<<gc[1]<<" "<<gc[2]<<" "<<phi1<<"     \n";
...
//______________________________________________________________________
void IlcTARGETVertexerPPZ::FindVertices(){
...
//________________________________________________________
void IlcTARGETVertexerPPZ::PrintStatus() const {
...
//________________________________________________________
Float_t IlcTARGETVertexerPPZ::Curv(Double_t x1,Double_t y1,

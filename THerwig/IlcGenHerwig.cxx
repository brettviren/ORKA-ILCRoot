...
/* $Id: IlcGenHerwig.cxx 51126 2011-08-17 13:37:49Z cvetan $ */
...
ClassImp(IlcGenHerwig)


  IlcGenHerwig::IlcGenHerwig() :
    IlcGenMC(),
    fAutPDF("LHAPDF"),
    fModPDF(19070),
    fStrucFunc(kCTEQ5L),
...
IlcGenHerwig::IlcGenHerwig(Int_t npart)
    :IlcGenMC(npart),
    fAutPDF("LHAPDF"),
    fModPDF(19070),
    fStrucFunc(kCTEQ5L),
...
// Constructor
    ...
    IlcHerwigRndm::SetHerwigRandom(GetRandom());
}
...
void IlcGenHerwig::SetEventListRange(Int_t eventFirst, Int_t eventLast)
{
  fEv1Pr = eventFirst;
  fEv2Pr = eventLast;
...
void IlcGenHerwig::Generate()
{
  ...
  while(1)
	...
	// 
	if (gIlc) {
	    if (gIlc->GetEvNumber()>=fEv1Pr &&
		gIlc->GetEvNumber()<=fEv2Pr) fHerwig->PrintEvt();
...
Bool_t IlcGenHerwig::CheckParton(const TParticle* parton1, const TParticle* parton2)
{
...
void IlcGenHerwig::AdjustWeights()
{
...
void IlcGenHerwig::KeepFullEvent()
{
    fKeep=1;
}

Bool_t IlcGenHerwig::DaughtersSelection(const TParticle* iparticle, const TClonesArray* particles)
{
//
...
Bool_t IlcGenHerwig::SelectFlavor(Int_t pid) const
{
...
Bool_t IlcGenHerwig::Stable(const TParticle*  particle) const
{
...
void IlcGenHerwig::FinishRun()
{
  fHerwig->Hwefin();
}

void IlcGenHerwig::FinishRunJimmy()
{
  fHerwig->Hwefin();
  fHerwig->Jmefin();
...
void IlcGenHerwig::MakeHeader()
{
//
// Make header for the simulated event
// 
  if (fHeader) delete fHeader;
  fHeader = new IlcGenHerwigEventHeader("Herwig");
//
// Event type  
    ((IlcGenHerwigEventHeader*) fHeader)->SetProcessType(fHerwig->GetIHPRO());
//
// Number of trials
    ((IlcGenHerwigEventHeader*) fHeader)->SetTrials(fTrials);
//
// Event weight (cross section)
    ((IlcGenHerwigEventHeader*) fHeader)->SetWeight(fHerwig->GetEVWGT());
//
// Event Vertex 

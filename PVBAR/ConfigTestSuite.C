...
void Config()
{
   ...
   // libraries required by geant321
    ...
    if (!gSystem->Getenv("CONFIG_FILE"))
    {
        ...
        IlcRunLoader* rl = IlcRunLoader::Open("gilc.root",IlcConfig::GetDefaultEventFolderName(),
                                              "recreate");
        if (rl == 0x0)
         {
...
//     gener->Init();
    ...
    IlcGenBox *gener = new IlcGenBox(1);
    ...
    //
	IlcITSvPPRasymm *ITS  = new IlcITSvPPRasymm("ITS","New ITS PPR detailed version with asymmetric services");
	...
	//
    //IlcITSvPPRsymm *ITS  = new IlcITSvPPRsymm("ITS","New ITS PPR detailed version with symmetric services");
    //ITS->SetMinorVersion(2);                                       // don't touch this parameter if you're not an ITS developer
    //ITS->SetReadDet(kFALSE);                                       // don't touch this parameter if you're not an ITS developer
    //ITS->SetWriteDet("$ILC_ROOT/ITS/ITSgeometry_vPPRsymm2.det"); // don't touch this parameter if you're not an ITS developer
    //ITS->SetThicknessDet1(200.);   // detector thickness on layer 1 must be in the range [100,300]
    //ITS->SetThicknessDet2(200.);   // detector thickness on layer 2 must be in the range [100,300]
    //ITS->SetThicknessChip1(200.);  // chip thickness on layer 1 must be in the range [150,300]
    //ITS->SetThicknessChip2(200.);  // chip thickness on layer 2 must be in the range [150,300]
	...
	if (geo == kHoles) {
        //=================== TOF parameters ============================
	    IlcTOF *TOF = new IlcTOFv2FHoles("TOF", "TOF with Holes");
	} else {
    ...
    {
        //=================== EMCAL parameters ============================
        IlcEMCAL *EMCAL = new IlcEMCALv1("EMCAL", "EMCALArch1a");
    }
    ...
    {
        //=================== ACORDE parameters ============================
        IlcVZERO *VZERO = new IlcVZEROv2("VZERO", "normal VZERO");
    }

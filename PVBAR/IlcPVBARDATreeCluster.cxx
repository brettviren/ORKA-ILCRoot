 ...
 **************************************************************************/
/* $Id: IlcPVBARDATreeCluster.cxx 54863 2012-02-28 12:10:29Z hristov $ */
...
ClassImp(IlcPVBARDATreeCluster)
//------------------------------------------------------------------------
IlcPVBARDATreeCluster::IlcPVBARDATreeCluster(const IlcPVBARDATreeCluster& cluster):
...
//------------------------------------------------------------------------
IlcPVBARDATreeCluster& IlcPVBARDATreeCluster::operator=(const IlcPVBARDATreeCluster& cluster){
  // Copy Operator
...
//------------------------------------------------------------------------
void IlcPVBARDATreeCluster::Print(Option_t *opt) const
{
  // Print out
  std::cout<<" IlcPVBARDATreeCluster:: Energy="<<fEnergy<<" NDigits="<<fNDigits
...
//------------------------------------------------------------------------
std::ostream& operator<<(std::ostream& out, const IlcPVBARDATreeCluster& cluster){
  // Print out
  out<<" IlcPVBARDATreeCluster:: Energy="<<cluster.fEnergy<<" NDigits="<<cluster.fNDigits
...
//------------------------------------------------------------------------
void IlcPVBARDATreeCluster::Reset(){
  // Reset information
...
//------------------------------------------------------------------------
bool IlcPVBARDATreeCluster::Append(IlcPVBARDATreeDigit& digit){
  // Add digit information and sum all energy
  //
  if(! digit.IsValid() ){
    std::cout<<" IlcPVBARDATreeCluster::Append():: Error!! Digit is not valid.."<<std::endl;
    return false;
  }
  IlcPVBARDATreeDigit* newfDigits = new IlcPVBARDATreeDigit[fNDigits+1];
  bool bsearching = true;
  int ndigit = fNDigits;
  while( ndigit-- ){
    if( fDigits[ndigit].GetAbsId() == digit.GetAbsId() ){
      std::cout<<" IlcPVBARDATreeCluster::Append():: Error!! The channel already exist."<<std::endl;
...
//------------------------------------------------------------------------
bool IlcPVBARDATreeCluster::Append(IlcPVBARDATreeCluster& cluster){
  // Add another cluster information and sum all energy
  //
  IlcPVBARDATreeDigit* newfDigits = new IlcPVBARDATreeDigit[fNDigits+cluster.fNDigits];
  ...
  while( ndigitsall-- ){
	...
	newfDigits[ndigitsall]= cluster.fDigits[--ndigits2];
      ...
      std::cout<<" IlcPVBARDATreeCluster::Append() Something wrong.. "<<std::endl;
      delete [] newfDigits;
      return false;
    }
  }
...
//------------------------------------------------------------------------
bool IlcPVBARDATreeCluster::IsNeighbor(const IlcPVBARDATreeDigit& digit) const{
...
//------------------------------------------------------------------------
bool IlcPVBARDATreeCluster::IsNeighbor(const IlcPVBARDATreeCluster& cluster) const{
...
//------------------------------------------------------------------------
bool IlcPVBARDATreeCluster::CalculateProperty(){
  ...
  int ndigits = fNDigits;
  while( ndigits-- ){
    weight = log(fDigits[ndigits].GetEnergy()/fEnergy) + 4.5; //4.5 is for PVBAR
    //std::cout<<" IlcPVBARDATreeCluster::CalculateProperty() DEBUG: ndigits="<<ndigits<<" weight="<<weight<<std::endl;
    if( weight > 0 ){
      totweight += weight;
  ...
  //std::cout<<" IlcPVBARDATreeCluster::CalculateProperty() DEBUG: totweight="<<totweight<<std::endl;
  if( totweight > 0 ){
    fRow /= totweight;
    fCol /= totweight;
  } else {

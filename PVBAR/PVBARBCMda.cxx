...
link: http://ilcinfo.cern.ch/static/phpBB3/viewtopic.php?f=4&t=17
reference run: /ilc/data/2009/LHC09c_PVBAR/000098979/raw
run type: LED
DA type: MON
...
int main(int argc, char **argv) {
  ...
  for(;;) {
	...
	dAs[stream.GetModule()] = new IlcPVBARDA2(stream.GetModule(),0);
	dAs[stream.GetModule()]->FillQualityHistograms(q);
	dAs[stream.GetModule()]->FillFiredCellsHistogram(nFired);
      } 
  ...
  TFile* f = new TFile("PVBAR_BCM.root","recreate");
  ...
  daqDA_FES_storeFile("PVBAR_BCM.root","BAD_CHANNELS");

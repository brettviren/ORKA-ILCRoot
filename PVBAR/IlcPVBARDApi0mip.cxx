 ...
 **************************************************************************/
/* $Id: IlcPVBARDApi0mip.cxx 52964 2011-11-17 12:12:19Z policheh $ */
...
#include <math.h>
#include "IlcLog.h"
#include "IlcPVBARDApi0mip.h"
ClassImp(IlcPVBARDApi0mip)
//----------------------------------------------------------------
IlcPVBARDApi0mip::IlcPVBARDApi0mip(int module,int iterid,const char* fopt) :
  ...
  TString shname="IlcPVBARDApi0mip_mod%d_iter%d";
  snprintf(hname,shname.Length(),shname.Data(),module,iterid);
  SetName(hname);

  TString shtitle="PVBAR MIP/pi0 Calibration DA for Module:%d Iteration:%d";
  ...
  TString sfname="IlcPVBARDApi0mip_mod%d.root";
  snprintf(fname,sfname.Length(),sfname.Data(),module);
  fTFile = TFile::Open(fname,fopt);
}
//----------------------------------------------------------------
IlcPVBARDApi0mip::IlcPVBARDApi0mip(const IlcPVBARDApi0mip& da):
  ...
  if( da.fCreateTree ){
    ...
    fTTree->Branch("IlcPVBARDATreeEvent","IlcPVBARDATreeEvent",&fEvent);
  } else {
    fCreateTree = false;
    fTTree = 0;
  }
...
//-------------------------------------------------------------------
IlcPVBARDApi0mip& IlcPVBARDApi0mip::operator= (const IlcPVBARDApi0mip&)
{
...
//-------------------------------------------------------------------
IlcPVBARDApi0mip::~IlcPVBARDApi0mip(){
  // Destructor

  //std::cout<<" DEBUGDEBUG :: IlcPVBARDApi0mip:: deconstructor "<<std::endl;
  if( fCreateHist ){
...
//-------------------------------------------------------------------
void IlcPVBARDApi0mip::FillDigit(float adc,int row,int col){
  // Put new digit information in event
  //

  if( fEvent==0 ) fEvent = new IlcPVBARDATreeEvent;
  float energy = adc * 0.005; // 5MeV/ch is fixed.
  if( energy > 0.020 ){  // Threshold at 20MeV.
    //std::cout<<" IlcPVBARDApi0mip::AppendDigit() DEBUG:: (row,col)=("<<row<<","<<col<<") adc="<<adc<<" energy="<<energy<<std::endl;
    fEvent->Fill(energy,row,col);
  }
}
//-------------------------------------------------------------------
void IlcPVBARDApi0mip::NewEvent(){
...
//----------------------------------------------------------------
bool IlcPVBARDApi0mip::CreateTree(){
  if(! fTFile ) {
    return false;
  }
  fTFile->cd();
...
//----------------------------------------------------------------
bool IlcPVBARDApi0mip::CreateHist(){
  ...
  if( fH1Time>0 ){
    std::cout<<" IlcPVBARDApi0mip:Warning!! Output object already exist : "<<fH1Time->GetName()<<std::endl;
    return false;
  }
  ...
  snprintf(htitle,100,"PVBAR pi0 mass vs Digit Id : Mod:%d Iter:%d",fMod,fIterId);
  fH2Pi0DigitId = new TH2F(hname,htitle,17920,0,17920,120,0,0.3);
      
  snprintf(hname,100,"h2_mipdigitid_mod%d_iter%d",fMod,fIterId);
  snprintf(htitle,100,"PVBAR MIP vs Digit Id : Mod:%d Iter:%d",fMod,fIterId);
  ...
  snprintf(htitle,100,"PVBAR pi0 mass vs pT vs Asym : Mod:%d Iter:%d",fMod,fIterId);
...
//-------------------------------------------------------------------
void IlcPVBARDApi0mip::FillTree(IlcPVBARDATreeEvent* event){
  // Fill all information (IlcPVBARDATreeEvent) into TTree
  // if event==NULL, the obtained information (=fEvent) is used.

  if(! fCreateTree ) CreateTree();
  if( event == 0 ){
    if( fEvent == 0 ){
      fEvent = new IlcPVBARDATreeEvent();
    }
...
//-------------------------------------------------------------------
void IlcPVBARDApi0mip::FillHist(IlcPVBARDATreeEvent* event){
  // Fill all information (IlcPVBARDATreeEvent) into histograms
  // if event==NULL, the obtained information (=fEvent) is used.
  ...
  int ndigits;
  int ndigitsall = 0;
  IlcPVBARDATreeDigit digit1, digit2;
  ...
  //std::cout<<" IlcPVBARDApi0mip::FillHisto() DEBUG:: Number of clusters: "<<nclt1<<std::endl;
  while( nclt1-- ){
    IlcPVBARDATreeCluster& clt1 = event->GetCluster(nclt1);
    ndigits = clt1.GetNDigits();
    while( ndigits-- ){
      IlcPVBARDATreeDigit& digit = clt1.GetDigit(ndigits);
    ...
    if( clt1.GetEnergy() > 0.2 ){
      nclt2 = nclt1;
      while( nclt2-- ){
	IlcPVBARDATreeCluster& clt2 = event->GetCluster(nclt2);
...
//-------------------------------------------------------------------
void IlcPVBARDApi0mip::Print(Option_t *option) const
{
  // Print Out

...
// IHEP CPV/PVBAR reconstruction algorithm used.
...
ClassImp(IlcPVBARIhepAnalyze)

//____________________________________________________________________________
IlcPVBARIhepAnalyze::IlcPVBARIhepAnalyze():
  fRunLoader(0),
  fFileName()
{
}
...
//____________________________________________________________________________
IlcPVBARIhepAnalyze::IlcPVBARIhepAnalyze(Text_t * name) : 
  fRunLoader(0),
  fFileName(name)
{
...
//____________________________________________________________________________
void IlcPVBARIhepAnalyze::AnalyzeCPV1(Int_t Nevents)
{
  //
  ...
  IlcPVBARLoader* please = dynamic_cast<IlcPVBARLoader*>(fRunLoader->GetLoader("PVBARLoader"));
  if ( please == 0 )
   {
...
/******************************************************************/     
    ...
    if (! (branchCPVimpacts =treeH->GetBranch("PVBARCpvImpacts")) ) {
      IlcWarning(Form("Couldn't find branch PVBARCpvImpacts. Exit.")) ;
      return;
    }
 
    // Create and fill arrays of hits for each CPV module
      
    Int_t nOfModules = PVBARgeom->GetNModules();
    TClonesArray **hitsPerModule = new TClonesArray *[nOfModules];
    Int_t iModule = 0; 	
    for (iModule=0; iModule < nOfModules; iModule++)
      hitsPerModule[iModule] = new TClonesArray("IlcPVBARImpact",100);
	...
	for (Int_t iImpact=0; iImpact<impacts->GetEntries(); iImpact++) {
	  ...
	  new(lhits[hitsPerModule[iModule]->GetEntriesFast()]) IlcPVBARImpact(*impact);
	}
      }
      fCpvImpacts->Clear();
    }
    ...
    IlcPVBARCpvRecPoint *cpvRecPoint ;
    Float_t xgen, ygen, zgen;
    while( ( cpvRecPoint = (IlcPVBARCpvRecPoint *)nextRP() ) ) {
      
      Float_t chi2dof = ((IlcPVBAREvalRecPoint*)cpvRecPoint)->Chi2Dof();
      hChi2->Fill(chi2dof);
      hEr->Fill(cpvRecPoint->GetEnergy());

      TVector3  locpos;
      cpvRecPoint->GetLocalPosition(locpos);
      Int_t PVBARModule = cpvRecPoint->GetPVBARMod();
      ...
      Int_t nCPVhits  = (hitsPerModule[PVBARModule-1])->GetEntriesFast();
	...
	impact = (IlcPVBARImpact*)(hitsPerModule[PVBARModule-1])->UncheckedAt(ihit);
	xgen   = impact->X();
	zgen   = impact->Z();
	ygen   = impact->Y();
	...
	Float_t distanceIPtoCPV = PVBARgeom->GetIPtoOuterCoverDistance() -
	  (PVBARgeom->GetFTPosition(1)+
	   PVBARgeom->GetFTPosition(2)+
	   PVBARgeom->GetCPVTextoliteThickness())/2;
	Float_t xoL,yoL,zoL ;
...
void IlcPVBARIhepAnalyze::AnalyzeEMC1(Int_t Nevents)
{
  //
  ...
  for ( Int_t ievent=0; ievent<Nevents; ievent++) {  
    ...
    if (! (branchEMCimpacts =treeH->GetBranch("PVBAREmcImpacts")) ) {
      IlcWarning(Form(" Couldn't find branch PVBAREmcImpacts. Exit."));
      return;
    }
    ...
    IlcPVBAREmcRecPoint *emcRecPoint ;
    Float_t xgen, ygen, zgen;
    while( ( emcRecPoint = (IlcPVBAREmcRecPoint *)nextRP() ) ) {
      
      Float_t chi2dof = ((IlcPVBAREvalRecPoint*)emcRecPoint)->Chi2Dof();
      hChi2->Fill(chi2dof);
      
      TVector3  locpos;
      emcRecPoint->GetLocalPosition(locpos);
      Int_t PVBARModule = emcRecPoint->GetPVBARMod();
      Int_t rpMult     = emcRecPoint->GetMultiplicity();
      Int_t rpMultX, rpMultZ;
      ((IlcPVBAREvalRecPoint*)emcRecPoint)->GetClusterLengths(rpMultX,rpMultZ);
      Float_t xrec  = locpos.X();
      Float_t zrec  = locpos.Z();
      Float_t dxmin = 1.e+10;
      Float_t dzmin = 1.e+10;
      ...
      Int_t nEMChits  = (hitsPerModule[PVBARModule-1])->GetEntriesFast();
      Float_t locImpX=1e10,locImpZ=1e10;         // local coords of closest impact
      Float_t gImpX=1e10, gImpZ=1e10,gImpY=1e10; // global coords of closest impact
...
//____________________________________________________________________________
void IlcPVBARIhepAnalyze::AnalyzeCPV2(Int_t Nevents)
{
  ...
  for (Int_t nev=0; nev<Nevents; nev++) 
    { 
	 ...
	 IlcError(Form("Can not get TreeH"));
      ...
      if (! (branchCPVimpacts =treeH->GetBranch("PVBARCpvImpacts")) )  return;
	...
	printf("CPV module %d has %d hits\n",iModule,nsum);
        ...
        IlcPVBARImpact* genHit1;
        IlcPVBARImpact* genHit2;
        Int_t irp1,irp2;
	for(irp1=0; irp1< nsum; irp1++)
	  {
	    genHit1 = (IlcPVBARImpact*)((hitsPerModule[iModule])->At(irp1));
	    for(irp2 = irp1+1; irp2<nsum; irp2++)
	      {
		genHit2 = (IlcPVBARImpact*)((hitsPerModule[iModule])->At(irp2));
...
//      		IlcInfo(Form("(dx dz dr): %f %f", dx, dz));
  ...
  //--------- Combinatoric distance between rec. hits in CPV
      ...
      if(nRecCPV)
	{
	  IlcPVBARCpvRecPoint* recHit1;
	  IlcPVBARCpvRecPoint* recHit2;
	  ...
	  while(TObject* obj1 = nextCPVrec1() )
	      ...
	      while (TObject* obj2 = nextCPVrec2())
		{
		  if(!obj2->IsEqual(obj1))
		    {
		      recHit1 = (IlcPVBARCpvRecPoint*)obj1;
		      recHit2 = (IlcPVBARCpvRecPoint*)obj2;
		      TVector3 locpos1;
		      TVector3 locpos2;
		      ...
		      Float_t dr = TMath::Sqrt(dx*dx + dz*dz);
		      if(recHit1->GetPVBARMod() == recHit2->GetPVBARMod())
			hDrijCPVr->Fill(dr);
		    }
		}
	    }	
...
void IlcPVBARIhepAnalyze::CpvSingle(Int_t nevents)
{
  ...
  IlcPVBARLoader* gime = dynamic_cast<IlcPVBARLoader*>(fRunLoader->GetLoader("PVBARLoader"));
  if ( gime == 0 )
   {
     IlcError(Form("Could not obtain the Loader object !"));
     return ;
  ...
  for(Int_t ievent=0; ievent<nevents; ievent++)
    {
      ...
      IlcPVBARCpvRecPoint* pt = (IlcPVBARCpvRecPoint*)(gime->CpvRecPoints())->At(0);
      if(pt) {
	TVector3 lpos;
	...
	hNrpX->Fill(rpMultX);
	hNrpZ->Fill(rpMultZ);
	hChi2->Fill(((IlcPVBAREvalRecPoint*)pt)->Chi2Dof());
...
void IlcPVBARIhepAnalyze::HitsCPV(Int_t nev)
{
...
//    IlcPVBARLoader * please = IlcPVBARLoader::GetInstance(GetFileName().Data(),"PVBAR");
//    const IlcPVBARGeometry *  fGeom  = please->PVBARGeometry();

//    Int_t ntracks = gIlc->GetEvent(ievent);
//    Int_t nOfModules = fGeom->GetNModules();
//    IlcInfo(Form(" Tracks: "<<ntracks<<"  Modules: "<<nOfModules));

//    if (! (branchCPVimpacts =gIlc->TreeH()->GetBranch("PVBARCpvImpacts")) )  return;
...
//  	    new(hits[hits.GetEntriesFast()]) IlcPVBARImpact(*impact);
//  	    Info(Form("done."));
//  	  }
//        }
//      }
...
//    Info(Form(" PVBAR event "<<ievent<<": "<<hits.GetEntries()<<" charged CPV hits."));

}


//  void IlcPVBARIhepAnalyze::ChargedHitsCPV(TClonesArray* hits, Int_t ievent, Int_t iModule)
//  {
//    // Cumulative list of charged CPV hits in event ievent 
//    // in PVBAR module iModule.

//    HitsCPV(hits,ievent,iModule);
//    TIter next(hits);

//    while(IlcPVBARCPVHit* cpvhit = (IlcPVBARCPVHit*)next())
//      {
...
//    Info(Form(" PVBAR module "<<iModule<<": "<<hits->GetEntries()<<" charged CPV hits."));
//  }

Bool_t IlcPVBARIhepAnalyze::IsCharged(Int_t pdgCode)
{
  // For HIJING

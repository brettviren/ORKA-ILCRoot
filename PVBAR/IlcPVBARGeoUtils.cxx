...
/* $Id: IlcPVBARGeometry.cxx 25590 2008-05-06 07:09:11Z prsnko $ */

//_________________________________________________________________________
// Geometry class  for PVBAR 
...
ClassImp(IlcPVBARGeoUtils)

//____________________________________________________________________________
IlcPVBARGeoUtils::IlcPVBARGeoUtils():
...
//____________________________________________________________________________
IlcPVBARGeoUtils::IlcPVBARGeoUtils(const IlcPVBARGeoUtils & rhs)
  : TNamed(rhs),
  fGeometryEMCA(0x0),fGeometryCPV(0x0),fGeometrySUPP(0x0),
  fNModules(0),fNCristalsInModule(0),fNPhi(0),fNZ(0),
  fNumberOfCPVPadsPhi(0),fNumberOfCPVPadsZ(0),
...
//____________________________________________________________________________
IlcPVBARGeoUtils::IlcPVBARGeoUtils(const Text_t* name, const Text_t* title) 
    : TNamed(name, title),
  fGeometryEMCA(0x0),fGeometryCPV(0x0),fGeometrySUPP(0x0),
  fNModules(0),fNCristalsInModule(0),fNPhi(0),fNZ(0),
  fNumberOfCPVPadsPhi(0),fNumberOfCPVPadsZ(0),
...
{ 
  ...
  fGeometryEMCA = new IlcPVBAREMCAGeometry() ;
  fGeometryCPV  = new IlcPVBARCPVGeometry() ;
  fGeometrySUPP = new IlcPVBARSupportGeometry() ;
...
//____________________________________________________________________________
IlcPVBARGeoUtils & IlcPVBARGeoUtils::operator = (const IlcPVBARGeoUtils  & /*rvalue*/) { 
...
//____________________________________________________________________________
IlcPVBARGeoUtils::~IlcPVBARGeoUtils(void)
{
  // dtor
  if(fGeometryEMCA){
  ...
  for(Int_t mod=0; mod<5; mod++){
    ...
    delete fPVBARMatrix[mod];
  }
}
//____________________________________________________________________________
Bool_t IlcPVBARGeoUtils::AbsToRelNumbering(Int_t absId, Int_t * relid) const
{
  // Converts the absolute numbering into the following array
  //  relid[0] = PVBAR Module number 1:fNModules 
  //  relid[1] = 0 if PbW04
  //           = -1 if CPV
  //  relid[2] = Row number inside a PVBAR module
  //  relid[3] = Column number inside a PVBAR module

  Float_t id = absId ;

  Int_t PVBARmodulenumber = (Int_t)TMath:: Ceil( id / fNCristalsInModule ) ; 
  
  if ( PVBARmodulenumber >  fNModules ) { // it is a CPV pad
    ...
    relid[0] = PVBARmodulenumber ;
    relid[1] = 0 ;
    id -= ( PVBARmodulenumber - 1 ) *  fNPhi * fNZ ; 
...
//____________________________________________________________________________
Bool_t IlcPVBARGeoUtils::RelToAbsNumbering(const Int_t * relid, Int_t &  absId) const
{
  // Converts the relative numbering into the absolute numbering
  // EMCA crystals:
  //  absId = from 1 to fNModules * fNPhi * fNZ
  // CPV pad:
  //  absId = from N(total PVBAR crystals) + 1
  //          to NCPVModules * fNumberOfCPVPadsPhi * fNumberOfCPVPadsZ

  if ( relid[1] ==  0 ) {                            // it is a PVBAR crystal
    absId =
      ( relid[0] - 1 ) * fNPhi * fNZ         // the offset of PVBAR modules
    ...
    absId =    fNPhi * fNZ *  fNModules         // the offset to separate EMCA crystals from CPV pads
      + ( relid[0] - 1 ) * fNumberOfCPVPadsPhi * fNumberOfCPVPadsZ   // the pads offset of PVBAR modules 
...
//____________________________________________________________________________
void IlcPVBARGeoUtils::RelPosInModule(const Int_t * relid, Float_t & x, Float_t & z) const 
{
  // Converts the relative numbering into the local PVBAR-module (x, z) coordinates

  if(relid[1]==0){ //this is PVBAR
    ...
    //Return to PVBAR local system  
    Double_t posL2[3]={posC[0],posC[1],posC[2]};
    const TGeoHMatrix *mPVBAR2 = GetMatrixForModule(mod) ;
    mPVBAR2->MasterToLocal(posC,posL2);
    x=posL2[0] ;
    z=-posL2[2];
    return ;
  }
    ...
    pos[2] = - ( fNumberOfCPVPadsZ  /2. - column - 0.5 ) * fPadSizeZ  ; // of center of PVBAR module
    ...
    const TGeoHMatrix *mPVBAR = GetMatrixForPVBAR(relid[0]) ;
    mPVBAR->MasterToLocal(posC,posL);
    x=posL[0] ;
    z=posL[1];
    return ;
...
//____________________________________________________________________________
void IlcPVBARGeoUtils::RelPosToAbsId(Int_t module, Double_t x, Double_t z, Int_t & absId) const
{
...
/*
  ...
  Double_t posG[3] ;
  char path[100] ;
  sprintf(path,"/ILCM_1/PVBAR_%d/PEMC_1/PCOL_1/PTIO_1/PCOR_1/PAGA_1/PTII_1",module) ;
  ...
  if (mPVBAR){
     mPVBAR->LocalToMaster(posL,posG);
  }
  else{
...
//____________________________________________________________________________
void IlcPVBARGeoUtils::RelPosToRelId(Int_t module, Double_t x, Double_t z, Int_t * relId) const
{
...
//____________________________________________________________________________
void IlcPVBARGeoUtils::RelPosInIlc(Int_t id, TVector3 & pos ) const
{
...
//____________________________________________________________________________
void IlcPVBARGeoUtils::Local2Global(Int_t mod, Float_t x, Float_t z,
				   TVector3& globalPosition) const 
{
  Double_t posL[3]={x,-fCrystalShift,-z} ; //Only for EMC!!!
  Double_t posG[3] ;
  const TGeoHMatrix *mPVBAR = GetMatrixForModule(mod) ;
  mPVBAR->LocalToMaster(posL,posG);
  globalPosition.SetXYZ(posG[0],posG[1],posG[2]) ;
}
//____________________________________________________________________________
void IlcPVBARGeoUtils::Global2Local(TVector3& localPosition,
				   ...
				   Int_t module) const
{
  ...
  Double_t posL[3]={0.,0.,0.} ;
  const TGeoHMatrix *mPVBAR = GetMatrixForModule(module) ;
  if(mPVBAR){
    mPVBAR->MasterToLocal(posG,posL);
...
//____________________________________________________________________________
Bool_t IlcPVBARGeoUtils::GlobalPos2RelId(TVector3 & global, Int_t * relId){
  //Converts position in global ILC coordinates to relId 
  //returns false if x,z coordinates are beyond PVBAR
  //distande to PVBAR surface is NOT calculated 
  TVector3 loc ;
...
//____________________________________________________________________________
Bool_t IlcPVBARGeoUtils::ImpactOnEmc(const TParticle * particle,
       Int_t & moduleNumber, Double_t & z, Double_t & x) const
{
  // Tells if a particle enters PVBAR and evaluates hit position
  Double_t vtx[3]={particle->Vx(),particle->Vy(),particle->Vz()} ;
  return ImpactOnEmc(vtx,particle->Theta(),particle->Phi(),moduleNumber,z,x);
}
 
//____________________________________________________________________________
Bool_t IlcPVBARGeoUtils::ImpactOnEmc(const Double_t * vtx, Double_t theta, Double_t phi, 
                                  Int_t & moduleNumber, Double_t & z, Double_t & x) const
{
  // calculates the impact coordinates on PVBAR of a neutral particle  
...
//____________________________________________________________________________
Bool_t IlcPVBARGeoUtils::ImpactOnEmc(const Double_t * vtx, const TVector3 &p,
                                  Int_t & moduleNumber, Double_t & z, Double_t & x) const
{
  // calculates the impact coordinates on PVBAR of a neutral particle  
...
//____________________________________________________________________________
void IlcPVBARGeoUtils::GetIncidentVector(const TVector3 &vtx, Int_t module, Float_t x,Float_t z, TVector3 &vInc) const {
  //Calculates vector pointing from vertex to current poisition in module local frame
  //Note that PVBAR local system and ILC global have opposite z directions

  Global2Local(vInc,vtx,module) ; 
  vInc.SetXYZ(vInc.X()+x,vInc.Y(),vInc.Z()+z) ;
}
//____________________________________________________________________________
const TGeoHMatrix * IlcPVBARGeoUtils::GetMatrixForModule(Int_t mod)const {
  //Provides shift-rotation matrix for module mod

  //If GeoManager exists, take matrixes from it
  if(gGeoManager){
    char path[255] ;
    snprintf(path,255,"/ILCM_1/PVBAR_%d/PEMC_1/PCOL_1/PTIO_1/PCOR_1/PAGA_1/PTII_1",mod) ;
    //    sprintf(path,"/ILCM_1/PVBAR_%d",relid[0]) ;
    if (!gGeoManager->cd(path)){
  ...
  if(fEMCMatrix[mod-1]){
    ...
    IlcWarning("Can not find PVBAR misalignment matrixes\n") ;
    IlcWarning("Either import TGeoManager from geometry.root or \n");
    IlcWarning("read stored matrixes from IlcESD Header: \n") ;
    IlcWarning("IlcPVBARGeoUtils::SetMisalMatrixes(header->GetPVBARMisalMatrix()) \n") ; 
    return 0 ;
  }
  return 0 ;
}
//____________________________________________________________________________
const TGeoHMatrix * IlcPVBARGeoUtils::GetMatrixForStrip(Int_t mod, Int_t strip)const {
  //Provides shift-rotation matrix for strip unit of the module mod

  //If GeoManager exists, take matrixes from it
  if(gGeoManager){
    char path[255] ;
    snprintf(path,255,"/ILCM_1/PVBAR_%d/PEMC_1/PCOL_1/PTIO_1/PCOR_1/PAGA_1/PTII_1/PSTR_%d",mod,strip) ;
    if (!gGeoManager->cd(path)){
      IlcWarning(Form("Geo manager can not find path %s \n",path));
      return 0 ;
    }
...
//____________________________________________________________________________
const TGeoHMatrix * IlcPVBARGeoUtils::GetMatrixForCPV(Int_t mod)const {
  ...
  if(gGeoManager){ 
    char path[255] ;
    //now apply possible shifts and rotations
    snprintf(path,255,"/ILCM_1/PVBAR_%d/PCPV_1",mod) ;
    if (!gGeoManager->cd(path)){
      IlcWarning(Form("Geo manager can not find path %s \n",path));
      return 0 ;
    }
...
//____________________________________________________________________________
const TGeoHMatrix * IlcPVBARGeoUtils::GetMatrixForPVBAR(Int_t mod)const {
  //Provides shift-rotation matrix for PVBAR (EMC+CPV) 

  //If GeoManager exists, take matrixes from it
  if(gGeoManager){

    char path[255] ;
    snprintf(path,255,"/ILCM_1/PVBAR_%d",mod) ;
  ...
  if(fPVBARMatrix[mod-1]){
    return fPVBARMatrix[mod-1] ;
  }
  else{
    IlcWarning("Can not find PVBAR misalignment matrixes\n") ;
    IlcWarning("Either import TGeoManager from geometry.root or \n");
...
//____________________________________________________________________________
void IlcPVBARGeoUtils::SetMisalMatrix(const TGeoHMatrix * m, Int_t mod){
  //Fills pointers to geo matrixes
 
  if(fPVBARMatrix[mod]){ //have been set already. Can not be changed any more
    return ;
  }
  if(m==NULL) //Matrix for non-existing modules? Remain zero, no need to re-set
    return ;
  fPVBARMatrix[mod]= new TGeoHMatrix(*m) ;
  ...
  fEMCMatrix[mod]->MultiplyLeft(fPVBARMatrix[mod]) ;
  fPVBARMatrix[mod]->LocalToMaster(locTII,globTII) ;
  ...
  for(Int_t irow = 0; irow < fGeometryEMCA->GetNStripX(); irow ++){
	...
	fStripMatrix[mod][istrip] = new((*fMisalArray)[nr])TGeoHMatrix(*(fEMCMatrix[mod])) ; //Use same rotation as PVBAR module
      nr++ ;
  ...
  fCPVMatrix[mod]->MultiplyLeft(fPVBARMatrix[mod]) ;
  fCPVMatrix[mod]->ReflectY(kFALSE) ;
  fPVBARMatrix[mod]->LocalToMaster(locCPV,globCPV) ;

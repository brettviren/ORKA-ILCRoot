...
/* $Id: IlcPVBARRecoParam.cxx 53524 2011-12-11 21:34:35Z policheh $ */

// Base class for the PVBAR reconstruction parameters.
...
ClassImp(IlcPVBARRecoParam)

TObjArray* IlcPVBARRecoParam::fgkMaps =0; //ALTRO mappings
TObject*   IlcPVBARRecoParam::fgkTrigParams =0; // Trigger parameters

//-----------------------------------------------------------------------------
IlcPVBARRecoParam::IlcPVBARRecoParam() :
...
//-----------------------------------------------------------------------------
IlcPVBARRecoParam::IlcPVBARRecoParam(const IlcPVBARRecoParam& ):
  IlcDetectorRecoParam(),
  fNonLinearityParams(3),
  fEMCClusteringThreshold(0.2),
  fEMCLocMaxCut(0.03),
...
//-----------------------------------------------------------------------------
IlcPVBARRecoParam& IlcPVBARRecoParam::operator = (const IlcPVBARRecoParam& recoParam)
{
  //Assignment operator.
...
//-----------------------------------------------------------------------------
void IlcPVBARRecoParam::Print(Option_t * /*option*/) const
{
  IlcDebug(2,Form("PVBAR reconstruction parameters:\n"
...
//-----------------------------------------------------------------------------
IlcPVBARRecoParam* IlcPVBARRecoParam::GetDefaultParameters()
{
  //Default parameters for the reconstruction

  IlcPVBARRecoParam* params = new IlcPVBARRecoParam();
  return params;
}

//-----------------------------------------------------------------------------
const TObjArray* IlcPVBARRecoParam::GetMappings()
{
  ...
  //  root [2] TObjArray* maps = IlcPVBARRecoParam::GetMappings();
  //  root [3] maps->Print();
  
  if(fgkMaps) return fgkMaps;
  
  IlcCDBEntry* entry = IlcCDBManager::Instance()->Get("PVBAR/Calib/Mapping");
  if(entry)
...
//-----------------------------------------------------------------------------
void IlcPVBARRecoParam::SetNonlinearityCorrectionVersion(const char * ver){
...
//-----------------------------------------------------------------------------
const TObject* IlcPVBARRecoParam::GetTriggerParameters()
{
  ...
  //  root [2] TObject* parameters = IlcPVBARRecoParam::GetTriggerParameters();
  //  root [3] parameters->Print();
  
  if(fgkTrigParams) return fgkTrigParams;
  
  IlcCDBEntry* entry = IlcCDBManager::Instance()->Get("PVBAR/Trigger/Parameters");
  if(entry)

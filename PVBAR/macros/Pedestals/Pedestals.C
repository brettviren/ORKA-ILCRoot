...
void Pedestals(const TString rawFile=0, const char *selectTrigger="CPHI")
{
  ...
  reader = IlcRawReader::Create(rawFile);
  reader->Reset();

  TStopwatch timer;
  timer.Start();

  IlcCaloRawStreamV3 *stream = new IlcCaloRawStreamV3(reader,"PVBAR");
  ...
  while (reader->NextEvent()) {
    ...
    IlcInfoGeneral("",Form("Reading event %d of type %d, time %d, trig.class \"%s\"",
	...
	if (module<0 || module>=modMax  || 
	  ...
	  IlcInfoGeneral("",Form("Wrong cell ID (m,x,z)=(%d,%d,%d)",module,cellX,cellZ));
	  break;
	}
...
TString GetTriggerClass(ULong64_t triggerMask)
{
  ...
  IlcCentralTrigger aCTP;
  TString configstr("");
  TString trclasses;
  if (!aCTP.LoadConfiguration(configstr)) { // Load CTP config from OCDB
    IlcInfoGeneral("","No trigger configuration found in OCDB! The trigger configuration information will not be used!");
    return trclasses;
  }

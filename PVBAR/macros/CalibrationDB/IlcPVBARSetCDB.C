/* $Id: IlcPVBARSetCDB.C 30923 2009-02-09 15:47:08Z hristov $ */
...
void IlcPVBARSetCDB()
{
  TControlBar *menu = new TControlBar("vertical","PVBAR CDB");
  menu->AddButton("Help to run PVBAR CDB","Help()",
		  "Explains how to use PVBAR CDB menus");
  ...
  menu->AddButton("Exit","gApplication->Terminate(0)","Quit ilcroot session");
  menu->Show();
}
...
void Help()
{
  TString string="\nSet calibration parameters and write them into ILC OCDB:";
...
void SetCC(Int_t flag=0)
{
  ...
  if      (flag == 0) {
    ...
    firstRun  =  0;
    lastRun   =  999999;
    objFormat = "PVBAR ideal pedestals and ADC gain factors (5x64x56)";
    cdb = new IlcPVBARCalibData();
    cdb->CreateNew();
  }
    ...
    firstRun  =  0;
    lastRun   =  999999;
    objFormat = "PVBAR fully decalibrated calibration coefficients (5x64x56)";
    ...
    firstRun  =  0;
    lastRun   =  999999;
    objFormat = "PVBAR residual calibration coefficients (5x64x56)";
  ...
  if(gSystem->Getenv("STORAGE")){
    ...
    IlcCDBManager::Instance()->SetSpecificStorage("PVBAR/*",DBFolder.Data());
  }
...
void GetCC(Int_t flag=0)
{
  ...
  IlcPVBARCalibData* clb  = new IlcPVBARCalibData(runNumber);

  TH2::AddDirectory(kFALSE);

  TH2F* hGain[5];

  TCanvas *cGain = new TCanvas("cGain","PVBAR EMC Gain factors",10,10,700,500);
  cGain->Divide(3,2);

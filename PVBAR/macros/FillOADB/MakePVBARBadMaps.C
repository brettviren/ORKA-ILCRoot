void MakePVBARBadMaps(){
  //Creates OADB entry with PVBAR bad maps for different periods
  //You probably will need alien connection
  
  
  //Init Bad channels map
  IlcOADBContainer badmapContainer(Form("PVBARBadMap"));
  ...
  TFile * fLHC10b = TFile::Open("alien:///ilc/cern.ch/user/p/prsnko/BadMaps/BadMap_LHC10b.root") ;
  if(fLHC10b->IsOpen()){
  ...
  TFile * fLHC10h1 = TFile::Open("alien:///ilc/cern.ch/user/p/prsnko/BadMaps/BadMap_LHC10h_period1.root") ;
  if(fLHC10h1->IsOpen()){
    for(Int_t mod=1;mod<4; mod++){
      sprintf(key,"PVBAR_BadMap_mod%d",mod) ;
  ...
  TFile * fLHC10h234 = TFile::Open("alien:///ilc/cern.ch/user/p/prsnko/BadMaps/BadMap_LHC10h_period234.root") ;

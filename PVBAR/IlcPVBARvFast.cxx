...
/* $Id: IlcPVBARvFast.cxx 32955 2009-06-16 12:54:07Z fca $ */
...
ClassImp(IlcPVBARvFast)

IlcPVBARvFast::IlcPVBARvFast() :
  fBigBoxX(0.),
  fBigBoxY(0.),
  fBigBoxZ(0.),
  fFastRecParticles(0),
...
IlcPVBARvFast::IlcPVBARvFast(const char *name, const char *title):
  IlcPVBAR(name,title),
  fBigBoxX(0.),
  fBigBoxY(0.),
  fBigBoxZ(0.),
  fFastRecParticles(new IlcPVBARFastRecParticle::FastRecParticlesList("IlcPVBARFastRecParticle", 100)),
  fNRecParticles(0),
  fRan(0),
  fResPara1(0.030), // GeV
  fResPara2(0.00003),
...
//____________________________________________________________________________
void IlcPVBARvFast::AddRecParticle(const IlcPVBARFastRecParticle & rp)
{  
  // Add a virtually reconstructed particle to the list 

  new( (*fFastRecParticles)[fNRecParticles] ) IlcPVBARFastRecParticle(rp) ;
  fNRecParticles++ ; 
}

//____________________________________________________________________________
void IlcPVBARvFast::CreateGeometry()
{
  // Create the geometry for GEANT
  
  IlcPVBARvFast *PVBARtmp = (IlcPVBARvFast*)gIlc->GetModule("PVBAR") ;
...
Float_t IlcPVBARvFast::GetBigBox(Int_t index) const
{
  // Get the X, Y or Z dimension of the box describing a PVBAR module
...
void IlcPVBARvFast::MakeBranch(Option_t* opt)
{  
...
Double_t IlcPVBARvFast::MakeEnergy(Double_t energy)
{  
...
TVector3 IlcPVBARvFast::MakePosition(Double_t energy, TVector3 pos, Double_t theta, Double_t phi)
{
...
//____________________________________________________________________________
Int_t IlcPVBARvFast::MakeType(IlcPVBARFastRecParticle & rp )
{
  // Generate a particle type using the performance of the EMC+PPSD setup

  Int_t rv =   IlcPVBARFastRecParticle::kUNDEFINED ;
  ...
  case 22:    // it's a photon              // NB: ALL VALUES SHOLD BE CHECKED !!!!
    ran = fRan.Rndm() ; 
    if( ran <= 0.9498 )
      rv =  IlcPVBARFastRecParticle::kNEUTRALHAFAST ; 
    else
      rv =  IlcPVBARFastRecParticle::kNEUTRALEMFAST ;     
    break ; 

  case 2112:  // it's a neutron
    ran = fRan.Rndm() ; 
    if ( ran <= 0.9998 )
      rv =  IlcPVBARFastRecParticle::kNEUTRALHASLOW ; 
    else 
      rv = IlcPVBARFastRecParticle::kNEUTRALEMSLOW ; 
    break ; 
  ...
  case 11:    // it's a electron
    ran = fRan.Rndm() ; 
    if ( ran <= 0.9996 )
      rv =  IlcPVBARFastRecParticle::kCHARGEDEMFAST ; 
    else 
      rv =  IlcPVBARFastRecParticle::kCHARGEDHAFAST ; 
    break; 
...
//___________________________________________________________________________
void IlcPVBARvFast::ResetPoints()
{
...
//___________________________________________________________________________
void IlcPVBARvFast::ResetFastRecParticles()
{
...
//___________________________________________________________________________
void IlcPVBARvFast::SetBigBox(Int_t index, Float_t value)
{
  // Set the size of the Box describing a PVBAR module
...
//____________________________________________________________________________
Double_t IlcPVBARvFast::SigmaE(Double_t energy)
{
...
//____________________________________________________________________________
Double_t IlcPVBARvFast::SigmaP(Double_t energy, Double_t incidence)
{
...
//____________________________________________________________________________
void IlcPVBARvFast::StepManager(void)
{
  // Only verifies if the particle reaches PVBAR and stops the tracking 
  ...
  if(energy > mass){
    ...
    IlcPVBARFastRecParticle rp(*part) ;

    // Adds the response of PVBAR to the particle
  ...
  // stop the track as soon PVBAR is reached

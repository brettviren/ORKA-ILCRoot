 ...
 **************************************************************************/
/* $Id: IlcPVBARRecParticle.cxx 53186 2011-11-25 14:28:01Z kharlov $ */
...
IlcPVBARRecParticle::IlcPVBARRecParticle(): 
  fPVBARTrackSegment(0),
  fDebug(kFALSE),
  fPos()
{
  // ctor
...
//____________________________________________________________________________
IlcPVBARRecParticle::IlcPVBARRecParticle(const IlcPVBARRecParticle & rp):
  IlcPVBARFastRecParticle(rp),
  fPVBARTrackSegment(rp.fPVBARTrackSegment),
  fDebug(kFALSE),
  fPos()
{
  // copy ctor
...
//____________________________________________________________________________
IlcPVBARRecParticle & IlcPVBARRecParticle::operator = (const IlcPVBARRecParticle &)
{
...
//____________________________________________________________________________
Int_t IlcPVBARRecParticle::GetNPrimaries() const  
{ 
  return -1;
}

//____________________________________________________________________________
Int_t IlcPVBARRecParticle::GetNPrimariesToRecParticles() const  
{ 
  ...
  Int_t emcRPindex = static_cast<IlcPVBARTrackSegment*>(PVBARLoader->TrackSegments()->At(GetPVBARTSIndex()))->GetEmcIndex();
  static_cast<IlcPVBAREmcRecPoint*>(PVBARLoader->EmcRecPoints()->At(emcRPindex))->GetPrimaries(rv) ; 
  return rv ; 
}

//____________________________________________________________________________
const TParticle * IlcPVBARRecParticle::GetPrimary() const  
{
...
//DP to be fixed: Why do we use this method here??? M.B. use RecPoint???
  ...
  if(module != 0){
   ...
   for(Int_t i = 0 ; i < sdigits->GetEntriesFast() ; i++){
     sdig = static_cast<IlcPVBARDigit *>(sdigits->At(i)) ;
     if((sdig->GetId() == AbsId)){
...
//____________________________________________________________________________
Int_t IlcPVBARRecParticle::GetPrimaryIndex() const  
{
  ...
  const IlcPVBARTrackSegment *ts          = PVBARLoader->TrackSegment(fPVBARTrackSegment);

  // Get EmcRecPoint corresponding to this TrackSegment
  Int_t emcRecPointIndex = ts->GetEmcIndex();

  const IlcPVBAREmcRecPoint  *emcRecPoint = PVBARLoader->EmcRecPoint(emcRecPointIndex);
  ...
  for (Int_t iDigit=0; iDigit<nDigits; iDigit++) {
    const IlcPVBARDigit * digit = PVBARLoader->Digit(digitList[iDigit]);
  ...
  if (bestDigitIndex>-1) {
    const IlcPVBARDigit * digit = PVBARLoader->Digit(digitList[bestDigitIndex]);
  ...
  Int_t    id       = (PVBARLoader->Digit(digitList[bestDigitIndex]))->GetId();

  for (Int_t iHit=0; iHit<nHits; iHit++) {
    const IlcPVBARHit * hit = PVBARLoader->Hit(iHit);
    if(hit->GetId() == id){
...
//____________________________________________________________________________
const TParticle * IlcPVBARRecParticle::GetPrimary(Int_t index) const  
{
  // Get one of the primary particles at the origine of the RecParticle
  if ( index > GetNPrimariesToRecParticles() ) { 
    if (fDebug) 
      Warning("GetPrimary", "IlcPVBARRecParticle::GetPrimary -> %d is larger that the number of primaries %d", 
	      index, GetNPrimaries()) ;
    return 0 ; 
  } 
  else { 
    ...
    Int_t emcRPindex = static_cast<IlcPVBARTrackSegment*>(PVBARLoader->TrackSegments()->At(GetPVBARTSIndex()))->GetEmcIndex();
    Int_t primaryindex = static_cast<IlcPVBAREmcRecPoint*>(PVBARLoader->EmcRecPoints()->At(emcRPindex))->GetPrimaries(dummy)[index] ; 
...
//____________________________________________________________________________
void IlcPVBARRecParticle::SetPID(Int_t type, Float_t weight)
{

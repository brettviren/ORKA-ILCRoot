...
/* $Id: IlcPVBARRawDigiProducer.cxx 53421 2011-12-05 15:19:38Z prsnko $ */

//This class produces PVBAR digits of one event
//using IlcPVBARRawFitter. 
//
//   For example:
//   TClonesArray *digits = new TClonesArray("IlcPVBARDigit",100);
//   IlcRawReader* rawReader = new IlcRawReaderDate("2006run2211.raw");
//   IlcPVBARRawDecoder dc(rawReader);
//   while (rawReader->NextEvent()) {
//     IlcPVBARRawDigiProducer producer;
...
ClassImp(IlcPVBARRawDigiProducer)

IlcPVBARCalibData * IlcPVBARRawDigiProducer::fgCalibData  = 0 ; 

//--------------------------------------------------------------------------------------
IlcPVBARRawDigiProducer::IlcPVBARRawDigiProducer():
  TObject(),
  fEmcMinE(0.),
  fCpvMinE(0.),
  fSampleQualityCut(1.),
  ...
  fADCValuesHG(0)
{
  ...
  fGeom=IlcPVBARGeometry::GetInstance() ;
  if(!fGeom) fGeom = IlcPVBARGeometry::GetInstance("ORKA");

  fEmcCrystals=fGeom->GetNCristalsInModule()*fGeom->GetNModules() ;
  fPulseGenerator = new IlcPVBARPulseGenerator();
  GetCalibrationParameters() ; 

}
//--------------------------------------------------------------------------------------
IlcPVBARRawDigiProducer::IlcPVBARRawDigiProducer(IlcRawReader *rawReader,
  ...
  fADCValuesHG(0)
{
  ...
  fRawStream = new IlcCaloRawStreamV3(rawReader,"PVBAR",mapping);

}
//--------------------------------------------------------------------------------------
IlcPVBARRawDigiProducer::IlcPVBARRawDigiProducer(const IlcPVBARRawDigiProducer &dp):
  TObject(),
  fEmcMinE(0.),
  fCpvMinE(0.),
  fSampleQualityCut(1.),
...
//--------------------------------------------------------------------------------------
IlcPVBARRawDigiProducer& IlcPVBARRawDigiProducer::operator= (const IlcPVBARRawDigiProducer &dp)
{
  // Assign operator
...
//--------------------------------------------------------------------------------------
void IlcPVBARRawDigiProducer::MakeDigits(TClonesArray *digits, IlcPVBARRawFitterv0* fitter) 
{
  // Create a temporary array of LG digits and then make digits from raw data

  TClonesArray *tmpLG = new TClonesArray("IlcPVBARDigit",10000) ;
  MakeDigits(digits, tmpLG, fitter);
  tmpLG->Delete();
  delete tmpLG;
}
//--------------------------------------------------------------------------------------
void IlcPVBARRawDigiProducer::MakeDigits(TClonesArray *digits, TClonesArray *tmpDigLG, IlcPVBARRawFitterv0* fitter) 
{
  //Makes the job.
...
//      time = CalibrateT(time,relId,!caloFlag) ;
    ...
    for(Int_t iDig=0 ; iDig < digits->GetEntriesFast() ; iDig++) { 
      IlcPVBARDigit * digHG = dynamic_cast<IlcPVBARDigit*>(digits->At(iDig)) ;
      if (!digHG) continue;
      IlcPVBARDigit * digLG = dynamic_cast<IlcPVBARDigit*>(tmpDigLG->At(iLG)) ;
      while(digLG && iLG<nLG1 && digHG->GetId()> digLG->GetId()){
	iLG++ ;
	digLG = dynamic_cast<IlcPVBARDigit*>(tmpDigLG->At(iLG)) ;
      }
      absId=digHG->GetId() ;
...
Double_t IlcPVBARRawDigiProducer::CalibrateE(Double_t amp, Int_t* relId, Bool_t isLowGain)
{
...
//____________________________________________________________________________
Double_t IlcPVBARRawDigiProducer::CalibrateT(Double_t time, Int_t * relId, Bool_t /* isLowGain */)
{
  //Calibrate time
  if(fgCalibData){
    Int_t   module = relId[0];
...
//____________________________________________________________________________
void IlcPVBARRawDigiProducer::CleanDigits(TClonesArray * digits)
{
  ...
  for(Int_t i=0; i<digits->GetEntriesFast(); i++){
    IlcPVBARDigit * digit = static_cast<IlcPVBARDigit*>(digits->At(i)) ;
    if(!digit)
      continue  ;
...
//____________________________________________________________________________
Bool_t IlcPVBARRawDigiProducer::IsInEMC(IlcPVBARDigit * digit) const
{
...
//____________________________________________________________________________
Bool_t IlcPVBARRawDigiProducer::IsInCPV(IlcPVBARDigit * digit) const
{
  // Tells if (true) or not (false) the digit is in a PVBAR-CPV module
  return digit->GetId() > fEmcCrystals ;
}
//____________________________________________________________________________
void IlcPVBARRawDigiProducer::GetCalibrationParameters() 
{
  // Set calibration parameters:
  // if calibration database exists, they are read from database,
  // otherwise, reconstruction stops in the constructor of IlcPVBARCalibData
  //

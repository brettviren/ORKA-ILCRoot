...
// 
// Typical use case:
//     IlcPVBARRawFitter *fitter=new IlcPVBARRawFitter();
...
ClassImp(IlcPVBARRawFitterv1)

//-----------------------------------------------------------------------------
IlcPVBARRawFitterv1::IlcPVBARRawFitterv1():
  IlcPVBARRawFitterv0(),
...
//-----------------------------------------------------------------------------
IlcPVBARRawFitterv1::IlcPVBARRawFitterv1(const IlcPVBARRawFitterv1 &PVBARFitter ):
  IlcPVBARRawFitterv0(PVBARFitter),
  fSampleParamsLow(0x0),
  fSampleParamsHigh(0x0),
  fToFit(0x0)
{
  //Copy constructor.
  fToFit = new TList() ;
  fSampleParamsLow =new TArrayD(*(PVBARFitter.fSampleParamsLow)) ;
  fSampleParamsHigh=new TArrayD(*(PVBARFitter.fSampleParamsHigh)) ;
}

//-----------------------------------------------------------------------------
IlcPVBARRawFitterv1& IlcPVBARRawFitterv1::operator = (const IlcPVBARRawFitterv1 &PVBARFitter)
{
  //Assignment operator.
  if(this != &PVBARFitter) {
    fToFit = new TList() ;
    if(fSampleParamsLow){
      fSampleParamsLow = PVBARFitter.fSampleParamsLow ;
      fSampleParamsHigh= PVBARFitter.fSampleParamsHigh ;
    }
    else{
      fSampleParamsLow =new TArrayD(*(PVBARFitter.fSampleParamsLow)) ; 
      fSampleParamsHigh=new TArrayD(*(PVBARFitter.fSampleParamsHigh)) ;
...
//-----------------------------------------------------------------------------
Bool_t IlcPVBARRawFitterv1::Eval(const UShort_t *signal, Int_t sigStart, Int_t sigLength)
{
  ...
  gMinuit->SetFCN(IlcPVBARRawFitterv1::UnfoldingChiSquare) ;  
...
//-----------------------------------------------------------------------------
Double_t IlcPVBARRawFitterv1::Gamma2(Double_t dt,Double_t en,Double_t b,TArrayD * params){  //Function for fitting samples
  //parameters:
  //dt-time after start
  //en-amplutude
  //function parameters
...
void IlcPVBARRawFitterv1::UnfoldingChiSquare(Int_t & /*nPar*/, Double_t * Grad, Double_t & fret, Double_t * x, Int_t iflag)
{

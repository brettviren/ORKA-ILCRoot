#include "IlcPVBARRcuDA1.h"
#include "TString.h"

ClassImp(IlcPVBARRcuDA1)

//----------------------------------------------------------------
IlcPVBARRcuDA1::IlcPVBARRcuDA1(Int_t module, Int_t rcu) : TNamed(),
...

{
  ...
  // Root file name: PVBAR_ModuleX_RCUY_Calib.root, where X - module number,
  // Y - RCU number. If no RCU specified (rcu<0), file name is simply
  // PVBAR_ModuleX_Calib.root.
  
  char name[128]; TString sname;

  if(rcu<0) { 
    sname="PVBAR_Module%d_Calib";
    snprintf(name,sname.Length(),sname.Data(),fMod);
  }
  else {
    sname="PVBAR_Module%d_RCU%d_Calib";
...
//-------------------------------------------------------------------
IlcPVBARRcuDA1::IlcPVBARRcuDA1(Int_t module, Int_t rcu, TObjArray* oldTimeEnergy) :
...
//-------------------------------------------------------------------
void IlcPVBARRcuDA1::FillHistograms(Float_t e[64][56][2], Float_t t[64][56][2]) 
{
...
//-------------------------------------------------------------------
void IlcPVBARRcuDA1::UpdateHistoFile()
{
...
//-----------------------------------------------------------------------
void IlcPVBARRcuDA1::SetWriteToFile(Bool_t write)
{
  if(!write) { 
    fWriteToFile = write;
    return;

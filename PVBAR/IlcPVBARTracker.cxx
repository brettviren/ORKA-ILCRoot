 ...
 **************************************************************************/
/* $Id: IlcPVBARTracker.cxx 43671 2010-09-19 18:45:08Z kharlov $ */
...
ClassImp(IlcPVBARTracker)

Bool_t IlcPVBARTracker::fgDebug = kFALSE ;  

//____________________________________________________________________________
IlcPVBARTracker::IlcPVBARTracker(): 
  IlcTracker()
{
  //--------------------------------------------------------------------
  ...
  //--------------------------------------------------------------------
  for (Int_t i=0; i<5; i++) 
      fModules[i]=new TClonesArray("IlcPVBAREmcRecPoint",777);
...
//____________________________________________________________________________
Int_t IlcPVBARTracker::LoadClusters(TTree *cTree) {
  //--------------------------------------------------------------------
  // This function loads the PVBAR clusters
  //--------------------------------------------------------------------

  TObjArray *arr=NULL;
  TBranch *branch=cTree->GetBranch("PVBAREmcRP");
  if (branch==0) {
  ...
  for (Int_t i=0; i<nentr; i++) {
    ...
    while (ncl--) {
      IlcPVBAREmcRecPoint *cl=(IlcPVBAREmcRecPoint*)arr->UncheckedAt(ncl);

      Int_t m=cl->GetPVBARMod();
      if ((m<1)||(m>5)) {
...
//      if (!cl->Misalign()) IlcWarning("Can't misalign this cluster !");
      ...
      new (module[idx]) IlcPVBAREmcRecPoint(*cl); 
...
//____________________________________________________________________________
Int_t IlcPVBARTracker::PropagateBack(IlcESDEvent *esd) {
  //--------------------------------------------------------------------
  // Called by IlcReconstruction 
  // Performs the track matching with the PVBAR modules
  // Makes the PID
  //--------------------------------------------------------------------
...
//     if ((status & IlcESDtrack::kTRDrefit) == 1) continue;
     ...
     //Loop over PVBAR modules
     Double_t dx=0,dz=0;
     Double_t minDistance=999. ;
     Int_t emcIndex=0  ;
     for(Int_t mod=1; mod<=5; mod++){
       if(fModules[mod-1]->GetEntriesFast()==0) //no PVBAR clusters in this module or module does not exist
         continue ;
       
       //Approximate direction to the current PVBAR module
      ...
      //t.CorrectForMaterial(...); // Correct for the TOF material, if needed
         ...
         for(Int_t icl=0;icl<fModules[mod-1]->GetEntriesFast();icl++){
           IlcPVBAREmcRecPoint * clu =static_cast<IlcPVBAREmcRecPoint*>(fModules[mod-1]->At(icl)) ;
...
//____________________________________________________________________________
IlcCluster *IlcPVBARTracker::GetCluster(Int_t index) const {
  //--------------------------------------------------------------------
...
//____________________________________________________________________________
void IlcPVBARTracker::UnloadClusters() {
  //--------------------------------------------------------------------
  // This function unloads the PVBAR clusters
  //--------------------------------------------------------------------

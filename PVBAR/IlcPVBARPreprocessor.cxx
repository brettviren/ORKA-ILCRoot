...
/* $Id: IlcPVBARPreprocessor.cxx 47523 2011-02-17 14:35:05Z policheh $ */

///////////////////////////////////////////////////////////////////////////////
// PVBAR Preprocessor class. It runs by Shuttle at the end of the run,
...
#include "IlcPVBARPreprocessor.h"
#include "IlcLog.h"
...
IlcPVBARPreprocessor::IlcPVBARPreprocessor() :
...
//_______________________________________________________________________________________
IlcPVBARPreprocessor::IlcPVBARPreprocessor(IlcShuttleInterface* shuttle):
...
//_______________________________________________________________________________________
UInt_t IlcPVBARPreprocessor::Process(TMap* /*valueSet*/)
{
...
Bool_t IlcPVBARPreprocessor::ProcessLEDRun()
{
  //Process LED run, update High Gain/Low Gain ratios.

  IlcPVBAREmcCalibData calibData;
...
Float_t IlcPVBARPreprocessor::HG2LG(Int_t mod, Int_t X, Int_t Z, TFile* f)
{
  //Calculates High gain to Low gain ratio 
  //for crystal at the position (X,Z) in the PVBAR module mod.
...
Bool_t IlcPVBARPreprocessor::FindBadChannelsEmc()
{
  ...
  for (Int_t i=0; i<2; i++) {
    ...
    IlcPVBAREmcBadChannelsMap badMap;
...
Bool_t IlcPVBARPreprocessor::DoFindBadChannelsEmc(Int_t system, TList* list, IlcPVBAREmcBadChannelsMap& badMap)
{
  //Creates the bad channels map for PVBAR EMC.
...
Bool_t IlcPVBARPreprocessor::CalibrateEmc()
{
  //Loop over two systems: DAQ and HLT.
  //For each system the same algorithm implemented in DoCalibrateEmc() invokes.

  IlcPVBAREmcCalibData*   lastCalib=0;
  const IlcPVBAREmcBadChannelsMap* badMap=0;
  ...
  for (Int_t i=0; i<2; i++) {
    ...
    if(!entryBCM)
      ...
      badMap = (IlcPVBAREmcBadChannelsMap*)entryBCM->GetObject();
    ...
    if(!entryEmc) 
      ...
      lastCalib = (IlcPVBAREmcCalibData*)entryEmc->GetObject();
...
Bool_t IlcPVBARPreprocessor::StoreReferenceEmc(Int_t system, TList* list)
{
  ...
  Bool_t resultRef = StoreReferenceFile(fileName.Data(),"CalibRefPVBAR.root");
  return resultRef;

}

Bool_t IlcPVBARPreprocessor::StoreReferenceLED(TList* list)
{
  ...
  Bool_t resultRef = StoreReferenceFile(fileName.Data(),"LEDRefPVBAR.root");
  return resultRef;
  
}


Bool_t IlcPVBARPreprocessor::DoCalibrateEmc(Int_t system, TList* list, const IlcPVBAREmcBadChannelsMap* badMap, IlcPVBAREmcCalibData& calibData)
{
  // Return kTRUE if OK.

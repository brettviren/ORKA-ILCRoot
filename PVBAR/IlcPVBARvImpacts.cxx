...
/* $Id: IlcPVBARvImpacts.cxx 49869 2011-05-18 04:49:51Z hristov $ */
...
// Implementation version vImpacts of PVBAR Manager class.
...
ClassImp(IlcPVBARvImpacts)

//____________________________________________________________________________
IlcPVBARvImpacts::IlcPVBARvImpacts():
...
//____________________________________________________________________________
IlcPVBARvImpacts::IlcPVBARvImpacts(const char *name, const char *title):
  ...
  fNPPSDImpacts()
{
  ...
  //     This part inherits from IlcPVBARv1
  //
  // We store impacts :
  //  - fEMCImpacts, fCPVImpacts which are
  //    TList of EMC and CPV modules respectively, each
  //    modules contains TClonesArray of IlcPVBARImpacts

  Int_t nPVBARModules = GetGeometry()->GetNModules();
  Int_t nCPVModules  = GetGeometry()->GetNModules();

  Int_t iPVBARModule;
  TClonesArray * impacts;
  for (iPVBARModule=0; iPVBARModule<nPVBARModules; iPVBARModule++) {
    fEMCImpacts->Add(new TClonesArray("IlcPVBARImpact",200)) ;
    fNEMCImpacts[iPVBARModule] = 0;
    impacts = dynamic_cast<TClonesArray *>(fEMCImpacts->At(iPVBARModule));
  }
  for (iPVBARModule=0; iPVBARModule<nCPVModules; iPVBARModule++) {
    fCPVImpacts->Add(new TClonesArray("IlcPVBARImpact",200)) ;
    fNCPVImpacts[iPVBARModule] = 0;
    impacts = dynamic_cast<TClonesArray *>(fCPVImpacts->At(iPVBARModule));
  }
...
//____________________________________________________________________________
void IlcPVBARvImpacts::AddImpact(const char* det, Int_t shunt, Int_t primary, Int_t track, Int_t module,
			   Int_t pid, TLorentzVector p, Float_t *xyz)
{
  ...
  if (strcmp(det,"EMC ")==0) {
    ...
    IlcFatal(Form("Wrong PVBAR configuration: det=%s",det));

  new((*impacts)[nImpacts]) IlcPVBARImpact(shunt,primary,track,pid,p,xyz) ;

  IlcDebugClass(1,Form("Module %d %s: ",module,det));
  if (IlcLog::GetGlobalDebugLevel()>0)
    (static_cast<IlcPVBARImpact*>((impacts->At(nImpacts))))->Print();
}

//____________________________________________________________________________
void IlcPVBARvImpacts::MakeBranch(Option_t *opt)
{  
  // Create new branch in the current Hits Root Tree containing
  // a list of PVBAR impacts (exact values of track coming to detector)

  IlcDetector::MakeBranch(opt);
  
  Int_t bufferSize = 32000 ;
  Int_t splitlevel = 0 ;
  fLoader->TreeH()->Branch("PVBAREmcImpacts" , "TList", &fEMCImpacts , bufferSize, splitlevel);
  fLoader->TreeH()->Branch("PVBARCpvImpacts" , "TList", &fCPVImpacts , bufferSize, splitlevel);
  
}

//____________________________________________________________________________
void IlcPVBARvImpacts::ResetHits() 
{              

...
link: see comments in the $ILC_ROOT/PVBAR/IlcPVBARRcuDA1.cxx
reference run: /ilc/data/2009/LHC09b_PVBAR/000075883/raw/09000075883017.20.root
run type: PHYSICS
DA type: MON 
...
Output files: PVBAR_Calib_Total.root contains cumulative statistics for a number of runs. 
...
int main(int argc, char **argv) {
  ...
  for(Int_t iMod = 0; iMod < 5; iMod++) {
    ...
    for(Int_t iRCU=0; iRCU<4; iRCU++) {
      ...
      path3 += iRCU;
      path3 += ".data";
      mapping[iMap] = new IlcCaloAltroMapping(path3.Data());
      iMap++;
    }
  }
  ...
  for(;;) {
    ...
    if (eventT==PHYSICS_EVENT) {
      ...
      rawReader = new IlcRawReaderDate((void*)event);
      IlcCaloRawStreamV3 stream(rawReader,"PVBAR",mapping);
	...
	if(dAs[stream.GetModule()])
	  ...
	  dAs[stream.GetModule()] = new IlcPVBARRcuDA1(stream.GetModule(),-1,0);
	  dAs[stream.GetModule()]->FillHistograms(e,t);
	}
...
//     //da1.UpdateHistoFile();
  ...
  sprintf(totfile,"PVBAR_Calib_Total.root");

...
/* $Id: IlcPVBARClusterizerv1.cxx 51445 2011-09-06 18:02:04Z hristov $ */

/* History of cvs commits:
 *
 * $Log: IlcPVBARClusterizerv1.cxx,v $
 ...
 * Bug fix for attempt to use IlcPVBAREmcRecPoint after its deletion
 *
 ...
 * Mode protection against absence of calib.data from IlcPVBARCalibData to IlcPVBARClusterizerv1::GetCalibrationParameters()
 *
 * Revision 1.104  2007/04/27 16:55:53  kharlov
 * Calibration stops if PVBAR CDB objects do not exist
 *
 ...
 * 1. Adding a possibility to read and reconstruct an old rcu formatted raw data. This is controlled by an option of IlcReconstruction and IlcPVBARReconstructor. 2. In case of raw data processing (without gilc.root) create the default IlcPVBARGeometry object. Most likely this should be moved to the CDB
 *
...
//  Results are stored in TreeR#, branches PVBAREmcRP (EMC recPoints),
//  PVBARCpvRP (CPV RecPoints) and IlcPVBARClusterizer (Clusterizer with all 
...
//  standalone mode.
// Use Case:
//  root [0] IlcPVBARClusterizerv1 * cl = new IlcPVBARClusterizerv1(<pointer_to_PVBAR_geometry_onject>)  
...
ClassImp(IlcPVBARClusterizerv1)
  
//____________________________________________________________________________
IlcPVBARClusterizerv1::IlcPVBARClusterizerv1() :
  IlcPVBARClusterizer(),
...
//____________________________________________________________________________
IlcPVBARClusterizerv1::IlcPVBARClusterizerv1(IlcPVBARGeometry *geom) :
  IlcPVBARClusterizer(geom),
  fDefaultInit(0),            fEmcCrystals(0),          fToUnfold(0),
  fWrite(0),                
  fNumberOfEmcClusters(0),    fNumberOfCpvClusters(0),
  fEmcClusteringThreshold(0), fCpvClusteringThreshold(0), 
...
//____________________________________________________________________________
void IlcPVBARClusterizerv1::Digits2Clusters(Option_t *option)
{
  ...
  if(strstr(option,"tim")){
    ...
    IlcInfo(Form("took %f seconds for Clusterizing\n",
		 gBenchmark->GetCpuTime("PVBARClusterizer"))); 
  }
...
//____________________________________________________________________________
Bool_t IlcPVBARClusterizerv1::FindFit(IlcPVBAREmcRecPoint * emcRP, IlcPVBARDigit ** maxAt, Float_t * maxAtEnergy,
...
{ 
  ...
  gMinuit->SetFCN(IlcPVBARClusterizerv1::UnfoldingChiSquare) ;  
  ...
  for(iDigit = 0; iDigit < nDigits; iDigit++){
    ...
    index++ ;   
    if(ierflg != 0){ 
      Warning("FindFit", "PVBAR Unfolding unable to set initial value for fit procedure : x = %f\n", x ) ;
      return kFALSE;
    }
    gMinuit->mnparm(index, "z",  z, 0.1, 0, 0, ierflg) ;
    index++ ;   
    if(ierflg != 0){
       Warning("FindFit", "PVBAR Unfolding unable to set initial value for fit procedure : z =%f\n", z ) ;
      return kFALSE;
    }
    gMinuit->mnparm(index, "Energy",  energy , 0.05*energy, 0., 4.*energy, ierflg) ;
    index++ ;   
    if(ierflg != 0){
      Warning("FindFit", "PVBAR Unfolding unable to set initial value for fit procedure : energy = %f\n", energy ) ;      
      return kFALSE;
    }
  }
  ...
  if(ierflg == 4){  // Minimum not found   
    Warning("FindFit", "PVBAR Unfolding fit not converged, cluster abandoned\n" );      
    return kFALSE ;
  }            
...
//____________________________________________________________________________
void IlcPVBARClusterizerv1::Init()
{
  // Make all memory allocations which can not be done in default constructor.
  // Attach the Clusterizer task to the list of PVBAR tasks
...
//____________________________________________________________________________
void IlcPVBARClusterizerv1::InitParameters()
{

  fNumberOfCpvClusters     = 0 ; 
  fNumberOfEmcClusters     = 0 ; 

  const IlcPVBARRecoParam* recoParam = IlcPVBARReconstructor::GetRecoParam();
...
//____________________________________________________________________________
Int_t IlcPVBARClusterizerv1::AreNeighbours(IlcPVBARDigit * d1, IlcPVBARDigit * d2)const
{
  ...
  if ( (relid1[0] == relid2[0]) && (relid1[1]==relid2[1]) ) { // inside the same PVBAR module 
...
//____________________________________________________________________________
Bool_t IlcPVBARClusterizerv1::CheckTimeGate(Float_t t1, Float_t amp1, Float_t t2, Float_t amp2)const{
...
//____________________________________________________________________________
Bool_t IlcPVBARClusterizerv1::IsInEmc(IlcPVBARDigit * digit) const
{
  // Tells if (true) or not (false) the digit is in a PVBAR-EMC module
...
//____________________________________________________________________________
Bool_t IlcPVBARClusterizerv1::IsInCpv(IlcPVBARDigit * digit) const
{
  // Tells if (true) or not (false) the digit is in a PVBAR-CPV module
...
//____________________________________________________________________________
void IlcPVBARClusterizerv1::WriteRecPoints()
{
  ...
  Float_t emcMinE= IlcPVBARReconstructor::GetRecoParam()->GetEMCMinE(); //Minimal digit energy
  TVector3 fakeVtx(0.,0.,0.) ;
  for(index = 0; index < nEmc; index++){
    IlcPVBAREmcRecPoint * rp =
      static_cast<IlcPVBAREmcRecPoint *>( fEMCRecPoints->At(index) );
    rp->Purify(emcMinE) ;
  ...
  for(index = 0; index < fEMCRecPoints->GetEntries(); index++){
    static_cast<IlcPVBAREmcRecPoint *>( fEMCRecPoints->At(index) )->SetIndexInList(index) ;
  }
  ...
  for(index = 0; index < fCPVRecPoints->GetEntries(); index++){
    IlcPVBARCpvRecPoint * rp = static_cast<IlcPVBARCpvRecPoint *>( fCPVRecPoints->At(index) );
    rp->EvalAll(fDigitsArr) ;
  ...
  for(index = 0; index < fCPVRecPoints->GetEntries(); index++)
    static_cast<IlcPVBARCpvRecPoint *>( fCPVRecPoints->At(index) )->SetIndexInList(index) ;
...
//____________________________________________________________________________
void IlcPVBARClusterizerv1::MakeClusters()
{
  ...
  const Int_t maxNDigits = 3584; // There is no clusters larger than PVBAR module ;)
  ...
  IlcPVBARRecPoint * clu = 0 ; 
  for(Int_t i=0; i<nDigits; i++){
    if(fDigitsUsed[i])
      continue ;

    digit=static_cast<IlcPVBARDigit*>(fDigitsArr->At(i)) ;
    ...
    if (( IsInEmc(digit) &&  Calibrate(digit->GetEnergy(),digit->GetId()) > fEmcClusteringThreshold ) || 
      ...
      if  ( IsInEmc(digit) ) {   
        ...
        fEMCRecPoints->AddAt(new  IlcPVBAREmcRecPoint(""), fNumberOfEmcClusters) ;
        clu = static_cast<IlcPVBAREmcRecPoint *>( fEMCRecPoints->At(fNumberOfEmcClusters) ) ; 
	...
	clu->AddDigit(*digit, Calibrate(digit->GetEnergy(),digit->GetId()),CalibrateT(digit->GetTime(),digit->GetId())) ;
        ...
        fCPVRecPoints->AddAt(new IlcPVBARCpvRecPoint(""), fNumberOfCpvClusters) ;
        clu =  static_cast<IlcPVBARCpvRecPoint *>( fCPVRecPoints->At(fNumberOfCpvClusters) ) ;  
      ...
      while (index < iDigitInCluster){ // scan over digits already in cluster 
        digit =  static_cast<IlcPVBARDigit*>( fDigitsArr->At(clusterdigitslist[index]) )  ;      
        index++ ; 
	  ...
	  if (iDigitInCluster >= maxNDigits) {
	    ...
	    return;
          ...
          digitN = static_cast<IlcPVBARDigit*>( fDigitsArr->At(j) ) ;
...
//____________________________________________________________________________
void IlcPVBARClusterizerv1::MakeUnfolding()
{
  ...
  if(fNumberOfEmcClusters > 0){
    ...
    for(index = 0 ; index < numberofNotUnfolded ; index++){
      ...
      IlcPVBAREmcRecPoint * emcRecPoint = static_cast<IlcPVBAREmcRecPoint *>( fEMCRecPoints->At(index) ) ;
      if(emcRecPoint->GetPVBARMod()> nModulesToUnfold)
        break ;
      
      Int_t nMultipl = emcRecPoint->GetMultiplicity() ; 
      IlcPVBARDigit ** maxAt = new IlcPVBARDigit*[nMultipl] ;
  ...
  if(fNumberOfCpvClusters > 0){
    ...
    for(index = 0 ; index < numberofCpvNotUnfolded ; index++){
      ...
      IlcPVBARRecPoint * recPoint = static_cast<IlcPVBARRecPoint *>( fCPVRecPoints->At(index) ) ;

      if(recPoint->GetPVBARMod()> nModulesToUnfold)
        break ;
      
      IlcPVBAREmcRecPoint * emcRecPoint = static_cast<IlcPVBAREmcRecPoint*>(recPoint) ; 
...
//____________________________________________________________________________
Double_t  IlcPVBARClusterizerv1::ShowerShape(Double_t x, Double_t z)
{ 
  // Shape of the shower (see PVBAR TDR)
...
//____________________________________________________________________________
void  IlcPVBARClusterizerv1::UnfoldCluster(IlcPVBAREmcRecPoint * iniEmc, 
                                          Int_t nMax, 
                                          IlcPVBARDigit ** maxAt, 
                                          Float_t * maxAtEnergy)
{ 
  ...
  Int_t iDigit ;
  for(iDigit = 0 ; iDigit < nDigits ; iDigit ++){
    digit = static_cast<IlcPVBARDigit*>( fDigitsArr->At(emcDigits[iDigit] ) ) ;   
    fGeom->AbsToRelNumbering(digit->GetId(), relid) ;
...
//      efit[iDigit] += epar * ShowerShape(xDigit - xpar,zDigit - zpar,vIncid) ;
  ...
  while(iparam < nPar ){
    ...
    epar = fitparameters[iparam+2] ;
    iparam += 3 ;    
//    fGeom->GetIncidentVector(fVtx,iniEmc->GetPVBARMod(),xpar,zpar,vIncid) ;

    IlcPVBAREmcRecPoint * emcRP = 0 ;  
    ...
    if(iniEmc->IsEmc()){ //create new entries in fEMCRecPoints...
      ...
      (*fEMCRecPoints)[fNumberOfEmcClusters] = new IlcPVBAREmcRecPoint("") ;
      emcRP = static_cast<IlcPVBAREmcRecPoint *>( fEMCRecPoints->At(fNumberOfEmcClusters) ) ;
      fNumberOfEmcClusters++ ;
      ...
      (*fCPVRecPoints)[fNumberOfCpvClusters] = new IlcPVBARCpvRecPoint("") ;
      emcRP = static_cast<IlcPVBAREmcRecPoint *>( fCPVRecPoints->At(fNumberOfCpvClusters) ) ;
      fNumberOfCpvClusters++ ;
    }
...
void IlcPVBARClusterizerv1::UnfoldingChiSquare(Int_t & nPar, Double_t * Grad, Double_t & fret, Double_t * x, Int_t iflag)
{
  ...
  IlcPVBAREmcRecPoint * emcRP = static_cast<IlcPVBAREmcRecPoint*>( toMinuit->At(0) )  ;
  TClonesArray * digits = static_cast<TClonesArray*>( toMinuit->At(1) )  ;
  // A bit buggy way to get an access to the geometry
  // To be revised!
  IlcPVBARGeometry *geom = static_cast<IlcPVBARGeometry *>(toMinuit->At(2));

//  TVector3 * vtx = static_cast<TVector3*>(toMinuit->At(3)) ;  //Vertex position
  
  //  IlcPVBAREmcRecPoint * emcRP = static_cast<IlcPVBAREmcRecPoint *>( gMinuit->GetObjectFit() ) ; // EmcRecPoint to fit
...
//  TVector3 vInc ;
  ...
  for( iDigit = 0 ; iDigit < nOdigits ; iDigit++) {
    ...
    digit = static_cast<IlcPVBARDigit*>( digits->At( emcDigits[iDigit] ) ); 
    ...
    geom->RelPosInModule(relid, xDigit, zDigit) ;
     ...
     if(iflag == 2){  // calculate gradient
       ...
       while(iParam < nPar ){
         ...
         iParam++ ;          
//         fGeom->GetIncidentVector(*vtx,emcRP->GetPVBARMod(),x[iParam-2],x[iParam-1],vInc) ;
...
//         fGeom->GetIncidentVector(*vtx,emcRP->GetPVBARMod(),xpar,zpar,vInc) ;
...
//____________________________________________________________________________
void IlcPVBARClusterizerv1::Print(const Option_t *)const
{
  ...
  if( strcmp(GetName(), "") !=0 ) {  
    ...
    message = " IlcPVBARClusterizerv1 not initialized " ;
...
//____________________________________________________________________________
void IlcPVBARClusterizerv1::PrintRecPoints(Option_t * option)
{
  // Prints list of RecPoints produced at the current pass of IlcPVBARClusterizer
  ...
  if(strstr(option,"all")) {
    ...
    Int_t index ;
    for (index = 0 ; index < fEMCRecPoints->GetEntries() ; index++) {
      IlcPVBAREmcRecPoint * rp = (IlcPVBAREmcRecPoint * )fEMCRecPoints->At(index) ; 
      TVector3  locpos;  
      ...
      printf("\n%6d  %8.2f  %3d     %2d     %4.1f   %4.1f   %4.1f   %4f  %4f    %2d     : ", 
	      rp->GetIndexInList(), rp->GetEnergy(), rp->GetMultiplicity(), rp->GetPVBARMod(), 
    ...
    for (index = 0 ; index < fCPVRecPoints->GetEntries() ; index++) {
      IlcPVBARCpvRecPoint * rp = (IlcPVBARCpvRecPoint * )fCPVRecPoints->At(index) ; 
      
      TVector3  locpos;  
      rp->GetLocalPosition(locpos);
      
      printf("\n%6d  %8.2f  %2d     %4.1f    %4.1f %4.1f \n", 
	     rp->GetIndexInList(), rp->GetEnergy(), rp->GetPVBARMod(), 
...
//____________________________________________________________________________
void IlcPVBARClusterizerv1::SetDistancesToBadChannels()
{
  ...
  for(Int_t iRP=0; iRP<fEMCRecPoints->GetEntries(); iRP++){
    rp = (IlcPVBAREmcRecPoint*)fEMCRecPoints->At(iRP);
    //evaluate distance to border
    relid[0]=rp->GetPVBARMod() ;
    relid[2]=1 ;
    relid[3]=1 ;
    ...
    for(Int_t iBad=0; iBad<fgCalibData->GetNumOfEmcBadChannels(); iBad++) {
      ...
      if(relid[0]!=rp->GetPVBARMod()) //We can not evaluate global position directly since 
...
//==================================================================================
Float_t IlcPVBARClusterizerv1::Calibrate(Float_t amp, Int_t absId) const{
...
//==================================================================================
Float_t IlcPVBARClusterizerv1::CalibrateT(Float_t time, Int_t absId)const{

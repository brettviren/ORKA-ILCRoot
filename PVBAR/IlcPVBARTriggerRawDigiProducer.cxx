 ...
 **************************************************************************/
//This class produces PVBAR trigger digits of one event.
...
ClassImp(IlcPVBARTriggerRawDigiProducer)

IlcPVBARTriggerRawDigiProducer::IlcPVBARTriggerRawDigiProducer()
...
IlcPVBARTriggerRawDigiProducer::IlcPVBARTriggerRawDigiProducer(IlcRawReader *rawReader)
  :fModules(kNMods, false),
   fSaturationThreshold(950),
   fParameters(0),
   fRawReader(rawReader),
   fRawStream(0),
   fTriggerReader(new IlcPVBARTriggerRawReader)
{
  SetAnalyseModule(2);
  SetAnalyseModule(3);
  SetAnalyseModule(4);

  fRawStream = new IlcCaloRawStreamV3(rawReader,"PVBAR");
}
...
void IlcPVBARTriggerRawDigiProducer::ProcessEvent(TClonesArray* tdigits)
{
  ...
  for(unsigned int mod = 0; mod < fModules.size(); ++mod) {
	...
	for(int branch = 0; branch < kNBranches; ++branch) {
	  ...
	  IlcPVBARTRURawReader* truReader = fTriggerReader->GetTRU(mod, TRURow, branch);
	  if( truReader->IsActive() ) {
		...
		if( triggered ){
		  ...
		  new((*tdigits)[iDigit]) IlcPVBARTriggerRawDigit(mod,xIdx,zIdx,TRURow,branch,TSmax); 
		  iDigit++;
		}// end  "if triggered"
...
int IlcPVBARTriggerRawDigiProducer::Get2x2Max(IlcPVBARTriggerRawReader* reader, IlcPVBARTriggerParameters* params, int mod, int xIdx, int zIdx)
{
  int max = 0;
  for(int timeBin = 0; timeBin < kNTRUTimeBins; ++timeBin) {
...
int IlcPVBARTriggerRawDigiProducer::Get2x2Signal(IlcPVBARTriggerRawReader* reader, IlcPVBARTriggerParameters* parameters, int mod, int xIdx, int zIdx, int timeBin)
{
  ...
  if( reader->GetTRU(mod, TRURow, branch)->IsActive() ){
    ...
    if( parameters )
      ...
      return signal - IlcPVBARTRURawReader::kDefaultSignalValue;
  }
  else
    return 0;
}

int IlcPVBARTriggerRawDigiProducer::Get4x4Max(IlcPVBARTriggerRawReader* reader, IlcPVBARTriggerParameters* params, int mod, int TRURow, int branch, int xIdx, int zIdx)
{
  int max = 0;
  for(int timeBin = 0; timeBin < kNTRUTimeBins; ++timeBin) {
...
int IlcPVBARTriggerRawDigiProducer::Get4x4Signal(IlcPVBARTriggerRawReader* reader, IlcPVBARTriggerParameters* params, int mod, int TRURow, int branch, int xIdx, int zIdx, int timeBin)
{
...
bool IlcPVBARTriggerRawDigiProducer::Is2x2Active(IlcPVBARTriggerRawReader* reader, int mod, int xIdx, int zIdx)
{
  const int TRURow = xIdx / kN2x2XPrTRURow;
  const int branch = zIdx / kN2x2ZPrBranch;
...
bool IlcPVBARTriggerRawDigiProducer::Is2x2Active(IlcPVBARTriggerRawReader* reader, int mod, int xIdx, int zIdx, int timeBin)
{
  const int TRURow = xIdx / kN2x2XPrTRURow;
  const int branch = zIdx / kN2x2ZPrBranch;

...
/* $Id: IlcPVBARebug.cxx 30520 2009-01-02 23:54:05Z fca $ */
...
//____________________________________________________________________________
IlcPVBARv1::IlcPVBARv1(IlcPVBARReconstructor * Reconstructioner, const char *name, const char *title):
  IlcPVBARv0(name,title)
{
  // ctor : title is used to identify the layout
  ...
  IlcPVBARGeometry::GetInstance(title, "") ; 

  if (GetGeometry()->IsInitialized() ) 
    Info("IlcPVBARv1", "IlcPVBAR %d : PVBAR geometry intialized for %s", Version(), GetGeometry()->GetName() );
  else
    Info("IlcPVBARv1", "IlcPVBAR %d : PVBAR geometry initialization failed !", Version() ) ;   

  // Defining the PVBAR Reconstructioner
...
//____________________________________________________________________________
void IlcPVBARv1::AddHit(Int_t shunt, Int_t primary, Int_t tracknumber, Int_t Id, Float_t * hits, Int_t trackpid, TLorentzVector p, Float_t * lpos)
{
  // Add a hit to the hit list.
  // A PVBAR hit is the sum of all hits in a single crystal
  ...
  newHit = new IlcPVBARHit(shunt, primary, tracknumber, Id, hits, trackpid, p, lpos) ;

  for ( hitCounter = fNhits-1 ; hitCounter >= 0 && !deja ; hitCounter-- ) {
    curHit = (IlcPVBARHit*) (*fHits)[hitCounter] ;
    if( *curHit == *newHit ) {
...
void IlcPVBARv1::Hits2SDigits()
{
  ...
  Int_t j ; 
  IlcPVBARHit   * hit ;
  IlcPVBARDigit * newdigit ;
  IlcPVBARDigit * curdigit ;
  Bool_t deja = kFALSE ; 
  ...
  for (itrack=0; itrack<gIlc->GetNtrack(); itrack++){
    ...
    for ( i = 0 ; i < fHits->GetEntries() ; i++ ) {
      hit = (IlcPVBARHit*)fHits->At(i) ;
    
      // Assign primary number only if contribution is significant
      if( hit->GetEnergy() > fDigitThreshold)
	newdigit = new IlcPVBARDigit( hit->GetPrimary(), hit->GetId(), Digitize( hit->GetEnergy() ) ) ;
      else
	newdigit = new IlcPVBARDigit( -1               , hit->GetId(), Digitize( hit->GetEnergy() ) ) ;
      deja =kFALSE ;

      for ( j = 0 ; j < fnSdigits ;  j++) { 
	curdigit = (IlcPVBARDigit*) fSDigits->At(j) ;
	if ( *curdigit == *newdigit) {
	  *curdigit = *curdigit + *newdigit ; 
      ...
      if ( !deja ) {
	new((*fSDigits)[fnSdigits]) IlcPVBARDigit(* newdigit) ;
	fnSdigits++ ;  
      }
  ...
  for (i = 0 ; i < fnSdigits ; i++) { 
    IlcPVBARDigit * digit = (IlcPVBARDigit *) fSDigits->At(i) ; 
...
//____________________________________________________________________________
void IlcPVBARv1::SDigits2Digits()
{
  ...
  Int_t idCurSDigit = ((IlcPVBARDigit *)fSDigits->At(0))->GetId() ;
  ...
  for(absID = 1; absID < GetGeometry()->GetNModules()*GetGeometry()->GetNPhi()*GetGeometry()->GetNZ(); absID++){
	...
	new((*fDigits)[fNdigits]) IlcPVBARDigit( -1,absID,Digitize(noise) ) ;
	fNdigits++ ;  
      }
    }
    else{ //add noise and may be remove the true hit
      Float_t signal = noise + Calibrate(((IlcPVBARDigit *)fSDigits->At(iCurSDigit))->GetAmp()) ;
      if( signal >fDigitThreshold ){
	IlcPVBARDigit * digit = (IlcPVBARDigit*) fSDigits->At(iCurSDigit) ;
	new((*fDigits)[fNdigits]) IlcPVBARDigit( *digit ) ;
	((IlcPVBARDigit *)fDigits->At(fNdigits))->SetAmp(Digitize(signal));
	fNdigits++ ;  
      }

      if(iCurSDigit < fSDigits->GetEntries()-1){
	iCurSDigit++ ;
	idCurSDigit = ((IlcPVBARDigit*)fSDigits->At(iCurSDigit))->GetId() ;
      }
      else
  ...
  for(idigit = iCurSDigit; idigit < fSDigits->GetEntries() ; idigit++){  //loop over CPV/PPSD digits
    ...
    IlcPVBARDigit * digit = (IlcPVBARDigit *) fSDigits->At(idigit) ; 
  ...
  for (i = 0 ; i < fNdigits ; i++) { 
    IlcPVBARDigit * digit = (IlcPVBARDigit *) fDigits->At(i) ; 
    digit->SetIndexInList(i) ;     
  }
...
void IlcPVBARv1::MakeBranch(Option_t* opt, char *file)
{ 
  // Called by IlcRun
...
void IlcPVBARv1::Reconstruction(IlcPVBARReconstructor * Reconstructioner)
{ 
...
//____________________________________________________________________________
void IlcPVBARv1::ResetReconstruction() 
{ 
  // Deleting reconstructed objects
...
void IlcPVBARv1::StepManager(void)
{
  ...
  if ( name == "ORKA" ) {
	...
	gMC->TrackCharge() != 0) {      
      ...
      TClonesArray *cpvDigits = new TClonesArray("IlcPVBARCPVDigit",0);   // array of digits for current hit
	...
	IlcPVBARCPVDigit  *cpvDigit1 = (IlcPVBARCPVDigit*) cpvDigits->UncheckedAt(idigit);
	Float_t x1 = cpvDigit1->GetXpad() ;
	Float_t z1 = cpvDigit1->GetYpad() ;
	for (Int_t jdigit=idigit+1; jdigit<ndigits; jdigit++) {
	  IlcPVBARCPVDigit  *cpvDigit2 = (IlcPVBARCPVDigit*) cpvDigits->UncheckedAt(jdigit);
	...
	IlcPVBARCPVDigit  *cpvDigit = (IlcPVBARCPVDigit*) cpvDigits->UncheckedAt(idigit);
	relid[0] = moduleNumber + 1 ;                             // CPV (or PVBAR) module number
	relid[1] =-1 ;                                            // means CPV
  ...
  if(gMC->CurrentVolID(copy) == gMC->VolId("PXTL") ) { //  We are inside a PBWO crystal
    ...
    if ( (xyze[3] != 0) || entered ) {  // Track is inside the crystal and deposits some energy or just entered 
      ...
      gMC->CurrentVolOffID(10, relid[0]) ; // get the PVBAR module number ;
  ...
  } // we are inside a PVBAR Xtal


}

//____________________________________________________________________________
void IlcPVBARv1::CPVDigitize (TLorentzVector p, Float_t *zxhit, Int_t moduleNumber, TClonesArray *cpvDigits)
{
  // ------------------------------------------------------------------------

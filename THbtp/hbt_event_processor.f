CCC ********************************************************************
CCC  Modifications for ILC-ROOT application at CERN - 12/15/2000
...
C ----------------------------------------------------------------------------
C
C     Source of Data for ILC Application:
C     ====================================
C
C File Unit#    File Name                For ILC File or Struc?
C ----------------------------------------------------------------------------
C     1      hbt_parameters.in          Call IlcHbtp_ function
C     2      event_text.in              Call IlcHbtp_ function
...
C     10     event_hbt_text.out         Call IlcHbtp_ function
...
CCC   Set Data I/O control for ILC or Standalone application
C     ILC = 1   ! This is for the ILC IlcRoot application
CCC   ILC = 0   ! This is for the standalone application

CCC   Initialize error code for ILC application:
      errorcode = 0
...
C     because we can easily assume that is already done
         Call ilchbtp_initialize
      Else If (ILC .eq. 0) Then
         Call initialize
      End If
...
CCC   For standalone version (ILC = 0), read parameters from
CCC   File Unit=1, 'hbt_parameters.in'
CCC   For ILC-ROOT version (ILC=1) load parameters from Call to C++ funct
      If(ILC .eq. 1) then
         Call IlcHbtp_SetParameters
      Else If(ILC .eq. 0) Then
...
C            by trk_maxlen in the 'trk' table.
      ...
      If(ILC .eq. 1) Then
CCC   For ILC application do the following:
...
CCC      Mark accepted tracks in all events
         ...
         Call IlcHbtp_GetNumberEvents(n_events)
         do i = 1,n_events
            Call IlcHbtp_SetActiveEventNumber(i)
            track_counter = 0
            Call IlcHbtp_GetNumberTracks(ntracks)
            do j = 1,ntracks
               Call IlcHbtp_GetTrack(j,flag,px,py,pz,ge_pid)
               eg_pid = ge_pid
...
C	    write(*,*) '  FFF: 1 calling PutTrack j = ',j
            Call IlcHbtp_PutTrack(j,flag4,px,py,pz,ge_pid)
         else
C	    write(*,*) '  FFF: 2 calling PutTrack j = ',j
            Call IlcHbtp_PutTrack(j,flag0,px,py,pz,ge_pid)
         end if
         end do
         end do
...
CCC   Initialize accepted track counters for this new event:
      ...
      If(ILC .eq. 1) Then
         Call IlcHbtp_GetNumberTracks(ntracks)
         do i = 1,ntracks
            Call IlcHbtp_GetTrack(i,flag,px,py,pz,ge_pid)
            eg_pid = ge_pid
	...
	open(unit=27,status='old',access='sequential',
     ...
     1       file='cpp_18.dat')
      ...
      do i = 1,max_c2_coul
         ...
         Call IlcHbtp_interp(coulradmin,coulradmax,coulradstep,
...
C----------------------------------------------------------------------
      ...
      subroutine IlcHbtp_interp(rmin,rmax,rstep,nrsteps,function,
     1                          ndim,r,answer)
      implicit none
...
C-------------------------------------
        ...
        If(ILC .eq. 1) Then
           Call IlcHbtp_SetActiveEventNumber(ievent-1)
...
C>>>>>>>>>>>>>>   Piotr, this needs to be replaced with Ilc random
C>>>>>>>>>>>>>>>   number generator

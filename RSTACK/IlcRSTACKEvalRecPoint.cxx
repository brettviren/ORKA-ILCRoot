...
/* $Id: IlcRSTACKEvalRecPoint.cxx 47119 2011-02-04 15:35:48Z policheh $ */
...
// It differs from IlcRSTACKClusterizerv1 in neighbour definition only
...
#include "IlcRSTACKRecCpvManager.h"
...
ClassImp(IlcRSTACKEvalRecPoint)

IlcRSTACKEvalRecPoint::IlcRSTACKEvalRecPoint() :
  fIsEmc(kFALSE),
  fIsCpv(kTRUE),
  fParent(-333),
  fChi2Dof(-111),
...
IlcRSTACKEvalRecPoint::IlcRSTACKEvalRecPoint(Bool_t cpv, IlcRSTACKEvalRecPoint* parent) : 
  fIsEmc(kFALSE),
  fIsCpv(kFALSE),
  fParent(-333),
  fChi2Dof(-111),
...
IlcRSTACKEvalRecPoint::IlcRSTACKEvalRecPoint(Int_t i, Bool_t cpv) : 
  fIsEmc(kFALSE),
  fIsCpv(kFALSE),
  fParent(-333),
  fChi2Dof(-111),
...
{
  // ctor
  IlcRSTACKEmcRecPoint* rp=0;

  IlcRSTACKLoader* fLoader = IlcRSTACKLoader::GetRSTACKLoader(fEventFolderName);

  if(cpv) {
    rp = (IlcRSTACKCpvRecPoint*)fLoader->CpvRecPoints()->At(i);
    fIsEmc = kFALSE;
    fIsCpv = kTRUE;
  }
  else {
    rp = (IlcRSTACKEmcRecPoint*)fLoader->EmcRecPoints()->At(i);
    fIsEmc = kTRUE;
    fIsCpv = kFALSE;
  }
  ...
  for(Int_t iDigit=0; iDigit<nDigits; iDigit++) {
    IlcRSTACKDigit* digit = (IlcRSTACKDigit*)fLoader->Digits()->At( digits[iDigit] );
...
IlcRSTACKClusterizer* IlcRSTACKEvalRecPoint::GetClusterizer()
{
  // returns clusterizer task
  TFolder* ilcF = (TFolder*)gROOT->FindObjectAny("YSIlc");
  TFolder* wPoolF = (TFolder*)ilcF->FindObject("WhiteBoard/RecPoints/RSTACK/SmP");
  IlcRSTACKClusterizer* clu = (IlcRSTACKClusterizer*)wPoolF->FindObject("RSTACK:clu-v1");
  if(!clu) { 
...
Bool_t IlcRSTACKEvalRecPoint::TooClose(IlcRSTACKRecPoint* pt) const 
{
...
Bool_t IlcRSTACKEvalRecPoint::NeedToSplit() const 
{
  // rec.point needs to be split
  return kFALSE;
}

void IlcRSTACKEvalRecPoint::DeleteParent()
{
  // delete parent
  fParent=-333;
}

void IlcRSTACKEvalRecPoint::UpdateWorkingPool()
{
  // update pool of rec.points
  Int_t i; //loop variable
  
  for(i=0; i<this->InWorkingPool(); i++) {
     IlcRSTACKEvalRecPoint* parent = (IlcRSTACKEvalRecPoint*)GetFromWorkingPool(i);
     TObjArray children;
     Int_t nChild = parent->HasChild(children);
     for(Int_t iChild=0; iChild<nChild; iChild++)
       {
	 ((IlcRSTACKEvalRecPoint*)children.At(iChild))->DeleteParent();
       }
  ...
  for(i=0; i<this->InWorkingPool(); i++) {
    IlcRSTACKEvalRecPoint* weak = (IlcRSTACKEvalRecPoint*)GetFromWorkingPool(i);
    if (weak->KillWeakPoint()) delete weak;
  }

  for(i=0; i<this->InWorkingPool(); i++) {
    IlcRSTACKEvalRecPoint* close = (IlcRSTACKEvalRecPoint*)GetFromWorkingPool(i);
    close->MergeClosePoint();
  }

  for(i=0; i<this->InWorkingPool(); i++)
    ((IlcRSTACKEvalRecPoint*)IlcRSTACKEvalRecPoint::GetFromWorkingPool(i))->SetIndexInList(i);

}

Int_t IlcRSTACKEvalRecPoint::HasChild(TObjArray& children)
{
  // returns the number of children
  for(Int_t iChild=0; iChild<InWorkingPool(); iChild++)
    {
      IlcRSTACKEvalRecPoint* child = (IlcRSTACKEvalRecPoint*)GetFromWorkingPool(iChild);
...
void IlcRSTACKEvalRecPoint::Init()
{
  // initialization
  IlcRSTACKClusterizer* clusterizer = GetClusterizer();
  if(!clusterizer) {
    IlcFatal("Cannot get clusterizer") ;
  }

  TClonesArray* digits = IlcRSTACKLoader::GetRSTACKLoader(fEventFolderName)->Digits();
  Float_t logWeight=0;  
...
void IlcRSTACKEvalRecPoint::MakeJob()
{
  // Reconstruction algoritm implementation.

  IlcRSTACKRecManager* recMng = GetReconstructionManager();
  ...
  for(Int_t iChild=0; iChild<nChild; iChild++) {
    IlcRSTACKEvalRecPoint* child = (IlcRSTACKEvalRecPoint*)children.At(iChild);
...
void IlcRSTACKEvalRecPoint::InitTwoGam(Float_t* gamma1, Float_t* gamma2)
{
...
//    yy = XY(yy/e);
  ...
  for(iDigit = 0 ; iDigit < nDigits ; iDigit ++)
    {
      digit = (IlcRSTACKDigit*)fLoader->Digits()->At( digits[iDigit] ); 
      eDigit = energies[iDigit];
      RSTACKgeom->AbsToRelNumbering(digit->GetId(), relid) ;
      RSTACKgeom->RelPosInModule(relid, iy, ix);
...
void IlcRSTACKEvalRecPoint::TwoGam(Float_t* gamma1, Float_t* gamma2)
{
...
void IlcRSTACKEvalRecPoint::UnfoldTwoMergedPoints(Float_t* gamma1, Float_t* gamma2)
{
  ...
  for(Int_t iDigit=0; iDigit<nDigits; iDigit++)
    {
      ...
      RSTACKgeom->AbsToRelNumbering(digit->GetId(), relid) ;
      Float_t x,z;     
      RSTACKgeom->RelPosInModule(relid, x, z);
  ...
  for( Int_t iMax=0; iMax<nMax; iMax++)
	...
	gamma = gamma2;
      ...
      IlcRSTACKEvalRecPoint* newRP = new IlcRSTACKEvalRecPoint(IsCPV(),this);
      ...
      for( Int_t iDigit = 0 ; iDigit < nDigits ; iDigit ++)
	{
	  ...
	  RSTACKgeom->AbsToRelNumbering(digit->GetId(), relid) ;
	  Float_t ix,iz;
	  RSTACKgeom->RelPosInModule(relid, ix, iz);
...
void IlcRSTACKEvalRecPoint::EvaluatePosition() 
{
...
Bool_t IlcRSTACKEvalRecPoint::KillWeakPoint()
{
...
void IlcRSTACKEvalRecPoint::SplitMergedShowers() 
{
...
void IlcRSTACKEvalRecPoint::MergeClosePoint() 
{
  // merge rec.points if they are too close
  IlcRSTACKLoader* fLoader = IlcRSTACKLoader::GetRSTACKLoader(fEventFolderName);
//    IlcRSTACKDigitizer* digitizer = fLoader->Digitizer();
...
//    Float_t fSlope = digitizer->GetSlope();
  ...
  for (Int_t i=0;i<InWorkingPool(); i++)
	...
	{
	  IlcRSTACKRecPoint* rp = (IlcRSTACKRecPoint*)obj;
	  if(GetRSTACKMod() == rp->GetRSTACKMod())
	    {
	      if(TooClose(rp))
		{
		  ...
		  ((IlcRSTACKEvalRecPoint*)rp)->Print();
		  ...
		  Float_t* energies = ((IlcRSTACKEmcRecPoint*)rp)->GetEnergiesList();
...
Int_t IlcRSTACKEvalRecPoint::UnfoldLocalMaxima() 
{
  ...
  for(iDigit=0; iDigit<nDigits; iDigit++)
    {
      ...
      IlcRSTACKDigit* digit = (IlcRSTACKDigit*)fLoader->Digits()->At( digitsList[iDigit] );
      RSTACKgeom->AbsToRelNumbering(digit->GetId(), relid) ;
      Float_t x,z;
      RSTACKgeom->RelPosInModule(relid, x, z);
      ...
      for(Int_t iMax=0; iMax<nMax; iMax++)
	{
	  IlcRSTACKDigit* digitMax = maxAt[iMax];
	  Float_t eMax = maxAtEnergy[iMax]; 
	  RSTACKgeom->AbsToRelNumbering(digitMax->GetId(), relid) ;
	  RSTACKgeom->RelPosInModule(relid, xMax, zMax);
	  Float_t dx = xMax - x;
	  Float_t dz = zMax - z;
	  Float_t amp,gx,gy;
	  GetReconstructionManager()->AG(eMax,dz,dx,amp,gx,gy); 
...
void IlcRSTACKEvalRecPoint::PrintPoint()
{
  // print rec.point to stdout
  IlcRSTACKCpvRecPoint::Print();
  ...
  IlcInfo(Form(message.Data(), Chi2Dof(), lpos.X(), lpos.Z(), GetRSTACKMod())) ;

}

IlcRSTACKRecManager* IlcRSTACKEvalRecPoint::GetReconstructionManager() const
{
  // returns reconstruction manager
  TFolder* ilcF = (TFolder*)gROOT->FindObjectAny("YSIlc");
  TFolder* wPoolF = (TFolder*)ilcF->FindObject("WhiteBoard/RecPoints/RSTACK/SmP");
  IlcRSTACKRecManager* recMng = (IlcRSTACKRecManager*)wPoolF->FindObject("IlcRSTACKRecManager");
  if(!recMng) { 
...
IlcRSTACKRecPoint* IlcRSTACKEvalRecPoint::Parent()
{
  // returns a parent
  if(fParent<0) return NULL;
  else
    return (IlcRSTACKRecPoint*)GetFromWorkingPool(fParent);
//    return fParent;
}

Float_t IlcRSTACKEvalRecPoint::Chi2Dof() const 
{
  // returns a chi^2 per degree of freedom
  return fChi2Dof;
}

const TObject* IlcRSTACKEvalRecPoint::GetWorkingPool()
{
...
void IlcRSTACKEvalRecPoint::AddToWorkingPool(TObject* obj)
{
  // add a rec.point to a pool
  ((TObjArray*)GetWorkingPool())->Add(obj);
}

TObject* IlcRSTACKEvalRecPoint::GetFromWorkingPool(Int_t i)
{
...
Int_t IlcRSTACKEvalRecPoint::InWorkingPool()
{
  // return the number of rec.points in a pool
  return ((TObjArray*)GetWorkingPool())->GetEntries();
}

void IlcRSTACKEvalRecPoint::RemoveFromWorkingPool(TObject* obj)
{
...
void IlcRSTACKEvalRecPoint::PrintWorkingPool()
{

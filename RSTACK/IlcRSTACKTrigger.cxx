 ...
 **************************************************************************/
/* $Id: IlcRSTACKTrigger.cxx 47329 2011-02-11 19:44:15Z kharlov $ */
...
//  IlcRSTACKTrigger *tr = new IlcRSTACKTrigger();//Init Trigger
...
ClassImp(IlcRSTACKTrigger)

//______________________________________________________________________
IlcRSTACKTrigger::IlcRSTACKTrigger()
  : IlcTriggerDetector(),
...
IlcRSTACKTrigger::IlcRSTACKTrigger(const IlcRSTACKTrigger & trig) : 
  IlcTriggerDetector(trig),
...
//_________________________________________________________________________
IlcRSTACKTrigger & IlcRSTACKTrigger::operator = (const IlcRSTACKTrigger &)
{
  Fatal("operator =", "no implemented");
  return *this;
}

void IlcRSTACKTrigger::CreateInputs()
{
   // inputs 
   ...
   fInputs.AddLast( new IlcTriggerInput( "0PH0",       name, 0 ) );
   fInputs.AddLast( new IlcTriggerInput( "RSTACK_JetHPt_L1",name, 1 ) );
   fInputs.AddLast( new IlcTriggerInput( "RSTACK_JetMPt_L1",name, 1 ) );
   fInputs.AddLast( new IlcTriggerInput( "RSTACK_JetLPt_L1",name, 1 ) );
 
}

//____________________________________________________________________________
void IlcRSTACKTrigger::FillTRU(const TClonesArray * /*digits*/, const IlcRSTACKGeometry * /*geom*/) const {
...
//     
//     dig    = static_cast<IlcRSTACKDigit *>(digits->At(idig)) ;
...
//______________________________________________________________________
void IlcRSTACKTrigger::GetCrystalPhiEtaIndexInModuleFromTRUIndex(Int_t itru, Int_t iphitru, Int_t ietatru,Int_t &iphiMod,Int_t &ietaMod) const 
{
...
//____________________________________________________________________________
Bool_t IlcRSTACKTrigger::IsPatchIsolated(Int_t iPatchType, const Int_t imod, const Int_t mtru, const Float_t maxamp, const Int_t maxphi, const Int_t maxeta) {
 ...
 //Get matrix of TRU or Module with maximum amplitude patch.
  ...
  IlcDebug(2,Form("Number of added Isol Cells %d, Patch Size %d",isolcells, ipatchcells));
  IlcDebug(2,Form("Patch: minrow %d, maxrow %d, mincol %d, maxcol %d",minrow,maxrow,mincol,maxcol));
  
  if(minrow < 0 || mincol < 0 || maxrow > rowborder || maxcol > colborder){
    IlcDebug(1,Form("Out of Module/TRU range, cannot isolate patch"));
    return kFALSE;
  }
  ...
  IlcDebug(2,Form("Type %d, Maximum amplitude %f, patch+isol square %f",iPatchType, maxamp, amp));

  if(amp < maxamp){
    IlcError(Form("Bad sum: Type %d, Maximum amplitude %f, patch+isol square %f",iPatchType, maxamp, amp));
    return kFALSE;
  }
  else
    amp-=maxamp; //Calculate energy in isolation patch that do not comes from maximum patch.
  
  IlcDebug(2, Form("Maximum amplitude %f, Out of patch %f",maxamp, amp));
...
//____________________________________________________________________________
void IlcRSTACKTrigger::MakeSlidingCell(const Int_t imod, TMatrixD &ampmax2, TMatrixD &ampmaxn)
{
...
//____________________________________________________________________________
void IlcRSTACKTrigger::Print(const Option_t * opt) const 
{
  ...
  in = (IlcTriggerInput*)fInputs.FindObject( "0PH0" );
  if(in->GetValue())
    printf( "             *** RSTACK LO is set ***\n") ; 
  
  printf( "             Jet Low Pt Threshold for L1 %10.2f\n", fL1JetLowPtThreshold) ;
  in = (IlcTriggerInput*)fInputs.FindObject( "RSTACK_JetLPt_L1" );
  if(in->GetValue())
    printf( "             *** RSTACK Jet Low Pt for L1 is set ***\n") ;
  
  printf( "             Jet Medium Pt Threshold for L1 %10.2f\n", fL1JetMediumPtThreshold) ;
  in = (IlcTriggerInput*)fInputs.FindObject( "RSTACK_JetMPt_L1" );
  if(in->GetValue())
    printf( "             *** RSTACK Jet Medium Pt for L1 is set ***\n") ;
  
  printf( "             Jet High Pt Threshold for L1 %10.2f\n", fL1JetHighPtThreshold) ;  
  in = (IlcTriggerInput*) fInputs.FindObject( "RSTACK_JetHPt_L1" );
  if(in->GetValue())
    printf( "              *** RSTACK Jet High Pt for L1 is set ***\n") ;
  
}

//____________________________________________________________________________
void IlcRSTACKTrigger::SetTriggers(const Int_t iMod, const TMatrixD & ampmax2, const TMatrixD & ampmaxn)  
{
...
//____________________________________________________________________________
void IlcRSTACKTrigger::Trigger(TClonesArray *digits) 
{
...
//   if(fSimulation)
//     fDigitsList = RSTACKLoader->Digits() ;
//   
//   if(!fDigitsList)
//     IlcFatal("Digits not found !") ;
//   
//   //Fill TRU Matrix  

#ifndef ILCRSTACKCLUSTERIZERV1_H
#define ILCRSTACKCLUSTERIZERV1_H
/* 2012-2013 Infrastructure for Large Collider Experiment.       *
 * All rights reserved.                                                   *
 * See cxx source for full Copyright notice                               */

/* $Id: IlcRSTACKClusterizerv1.h 43634 2010-09-17 09:17:08Z prsnko $ */

/* History of cvs commits:
 *
 * $Log: IlcRSTACKClusterizerv1.h,v $
...
//  RSTACKCpvRP (CPV RecPoints) and IlcRSTACKClusterizer
//
...
class IlcRSTACKClusterizerv1 : public IlcRSTACKClusterizer {
  
public:
  
  IlcRSTACKClusterizerv1() ;
  IlcRSTACKClusterizerv1(IlcRSTACKGeometry *geom);
  virtual ~IlcRSTACKClusterizerv1()  ;

  void    InitParameters() ;
  
  virtual Int_t   AreNeighbours(IlcRSTACKDigit * d1, IlcRSTACKDigit * d2)const ; 
...
protected:
  ...
  virtual Bool_t IsInEmc (IlcRSTACKDigit * digit)const ;     // Tells if id digit is in EMC
  virtual Bool_t IsInCpv (IlcRSTACKDigit * digit)const ;     // Tells if id digit is in CPV
...
private:
  IlcRSTACKClusterizerv1(const IlcRSTACKClusterizerv1 & clu) ;
  IlcRSTACKClusterizerv1 & operator = (const IlcRSTACKClusterizerv1 & obj);

  Bool_t  FindFit(IlcRSTACKEmcRecPoint * emcRP, IlcRSTACKDigit ** MaxAt, Float_t * maxAtEnergy, 
		  Int_t NPar, Float_t * FitParametres) const; //Used in UnfoldClusters, calls TMinuit
  void    Init() ;

  virtual void   MakeUnfolding() ;
  void           UnfoldCluster(IlcRSTACKEmcRecPoint * iniEmc,Int_t Nmax, 
		       IlcRSTACKDigit ** maxAt,Float_t * maxAtEnergy ) ; //Unfolds cluster using TMinuit package
  void           PrintRecPoints(Option_t * option) ;

private:

  Bool_t  fDefaultInit;              //! Says if the task was created by defaut ctor (only parameters are initialized)
  Int_t   fEmcCrystals ;             // number of EMC cristals in RSTACK
  ...
  ClassDef(IlcRSTACKClusterizerv1,8)   // Clusterizer implementation version 1

};

#endif // IlcRSTACKCLUSTERIZERV1_H

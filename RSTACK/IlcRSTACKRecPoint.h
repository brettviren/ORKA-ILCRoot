#ifndef ILCRSTACKRECPOINT_H
#define ILCRSTACKRECPOINT_H
...
class IlcRSTACKRecPoint : public IlcCluster {

 public:
  
  typedef TObjArray RecPointsList ; 

  IlcRSTACKRecPoint() ;                   // ctor         
  IlcRSTACKRecPoint(const char * opt) ;   // ctor 

  IlcRSTACKRecPoint(const IlcRSTACKRecPoint &rp);
  IlcRSTACKRecPoint& operator= (const IlcRSTACKRecPoint &rp);

  
  virtual ~IlcRSTACKRecPoint();
...
//   Fatal("AddDigit", "use AddDigit(IlcRSTACKDigit & digit, Float_t Energy)") ; 
//  }
  virtual  void   AddDigit(IlcRSTACKDigit & digit, Float_t Energy, Float_t time=0) = 0 ; 
  ...
  virtual void    EvalRSTACKMod(IlcRSTACKDigit * digit) ;  
  ...
  virtual Int_t   GetRSTACKMod(void) const {return fRSTACKMod ; }
...
protected:
  ...
  Int_t     fRSTACKMod ;    // RSTACK Module number in which the RecPoint is found
  ...
  ClassDef(IlcRSTACKRecPoint,3) // RecPoint for RSTACK (Base Class)
 
};

#endif // IlcRSTACKRECPOINT_H

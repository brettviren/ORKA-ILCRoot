 ...
 **************************************************************************/
/* $Id: IlcRSTACKDApi0mip.cxx 52964 2011-11-17 12:12:19Z policheh $ */
...
#include <math.h>
#include "IlcLog.h"
#include "IlcRSTACKDApi0mip.h"
ClassImp(IlcRSTACKDApi0mip)
//----------------------------------------------------------------
IlcRSTACKDApi0mip::IlcRSTACKDApi0mip(int module,int iterid,const char* fopt) :
  ...
  TString shname="IlcRSTACKDApi0mip_mod%d_iter%d";
  snprintf(hname,shname.Length(),shname.Data(),module,iterid);
  SetName(hname);

  TString shtitle="RSTACK MIP/pi0 Calibration DA for Module:%d Iteration:%d";
  ...
  TString sfname="IlcRSTACKDApi0mip_mod%d.root";
  snprintf(fname,sfname.Length(),sfname.Data(),module);
  fTFile = TFile::Open(fname,fopt);
}
//----------------------------------------------------------------
IlcRSTACKDApi0mip::IlcRSTACKDApi0mip(const IlcRSTACKDApi0mip& da):
  ...
  if( da.fCreateTree ){
    ...
    fTTree->Branch("IlcRSTACKDATreeEvent","IlcRSTACKDATreeEvent",&fEvent);
  } else {
    fCreateTree = false;
    fTTree = 0;
  }
...
//-------------------------------------------------------------------
IlcRSTACKDApi0mip& IlcRSTACKDApi0mip::operator= (const IlcRSTACKDApi0mip&)
{
  // Operator= is not implemented yet
  IlcFatal("Operator= is not implemented");  
  return *this;
}
//-------------------------------------------------------------------
IlcRSTACKDApi0mip::~IlcRSTACKDApi0mip(){
  // Destructor

  //std::cout<<" DEBUGDEBUG :: IlcRSTACKDApi0mip:: deconstructor "<<std::endl;
  if( fCreateHist ){
...
//-------------------------------------------------------------------
void IlcRSTACKDApi0mip::FillDigit(float adc,int row,int col){
  // Put new digit information in event
  //

  if( fEvent==0 ) fEvent = new IlcRSTACKDATreeEvent;
  float energy = adc * 0.005; // 5MeV/ch is fixed.
  if( energy > 0.020 ){  // Threshold at 20MeV.
    //std::cout<<" IlcRSTACKDApi0mip::AppendDigit() DEBUG:: (row,col)=("<<row<<","<<col<<") adc="<<adc<<" energy="<<energy<<std::endl;
    fEvent->Fill(energy,row,col);
  }
}
//-------------------------------------------------------------------
void IlcRSTACKDApi0mip::NewEvent(){
...
//----------------------------------------------------------------
bool IlcRSTACKDApi0mip::CreateTree(){
  if(! fTFile ) {
    return false;
  }
  fTFile->cd();
...
//----------------------------------------------------------------
bool IlcRSTACKDApi0mip::CreateHist(){
  ...
  if( fH1Time>0 ){
    std::cout<<" IlcRSTACKDApi0mip:Warning!! Output object already exist : "<<fH1Time->GetName()<<std::endl;
    return false;
  }
  ...
  snprintf(htitle,100,"RSTACK pi0 mass vs Digit Id : Mod:%d Iter:%d",fMod,fIterId);
  fH2Pi0DigitId = new TH2F(hname,htitle,17920,0,17920,120,0,0.3);
      
  snprintf(hname,100,"h2_mipdigitid_mod%d_iter%d",fMod,fIterId);
  snprintf(htitle,100,"RSTACK MIP vs Digit Id : Mod:%d Iter:%d",fMod,fIterId);
  ...
  snprintf(htitle,100,"RSTACK pi0 mass vs pT vs Asym : Mod:%d Iter:%d",fMod,fIterId);
...
//-------------------------------------------------------------------
void IlcRSTACKDApi0mip::FillTree(IlcRSTACKDATreeEvent* event){
  // Fill all information (IlcRSTACKDATreeEvent) into TTree
  // if event==NULL, the obtained information (=fEvent) is used.

  if(! fCreateTree ) CreateTree();
  if( event == 0 ){
    if( fEvent == 0 ){
      fEvent = new IlcRSTACKDATreeEvent();
    }
...
//-------------------------------------------------------------------
void IlcRSTACKDApi0mip::FillHist(IlcRSTACKDATreeEvent* event){
  // Fill all information (IlcRSTACKDATreeEvent) into histograms
  // if event==NULL, the obtained information (=fEvent) is used.
  ...
  int ndigits;
  int ndigitsall = 0;
  IlcRSTACKDATreeDigit digit1, digit2;
  ...
  //std::cout<<" IlcRSTACKDApi0mip::FillHisto() DEBUG:: Number of clusters: "<<nclt1<<std::endl;
  while( nclt1-- ){
    IlcRSTACKDATreeCluster& clt1 = event->GetCluster(nclt1);
    ndigits = clt1.GetNDigits();
    while( ndigits-- ){
      IlcRSTACKDATreeDigit& digit = clt1.GetDigit(ndigits);
    ...
    if( clt1.GetEnergy() > 0.2 ){
      nclt2 = nclt1;
      while( nclt2-- ){
	IlcRSTACKDATreeCluster& clt2 = event->GetCluster(nclt2);
...
//-------------------------------------------------------------------
void IlcRSTACKDApi0mip::Print(Option_t *option) const
{
  // Print Out

...
//  Input:  IlcRSTACKDigit or TClonesArray of IlcRSTACKDigits
...
//--
//  Author: Yuri Kharlov
//  Reimplemented from IlcHMPIDRawStream
//  6 July 2008
//_________________________________________________________________________
...
#include "IlcRSTACKCpvRawWrite.h"
#include "IlcRSTACKDigit.h"
#include "IlcRSTACKGeometry.h"

ClassImp(IlcRSTACKCpvRawWrite)

//____________________________________________________________________________
IlcRSTACKCpvRawWrite::IlcRSTACKCpvRawWrite() : 
  TObject(),
  fNDDL(5),
  fNRow(16),
  fNDilogic(10),
...
//____________________________________________________________________________
void IlcRSTACKCpvRawWrite::WriteRaw(const TObjArray *pDigAll)
{
...
// Returns: none
  ...
  for(Int_t iCh=0;iCh<fNDDL;iCh++){//chambers loop
    ...
    IlcFstream* ddlL;                                 //output stream
    
    IlcRawDataHeaderSim header; header.SetAttribute(0);  //empty DDL header
    
    ddlL = new IlcFstream(IlcDAQ::DdlFileName("CPV",iCh));
    ...
    for(Int_t iDig=0;iDig<pDigCh->GetEntriesFast();iDig++){//digits loop
      IlcRSTACKDigit *pDig1=(IlcRSTACKDigit*)pDigCh->At(iDig);
    ...
    for(Int_t row = 0; row < fNRow; row++){ //IlcHMPIDRawStream::kNRows=25!
	...
	cntLpad=0;
	  ...
	  if (isDigThere[iCh][row][dil][pad]!=-1) {
	    IlcRSTACKDigit *pDig=(IlcRSTACKDigit*)pDigCh->At(isDigThere[iCh][row][dil][pad]);             
...
//____________________________________________________________________________
void IlcRSTACKCpvRawWrite::HWaddress(const IlcRSTACKDigit *digit, 
				   UInt_t &w32, Int_t &ddl, Int_t &row, Int_t &dilogic, Int_t &address)
{
// Convert IlcRSTACKDigit to raw word format
// Arguments: w32,ddl,row,dilogic,address where to write the results
  ...
  w32=0;
  IlcBitPacking::PackWord(charge ,w32, 0,11); // 0000 0rrr rrdd ddaa aaaa qqqq qqqq qqqq  Qdc          bits (00..11) counts (0..4095)
  assert(0<=address && address<fNPad);
  IlcBitPacking::PackWord(address,w32,12,17); // 3322 2222 2222 1111 1111 1000 0000 0000  DILOGIC addr bits (12..17) counts (0..47)
  assert(0<=dilogic && dilogic<fNDilogic);
  IlcBitPacking::PackWord(dilogic,w32,18,21); // 1098 7654 3210 9876 5432 1098 7654 3210  DILOGIC No.  bits (18..21) counts (1..10)
  assert(0<=row && row<fNRow);
  IlcBitPacking::PackWord(row ,w32,22,26); //                                          Row No.   bits (22..26) counts (1..24)
  IlcBitPacking::PackWord((UInt_t)0, w32,27,27);  //To make sure set the 27th bit to Zero so we can distinguish it from the EoE
}

//____________________________________________________________________________
void IlcRSTACKCpvRawWrite::WriteEoE(IlcFstream *ddl,UInt_t row,UInt_t dil,UInt_t wordCnt  )
{
  ...
  UInt_t w32=0;
  assert(0<=row&&row<=fNRow);
  IlcBitPacking::PackWord((UInt_t)row     ,w32,22,26);  // row number (1...24)
  assert(0<=dil&&dil<=fNDilogic);
  IlcBitPacking::PackWord((UInt_t)dil     ,w32,18,21);  // DILOGIC number (1...10)
  IlcBitPacking::PackWord(          e     ,w32, 7,17);  // event number -- not used
  IlcBitPacking::PackWord((UInt_t)wordCnt ,w32, 0, 6);  // word counter (0...47)
  IlcBitPacking::PackWord((UInt_t)1       ,w32,27,27);  // bit 27 is always 1 by definition of EoE
  ddl->WriteBuffer((char*)&w32,sizeof(w32));      
}

//____________________________________________________________________________
void IlcRSTACKCpvRawWrite::WriteRowMarker(IlcFstream *ddl,UInt_t size)
{
  ...
  IlcBitPacking::PackWord(size,  w32, 16,31);  //number of roaw written after row marker (digits and EoE)
  IlcBitPacking::PackWord(marker,w32,0,15);    //the marker word
  ddl->WriteBuffer((char*)&w32,sizeof(w32));              
}

//____________________________________________________________________________
void IlcRSTACKCpvRawWrite::WriteSegMarker(IlcFstream *ddl,UInt_t row, Int_t nwInSeg)
{
  ...
  IlcBitPacking::PackWord((UInt_t)2736   ,w32,20,31); //ab0 the segment marker word
  IlcBitPacking::PackWord((UInt_t)nwInSeg,w32, 8,19); //number of words in the segment
  IlcBitPacking::PackWord((UInt_t)(row/8),w32, 0, 7); //segment 0,1,2    
  ddl->WriteBuffer((char*)&w32,sizeof(w32)); 

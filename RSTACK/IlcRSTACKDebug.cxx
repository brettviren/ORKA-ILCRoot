...
/* $Id: IlcRSTACKebug.cxx 30520 2009-01-02 23:54:05Z fca $ */
...
//____________________________________________________________________________
IlcRSTACKv1::IlcRSTACKv1(IlcRSTACKReconstructor * Reconstructioner, const char *name, const char *title):
  IlcRSTACKv0(name,title)
{
  // ctor : title is used to identify the layout
  ...
  IlcRSTACKGeometry::GetInstance(title, "") ; 

  if (GetGeometry()->IsInitialized() ) 
    Info("IlcRSTACKv1", "IlcRSTACK %d : RSTACK geometry intialized for %s", Version(), GetGeometry()->GetName() );
  else
    Info("IlcRSTACKv1", "IlcRSTACK %d : RSTACK geometry initialization failed !", Version() ) ;   

  // Defining the RSTACK Reconstructioner
...
//____________________________________________________________________________
void IlcRSTACKv1::AddHit(Int_t shunt, Int_t primary, Int_t tracknumber, Int_t Id, Float_t * hits, Int_t trackpid, TLorentzVector p, Float_t * lpos)
{
  // Add a hit to the hit list.
  // A RSTACK hit is the sum of all hits in a single crystal
  ...
  newHit = new IlcRSTACKHit(shunt, primary, tracknumber, Id, hits, trackpid, p, lpos) ;

  for ( hitCounter = fNhits-1 ; hitCounter >= 0 && !deja ; hitCounter-- ) {
    curHit = (IlcRSTACKHit*) (*fHits)[hitCounter] ;
    if( *curHit == *newHit ) {
...
void IlcRSTACKv1::Hits2SDigits()
{
  ...
  Int_t j ; 
  IlcRSTACKHit   * hit ;
  IlcRSTACKDigit * newdigit ;
  IlcRSTACKDigit * curdigit ;
  Bool_t deja = kFALSE ; 
  ...
  for (itrack=0; itrack<gIlc->GetNtrack(); itrack++){
    ...
    for ( i = 0 ; i < fHits->GetEntries() ; i++ ) {
      hit = (IlcRSTACKHit*)fHits->At(i) ;
    
      // Assign primary number only if contribution is significant
      if( hit->GetEnergy() > fDigitThreshold)
	newdigit = new IlcRSTACKDigit( hit->GetPrimary(), hit->GetId(), Digitize( hit->GetEnergy() ) ) ;
      else
	newdigit = new IlcRSTACKDigit( -1               , hit->GetId(), Digitize( hit->GetEnergy() ) ) ;
      deja =kFALSE ;

      for ( j = 0 ; j < fnSdigits ;  j++) { 
	curdigit = (IlcRSTACKDigit*) fSDigits->At(j) ;
	if ( *curdigit == *newdigit) {
	  *curdigit = *curdigit + *newdigit ; 
      ...
      if ( !deja ) {
	new((*fSDigits)[fnSdigits]) IlcRSTACKDigit(* newdigit) ;
	fnSdigits++ ;  
      }
  ...
  for (i = 0 ; i < fnSdigits ; i++) { 
    IlcRSTACKDigit * digit = (IlcRSTACKDigit *) fSDigits->At(i) ; 
    digit->SetIndexInList(i) ;     
  }

  gIlc->TreeS()->Fill() ;
  gIlc->TreeS()->Write(0,TObject::kOverwrite) ;
 

}
//____________________________________________________________________________
void IlcRSTACKv1::SDigits2Digits()
{
  ...
  Int_t idCurSDigit = ((IlcRSTACKDigit *)fSDigits->At(0))->GetId() ;
  ...
  for(absID = 1; absID < GetGeometry()->GetNModules()*GetGeometry()->GetNPhi()*GetGeometry()->GetNZ(); absID++){
	...
	new((*fDigits)[fNdigits]) IlcRSTACKDigit( -1,absID,Digitize(noise) ) ;
	fNdigits++ ;  
      }
    }
    else{ //add noise and may be remove the true hit
      Float_t signal = noise + Calibrate(((IlcRSTACKDigit *)fSDigits->At(iCurSDigit))->GetAmp()) ;
      if( signal >fDigitThreshold ){
	IlcRSTACKDigit * digit = (IlcRSTACKDigit*) fSDigits->At(iCurSDigit) ;
	new((*fDigits)[fNdigits]) IlcRSTACKDigit( *digit ) ;
	((IlcRSTACKDigit *)fDigits->At(fNdigits))->SetAmp(Digitize(signal));
	fNdigits++ ;  
      }

      if(iCurSDigit < fSDigits->GetEntries()-1){
	iCurSDigit++ ;
	idCurSDigit = ((IlcRSTACKDigit*)fSDigits->At(iCurSDigit))->GetId() ;
      }
      else
  ...
  for(idigit = iCurSDigit; idigit < fSDigits->GetEntries() ; idigit++){  //loop over CPV/PPSD digits
    ...
    IlcRSTACKDigit * digit = (IlcRSTACKDigit *) fSDigits->At(idigit) ; 
  ...
  for (i = 0 ; i < fNdigits ; i++) { 
    IlcRSTACKDigit * digit = (IlcRSTACKDigit *) fDigits->At(i) ; 
    digit->SetIndexInList(i) ;     
  }

  gIlc->TreeD()->Fill() ;

  gIlc->TreeD()->Write(0,TObject::kOverwrite) ;
 
}

//___________________________________________________________________________
void IlcRSTACKv1::MakeBranch(Option_t* opt, char *file)
{ 
  // Called by IlcRun
  ...
  if( cH ){
    ...
    fnSdigits = 0 ;
    gIlc->MakeBranchInTree(gIlc->TreeS(),branchname,&fSDigits,fBufferSize,file);  
  }
  ...
  if( cH ){
    ...
    gIlc->MakeBranchInTree(gIlc->TreeD(),branchname,&fDigits,fBufferSize,file);  
  }
  ...
  if( cH ){
    ...
    if ( fEmcRecPoints && gIlc->TreeR() ) {
      sprintf(branchname,"%sEmcRP",GetName()) ;
      gIlc->MakeBranchInTree(gIlc->TreeR(),branchname,"TObjArray",&fEmcRecPoints, fBufferSize, splitlevel,file); 
    }

    if(fPpsdRecPoints)
      fPpsdRecPoints->Delete() ; 

    if ( fPpsdRecPoints && gIlc->TreeR() ) {
      sprintf(branchname,"%sPpsdRP",GetName()) ;
      gIlc->MakeBranchInTree(gIlc->TreeR(),branchname,"TObjArray",&fPpsdRecPoints, fBufferSize, splitlevel,file); 
    }

    if(fTrackSegments)
      fTrackSegments->Clear() ; 
    
    if ( fTrackSegments && gIlc->TreeR() ) { 
      sprintf(branchname,"%sTS",GetName()) ;
      gIlc->MakeBranchInTree(gIlc->TreeR(),branchname,&fTrackSegments,fBufferSize,file);
    }
    
    if(fRecParticles)
      fRecParticles->Clear() ; 
    
    if ( fRecParticles && gIlc->TreeR() ) { 
      sprintf(branchname,"%sRP",GetName()) ;
      gIlc->MakeBranchInTree(gIlc->TreeR(),branchname,&fRecParticles,fBufferSize,file); 
    }
...
void IlcRSTACKv1::Reconstruction(IlcRSTACKReconstructor * Reconstructioner)
{ 
  ...
  gIlc->TreeR()->Write(0,TObject::kOverwrite) ;
...
//____________________________________________________________________________
void IlcRSTACKv1::ResetReconstruction() 
{ 
  // Deleting reconstructed objects
...
void IlcRSTACKv1::StepManager(void)
{
  ...
  Int_t tracknumber =  gIlc->GetCurrentTrackNumber() ; 
  Int_t primary     =  gIlc->GetPrimary( gIlc->GetCurrentTrackNumber() ); 
  ...
  if ( name == "IHEP" ) {       // ======> CPV is a IHEP's one
	...
	gMC->TrackCharge() != 0) {      
      ...
      TClonesArray *cpvDigits = new TClonesArray("IlcRSTACKCPVDigit",0);   // array of digits for current hit
	...
	IlcRSTACKCPVDigit  *cpvDigit1 = (IlcRSTACKCPVDigit*) cpvDigits->UncheckedAt(idigit);
	Float_t x1 = cpvDigit1->GetXpad() ;
	Float_t z1 = cpvDigit1->GetYpad() ;
	for (Int_t jdigit=idigit+1; jdigit<ndigits; jdigit++) {
	  IlcRSTACKCPVDigit  *cpvDigit2 = (IlcRSTACKCPVDigit*) cpvDigits->UncheckedAt(jdigit);
	...
	IlcRSTACKCPVDigit  *cpvDigit = (IlcRSTACKCPVDigit*) cpvDigits->UncheckedAt(idigit);
	relid[0] = moduleNumber + 1 ;                             // CPV (or RSTACK) module number
	relid[1] =-1 ;                                            // means CPV
  ...
  if(gMC->CurrentVolID(copy) == gMC->VolId("PXTL") ) { //  We are inside a PBWO crystal
    ...
    if ( (xyze[3] != 0) || entered ) {  // Track is inside the crystal and deposits some energy or just entered 
      ...
      gMC->CurrentVolOffID(10, relid[0]) ; // get the RSTACK module number ;
  ...
  } // we are inside a RSTACK Xtal


}

//____________________________________________________________________________
void IlcRSTACKv1::CPVDigitize (TLorentzVector p, Float_t *zxhit, Int_t moduleNumber, TClonesArray *cpvDigits)
{
  // ------------------------------------------------------------------------
  ...
  for (Int_t iter=0; iter<nIter; iter++) {
	...
	new(ldigits[cpvDigits->GetEntriesFast()]) IlcRSTACKCPVDigit(kxg,kzg,qpad);
      }
    }
  }
}
...
//____________________________________________________________________________
Float_t IlcRSTACKv1::CPVPadResponseFunction(Float_t qhit, Float_t zhit, Float_t xhit) {
  // ------------------------------------------------------------------------
...
//____________________________________________________________________________
Double_t IlcRSTACKv1::CPVCumulPadResponse(Double_t x, Double_t y) {
  // ------------------------------------------------------------------------
  // Cumulative pad response function

 ...
 **************************************************************************/
/* $Id: IlcRSTACKTracker.cxx 43671 2010-09-19 18:45:08Z kharlov $ */
...
ClassImp(IlcRSTACKTracker)

Bool_t IlcRSTACKTracker::fgDebug = kFALSE ;  

//____________________________________________________________________________
IlcRSTACKTracker::IlcRSTACKTracker(): 
  IlcTracker()
{
  //--------------------------------------------------------------------
  ...
  //--------------------------------------------------------------------
  for (Int_t i=0; i<5; i++) 
      fModules[i]=new TClonesArray("IlcRSTACKEmcRecPoint",777);
...
//____________________________________________________________________________
Int_t IlcRSTACKTracker::LoadClusters(TTree *cTree) {
  //--------------------------------------------------------------------
  // This function loads the RSTACK clusters
  //--------------------------------------------------------------------

  TObjArray *arr=NULL;
  TBranch *branch=cTree->GetBranch("RSTACKEmcRP");
  if (branch==0) {
  ...
  for (Int_t i=0; i<nentr; i++) {
    ...
    while (ncl--) {
      IlcRSTACKEmcRecPoint *cl=(IlcRSTACKEmcRecPoint*)arr->UncheckedAt(ncl);

      Int_t m=cl->GetRSTACKMod();
      if ((m<1)||(m>5)) {
...
//      if (!cl->Misalign()) IlcWarning("Can't misalign this cluster !");
      ...
      new (module[idx]) IlcRSTACKEmcRecPoint(*cl); 
...
//____________________________________________________________________________
Int_t IlcRSTACKTracker::PropagateBack(IlcESDEvent *esd) {
  //--------------------------------------------------------------------
  // Called by IlcReconstruction 
  // Performs the track matching with the RSTACK modules
  // Makes the PID
  //--------------------------------------------------------------------
...
//     if ((status & IlcESDtrack::kTRDrefit) == 1) continue;
     ...
     //Loop over RSTACK modules
     Double_t dx=0,dz=0;
     Double_t minDistance=999. ;
     Int_t emcIndex=0  ;
     for(Int_t mod=1; mod<=5; mod++){
       if(fModules[mod-1]->GetEntriesFast()==0) //no RSTACK clusters in this module or module does not exist
         continue ;
       
       //Approximate direction to the current RSTACK module
      ...
      //t.CorrectForMaterial(...); // Correct for the TOF material, if needed
         ...
         for(Int_t icl=0;icl<fModules[mod-1]->GetEntriesFast();icl++){
           IlcRSTACKEmcRecPoint * clu =static_cast<IlcRSTACKEmcRecPoint*>(fModules[mod-1]->At(icl)) ;
...
//____________________________________________________________________________
IlcCluster *IlcRSTACKTracker::GetCluster(Int_t index) const {
  //--------------------------------------------------------------------
...
//____________________________________________________________________________
void IlcRSTACKTracker::UnloadClusters() {
  //--------------------------------------------------------------------
  // This function unloads the RSTACK clusters
  //--------------------------------------------------------------------

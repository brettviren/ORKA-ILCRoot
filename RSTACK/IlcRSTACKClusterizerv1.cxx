...
/* $Id: IlcRSTACKClusterizerv1.cxx 51445 2011-09-06 18:02:04Z hristov $ */

/* History of cvs commits:
 *
 * $Log: IlcRSTACKClusterizerv1.cxx,v $
 ...
 * Bug fix for attempt to use IlcRSTACKEmcRecPoint after its deletion
 *
 ...
 * Mode protection against absence of calib.data from IlcRSTACKCalibData to IlcRSTACKClusterizerv1::GetCalibrationParameters()
 *
 * Revision 1.104  2007/04/27 16:55:53  kharlov
 * Calibration stops if RSTACK CDB objects do not exist
 *
 ...
 * Introduce IlcDebug and IlcLog (B.Polichtchouk)
 *
 ...
 * Update of the RSTACK code needed in order to read and reconstruct the beam test raw data (i.e. without an existing gilc.root)
 *
 ...
 * 1. Adding a possibility to read and reconstruct an old rcu formatted raw data. This is controlled by an option of IlcReconstruction and IlcRSTACKReconstructor. 2. In case of raw data processing (without gilc.root) create the default IlcRSTACKGeometry object. Most likely this should be moved to the CDB
 *
 ...
 * Add fEnergy to IlcRSTACKDigit and operate with EMC amplitude in energy units (Yu.Kharlov)
 *
...
//  Results are stored in TreeR#, branches RSTACKEmcRP (EMC recPoints),
//  RSTACKCpvRP (CPV RecPoints) and IlcRSTACKClusterizer (Clusterizer with all 
...
//  standalone mode.
// Use Case:
//  root [0] IlcRSTACKClusterizerv1 * cl = new IlcRSTACKClusterizerv1(<pointer_to_RSTACK_geometry_onject>)  
...
ClassImp(IlcRSTACKClusterizerv1)
  
//____________________________________________________________________________
IlcRSTACKClusterizerv1::IlcRSTACKClusterizerv1() :
  IlcRSTACKClusterizer(),
...
//____________________________________________________________________________
IlcRSTACKClusterizerv1::IlcRSTACKClusterizerv1(IlcRSTACKGeometry *geom) :
  IlcRSTACKClusterizer(geom),
  fDefaultInit(0),            fEmcCrystals(0),          fToUnfold(0),
  fWrite(0),                
  fNumberOfEmcClusters(0),    fNumberOfCpvClusters(0),
  fEmcClusteringThreshold(0), fCpvClusteringThreshold(0), 
...
//____________________________________________________________________________
void IlcRSTACKClusterizerv1::Digits2Clusters(Option_t *option)
{
  ...
  if(strstr(option,"tim"))
    gBenchmark->Start("RSTACKClusterizer"); 
  ...
  IlcDebug(2,Form(" ---- Printing clusters (%d)\n",
  ...
  if(strstr(option,"tim")){
    gBenchmark->Stop("RSTACKClusterizer");
    IlcInfo(Form("took %f seconds for Clusterizing\n",
		 gBenchmark->GetCpuTime("RSTACKClusterizer"))); 
  }
...
//____________________________________________________________________________
Bool_t IlcRSTACKClusterizerv1::FindFit(IlcRSTACKEmcRecPoint * emcRP, IlcRSTACKDigit ** maxAt, Float_t * maxAtEnergy,
...
{ 
  ...
  gMinuit->SetFCN(IlcRSTACKClusterizerv1::UnfoldingChiSquare) ;  
  ...
  for(iDigit = 0; iDigit < nDigits; iDigit++){
    ...
    index++ ;   
    if(ierflg != 0){ 
      Warning("FindFit", "RSTACK Unfolding unable to set initial value for fit procedure : x = %f\n", x ) ;
      return kFALSE;
    }
    gMinuit->mnparm(index, "z",  z, 0.1, 0, 0, ierflg) ;
    index++ ;   
    if(ierflg != 0){
       Warning("FindFit", "RSTACK Unfolding unable to set initial value for fit procedure : z =%f\n", z ) ;
      return kFALSE;
    }
    gMinuit->mnparm(index, "Energy",  energy , 0.05*energy, 0., 4.*energy, ierflg) ;
    index++ ;   
    if(ierflg != 0){
      Warning("FindFit", "RSTACK Unfolding unable to set initial value for fit procedure : energy = %f\n", energy ) ;      
      return kFALSE;
    }
  }
  ...
  if(ierflg == 4){  // Minimum not found   
    Warning("FindFit", "RSTACK Unfolding fit not converged, cluster abandoned\n" );      
    return kFALSE ;
  }            
...
//____________________________________________________________________________
void IlcRSTACKClusterizerv1::Init()
{
  // Make all memory allocations which can not be done in default constructor.
  // Attach the Clusterizer task to the list of RSTACK tasks
...
//____________________________________________________________________________
void IlcRSTACKClusterizerv1::InitParameters()
{

  fNumberOfCpvClusters     = 0 ; 
  fNumberOfEmcClusters     = 0 ; 

  const IlcRSTACKRecoParam* recoParam = IlcRSTACKReconstructor::GetRecoParam();
  if(!recoParam) IlcFatal("Reconstruction parameters are not set!");
...
//____________________________________________________________________________
Int_t IlcRSTACKClusterizerv1::AreNeighbours(IlcRSTACKDigit * d1, IlcRSTACKDigit * d2)const
{
  ...
  if ( (relid1[0] == relid2[0]) && (relid1[1]==relid2[1]) ) { // inside the same RSTACK module 
...
//____________________________________________________________________________
Bool_t IlcRSTACKClusterizerv1::CheckTimeGate(Float_t t1, Float_t amp1, Float_t t2, Float_t amp2)const{
...
//____________________________________________________________________________
Bool_t IlcRSTACKClusterizerv1::IsInEmc(IlcRSTACKDigit * digit) const
{
...
//____________________________________________________________________________
Bool_t IlcRSTACKClusterizerv1::IsInCpv(IlcRSTACKDigit * digit) const
{
...
//____________________________________________________________________________
void IlcRSTACKClusterizerv1::WriteRecPoints()
{
  ...
  Float_t emcMinE= IlcRSTACKReconstructor::GetRecoParam()->GetEMCMinE(); //Minimal digit energy
  TVector3 fakeVtx(0.,0.,0.) ;
  for(index = 0; index < nEmc; index++){
    IlcRSTACKEmcRecPoint * rp =
      static_cast<IlcRSTACKEmcRecPoint *>( fEMCRecPoints->At(index) );
    rp->Purify(emcMinE) ;
  ...
  for(index = 0; index < fEMCRecPoints->GetEntries(); index++){
    static_cast<IlcRSTACKEmcRecPoint *>( fEMCRecPoints->At(index) )->SetIndexInList(index) ;
  }
  ...
  for(index = 0; index < fCPVRecPoints->GetEntries(); index++){
    IlcRSTACKCpvRecPoint * rp = static_cast<IlcRSTACKCpvRecPoint *>( fCPVRecPoints->At(index) );
    rp->EvalAll(fDigitsArr) ;
  ...
  for(index = 0; index < fCPVRecPoints->GetEntries(); index++)
    static_cast<IlcRSTACKCpvRecPoint *>( fCPVRecPoints->At(index) )->SetIndexInList(index) ;
...
//____________________________________________________________________________
void IlcRSTACKClusterizerv1::MakeClusters()
{
  ...
  const Int_t maxNDigits = 3584; // There is no clusters larger than RSTACK module ;)
  ...
  IlcRSTACKRecPoint * clu = 0 ; 
  for(Int_t i=0; i<nDigits; i++){
    if(fDigitsUsed[i])
      continue ;

    digit=static_cast<IlcRSTACKDigit*>(fDigitsArr->At(i)) ;
    ...
    if (( IsInEmc(digit) &&  Calibrate(digit->GetEnergy(),digit->GetId()) > fEmcClusteringThreshold ) || 
      ...
      if  ( IsInEmc(digit) ) {   
        ...
        fEMCRecPoints->AddAt(new  IlcRSTACKEmcRecPoint(""), fNumberOfEmcClusters) ;
        clu = static_cast<IlcRSTACKEmcRecPoint *>( fEMCRecPoints->At(fNumberOfEmcClusters) ) ; 
	...
	clu->AddDigit(*digit, Calibrate(digit->GetEnergy(),digit->GetId()),CalibrateT(digit->GetTime(),digit->GetId())) ;
        ...
        fCPVRecPoints->AddAt(new IlcRSTACKCpvRecPoint(""), fNumberOfCpvClusters) ;
        clu =  static_cast<IlcRSTACKCpvRecPoint *>( fCPVRecPoints->At(fNumberOfCpvClusters) ) ;  
      ...
      IlcRSTACKDigit * digitN ; 
      index = 0 ;
      while (index < iDigitInCluster){ // scan over digits already in cluster 
        digit =  static_cast<IlcRSTACKDigit*>( fDigitsArr->At(clusterdigitslist[index]) )  ;      
        index++ ; 
        for(Int_t j=iFirst; j<nDigits; j++){
	  if (iDigitInCluster >= maxNDigits) {
	    IlcError(Form("The number of digits in cluster is more than %d, skip the rest of event",
			  maxNDigits));
	    return;
	  }
          if(fDigitsUsed[j]) 
            continue ;        //look through remaining digits
          digitN = static_cast<IlcRSTACKDigit*>( fDigitsArr->At(j) ) ;
...
//____________________________________________________________________________
void IlcRSTACKClusterizerv1::MakeUnfolding()
{
  ...
  if(fNumberOfEmcClusters > 0){
    ...
    for(index = 0 ; index < numberofNotUnfolded ; index++){
      ...
      IlcRSTACKEmcRecPoint * emcRecPoint = static_cast<IlcRSTACKEmcRecPoint *>( fEMCRecPoints->At(index) ) ;
      if(emcRecPoint->GetRSTACKMod()> nModulesToUnfold)
        break ;
      
      Int_t nMultipl = emcRecPoint->GetMultiplicity() ; 
      IlcRSTACKDigit ** maxAt = new IlcRSTACKDigit*[nMultipl] ;
  ...
  if(fNumberOfCpvClusters > 0){
    ...
    for(index = 0 ; index < numberofCpvNotUnfolded ; index++){
      ...
      IlcRSTACKRecPoint * recPoint = static_cast<IlcRSTACKRecPoint *>( fCPVRecPoints->At(index) ) ;

      if(recPoint->GetRSTACKMod()> nModulesToUnfold)
        break ;
      
      IlcRSTACKEmcRecPoint * emcRecPoint = static_cast<IlcRSTACKEmcRecPoint*>(recPoint) ; 
...
//____________________________________________________________________________
Double_t  IlcRSTACKClusterizerv1::ShowerShape(Double_t x, Double_t z)
{ 
  // Shape of the shower (see RSTACK TDR)
...
//____________________________________________________________________________
void  IlcRSTACKClusterizerv1::UnfoldCluster(IlcRSTACKEmcRecPoint * iniEmc, 
                                          Int_t nMax, 
                                          IlcRSTACKDigit ** maxAt, 
                                          Float_t * maxAtEnergy)
{ 
  ...
  Int_t iDigit ;
  for(iDigit = 0 ; iDigit < nDigits ; iDigit ++){
    digit = static_cast<IlcRSTACKDigit*>( fDigitsArr->At(emcDigits[iDigit] ) ) ;   
    fGeom->AbsToRelNumbering(digit->GetId(), relid) ;
...
//      efit[iDigit] += epar * ShowerShape(xDigit - xpar,zDigit - zpar,vIncid) ;
  ...
  while(iparam < nPar ){
    ...
    epar = fitparameters[iparam+2] ;
    iparam += 3 ;    
//    fGeom->GetIncidentVector(fVtx,iniEmc->GetRSTACKMod(),xpar,zpar,vIncid) ;

    IlcRSTACKEmcRecPoint * emcRP = 0 ;  
    ...
    if(iniEmc->IsEmc()){ //create new entries in fEMCRecPoints...
      ...
      (*fEMCRecPoints)[fNumberOfEmcClusters] = new IlcRSTACKEmcRecPoint("") ;
      emcRP = static_cast<IlcRSTACKEmcRecPoint *>( fEMCRecPoints->At(fNumberOfEmcClusters) ) ;
      fNumberOfEmcClusters++ ;
      ...
      (*fCPVRecPoints)[fNumberOfCpvClusters] = new IlcRSTACKCpvRecPoint("") ;
      emcRP = static_cast<IlcRSTACKEmcRecPoint *>( fCPVRecPoints->At(fNumberOfCpvClusters) ) ;
      fNumberOfCpvClusters++ ;
    }
...
void IlcRSTACKClusterizerv1::UnfoldingChiSquare(Int_t & nPar, Double_t * Grad, Double_t & fret, Double_t * x, Int_t iflag)
{
  ...
  IlcRSTACKEmcRecPoint * emcRP = static_cast<IlcRSTACKEmcRecPoint*>( toMinuit->At(0) )  ;
  TClonesArray * digits = static_cast<TClonesArray*>( toMinuit->At(1) )  ;
  // A bit buggy way to get an access to the geometry
  // To be revised!
  IlcRSTACKGeometry *geom = static_cast<IlcRSTACKGeometry *>(toMinuit->At(2));

//  TVector3 * vtx = static_cast<TVector3*>(toMinuit->At(3)) ;  //Vertex position
  
  //  IlcRSTACKEmcRecPoint * emcRP = static_cast<IlcRSTACKEmcRecPoint *>( gMinuit->GetObjectFit() ) ; // EmcRecPoint to fit
...
//  TVector3 vInc ;
  ...
  for( iDigit = 0 ; iDigit < nOdigits ; iDigit++) {
    ...
    digit = static_cast<IlcRSTACKDigit*>( digits->At( emcDigits[iDigit] ) ); 
    ...
    geom->RelPosInModule(relid, xDigit, zDigit) ;
     ...
     if(iflag == 2){  // calculate gradient
       ...
       while(iParam < nPar ){
         ...
         iParam++ ;          
//         fGeom->GetIncidentVector(*vtx,emcRP->GetRSTACKMod(),x[iParam-2],x[iParam-1],vInc) ;
...
//         fGeom->GetIncidentVector(*vtx,emcRP->GetRSTACKMod(),xpar,zpar,vInc) ;
...
//____________________________________________________________________________
void IlcRSTACKClusterizerv1::Print(const Option_t *)const
{
  ...
  if( strcmp(GetName(), "") !=0 ) {  
    ...
    message = " IlcRSTACKClusterizerv1 not initialized " ;
  
  IlcInfo(Form("%s, %s %s %s %s %f %f %f %f %f %f", message.Data(),  
       taskName.Data(), 
       GetTitle(),
       taskName.Data(), 
       GetName(), 
...
//____________________________________________________________________________
void IlcRSTACKClusterizerv1::PrintRecPoints(Option_t * option)
{
  // Prints list of RecPoints produced at the current pass of IlcRSTACKClusterizer
  ...
  if(strstr(option,"all")) {
    ...
    Int_t index ;
    for (index = 0 ; index < fEMCRecPoints->GetEntries() ; index++) {
      IlcRSTACKEmcRecPoint * rp = (IlcRSTACKEmcRecPoint * )fEMCRecPoints->At(index) ; 
      TVector3  locpos;  
      ...
      printf("\n%6d  %8.2f  %3d     %2d     %4.1f   %4.1f   %4.1f   %4f  %4f    %2d     : ", 
	      rp->GetIndexInList(), rp->GetEnergy(), rp->GetMultiplicity(), rp->GetRSTACKMod(), 
    ...
    for (index = 0 ; index < fCPVRecPoints->GetEntries() ; index++) {
      IlcRSTACKCpvRecPoint * rp = (IlcRSTACKCpvRecPoint * )fCPVRecPoints->At(index) ; 
      
      TVector3  locpos;  
      rp->GetLocalPosition(locpos);
      
      printf("\n%6d  %8.2f  %2d     %4.1f    %4.1f %4.1f \n", 
	     rp->GetIndexInList(), rp->GetEnergy(), rp->GetRSTACKMod(), 
...
//____________________________________________________________________________
void IlcRSTACKClusterizerv1::SetDistancesToBadChannels()
{
  ...
  TMatrixF gmat;
  TVector3 gposRecPoint; // global (in ILC frame) position of rec. point
  ...
  for(Int_t iRP=0; iRP<fEMCRecPoints->GetEntries(); iRP++){
    rp = (IlcRSTACKEmcRecPoint*)fEMCRecPoints->At(iRP);
    //evaluate distance to border
    relid[0]=rp->GetRSTACKMod() ;
    relid[2]=1 ;
    relid[3]=1 ;
    ...
    for(Int_t iBad=0; iBad<fgCalibData->GetNumOfEmcBadChannels(); iBad++) {
      ...
      if(relid[0]!=rp->GetRSTACKMod()) //We can not evaluate global position directly since 
        continue ;                   //bad channels can be in the module which does not exist in simulations.
      rp->GetGlobalPosition(gposRecPoint,gmat);
      fGeom->RelPosInIlc(badIds[iBad],gposBadChannel);
      IlcDebug(2,Form("BC position:[%.3f,%.3f,%.3f], RP position:[%.3f,%.3f,%.3f]. E=%.3f\n",
...
//==================================================================================
Float_t IlcRSTACKClusterizerv1::Calibrate(Float_t amp, Int_t absId) const{
...
//==================================================================================
Float_t IlcRSTACKClusterizerv1::CalibrateT(Float_t time, Int_t absId)const{

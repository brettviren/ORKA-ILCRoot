#include "IlcRSTACKDA2.h"
#include "TString.h"

ClassImp(IlcRSTACKDA2)

//----------------------------------------------------------------
IlcRSTACKDA2::IlcRSTACKDA2(int module) : TNamed(),
 fHistoFile(0),fFiredCells(0),fMod(module)

{
  // Create IlcRSTACKDA2 ("Bad channels finder") object.
  // module is the RSTACK module number (0..4).
  // Quality histogram names: module_iX_iZ_gain.
  // Root file name: RSTACK_ModuleX_BCM.root, where X - module number.
  
  char name[128];
  TString sname="RSTACK_Module%d_BCM";
  snprintf(name,sname.Length(),sname.Data(),fMod);
  SetName(name);

  SetTitle("Detector Algorithm to check for RSTACK channels quality");
...
//----------------------------------------------------------------
IlcRSTACKDA2::IlcRSTACKDA2(Int_t module, TObjArray* oldHistos) : TNamed(),
 fHistoFile(0),fFiredCells(0),fMod(module)
...
//-------------------------------------------------------------------
IlcRSTACKDA2::IlcRSTACKDA2(const IlcRSTACKDA2& da) : TNamed(da),
...
//-------------------------------------------------------------------
IlcRSTACKDA2& IlcRSTACKDA2::operator= (const IlcRSTACKDA2& da)
{
  //Assignment operator.
...
//-------------------------------------------------------------------
void IlcRSTACKDA2::FillQualityHistograms(Float_t quality[64][56][2]) 
{
...
//-------------------------------------------------------------------
void  IlcRSTACKDA2::FillFiredCellsHistogram(Int_t nCells)
{
  fFiredCells->Fill(nCells);
}

//-------------------------------------------------------------------
void IlcRSTACKDA2::UpdateHistoFile()
{

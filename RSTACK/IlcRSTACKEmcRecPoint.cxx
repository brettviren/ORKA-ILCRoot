...
/* $Id: IlcRSTACKEmcRecPoint.cxx 47523 2011-02-17 14:35:05Z policheh $ */
...
//  RecPoint implementation for RSTACK-EMC 
...
ClassImp(IlcRSTACKEmcRecPoint)

//____________________________________________________________________________
IlcRSTACKEmcRecPoint::IlcRSTACKEmcRecPoint() : 
  IlcRSTACKRecPoint(),
...
//____________________________________________________________________________
IlcRSTACKEmcRecPoint::IlcRSTACKEmcRecPoint(const char * opt) : 
  IlcRSTACKRecPoint(opt),
  fCoreEnergy(0.), fDispersion(0.),
  fEnergyList(0), fTime(-1.), fNExMax(0),
  fM2x(0.), fM2z(0.), fM3x(0.), fM4z(0.),
  fPhixe(0.), fDistToBadCrystal(-1), fDebug(0)
...
//____________________________________________________________________________
IlcRSTACKEmcRecPoint::IlcRSTACKEmcRecPoint(const IlcRSTACKEmcRecPoint & rp) : 
  IlcRSTACKRecPoint(rp),
...
//____________________________________________________________________________
void IlcRSTACKEmcRecPoint::AddDigit(IlcRSTACKDigit & digit, Float_t Energy, Float_t time)
{
...
//____________________________________________________________________________
Bool_t IlcRSTACKEmcRecPoint::AreNeighbours(IlcRSTACKDigit * digit1, IlcRSTACKDigit * digit2 ) const
{
  ...
  Int_t relid1[4] ; 
  RSTACKgeom->AbsToRelNumbering(digit1->GetId(), relid1) ; 

  Int_t relid2[4] ; 
  RSTACKgeom->AbsToRelNumbering(digit2->GetId(), relid2) ; 
...
//____________________________________________________________________________
Int_t IlcRSTACKEmcRecPoint::Compare(const TObject * obj) const
{
  // Compares two RecPoints according to their position in the RSTACK modules
  
  const Float_t delta = 1 ; //Width of "Sorting row". If you changibg this 
                      //value (what is senseless) change as vell delta in
                      //IlcRSTACKTrackSegmentMakerv* and other RecPoints...
  Int_t rv ; 

  IlcRSTACKEmcRecPoint * clu = (IlcRSTACKEmcRecPoint *)obj ; 

 
  Int_t RSTACKmod1 = GetRSTACKMod() ;
  Int_t RSTACKmod2 = clu->GetRSTACKMod() ;
...
void IlcRSTACKEmcRecPoint::ExecuteEvent(Int_t event, Int_t, Int_t) /*const*/
{
  ...
  IlcRSTACKLoader* RSTACKLoader = dynamic_cast<IlcRSTACKLoader*>(rn->GetLoader("RSTACKLoader"));
  if (RSTACKLoader == 0x0) 
    {
      IlcError(Form("Cannot find RSTACK Loader from Run Loader"));
      return;
    }
  ...
  case kButton1Down: {
    ...
    const Int_t kMulDigit = IlcRSTACKEmcRecPoint::GetDigitsMultiplicity() ; 
    ...
    for(iDigit=0; iDigit<kMulDigit; iDigit++) {
      digit = (IlcRSTACKDigit *) digits->At(fDigitsList[iDigit])  ;
    ...
    ximax += RSTACKgeom->GetCrystalSize(0) / 2. ;
    zimax += RSTACKgeom->GetCrystalSize(2) / 2. ;
    ximin -= RSTACKgeom->GetCrystalSize(0) / 2. ;
    zimin -= RSTACKgeom->GetCrystalSize(2) / 2. ;
    Int_t xdim = (int)( (ximax - ximin ) / RSTACKgeom->GetCrystalSize(0) + 0.5  ) ; 
    Int_t zdim = (int)( (zimax - zimin ) / RSTACKgeom->GetCrystalSize(2) + 0.5 ) ;
...
//____________________________________________________________________________
void  IlcRSTACKEmcRecPoint::EvalDispersion(Float_t logWeight,TClonesArray * digits, TVector3 & /* vInc */)
{
 ...
 // Calculates the center of gravity in the local RSTACK-module coordinates 
...
//______________________________________________________________________________
void IlcRSTACKEmcRecPoint::EvalCoreEnergy(Float_t logWeight, Float_t coreRadius, TClonesArray * digits)
{
...
//____________________________________________________________________________
void  IlcRSTACKEmcRecPoint::EvalElipsAxis(Float_t logWeight,TClonesArray * digits, TVector3 & /* vInc */)
{
...
//   Double_t CosZ ;
//   IlcRSTACKGeometry * RSTACKgeom = IlcRSTACKGeometry::GetInstance() ;
//   Double_t DistanceToIP= (Double_t ) RSTACKgeom->GetIPtoCrystalSurface() ;
...
//____________________________________________________________________________
void  IlcRSTACKEmcRecPoint::EvalMoments(Float_t logWeight,TClonesArray * digits, TVector3 & /* vInc */)
{
  ...
  for(iDigit=0; iDigit<fMulDigit; iDigit++) {
    digit = (IlcRSTACKDigit *) digits->At(fDigitsList[iDigit])  ;
    RSTACKgeom->AbsToRelNumbering(digit->GetId(), relid) ;
    RSTACKgeom->RelPosInModule(relid, xiRSTACK, ziRSTACK);
    xi    = xiRSTACK*cosPhi + ziRSTACK*sinPhi;
    zi    = ziRSTACK*cosPhi - xiRSTACK*sinPhi;
    if (fAmp>0 && fEnergyList[iDigit]>0) {
      w     = TMath::Max(0.,logWeight+TMath::Log(fEnergyList[iDigit]/fAmp ) ) ;
      x    += w * xi ;
      z    += w * zi ; 
...
//______________________________________________________________________________
void  IlcRSTACKEmcRecPoint::EvalPrimaries(TClonesArray * digits)
{
  ...
  digit = static_cast<IlcRSTACKDigit *>(digits->At( fDigitsList[imaxDigit] )) ; 
  ...
  for (Int_t index = 0 ; index < GetDigitsMultiplicity() ; index++ ) { // all digits
    if(index==imaxDigit) //already in
      continue ; 
    digit = static_cast<IlcRSTACKDigit *>(digits->At( fDigitsList[index] )) ; 
...
//____________________________________________________________________________
void IlcRSTACKEmcRecPoint::EvalAll(TClonesArray * digits )
{
//   EvalCoreEnergy(logWeight, digits);
  EvalTime(digits) ;
  EvalPrimaries(digits) ;
  IlcRSTACKRecPoint::EvalAll(digits) ;
}
//____________________________________________________________________________
void IlcRSTACKEmcRecPoint::EvalAll(Float_t logWeight, TVector3 &vtx, TClonesArray * digits )
{
...
//____________________________________________________________________________
void IlcRSTACKEmcRecPoint::EvalLocalPosition(Float_t logWeight, TVector3 &vtx, TClonesArray * digits, TVector3 &vInc)
{
  // Calculates the center of gravity in the local RSTACK-module coordinates 
  Float_t wtot = 0. ;
  ...
  RSTACKgeom->GetIncidentVector(vtx,GetRSTACKMod(),x,z,vInc) ;
...
//____________________________________________________________________________
Float_t IlcRSTACKEmcRecPoint::GetMaximalEnergy(void) const
{
...
//____________________________________________________________________________
Int_t IlcRSTACKEmcRecPoint::GetMultiplicityAtLevel(Float_t H) const
{
...
//____________________________________________________________________________
Int_t  IlcRSTACKEmcRecPoint::GetNumberOfLocalMax( IlcRSTACKDigit **  maxAt, Float_t * maxAtEnergy,
...
{ 
  ...
  for(iDigit = 0; iDigit < fMulDigit; iDigit++)
    maxAt[iDigit] = (IlcRSTACKDigit*) digits->At(fDigitsList[iDigit])  ;
  ...
  for(iDigit = 0 ; iDigit < fMulDigit; iDigit++) {   
	...
	digitN = (IlcRSTACKDigit *) digits->At(fDigitsList[iDigitN]) ; 
...
//____________________________________________________________________________
void IlcRSTACKEmcRecPoint::EvalTime(TClonesArray * /*digits*/)
{
...
/*
  ...
  fTime = ((IlcRSTACKDigit*) digits->At(fDigitsList[maxAt]))->GetTime() ;
*/  
}
//____________________________________________________________________________
void IlcRSTACKEmcRecPoint::Purify(Float_t threshold){
...
//____________________________________________________________________________
void IlcRSTACKEmcRecPoint::Print(Option_t *) const
{
  // Print the list of digits belonging to the cluster
  
  TString message ; 
  message  = "IlcRSTACKEmcRecPoint:\n" ;

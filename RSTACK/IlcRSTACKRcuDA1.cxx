#include "IlcRSTACKRcuDA1.h"
#include "TString.h"

ClassImp(IlcRSTACKRcuDA1)

//----------------------------------------------------------------
IlcRSTACKRcuDA1::IlcRSTACKRcuDA1(Int_t module, Int_t rcu) : TNamed(),
...

{
  ...
  // Root file name: RSTACK_ModuleX_RCUY_Calib.root, where X - module number,
  // Y - RCU number. If no RCU specified (rcu<0), file name is simply
  // RSTACK_ModuleX_Calib.root.
  
  char name[128]; TString sname;

  if(rcu<0) { 
    sname="RSTACK_Module%d_Calib";
    snprintf(name,sname.Length(),sname.Data(),fMod);
  }
  else {
    sname="RSTACK_Module%d_RCU%d_Calib";
...
//-------------------------------------------------------------------
IlcRSTACKRcuDA1::IlcRSTACKRcuDA1(Int_t module, Int_t rcu, TObjArray* oldTimeEnergy) :
...
//-------------------------------------------------------------------
IlcRSTACKRcuDA1::~IlcRSTACKRcuDA1()
{
  // Destructor
...
//-------------------------------------------------------------------
void IlcRSTACKRcuDA1::FillHistograms(Float_t e[64][56][2], Float_t t[64][56][2]) 
{
...
//-------------------------------------------------------------------
void IlcRSTACKRcuDA1::UpdateHistoFile()
{
...
//-----------------------------------------------------------------------
void IlcRSTACKRcuDA1::SetWriteToFile(Bool_t write)
{
  if(!write) { 
    fWriteToFile = write;
    return;

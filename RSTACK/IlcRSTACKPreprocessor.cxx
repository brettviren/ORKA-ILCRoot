...
/* $Id: IlcRSTACKPreprocessor.cxx 47523 2011-02-17 14:35:05Z policheh $ */

///////////////////////////////////////////////////////////////////////////////
// RSTACK Preprocessor class. It runs by Shuttle at the end of the run,
...
#include "IlcRSTACKPreprocessor.h"
#include "IlcLog.h"
...
ClassImp(IlcRSTACKPreprocessor)
...
IlcRSTACKPreprocessor::IlcRSTACKPreprocessor() :
IlcPreprocessor("PHS",0)
{
  //default constructor
}

//_______________________________________________________________________________________
IlcRSTACKPreprocessor::IlcRSTACKPreprocessor(IlcShuttleInterface* shuttle):
IlcPreprocessor("PHS",shuttle)
{
  // Constructor
...
//_______________________________________________________________________________________
UInt_t IlcRSTACKPreprocessor::Process(TMap* /*valueSet*/)
{
...
Bool_t IlcRSTACKPreprocessor::ProcessLEDRun()
{
  //Process LED run, update High Gain/Low Gain ratios.

  IlcRSTACKEmcCalibData calibData;
  ...
  const IlcRSTACKEmcCalibData* clb=0;
  IlcCDBEntry* entryCalib = GetFromOCDB("Calib", "EmcGainPedestals");

  if(!entryCalib)
    Log(Form("Cannot find any IlcCDBEntry for [Calib, EmcGainPedestals]!"));
  else
    clb = (IlcRSTACKEmcCalibData*)entryCalib->GetObject();
  ...
  while ((source = dynamic_cast<TObjString *> (iter.Next()))) {
	...
	for(Int_t row=0; row<nRow; row++) {
	  ...
	  if(ratio != 16.) {
	    ...
	    IlcInfo(Form("mod %d iX %d iZ %d  ratio %.3f\n",mod,row,col,ratio));
	  }
	}
      }
    }
...
Float_t IlcRSTACKPreprocessor::HG2LG(Int_t mod, Int_t X, Int_t Z, TFile* f)
{
  //Calculates High gain to Low gain ratio 
  //for crystal at the position (X,Z) in the RSTACK module mod.
  ...
  IlcInfo(Form("%s: %.1f entries, mean=%.3f, peak=%.3f, rms= %.3f. HG/LG = %.3f\n",
...
Bool_t IlcRSTACKPreprocessor::FindBadChannelsEmc()
{
  ...
  for (Int_t i=0; i<2; i++) {
    ...
    IlcRSTACKEmcBadChannelsMap badMap;
...
Bool_t IlcRSTACKPreprocessor::DoFindBadChannelsEmc(Int_t system, TList* list, IlcRSTACKEmcBadChannelsMap& badMap)
{
  //Creates the bad channels map for RSTACK EMC.
...
Bool_t IlcRSTACKPreprocessor::CalibrateEmc()
{
  //Loop over two systems: DAQ and HLT.
  //For each system the same algorithm implemented in DoCalibrateEmc() invokes.

  IlcRSTACKEmcCalibData*   lastCalib=0;
  const IlcRSTACKEmcBadChannelsMap* badMap=0;
  IlcCDBEntry* entryBCM=0;
  IlcCDBEntry* entryEmc=0;
  TList* list=0;
  TString path;
  ...
  for (Int_t i=0; i<2; i++) {
    ...
    if(!entryBCM)
      Log(Form("WARNING!! Cannot find any IlcCDBEntry for [%s, EmcBadChannels]!",path.Data()));
    else
      badMap = (IlcRSTACKEmcBadChannelsMap*)entryBCM->GetObject();

    if(!badMap)
      Log(Form("WARNING!! Nothing for %s in IlcCDBEntry. All cells considered GOOD!",sysn[i]));
    ...
    if(!entryEmc) 
      ...
      lastCalib = (IlcRSTACKEmcCalibData*)entryEmc->GetObject();
...
Bool_t IlcRSTACKPreprocessor::StoreReferenceEmc(Int_t system, TList* list)
{
  ...
  Bool_t resultRef = StoreReferenceFile(fileName.Data(),"CalibRefRSTACK.root");
  return resultRef;

}

Bool_t IlcRSTACKPreprocessor::StoreReferenceLED(TList* list)
{
  ...
  Bool_t resultRef = StoreReferenceFile(fileName.Data(),"LEDRefRSTACK.root");
  return resultRef;
  
}


Bool_t IlcRSTACKPreprocessor::DoCalibrateEmc(Int_t system, TList* list, const IlcRSTACKEmcBadChannelsMap* badMap, IlcRSTACKEmcCalibData& calibData)
{
  // Return kTRUE if OK.
	...
	if(htitl.Contains("and gain 1")) {
	  ...
	  if(badMap) {
	    if(badMap->IsBadChannel(5-md,Z+1,X+1)) {
	      IlcInfo(Form("Cell mod=%d col=%d row=%d is bad. Histogram %s rejected.",

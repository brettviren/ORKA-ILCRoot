...
// 
// Typical use case:
//     IlcRSTACKRawFitter *fitter=new IlcRSTACKRawFitter();
...
ClassImp(IlcRSTACKRawFitterv1)

//-----------------------------------------------------------------------------
IlcRSTACKRawFitterv1::IlcRSTACKRawFitterv1():
...
//-----------------------------------------------------------------------------
IlcRSTACKRawFitterv1::IlcRSTACKRawFitterv1(const IlcRSTACKRawFitterv1 &RSTACKFitter ):
  IlcRSTACKRawFitterv0(RSTACKFitter),
  fSampleParamsLow(0x0),
  fSampleParamsHigh(0x0),
  fToFit(0x0)
{
  //Copy constructor.
  fToFit = new TList() ;
  fSampleParamsLow =new TArrayD(*(RSTACKFitter.fSampleParamsLow)) ;
  fSampleParamsHigh=new TArrayD(*(RSTACKFitter.fSampleParamsHigh)) ;
}

//-----------------------------------------------------------------------------
IlcRSTACKRawFitterv1& IlcRSTACKRawFitterv1::operator = (const IlcRSTACKRawFitterv1 &RSTACKFitter)
{
  //Assignment operator.
  if(this != &RSTACKFitter) {
    fToFit = new TList() ;
    if(fSampleParamsLow){
      fSampleParamsLow = RSTACKFitter.fSampleParamsLow ;
      fSampleParamsHigh= RSTACKFitter.fSampleParamsHigh ;
    }
    else{
      fSampleParamsLow =new TArrayD(*(RSTACKFitter.fSampleParamsLow)) ; 
      fSampleParamsHigh=new TArrayD(*(RSTACKFitter.fSampleParamsHigh)) ;
...
//-----------------------------------------------------------------------------
Bool_t IlcRSTACKRawFitterv1::Eval(const UShort_t *signal, Int_t sigStart, Int_t sigLength)
{
  ...
  if (fPedSubtract) {
    ...
    if (fCalibData) {
      ...
      IlcWarning(Form("Can not read data from OCDB")) ;
    }
    fEnergy-=pedestal ;
  }
  ...
  gMinuit->SetFCN(IlcRSTACKRawFitterv1::UnfoldingChiSquare) ;  
  ...
  if(ierflg != 0){
    //	  IlcWarning(Form("Unable to set initial value for fit procedure : t0=%e\n",1.*tStart) ) ;
   fEnergy =   0. ;
    fTime   =-999. ;
    fQuality= 999. ;
  ...
  if(ierflg != 0){
    //	  IlcWarning(Form("Unable to set initial value for fit procedure : E=%e\n", amp0)) ;
    fEnergy =   0. ;
    fTime   =-999. ;
    fQuality= 999. ;
    return kTRUE ; //will scan further
...
//-----------------------------------------------------------------------------
Double_t IlcRSTACKRawFitterv1::Gamma2(Double_t dt,Double_t en,Double_t b,TArrayD * params){  //Function for fitting samples
  //parameters:
  //dt-time after start
  //en-amplutude
  //function parameters
...
void IlcRSTACKRawFitterv1::UnfoldingChiSquare(Int_t & /*nPar*/, Double_t * Grad, Double_t & fret, Double_t * x, Int_t iflag)
{

...
#include "IlcGenTherminator.h"
#include "IlcLog.h"
#include "IlcRun.h"

ClassImp(IlcGenTherminator)

using namespace std;

IlcGenTherminator::IlcGenTherminator():
  IlcGenMC(),
  fNt(0),
  fEventNumber(0),
  fFileName(""),
...
IlcGenTherminator::IlcGenTherminator(Int_t npart):
  IlcGenMC(npart),
  fNt(0),
  fEventNumber(0),
  fFileName(""),
...
void IlcGenTherminator::Generate()
{
  // Run single event generation with the Therminator model
  IlcWarning("Generating event from IlcGenTherminator");
  ...
  IlcWarning("Generated");
  ((TTherminator *) fMCEvGen)->ImportParticles(&fParticles);

  Int_t np = fParticles.GetEntriesFast();
  IlcWarning(Form("Imported %d particles", np));
...
// Builds the event header, to be called after each event
  IlcGenEventHeader* header = new IlcGenHijingEventHeader("Therminator");

  // Header
  //  IlcGenEventHeader* header = new IlcGenEventHeader("Therminator");
  // Event Vertex
//   header->SetPrimaryVertex(eventVertex);
//   header->SetNProduced(fNprimaries);

  ((IlcGenHijingEventHeader*) header)->SetNProduced(fNprimaries);
  ((IlcGenHijingEventHeader*) header)->SetPrimaryVertex(eventVertex);
  ((IlcGenHijingEventHeader*) header)->SetInteractionTime(eventTime);
  ((IlcGenHijingEventHeader*) header)->SetImpactParameter(0.0);
  ((IlcGenHijingEventHeader*) header)->SetTotalEnergy(0.0);
  ((IlcGenHijingEventHeader*) header)->SetHardScatters(0);
  ((IlcGenHijingEventHeader*) header)->SetParticipants(0, 0);
  ((IlcGenHijingEventHeader*) header)->SetCollisions(0, 0, 0, 0);
  ((IlcGenHijingEventHeader*) header)->SetSpectators(0, 0, 0, 0);
  ((IlcGenHijingEventHeader*) header)->SetReactionPlaneAngle(evrot);
...
// 					      fHijing->GetHINT1(38),
// 					      fHijing->GetHINT1(39));
//     ((IlcGenHijingEventHeader*) header)->SetJets(jet1, jet2, jet3, jet4);
// Bookkeeping for kinematic bias
//     ((IlcGenHijingEventHeader*) header)->SetTrials(fTrials);
// Event Vertex
  header->SetPrimaryVertex(fVertex);
  header->SetInteractionTime(fTime);
  AddHeader(header);
  fCollisionGeometry = (IlcGenHijingEventHeader*)  header;
...
void IlcGenTherminator::SetFileName(const char *infilename)
{
  // Set parameter filename
  fFileName = infilename;
}
void IlcGenTherminator::SetEventNumberInFile(int evnum)
{
  // Set number of events to generate - default: 1
  fEventNumber = evnum;
}

void IlcGenTherminator::ReadShareParticleTable()
{
  ...
  IlcWarning(Form("Reading particle types from particles.data"));

  TString aroot = gSystem->Getenv("ILC_ROOT");
  ...
  if ((in) && (in.is_open()))
      ...
      while (in>>str)
	{
	  ...
	  if (!tParticleType) {
	    ...
	    IlcWarning(Form("Added particle %s with PDG PID %d charge %d", str, (int) mc, charge));
	    //	    IlcWarning(Form("Quantum numbers q s c aq as ac tI3 %lf %lf %lf %lf %lf %lf %lf", q, s, c, aq, as, ac, tI3));
...
void IlcGenTherminator::CreateTherminatorInputFile()
{
  // Create Therminator input file
  const char *aroot = gSystem->Getenv("ILC_ROOT");
...
void IlcGenTherminator::SetModel(const char *model)
{
  // Set the freeze-out model to use
  fFreezeOutModel = model;
  IlcWarning(Form("Selected model %s", fFreezeOutModel.Data()));
  IlcWarning(Form("FOHSLocation is %s", fFOHSlocation.Data()));
}

void IlcGenTherminator::SetLhyquidSet(const char *set)
{
  // Select one of pregenerated Lhyquid hypersurfaces
  const char *aroot = gSystem->Getenv("ILC_ROOT");
  if (strstr(set, "LHC500C0005")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, centrality 0-5 percent"));
    IlcWarning(Form("  initial temperature at tau=1 fm in the center Ti=500 MeV"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC500C0005/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC500C0005");
  }
  if (strstr(set, "LHC500C0510")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, centrality 5-10 percent"));
    IlcWarning(Form("  initial temperature at tau=1 fm in the center Ti=500 MeV"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC500C0510/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC500C0510");
  }
  if (strstr(set, "LHC500C1020")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, centrality 10-20 percent"));
    IlcWarning(Form("  initial temperature at tau=1 fm in the center Ti=500 MeV"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC500C1020/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC500C1020");
  }
  else if (strstr(set, "LHC500C2030")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, centrality 20-30 percent"));
    IlcWarning(Form("  initial temperature at tau=1 fm in the center Ti=500 MeV"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC500C2030/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC500C2030");
  }
  else if (strstr(set, "LHC500C3040")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, centrality 30-40 percent"));
    IlcWarning(Form("  initial temperature at tau=1 fm in the center Ti=500 MeV"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC500C3040/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC500C3040");
  }
  else if (strstr(set, "LHC500C4050")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, centrality 40-50 percent"));
    IlcWarning(Form("  initial temperature at tau=1 fm in the center Ti=500 MeV"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC500C4050/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC500C4050");
  }
  else if (strstr(set, "LHC276TC0005")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, 2.76TeV, centrality 00-05 percent"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC276TC0005/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC276TC0005");
  }
  else if (strstr(set, "LHC276TC0510")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, 2.76TeV, centrality 05-10 percent"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC276TC0510/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC276TC0510");
  }
  else if (strstr(set, "LHC276TC1020")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, 2.76TeV, centrality 10-20 percent"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC276TC1020/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC276TC1020");
  }
  else if (strstr(set, "LHC276TC2030")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, 2.76TeV, centrality 20-30 percent"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC276TC2030/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC276TC2030");
  }
  else if (strstr(set, "LHC276TC3040")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, 2.76TeV, centrality 30-40 percent"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC276TC3040/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC276TC3040");
  }
  else if (strstr(set, "LHC276TC4050")) {
    IlcWarning(Form("IlcGenTherminator: Selected default Lhyquid hypersurface"));
    IlcWarning(Form("  Pb-Pb collisions, 2.76TeV, centrality 40-50 percent"));
    IlcWarning(Form("  freeze-out criteria Tf=145 MeV"));
    IlcWarning(Form("  for details see $(ILC_ROOT)/TTherminator/data/LHC276TC4050/FO.txt"));
    fFOHSlocation.Append(aroot);
    fFOHSlocation.Append("/TTherminator/data/LHC276TC4050");
  }
  else {
    IlcWarning(Form("Did not find Lhyquid set %s", set));
    IlcWarning(Form("Reverting to default: current directory"));
    fFOHSlocation += "";
  }

}

void IlcGenTherminator::SetLhyquidInputDir(const char *inputdir)
{

void MakeForwardMisAlignment(Int_t inputRun){
  ...
  cdb->SetRun(inputRun);
  IlcCDBEntry *inputEntry = cdb->Get("ITS/Align/Data");
  TClonesArray *inputArray = (TClonesArray*)inputEntry->GetObject();
  IlcAlignObjParams *itsAlign = (IlcAlignObjParams*)inputArray->At(0);
  itsAlign->Print();
  printf("\n\n\n");
  ...
  const char* baseSymName ="FRAME/Sector"; //base of symbolic name corresponding to base of path "ILCM_1/B077_1/BSEGMO";
  TString symname;
  ...
  for(Int_t sm=0; sm<18; sm++){
    ...
    new((*array)[sm]) IlcAlignObjParams(symname.Data(),dvoluid,0.,0.,0.,0.,0.,0.,kTRUE);
  }

  new((*array)[18]) IlcAlignObjParams("CP1",dvoluid,inputTrans[0],inputTrans[1],0.,0.,0.,0.,kTRUE);
  array->At(18)->Print();
  new((*array)[19]) IlcAlignObjParams("CP3",dvoluid,inputTrans[0],inputTrans[1],0.,0.,0.,0.,kTRUE);
  array->At(19)->Print();
  printf("\n\n\n");

  // FMD
  TClonesArray *arrayFMD = new TClonesArray("IlcAlignObjParams",4);
  new((*arrayFMD)[0]) IlcAlignObjParams("FMD/FMD2_T",0,inputTrans[0],inputTrans[1],inputTrans[2],inputAngles[0],inputAngles[1],inputAngles[2],kTRUE);
  new((*arrayFMD)[1]) IlcAlignObjParams("FMD/FMD2_B",0,inputTrans[0],inputTrans[1],inputTrans[2],inputAngles[0],inputAngles[1],inputAngles[2],kTRUE);
  new((*arrayFMD)[2]) IlcAlignObjParams("FMD/FMD3_T",0,inputTrans[0],inputTrans[1],inputTrans[2],inputAngles[0],inputAngles[1],inputAngles[2],kTRUE);
  new((*arrayFMD)[3]) IlcAlignObjParams("FMD/FMD3_B",0,inputTrans[0],inputTrans[1],inputTrans[2],inputAngles[0],inputAngles[1],inputAngles[2],kTRUE);
  arrayFMD->Print();
  printf("\n\n\n");
  ...
  if( TString(gSystem->Getenv("TOCDB")) != TString("kTRUE") ){
    ...
    IlcCDBMetaData* mdFMD = new IlcCDBMetaData();
    mdFMD->SetResponsible("FMD Experts");
    mdFMD->SetComment("Misalignment for FMD using ITS object");
    md->SetIlcRootVersion(gSystem->Getenv("ARVERSION"));
    IlcCDBId idFMD("FMD/Align/Data",0,IlcCDBRunRange::Infinity());

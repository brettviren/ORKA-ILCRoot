  ...
  man->SetRun(0);
	IlcCDBId id(IlcQA::GetGRPPath(), 0, IlcCDBRunRange::Infinity());
...
TString ParseConfig(char * option)
{
  ...
  if (!in)
		IlcError("Config.C file not found in current path") ; 	
	...
	if (soption.Contains("system") || soption.Contains("fIlcBeamType")){
		while (getline(in, line)) { 
			sline = line ;		
			if (sline.Contains("IlcSimulation::Instance()->SetTriggerConfig")) 
				break ;
		}
	...
	// get the beam energy
	else if (soption.Contains("fIlcBeamEnergy")) {
		while (getline(in, line)) { 
			sline = line ;		
	...
	// get the number of active detectors
		...
		while (getline(in, line)) {
			sline = line ; 
			sline.ReplaceAll(" ", "") ; 
			if (sline.Contains("IlcMagF*field=newIlcMagF(")) {
...
TMap *GetGRPList() {
  ...
  TString system = ParseConfig("fIlcBeamType") ;  
	TString fSystem = system;
  TMap *map = new TMap();
  map->SetName("MONTECARLO");

  //DAQ
	map->Add(new TObjString("fRunType"),new TObjString(IlcQA::GetRunTypeName(IlcQA::kPHYSICS)));
	map->Add(new TObjString("fIlcStartTime"),new TObjString("0"));
  map->Add(new TObjString("fIlcStopTime"),new TObjString("9999"));
	map->Add(new TObjString("fIlcBeamEnergy"),new TObjString(ParseConfig("fIlcBeamEnergy")));

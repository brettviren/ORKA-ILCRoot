...
void MakePipeMisalignment(Int_t inputRun=0,
			  const char* ocdbPath="local://$ILC_ROOT/OCDB",
			  Bool_t verbose=true, 
			  const char* outFileName=0)
{
  ...
  cdb->SetDefaultStorage("alien://Folder=/ilc/data/2010/OCDB");
  cdb->SetRun(inputRun);
  ...
  // Get ITS alignment 
  IlcCDBEntry* itsEntry = cdb->Get("ITS/Align/Data");
  if (!itsEntry) { 
  ...
  IlcAlignObjParams* itsAlign = 
    static_cast<IlcAlignObjParams*>(itsArray->At(0));
  if (!itsAlign) { 
  ...
  if (!gGeoManager->cd("/ILCM_1/CP_1/Cp1_1/CpSupC_1")) return;
  Double_t pCpSupCA[3];
  gGeoManager->LocalToMaster(local, pCpSupCA);

  if (!gGeoManager->cd("/ILCM_1/CP_1/Cp3_1/CpSupC_3")) return;
  Double_t pCpSupCC[3];
  gGeoManager->LocalToMaster(local, pCpSupCC);

  if (!gGeoManager->cd("/ILCM_1/CP_1/Cp1_1")) return;
  Double_t pCp1[3];
  gGeoManager->LocalToMaster(local, pCp1);

  if (!gGeoManager->cd("/ILCM_1/CP_1/Cp1_1/CP1MO_1/CP1AT_1")) return;
  do { 
  ...
  // Output array 
  TClonesArray* structArray = new TClonesArray("IlcAlignObjParams", 20);

  //dummy vol id
  UShort_t dvoluid = 
    IlcGeomManager::LayerToVolUID(IlcGeomManager::kInvalidLayer,0); 

  //base of symbolic name corresponding to base of path "ILCM_1/B077_1/BSEGMO";
  const char* baseSymName = "FRAME/Sector"; 
  for(Int_t sm=0; sm<18; sm++){
    TString symname =  baseSymName;
    symname         += sm;
    new((*structArray)[sm]) IlcAlignObjParams(symname.Data(),dvoluid,
					     0.,0.,0.,0.,0.,0.,kTRUE);
  }
   
  // --- Beam pipe ---------------------------------------------------
  IlcAlignObjParams* c1Align = 
    new((*structArray)[18]) IlcAlignObjParams("CP1",dvoluid,
					      dXc, dYc, 0, rotX, rotY, 0,kTRUE);
  c1Align->Print();

  // --- C side Collar -----------------------------------------------
  IlcAlignObjParams* cSupCAlign = 
    new((*structArray)[19]) IlcAlignObjParams("CPSUPC",
					      dvoluid, 
  ...
  IlcCDBStorage* storage = cdb->GetStorage(ocdbPath);
  if (!storage){

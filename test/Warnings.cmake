...
set(ar "$ENV{ILC_ROOT}")
#warnings exceptions
set(exceptions "always evaluates both arguments" "std::" "TGeoMatrix::operator\\*" "struct IlcRawDataHeader" "/usr/local/dim" " from " "JETAN/fastjet/fastjet/" "G__" "base class .struct IlcHLTCaloChannelDataStruct. has a non-virtual destructor")
...
if(ar AND rs)
  ...
  set(buildLog "$ENV{ILC_INSTALL}/make.log")
  ...
  foreach(file ${fileList})
    ...
    string(REGEX MATCH "${ar}" "ilcroot" "${ilcroot}")
    if(root)
      ...
      set(ilcroot ${ar})
      string(REGEX REPLACE "${ilcroot}[/]*" "" "file" "${file}")
      set(aliOut "${aliOut}   |\n")
	...
	if(ilcroot)
	  string(REGEX REPLACE "${ilcroot}" "" "line" "${line}")
...
foreach( blameFile ${fileList} )
  ...
  execute_process(COMMAND "${svn_command}" "blame" "${blameFile}"
    WORKING_DIRECTORY "$ENV{ILC_ROOT}"
    OUTPUT_FILE tmpViols
    OUTPUT_VARIABLE tmpVio)
  wcl(fileLen "$ENV{ILC_ROOT}/tmpViols")
  set(anlys "${anlys}   |--| ${mainFile} |-- ${fileLen} lines\n")  
  set(nameList "")
  file(STRINGS "$ENV{ILC_ROOT}/tmpViols" violData)
...
message("${anlys}")
    ...
    message("${rootOut}")
    message("\n\n==============================ILCROOT FILE WARNINGS==============================\n")
    message("${aliOut}")
  else()
    message(WARNING "Environment Variable ILC_ROOT and ROOTSYS must be set")
  endif(ar AND rs)
...
ilcroot -b << EOF
  IlcQAManager * qam = IlcQAManager::QAManager($MODE) ; 
...
ilcroot -b << EOF
  IlcQAv1::SetQARefStorage("local://$ILC_ROOT/QAref") ;
  IlcQAChecker::Instance()->Run("$MERGEDOUT")
 .q
EOF
...
if [ "$1" == "-s" -o "$1" == "-r" -o "$1" == -a ] ; then
  ...
  if [ "$1" == "-s" ] ; then 
    BASEDIR="/ilc/sim/"$PRODYEAR"/"$PRODCYCLE"/"$RUNNUMBER 
    MODE="IlcQAv1::kSIMMODE"
   fi
  if [ "$1" == "-r" ] ; then 
    BASEDIR="/ilc/data/"$PRODYEAR"/"$PRODCYCLE"/"$RUNNUMBER 
    MODE="IlcQAv1::kRECMODE"
  fi
  SetupAliEnEnv 
  MakeXML
  Merge

...
IlcQA * fQAResult = NULL ; 
TCanvas * fCa = NULL ; 

void menuQA()
{
	cout << IlcQA::GetQAResultFileName()  << endl ; 
	TFile * qaResultFile = TFile::Open("QA.root") ; //IlcQA::GetQAResultFileName() ) ; 
	if ( ! qaResultFile ) {
		printf("File %s not found in current directory\n", IlcQA::GetQAResultFileName() ) ; 
		return ; 
	}
	fQAResult = (IlcQA *)qaResultFile->Get("QA") ; 
	if ( ! fQAResult ) {
	...
	for (Int_t det = 0 ; det < listOfDetectors->GetEntries() ; det++) {
		...
		if (fQAResult->IsSetAny(IlcQA::GetDetIndex(detName)))
...
void MakeDetMenu(char * detName)
{
	...
	for (Int_t task = 0 ; task < listOfTasks->GetEntries() ; task++) {
		...
		IlcQA::ILCTASK_t tt = IlcQA::kNULLTASK ; 
		if (taskName.Contains(IlcQA::GetTaskName(IlcQA::kRAWS)) )
			tt = IlcQA::kRAW ; 
		else if (taskName.Contains(IlcQA::GetTaskName(IlcQA::kRECPOINTS)) ||
				taskName.Contains(IlcQA::GetTaskName(IlcQA::kESDS)) )
			tt = IlcQA::kREC ; 
		else if (taskName.Contains(IlcQA::GetTaskName(IlcQA::kHITS)) ||
						 taskName.Contains(IlcQA::GetTaskName(IlcQA::kSDIGITS)) ||
						 taskName.Contains(IlcQA::GetTaskName(IlcQA::kDIGITS)) )
			tt = IlcQA::kSIM ;
		if (fQAResult->IsSetAny(IlcQA::GetDetIndex(detName), tt))

...
class IlcGenPythia ;
//--- Functions ---
IlcGenPythia *PythiaHard(PDC07Proc_t );
IlcGenPythia *PythiaHVQ(PDC07Proc_t );
...
void Config()
{
  ...
  if (proc <=   kBeautypp14000wmi) {
    ...
    IlcGenPythia *pythia = PythiaHard(proc);
    // FeedDown option
    pythia->SetFeedDownHigherFamily(kFALSE);
    // Set Count mode
    if(nEvts>0) pythia->SetCountMode(IlcGenPythia::kCountParents);
  ...
  if(iquenching == 1){
    ...
    IlcPythia::Instance()->InitQuenching(0.,0.1,k,0,0.95,6);
  ...
  //  IlcMagF* field = new IlcMagF("Maps","Maps", -1., -1., IlcMagF::k5kGUniform);
...
//
   ...
   if (iTRD)
    ...
    {
        //=================== EMCAL parameters ============================
        IlcEMCAL *EMCAL = new IlcEMCALv2("EMCAL", "EMCAL_COMPLETEV1 scTh=0.176 pbTh=0.144");
    }
...
IlcGenPythia *PythiaHVQ(PDC07Proc_t proc) {
//*******************************************************************//
...
IlcGenPythia *PythiaHard(PDC07Proc_t proc) {
//*******************************************************************//
...
//*******************************************************************//
  ...
  case kPyJetJet:
    ...
    IlcGenPythia * gener = new IlcGenPythia(nEvts);

...
#include "IlcAnalysisTaskEfficiency.h"
...
ClassImp(IlcAnalysisTaskEfficiency)

//________________________________________________________________________
  IlcAnalysisTaskEfficiency::IlcAnalysisTaskEfficiency(const char *name) 
...
//________________________________________________________________________
void IlcAnalysisTaskEfficiency::UserCreateOutputObjects()
{
  // Create histograms
...
//________________________________________________________________________
void IlcAnalysisTaskEfficiency::UserExec(Option_t *) 
{
  // Event loop
  ...
  //IlcESDInputHandler* esdI = (IlcESDInputHandler*) 
  //(IlcAnalysisManager::GetAnalysisManager())->GetInputEventHandler();
  //IlcESDEvent* hltEvent = esdI->GetHLTEvent();

  IlcStack* stack = MCEvent()->Stack();
  IlcESDEvent* esdE = (IlcESDEvent*) fInputEvent;
    
  // CKB
  // Fetch the SPD vertex:
  Double_t vtxSPD[3];
  const IlcESDVertex* spdVertex = esdE->GetPrimaryVertexSPD();
  if (spdVertex) {
  ...
  Double_t vtxTPC[3];
  const IlcESDVertex* tpcVertex = esdE->GetPrimaryVertexTPC();
  if (tpcVertex) {
  ...
  for (Int_t iTracks = 0; iTracks < esdE->GetNumberOfTracks(); iTracks++) {
    ...
    IlcVParticle *track = esdE->GetTrack(iTracks);
    ...
    const IlcExternalTrackParam *tpcPin = 0x0;
    //Double_t phiIn=0.;
    Float_t phi = 0.;
    Float_t eta = 0.;
    Float_t pt  = 0.;
    ...
    // 2. TPC only tracks
    // Track selection copied from PWGPP/IlcAnalysisTaskQAsym
    ...
    if(fTrackType==0){
      ...
      if (!tpcPin) continue;
      tpcP = IlcESDtrackCuts::
	GetTPCOnlyTrack(dynamic_cast<IlcESDEvent*>(esdE),
			esdtrack->GetID());
      if (!tpcP) continue;
      if (!fCuts->AcceptTrack(tpcP)) continue;
	...
	IlcMCParticle* mcPtest = (IlcMCParticle*) MCEvent()->GetTrack(ind);
	...
	//	Int_t ind = TMath::Abs(esdtrack->GetLabel());
	IlcMCParticle* mcP = (IlcMCParticle*) MCEvent()->GetTrack(ind);
	...
	if (tpcP){
	  ...
	  IlcExternalTrackParam * tpcPCopy = new IlcExternalTrackParam(*tpcP);
	  Double_t xk = 43.6;  // cm
	  Double_t bz = 5.0;   // kG
	  if(tpcPCopy->PropagateTo(xk,bz)){
	    ...
	    IlcMCParticle* mcPart = (IlcMCParticle*) MCEvent()->GetTrack(abs(labeltpcits));
	    Int_t ntref = mcPart->GetNumberOfTrackReferences();
		 
	    for (Int_t k = 0; k < ntref; k++) {
	      IlcTrackReference* ref = mcPart->GetTrackReference(k);
	      Float_t xhits = ref->X();
	      Float_t yhits = ref->Y();
	...
	const IlcExternalTrackParam *trd = esdtrack->GetOuterParam();
	  
	if (trd){Int_t labeltpctrd = track->GetLabel();
	  
	  IlcExternalTrackParam * trdCopy = new IlcExternalTrackParam(*trd);
	  ...
	  if(trdCopy->PropagateTo(xktrd,bztrd)){
	    ...
	    IlcMCParticle* mcPart = (IlcMCParticle*) MCEvent()->GetTrack(abs(labeltpctrd));
  ...
  const IlcMultiplicity *spdMult = esdE->GetMultiplicity();
  ...
  IlcGenEventHeader*       header = MCEvent()->GenEventHeader();
  // some test
  IlcGenCocktailEventHeader* cH = dynamic_cast<IlcGenCocktailEventHeader*> (header);
  IlcGenPythiaEventHeader* pH;
  if (cH == 0) 
    {
      header->Dump();
      pH = dynamic_cast<IlcGenPythiaEventHeader*>(header);
    } else {
    TObject* entry = cH->GetHeaders()->FindObject("Pythia");
    pH = dynamic_cast<IlcGenPythiaEventHeader*>(entry);
  }
  Int_t iproc = -1;
  ...
  for (Int_t i = 0; i < nmc; i++) {
    IlcMCParticle* mcP = (IlcMCParticle*) MCEvent()->GetTrack(i);
    ...
    for (Int_t j = 0; j < ntr; j++) {
      ...
      IlcTrackReference* ref = mcP->GetTrackReference(j);
	...
	reco = kTRUE;
	multR++;
	//IlcESDtrack* test = esdE->GetTrack(j);
	if (multR > 1) {
	  Int_t nclus = 0;
	  IlcESDtrack* track = esdE->GetTrack(jold);
...
//________________________________________________________________________
void IlcAnalysisTaskEfficiency::Terminate(Option_t *) 
{

}  


Bool_t IlcAnalysisTaskEfficiency::SelectJouri(IlcESDtrack* track) 
{
  ...
  const IlcESDVertex *vtx=esdE->GetPrimaryVertexSPD();
  if (!vtx->GetStatus()) selected = kFALSE;
    
  Double_t zv=vtx->GetZv();

    
  const IlcExternalTrackParam *ct=track->GetTPCInnerParam();
...
Int_t IlcAnalysisTaskEfficiency::GetIndexAC(IlcESDtrack *track){
  if(!track)return -1;
...
Int_t IlcAnalysisTaskEfficiency::ConvertePDG(Int_t pdg)
{

...
#include "IlcAnalysisTaskCluster.h"
...
ClassImp(IlcAnalysisTaskCluster)

  //________________________________________________________________________
  IlcAnalysisTaskCluster::IlcAnalysisTaskCluster(const char *name) 
...
//________________________________________________________________________
void IlcAnalysisTaskCluster::UserCreateOutputObjects()
{
  // Create histograms
  // Called once
...
void IlcAnalysisTaskCluster::UserExec(Option_t *) 
{
  ...
  const IlcVVertex* vertex = event->GetPrimaryVertex();
  Float_t vz = vertex->GetZ();
  if (TMath::Abs(vz) > 10.) return;

  for (Int_t iTrack = 0; iTrack < event->GetNumberOfTracks(); iTrack++) {
   
    IlcVParticle *track = event->GetTrack(iTrack);
    ...
    const IlcExternalTrackParam *tpcP = 0x0;
...
//________________________________________________________________________
void IlcAnalysisTaskCluster::Terminate(Option_t *) 
{

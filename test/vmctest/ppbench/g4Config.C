// $Id: g4Config.C 52112 2011-10-13 15:44:47Z ivana $
//
// Configuration macro for running ilcroot with Geant4
...
void Config()
{
  ...
  IlcTPC* TPC = (IlcTPC*)gIlc->GetDetector("TPC");
  if ( ! TPC )
  ...
  if (gClassTable->GetID("TGeant4") == -1) {
    ...
    if (!gInterpreter->IsLoaded("$ILC/geant4_vmc/examples/macro/g4libs.C")) {
      gROOT->LoadMacro("$ILC/geant4_vmc/examples/macro/g4libs.C");
...
*/
  ...
  gROOT->LoadMacro("$ILC_ROOT/test/vmctest/ppbench/genExtFileConfig.C");

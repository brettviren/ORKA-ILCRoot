...
NEVENTS=1
G3CONFIG="$ILC_ROOT/test/vmctest/lego/g3Config.C" 
G4CONFIG="$ILC_ROOT/test/vmctest/lego/g4Config.C" 
...
if [ "$RUNG3" = "1" ]; then 
  ...
  ilcroot -b -q  sim.C\($NEVENTS,\"$G3CONFIG\",\"$DET\"\)  2>&1 | tee sim.log
  rm -fr $G3OUTDIR
  mkdir -p $G3OUTDIR
...
if [ "$RUNG4" = "1" ]; then 
  ...
  rm -rf *.root *.dat *.log fort* hlt hough raw* recraw/*.root recraw/*.log
  ilcroot -b -q  sim.C\($NEVENTS,\"$G4CONFIG\",\"$DET\"\)  2>&1 | tee sim.log
  rm -fr $G4OUTDIR
  mkdir -p $G4OUTDIR

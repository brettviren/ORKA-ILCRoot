...
// Needed to produce simulated RAW data
  gSystem->Setenv("ILCMDC_RAWDB1","./mdc1");
  gSystem->Setenv("ILCMDC_RAWDB2","./mdc2");
  gSystem->Setenv("ILCMDC_TAGDB","./mdc1/tag");
  gSystem->Setenv("ILCMDC_RUNDB","./mdc1/meta");
  ...
  gSystem->Exec("ilcroot -b -q 'sim.C(0)' > sim.log 2>&1");
  cout<<">>>>> BACKGROUND RECONSTRUCTION <<<<<"<<endl;
  gSystem->Exec("ilcroot -b -q 'rec.C(0)' > rec.log 2>&1");
  ...
  gSystem->Exec("cp BackgroundFull/IlcESDs.root Background/");
  ...
  gSystem->Exec("ilcroot -b -q 'sim.C(4)' > sim.log 2>&1");
  gSystem->ChangeDirectory("../");
  gSystem->Exec("mkdir BackgroundSDigits");
  gSystem->Exec("cp Background/*SDigits.root BackgroundSDigits");
  gSystem->Exec("cp BackgroundFull/gilc.root BackgroundSDigits/");
  gSystem->Exec("cp BackgroundFull/IlcESDs.root BackgroundSDigits/");
  ...
  gSystem->Exec("ilcroot -b -q 'sim.C(1)' > sim.log 2>&1");
  cout<<">>>>> MERGED RECONSTRUCTION <<<<<"<<endl;
  gSystem->Exec("ilcroot -b -q 'rec.C(1)' > rec.log 2>&1");
  gSystem->ChangeDirectory("../");
  ...
  gSystem->Exec("cp Merged/gilc.root Signal/");
  gSystem->Exec("cp Merged/Kinematics.root Signal/");
  gSystem->Exec("cp -a Merged/GRP Signal/");
  gSystem->ChangeDirectory("Signal/");
  gSystem->Exec("ilcroot -b -q 'sim.C(2)' > sim.log 2>&1");
  cout<<">>>>> SIGNAL RECONSTRUCTION <<<<<"<<endl;
  gSystem->Exec("ilcroot -b -q 'rec.C(2)' > rec.log 2>&1");
  gSystem->ChangeDirectory("../");
  
  //  cout<<">>>>> TAG <<<<<"<<endl;
  //  gSystem->Exec("ilcroot -b -q tag.C > tag.log 2>&1");
  //  cout<<">>>>> CHECK ESD <<<<<"<<endl;
  //  gSystem->Exec("ilcroot -b -q CheckESD.C > check.log 2>&1");
  //  cout<<">>>>> AOD <<<<<"<<endl;
  //  gSystem->Exec("ilcroot -b -q CreateAODfromESD.C > aod.log 2>&1");
}

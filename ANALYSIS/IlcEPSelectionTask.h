#ifndef ILCEPSELECTIONTASK_H
#define ILCEPSELECTIONTASK_H
...
class IlcEPSelectionTask : public IlcAnalysisTaskSE {
 ...
 public:
  ...
  IlcEPSelectionTask();
  IlcEPSelectionTask(const char *name);
  virtual ~IlcEPSelectionTask();
  ...
  TVector2 GetQ(IlcEventplane* EP, TObjArray* event);
  void GetQsub(TVector2& Qsub1, TVector2& Qsub2, TObjArray* event,IlcEventplane* EP);
  ...
  void SetPhiDist();
  void SetPersonalESDtrackCuts(IlcESDtrackCuts* trackcuts);
 ...
 private:
  ...
  IlcEPSelectionTask(const IlcEPSelectionTask& ep);
  IlcEPSelectionTask& operator= (const IlcEPSelectionTask& ep); 

  TObjArray* GetAODTracksAndMaxID(IlcAODEvent* aod, Int_t& maxid);
  ...
  IlcESDtrackCuts* fESDtrackCuts;       // track cuts
  
  IlcOADBContainer* fEPContainer;	//! OADB Container

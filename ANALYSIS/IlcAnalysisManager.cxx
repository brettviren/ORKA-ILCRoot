...
/* $Id: IlcAnalysisManager.cxx 58336 2012-08-28 15:07:04Z hristov $ */
// Author: Andrei Gheata, 31/05/2006

//==============================================================================
//   IlcAnalysisManager - Manager analysis class. Allows creation of several
...
using std::cout;
using std::endl;
ClassImp(IlcAnalysisManager)

IlcAnalysisManager *IlcAnalysisManager::fgAnalysisManager = NULL;
TString IlcAnalysisManager::fgCommonFileName = "";
Int_t IlcAnalysisManager::fPBUpdateFreq = 1;

//______________________________________________________________________________
IlcAnalysisManager::IlcAnalysisManager(const char *name, const char *title)
                   :TNamed(name,title),
                    fTree(0),
...
//______________________________________________________________________________
IlcAnalysisManager::IlcAnalysisManager(const IlcAnalysisManager& other)
                   :TNamed(other),
                    fTree(NULL),
...
//______________________________________________________________________________
IlcAnalysisManager& IlcAnalysisManager::operator=(const IlcAnalysisManager& other)
{
// Assignment
   if (&other != this) {
...
//______________________________________________________________________________
IlcAnalysisManager::~IlcAnalysisManager()
{
// Destructor.
...
//______________________________________________________________________________
Int_t IlcAnalysisManager::GetEntry(Long64_t entry, Int_t getall)
{
...
//______________________________________________________________________________
Int_t IlcAnalysisManager::GetRunFromAlienPath(const char *path)
{
// Attempt to extract run number from input data path. Works only for paths to
// ilc data in alien.
//    sim:  /ilc/sim/<production>/run_no/...
//    data: /ilc/data/year/period/000run_no/... (ESD or AOD)
   TString type = "unknown";
   TString s(path);
   if (s.Contains("/ilc/data")) type = "real";
   else if (s.Contains("/ilc/sim")) type = "simulated";
   TString srun;
   Int_t ind1, ind2;
   ind1 = s.Index("/00");
   if (ind1>0) {
...
//______________________________________________________________________________
Bool_t IlcAnalysisManager::Init(TTree *tree)
{
  ...
  // Init() will be called many times when running with PROOF.
   ...
   if (fDebug > 1) {
      printf("->IlcAnalysisManager::Init(%s)\n", tree->GetName());
   }
   ...
   fTree = tree;
   fTable.Rehash(100);
   IlcAnalysisDataContainer *top = fCommonInput;
   if (!top) top = (IlcAnalysisDataContainer*)fInputs->At(0);
   if (!top) {
...
//______________________________________________________________________________
void IlcAnalysisManager::SlaveBegin(TTree *tree)
{
  // The SlaveBegin() function is called after the Begin() function.
  // When running with PROOF SlaveBegin() is called on each slave server.
  // The tree argument is deprecated (on PROOF 0 is passed).
   if (fDebug > 1) printf("->IlcAnalysisManager::SlaveBegin()\n");
   ...
   while ((task=(IlcAnalysisTask*)next())) {
      ...
      if (getsysInfo) IlcSysInfo::AddStamp(Form("%s_CREATEOUTOBJ",task->ClassName()), 0, itask, 0);
      itask++;
   }
...
//______________________________________________________________________________
Bool_t IlcAnalysisManager::Notify()
{
   ...
   if (!TObject::TestBit(IlcAnalysisManager::kTrueNotify)) return kFALSE;
   ...
   if (IsCollectThroughput()) {
      ...
      fCurrentDescriptor = new IlcAnalysisFileDescriptor(curfile);
      fFileDescriptors->Add(fCurrentDescriptor);
   }   
   
   if (fDebug > 1) printf("->IlcAnalysisManager::Notify() file: %s\n", curfile->GetName());
   Int_t run = IlcAnalysisManager::GetRunFromAlienPath(curfile->GetName());
   ...
   if (fDebug > 1) printf("<-IlcAnalysisManager::Notify()\n");
   return kTRUE;
}    

//______________________________________________________________________________
Bool_t IlcAnalysisManager::Process(Long64_t)
{
...
void IlcAnalysisManager::PackOutput(TList *target)
{
  ...
  // stage in PROOF case for each slave.
   ...
   IlcAnalysisTask *task;
   while ((task=(IlcAnalysisTask*)nexttask())) {
...
//               TFile *file = IlcAnalysisManager::OpenFile(output, openoption, kTRUE);
...
//               file->Close();
            ...
            IlcAnalysisDataWrapper *wrap = 0;
            if (isManagedByHandler) {
               wrap = new IlcAnalysisDataWrapper(fOutputEventHandler->GetTree());
         ...
         // Special outputs. The file must be opened and connected to the container.
            ...
            if (!file) {
               IlcAnalysisTask *producer = output->GetProducer();
               Fatal("PackOutput", 
...
//               file->Clear();
   ...
   if (cdir) cdir->cd();
   if (fDebug > 1) printf("<-IlcAnalysisManager::PackOutput: output list contains %d containers\n", target->GetSize());
}

//______________________________________________________________________________
void IlcAnalysisManager::ImportWrappers(TList *source)
{
// Import data in output containers from wrappers coming in source.
   if (fDebug > 1) printf("->IlcAnalysisManager::ImportWrappers()\n");
   TIter next(fOutputs);
   IlcAnalysisDataContainer *cont;
   IlcAnalysisDataWrapper   *wrap;
   Int_t icont = 0;
   ...
   while ((cont=(IlcAnalysisDataContainer*)next())) {
      ...
      if (cont->IsSpecialOutput() || inGrid) {
         ...
         wrap = new IlcAnalysisDataWrapper(obj);
         wrap->SetDeleteData(kFALSE);
      }   
      if (!wrap) wrap = (IlcAnalysisDataWrapper*)source->FindObject(cont->GetName());
      if (!wrap) {
   ...
   if (cdir) cdir->cd();
   if (fDebug > 1) printf("<-IlcAnalysisManager::ImportWrappers(): %d containers imported\n", icont);
}

//______________________________________________________________________________
void IlcAnalysisManager::UnpackOutput(TList *source)
{
  // Called by IlcAnalysisSelector::Terminate only on the client.
   if (fDebug > 1) printf("->IlcAnalysisManager::UnpackOutput()\n");
   if (!source) {
   ...
   while ((output=(IlcAnalysisDataContainer*)next())) {
      ...
      if (output->HasConsumers()) {
         ...
         for (Int_t i=0; i<ncons; i++) {
            IlcAnalysisTask *task = (IlcAnalysisTask*)list->At(i);
...
//______________________________________________________________________________
void IlcAnalysisManager::Terminate()
{
  // The Terminate() function is the last function to be called during
  // a query. It always runs on the client, it can be used to present
  // the results graphically.
   if (fDebug > 1) printf("->IlcAnalysisManager::Terminate()\n");
   TDirectory *cdir = gDirectory;
   gROOT->cd();
   IlcAnalysisTask *task;
   IlcAnalysisDataContainer *output;
   ...
   Int_t itask = 0;
   while (!IsSkipTerminate() && (task=(IlcAnalysisTask*)next())) {
      // Save all the canvases produced by the Terminate
      TString pictname = Form("%s_%s", task->GetName(), task->ClassName());
      task->Terminate();
      gROOT->cd();
      if (getsysInfo) 
         IlcSysInfo::AddStamp(Form("%s_TERMINATE",task->ClassName()),0, itask, 2);
      itask++;   
   ...
   icont = 0;
   TList filestmp;
   while ((output=(IlcAnalysisDataContainer*)next1())) {
      ...
      if (!f.IsZombie()) {
         ...
         if (!gROOT->IsBatch()) {
            ...
            for (itask=0; itask<fTopTasks->GetEntriesFast(); itask++) {
               task = (IlcAnalysisTask*)fTopTasks->At(itask);
               canvas->cd(ipad++);
...
//______________________________________________________________________________
void IlcAnalysisManager::ProfileTask(Int_t itop, const char *option) const
{
// Profiles the task having the itop index in the list of top (first level) tasks.
   IlcAnalysisTask *task = (IlcAnalysisTask*)fTopTasks->At(itop);
   if (!task) {
...
//______________________________________________________________________________
void IlcAnalysisManager::ProfileTask(const char *name, const char */*option*/) const
{
...
// was used.
   ...
   IlcAnalysisTask *task = (IlcAnalysisTask*)fTopTasks->FindObject(name);
   if (!task) {
...
//______________________________________________________________________________
void IlcAnalysisManager::AddTask(IlcAnalysisTask *task)
{
...
//______________________________________________________________________________
IlcAnalysisTask *IlcAnalysisManager::GetTask(const char *name) const
{
// Retreive task by name.
   if (!fTasks) return NULL;
   return (IlcAnalysisTask*)fTasks->FindObject(name);
}

//______________________________________________________________________________
IlcAnalysisDataContainer *IlcAnalysisManager::CreateContainer(const char *name, 
                                TClass *datatype, EIlcAnalysisContType type, const char *filename)
{
...
// the output object to a folder inside the output file
   ...
   IlcAnalysisDataContainer *cont = new IlcAnalysisDataContainer(name, datatype);
...
//______________________________________________________________________________
Bool_t IlcAnalysisManager::ConnectInput(IlcAnalysisTask *task, Int_t islot,
                                        IlcAnalysisDataContainer *cont)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisManager::ConnectOutput(IlcAnalysisTask *task, Int_t islot,
                                        IlcAnalysisDataContainer *cont)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::CleanContainers()
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisManager::InitAnalysis()
{
...
// and data containers are properly connected
   ...
   for (i=0; i<ntop; i++) {
      task = (IlcAnalysisTask*)fTopTasks->At(i);
      task->SetUsed();
   }
   Int_t norphans = 0;
   next.Reset();
   ...
   while ((cont=(IlcAnalysisDataContainer*)nextcont())) {
      if (!cont->IsPostEventLoop() && !fOutputs->FindObject(cont)) {
         ...
         // Check if one of the consumers is post event loop
            ...
            while ((task=(IlcAnalysisTask*)nextconsumer())) {
               if (task->IsPostEventLoop()) {
                  fOutputs->Add(cont);
                  break;
               }
   ...
   while ((cont=(IlcAnalysisDataContainer*)nextout())) {
...
//______________________________________________________________________________
void IlcAnalysisManager::AddBranches(const char *branches)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::CheckBranches(Bool_t load)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisManager::CheckTasks() const
{
// Check consistency of tasks.
   ...
   // Get the pointer to IlcAnalysisTaskSE::Class()
   TClass *badptr = (TClass*)gROOT->ProcessLine("IlcAnalysisTaskSE::Class()");
...
//______________________________________________________________________________
void IlcAnalysisManager::PrintStatus(Option_t *option) const
{
...
//______________________________________________________________________________
void IlcAnalysisManager::ResetAnalysis()
{
...
//______________________________________________________________________________
void IlcAnalysisManager::RunLocalInit()
{
...
//______________________________________________________________________________
Long64_t IlcAnalysisManager::StartAnalysis(const char *type, Long64_t nentries, Long64_t firstentry)
{
// Start analysis having a grid handler.
   if (!fGridHandler) {
      ...
      Info("===", "Add an IlcAnalysisAlien object as plugin for this manager and configure it.");
      return -1;
   }
   TTree *tree = NULL;
...
//______________________________________________________________________________
Long64_t IlcAnalysisManager::StartAnalysis(const char *type, TTree * const tree, Long64_t nentries, Long64_t firstentry)
{
...
// MIX. Process nentries starting from firstentry
   ...
   if (fDebug > 1) {
      ...
      IlcLog::SetGlobalLogLevel(IlcLog::kInfo);
   }
   ...
   if (fMode == kGridAnalysis) {
      ...
      if (!anaType.Contains("terminate")) {
         ...
         if (fGridHandler->GetRunMode() == IlcAnalysisGrid::kOffline) {if (cdir) cdir->cd(); return 0;}
   ...
   Bool_t getsysInfo = ((fNSysInfo>0) && (fMode==kLocalAnalysis))?kTRUE:kFALSE;
   if (getsysInfo) IlcSysInfo::AddStamp("Start", 0);
   ...
   switch (fMode) {
      case kLocalAnalysis:
         if (!tree && !fGridHandler) {
            ...
            Int_t itask = 0;
            Bool_t dirStatus = TH1::AddDirectoryStatus();
            while ((task=(IlcAnalysisTask*)nextT())) {
               TH1::AddDirectory(kFALSE);
               task->CreateOutputObjects();
               if (!task->CheckPostData()) {
                  Error("SlaveBegin","####### IMPORTANT! ####### \n\n\n\
         ...
         fSelector = new IlcAnalysisSelector(this);
         ...
         // Run tree-based analysis via IlcAnalysisSelector  
      ...
      case kProofAnalysis:
         ...
         if (chain) {
            ...
            retv = chain->Process("IlcAnalysisSelector", "", nentries, firstentry);
         } else {
...
//______________________________________________________________________________
Long64_t IlcAnalysisManager::StartAnalysis(const char *type, const char *dataset, Long64_t nentries, Long64_t firstentry)
{
...
// LOCAL, PROOF or GRID. Process nentries starting from firstentry.
   ...
   if (fGridHandler) {
      ...
      if (fGridHandler->GetRunMode() == IlcAnalysisGrid::kTest) {
   ...
   gROOT->ProcessLine(line);
   Long_t retv;
   line = Form("gProof->Process(\"%s\", \"IlcAnalysisSelector\", \"\", %lld, %lld);",
...
//______________________________________________________________________________
TFile *IlcAnalysisManager::OpenFile(IlcAnalysisDataContainer *cont, const char *option, Bool_t ignoreProof)
{
...
// be optionally ignored.
  ...
  TFile *f = NULL;
  if (filename.IsNull()) {
    ::Error("IlcAnalysisManager::OpenFile", "No file name specified for container %s", cont->GetName());
    return NULL;
  }
  if (mgr->GetAnalysisType()==IlcAnalysisManager::kProofAnalysis && cont->IsSpecialOutput()
      && !ignoreProof)
    ...
    if (f) {
      ...
      if ((opt=="UPDATE") && (opt!=f->GetOption())) 
        ::Info("IlcAnalysisManager::OpenFile", "File %s already opened in %s mode!", cont->GetFileName(), f->GetOption());
    } else {
  ...
  ::Fatal("IlcAnalysisManager::OpenFile", "File %s could not be opened", filename.Data());
  cont->SetFile(NULL);
  return NULL;
}    
 
//______________________________________________________________________________
TFile *IlcAnalysisManager::OpenProofFile(IlcAnalysisDataContainer *cont, const char *option, const char *extaod)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::ExecAnalysis(Option_t *option)
{
// Execute analysis.
   ...
   if (!TObject::TestBit(IlcAnalysisManager::kTrueNotify)) {
      TObject::SetBit(IlcAnalysisManager::kTrueNotify);
      Notify();
   }   
   ...
   TDirectory *cdir = gDirectory;
   Bool_t getsysInfo = ((fNSysInfo>0) && (fMode==kLocalAnalysis))?kTRUE:kFALSE;
   if (getsysInfo && ((fNcalls%fNSysInfo)==0)) IlcSysInfo::AddStamp("Exec_start", (Int_t)fNcalls);
   if (!fInitOK) {
   ...
   if (fTree) {
      if (getsysInfo && ((fNcalls%fNSysInfo)==0)) 
         IlcSysInfo::AddStamp("Handlers_BeginEventGroup",fNcalls, 1002, 0);
      TIter next(fTasks);
   // De-activate all tasks
      while ((task=(IlcAnalysisTask*)next())) task->SetActive(kFALSE);
      IlcAnalysisDataContainer *cont = fCommonInput;
      if (!cont) cont = (IlcAnalysisDataContainer*)fInputs->At(0);
      if (!cont) {
...
//    Call BeginEvent() for optional input/output and MC services 
      ...
      gROOT->cd();
      if (getsysInfo && ((fNcalls%fNSysInfo)==0)) 
         IlcSysInfo::AddStamp("Handlers_BeginEvent",fNcalls, 1000, 0);
//
//    Execute the tasks
//      TIter next1(cont->GetConsumers());
      TIter next1(fTopTasks);
      Int_t itask = 0;
      while ((task=(IlcAnalysisTask*)next1())) {
         if (fDebug >1) {
         ...
         gROOT->cd();
         if (getsysInfo && ((fNcalls%fNSysInfo)==0)) 
            IlcSysInfo::AddStamp(task->ClassName(), fNcalls, itask, 1);
         itask++;   
      }
//
//    Call FinishEvent() for optional output and MC services 
      ...
      if (getsysInfo && ((fNcalls%fNSysInfo)==0)) 
         IlcSysInfo::AddStamp("Handlers_FinishEvent",fNcalls, 1001, 1);
      if (cdir) cdir->cd();   
      return;
   }   
...
//  Call BeginEvent() for optional input/output and MC services 
   ...
   while ((task=(IlcAnalysisTask*)next2())) {
...
// Call FinishEvent() for optional output and MC services 
   ...
   if (fMCtruthEventHandler) fMCtruthEventHandler->FinishEvent();
   if (getsysInfo && ((fNcalls%fNSysInfo)==0)) 
      IlcSysInfo::AddStamp("Handlers_FinishEvent",fNcalls, 1000, 1);
   if (cdir) cdir->cd();   
}

//______________________________________________________________________________
Bool_t IlcAnalysisManager::IsPipe(std::ostream &out)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::SetInputEventHandler(IlcVEventHandler* const handler)
{
// Set the input event handler and create a container for it.
   Changed();
   fInputEventHandler   = handler;
   if (!fCommonInput) fCommonInput = CreateContainer("cAUTO_INPUT", TChain::Class(), IlcAnalysisManager::kInputContainer);
}

//______________________________________________________________________________
void IlcAnalysisManager::SetOutputEventHandler(IlcVEventHandler* const handler)
{
// Set the input event handler and create a container for it.
   Changed();
   fOutputEventHandler   = handler;
   if (!fCommonOutput) fCommonOutput = CreateContainer("cAUTO_OUTPUT", TTree::Class(), IlcAnalysisManager::kOutputContainer, "default");
   fCommonOutput->SetSpecialOutput();
}

//______________________________________________________________________________
void IlcAnalysisManager::SetDebugLevel(UInt_t level)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::SetUseProgressBar(Bool_t flag, Int_t freq)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::RegisterExtraFile(const char *fname)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisManager::GetFileFromWrapper(const char *filename, const TList *source)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::GetAnalysisTypeString(TString &type) const
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisManager::ValidateOutputFiles() const
{
...
//______________________________________________________________________________
void IlcAnalysisManager::ProgressBar(const char *opname, Long64_t current, Long64_t size, TStopwatch * const watch, Bool_t last, Bool_t refresh)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::DoLoadBranch(const char *name) 
{
...
//______________________________________________________________________________
void IlcAnalysisManager::AddStatisticsTask(UInt_t offlineMask)
{
// Add the statistics task to the manager.
  ...
  TString line = Form("IlcAnalysisTaskStat::AddToManager(%u);", offlineMask);
  gROOT->ProcessLine(line);
}  

//______________________________________________________________________________
void IlcAnalysisManager::CountEvent(Int_t ninput, Int_t nprocessed, Int_t nfailed, Int_t naccepted)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::AddStatisticsMsg(const char *line)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::WriteStatisticsMsg(Int_t)
{
// If fStatistics is present, write the file in the format ninput_nprocessed_nfailed_naccepted.stat
   ...
   AddStatisticsMsg(Form("Number of accepted events for mask %s: %lld", IlcAnalysisStatistics::GetMaskAsString(fStatistics->GetOfflineMask()), fStatistics->GetNaccepted()));
...
//______________________________________________________________________________
const char* IlcAnalysisManager::GetOADBPath()
{
...
//______________________________________________________________________________
void IlcAnalysisManager::SetGlobalStr(const char *key, const char *value)
{
...
// can be then retrieved by a given analysis macro via GetGlobalStr(key).
   IlcAnalysisManager *mgr = IlcAnalysisManager::GetAnalysisManager();
   if (!mgr) {
      ::Error("IlcAnalysisManager::SetGlobalStr", "No analysis manager defined");
      return;
   }   
   Bool_t valid = kFALSE;
   TString existing = IlcAnalysisManager::GetGlobalStr(key, valid);
   if (valid) {
      ::Error("IlcAnalysisManager::SetGlobalStr", "Global %s = %s already defined.", key, existing.Data());
      return;
   }
...
//______________________________________________________________________________
const char *IlcAnalysisManager::GetGlobalStr(const char *key, Bool_t &valid)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::SetGlobalInt(const char *key, Int_t value)
{
...
// can be then retrieved by a given analysis macro via GetGlobalInt(key).
   ...
   Bool_t valid = kFALSE;
   Int_t existing = IlcAnalysisManager::GetGlobalInt(key, valid);
   if (valid) {
      ::Error("IlcAnalysisManager::SetGlobalInt", "Global %s = %i already defined.", key, existing);
      return;
   }
...
//______________________________________________________________________________
Int_t IlcAnalysisManager::GetGlobalInt(const char *key, Bool_t &valid)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::SetGlobalDbl(const char *key, Double_t value)
{
...
// can be then retrieved by a given analysis macro via GetGlobalInt(key).
   ...
   Bool_t valid = kFALSE;
   Double_t existing = IlcAnalysisManager::GetGlobalDbl(key, valid);
   if (valid) {
      ::Error("IlcAnalysisManager::SetGlobalInt", "Global %s = %g already defined.", key, existing);
      return;
   }
...
//______________________________________________________________________________
Double_t IlcAnalysisManager::GetGlobalDbl(const char *key, Bool_t &valid)
{
...
//______________________________________________________________________________
void IlcAnalysisManager::AddClassDebug(const char *className, Int_t debugLevel)
{
// Sets Class debug level
   ...
   // substracting DebugOffset, beacuse of IlcLog::SetClassDebugLevel()
   debugLevel -= IlcLog::kDebug-1;

   TNamed *debugOpt = (TNamed*)fDebugOptions->FindObject(className);
   if (!debugOpt) {
     IlcInfo(TString::Format("Adding debug level %d for class %s",debugLevel+IlcLog::kDebug-1,className).Data());
     fDebugOptions->Add(new TNamed(className,TString::Format("%d",debugLevel).Data()));
      ...
      if (debugLevel > oldDebug) {
         IlcWarning(TString::Format("Overwriting debug level to %d class %s, because it is higher then previously set (%d).",debugLevel+IlcLog::kDebug-1,className,oldDebug+IlcLog::kDebug-1).Data());
         debugOpt->SetTitle(TString::Format("%d",debugLevel).Data());
      } else {
         IlcWarning(TString::Format("Ignoring debug level to %d class %s, because it is smaller then previously set (%d).",debugLevel+IlcLog::kDebug-1,className,oldDebug+IlcLog::kDebug-1).Data());
      }
   }
}

//______________________________________________________________________________
void IlcAnalysisManager::ApplyDebugOptions()
{
// Apply debug options
   ...
   while ((debug=dynamic_cast<TNamed*>(next()))) {
      ...
      IlcInfo(TString::Format("ApplyDebugOptions : Class=%s debulLevel=%d",debug->GetName(),debugLevel.Atoi()+IlcLog::kDebug-1).Data());
      IlcLog::SetClassDebugLevel(debug->GetName(), debugLevel.Atoi());
   }
}

//______________________________________________________________________________
void IlcAnalysisManager::Lock()
{
...
//______________________________________________________________________________
void IlcAnalysisManager::UnLock()
{
...
//______________________________________________________________________________
void IlcAnalysisManager::Changed()
{

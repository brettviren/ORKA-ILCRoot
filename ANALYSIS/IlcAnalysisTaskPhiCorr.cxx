...
#include "IlcAnalysisTaskPhiCorr.h"
#include "IlcMultiEventInputHandler.h"

ClassImp(IlcAnalysisTaskPhiCorr)

//________________________________________________________________________
IlcAnalysisTaskPhiCorr::IlcAnalysisTaskPhiCorr(const char *name) 
    : IlcAnalysisTaskME(name), fHists(0), fHistDphiCO(0),  fHistDphiUC(0)
{
  // Constructor
...
//________________________________________________________________________
void IlcAnalysisTaskPhiCorr::UserCreateOutputObjects()
{
  // Create histograms
  // Called once
...
//________________________________________________________________________
void IlcAnalysisTaskPhiCorr::UserExec(Option_t *) 
{
  // Main loop
  // Called for each event
	...
	IlcAODEvent* aod = (IlcAODEvent*) GetEvent(iev);
	...
	for (Int_t jtr = itr+1; jtr < ntrack; jtr++) {
	  IlcVParticle* track1 = fMixedEvent.GetTrack(itr);
	  IlcVParticle* track2 = fMixedEvent.GetTrack(jtr);
...
//________________________________________________________________________
void IlcAnalysisTaskPhiCorr::Terminate(Option_t *) 
{
  // Draw result to the screen
  // Called once at the end of the query

  TCanvas *c1 = new TCanvas("IlcAnalysisTaskPt","Pt",10,10,510,510);
  c1->cd(1)->SetLogy();

/* $Id: IlcTriggerAnalysis.cxx 35782 2009-10-22 11:54:31Z jgrosseo $ */
...
ClassImp(IlcTriggerAnalysis)

IlcTriggerAnalysis::IlcTriggerAnalysis() :
  fSPDGFOThreshold(2),
  fSPDGFOEfficiency(0),
  fV0TimeOffset(0),
  fV0AdcThr(0),
...
void IlcTriggerAnalysis::EnableHistograms()
{
...
const char* IlcTriggerAnalysis::GetTriggerName(Trigger trigger) 
{
...
Bool_t IlcTriggerAnalysis::IsTriggerFired(const IlcESDEvent* aEsd, Trigger trigger)
{
...
Bool_t IlcTriggerAnalysis::IsTriggerBitFired(const IlcESDEvent* aEsd, Trigger trigger) const
{
...
Bool_t IlcTriggerAnalysis::IsTriggerBitFired(ULong64_t triggerMask, Trigger trigger) const
{
  // checks if an event is fired using the trigger bits
  //
...
Bool_t IlcTriggerAnalysis::IsTriggerBitFired(const IlcESDEvent* aEsd, ULong64_t tclass) const
{
...
Int_t IlcTriggerAnalysis::EvaluateTrigger(const IlcESDEvent* aEsd, Trigger trigger)
{
  ...
  switch (triggerNoFlags)
  {
    ...
    {
      if(!offline) 
        IlcFatal(Form("Online trigger not available for trigger %d", triggerNoFlags));
    ...
    {
      if (offline)
        IlcFatal(Form("Offline trigger not available for trigger %d", triggerNoFlags));
    ...
    {
      if (offline)
        IlcFatal(Form("Offline trigger not available for trigger %d - use centrality selection", triggerNoFlags));
      if (aEsd->GetVZEROData()) {
	if (aEsd->GetVZEROData()->TestBit(IlcESDVZERO::kTriggerChargeBitsFilled)) {
	  if (aEsd->GetVZEROData()->GetTriggerBits() & (1<<IlcESDVZERO::kCTA2andCTC2))
	    decision = kTRUE;
	}
	else
	  IlcWarning("V0 centrality trigger bits were not filled!");
      }
      break;
    }
    case kSemiCentral:
    {
      ...
      if (aEsd->GetVZEROData()) {
	if (aEsd->GetVZEROData()->TestBit(IlcESDVZERO::kTriggerChargeBitsFilled)) {
	  if (aEsd->GetVZEROData()->GetTriggerBits() & (1<<IlcESDVZERO::kCTA1andCTC1))
	    decision = kTRUE;
	}
	else
	  IlcWarning("V0 centrality trigger bits were not filled!");
  ...
  case kEMCAL:
    ...
    default:
    {
      IlcFatal(Form("Trigger type %d not implemented", triggerNoFlags));
    }
  }  
...
Bool_t IlcTriggerAnalysis::IsLaserWarmUpTPCEvent(const IlcESDEvent* esd)
{
  //
  ...
  {
    IlcESDtrack *track = esd->GetTrack(i);
    if (!track) 
      continue;
    ...
    if ((status&IlcESDtrack::kITSrefit)==IlcESDtrack::kITSrefit) continue; // explicitly ask for tracks without ITS refit
    if ((status&IlcESDtrack::kTPCrefit)!=IlcESDtrack::kTPCrefit) continue;
...
Bool_t IlcTriggerAnalysis::IsOfflineTriggerFired(const IlcESDEvent* aEsd, Trigger trigger)
{
  ...
  if (decision && (trigger & kOneParticle))
  {
    ...
    const IlcESDVertex* vertex = aEsd->GetPrimaryVertexSPD();
  ...
  if (decision && (trigger & kOneTrack))
  {
	...
	if (fTPCOnly == kFALSE){
	  ...
	  // TPC only tracks
	  IlcESDtrack *tpcTrack = fEsdTrackCuts->GetTPCOnlyTrack((IlcESDEvent*)aEsd, i);
	  if (!tpcTrack){
	    IlcDebug(3,Form("track %d is NOT a TPC track",i));
	    continue;
	  }
	  else{
	    IlcDebug(3,Form("track %d IS a TPC track",i));
	    if (!(fEsdTrackCuts->AcceptTrack(tpcTrack))) {
	      IlcDebug(2, Form("TPC track %d NOT ACCEPTED, pt = %f, eta = %f",i,tpcTrack->Pt(), tpcTrack->Eta()));
	      delete tpcTrack; tpcTrack = 0x0;
	      continue;
	    } // end if the TPC track is not accepted
	    else{
	      IlcDebug(2, Form("TPC track %d ACCEPTED, pt = %f, eta = %f",i,tpcTrack->Pt(), tpcTrack->Eta()));
	      decision = kTRUE;
	      delete tpcTrack; tpcTrack = 0x0;
	      break;
      ...
      IlcDebug(4,Form("Check on the vertex not passed\n"));
      for (Int_t i=0; i<aEsd->GetNumberOfTracks(); ++i){
        if (fEsdTrackCuts->AcceptTrack(aEsd->GetTrack(i))){
	  IlcDebug(4,Form("pt of track = %f --> check would be passed if the vertex was ok\n",aEsd->GetTrack(i)->Pt()));
	  break;
	}
      }
    }
    if (!decision) IlcDebug(3,("Check for kOneTrack NOT passed\n"));
  }

  return decision;
}

Bool_t IlcTriggerAnalysis::IsTriggerClassFired(const IlcESDEvent* aEsd, const Char_t* tclass) const 
{
  ...
  for (Int_t i=1;i<tcltokens->GetEntriesFast();i=i+2) {
    ...
    if (op == '&') tcl=tcl && inpnext;
       ...
       IlcError(Form("Syntax error in %s", tclass));
       delete tcltokens;
       tcltokens = 0;
       //      tcltokens->Delete();
       return kFALSE;
...
Bool_t IlcTriggerAnalysis::IsInputFired(const IlcESDEvent* aEsd, Char_t level, UInt_t input) const
{
  ...
  switch (level)
  {
    ...
    default:
      IlcError(Form("Wrong level %i",level));
      return kFALSE;
  }
}

Bool_t IlcTriggerAnalysis::IsL0InputFired(const IlcESDEvent* aEsd, UInt_t input) const 
{
  // Checks if corresponding bit in mask is on
...
Bool_t IlcTriggerAnalysis::IsL1InputFired(const IlcESDEvent* aEsd, UInt_t input) const
{
  // Checks if corresponding bit in mask is on
...
Bool_t IlcTriggerAnalysis::IsL2InputFired(const IlcESDEvent* aEsd, UInt_t input) const 
{
  // Checks if corresponding bit in mask is on
...
void IlcTriggerAnalysis::FillHistograms(const IlcESDEvent* aEsd) 
{
  ...
  IlcESDZDC* zdcData = aEsd->GetESDZDC();
  if (zdcData)
  {
  ...
  {
    fHistZDC->Fill(-1);
    IlcError("IlcESDZDC not available");
  }
...
void IlcTriggerAnalysis::FillTriggerClasses(const IlcESDEvent* aEsd)
{
...
Int_t IlcTriggerAnalysis::SSDClusters(const IlcESDEvent* aEsd)
{
...
Int_t IlcTriggerAnalysis::SPDFiredChips(const IlcESDEvent* aEsd, Int_t origin, Bool_t fillHists, Int_t layer)
{
  ...
  if (!mult)
  {
    IlcError("IlcMultiplicity not available");
    return -1;
  }
...
Bool_t IlcTriggerAnalysis::SPDGFOTrigger(const IlcESDEvent* aEsd, Int_t origin)
{
...
Bool_t IlcTriggerAnalysis::IsSPDClusterVsTrackletBG(const IlcESDEvent* aEsd, Bool_t fillHists){
  //rejects BG based on the cluster vs tracklet correlation
  // returns true if the event is BG
  const IlcMultiplicity* mult = aEsd->GetMultiplicity();
  if (!mult){
    IlcFatal("No multiplicity object"); // TODO: Should this be fatal?
  }
...
IlcTriggerAnalysis::V0Decision IlcTriggerAnalysis::V0Trigger(const IlcESDEvent* aEsd, IlcSide side, Bool_t online, Bool_t fillHists)
{
  ...
  IlcESDVZERO* esdV0 = aEsd->GetVZEROData();
  if (!esdV0)
  {
    IlcError("IlcESDVZERO not available");
    return kV0Invalid;
  }
  IlcDebug(2,Form("In V0Trigger: %f %f",esdV0->GetV0ATime(),esdV0->GetV0CTime()));
  ...
  else if (side == kCSide)
  {
   ...
   if (esdV0->TestBit(IlcESDVZERO::kDecisionFilled)) {
    if (online) {
      if (esdV0->TestBit(IlcESDVZERO::kOnlineBitsFilled)) {
	...
	IlcWarning("V0 online trigger analysis is not yet available!");
	return kV0BB;
      }
    }
    else {
...
Float_t IlcTriggerAnalysis::V0CorrectLeadingTime(Int_t i, Float_t time, Float_t adc, Int_t runNumber) const
{
...
Float_t IlcTriggerAnalysis::V0LeadingTimeWeight(Float_t adc) const
{
...
Bool_t IlcTriggerAnalysis::ZDCTDCTrigger(const IlcESDEvent* aEsd, IlcSide side, Bool_t useZN, Bool_t useZP, Bool_t fillHists) const
{
  // Returns if ZDC triggered, based on TDC information 
  
  IlcESDZDC *esdZDC = aEsd->GetESDZDC();
...
Bool_t IlcTriggerAnalysis::ZDCTimeTrigger(const IlcESDEvent *aEsd, Bool_t fillHists) const
{
  ...
  if(fMC) {
	...
	for(Int_t j = 0; j < 4; ++j) {
	  if (esdZDC->GetZDCTDCData(12,j) != 0) {
	    ...
	    if (esdZDC->TestBit(IlcESDZDC::kCorrectedTDCFilled)) {
...
Bool_t IlcTriggerAnalysis::ZDCTrigger(const IlcESDEvent* aEsd, IlcSide side) const
{
...
Int_t IlcTriggerAnalysis::FMDHitCombinations(const IlcESDEvent* aEsd, IlcSide side, Bool_t fillHists)
{
  // returns number of hit combinations agove threshold
  //
  // Authors: FMD team, Hans Dalsgaard (code merged from FMD/IlcFMDOfflineTrigger)

  if (!fDoFMD)
    return -1;

  // Workaround for IlcESDEvent::GetFMDData is not const!
  const IlcESDFMD* fmdData = (const_cast<IlcESDEvent*>(aEsd))->GetFMDData();
  if (!fmdData)
  {
    IlcError("IlcESDFMD not available");
    return -1;
  }
    ...
    for (UShort_t ir = 0; ir < nRings; ir++) {
      ...
      for (UShort_t sec =0; sec < nsec;  sec++) {
        for (UShort_t strip = 0; strip < nstr; strip++) {
          ...
          if (mult == IlcESDFMD::kInvalidMult) continue;
...
Bool_t IlcTriggerAnalysis::FMDTrigger(const IlcESDEvent* aEsd, IlcSide side)
{
...
Long64_t IlcTriggerAnalysis::Merge(TCollection* list)
{
  ...
  while ((obj = iter->Next())) {
    ...
    IlcTriggerAnalysis* entry = dynamic_cast<IlcTriggerAnalysis*> (obj);
    if (entry == 0) 
      continue;
...
void IlcTriggerAnalysis::SaveHistograms() const
{
...
void IlcTriggerAnalysis::PrintTriggerClasses() const
{
...
//----------------------------------------------------------------------------------------------------
IlcTriggerAnalysis::T0Decision IlcTriggerAnalysis::T0Trigger(const IlcESDEvent* aEsd, Bool_t online, Bool_t fillHists) 
{
  ...
  const IlcESDTZERO* esdT0 = aEsd->GetESDTZERO();
  if (!esdT0)
  {
    IlcError("IlcESDTZERO not available");
    return kT0Invalid;
  }
  //????  IlcDebug(2,Form("In T0Trigger: %f %f",esdV0->GetV0ATime(),esdV0->GetV0CTime()));
  Float_t  tvdc[5] ;
...
//----------------------------------------------------------------------------------------------------
Bool_t IlcTriggerAnalysis::EMCALCellsTrigger(const IlcESDEvent *aEsd)
{
  //
  // Returns the EMCAL trigger decision
  // so far only implemented for LHC11a data
  // see http://ilcsoft.cern.ch/viewvc/trunk/PWGGA/EMCALTasks/IlcEmcalPhysicsSelection.cxx?view=markup&root=IlcRoot Revision 56136
  //
  ...
  IlcAnalysisManager *am = IlcAnalysisManager::GetAnalysisManager();
  am->LoadBranch("EMCALCells.");
  am->LoadBranch("CaloClusters");
  */

  // Get EMCAL cells
  IlcVCaloCells *cells = aEsd->GetEMCALCells();

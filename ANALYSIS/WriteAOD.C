#if 0
  #include "$(ILC_ROOT)/TPC/alles.h"
  #include "IlcReader.h"
  #include "IlcReaderKineTree.h"
  #include "IlcReaderESDTree.h"
  #include "IlcAODParticleCut.h"
  #include "IlcAOD.h"
  #include "IlcAODPairCut.h"
  #include "TSystem.h"
...
//these names I use when analysis is done directly from CASTOR files via RFIO
  ...
  if(!kine)
   ...
   {
    IlcReaderAOD* aodreader = new IlcReaderAOD("AOD.root");
   ...
   {
     // set reader for ESD
     IlcReaderESDTree* muonreader = new IlcReaderESDTree("IlcESDs.root");
  ...
  IlcAODParticleCut* readerpartcut= new IlcAODParticleCut();
  ...
  while (PID[l] != 0)
   {
     ...
     IlcAODParticleCut * pcut = (IlcAODParticleCut*)readerpartcut->Clone();
     IlcAODPIDCut* pidcut = new IlcAODPIDCut(PID[l],PIDprob[l]);
...
//   readerpartcut->SetPtRange(0.0,1.2);
   ...
   IlcReaderAOD::WriteAOD(reader,outfile,"IlcAODParticle",multcheck);

 ...
 */
#ifndef ILCESDV0KINECUTS_H
#define ILCESDV0KINECUTS_H
...
class IlcESDv0KineCuts : public TObject{
 public:
  ...
  IlcESDv0KineCuts();
  virtual ~IlcESDv0KineCuts();

  IlcESDv0KineCuts(const IlcESDv0KineCuts &ref);
  IlcESDv0KineCuts &operator=(const IlcESDv0KineCuts &ref);

  // main selection function - called once per V0 candidate
  Bool_t ProcessV0(IlcESDv0* const v0, Int_t &pdgV0, Int_t &pdgP, Int_t &pdgN) const;
  Bool_t ProcessV0(IlcESDv0* const v0, Int_t &pdgP, Int_t &pdgN) const;

  // must be called by the user
  void SetEvent(IlcESDEvent* const event);
  void SetEvent(IlcVEvent* const event);
  void SetPrimaryVertex(IlcKFVertex* const v);
  ...
  IlcKFParticle *CreateMotherParticle(const IlcVTrack* const pdaughter, const IlcVTrack* const ndaughter, Int_t pspec, Int_t nspec) const;
  ...
  Int_t  PreselectV0(IlcESDv0* const v0) const;

  Bool_t CaseGamma(IlcESDv0* const v0, Int_t &pdgV0, Int_t &pdgP, Int_t &pdgN) const;
  Bool_t CaseK0(IlcESDv0* const v0, Int_t &pdgV0, Int_t &pdgP, Int_t &pdgN) const;
  Bool_t CaseLambda(IlcESDv0* const v0, Int_t &pdgV0, Int_t &pdgP, Int_t &pdgN, Int_t id) const;

  Bool_t V0CutsCommon(const IlcESDv0 * const v0) const;
  Bool_t SingleTrackCuts(IlcESDv0 * const v0) const;
  void   Armenteros(IlcESDv0* const v0, Float_t val[2]) const;
  Bool_t CheckSigns(IlcESDv0* const v0) const;

  Double_t PsiPair(IlcESDv0* const v0) const;
  Bool_t   GetConvPosXY(IlcESDtrack * const ptrack, IlcESDtrack * const ntrack, Double_t convpos[2]) const;
  Bool_t   GetHelixCenter(IlcESDtrack * const track, Double_t b, Int_t charge, Double_t center[2]) const;

 protected:
  void Copy(TObject &ref) const;

 private:
  Bool_t    GammaEffCuts(IlcESDv0 * const v0);  // set of cuts optimized for high gamma efficiency

 private:
  IlcESDEvent           *fEvent;          // current event
  IlcKFVertex           *fPrimaryVertex;  // primary vertex
  ...
  // gamma cut values
  Float_t               fGcutChi2NDF;      // Chi2NF cut value for the IlcKFparticle gamma
  ...
  // K0 cut values
  Float_t               fK0cutChi2NDF;     // Chi2NF cut value for the IlcKFparticle K0
  ...
  Float_t               fLcutChi2NDF;      // Chi2NF cut value for the IlcKFparticle K0

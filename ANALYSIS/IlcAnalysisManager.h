#ifndef ILCANALYSISMANAGER_H
#define ILCANALYSISMANAGER_H
/* 2012-2013 Infrastructure for Large Collider Experiment.       *
 * All rights reserved.                                                   *
 * See cxx source for full Copyright notice                               */

/* $Id: IlcAnalysisManager.h 58336 2012-08-28 15:07:04Z hristov $ */
// Author: Andrei Gheata, 31/05/2006

//==============================================================================
//   IlcAnalisysManager - Manager analysis class. Allows creation of several
...
class TStopwatch;
class TMap;
class IlcAnalysisSelector;
class IlcAnalysisDataContainer;
class IlcAnalysisFileDescriptor;
class IlcAnalysisTask;
class IlcVEventHandler;
class IlcVEventPool;
class IlcAnalysisGrid;
class IlcAnalysisStatistics;


class IlcAnalysisManager : public TNamed {

public:

enum EIlcAnalysisContType {
...
enum EIlcAnalysisFlags {
   ...
   IlcAnalysisManager(const char *name = "mgr", const char *title="");
   virtual            ~IlcAnalysisManager();

   IlcAnalysisManager(const IlcAnalysisManager& other);
   IlcAnalysisManager& operator=(const IlcAnalysisManager& other);
   ...
   TFile              *OpenProofFile(IlcAnalysisDataContainer *cont, const char *option, const char *extaod="");
   static TFile       *OpenFile(IlcAnalysisDataContainer *cont, const char *option, Bool_t ignoreProof=kFALSE);
   ...
   // Getters/Setters
   static IlcAnalysisManager *GetAnalysisManager() {return fgAnalysisManager;}
   EIlcAnalysisExecMode 
   ...
   IlcAnalysisGrid*    GetGridHandler()           {return fGridHandler;}
   TObjArray          *GetInputs() const          {return fInputs;}
   IlcVEventHandler*   GetInputEventHandler() const   {return fInputEventHandler;}
   IlcVEventHandler*   GetMCtruthEventHandler() const {return fMCtruthEventHandler;}
   Int_t               GetNsysInfo() const        {return fNSysInfo;}
   IlcVEventHandler*   GetOutputEventHandler() const  {return fOutputEventHandler;}
   ...
   void                RunLocalInit();
   void                SetAnalysisType(EIlcAnalysisExecMode mode) {fMode = mode;}
   ...
   void                SetEventPool(IlcVEventPool* const epool)   {Changed(); fEventPool = epool;}
   void                SetFileInfoLog(const char *name) {TObject::SetBit(kCollectThroughput,kTRUE); fFileInfoLog = name;}
   void                SetGridHandler(IlcAnalysisGrid * const handler) {Changed(); fGridHandler = handler;}
   void                SetInputEventHandler(IlcVEventHandler* const handler);
   void                SetMCtruthEventHandler(IlcVEventHandler* const handler) {Changed(); fMCtruthEventHandler = handler;}
   void                SetNSysInfo(Long64_t nevents)              {fNSysInfo = nevents;}
   void                SetOutputEventHandler(IlcVEventHandler* const handler);
   void                SetRunFromPath(Int_t run)                  {fRunFromPath = run;}
   void                SetSelector(IlcAnalysisSelector * const sel)      {fSelector = sel;}
   ...
   // Container handling
   IlcAnalysisDataContainer *CreateContainer(const char *name, TClass *datatype, 
                       EIlcAnalysisContType type     = kExchangeContainer, 
                       const char          *filename = NULL);
   
   // Including tasks and getting them
   void                 AddTask(IlcAnalysisTask *task);
   IlcAnalysisTask     *GetTask(const char *name) const;
   
   // Connecting data containers to task inputs/outputs
   Bool_t               ConnectInput(IlcAnalysisTask *task, Int_t islot,
                                     IlcAnalysisDataContainer *cont);
   Bool_t               ConnectOutput(IlcAnalysisTask *task, Int_t islot,
                                     IlcAnalysisDataContainer *cont);
   // Garbage collection
   ...
   const IlcAnalysisStatistics *GetStatistics() const {return fStatistics;}
   void                 SetStatistics(IlcAnalysisStatistics *stat) {fStatistics = stat;}
...
private:
   ...
   IlcVEventHandler       *fInputEventHandler;   //  Optional common input  event handler
   IlcVEventHandler       *fOutputEventHandler;  //  Optional common output event handler
   IlcVEventHandler       *fMCtruthEventHandler; //  Optional common MC Truth event handler
   IlcVEventPool          *fEventPool;           //  Event pool for mixing analysis
   ...
   EIlcAnalysisExecMode    fMode;                // Execution mode
   ...
   IlcAnalysisFileDescriptor *fCurrentDescriptor; //! Current file descriptor
   IlcAnalysisDataContainer *fCommonInput;       // Common input container
   IlcAnalysisDataContainer *fCommonOutput;      // Common output container
   IlcAnalysisSelector    *fSelector;            //! Current selector
   IlcAnalysisGrid        *fGridHandler;         //! Grid handler plugin
   ...
   IlcAnalysisStatistics  *fStatistics;          // Statistics info about input events
   TMap                   *fGlobals;             // Map with global variables
   static TString          fgCommonFileName;     //! Common output file name (not streamed)
   static IlcAnalysisManager *fgAnalysisManager; //! static pointer to object instance
   ClassDef(IlcAnalysisManager,17)  // Analysis manager class
};   
#endif

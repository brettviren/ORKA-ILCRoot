...
#ifdef PASS1RECO
#include "IlcITSRecPoint.h"
#endif



ClassImp(IlcBackgroundSelection)

IlcBackgroundSelection::IlcBackgroundSelection():
  IlcAnalysisCuts(), fOutputHist(0), fACut(0), fBCut(0), fDeltaPhiCut(10)
{
  // ctor
...
IlcBackgroundSelection::IlcBackgroundSelection(const char* name, const char* title):
  IlcAnalysisCuts(name,title), fOutputHist(0), fACut(0), fBCut(0), fDeltaPhiCut(10)
{
  // ctor
  fOutputHist = new TList();
  fOutputHist->SetOwner();
...
IlcBackgroundSelection::IlcBackgroundSelection(const IlcBackgroundSelection& obj) : IlcAnalysisCuts(obj),
...
Bool_t IlcBackgroundSelection::IsSelected(TObject* const obj) 
{
...
#ifdef PASS1RECO
  // Get rec points
  IlcESDInputHandlerRP* handlerRP = dynamic_cast<IlcESDInputHandlerRP*> (IlcAnalysisManager::GetAnalysisManager()->GetInputEventHandler());
  if (!handlerRP)
    IlcFatal("Cannot get the IlcESDInputHandlerRP");

  TTree* itsClusterTree = handlerRP->GetTreeR("ITS");
  if (!itsClusterTree){
    IlcError("Cannot get the ITS Cluster tree");
    return kFALSE;
  }
  //    IlcFatal("Cannot get the ITS Cluster tree");

  TClonesArray* itsClusters = new TClonesArray("IlcITSRecPoint");
  ...
  IlcESDEvent * esdEv = (IlcESDEvent*) obj;
  ...
  for (Int_t iIts=0; iIts < nItsSubs; iIts++) {
    ...
    // loop over clusters
    while (nClusters--) {
      IlcITSRecPoint* cluster = (IlcITSRecPoint*) itsClusters->UncheckedAt(nClusters);
  ...
  const IlcMultiplicity* mult = esdEv->GetMultiplicity();
  if (!mult){
...
void   IlcBackgroundSelection::Init(){
...
void IlcBackgroundSelection::BookClusterVsTrackletsHisto(const char * trigger_name){
  ...
  // IlcInfo(Form("Creating histos: %s, all and accepted", GetClusterVsTrackletsHistoName(trigger_name)));
...
void IlcBackgroundSelection::BookDeltaPhiHisto(const char * trigger_name){
  ...
  // IlcInfo(Form("Creating histos: %s, all and accepted", GetDeltaPhiHistoName(trigger_name)));
...
TH2F * IlcBackgroundSelection::GetClusterVsTrackletsHisto(const char * trigger_name){
  ...
  if(!fOutputHist) {IlcError("List of histos not initialized");return 0;}
...
TH1F * IlcBackgroundSelection::GetDeltaPhiHisto(const char * trigger_name){
...
TH2F * IlcBackgroundSelection::GetClusterVsTrackletsHistoAccepted(const char * trigger_name){
...
TH1F * IlcBackgroundSelection::GetDeltaPhiHistoAccepted(const char * trigger_name){
...
const char * IlcBackgroundSelection::GetClusterVsTrackletsHistoName(const char * trigger_name){
...
const char * IlcBackgroundSelection::GetClusterVsTrackletsHistoNameAccepted(const char * trigger_name){
...
const char * IlcBackgroundSelection::GetDeltaPhiHistoName(const char * trigger_name){
...
const char * IlcBackgroundSelection::GetDeltaPhiHistoNameAccepted(const char * trigger_name){
...
Long64_t IlcBackgroundSelection::Merge(TCollection* const list)
{
  // Merge a list of IlcBackgroundSelection objects with this (needed for
  // PROOF).
  ...
  while ((obj = iter->Next())) {
    ...
    IlcBackgroundSelection* entry = dynamic_cast<IlcBackgroundSelection*> (obj);
    if (entry == 0) 
      continue;
    ...
    while(areListsDifferent) {
      if(iloop>maxLoops) IlcFatal("Infinite Loop?");
      iloop++;
      // sort
      hlist->Sort();
      fOutputHist->Sort();
	...
	if(!otherlist->FindObject(hist->GetName())){
	  //IlcInfo(Form("Adding object %s",hist->GetName()));
    ...
    if (hlist->GetSize() != fOutputHist->GetSize()) {
      IlcFatal("Mismatching size!");
    }
    Int_t nhist =  fOutputHist->GetSize();
    for(Int_t ihist = 0; ihist < nhist; ihist++){
      if(strcmp(fOutputHist->At(ihist)->GetName(),hlist->At(ihist)->GetName())){
	IlcFatal(Form("Mismatching histos: %s -> %s", fOutputHist->At(ihist)->GetName(),hlist->At(ihist)->GetName()));
      }
    }

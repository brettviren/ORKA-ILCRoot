...
/* $Id: IlcEventPoolSparse.cxx 46594 2011-01-23 20:17:57Z morsch $ */
 

/*
  Realisation of an IlcVEventPool based on THnSparse
  ...
  aliensh:> find -x charm /ilc/sim/PDC_08/LHC08x/180001 tag.root > 180001.xml
...
void testpool(const char * dirname = ".", const char * pattern = "Run180001") {
  ...
  IlcEventPoolSparse * pool = 
    new IlcEventPoolSparse("test", "test", fChain, dim, vars, nbins, xmin, xmax, chunksize);
...
ClassImp(IlcEventPoolSparse)

// _________________________________________________________________________
IlcEventPoolSparse::IlcEventPoolSparse() :
  fHnSparseI(),
  fChunkSize(1024 * 16),
  fN(0),
  fPool(0x0),
...
// _________________________________________________________________________
IlcEventPoolSparse::IlcEventPoolSparse(const char* name, const char* title, TChain * tagchain, Int_t dim,
...
// _________________________________________________________________________
void  IlcEventPoolSparse::GetCurrentBin(Float_t* xbin) {
...
void  IlcEventPoolSparse::Init(){
  ...
  // Taken and modified from IlcAnalysisTag

  if (!fTagChain) {
    IlcError("Please provide a tag chain!");
    return;
  }
  ...
  IlcEventTag *evTag = 0;  
  fTagChain->SetBranchAddress("IlcTAG",&tag); 	 
...
//       if (fEvCut)  fEvCut->UpdateFormulaLeaves();
	...
	if (strstr(tagfile->GetName(),"ESD")) fChain = new TChain("esdTree");
	  ...
	  IlcError("Only ESD and AOD type is implemented!!!");
	  delete [] x;
	  return;
	}
      }
...
// 	    IlcFileTag *eftag = (IlcFileTag *) tag->GetFileTag(ifs);
...
// 	      evTag = (IlcEventTag *) eftag->GetEventTag(i); 	 
...
// _________________________________________________________________________
void IlcEventPoolSparse::SetRunCut(const char * cut){
  // Run selection cuts
...
// _________________________________________________________________________
void IlcEventPoolSparse::SetLHCCut(const char * cut){
  // LHC selection cuts
...
// _________________________________________________________________________
void IlcEventPoolSparse::SetDetCut(const char * cut){
...
// _________________________________________________________________________
void IlcEventPoolSparse::SetEventCut(const char * cut){
...
// _________________________________________________________________________
void IlcEventPoolSparse::SetRunCut(IlcRunTagCuts* cut)
{
  fRunTagCut = cut;
}
// _________________________________________________________________________
void IlcEventPoolSparse::SetEventCut(IlcEventTagCuts* cut)
{
  fEventTagCut = cut;
}
// _________________________________________________________________________
void IlcEventPoolSparse::SetDetectorCut(IlcDetectorTagCuts* cut)
{
  fDetectorTagCut = cut;
}
// _________________________________________________________________________
void IlcEventPoolSparse::SetLHCCut(IlcLHCTagCuts* cut)
{
  fLHCTagCut = cut;
}

// _________________________________________________________________________
void IlcEventPoolSparse::Set(Int_t n){

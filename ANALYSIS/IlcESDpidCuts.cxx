...
ClassImp(IlcESDpidCuts)

const Int_t IlcESDpidCuts::kNcuts = 3;

//_____________________________________________________________________
IlcESDpidCuts::IlcESDpidCuts(const Char_t *name, const Char_t *title):
  ...
  , fHcutCorrelation(NULL)
{
  ...
  memset(fCutTPCnSigma, 0, sizeof(Float_t)* IlcPID::kSPECIES * 2);
  memset(fCutTOFnSigma, 0, sizeof(Float_t)* IlcPID::kSPECIES * 2);

  memset(fHclusterRatio, 0, sizeof(TH1F *) * 2);
  memset(fHnSigmaTPC, 0, sizeof(TH1F *) * IlcPID::kSPECIES * 2);
  memset(fHnSigmaTOF, 0, sizeof(TH1F *) * IlcPID::kSPECIES * 2);
}

//_____________________________________________________________________
IlcESDpidCuts::IlcESDpidCuts(const IlcESDpidCuts &ref):
    IlcAnalysisCuts(ref)
  , fESDpid(NULL)
  ...
  , fHcutCorrelation(NULL)
{
  ...
  // Copy constructor
  //
  fESDpid = new IlcESDpid(*ref.fESDpid);
  memcpy(fCutTPCnSigma, ref.fCutTPCnSigma, sizeof(Float_t) * IlcPID::kSPECIES * 2);
  memcpy(fCutTOFnSigma, ref.fCutTOFnSigma, sizeof(Float_t) * IlcPID::kSPECIES * 2);
...
//_____________________________________________________________________
IlcESDpidCuts &IlcESDpidCuts::operator=(const IlcESDpidCuts &ref){
  //
  // Assignment operator
  //
  if(this != &ref)
...
//_____________________________________________________________________
Bool_t IlcESDpidCuts::IsSelected(TObject *obj){
  //
  // Select Track
  // 
  IlcESDtrack * trk = dynamic_cast<IlcESDtrack*>(obj);
  if(!trk){
    IlcError("Provided object is not IlcESDtrack!");
    return kFALSE;
  }
  const IlcESDEvent* evt = trk->GetESDEvent();
  if(!evt){
    IlcError("No IlcESDEvent!");
    return kFALSE;
  }
...
//_____________________________________________________________________
void IlcESDpidCuts::Copy(TObject &c) const {
  //
  // Copy function
  //
  IlcESDpidCuts &target = dynamic_cast<IlcESDpidCuts &>(c);

  target.fESDpid = new IlcESDpid(*fESDpid);
  ...
  memcpy(target.fCutTPCnSigma, fCutTPCnSigma, sizeof(Float_t) * IlcPID::kSPECIES * 2);
  memcpy(target.fCutTOFnSigma, fCutTOFnSigma, sizeof(Float_t) * IlcPID::kSPECIES * 2);
 
  IlcESDpidCuts::Copy(c);
}

//_____________________________________________________________________
Long64_t IlcESDpidCuts::Merge(TCollection *coll){
  //
  // Merge Cut objects
  //
  if(!coll) return 0;
  ...
  TObject *o = NULL; 
  IlcESDpidCuts *ref = NULL;
  Int_t counter = 0;
  while((o = iter->Next())){
    ref = dynamic_cast<IlcESDpidCuts *>(o);
    if(!ref) continue;
...
//_____________________________________________________________________
void IlcESDpidCuts::DefineHistograms(Color_t color){
  //
  ...
  for(Int_t imode = 0; imode < 2; imode++){
    ...
    for(Int_t ispec = 0; ispec < IlcPID::kSPECIES; ispec++){
      snprintf(hname, 256, "fHnSigma%sTPC%s", IlcPID::ParticleName(ispec), imode ? "after" : "before");
      snprintf(htitle, 256, "TPC sigma for %s %s cuts;sigma;Entries", IlcPID::ParticleName(ispec), imode ? "after" : "before");
      fHnSigmaTPC[ispec][imode] = new TH1F(hname, htitle, 200, -10., 10.);
      snprintf(hname, 256, "fHnSigma%sTOF%s", IlcPID::ParticleName(ispec), imode ? "after" : "before");
      snprintf(htitle, 256, "TOF sigma for %s %s cuts;sigma;Entries", IlcPID::ParticleName(ispec), imode ? "after" : "before");
...
//_____________________________________________________________________
Bool_t IlcESDpidCuts::AcceptTrack(const IlcESDtrack *track, const IlcESDEvent *event){
  //
  ...
  if(fTOFsigmaCutRequired && event == 0)  {
    IlcError("No event pointer. Need event pointer for T0 for TOF cut");
    return (0);
  }
  ...
  Float_t nsigmaTPC[IlcPID::kSPECIES], nsigma;   // need all sigmas for QA plotting
  for(Int_t ispec = 0; ispec < IlcPID::kSPECIES; ispec++){
    nsigmaTPC[ispec] = nsigma = fESDpid->NumberOfSigmasTPC(track,(IlcPID::EParticleType)ispec);
  ...
  Float_t nsigmaTOF[IlcPID::kSPECIES];    // see above
  Bool_t hasTOFpid = track->GetStatus() & IlcESDtrack::kTOFpid; // only apply TOF n-sigma cut when PID Status Bit is set
  Double_t times[IlcPID::kSPECIES];
  track->GetIntegratedTimes(times);
  for(Int_t ispec = 0; ispec < IlcPID::kSPECIES; ispec++){
    
    if(hasTOFpid && event) nsigmaTOF[ispec] = nsigma = fESDpid->NumberOfSigmasTOF(track,(IlcPID::EParticleType)ispec, event->GetT0());
...
//_____________________________________________________________________
void IlcESDpidCuts::SaveHistograms(const Char_t * location){
  //
  // Save the histograms to a file
  //
  if(!HasHistograms()){
    IlcError("Histograms not on - Exiting");
    return;
  }
...
//_____________________________________________________________________
void IlcESDpidCuts::DrawHistograms(){
  //
  // Draw the Histograms
  //

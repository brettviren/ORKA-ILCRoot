...
IlcMixInfo::IlcMixInfo(const char *name, const char *title) :
   TNamed(name, title),
   fHistogramList(0)
{
   //
...
//_________________________________________________________________________________________________
IlcMixInfo::IlcMixInfo(const IlcMixInfo &obj) :
   TNamed(obj),
...
//_________________________________________________________________________________________________
void IlcMixInfo::CreateHistogram(IlcMixInfo::EInfoHistorgramType type, Int_t nbins, Int_t min, Int_t max)
{
   //
...
//_________________________________________________________________________________________________
void IlcMixInfo::FillHistogram(IlcMixInfo::EInfoHistorgramType type, Int_t value)
{
   //
   // Create mix info histograms
   //
   ...
   if (!fHistogramList) {
      IlcError("fHistogramList is null");
      return;
   }
   TH1I *hist = (TH1I *) fHistogramList->FindObject(GetNameHistogramByType(type));
   if (hist) {
      hist->Fill(value);
      IlcDebug(IlcLog::kDebug, Form("%s was filled with %d sum is %.0f", GetNameHistogramByType(type), value, hist->GetBinContent(value)));
   } else {
      IlcError(Form("Problem filling histogram %s", GetNameHistogramByType(type)));
   }
}

//_________________________________________________________________________________________________
const char *IlcMixInfo::GetNameHistogramByType(Int_t index) const
{
   //
   // Retruns name of cut
   //
...
//_________________________________________________________________________________________________
const char *IlcMixInfo::GetTitleHistogramByType(Int_t index) const
{
   //
   // Retruns name of cut
   //
...
//_________________________________________________________________________________________________
void IlcMixInfo::Print(Option_t *option) const
{
   //
   // Print Mix info
   //
   ...
   if (option)
      IlcInfo(Form("Name %s with option is %s", GetName(), option));
...
//_________________________________________________________________________________________________
void IlcMixInfo::Draw(Option_t *option)
{
   //
...
//     upperPad1->cd(1);
   ...
   if (!hMain) {
      IlcError("hMain is null");
      return;
   }
   TH1I *hMix =  GetHistogramByType(kMixedEvents);
   if (!hMix) {
      IlcError("hMix is null");
      return;
   }
   ...
   TH2I *hMixInfo2D = 0;
//     TH1I *hOK=0,*hBad=0;
   IlcMixEventPool *evPool = (IlcMixEventPool *) GetEventPool("mixEventPool");
   if (evPool) {
...
//     legend->Draw();
   ...
   cMain->cd(2)->AddExec("dynamic", Form("IlcMixInfo::DynamicExec((IlcMixInfo*)0x%lx)", (ULong_t)this));
}

//_________________________________________________________________________________________________
void IlcMixInfo::DynamicExec(IlcMixInfo *const mixInfo)
{
   //
...
//    Int_t mixNum = 1;
   if (text) {
      if (mixInfo) {
         IlcMixEventPool *evPool = (IlcMixEventPool *) mixInfo->GetEventPool("mixEventPool");
         if (evPool) {
...
//_________________________________________________________________________________________________
Long64_t IlcMixInfo::Merge(TCollection *list)
{
   //
   // Merge function
   //
   ...
   IlcMixInfo *mi = 0;
   Long64_t counter = 0;
   while ((mi = (IlcMixInfo *) nxfc())) {
      // Do not merge with ourself
      if (mi == this) continue;
      // Make sure that it is a IlcMixInfo
      if (!mi->InheritsFrom(IlcMixInfo::Class())) {
...
TH1I *IlcMixInfo::GetHistogramByType(Int_t index) const
{
   //
   // GetHistogramByType
   //
...
//_________________________________________________________________________________________________
void IlcMixInfo::Add(IlcMixInfo *mi)
{
   //
   // adds IlcMixInfo
   //

//    IlcInfo(Form("Adding %p", mi));
   if (!mi) return;
   if (!fHistogramList) return;
   TH1I *hMain = GetHistogramByType(kMainEvents);
   if (!hMain) {
...
//_________________________________________________________________________________________________
void IlcMixInfo::SetEventPool(IlcMixEventPool *evPool)
{
   //
   // Sets event pool
   //
...
//_________________________________________________________________________________________________
IlcMixEventPool *IlcMixInfo::GetEventPool(const char *name)
{
   //
   // Gets event pool
   //
   ...
   return (IlcMixEventPool *) fHistogramList->FindObject(name);
}

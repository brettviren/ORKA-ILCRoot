...
#ifndef ILCMIXINPUTEVENTHANDLER_H
#define ILCMIXINPUTEVENTHANDLER_H
...
class IlcMixInputEventHandler : public IlcMultiInputEventHandler {

public:
   IlcMixInputEventHandler(const Int_t size = 1, const Int_t mixNum = 1);

   // From the interface
   virtual Bool_t  Init(Option_t *opt) { return IlcMultiInputEventHandler::Init(opt); }
   ...
   void                    SelectCollisionCandidates(UInt_t offlineTriggerMask = IlcVEvent::kMB) {fOfflineTriggerMask = offlineTriggerMask;}
...
private:
   ...
   IlcMixInputEventHandler(const IlcMixInputEventHandler &handler);
   IlcMixInputEventHandler &operator=(const IlcMixInputEventHandler &handler);

   ClassDef(IlcMixInputEventHandler, 3)
};

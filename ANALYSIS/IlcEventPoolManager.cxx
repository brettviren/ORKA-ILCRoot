...
using std::cout;
using std::endl;
ClassImp(IlcEventPool)

void IlcEventPool::PrintInfo() const
{
...
Bool_t IlcEventPool::EventMatchesBin(Int_t mult, Double_t zvtx) const
{
  return EventMatchesBin((Double_t) mult, zvtx);
}

Bool_t IlcEventPool::EventMatchesBin(Double_t mult, Double_t zvtx) const
{
...
Int_t IlcEventPool::NTracksInPool() const
{
...
Int_t IlcEventPool::SetEventMultRange(Int_t multMin, Int_t multMax)
{
...
Int_t IlcEventPool::SetEventMultRange(Double_t multMin, Double_t multMax)
{
  fMultMin = multMin;
  fMultMax = multMax;
  return 0;
...
Int_t IlcEventPool::SetEventZvtxRange(Double_t zvtxMin, Double_t zvtxMax)
{
  fZvtxMin = zvtxMin;
  fZvtxMax = zvtxMax;
  return 0;
...
Int_t IlcEventPool::GlobalEventIndex(Int_t j) const
{
  // Index returned from passing local pool event index.

  if (j < 0 || j >= (Int_t)fEventIndex.size()) {
    cout << "ERROR in IlcEventPool::GlobalEventIndex(): "
...
Int_t IlcEventPool::UpdatePool(TObjArray *trk)
{
...
TObject* IlcEventPool::GetRandomTrack() const
{
...
TObjArray* IlcEventPool::GetEvent(Int_t i) const
{
  if (i<0 || i>=(Int_t)fEvents.size()) {
    cout << "IlcEventPool::GetEvent(" 
...
TObjArray* IlcEventPool::GetRandomEvent() const
{
  UInt_t ranEvt = gRandom->Integer(fEvents.size());
  TObjArray *tca = fEvents.at(ranEvt);
  return tca;
...
Int_t IlcEventPool::NTracksInEvent(Int_t iEvent) const
{
...
ClassImp(IlcEventPoolManager)

IlcEventPoolManager::IlcEventPoolManager(Int_t depth,     Int_t minNTracks,
...
fDebug(0), fNMultBins(0), fNZvtxBins(0), fEvPool(0), fTargetTrackDepth(minNTracks) 
{
  ...
  cout << "IlcEventPoolManager initialized." << endl;
}

Int_t IlcEventPoolManager::InitEventPools(Int_t depth, 
                                       Int_t nMultBins, Double_t *multbin, 
                                       Int_t nZvtxBins, Double_t *zvtxbin)
{
  // Assign IlcEventPoolManager members.

  fNMultBins = nMultBins;
  fNZvtxBins = nZvtxBins;

  for (Int_t iM=0; iM<nMultBins; iM++) {
    std::vector<IlcEventPool*> evp;
    for (Int_t iZ=0; iZ<nZvtxBins; iZ++) {
      evp.push_back(new IlcEventPool(depth, 
...
IlcEventPool *IlcEventPoolManager::GetEventPool(Int_t iMult, Int_t iZvtx) const
{
...
IlcEventPool *IlcEventPoolManager::GetEventPool(Int_t centVal, Double_t zVtxVal) const
{
  return GetEventPool((Double_t)centVal, zVtxVal);
}

IlcEventPool *IlcEventPoolManager::GetEventPool(Double_t centVal, Double_t zVtxVal) const
{
  ...
  for (Int_t iM=0; iM<fNMultBins; iM++) {
    for (Int_t iZ=0; iZ<fNZvtxBins; iZ++) {
      IlcEventPool* pool = GetEventPool(iM, iZ);
...
Int_t IlcEventPoolManager::UpdatePools(TObjArray *trk)
{

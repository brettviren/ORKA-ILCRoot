#ifndef ILCANALYSISILCEN_H
#define ILCANALYSISILCEN_H
...
class IlcAnalysisTaskCfg;
class TGridJDL;

class IlcAnalysisAlien : public IlcAnalysisGrid {

public:

   IlcAnalysisAlien();
   IlcAnalysisAlien(const char *name);
   virtual ~IlcAnalysisAlien();
   IlcAnalysisAlien(const IlcAnalysisAlien& other); 
   IlcAnalysisAlien& operator=(const IlcAnalysisAlien& other);
// Setters   
   ...
   virtual void        SetIlcROOTVersion(const char *version)            {fIlcROOTVersion=version;}
   ...
   virtual void        SetDataPattern(const char *pattern="*IlcESDs.root") {fDataPattern = pattern;}
...
//Utilities
   void                AddModule(IlcAnalysisTaskCfg *module);
   void                AddModules(TObjArray *list);
   IlcAnalysisManager *CreateAnalysisManager(const char *name, const char *filename="");
   Int_t               GetNmodules() const;
   IlcAnalysisTaskCfg *GetModule(const char *name);
   Bool_t              LoadModules();
   ...
   static IlcAnalysisManager *LoadAnalysisManager(const char *fname);
...
// PROOF mode
   ...
   virtual void        SetIlcRootMode(const char *mode)                  {fIlcRootMode = mode;}
...
protected:
   ...
   Bool_t              IsUsingTags() const {return TObject::TestBit(IlcAnalysisGrid::kUseTags);}
   Bool_t              LoadModule(IlcAnalysisTaskCfg *mod);
...
private:
   ...
   TString          fIlcRootMode;     // IlcRoot mode among the list supported by the proof cluster
   ...
   TObjArray       *fModules;         // List of IlcAnalysisTaskCfg modules
   TMap             fProofParam;      // Key-value pairs for proof mode
   
   ClassDef(IlcAnalysisAlien, 20)   // Class providing some AliEn utilities
};
#endif

...
class IlcAnalysisGrid : public TNamed {
...
enum EPluginBits {
   ...
   IlcAnalysisGrid() : TNamed(), fSpecialBits(0) {}
   IlcAnalysisGrid(const char *name) : TNamed(name,""), fSpecialBits(0) {}
   virtual ~IlcAnalysisGrid() {}
   IlcAnalysisGrid(const IlcAnalysisGrid& other); 
   IlcAnalysisGrid& operator=(const IlcAnalysisGrid& other);
// Getters
...
// Setters   
   ...
   virtual void        SetIlcROOTVersion(const char *version)            = 0;
...
// PROOF mode
   ...
   virtual void        SetIlcRootMode(const char *mode)                  = 0;
...
protected:
   ...
   ClassDef(IlcAnalysisGrid, 2)   // Base class for GRID utilities
};
#endif

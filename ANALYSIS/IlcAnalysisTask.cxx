...
/* $Id: IlcAnalysisTask.cxx 52122 2011-10-14 08:43:45Z agheata $ */
// Author: Andrei Gheata, 31/05/2006

//==============================================================================
//   IlcAnalisysTask - Class representing a basic analysis task. Any
...
// connected by the derived analysis task, but from the level of IlcAnalysisManager:
//
//   IlcAnalysisManager::ConnectInput(IlcAnalysisTask *task, Int_t islot,
//                                   IlcAnalysisDataContainer *cont)
//   IlcAnalysisManager::ConnectOutput(IlcAnalysisTask *task, Int_t islot,
//                                    IlcAnalysisDataContainer *cont)
...
ClassImp(IlcAnalysisTask)

//______________________________________________________________________________
IlcAnalysisTask::IlcAnalysisTask()
                :fReady(kFALSE),
                 fInitialized(kFALSE),
                 fNinputs(0),
                 fNoutputs(0),
...
//______________________________________________________________________________
IlcAnalysisTask::IlcAnalysisTask(const char *name, const char *title)
                :TTask(name,title),
                 fReady(kFALSE),
                 fInitialized(kFALSE),
                 fNinputs(0),
...
//______________________________________________________________________________
IlcAnalysisTask::IlcAnalysisTask(const IlcAnalysisTask &task)
                :TTask(task),
                 fReady(task.fReady),
...
//______________________________________________________________________________
IlcAnalysisTask& IlcAnalysisTask::operator=(const IlcAnalysisTask& task)
{
// Assignment
   ...
   fPublishedData = 0;
   Int_t i;
   for (i=0; i<fNinputs; i++) fInputs->AddAt(new IlcAnalysisDataSlot(*task.GetInputSlot(i)),i);
   fOutputReady = new Bool_t[(fNoutputs)?fNoutputs:2];
   for (i=0; i<fNoutputs; i++) {
      fOutputReady[i] = IsOutputReady(i);
      fOutputs->AddAt(new IlcAnalysisDataSlot(*task.GetOutputSlot(i)),i);
   }         
   fBranchNames = task.fBranchNames;
   return *this;
}
...
// Check if all input/output slots are connected. If this is the case fReady=true
   ...
   for (i=0; i<fNoutputs; i++) {
      slot = (IlcAnalysisDataSlot*)fOutputs->At(i);
      if (!slot) {
...
//______________________________________________________________________________
void IlcAnalysisTask::CheckNotify(Bool_t init)
{
...
// to a container where the data was published.
   ...
   IlcAnalysisDataContainer *cinput;
...
//______________________________________________________________________________
Bool_t IlcAnalysisTask::CheckPostData() const
{
...
// not have output slots this returns always kTRUE.
   ...
   IlcAnalysisDataContainer *coutput;
...
//______________________________________________________________________________
Bool_t IlcAnalysisTask::CheckOwnership() const
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisTask::ConnectInput(Int_t islot, IlcAnalysisDataContainer *cont)
{
// Connect an input slot to a data container.
   IlcAnalysisDataSlot *input = GetInputSlot(islot);
   if (!input) {
...
//______________________________________________________________________________
Bool_t IlcAnalysisTask::ConnectOutput(Int_t islot, IlcAnalysisDataContainer *cont)
{
// Connect an output slot to a data container.
   IlcAnalysisDataSlot *output = GetOutputSlot(islot);
   if (!output) {
...
//______________________________________________________________________________
void IlcAnalysisTask::DefineInput(Int_t islot, TClass *type)
{
// Define an input slot and its type.
   IlcAnalysisDataSlot *input = new IlcAnalysisDataSlot(type, this);
   if (fNinputs<islot+1) fNinputs = islot+1;
   fInputs->AddAtAndExpand(input, islot);
}

//______________________________________________________________________________
void IlcAnalysisTask::DefineOutput(Int_t islot, TClass *type)
{
// Define an output slot and its type.
   IlcAnalysisDataSlot *output = new IlcAnalysisDataSlot(type, this);
...
//______________________________________________________________________________
TClass *IlcAnalysisTask::GetInputType(Int_t islot) const
{
...
//______________________________________________________________________________
TClass *IlcAnalysisTask::GetOutputType(Int_t islot) const
{
...
//______________________________________________________________________________
TObject *IlcAnalysisTask::GetInputData(Int_t islot) const
{
...
//______________________________________________________________________________
TObject *IlcAnalysisTask::GetOutputData(Int_t islot) const
{
...
//______________________________________________________________________________
char *IlcAnalysisTask::GetBranchAddress(Int_t islot, const char *branch) const
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisTask::SetBranchAddress(Int_t islot, const char *branch, void *address) const
{
...
//______________________________________________________________________________
void IlcAnalysisTask::EnableBranch(Int_t islot, const char *bname) const
{
...
// task. "*" will enable everything.
   ...
   IlcAnalysisDataSlot::EnableBranch(bname, tree);
}

//______________________________________________________________________________
void IlcAnalysisTask::FinishTaskOutput()
{
...
//______________________________________________________________________________
void IlcAnalysisTask::ConnectInputData(Option_t *)
{
// Overload and connect your branches here.
}

//______________________________________________________________________________
void IlcAnalysisTask::LocalInit()
{
// The method LocalInit() may be implemented to call locally (on the client)
// all initialization methods of the class. It is not mandatory and was created
// in order to minimize the complexity and readability of the analysis macro.
...
//______________________________________________________________________________
void IlcAnalysisTask::CreateOutputObjects()
{
...
//______________________________________________________________________________
TFile *IlcAnalysisTask::OpenFile(Int_t iout, Option_t *option) const
{
...
// }
   ...
   IlcAnalysisDataContainer *cont = GetOutputSlot(iout)->GetContainer();
   return IlcAnalysisManager::OpenFile(cont, option);
}

//______________________________________________________________________________
Bool_t IlcAnalysisTask::Notify()
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisTask::NotifyBinChange()
{
...
//______________________________________________________________________________
void IlcAnalysisTask::Terminate(Option_t *)
{
// Method called by the framework at the end of data processing.
}

//______________________________________________________________________________
Bool_t IlcAnalysisTask::PostData(Int_t iout, TObject *data, Option_t *option)
{
...
// an associated file name defined. The option represents the method to open the file.
   fPublishedData = 0;
   IlcAnalysisDataSlot *output = GetOutputSlot(iout);
   if (!output) {
...
//______________________________________________________________________________
void IlcAnalysisTask::SetUsed(Bool_t flag)
{
// Set 'used' flag recursively to task and all daughter tasks.
   ...
   for (Int_t i=0; i<nd; i++) {
      task = (IlcAnalysisTask*)fTasks->At(i);
      task->SetUsed(flag);
   }
}   

//______________________________________________________________________________
Bool_t IlcAnalysisTask::CheckCircularDeps()
{
...
// a hierarchical parent as subtask.
   ...
   for (Int_t i=0; i<ntasks; i++) {
      task = (IlcAnalysisTask*)tasks->At(i);
...
//______________________________________________________________________________
void IlcAnalysisTask::PrintTask(Option_t *option, Int_t indent) const
{
// Print task info.
   TString opt(option);
   opt.ToLower();
   ...
   if (!dep || (dep && IsChecked())) {
      ...
      if (dep) const_cast<IlcAnalysisTask*>(this)->SetChecked(kFALSE);
      else {
...
//______________________________________________________________________________
void IlcAnalysisTask::PrintContainers(Option_t *option, Int_t indent) const
{
...
//______________________________________________________________________________
void IlcAnalysisTask::SetPostEventLoop(Bool_t flag)
{
...
//______________________________________________________________________________
void IlcAnalysisTask::GetBranches(const char *type, TString &result) const
{

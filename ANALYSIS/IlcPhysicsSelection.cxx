/* $Id: IlcPhysicsSelection.cxx 35782 2009-10-22 11:54:31Z jgrosseo $ */
...
// the IsEventInBinZero method of IlcAnalysisTaskSE, and should thus
...
using std::cout;
using std::endl;
ClassImp(IlcPhysicsSelection)

IlcPhysicsSelection::IlcPhysicsSelection() :
  IlcAnalysisCuts("IlcPhysicsSelection", "IlcPhysicsSelection"),
  fCurrentRun(-1),
  fMC(kFALSE),
  fCollTrigClasses(),
  fBGTrigClasses(),
  ...
  
{
  ...
  IlcLog::SetClassDebugLevel("IlcPhysicsSelection", IlcLog::kWarning);
}
...
UInt_t IlcPhysicsSelection::CheckTriggerClass(const IlcESDEvent* aEsd, const char* trigger, Int_t& triggerLogic) const
{
  ...
  //   if successful, YY is returned (for association between entry in fCollTrigClasses and IlcVEvent::EOfflineTriggerTypes)
  //   triggerLogic is filled with ZZ, defaults to kCINT1
  
  Bool_t foundBCRequirement = kFALSE;
  Bool_t foundCorrectBC = kFALSE;
  
  UInt_t returnCode = IlcVEvent::kUserDefined;
  triggerLogic = kCINT1;
  
  IlcDebug(IlcLog::kDebug+1, Form("Processing event with triggers %s", aEsd->GetFiredTriggerClasses().Data()));
  ...
  for (Int_t i=0; i < tokens->GetEntries(); i++)
  {
    ...
    if (str2[0] == '+' || str2[0] == '-')
    {
      ...
      for (Int_t j=0; j < tokens2->GetEntries(); j++)
      {
        ...
        if (!flag && aEsd->IsTriggerClassFired(str3))
        {
          IlcDebug(IlcLog::kDebug+1, Form("Rejecting event because trigger class %s is present", str3.Data()));
          delete tokens2;
          delete tokens;
          return kFALSE;
        }
      ...
      {
        IlcDebug(IlcLog::kDebug+1, Form("Rejecting event because (none of the) trigger class(es) %s is present", str2.Data()));
        delete tokens;
        return kFALSE;
      }
    }
    else if (str2[0] == '#')
    {
      ...
      IlcDebug(IlcLog::kDebug+1, Form("Checking for bunch crossing number %d", bcNumber));
      
      if (aEsd->GetBunchCrossNumber() == bcNumber)
      {
        foundCorrectBC = kTRUE;
        IlcDebug(IlcLog::kDebug+1, Form("Found correct bunch crossing %d", bcNumber));
      }
    }
    else if (str2[0] == '&')
    {
    ...
    else if (str2[0] == '*')
    {
      ...
      IlcFatal(Form("Invalid trigger syntax: %s", trigger));
  }
...
TObject *IlcPhysicsSelection::GetStatistics(const Option_t *option) const
{
...
//______________________________________________________________________________
Bool_t IlcPhysicsSelection::EvaluateTriggerLogic(const IlcESDEvent* aEsd, IlcTriggerAnalysis* triggerAnalysis, const char* triggerLogic, Bool_t offline)
{
  ...
  while (1)
  {
    IlcDebug(IlcLog::kDebug, trigger.Data());
    ...
    if (!param)
    {
      ...
      Int_t bit = gInterpreter->ProcessLine(Form("IlcTriggerAnalysis::k%s;", token.Data()), &error);
      
      if (error > 0)
        IlcFatal(Form("Trigger token %s unknown", token.Data()));
      
      param = new TParameter<Int_t>(token, bit);
      fCashedTokens->Add(param);
      IlcDebug(IlcLog::kDebug, "Added token");
    }
    
    Long64_t bit = param->GetVal();
    
    IlcDebug(IlcLog::kDebug, Form("Tok %d %d %s %lld", pos[0], pos[1], token.Data(), bit));
    
    if (offline) 
      bit |= IlcTriggerAnalysis::kOfflineFlag;
    
    if(aEsd && triggerAnalysis) {
      trigger.ReplaceAll(token, Form("%d", triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) bit)));
      //      if(fHistStatisticsTokens) 	      
    }
  }
  ...
  if (formula.Compile() > 0)
    IlcFatal(Form("Could not evaluate trigger logic %s (evaluated to %s)", triggerLogic, trigger.Data()));
  Bool_t result = formula.Eval(0);
  
  IlcDebug(IlcLog::kDebug, Form("%s --> %d", trigger.Data(), result));
  
  return result;
}

//______________________________________________________________________________
UInt_t IlcPhysicsSelection::IsCollisionCandidate(const IlcESDEvent* aEsd)
{
  // checks if the given event is a collision candidate
  //
  // returns a bit word describing the fired offline triggers (see IlcVEvent::EOfflineTriggerTypes)
  IlcAnalysisManager *mgr = IlcAnalysisManager::GetAnalysisManager();
  if (!mgr) {
    IlcError("Cannot get the analysis manager");
    return 0;
  } 
  mgr->LoadBranch("IlcESDHeader.");
  mgr->LoadBranch("IlcESDRun.");

  if (fCurrentRun != aEsd->GetRunNumber()) {
    if (!Initialize(aEsd))
      IlcFatal(Form("Could not initialize for run %d", aEsd->GetRunNumber()));
    if(fComputeBG) SetBIFactors(aEsd); // is this safe here?
  }
  const IlcESDHeader* esdHeader = aEsd->GetHeader();
  if (!esdHeader)
  {
    IlcError("ESD Header could not be retrieved");
    return kFALSE;
  }
  ...
  else
  {
    if (esdHeader->GetEventType() != 0)
      IlcFatal(Form("Invalid event type for MC: %d", esdHeader->GetEventType()));
  }
  
  mgr->LoadBranch("IlcMultiplicity.");
//mgr->LoadBranch("IlcESDFMD.");
  mgr->LoadBranch("IlcESDVZERO.");
  mgr->LoadBranch("IlcESDZDC.");
  ...
  for (Int_t i=0; i < count; i++)
  {
    ...
    IlcDebug(IlcLog::kDebug+1, Form("Processing trigger class %s", triggerClass));
  
    IlcTriggerAnalysis* triggerAnalysis = static_cast<IlcTriggerAnalysis*> (fTriggerAnalysis.At(i));
    ...
    if (singleTriggerResult)
    {
      ...
      if (fBin0CallBack != "") {
	  isBin0 = ((IlcAnalysisTaskSE*)mgr->GetTask(fBin0CallBack.Data()))->IsEventInBinZero();
      ...
      // (with all IlcTriggerAnalysis tokens? Only we the tokens
      // actually used in the selection?) and clean up

      // hardware trigger
      Int_t fastORHW   = triggerAnalysis->EvaluateTrigger(aEsd, IlcTriggerAnalysis::kSPDGFO); // SPD number of chips from trigger bits (!)
      //      Int_t fastORHWL1 = triggerAnalysis->EvaluateTrigger(aEsd, IlcTriggerAnalysis::kSPDGFOL1); // SPD number of chips from trigger bits in second layer (!)
      Bool_t v0AHW     = fSkipV0 ? 0 : triggerAnalysis->EvaluateTrigger(aEsd, IlcTriggerAnalysis::kV0A);// should replay hw trigger
      Bool_t v0CHW     = fSkipV0 ? 0 : triggerAnalysis->EvaluateTrigger(aEsd, IlcTriggerAnalysis::kV0C);// should replay hw trigger
      
      // offline trigger
      Int_t fastOROffline   = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kSPDGFO)); // SPD number of chips from clusters (!)
      Int_t fastOROfflineL1 = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kSPDGFOL1)); // SPD number of chips from clusters in second layer (!)
      Bool_t v0A       = fSkipV0 ? 0 : triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kV0A));
      Bool_t v0C       = fSkipV0 ? 0 : triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kV0C));
      Bool_t v0ABG = fSkipV0 ? 0 : triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kV0ABG));
      Bool_t v0CBG = fSkipV0 ? 0 : triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kV0CBG));
      Bool_t v0BG = v0ABG || v0CBG;
      Bool_t t0       = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kT0    ));
      Bool_t t0BG     = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kT0BG    ));
      Bool_t t0PileUp = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kT0Pileup));

      // fmd
      // Bool_t fmdA = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kFMDA));
      // Bool_t fmdC = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kFMDC));
      ...
      // ZDC
      // Bool_t zdcA = triggerAnalysis->IsOfflineTriggerFired(aEsd, IlcTriggerAnalysis::kZDCA);
      // Bool_t zdcC = triggerAnalysis->IsOfflineTriggerFired(aEsd, IlcTriggerAnalysis::kZDCC);
      Bool_t zdcA    = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kZDCTDCA));
      Bool_t zdcC    = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kZDCTDCC));
      Bool_t zdcTime = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kZDCTime));

      Bool_t laserCut = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kTPCLaserWarmUp));
      ...
      Bool_t bgID = kFALSE;
      bgID = triggerAnalysis->EvaluateTrigger(aEsd,  (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kSPDClsVsTrkBG | IlcTriggerAnalysis::kOfflineFlag)); // FIXME: temporarily, we keep both ways to validate the new one. if the external BG id is not set, it will use the new one
      ...
      // Bool_t emcCut = triggerAnalysis->EvaluateTrigger(aEsd, (IlcTriggerAnalysis::Trigger) (IlcTriggerAnalysis::kOfflineFlag | IlcTriggerAnalysis::kEMCAL));
      ...
      IlcDebug(IlcLog::kDebug, Form("Triggers from OADB [0x%x][%d][%s][%s]",singleTriggerResult,IlcOADBPhysicsSelection::GetActiveBit(singleTriggerResult),triggerLogicOffline.Data(),triggerLogicOnline.Data()));
	...
	if (!onlineTrigger)
	  {
	    IlcDebug(IlcLog::kDebug, "Rejecting event because hardware trigger is not fired");
	    continue;
	  } else {       
	...
	if ( offlineTrigger )
	  {
		...
		IlcDebug(IlcLog::kDebug, Form("Accepted event for histograms with trigger logic %d", triggerLogic));
		...
		if((i < fCollTrigClasses.GetEntries() || fSkipTriggerClassSelection) && (iHistStat==kStatIdxAll))
	      ...
	      IlcDebug(IlcLog::kDebug, "Rejecting event because of V0 BG flag");
	  }
	else
	  IlcDebug(IlcLog::kDebug, Form("Rejecting event because trigger logic %d is not fulfilled", triggerLogic));
      }
    }
  }
 
  if (accept)
    IlcDebug(IlcLog::kDebug, Form("Accepted event as collision candidate with bit mask %d", accept));
  
  return accept;
}


// const char * IlcPhysicsSelection::GetFillingScheme(UInt_t runNumber)  {
...
//   }
//   else {
//     IlcError(Form("Unknown filling scheme (run %d)", runNumber));
//   }

//   return "Unknown";
// }

// const char * IlcPhysicsSelection::GetBXIDs(UInt_t runNumber, const char * trigger)  {
...
//     //    else IlcError(Form("Unknown trigger: %s", trigger));
//   } 
//   else if (runNumber >= 104315 && runNumber <= 104321) {
...
//   else {
//     IlcWarning(Form("Unknown run %d, using all BXs!",runNumber));
//   }

//   return "";
// }

Bool_t IlcPhysicsSelection::Initialize(const IlcESDEvent* aEsd)
{
  // initializes the object for the given ESD
  
  IlcInfo(Form("Initializing for beam type: %s", aEsd->GetESDRun()->GetBeamType()));
  fIsPP = kTRUE;
...
Bool_t IlcPhysicsSelection::Initialize(Int_t runNumber)
{
  ...
  TString oadbfilename = IlcPhysicsSelection::GetOADBFileName();
  ...
  if(!fPSOADB || !fUsingCustomClasses) { // if it's already set and custom class is required, we use the one provided by the user
    ...
    IlcOADBContainer * psContainer = (IlcOADBContainer*) foadb->Get("physSel");
    if (!psContainer) IlcFatal("Cannot fetch OADB container for Physics selection");
    fPSOADB = (IlcOADBPhysicsSelection*) psContainer->GetObject(runNumber, fIsPP ? "oadbDefaultPP" : "oadbDefaultPbPb");
    if (!fPSOADB) IlcFatal(Form("Cannot find physics selection object for run %d", runNumber));
  } else {
    IlcInfo("Using Custom OADB");
  }
  if(!fFillOADB || !fUsingCustomClasses) { // if it's already set and custom class is required, we use the one provided by the user
    IlcOADBContainer * fillContainer = (IlcOADBContainer*) foadb->Get("fillScheme");
    if (!fillContainer) IlcFatal("Cannot fetch OADB container for filling scheme");
    fFillOADB = (IlcOADBFillingScheme*) fillContainer->GetObject(runNumber, "Default");
    if (!fFillOADB) IlcFatal(Form("Cannot find  filling scheme object for run %d", runNumber));
  }
  if(!fTriggerOADB || !fUsingCustomClasses) { // if it's already set and custom class is required, we use the one provided by the user
    IlcOADBContainer * triggerContainer = (IlcOADBContainer*) foadb->Get("trigAnalysis");
    if (!triggerContainer) IlcFatal("Cannot fetch OADB container for trigger analysis");
    fTriggerOADB = (IlcOADBTriggerAnalysis*) triggerContainer->GetObject(runNumber, "Default");
    fTriggerOADB->Print();
    if (!fTriggerOADB) IlcFatal(Form("Cannot find  trigger analysis object for run %d", runNumber));
  }

  
  if(!fBin0CallBack) 
    IlcError("Bin0 Callback not set: will not fill the statistics for the bin 0");
  ...
  //   IlcFatal("Processing several runs with different trigger schemes is not supported");
  
  if(fComputeBG && fCurrentRun != -1 && fCurrentRun != runNumber) 
    IlcFatal("Cannot process several runs because BG computation is requested");

  if(fComputeBG && !fUseBXNumbers) 
    IlcFatal("Cannot compute BG if BX numbers are not used");
  
  if(fUseBXNumbers && fFillingScheme != "" && fFillingScheme != fFillOADB->GetFillingSchemeName())
    IlcFatal("Cannot process runs with different filling scheme if usage of BX numbers is requested");

  fFillingScheme      = fFillOADB->GetFillingSchemeName();

  IlcInfo(Form("Initializing for run %d", runNumber));
  ...
  if (fCurrentRun == -1)
  {
    ...
    for (Int_t i=0; i<count; i++)
    {
      ...
      IlcTriggerAnalysis* triggerAnalysis = new IlcTriggerAnalysis;
...
TH2F * IlcPhysicsSelection::BookHistStatistics(const char * tag) {
...
void IlcPhysicsSelection::Print(const Option_t *option) const
{
  ...
  IlcTriggerAnalysis* triggerAnalysis = dynamic_cast<IlcTriggerAnalysis*> (fTriggerAnalysis.At(0));
...
Long64_t IlcPhysicsSelection::Merge(TCollection* list)
{
  // Merge a list of IlcMultiplicityCorrection objects with this (needed for
  // PROOF).
  ...
  while ((obj = iter->Next())) {
    ...
    IlcPhysicsSelection* entry = dynamic_cast<IlcPhysicsSelection*> (obj);
    if (entry == 0) 
      continue;
    ...
    if (fCurrentRun != currentRun)
       IlcWarning(Form("Current run %d not matching the one to be merged with %d", fCurrentRun, currentRun));
    ...
    if (fBGStatOffset != bgstatoffset)
      IlcWarning(Form("Current run %d not matching the one to be merged with %d", fBGStatOffset, bgstatoffset));
    

    
    // Merge the OADBs (Take just the first instance you find
    if (!fPSOADB && entry->GetOADBPhysicsSelection()) {
      fPSOADB = (IlcOADBPhysicsSelection*) entry->GetOADBPhysicsSelection()->Clone();
    }
    if (!fFillOADB && entry->GetOADBFillingScheme()){
      fFillOADB = (IlcOADBFillingScheme*) entry->GetOADBFillingScheme()->Clone();
    }
...
void IlcPhysicsSelection::SaveHistograms(const char* folder) 
{
  ...
  if (fComputeBG) {      
    IlcInfo("BG estimate assumes that for a given run you only have A and C triggers separately or"
	...
	if (nEvents[kClassB]>0) {
	  ...
	  if(icol == 1) {
	    if(fBIFactorAC > 0 || fBIFactorA > 0 || fBIFactorC > 0) {
	      if (fBIFactorAC <= 0 && (fBIFactorA <= 0 || fBIFactorC <= 0)) {
		IlcError("Not all intensities set!, assuming equal intensities");
		fBIFactorA  = 1;
		fBIFactorC  = 1;
		fBIFactorAC = 1;
	      } else {
		IlcInfo("Using ratio of number of bunch crossing embedded in the intensity measurements");
		ratioToB[kClassA]  = ratioToB[kClassA]  >0 ? 1  : 0;
		ratioToB[kClassC]  = ratioToB[kClassC]  >0 ? 1  : 0;
		ratioToB[kClassAC] = ratioToB[kClassAC] >0 ? 1  : 0;
		IlcInfo(Form(" - BI Factor A:  %f",  fBIFactorA ));
		IlcInfo(Form(" - BI Factor C:  %f",  fBIFactorC ));
		IlcInfo(Form(" - BI Factor AC: %f",  fBIFactorAC ));
		
	      }
	    } else {
	      IlcWarning("Intensities not set!, assuming equal intensities");
	      fBIFactorA  = 1;
	      fBIFactorC  = 1;
	      fBIFactorAC = 1;
	    }
  ...
  for (Int_t i=0; i < count; i++)
	...
	triggerClass += ((TObjString*) fBGTrigClasses.At(i - fCollTrigClasses.GetEntries()))->String();
      ...
      static_cast<IlcTriggerAnalysis*> (fTriggerAnalysis.At(i))->SaveHistograms();
...
Int_t IlcPhysicsSelection::GetStatRow(const char * triggerBXClass, UInt_t offlineTriggerType, UInt_t ** rowIDs) const {
  ...
  // offlineTriggerType is one of the types defined in IlcVEvent
  // User should delete rowIDs if no longer needed

  if(!fHistStatistics[0]) {
    IlcWarning("Not initialized, returning 0");
    return 0;
  }
...
void IlcPhysicsSelection::SetBIFactors(const IlcESDEvent * aESD) {
  // Set factors for realtive bunch intesities
  if(!aESD) { 
    IlcFatal("ESD not given");
  }
  Int_t run = aESD->GetRunNumber();
  if (run > 105268) {
    // intensities stored in the ESDs
    const IlcESDRun* esdRun = aESD->GetESDRun();
    ...
    if (intAB > 0 && intAA > 0) {
      ...
      IlcWarning("Cannot set fBIFactorA");
    }
    
    if (intCB > 0 && intCC > 0) {
      fBIFactorC = intCB/intCC;
    } else {
      IlcWarning("Cannot set fBIFactorC");
    }
	...
	intCB > 0 && intCC > 0) {
      ...
      IlcWarning("Cannot set fBIFactorAC");
    }
...
const char * IlcPhysicsSelection::GetTriggerString(TObjString * obj) { 
...
void IlcPhysicsSelection::AddCollisionTriggerClass(const char* className){
  IlcError("This method is deprecated! Will be removed soon! Please use SetCustomOADBObjects() instead!");
  if(!fPSOADB) {
    fPSOADB = new IlcOADBPhysicsSelection("CustomPS");   
  ...
  fPSOADB->AddCollisionTriggerClass   ( IlcVEvent::kUserDefined,classNameStripped.Data(),"B",0);
...
void IlcPhysicsSelection::AddBGTriggerClass(const char* className){
  ...
  fPSOADB->AddBGTriggerClass   ( IlcVEvent::kUserDefined,classNameStripped.Data(),"AC",0);

...
ClassImp(IlcCentralitySelectionTask)


//________________________________________________________________________
IlcCentralitySelectionTask::IlcCentralitySelectionTask():
IlcAnalysisTaskSE(),
  fAnalysisInput("ESD"),
  fIsMCInput(kFALSE),
  fCurrentRun(-1),
...
{   
  // Default constructor
  IlcInfo("Centrality Selection enabled.");

  fUseScaling=kTRUE;
  fUseCleaning=kTRUE;
  fFillHistos=kFALSE;
  fBranchNames="ESD:IlcESDRun.,IlcESDHeader.,IlcESDZDC.,IlcESDFMD.,IlcESDVZERO.,IlcESDTZERO."
    ",SPDVertex.,TPCVertex.,PrimaryVertex.,IlcMultiplicity.,Tracks ";
}   

//________________________________________________________________________
IlcCentralitySelectionTask::IlcCentralitySelectionTask(const char *name):
...
//________________________________________________________________________
IlcCentralitySelectionTask& IlcCentralitySelectionTask::operator=(const IlcCentralitySelectionTask& c)
{
  // Assignment operator
  if (this!=&c) {
...
//________________________________________________________________________
IlcCentralitySelectionTask::IlcCentralitySelectionTask(const IlcCentralitySelectionTask& ana):
  IlcAnalysisTaskSE(ana),
...
//________________________________________________________________________
void IlcCentralitySelectionTask::UserCreateOutputObjects()
{  
  // Create the output containers
  if(fDebug>1) printf("AnalysisCentralitySelectionTask::UserCreateOutputObjects() \n");
  IlcLog::SetClassDebugLevel("IlcCentralitySelectionTask", IlcLog::kInfo);
  ...
  fTrackCuts = IlcESDtrackCuts::GetStandardTPCOnlyTrackCuts();
}

//________________________________________________________________________
void IlcCentralitySelectionTask::UserExec(Option_t */*option*/)
{ 
  // Execute analysis for current event:
  if(fDebug>1) printf(" **** IlcCentralitySelectionTask::UserExec() \n");
  ...
  if(fAnalysisInput.CompareTo("ESD")==0){
    ...
    if (SetupRun(esd)<0) {
      IlcError("Centrality File not available for this run");
      return;
    }
    ...
    // ***** V0 info    
    IlcESDVZERO* esdV0 = esd->GetVZEROData();
    if (!esdV0)
      {
    ...
    // ***** T0 info    
    const IlcESDTZERO* esdT0 = esd->GetESDTZERO();
    if (!esdT0)
      {
    ...
    spdCorr = IlcESDUtils::GetCorrSPD2(nClusters[1],zvtx);    

    // ***** FMD info
    IlcESDFMD *fmd = esd->GetFMDData();
    ...
    for(UShort_t det=1;det<=3;det++) {
 	...
 	for(UShort_t sec =0; sec < nsec;  sec++)  {
 	  for(UShort_t strip = 0; strip < nstr; strip++) {
 	    ...
 	    if(fmdMult == 0 || fmdMult == IlcESDFMD::kInvalidMult) continue;
    ...
    // ***** ZDC info
    IlcESDZDC *esdZDC = esd->GetESDZDC();
    zdcEnergyCal = esdZDC->IlcESDZDC::TestBit(IlcESDZDC::kEnergyCalibratedSignal);
    if (zdcEnergyCal) {      
  ...
  if (fFillHistos) {    
    ...
    if (((IlcInputEventHandler*)(IlcAnalysisManager::GetAnalysisManager()->GetInputEventHandler()))->IsEventSelected() & IlcVEvent::kMB) { // fill the QA histograms only for MB events!
...
//________________________________________________________________________
void IlcCentralitySelectionTask::Terminate(Option_t */*option*/)
{
  // Terminate analysis
}
//________________________________________________________________________
Int_t IlcCentralitySelectionTask::SetupRun(const IlcESDEvent* const esd)
{
  // Setup files for run
  ...
  TString fileName =(Form("%s/COMMON/CENTRALITY/data/centrality.root", IlcAnalysisManager::GetOADBPath()));
  IlcInfo(Form("Setup Centrality Selection for run %d with file %s\n",fCurrentRun,fileName.Data()));

  IlcOADBContainer *con = new IlcOADBContainer("OADB");
  con->InitFromFile(fileName,"Centrality");

  IlcOADBCentrality*  centOADB = 0;
  centOADB = (IlcOADBCentrality*)(con->GetObject(fCurrentRun));
  if (!centOADB) {
    IlcWarning(Form("Centrality OADB does not exist for run %d, using Default \n",fCurrentRun ));
    centOADB  = (IlcOADBCentrality*)(con->GetDefaultObject("oadbDefault"));
  }
  ...
  if (!fHtempV0M) IlcWarning(Form("Calibration for V0M does not exist in %s", path.Data()));
  if (!fHtempV0A) IlcWarning(Form("Calibration for V0A does not exist in %s", path.Data()));
  if (!fHtempV0C) IlcWarning(Form("Calibration for V0C does not exist in %s", path.Data()));
  if (!fHtempTRK) IlcWarning(Form("Calibration for TRK does not exist in %s", path.Data()));
  if (!fHtempCL1) IlcWarning(Form("Calibration for CL1 does not exist in %s", path.Data()));
  if (!fHtempCND) IlcWarning(Form("Calibration for CND does not exist in %s", path.Data()));
  if (!fHtempZEMvsZDC) IlcWarning(Form("Calibration for ZEMvsZDC does not exist in %s", path.Data()));
...
//________________________________________________________________________
Bool_t IlcCentralitySelectionTask::IsOutlierV0MSPD(Float_t spd, Float_t v0, Int_t cent) const
{
  // Clean outliers
...
//________________________________________________________________________
Bool_t IlcCentralitySelectionTask::IsOutlierV0MTPC(Int_t tracks, Float_t v0, Int_t cent) const
{
  // Clean outliers
...
//________________________________________________________________________
Bool_t IlcCentralitySelectionTask::IsOutlierV0MZDC(Float_t zdc, Float_t v0) const
{
  // Clean outliers
...
//________________________________________________________________________
Bool_t IlcCentralitySelectionTask::IsOutlierV0MZDCECal(Float_t zdc, Float_t v0) const
{
  // Clean outliers

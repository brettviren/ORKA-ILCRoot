...
/* $Id: IlcESDtrackCuts.cxx 24534 2008-03-16 22:22:11Z fca $ */
...
ClassImp(IlcESDtrackCuts)

// Cut names
const Char_t* IlcESDtrackCuts::fgkCutNames[kNCuts] = {
 "require TPC refit",
...
IlcESDtrackCuts* IlcESDtrackCuts::fgMultEstTrackCuts[IlcESDtrackCuts::kNMultEstTrackCuts] = { 0, 0, 0, 0 };
Char_t IlcESDtrackCuts::fgBeamTypeFlag = -1;

//____________________________________________________________________
IlcESDtrackCuts::IlcESDtrackCuts(const Char_t* name, const Char_t* title) : IlcAnalysisCuts(name,title),
...
IlcESDtrackCuts::IlcESDtrackCuts(const IlcESDtrackCuts &c) : IlcAnalysisCuts(c),
  fCutMinNClusterTPC(0),
  fCutMinNClusterITS(0),
  fCutMinNCrossedRowsTPC(0),
  fCutMinRatioCrossedRowsOverFindableClustersTPC(0),
  ...
  fhCutCorrelation(0)
{
  ...
  ((IlcESDtrackCuts &) c).Copy(*this);
}

IlcESDtrackCuts::~IlcESDtrackCuts()
{
  //
  // destructor
  //
...
//_____________________________________________________________________________
IlcESDtrackCuts &IlcESDtrackCuts::operator=(const IlcESDtrackCuts &c)
{
  //
  // Assignment operator
  //

  if (this != &c) ((IlcESDtrackCuts &) c).Copy(*this);
  return *this;
}

//_____________________________________________________________________________
void IlcESDtrackCuts::Copy(TObject &c) const
{
  //
  // Copy function
  //

  IlcESDtrackCuts& target = (IlcESDtrackCuts &) c;
...
//_____________________________________________________________________________
Long64_t IlcESDtrackCuts::Merge(TCollection* list) {
  // Merge a list of IlcESDtrackCuts objects with this (needed for PROOF)
  ...
  while ((obj = iter->Next())) {
    ...
    IlcESDtrackCuts* entry = dynamic_cast<IlcESDtrackCuts*>(obj);
    if (entry == 0)
      continue;
...
void IlcESDtrackCuts::SetMinNClustersTPCPtDep(TFormula *f1, Float_t ptmax) 
{
  //
...
//____________________________________________________________________
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardTPCOnlyTrackCuts()
{
  // creates an IlcESDtrackCuts object and fills it with standard (pre data-taking) values for TPC-only cuts
  
  IlcInfoClass("Creating track cuts for TPC-only.");
  
  IlcESDtrackCuts* esdTrackCuts = new IlcESDtrackCuts;
...
//____________________________________________________________________
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardITSTPCTrackCuts2009(Bool_t selPrimaries)
{
  // creates an IlcESDtrackCuts object and fills it with standard values for ITS-TPC cuts for pp 2009 data
  
  IlcInfoClass("Creating track cuts for ITS+TPC (2009 definition).");
  ...
  esdTrackCuts->SetClusterRequirementITS(IlcESDtrackCuts::kSPD,
					 IlcESDtrackCuts::kAny);
  if(selPrimaries) {
...
//____________________________________________________________________
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardITSTPCTrackCuts2011(Bool_t selPrimaries, Int_t clusterCut)
{
  // creates an IlcESDtrackCuts object and fills it with standard values for ITS-TPC cuts for pp 2011 data  
  ...
  IlcInfoClass("Creating track cuts for ITS+TPC (2011 definition).");
  ...
  if(clusterCut == 0)  esdTrackCuts->SetMinNClustersTPC(50);
    ...
    IlcWarningClass(Form("Wrong value of the clusterCut parameter (%d), using cut on Nclusters",clusterCut));
    esdTrackCuts->SetMinNClustersTPC(50);
  }
  esdTrackCuts->SetMaxChi2PerClusterTPC(4);
  esdTrackCuts->SetAcceptKinkDaughters(kFALSE);
...
//____________________________________________________________________
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardITSTPCTrackCuts2010(Bool_t selPrimaries,Int_t clusterCut)
{
  // creates an IlcESDtrackCuts object and fills it with standard values for ITS-TPC cuts for pp 2010 data  
  // if clusterCut = 1, the cut on the number of clusters is replaced by
  // a cut on the number of crossed rows and on the ration crossed
  // rows/findable clusters

  IlcInfoClass("Creating track cuts for ITS+TPC (2010 definition).");
...
//____________________________________________________________________
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardITSPureSATrackCuts2009(Bool_t selPrimaries, Bool_t useForPid)
{
  // creates an IlcESDtrackCuts object and fills it with standard values for ITS pure SA tracks
...
//____________________________________________________________________
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardITSPureSATrackCuts2010(Bool_t selPrimaries, Bool_t useForPid)
{
  // creates an IlcESDtrackCuts object and fills it with standard values for ITS pure SA tracks - pp 2010
...
//____________________________________________________________________
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardITSSATrackCuts2009(Bool_t selPrimaries, Bool_t useForPid)
{
  // creates an IlcESDtrackCuts object and fills it with standard values for ITS pure SA tracks
...
//____________________________________________________________________
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardITSSATrackCuts2010(Bool_t selPrimaries, Bool_t useForPid)
{
  // creates an IlcESDtrackCuts object and fills it with standard values for ITS pure SA tracks --pp 2010
...
//____________________________________________________________________
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardITSSATrackCutsPbPb2010(Bool_t selPrimaries, Bool_t useForPid)
{
  // creates an IlcESDtrackCuts object and fills it with standard values for ITS pure SA tracks -- PbPb 2010
  
  IlcESDtrackCuts* esdTrackCuts = GetStandardITSSATrackCuts2010(selPrimaries, useForPid);
...
IlcESDtrackCuts* IlcESDtrackCuts::GetStandardV0DaughterCuts()
{
  // creates a IlcESDtrackCuts object and fills it with standard cuts for V0 daughters
  IlcESDtrackCuts* esdTrackCuts = new IlcESDtrackCuts;
  esdTrackCuts->SetRequireTPCRefit(kTRUE);
  esdTrackCuts->SetMinNClustersTPC(70);
  esdTrackCuts->SetAcceptKinkDaughters(kFALSE);
...
//____________________________________________________________________
Int_t IlcESDtrackCuts::GetReferenceMultiplicity(const IlcESDEvent* esd, Bool_t tpcOnly)
{
  ...
  if (!tpcOnly)
  {
    IlcErrorClass("Not implemented for global tracks!");
    return -1;
  }
  
  static IlcESDtrackCuts* esdTrackCuts = 0;
  if (!esdTrackCuts)
  {
...
//____________________________________________________________________
Float_t IlcESDtrackCuts::GetSigmaToVertex(const IlcESDtrack* const esdTrack)
{
  ...
  if (bCov[0]<=0 || bCov[2]<=0) {
    IlcDebugClass(1, "Estimated b resolution lower or equal zero!");
    bCov[0]=0; bCov[2]=0;
  }
...
void IlcESDtrackCuts::EnableNeededBranches(TTree* tree)
{
...
//____________________________________________________________________
Bool_t IlcESDtrackCuts::AcceptTrack(const IlcESDtrack* esdTrack) 
{
  // 
  ...
  esdTrack->GetImpactParameters(b,bCov);
  if (bCov[0]<=0 || bCov[2]<=0) {
    IlcDebug(1, "Estimated b resolution lower or equal zero!");
    bCov[0]=0; bCov[2]=0;
  }
  ...
  if (extCov[14] < 0) 
  {
    IlcWarning(Form("GetSigma1Pt2() returns negative value for external covariance matrix element fC[14]: %f. Corrupted track information, track will not be accepted!", extCov[14]));
    return kFALSE;
  }
  ...
  // track quality cuts
  if (fCutRequireTPCRefit && (status&IlcESDtrack::kTPCrefit)==0)
    cuts[0]=kTRUE;
  if (fCutRequireTPCStandAlone && (status&IlcESDtrack::kTPCin)==0)
    cuts[1]=kTRUE;
  if (fCutRequireITSRefit && (status&IlcESDtrack::kITSrefit)==0)
    cuts[2]=kTRUE;
  ...
  if(fCutRequireITSStandAlone || fCutRequireITSpureSA){
    if ((status & IlcESDtrack::kITSin) == 0 || (status & IlcESDtrack::kTPCin)){
      // TPC tracks
      cuts[31] = kTRUE; 
    }else{
      // ITS standalone tracks
      if(fCutRequireITSStandAlone && !fCutRequireITSpureSA){
	if(status & IlcESDtrack::kITSpureSA) cuts[31] = kTRUE;
  ...
  //kTOFout
  if (fCutRequireTOFout && (status&IlcESDtrack::kTOFout)==0)
    cuts[40]=kTRUE;

  // TOF signal Dz cut
  Float_t dxTOF = esdTrack->GetTOFsignalDx();
  Float_t dzTOF = esdTrack->GetTOFsignalDz();
  if (fFlagCutTOFdistance && (esdTrack->GetStatus() & IlcESDtrack::kTOFout) == IlcESDtrack::kTOFout){ // applying the TOF distance cut only if requested, and only on tracks that reached the TOF and where associated with a TOF hit 
	  if (fgBeamTypeFlag < 0) {  // the check on the beam type was not done yet
		  const IlcESDEvent* event = esdTrack->GetESDEvent();
		  if (event){
			  TString beamTypeESD = event->GetBeamType();
			  IlcDebug(2,Form("Beam type from ESD event = %s",beamTypeESD.Data()));
			  ...
			  IlcFatal("Beam type not available, but it is needed to apply the TOF cut!");
		  }
	  }
	  ...
	  if (fgBeamTypeFlag == 1){ // we are in PbPb collisions --> apply the cut on TOF signal Dz
		  ...
		  IlcDebug(3,Form("TOF check (with fCutTOFdistance = %f) --> dx = %f, dz = %f, radius = %f", fCutTOFdistance, dxTOF, dzTOF, radiusTOF));
		  if (radiusTOF > fCutTOFdistance){
			  IlcDebug(2, Form("************* the radius is outside the range! %f > %f, the track will be skipped", radiusTOF, fCutTOFdistance));
			  cuts[41] = kTRUE;
		  }
	  }
  }
  ...
  if (!cut)
  {
    ...
    {
      const IlcESDEvent* esdEvent = esdTrack->GetESDEvent();
      
      if (!esdEvent)
	IlcFatal("fCutMaxChi2TPCConstrainedVsGlobal set but ESD event not set in IlcESDTrack. Use IlcESDTrack::SetESDEvent before calling IlcESDtrackCuts.");
      
      // get vertex
      const IlcESDVertex* vertex = 0;
  ...
  if (fHistogramsOn) {
    ...
    for (Int_t i=0; i<kNCuts; i++) {
      if (fhCutStatistics->GetXaxis()->FindBin(fgkCutNames[i]) < 1)
        IlcFatal(Form("Inconsistency! Cut %d with name %s not found", i, fgkCutNames[i]));
...
//____________________________________________________________________
Bool_t IlcESDtrackCuts::CheckITSClusterRequirement(ITSClusterRequirement req, Bool_t clusterL1, Bool_t clusterL2)
{
...
//____________________________________________________________________
IlcESDtrack* IlcESDtrackCuts::GetTPCOnlyTrack(const IlcESDEvent* esd, Int_t iTrack)
{
  // Utility function to create a TPC only track from the given esd track
  // 
  // IMPORTANT: The track has to be deleted by the user
  //
  // NB. most of the functionality to get a TPC only track from an ESD track is in IlcESDtrack, where it should be
  ...
  if(!esd->GetPrimaryVertexTPC()->GetStatus())
    return 0; // TPC Vertex is created by default in IlcESDEvent, do not use in this case

  IlcESDtrack* track = esd->GetTrack(iTrack);
  if (!track)
    return 0;

  IlcESDtrack *tpcTrack = new IlcESDtrack();
...
//____________________________________________________________________
TObjArray* IlcESDtrackCuts::GetAcceptedTracks(const IlcESDEvent* esd, Bool_t bTPC)
{
  //
  ...
  for (Int_t iTrack = 0; iTrack < esd->GetNumberOfTracks(); iTrack++) {
    if(bTPC){
      ...
      IlcESDtrack *tpcTrack = GetTPCOnlyTrack(esd, iTrack);
      if (!tpcTrack)
        continue;
...
//____________________________________________________________________
Int_t IlcESDtrackCuts::CountAcceptedTracks(const IlcESDEvent* const esd)
{
  //
...
//____________________________________________________________________
 void IlcESDtrackCuts::DefineHistograms(Int_t color) {
   // 
...
//____________________________________________________________________
Bool_t IlcESDtrackCuts::LoadHistograms(const Char_t* dir)
{
  //
...
//____________________________________________________________________
void IlcESDtrackCuts::SaveHistograms(const Char_t* dir) {
  //
  // saves the histograms in a directory (dir)
  //

  if (!fHistogramsOn) {
    IlcDebug(0, "Histograms not on - cannot save histograms!!!");
    return;
  }
...
//____________________________________________________________________
void IlcESDtrackCuts::DrawHistograms()
{
...
//--------------------------------------------------------------------------
void IlcESDtrackCuts::SetPtDepDCACuts(Double_t pt) {
  //
...
//--------------------------------------------------------------------------
Bool_t IlcESDtrackCuts::CheckPtDepDCA(TString dist,Bool_t print) const {
  //
  ...
  if(!dist.Contains("pt")) {
    if(print) IlcError("string must contain \"pt\"");
    retval= kFALSE;
  } 
  return retval;
}

 void IlcESDtrackCuts::SetMaxDCAToVertexXYPtDep(const char *dist){
 ...
 void IlcESDtrackCuts::SetMaxDCAToVertexZPtDep(const char *dist){
 ...
 void IlcESDtrackCuts::SetMinDCAToVertexXYPtDep(const char *dist){
 ...
 void IlcESDtrackCuts::SetMinDCAToVertexZPtDep(const char *dist){
...
IlcESDtrackCuts* IlcESDtrackCuts::GetMultEstTrackCuts(MultEstTrackCuts cut)
{
  ...
  if (!fgMultEstTrackCuts[kMultEstTrackCutGlobal])
  {
    ...
    fgMultEstTrackCuts[kMultEstTrackCutGlobal] = new IlcESDtrackCuts();
    ...
    fgMultEstTrackCuts[kMultEstTrackCutITSSA] = new IlcESDtrackCuts();
    fgMultEstTrackCuts[kMultEstTrackCutITSSA]->SetRequireITSRefit(kTRUE);
    
    // primary selection for tracks with SPD hits
    fgMultEstTrackCuts[kMultEstTrackCutDCAwSPD] = new IlcESDtrackCuts();
    fgMultEstTrackCuts[kMultEstTrackCutDCAwSPD]->SetClusterRequirementITS(IlcESDtrackCuts::kSPD, IlcESDtrackCuts::kAny);
    ...
    fgMultEstTrackCuts[kMultEstTrackCutDCAwoSPD] = new IlcESDtrackCuts();
    fgMultEstTrackCuts[kMultEstTrackCutDCAwoSPD]->SetClusterRequirementITS(IlcESDtrackCuts::kSPD, IlcESDtrackCuts::kNone);
...
Int_t IlcESDtrackCuts::GetReferenceMultiplicity(const IlcESDEvent* esd, MultEstTrackType trackType, Float_t etaRange) 
{
  // Get multiplicity estimate based on TPC/ITS tracks and tracklets
  // Adapted for IlcESDtrackCuts from a version developed by: Ruben Shahoyan, Anton Alkin, Arvinder Palaha
  //
  ...
  const IlcESDVertex* vertices[2];
  vertices[0] = esd->GetPrimaryVertexSPD();
  vertices[1] = esd->GetPrimaryVertexTracks();
  
  if (!vertices[0]->GetStatus())
  {
    IlcDebugClass(IlcLog::kDebug, "No SPD vertex. Not able to make a reliable multiplicity estimate.");
    return -1;
  }
  
  if (vertices[0]->IsFromVertexerZ() && vertices[0]->GetDispersion() > 0.02)
  {
    IlcDebugClass(IlcLog::kDebug, "Vertexer z dispersion > 0.02. Not able to make a reliable multiplicity estimate.");
    return -2;
  }
  ...
  {
    const IlcMultiplicity* spdmult = esd->GetMultiplicity();    // spd multiplicity object
  ...
  {
    IlcDebugClass(IlcLog::kDebug, "No track vertex. Not able to make a reliable multiplicity estimate.");
    return -3;
  }
  ...
  {
    IlcDebugClass(IlcLog::kDebug, Form("Displaced vertices %f > %f",displacement,maxDisplacement));
    return -4;
  }
  ...
  for(Int_t itracks = 0; itracks < nESDTracks; itracks++) { // flag the tracks
    IlcESDtrack* track = esd->GetTrack(itracks);

    // if track is a secondary from a V0, flag as a secondary
    if (track->IsOn(IlcESDtrack::kMultInV0)) {
      track->SetBit(kSecBit);
      continue;
    }
    
    //secondary?
    if (track->IsOn(IlcESDtrack::kMultSec)) {
      track->SetBit(kSecBit);
      continue;
    }
    ...
    //*******************************************************************************************************
    if (track->IsOn(IlcESDtrack::kITSin) && !track->IsOn(IlcESDtrack::kITSpureSA) && trackType == kTrackletsITSTPC) { // track has ITS part but is not an ITS_SA
      //*******************************************************************************************************
      // TPC+ITS
      if (track->IsOn(IlcESDtrack::kTPCin)) {  // Global track, has ITS and TPC contributions
    ...
    if (track->IsOn(IlcESDtrack::kITSin) && track->IsOn(IlcESDtrack::kITSpureSA) && trackType == kTrackletsITSSA){
...
//____________________________________________________________________
void IlcESDtrackCuts::SetRequireStandardTOFmatchCuts(){

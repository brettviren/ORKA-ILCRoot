...
using std::cout;
using std::ifstream;
ClassImp(IlcFileMerger)

ProcInfo_t procInfo;//TMP

////////////////////////////////////////////////////////////////////////

IlcFileMerger::IlcFileMerger():
  TNamed(),
  fRejectMask(0),
  fAcceptMask(0),
  fMaxFilesOpen(800),
...
IlcFileMerger::IlcFileMerger(const char* name):
  TNamed(name,name),
  fRejectMask(0),
  fAcceptMask(0),
  fMaxFilesOpen(800),
...
void IlcFileMerger::IterAlien(const char* outputDir, const char* outputFileName, const char* pattern, Bool_t dontOverwrite){
...
void IlcFileMerger::IterList(const TList* namesList, const char* outputFileName, Bool_t dontOverwrite)
{
  // merge in steps or in one go
  //
  gSystem->GetProcInfo(&procInfo);
  IlcInfo(Form(">> memory usage %ld %ld", procInfo.fMemResident, procInfo.fMemVirtual));
  //
...
void IlcFileMerger::IterTXT( const char * fileList,  const char* outputFileName, Bool_t dontOverwrite){
  
  // Merge the files indicated in the list - fileList
  // ASCII file option example: 
  // find `pwd`/ | grep IlcESDfriends_v1.root > calib.list
...
void IlcFileMerger::StoreResults(TObjArray * array, const char* outputFileName){
  //
...
void IlcFileMerger::StoreSeparateResults(TObjArray * array, const char* outputFileName){
  //
...
void IlcFileMerger::Merge(TFile* fileIn, TObjArray * array){
  //
  // Merging procedure
  //
  if (!array) return;
  ...
  for (Int_t ical=0; ical<farr->GetEntries(); ical++){
    ...
    IlcSysInfo::AddStamp(name.Data(),1,ical,counter);  
  }
  ...
  for (Int_t i=0; i<entries; i++){
    ...
    IlcSysInfo::AddStamp(currentObject->GetName(),2,i,counter);  
    delete templist;
  }
  carray->Delete();
  delete carray;
...
Bool_t IlcFileMerger::IsAccepted(TString name){
  //
  // Accept/reject logic
...
void IlcFileMerger::AddReject(const char *reject){
  //
...
void IlcFileMerger::AddAccept(const char *accept){
  //
  // add reject string to the list of entries to be rejected for merging
  //
...
int IlcFileMerger::MergeRootfile( TDirectory *target, TList *sourcelist)
{
  ...
  while(first_source) {
	...
	continue;
      ...
      IlcSysInfo::AddStamp(nameK.Data(),1,++counterK,counterF++); 
      // read object from first source file
      //current_sourcedir->cd();

      TObject *obj = key->ReadObj();
      if (!obj) {
	IlcError(Form("Failed to get the object with key %s from %s",key->GetName(),current_sourcedir->GetFile()->GetName()));
	continue;
      }
...
//___________________________________________________________________________
int IlcFileMerger::OpenNextChunks(const TList* namesList, TList* filesList, Int_t from, Int_t to)
{
  gSystem->GetProcInfo(&procInfo);
  IlcInfo(Form(">> memory usage %ld %ld", procInfo.fMemResident, procInfo.fMemVirtual));
...
//___________________________________________________________________________
int IlcFileMerger::AddFile(TList* namesList, std::string entry)
{

#ifndef ILCANALYSISTASKCFG_H
#define ILCANALYSISTASKCFG_H
...
class IlcAnalysisTaskCfg : public TNamed {
public:
enum ETaskCfgFlags {
  kLoaded      = BIT(14)
}; 
...
public:  
  IlcAnalysisTaskCfg();
  IlcAnalysisTaskCfg(const char *name);
  IlcAnalysisTaskCfg(const IlcAnalysisTaskCfg &other); 
  virtual ~IlcAnalysisTaskCfg();
  
  // Assignment
  IlcAnalysisTaskCfg& operator=(const IlcAnalysisTaskCfg &other);
  ...
  Bool_t                    IsLoaded() const {return TObject::TestBit(IlcAnalysisTaskCfg::kLoaded);}
  void                      Print(Option_t *option="") const;
  void                      SaveAs(const char *filename, Option_t *option = "") const;
  static TObjArray         *ExtractModulesFrom(const char *filename);
    
  ClassDef(IlcAnalysisTaskCfg,2)  // Class describing how to run a analysis task
};
#endif

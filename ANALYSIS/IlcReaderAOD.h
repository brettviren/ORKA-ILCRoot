#ifndef ILCREADERAOD_H
#define ILCREADERAOD_H
...
class IlcReaderAOD: public IlcReader
{
  public:
    IlcReaderAOD(const Char_t* aodfilename = "AOD.root");
    virtual ~IlcReaderAOD();
    ...
    static Int_t WriteAOD(IlcReader* reader, const char* outfilename = "IlcAOD.root", //reads tracks from runs and writes them to file
                          const char* pclassname = "IlcAODParticle", Bool_t multcheck = kFALSE);
  ...
  private:
    ...
    TTree*        fTree;//!tree
    TFile*        fFile;//!file
    IlcAOD*       fSimBuffer;//!buffer array that tree is read to
    IlcAOD*       fRecBuffer;//!
    
    IlcReaderAOD(const IlcReaderAOD&); // Not implemented
    IlcReaderAOD& operator=(const IlcReaderAOD&); // Not implemented
    
    ClassDef(IlcReaderAOD,1)
};

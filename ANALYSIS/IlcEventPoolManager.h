#ifndef IlcEventPoolManager_h
#define IlcEventPoolManager_h
...
// $ILC_ROOT/PWG4/Correlations/IlcAnalysisTaskPhiCorrelations.cxx
//
// Authors: A. Adare and C. Loizides

using std::deque;

class IlcEventPool : public TObject
{
 public:
 IlcEventPool(Int_t d) 
   : fEvents(0),
    fNTracksInEvent(0),
    fEventIndex(0),
    fMixDepth(d), 
 ...
 IlcEventPool(Int_t d, Double_t multMin, Double_t multMax, 
...
protected:
  ...
  ClassDef(IlcEventPool,1) // Event pool class
};

class IlcEventPoolManager : public TObject
{
public:
  IlcEventPoolManager() 
    : fDebug(0),
    ...
    fEvPool(0),
    fTargetTrackDepth(0) {;}
  IlcEventPoolManager(Int_t maxEvts, Int_t minNTracks,
  ...
  IlcEventPool *GetEventPool(Int_t iMult, Int_t iZvtx) const;
  IlcEventPool *GetEventPool(Int_t    centVal, Double_t zvtxVal) const;
  IlcEventPool *GetEventPool(Double_t centVal, Double_t zvtxVal) const;
...
protected:
  ...
  std::vector<std::vector<IlcEventPool*> > fEvPool; // pool in bins of [fMultBin][fZvtxBin]
  Int_t      fTargetTrackDepth;                         // Required track size, same for all pools.

  ClassDef(IlcEventPoolManager,1)
};
#endif

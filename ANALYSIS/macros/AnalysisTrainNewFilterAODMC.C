...
// CAF mode (requires root v5-23-02 + ilcroot v4-16-Rev08)
//    root[2] AnalysisTrainNew("proof")
// Local mode requires IlcESDs.root or IlcAOD.root in ./data directory
...
//==============================================================================
TString     proof_cluster      = "ilc-caf.cern.ch";
...
// in the deployed IlcRoot versions. Par file search path: local dir, if not there $ILC_ROOT.
...
TString     ilcroot_version    = "v4-21-13-AN";  // *CHANGE ME IF MORE RECENT IN GRID*                                          
// Change production base directory here (test mode)
TString     alien_datadir      = "/ilc/sim/LHC11a3";
               // Work directory in GRID (DON'T CHANGE)
TString     grid_workdir       = "/ilc/cern.ch/user/a/ilcdaq/AOD/AOD$2";
...
//TString     mergeExclude       = "IlcAOD.root IlcAOD.VertexingHF.root IlcAOD.Jets.root deltaAODPartCorr.root IlcAOD.Muons.root IlcAOD.Dimuons.root IlcAOD.Dielectron.root";
...
//==============================================================================
TString     configPWG2femto    = "$ILC_ROOT/PWG2/FEMTOSCOPY/macros/Train/Train3/ConfigFemtoAnalysis.C";
TString     configPWG3d2h      = "$ILC_ROOT/PWG3/vertexingHF/ConfigVertexingHF_highmult.C";
//TString     configPWG3d2h      = "$ILC_ROOT/PWG3/vertexingHF/ConfigVertexingHF.C";
// Temporaries.
TString anaPars = "";
TString anaLibs = "";
// Function signatures
class IlcAnalysisAlien;
...
//   }   
   ...
   if (iAODanalysis) {
   // AOD input handler
      IlcAODInputHandler *aodH = new IlcAODInputHandler();
      if (iPWG3d2h) aodH->AddFriend("IlcAOD.VertexingHF.root");
   ...
   if (iAODhandler) {
      ...
      if (iAODanalysis) {
         ...
         if (iJETAN)         aodHandler->SetOutputFileName("IlcAOD.Jets.root");
//         if (iPWG3vertexing) aodHandler->SetOutputFileName("IlcAOD.VertexingHF.root");
      } 
   }
   // Debugging if needed
   ...
   // mapped from IlcRoot and pecified in the jdl input list.
   ...
   //                                                                                                                                      
   if (usePLUGIN) {                                                                                                                        
      IlcAnalysisGrid *alienHandler = CreateAlienHandler(plugin_mode);                                                                     
      IlcAnalysisManager::GetAnalysisManager()->SetGridHandler(alienHandler);                                                              
   }                                                                                                                                       
...
// Add all analysis task wagons to the train                                                                                               
  ...
  //
   if (useTender) {
      ...
      IlcAnalysisTaskSE *tender = AddTaskTender(kTRUE);
//      tender->SelectCollisionCandidates();
   ...
   // Physics selection task
      ...
      IlcPhysicsSelectionTask *physSel = AddTaskPhysicsSelection(useMC);
   ...
   if (iESDfilter && !iAODanalysis) {
      ...
      if (iMUONcopyAOD) {
         ...
         IlcAnalysisTaskESDfilter *taskesdfilter = AddTaskESDFilter(useKFILTER, kTRUE, kTRUE, usePhysicsSelection, kFALSE, IlcESDpid::kTOF_T0, kTRUE);
      } else {
         IlcAnalysisTaskESDfilter *taskesdfilter = AddTaskESDFilter(useKFILTER, kFALSE, kFALSE, usePhysicsSelection,kFALSE, IlcESDpid::kTOF_T0, kTRUE);
      }   
   }   
   ...
   if (useAODTAGS) {
      ...
      mgr->AddTask(tagTask);
      IlcAnalysisDataContainer *coutTags = mgr->CreateContainer("cTag",  TTree::Class(), 
                                           IlcAnalysisManager::kOutputContainer, "AOD.tag.root");
      mgr->ConnectInput (tagTask, 0, mgr->GetCommonInputContainer());
      mgr->ConnectOutput(tagTask, 1, coutTags);
   }   
    
// ********** PWG2 wagons ******************************************************
   IlcAnalysisManager::SetCommonFileName("PWG2histograms.root");

   // Proton analysis
   if (iPWG2spectra) {
      // protons
      if (iPWG2protons) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/SPECTRA/macros/AddTaskProtons.C");
        IlcAnalysisTaskProtons *taskprotons = AddTaskProtons();
        if (!taskprotons) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskProtons cannot run for this train conditions - EXCLUDED");
      }  
      // cascades
      if (iPWG2checkcascade) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/SPECTRA/macros/AddTaskCheckCascade.C");
        IlcAnalysisTaskCheckCascade *taskcheckcascade = AddTaskCheckCascade(iCollision,runOnData);
        if (!taskcheckcascade) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskCheckCascade cannot run for this train conditions - EXCLUDED");
      }  
      // v0's
      if (iPWG2checkv0) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/SPECTRA/macros/AddTaskCheckV0.C");
        IlcAnalysisTaskCheckV0 *taskcheckV0 = AddTaskCheckV0();
        if (!taskcheckV0) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskCheckV0 cannot run for this train conditions - EXCLUDED");
      }  
      // strangeness
      if (iPWG2strange) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/SPECTRA/macros/AddTaskStrange.C");
        IlcAnalysisTaskStrange *taskstrange = AddTaskStrange();
        if (!taskstrange) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskStrange cannot run for this train conditions - EXCLUDED");
      }  
      // performance cascades
      if (iPWG2perfcascade) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/SPECTRA/macros/AddTaskCheckPerformanceCascade.C");
        IlcAnalysisTaskCheckPerformanceCascade *taskperfcascade = AddTaskCheckPerformanceCascade();
        if (!taskperfcascade) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskCheckPerformanceCascade cannot run for this train conditions - EXCLUDED");
      }  
       // central
      if (iPWG2central) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/SPECTRA/macros/AddTaskCentral.C");
        IlcAnalysisTaskCentral *taskcentral = AddTaskCentral();
        if (!taskcentral) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskCentral cannot run for this train conditions - EXCLUDED");
      }  
  }   
   
   // Femtoscopy analysis modules
   if (iPWG2femto) {
      gROOT->LoadMacro("$ILC_ROOT/PWG2/FEMTOSCOPY/macros/AddTaskFemto.C");
      IlcAnalysisTaskFemto *taskfemto = AddTaskFemto(configPWG2femto);
      if (!taskfemto) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskFemto cannot run for this train conditions - EXCLUDED");
   }   

   // Kink analysis
   if (iPWG2kink) {
      if (iPWG2kinkESDMC) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKink.C");
        IlcAnalysisKinkESDMC *taskkink1 = AddTaskKink();
        if (!taskkink1) ::Warning("AnalysisTrainNew", "IlcAnalysisKinkESDMC cannot run for this train conditions - EXCLUDED");
      }   
      if (iPWG2kinkLSKstar) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKinkResLikeSignKstar.C");
        IlcResonanceKinkLikeSign *taskkink2 = AddTaskKinkResLikeSignKstar();
        if (!taskkink2) ::Warning("AnalysisTrainNew", "IlcResonanceKinkLikeSign cannot run for this train conditions - EXCLUDED");
      }  
      if (iPWG2kinkLSL1520) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKinkResLikeSignL1520.C");
        IlcResonanceKinkLikeSign *taskkink3 = AddTaskKinkResLikeSignL1520();
        if (!taskkink3) ::Warning("AnalysisTrainNew", "IlcResonanceKinkLikeSign cannot run for this train conditions - EXCLUDED");
      }  
      if (iPWG2kinkLSPhi) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKinkResLikeSignPhi.C");
        IlcResonanceKinkLikeSign *taskkink4 = AddTaskKinkResLikeSignPhi();
        if (!taskkink4) ::Warning("AnalysisTrainNew", "IlcResonanceKinkLikeSign cannot run for this train conditions - EXCLUDED");
      }  
      if (iPWG2kinkKstarESD) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKinkResonanceKstarESD.C");
        IlcAnalysisTaskKinkResonance *taskkink5 = AddTaskKinkResonanceKstarESD();
        if (!taskkink5) ::Warning("AnalysisTrainNew", "IlcAnalysisKinkResonanceKstarESD cannot run for this train conditions - EXCLUDED");
      }   
      if (iPWG2kinkKstarMC) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKinkResonanceKstarMC.C");
        IlcAnalysisTaskKinkResonance *taskkink6 = AddTaskKinkResonanceKstarMC();
        if (!taskkink6) ::Warning("AnalysisTrainNew", "IlcAnalysisKinkResonanceKstarMC cannot run for this train conditions - EXCLUDED");
      }   
      if (iPWG2kinkL1520ESD) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKinkResonanceL1520ESD.C");
        IlcAnalysisTaskKinkResonance *taskkink7 = AddTaskKinkResonanceL1520ESD();
        if (!taskkink7) ::Warning("AnalysisTrainNew", "IlcAnalysisKinkResonanceL1520ESD cannot run for this train conditions - EXCLUDED");
      }   
      if (iPWG2kinkL1520MC) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKinkResonanceL1520MC.C");
        IlcAnalysisTaskKinkResonance *taskkink8 = AddTaskKinkResonanceL1520MC();
        if (!taskkink8) ::Warning("AnalysisTrainNew", "IlcAnalysisKinkResonanceL1520MC cannot run for this train conditions - EXCLUDED");
      }   
      if (iPWG2kinkPhiESD) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKinkResonancePhiESD.C");
        IlcAnalysisTaskKinkResonance *taskkink9 = AddTaskKinkResonancePhiESD();
        if (!taskkink9) ::Warning("AnalysisTrainNew", "IlcAnalysisKinkResonancePhiESD cannot run for this train conditions - EXCLUDED");
      }   
      if (iPWG2kinkPhiMC) {
        gROOT->LoadMacro("$ILC_ROOT/PWG2/KINK/macros/AddTaskKinkResonancePhiMC.C");
        IlcAnalysisTaskKinkResonance *taskkink10 = AddTaskKinkResonancePhiMC();
        if (!taskkink10) ::Warning("AnalysisTrainNew", "IlcAnalysisKinkResonancePhiMC cannot run for this train conditions - EXCLUDED");
      }   
   }   

   // Event characterization
   if (iPWG2evchar) {
      gROOT->LoadMacro("$ILC_ROOT/PWG2/EVCHAR/macros/AddTaskSPDdNdEta.C");
      IlcAnalysisTaskSPDdNdEta *taskspddndeta = AddTaskSPDdNdEta();
      if (!taskspddndeta) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskSPDdNdEta cannot run for this train conditions - EXCLUDED");
      taskspddndeta->SetReadMC(useMC);
   }   

   // Unicor
   if (iPWG2unicor) {
      gROOT->LoadMacro("$ILC_ROOT/PWG2/UNICOR/AddTaskUnicor.C");
      IlcAnalysisTaskUnicor *taskunicor = AddTaskUnicor();
      if (!taskunicor) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskUnicor cannot run for this train conditions - EXCLUDED");
   }   

   // FMD
   IlcAnalysisManager::SetCommonFileName("forward.root");
   if (iPWG2forward) {
      gROOT->LoadMacro("$ILC_ROOT/PWG2/FORWARD/analysis/AddTaskFMD.C");
      IlcFMDAnalysisTaskSE *taskfmd = AddTaskFMD();
      if (!taskfmd) ::Warning("AnalysisTrainNew", "IlcFMDAnalysisTaskSE cannot run for this train conditions - EXCLUDED");
   }   
   IlcAnalysisManager::SetCommonFileName("PWG2histograms.root");

   // Flow analysis
   if (iPWG2flow) {
      gROOT->LoadMacro("$ILC_ROOT/PWG2/FLOW/macros/AddTaskFlow.C");
      Bool_t SP       = kTRUE;
      ...
      IlcAnalysisTaskFlowEvent* taskFE = AddTaskFlow(type,METHODS,QA,WEIGHTS);
      if (!taskFE) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskFlowEvent cannot run for this train conditions - EXCLUDED");
   }   

   // PWG2 resonances
   IlcAnalysisManager::SetCommonFileName("resonances.root");
   if (iPWG2res) {
      gROOT->LoadMacro("$ILC_ROOT/PWG2/RESONANCES/macros/train/AddAnalysisTaskRsnNew.C");
      gROOT->LoadMacro("$ILC_ROOT/PWG2/RESONANCES/macros/train/AddAnalysisTaskRsnEffNew.C");
      TString path = gROOT->GetMacroPath();
      path += ":$ILC_ROOT/PWG2/RESONANCES/macros/train";
...
// ********** PWG3 wagons ******************************************************
   IlcAnalysisManager::SetCommonFileName("PWG3histograms.root");
           
   // PWG3 vertexing
   if (iPWG3vertexing) {
      gROOT->LoadMacro("$ILC_ROOT/PWG3/vertexingHF/macros/AddTaskVertexingHF.C");
   ...
   // PWG3 electrons
   if (iPWG3hfe) {
      gROOT->LoadMacro("$ILC_ROOT/PWG3/hfe/AddTaskHFE.C");
      IlcAnalysisTaskHFE *taskHFE = AddTaskHFE();
      if (!taskHFE) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskHFE cannot run for this train conditions - EXCLUDED");
   }   

   // PWG3 JPSI filtering
   if (iPWG3JPSIfilter) {
      gROOT->LoadMacro("$ILC_ROOT/PWG3/dielectron/macros/AddTaskJPSIFilter.C");
      IlcAnalysisTask *taskJPSIfilter = AddTaskJPSIFilter();
   ...
   // PWG3 JPSI
   if (iPWG3JPSI) {
      gROOT->LoadMacro("$ILC_ROOT/PWG3/dielectron/macros/AddTaskJPSI.C");
      IlcAnalysisTask *taskJPSI = AddTaskJPSI();
      if (!taskJPSI) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskMultiDielectron cannot run for this train conditions - EXCLUDED");
   }   

   // PWG3 D2h
   if (iPWG3d2h) {
      gROOT->LoadMacro("$ILC_ROOT/PWG3/vertexingHF/AddD2HTrain.C");
   ...
   // PWG3 muon
   if (iPWG3MuonTrain) {
      gROOT->LoadMacro("$ILC_ROOT/PWG3/muon/AddPWG3MuonTrain.C");
...
// ********** PWG4 wagons ******************************************************
   IlcAnalysisManager::SetCommonFileName("PWG4histograms.root");

    // Jet analysis
   if (iJETAN) {
      gROOT->LoadMacro("$ILC_ROOT/PWG4/macros/AddTaskJets.C");
      if (!iAODanalysis) {
         // Default jet reconstructor running on ESD's
         IlcAnalysisTaskJets * taskjets = AddTaskJets();
         if (!taskjets) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskJets cannot run for this train conditions - EXCLUDED");
         if (iJETANdelta) {
            AddTaskJetsDelta("IlcAOD.Jets.root");
            mgr->RegisterExtraFile("IlcAOD.Jets.root");
         }   
      } else {
         // AOD-based analysis. Add all reconstructors to write into delta AOD's
         if (iJETANdelta) {
            Int_t ntasksjets = AddTaskJetsDelta("IlcAOD.Jets.root",0,kFALSE);
   ...
   if (iPWG4partcorr) {
      ...
      gROOT->LoadMacro("$ILC_ROOT/PWG4/macros/AddTaskPartCorr.C");
      IlcAnalysisTaskParticleCorrelation *taskpartcorrPHOS = AddTaskPartCorr("AOD", "PHOS", kFALSE, isSimulation);
      if (!taskpartcorrPHOS) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskParticleCorrelation PHOS cannot run for this train conditions - EXCLUDED");
      IlcAnalysisTaskParticleCorrelation *taskpartcorrEMCAL = AddTaskPartCorr("AOD", "EMCAL", kFALSE, isSimulation);
      if (!taskpartcorrEMCAL) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskParticleCorrelation EMCAL cannot run for this train conditions - EXCLUDED");
      mgr->RegisterExtraFile("deltaAODPartCorr.root");
   }   

   // PWG4 gamma conversion analysis
   if (iPWG4gammaconv) {
      gROOT->LoadMacro("$ILC_ROOT/PWG4/macros/AddTaskGammaConversion.C");
      TString cdir = gSystem->WorkingDirectory();
      gSystem->ChangeDirectory(gSystem->ExpandPathName("$ILC_ROOT/PWG4/macros/"));
...
//      gcArguments.Append(Form(" -set-cut-selection %s",kGCAnalysisCutSelectionId.Data()));
      ...
      IlcAnalysisTaskGammaConversion * taskGammaConversion = AddTaskGammaConversion(gcArguments,mgr->GetCommonInputContainer());
      gSystem->ChangeDirectory(cdir);
      taskGammaConversion->SelectCollisionCandidates();
      if (!taskGammaConversion) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskGammaConversion cannot run for these train conditions - EXCLUDED");
   }   

   // PWG4 omega to 3 pions analysis
   if (iPWG4omega3pi) {
      gROOT->LoadMacro("$ILC_ROOT/PWG4/macros/AddTaskomega3pi.C");
      IlcAnalysisTaskOmegaPi0PiPi *taskomega3pi = AddTaskomega3pi();
      if (!taskomega3pi) ::Warning("AnalysisTrainNew", "IlcAnalysisTaskomega3pi cannot run for these train conditions - EXCLUDED");
   }   
}
   ...
   switch (imode) {
      ...
      case 2:
         if (useCPAR) {
            ...
            success &= LoadLibrary("ANALYSISilc", mode, kTRUE);
            ...
            success &= LoadLibrary("libANALYSISilc.so", mode);
      ...
      case 1:
         ...
         if (ires < 0) {
            ...
            success &= LoadLibrary("ANALYSISilc", mode);
...
//      TFile::Cp(gSystem->ExpandPathName("$(ILC_ROOT)/PWG2/FEMTOSCOPY/macros/ConfigFemtoAnalysis.C"), Form("%s/ConfigFemtoAnalysis.C", train_name.Data()));
...
// Load a module library in a given mode. Reports success.
   ...
   switch (imode) {
      ...
      case 1:
         ...
         if (result<0) {
            result = gProof->UploadPackage(gSystem->ExpandPathName(Form("$ILC_ROOT/%s.par", module)));
            if (result<0) {
               ::Error("AnalysisTrainNew.C::LoadLibrary", "Could not find module %s.par in current directory nor in $ILC_ROOT", module);
               return kFALSE;
            }
         }   
      ...
      case 0:
         if (iAODanalysis) {
            if (!local_xmldataset.Length()) {
               ...
               if (gSystem->AccessPathName("data/IlcAOD.root")) 
                  ::Error("AnalysisTrainNew.C::CreateChain", "File: IlcAOD.root not in ./data dir");
               else {
                  if (!saveTrain) chain->Add("data/IlcAOD.root");
                  else            chain->Add("../data/IlcAOD.root");
               }   
            } else {
               // Interactive AOD
            ...
            if (!local_xmldataset.Length()) {
               ...
               if (gSystem->AccessPathName("data/IlcESDs.root")) 
                  ::Error("AnalysisTrainNew.C::CreateChain", "File: IlcESDs.root not in ./data dir");
               else {
                  if (!saveTrain) chain->Add("data/IlcESDs.root");
                  else            chain->Add("../data/IlcESDs.root");
               }   
            } else {
               // Interactive ESD
...
Int_t SetupPar(char* pararchivename)
{
   ...
   if (gSystem->AccessPathName(Form("%s.par", pararchivename))) {
      if (!gSystem->AccessPathName(Form("%s/%s.par", gSystem->Getenv("ILC_ROOT"),pararchivename))) {
         ::Info("AnalysisTrainNew.C::SetupPar", "Getting %s.par from $ILC_ROOT", pararchivename);
         TFile::Cp(gSystem->ExpandPathName(Form("$ILC_ROOT/%s.par", pararchivename)), 
...
//______________________________________________________________________________
IlcAnalysisAlien* CreateAlienHandler(const char *plugin_mode)
{
// Check if user has a valid token, otherwise make one. This has limitations.
// One can always follow the standard procedure of calling alien-token-init then
   IlcAnalysisAlien *plugin = new IlcAnalysisAlien();
...
// Set versions of used packages
   ...
   plugin->SetIlcROOTVersion(ilcroot_version);
...
// (can be like: file.root or file.root@ILC::Niham::File)
...
//   plugin->SetOutputArchive("log_archive.zip:stdout,stderr@ILC::NIHAM::File root_archive.zip:IlcAOD.root,AOD.tag.root@ILC::NIHAM::File");
   ...
   IlcAnalysisDataContainer *output;
   while ((output=(IlcAnalysisDataContainer*)next())) {
...
//      plugin->SetFriendChainName("IlcAOD.VertexingHF.root");
...
// UseOverwrite mode
   ...
   plugin->SetExecutableCommand("ilcroot -b -q");
...
// train_[trainName]_ddMonthyyyy_time.C
   ...
   out << "   ilcroot_version = " << "\"" << ilcroot_version.Data() << "\";" << endl;

...
UInt_t kTriggerInt = IlcVEvent::kAnyINT;
UInt_t kTriggerMuonAll = IlcVEvent::kMUL7 | IlcVEvent::kMUSH7 | IlcVEvent::kMUU7 | IlcVEvent::kMUS7
                       | IlcVEvent::kMUSPB | IlcVEvent::kMUSHPB | IlcVEvent::kMuonLikePB | IlcVEvent::kMuonUnlikePB;
UInt_t kTriggerMuonBarell = IlcVEvent::kMUU7;
UInt_t kTriggerEMC   = IlcVEvent::kEMC7 | IlcVEvent::kEMCEJE | IlcVEvent::kEMCEGA;
UInt_t kTriggerHM   = IlcVEvent::kHighMult;
...
void LoadLibraries()
{
  gSystem->SetIncludePath("-I. -I$ROOTSYS/include -I$ILC_ROOT/include -I$ILC_ROOT -I$ILC_ROOT/ITS -I$ILC_ROOT/TRD -I$ILC_ROOT/PWGPP -I$ILC_ROOT/PWGPP/TRD");
  gSystem->Load("libANALYSIS");
  gSystem->Load("libANALYSISilc");
  gSystem->Load("libCORRFW");
  gSystem->Load("libTENDER");
  gSystem->Load("libPWGPP.so");
  gSystem->Load("libIlcHLTTrigger.so");
...
void AddAnalysisTasks()
{
  ...
  if (doEventStat) {
      ...
      IlcPhysicsSelectionTask* physSelTask = AddTaskPhysicsSelection(kTRUE /*MC*/);
  }
  ...
  if (doVertex) {
    ...
    IlcAnalysisTaskVertexESD* taskvertexesd =  AddTaskVertexESD(kTRUE, kTriggerMask);
...
//  taskv0qa->SelectCollisionCandidates();
  ...
  if (doHLT) {
    gROOT->LoadMacro("$ILC_ROOT/PWGPP/TPC/macros/AddTaskPerformanceTPCdEdxQA.C");
    IlcPerformanceTask *hltQA = AddTaskPerformanceTPCdEdxQA(kTRUE, kTRUE, kFALSE,0,kTRUE);
  ...
  // offline trigger in AddTask
     ...
     IlcAnalysisTaskITSsaTracks *itssaTracks = AddTaskITSsaTracks(kTRUE,kFALSE);
  ...
  if(doCALO) {
      ...
      IlcAnalysisTaskCaloTrackCorrelation *taskCaloQA = AddTaskCalorimeterQA("ESD", 2011, kFALSE, kTRUE);
  ...
  if (doPIDResponse) {
    ...
    IlcAnalysisTaskPIDResponse *PIDResponse = AddTaskPIDResponse(kTRUE);
  ...
  if (doHMPID) {
    ...
    IlcAnalysisTaskSE* taskhmpidqa= AddTaskHmpidQA(kTRUE);
      // offline mask set in AddTask to kMB
  ...
  if (doFMD) {
    ...
    IlcAnalysisTaskSE *forwardQA = (IlcAnalysisTaskSE *)AddTaskForwardQA(kTRUE, (Bool_t)doCentrality);

...
//==============================================================================
//   IlcAnalysisAlien - AliEn utility class. Provides interface for creating
// a personalized JDL, finding and creating a dataset.
//==============================================================================

#include "IlcAnalysisAlien.h"
...
IlcAnalysisAlien::IlcAnalysisAlien()
                 :IlcAnalysisGrid(),
                  fGridJDL(NULL),
                  fMergingJDL(NULL),
                  fPrice(0),
...
//______________________________________________________________________________
IlcAnalysisAlien::IlcAnalysisAlien(const char *name)
                 :IlcAnalysisGrid(name),
                  fGridJDL(NULL),
                  fMergingJDL(NULL),
                  fPrice(0),
                  fTTL(0),
...
//______________________________________________________________________________
IlcAnalysisAlien::IlcAnalysisAlien(const IlcAnalysisAlien& other)
                 :IlcAnalysisGrid(other),
                  fGridJDL(NULL),
                  fMergingJDL(NULL),
                  fPrice(other.fPrice),
                  fTTL(other.fTTL),
                  ...
                  fIlcROOTVersion(other.fIlcROOTVersion),
                  ...
                  fIlcRootMode(other.fIlcRootMode),
...
// Copy ctor.
   ...
   if (other.fModules) {
      ...
      IlcAnalysisTaskCfg *mod, *crt;
      while ((crt=(IlcAnalysisTaskCfg*)next())) {
         mod = new IlcAnalysisTaskCfg(*crt);
         fModules->Add(mod);
      }
   }   
}
...
//______________________________________________________________________________
IlcAnalysisAlien &IlcAnalysisAlien::operator=(const IlcAnalysisAlien& other)
{
// Assignment.
   if (this != &other) {
      IlcAnalysisGrid::operator=(other);
      fGridJDL = (TGridJDL*)gROOT->ProcessLine("new TAlienJDL()");
      fMergingJDL = (TGridJDL*)gROOT->ProcessLine("new TAlienJDL()");
      fPrice                   = other.fPrice;
      fTTL                     = other.fTTL;
...
//______________________________________________________________________________
void IlcAnalysisAlien::AddAdditionalLibrary(const char *name)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::AddModule(IlcAnalysisTaskCfg *module)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::AddModules(TObjArray *list)
{
// Adding a list of modules. Checks if already existing. Becomes owned by this.
   TIter next(list);
   IlcAnalysisTaskCfg *module;
   while ((module = (IlcAnalysisTaskCfg*)next())) AddModule(module);
}   

//______________________________________________________________________________
Bool_t IlcAnalysisAlien::CheckDependencies()
{
// Check if all dependencies are satisfied. Reorder modules if needed.
   ...
   IlcAnalysisTaskCfg *mod = 0;
   IlcAnalysisTaskCfg *dep = 0;
   TString depname;
   Int_t i, j, k;
   for (i=0; i<nmodules; i++) {
      mod = (IlcAnalysisTaskCfg*) fModules->At(i);
...
//______________________________________________________________________________
IlcAnalysisManager *IlcAnalysisAlien::CreateAnalysisManager(const char *name, const char *filename)
{
// Create the analysis manager and optionally execute the macro in filename.
   ...
   if (mgr) return mgr;
   mgr = new IlcAnalysisManager(name);
   mgr->SetGridHandler((IlcAnalysisGrid*)this);
   if (strlen(filename)) {
...
//______________________________________________________________________________
Int_t IlcAnalysisAlien::GetNmodules() const
{
...
//______________________________________________________________________________
IlcAnalysisTaskCfg *IlcAnalysisAlien::GetModule(const char *name)
{
// Get a module by name.
   if (!fModules) return 0;
   return (IlcAnalysisTaskCfg*)fModules->FindObject(name);
}
   
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::LoadModule(IlcAnalysisTaskCfg *mod)
{
// Load a given module.
   ...
   for (Int_t j=0; j<ndeps; j++) {
      depname = mod->GetDependency(j);
      IlcAnalysisTaskCfg *dep = GetModule(depname);
      if (!dep) {
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::GenerateTrain(const char *name)
{
...
//   SetExecutableCommand("ilcroot -b -q ");
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::GenerateTest(const char *name, const char *modname)
{
// Generate test macros for a single module or for the full train.
   fAdditionalLibs = "";
   if (strlen(modname)) {
      if (!CheckDependencies()) return kFALSE;
      IlcAnalysisTaskCfg *mod = GetModule(modname);
      if (!mod) {
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::LoadModules()
{
// Load all modules by executing the AddTask macros. Checks first the dependencies.
   ...
   IlcAnalysisTaskCfg *mod;
   for (Int_t imod=0; imod<nmodules; imod++) {
      mod = (IlcAnalysisTaskCfg*)fModules->At(imod);
      if (!LoadModule(mod)) return kFALSE;
   }
   return kTRUE;
}      
...
//______________________________________________________________________________
void IlcAnalysisAlien::SetRunPrefix(const char *prefix)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::AddIncludePath(const char *path)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::AddRunNumber(Int_t run)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::AddRunList(const char* runList)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::AddRunNumber(const char* run)
{
// Add a run number to the list of runs to be processed.
   TString runs = run;
   TObjString *os;
...
//______________________________________________________________________________
void IlcAnalysisAlien::AddDataFile(const char *lfn)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::AddExternalPackage(const char *package)
{
// Adds external packages w.r.t to the default ones (root,ilcroot and gapi)
   if (fExternalPackages) fExternalPackages += " ";
   fExternalPackages += package;
}   
      
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::Connect()
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::CheckFileCopy(const char *alienpath)
{
// Check if file copying is possible.
   ...
   if (!TFile::Cp(stest.Data(), Form("alien://%s/%s",alienpath, stest.Data()))) {
      Error("CheckFileCopy", "Cannot copy files to Alien destination: <%s> This may be temporary, or: \
           ...
           \n# 2. Check the storage availability at: http://ilcmonitor.cern.ch/stats?page=SE/table \
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::CheckInputData()
{
// Check validity of input data. If necessary, create xml files.
   ...
   if (!fInputFiles && !fRunNumbers.Length() && !fRunRange[0]) {
      ...
      if (fDataPattern.Contains("tag") && TestBit(IlcAnalysisGrid::kTest))
         TObject::SetBit(IlcAnalysisGrid::kUseTags, kTRUE); // ADDED (fix problem in determining the tag usage in test mode) 
      return kTRUE;
   }
   ...
   if (!TestBit(IlcAnalysisGrid::kTest)) CdWork();
   TString file;
   ...
   if (fInputFiles) {
      ...
      while ((objstr=(TObjString*)next())) {
         ...
         if (!checked) {
            ...
            isXml = isxml;
            useTags = usetags;
            TObject::SetBit(IlcAnalysisGrid::kUseTags, useTags);
         } else {
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::CopyLocalDataset(const char *griddir, const char *pattern, Int_t nfiles, const char *output, const char *archivefile, const char *outputdir)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::CreateDataset(const char *pattern)
{
// Create dataset for the grid data directory + run number.
   const Int_t gMaxEntries = 15000;
   if (fProductionMode || TestBit(IlcAnalysisGrid::kOffline)) return kTRUE;
   if (!Connect()) {
   ...
   if (TestBit(IlcAnalysisGrid::kTest)) options += Form("-l %d ", fNtestFiles);
...
//      CdWork();
      if (TestBit(IlcAnalysisGrid::kTest)) file = "wn.xml";
      else file = Form("%s.xml", gSystem->BaseName(path));
      while (1) {
         ncount = 0;
         stage++;
         if (gSystem->AccessPathName(file) || TestBit(IlcAnalysisGrid::kTest) || fOverwriteMode) {
            command = "find ";
      ...
      if (!TestBit(IlcAnalysisGrid::kTest) && (!fileExists || fOverwriteMode)) {
...
//         CdWork();
         ...
         if (TestBit(IlcAnalysisGrid::kTest)) break;
...
//         CdWork();
         ...
         if (FileExists(file) && fNrunsPerMaster<2 && !TestBit(IlcAnalysisGrid::kTest)) {         
            if (fOverwriteMode) gGrid->Rm(file);
            else {
               Info("CreateDataset", "\n#####   Dataset %s exist. Skipping creation...", file.Data());
               continue;
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::CreateJDL()
{
...
// specified by fJDLName.
   ...
   TObjArray *arr = 0;
   Bool_t copy = kTRUE;
   if (fProductionMode || TestBit(IlcAnalysisGrid::kOffline) || TestBit(IlcAnalysisGrid::kTest)) copy = kFALSE;
   Bool_t generate = kTRUE;
   if (TestBit(IlcAnalysisGrid::kTest) || TestBit(IlcAnalysisGrid::kSubmit)) generate = kFALSE;
   if (!Connect()) {
   ...
   TString workdir;
   if (!fProductionMode && !fGridWorkingDir.BeginsWith("/ilc")) workdir = gGrid->GetHomeDirectory();
   if (!fProductionMode &&  !TestBit(IlcAnalysisGrid::kTest)) CdWork();
   workdir += fGridWorkingDir;
   if (generate) {
      TObjString *os;
      if (!fInputFiles) {
      ...
      if (!fIlcROOTVersion.IsNull()) {
         fGridJDL->AddToPackages("IlcRoot", fIlcROOTVersion,"VO_ILC", "List of requested packages");
         fMergingJDL->AddToPackages("IlcRoot", fIlcROOTVersion, "VO_ILC", "List of requested packages");
      }   
      ...
      if (fOutputArchive.Length()) {
         ...
         if (TestBit(IlcAnalysisGrid::kDefaultOutputs)) {
   ...
   if (copy) {
      ...
      if (TestBit(IlcAnalysisGrid::kSubmit)) {
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::WriteJDL(Bool_t copy)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::FileExists(const char *lfn)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::DirectoryExists(const char *dirname)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::CheckDataType(const char *lfn, Bool_t &isCollection, Bool_t &isXml, Bool_t &useTags)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::EnablePackage(const char *package)
{
// Enables a par file supposed to exist in the current directory.
   ...
   if (!TObject::TestBit(IlcAnalysisGrid::kUsePars))
      Info("EnablePackage", "AliEn plugin will use .par packages");
   TObject::SetBit(IlcAnalysisGrid::kUsePars, kTRUE);
   if (!fPackages) {
      fPackages = new TObjArray();
      fPackages->SetOwner();
   }
...
//______________________________________________________________________________
TChain *IlcAnalysisAlien::GetChainForTestMode(const char *treeName) const
{
...
//______________________________________________________________________________
const char *IlcAnalysisAlien::GetJobStatus(Int_t jobidstart, Int_t lastid, Int_t &nrunning, Int_t &nwaiting, Int_t &nerror, Int_t &ndone)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::IsCollection(const char *lfn) const
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::IsSingleOutput() const
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::Print(Option_t *) const
{
// Print current plugin settings.
   ...
   if (mgr && mgr->IsProofMode()) {
      ...
      if (TestBit(IlcAnalysisGrid::kTest))
      ...
      printf("=   IlcRoot version requested_____________________ %s\n", fIlcROOTVersion.Data());
      if (!fIlcRootMode.IsNull())
      printf("=   Requested IlcRoot mode________________________ %s\n", fIlcRootMode.Data());  
      if (fNproofWorkers)
   ...
   printf("=   Version of IlcRoot requested: ________________ %s\n", fIlcROOTVersion.Data());
   if (fUser.Length()) 
...
//______________________________________________________________________________
void IlcAnalysisAlien::SetDefaults()
{
// Set default values for everything. What cannot be filled will be left empty.
   ...
   fGridWorkingDir             = "";
   fGridDataDir                = "";  // Can be like: /ilc/sim/PDC_08a/LHC08c9/
   fDataPattern                = "*IlcESDs.root";  // Can be like: *IlcESDs.root, */pass1/*IlcESDs.root, ...
   fFriendChainName            = "";
   fGridOutputDir              = "output";
   fOutputArchive              = "log_archive.zip:std*@disk=1 root_archive.zip:*.root@disk=2";
   fOutputFiles                = "";  // Like "IlcAODs.root histos.root"
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::CheckMergedFiles(const char *filename, const char *aliendir, Int_t nperchunk, const char *jdl)
{
...
//______________________________________________________________________________
IlcAnalysisManager *IlcAnalysisAlien::LoadAnalysisManager(const char *fname)
{
// Loat the analysis manager from a file.
   ...
   IlcAnalysisManager *mgr = 0;
   TKey *key;
   while ((key=(TKey*)nextkey())) {
      if (!strcmp(key->GetClassName(), "IlcAnalysisManager"))
         mgr = (IlcAnalysisManager*)file->Get(key->GetName());
   }
   if (!mgr) 
...
//______________________________________________________________________________
Int_t IlcAnalysisAlien::SubmitSingleJob(const char *query)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::MergeOutput(const char *output, const char *basedir, Int_t nmaxmerge, Int_t stage)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::MergeOutputs()
{
// Merge analysis outputs existing in the AliEn space.
   if (TestBit(IlcAnalysisGrid::kTest)) return kTRUE;
   if (TestBit(IlcAnalysisGrid::kOffline)) return kFALSE;
   if (!Connect()) {
      Error("MergeOutputs", "Cannot merge outputs without grid connection. Terminate will NOT be executed");
      return kFALSE;
   }
   if (fMergeViaJDL) {
      if (!TestBit(IlcAnalysisGrid::kMerge)) {
   ...
   if (!fOutputFiles.Length()) {
      Error("MergeOutputs", "No output file names defined. Are you running the right IlcAnalysisAlien configuration ?");
      return kFALSE;
   }
...
//______________________________________________________________________________
void IlcAnalysisAlien::SetDefaultOutputs(Bool_t flag)
{
...
// rather than the files defined by SetOutputFiles
   if (flag && !TObject::TestBit(IlcAnalysisGrid::kDefaultOutputs))
      Info("SetDefaultOutputs", "Plugin will use the output files taken from analysis manager");
   TObject::SetBit(IlcAnalysisGrid::kDefaultOutputs, flag);
}
      
//______________________________________________________________________________
void IlcAnalysisAlien::SetOutputFiles(const char *list)
{
// Manually set the output files list.
// Removes duplicates. Not allowed if default outputs are not disabled.
   if (TObject::TestBit(IlcAnalysisGrid::kDefaultOutputs)) {
...
//______________________________________________________________________________
void IlcAnalysisAlien::SetOutputArchive(const char *list)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::SetPreferedSE(const char */*se*/)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::SetProofParameter(const char *pname, const char *value)
{
...
//______________________________________________________________________________
const char *IlcAnalysisAlien::GetProofParameter(const char *pname) const
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::StartAnalysis(Long64_t /*nentries*/, Long64_t /*firstEntry*/)
{
// Start remote grid analysis.
   IlcAnalysisManager *mgr = IlcAnalysisManager::GetAnalysisManager();
   Bool_t testMode = TestBit(IlcAnalysisGrid::kTest);
...
//         Info("StartAnalysis", "Dataset %s found", dataset.Data());
      ...
      // Is a given ilcroot mode requested ?
      TList optionsList;
      TString parLibs;
      if (!fIlcRootMode.IsNull()) {
         TString ilcrootMode = fIlcRootMode;
         if (ilcrootMode == "default") ilcrootMode = "";
         Info("StartAnalysis", "You are requesting IlcRoot mode: %s", fIlcRootMode.Data());
         optionsList.SetOwner();
         optionsList.Add(new TNamed("ILCROOT_MODE", ilcrootMode.Data()));
         // Check the additional libs to be loaded
         TString extraLibs;
         Bool_t parMode = kFALSE;
         if (!ilcrootMode.IsNull()) extraLibs = "ANALYSIS:OADB:ANALYSISilc";
        ...
        if (!extraLibs.IsNull()) {
          ...
          optionsList.Add(new TNamed("ILCROOT_EXTRA_LIBS",extraLibs.Data()));
        }
         // Check extra includes
         if (!fIncludePath.IsNull()) {
            TString includePath = fIncludePath;
            includePath.ReplaceAll(" ",":");
            includePath.ReplaceAll("$ILC_ROOT/","");
            includePath.ReplaceAll("${ILC_ROOT}/","");
            includePath.ReplaceAll("-I","");
            includePath.Remove(TString::kTrailing, ':');
            Info("StartAnalysis", "Adding extra includes: %s",includePath.Data()); 
            optionsList.Add(new TNamed("ILCROOT_EXTRA_INCLUDES",includePath.Data()));
         }
         // Check if connection to grid is requested
         if (TestSpecialBit(kProofConnectGrid)) 
            optionsList.Add(new TNamed("ILCROOT_ENABLE_ALIEN", "1"));
         // Enable IlcRoot par
         if (testMode) {
         // Enable proof lite package
            TString ilcrootLite = gSystem->ExpandPathName("$ILC_ROOT/ANALYSIS/macros/IlcRootProofLite.par");
            for (Int_t i=0; i<optionsList.GetSize(); i++) {
               TNamed *obj = (TNamed*)optionsList.At(i);
               printf("%s  %s\n", obj->GetName(), obj->GetTitle());
            }   
            if (!gROOT->ProcessLine(Form("gProof->UploadPackage(\"%s\");",ilcrootLite.Data()))
              && !gROOT->ProcessLine(Form("gProof->EnablePackage(\"%s\", (TList*)%p);",ilcrootLite.Data(),&optionsList))) {
                  Info("StartAnalysis", "IlcRootProofLite enabled");
            } else {                      
               Error("StartAnalysis", "There was an error trying to enable package IlcRootProofLite.par");
               return kFALSE;
            }   
         } else {
           if ( ! fIlcROOTVersion.IsNull() ) {
             if (gROOT->ProcessLine(Form("gProof->EnablePackage(\"VO_ILC@IlcRoot::%s\", (TList*)%p, kTRUE);", 
                                         fIlcROOTVersion.Data(), &optionsList))) {
                Error("StartAnalysis", "There was an error trying to enable package VO_ILC@IlcRoot::%s", fIlcROOTVersion.Data());
                return kFALSE;
             }
           }
         }
         ...
         if (fAdditionalLibs.Contains(".so") && !testMode) {
            Error("StartAnalysis", "You request additional libs to be loaded but did not enabled any IlcRoot mode. Please refer to: \
                   \n http://aaf.cern.ch/node/83 and use a parameter for SetIlcRootMode()");
            return kFALSE;       
         }
      }
...
//   if (!fCloseSE.Length()) fCloseSE = gSystem->Getenv("alien_CLOSE_SE");
   if (TestBit(IlcAnalysisGrid::kOffline)) {
      Info("StartAnalysis","\n##### OFFLINE MODE ##### Files to be used in GRID are produced but not copied \
      \n                         there nor any job run. You can revise the JDL and analysis \
      \n                         macro then run the same in \"submit\" mode.");
   } else if (TestBit(IlcAnalysisGrid::kTest)) {
      Info("StartAnalysis","\n##### LOCAL MODE #####   Your analysis will be run locally on a subset of the requested \
      \n                         dataset.");
   } else if (TestBit(IlcAnalysisGrid::kSubmit)) {
      Info("StartAnalysis","\n##### SUBMIT MODE #####  Files required by your analysis are copied to your grid working \
      \n                         space and job submitted.");
   } else if (TestBit(IlcAnalysisGrid::kMerge)) {
...
//______________________________________________________________________________
const char *IlcAnalysisAlien::GetListOfFiles(const char *type)
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::Submit()
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::SubmitMerging()
{
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::SubmitNext()
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::WriteAnalysisFile()
{
// Write current analysis manager into the file <analysisFile>
   ...
   if (!TestBit(IlcAnalysisGrid::kSubmit)) {  
      ...
      // Check analysis type
      TObject *handler;
      if (mgr->GetMCtruthEventHandler()) TObject::SetBit(IlcAnalysisGrid::kUseMC);
      handler = (TObject*)mgr->GetInputEventHandler();
      if (handler) {
         if (handler->InheritsFrom("IlcMultiInputEventHandler")) {
            IlcMultiInputEventHandler *multiIH = (IlcMultiInputEventHandler*)handler;
            if (multiIH->GetFirstInputEventHandler()->InheritsFrom("IlcESDInputHandler")) TObject::SetBit(IlcAnalysisGrid::kUseESD);
            if (multiIH->GetFirstInputEventHandler()->InheritsFrom("IlcAODInputHandler")) TObject::SetBit(IlcAnalysisGrid::kUseAOD);
         } else {
            if (handler->InheritsFrom("IlcESDInputHandler")) TObject::SetBit(IlcAnalysisGrid::kUseESD);
            if (handler->InheritsFrom("IlcAODInputHandler")) TObject::SetBit(IlcAnalysisGrid::kUseAOD);
         }
      }
      ...
      if (file) {
         ...
         if (!TestBit(IlcAnalysisGrid::kTest)) mgr->SetSkipTerminate(kTRUE);
...
//______________________________________________________________________________
void IlcAnalysisAlien::WriteAnalysisMacro()
{
// Write the analysis macro that will steer the analysis in grid mode.
   if (!TestBit(IlcAnalysisGrid::kSubmit)) {  
      ...
      if (IsUseMCchain()) {
         ...
         if (TObject::TestBit(IlcAnalysisGrid::kUseESD)) comment += "ESD";
         if (TObject::TestBit(IlcAnalysisGrid::kUseAOD)) {
            type = "AOD";
            comment += "AOD";
         }   
      }
      ...
      if (TObject::TestBit(IlcAnalysisGrid::kUseMC)) comment += "/MC";
      else comment += " data";
      ...
      if (!fExecutableCommand.Contains("ilcroot")) {
      ...
      TString setupPar = "IlcAnalysisAlien::SetupPar";
      if (!fPackages) {
         if (!fExecutableCommand.Contains("ilcroot")) {         
         ...
         out << "   gSystem->Load(\"libANALYSISilc\");" << endl;
         ...
         while ((obj=next())) {
            ...
            if (pkgname == "ANALYSISilc" ||
                pkgname == "ANALYSISilc.par") hasANALYSISilc = kTRUE;
            if (pkgname == "CORRFW" ||
                pkgname == "CORRFW.par")    hasCORRFW = kTRUE;
         }
         if (hasANALYSISilc) setupPar = "SetupPar";   
         ...
         if (!hasANALYSISilc)   out << "   gSystem->Load(\"libANALYSISilc\");" << endl;
         else out << "   if (!" << setupPar << "(\"ANALYSISilc\")) return;" << endl;
      ...
      // Get the include path from the interpreter and remove entries pointing to IlcRoot
      ...
      out << "      if (current.Contains(\"IlcRoot\") || current.Contains(\"ILC_ROOT\")) continue;" << endl;
      ...
      out << "   gROOT->ProcessLine(\".include $ILC_ROOT/include\");" << endl;
      out << "   printf(\"Include path: %s\\n\", gSystem->GetIncludePath());" << endl << endl;
      if (fAdditionalLibs.Length()) {
         out << "// Add aditional IlcRoot libraries" << endl;
         TObjArray *list = fAdditionalLibs.Tokenize(" ");
         TIter next(list);
         TObjString *str;
         while((str=(TObjString*)next())) {
...
//      out << "   printf(\"%s\\n\", gSystem->GetLibraries());" << endl;
      ...
      TString analysisFile = fExecutable;
      analysisFile.ReplaceAll(".sh", ".root");
      out << "   IlcAnalysisManager *mgr = IlcAnalysisAlien::LoadAnalysisManager(\"" 
          << analysisFile << "\");" << endl;
      out << "   if (!mgr) return;" << endl;
      if (IsLocalTest()) {
         out << "   IlcAnalysisAlien *plugin = new IlcAnalysisAlien();" << endl;
         ...
         if (IlcAnalysisManager::GetAnalysisManager()) {
            out << "   mgr->SetDebugLevel(" << IlcAnalysisManager::GetAnalysisManager()->GetDebugLevel() << ");" << endl;
            out << "   mgr->SetNSysInfo(" << IlcAnalysisManager::GetAnalysisManager()->GetNsysInfo() << ");" << endl;
         } else {
      ...
      if (IlcAnalysisManager::GetAnalysisManager()) {
         if (IlcAnalysisManager::GetAnalysisManager()->GetDebugLevel()>3) {
            out << "   gEnv->SetValue(\"XNet.Debug\", \"1\");" << endl;
         } else {
            if (TestBit(IlcAnalysisGrid::kTest))            
               out << "   IlcLog::SetGlobalLogLevel(IlcLog::kWarning);" << endl;
            else
               out << "   IlcLog::SetGlobalLogLevel(IlcLog::kError);" << endl;
         }
      }   
      if (!IsLocalTest()) {
      ...
      if (!IsLocalTest()) {
         ...
         out << "   }" << endl;
         out << "   IlcAnalysisManager *mgr = IlcAnalysisManager::GetAnalysisManager();" << endl;
         ...
         out << "      if (mgr) {" << endl;
         out << "         Int_t nrun = IlcAnalysisManager::GetRunFromAlienPath(filename);" << endl;
...
//______________________________________________________________________________
void IlcAnalysisAlien::WriteMergingMacro()
{
// Write a macro to merge the outputs per master job.
   if (!fMergeViaJDL) return;
   if (!fOutputFiles.Length()) {
      Error("WriteMergingMacro", "No output file names defined. Are you running the right IlcAnalysisAlien configuration ?");
      return;
   }   
   ...
   if (!TestBit(IlcAnalysisGrid::kSubmit)) {  
      ...
      out << "      merged = IlcAnalysisAlien::MergeOutput(outputFile, outputDir, " << fMaxMergeFiles << ", stage);" << endl;
      out << "      if (!merged) {" << endl;
...
//______________________________________________________________________________
Bool_t IlcAnalysisAlien::SetupPar(const char *package)
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::WriteExecutable()
{
// Generate the alien executable script.
   if (!TestBit(IlcAnalysisGrid::kSubmit)) {  
      ...
      out << "which root" << endl;
      out << "echo \"############## ILC_ROOT : ##############\"" << endl;
      out << "echo $ILC_ROOT" << endl;
      out << "echo \"############## which ilcroot : ##############\"" << endl;
      out << "which ilcroot" << endl;
...
//______________________________________________________________________________
void IlcAnalysisAlien::WriteMergeExecutable()
{
...
//______________________________________________________________________________
void IlcAnalysisAlien::WriteProductionFile(const char *filename) const
{
...
// To submit, one has to: submit jdl XXX for all lines
   ...
   TString workdir;
   if (!fProductionMode && !fGridWorkingDir.BeginsWith("/ilc"))
...
//______________________________________________________________________________
void IlcAnalysisAlien::WriteValidationScript(Bool_t merge)
{
// Generate the alien validation script.
   ...
   TString outStream = "";
   if (!TestBit(IlcAnalysisGrid::kTest)) outStream = " >> stdout";
   if (!TestBit(IlcAnalysisGrid::kSubmit)) {  
      ofstream out;

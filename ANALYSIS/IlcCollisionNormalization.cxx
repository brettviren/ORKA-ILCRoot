//-------------------------------------------------------------------------
//                      Implementation of   Class IlcCollisionNormalization
//
...
ClassImp(IlcCollisionNormalization)

const char * IlcCollisionNormalization::fgkProcLabel[] = {"SD","DD","ND", "Unknown"};

IlcCollisionNormalization::IlcCollisionNormalization() :
  TObject(),
  fNbinsVz(0),
  fMinVz(0),
  fMaxVz(0),
...
IlcCollisionNormalization::IlcCollisionNormalization(Int_t nbinz, Float_t minz, Float_t maxz):
  TObject(),
  fNbinsVz(0),
  fMinVz(0),
  fMaxVz(0),
...
IlcCollisionNormalization::IlcCollisionNormalization(const char * dataFile, const char * dataListName, 
...

{
  ...
  if (!fdata->IsOpen() || !fmc->IsOpen() || !fstat->IsOpen()) {
    IlcFatal("Cannot open input file(s)");
  }
  
  TList * ldata = (TList*) fdata->Get(dataListName);
  TList * lmc   = (TList*) fmc  ->Get(mcListName  );

  IlcCollisionNormalization * cndata = (IlcCollisionNormalization*) ldata->FindObject("IlcCollisionNormalization");
  IlcCollisionNormalization * cnmc   = (IlcCollisionNormalization*) lmc  ->FindObject("IlcCollisionNormalization");
...
void IlcCollisionNormalization::BookAllHistos(){
  // books all histos
...
TH1 * IlcCollisionNormalization::BookVzHisto(const char * name , const char * title, Bool_t vzOnly) {
...
TH2F * IlcCollisionNormalization::GetVzMCGen (Int_t procType) { 
...
TH2F *   IlcCollisionNormalization::GetVzMCRec       (Int_t procType) { 
...
TH2F *   IlcCollisionNormalization::GetVzMCTrg       (Int_t procType) { 
...
Double_t IlcCollisionNormalization::ComputeNint() {
  ...
  IlcInfo(Form("Input Events (No cuts: %d, After Phys. Sel.:%d)",
	       Int_t(fInputEvents),
  ...
  if (fHistStatBin0->GetNbinsY() > 4) { // FIXME: we need a better criterion to decide...
    IlcInfo("Using BG computed by Physics Selection");
    // WARNING
    ...
    for(Int_t ibiny =1; ibiny <= nbiny; ibiny++){
      ...
      if((ibiny+1)==nbiny) IlcFatal("Cannot compute bg offset");
    }
    
    if(fVerbose > 2) IlcInfo(Form("BG Offset: %d",bgOffset));
    

    fBgEvents = fHistStatBin0->GetBinContent(fHistStatBin0->GetNbinsX(), bgOffset+IlcPhysicsSelection::kStatRowBG);
    fBgEvents += fHistStatBin0->GetBinContent(fHistStatBin0->GetNbinsX(),bgOffset+IlcPhysicsSelection::kStatRowAcc);
    Int_t cint1B = (Int_t) fHistStatBin0->GetBinContent(fHistStatBin0->GetNbinsX(),1);	
    if (cint1B != Int_t(triggeredEventsWith0MultWithBG)) {
      IlcWarning(Form("Events in bin0 from physics selection and local counter not consistent: %d - %d", cint1B, Int_t(triggeredEventsWith0MultWithBG)));
    }
  } else {
    IlcInfo("Computing BG using CINT1A/B/C/E, ignoring intensities");
    IlcError("This will only work for early runs!!! USE BG FROM PHYSICS SELECTION INSTEAD");
  ...
  if(fVerbose > 0) IlcInfo(Form("Measured events with vertex: %d",allEventsWithVertex));
  Double_t bin0 = fHistVzData->Integral(0, fHistVzData->GetNbinsX()+1,
					1, 1);
  if(fVerbose > 0) IlcInfo(Form("Zero Bin, Meas: %2.2f, BG: %2.2f, Meas - BG: %2.2f", bin0, fBgEvents, triggeredEventsWith0Mult)); 
  ...
  if(fVerbose > 1) IlcInfo(Form("Results in |Vz| < %3.3f",fZRange));
  if(fVerbose > 1) IlcInfo(Form(" Events in Bin0: %2.2f, With > 1 track: %2.2f, All corrected: %2.2f",
  ...
  if(fVerbose > 1) {    
    IlcInfo(Form("Trigger Efficiency in the zero bin: %3.3f", fTrigEffBin0 ));
  }
  

  IlcInfo(Form("Number of collisions in full phase space: %2.2f", fAllEvents));
//   effVtxTrig->Draw();
//   new TCanvas();
...
Long64_t IlcCollisionNormalization::Merge(TCollection* list)
{
  // Merge a list of IlcCollisionNormalization objects with this
  // (needed for PROOF).  
  ...
  while ((obj = iter->Next())) {
    ...
    IlcCollisionNormalization* entry = dynamic_cast<IlcCollisionNormalization*> (obj);
    if (entry == 0) 
      continue;
    Int_t ihist = -1;
...
void IlcCollisionNormalization::FillVzMCGen(Float_t vz, Int_t ntrk, IlcMCEvent * mcEvt) {
...
void IlcCollisionNormalization::FillVzMCRec(Float_t vz, Int_t ntrk, IlcMCEvent * mcEvt){
...
void IlcCollisionNormalization::FillVzMCTrg(Float_t vz, Int_t ntrk, IlcMCEvent * mcEvt) {      
...
Int_t IlcCollisionNormalization::GetProcessType(const IlcMCEvent * mcEvt) {

  // Determine if the event was generated with pythia or phojet and return the process type

  // Check if mcEvt is fine
  if (!mcEvt) {
    IlcFatal("NULL mc event");
  } 

  // Determine if it was a pythia or phojet header, and return the correct process type
  IlcGenPythiaEventHeader * headPy  = 0;
  IlcGenDPMjetEventHeader * headPho = 0;
  IlcGenEventHeader * htmp = mcEvt->GenEventHeader();
  if(!htmp) {
    IlcFatal("Cannot Get MC Header!!");
  }
  if( TString(htmp->IsA()->GetName()) == "IlcGenPythiaEventHeader") {
    headPy =  (IlcGenPythiaEventHeader*) htmp;
  } else if (TString(htmp->IsA()->GetName()) == "IlcGenDPMjetEventHeader") {
    headPho = (IlcGenDPMjetEventHeader*) htmp;
  } else {
    IlcError("Unknown header");
  }
  ...
  IlcError(Form("Unknown header: %s", htmp->IsA()->GetName()));
  return kProcUnknown;
}

Double_t IlcCollisionNormalization::GetProcessWeight(Int_t proc) {
  ...
  if (fHistProcTypes->GetBinContent(fHistProcTypes->FindBin(kProcUnknown)) > 0) {
    IlcError("There were unknown processes!!!");
  }
  ...
  if (fVerbose > 2) {
    IlcInfo(Form("Total MC evts: %f",total));
    IlcInfo(Form(" Frac SD %4.4f", lSD));
    IlcInfo(Form(" Frac DD %4.4f", lDD));
    IlcInfo(Form(" Frac ND %4.4f", lND));
  }
...
void IlcCollisionNormalization::GetRelativeFractions(Int_t origin, Float_t& refSD, Float_t& refDD, Float_t& refND, Float_t& errorSD, Float_t& errorDD, Float_t& errorND)
{
  ...
  if(TMath::Abs(fEnergy-900)<epsilon) {
	...
	IlcInfo("PYTHIA x-sections");
      ...
      case 0: // UA5
	IlcInfo("UA5 x-sections a la first paper");
	...
	break;
      ...
      case 10: // UA5
	IlcInfo("UA5 x-sections hadron level definition for Pythia"); 
	...
	break;
      ...
      case 11: // UA5
	IlcInfo("UA5 x-sections hadron level definition for Phojet"); 
	...
	IlcInfo("Tel-aviv model x-sections");
	refSD = 0.171;
	refDD = 0.094;
	refND = 1 - refSD - refDD;
	break;
      ...
      case 3: // durham model
	IlcInfo("Durham model x-sections");
	refSD = 0.190;
	refDD = 0.125;
	refND = 1 - refSD - refDD;
      break;
      default:
	IlcFatal(Form("Unknown origin %d, Energy %f", origin, fEnergy));
      }
  }
    ...
    switch (origin)
      ...
      IlcInfo("E710 x-sections hadron level definition for Pythia");
      // ND: 0.705709
	...
	IlcInfo("E710 x-sections hadron level definition for Phojet"); 
	// ND: 0.817462
    ...
    IlcFatal(Form("Unknown energy %f", fEnergy));
  }

...
/* $Id: IlcESDv0Cuts.cxx 24534 2008-03-16 22:22:11Z fca $ */
...
ClassImp(IlcESDv0Cuts)

// Cut names
const Char_t* IlcESDv0Cuts::fgkCutNames[kNCuts] = {
...
//____________________________________________________________________
IlcESDv0Cuts::IlcESDv0Cuts(const Char_t* name, const Char_t* title) : IlcAnalysisCuts(name,title),
...
IlcESDv0Cuts::IlcESDv0Cuts(const IlcESDv0Cuts &c) : IlcAnalysisCuts(c),
  fCutMinDcaPosToVertex(0),
  fCutMinDcaNegToVertex(0),
  fCutMaxChi2(0),
  fCutMaxDcaV0Daughters(0),
  ...
  fhCutCorrelation(0)
{
  ...
  ((IlcESDv0Cuts &) c).Copy(*this);
}

IlcESDv0Cuts::~IlcESDv0Cuts()
{
  //
  // destructor
  //
...
//_____________________________________________________________________________
IlcESDv0Cuts &IlcESDv0Cuts::operator=(const IlcESDv0Cuts &c)
{
  //
  // Assignment operator
  //

  if (this != &c) ((IlcESDv0Cuts &) c).Copy(*this);
  return *this;
}

//_____________________________________________________________________________
void IlcESDv0Cuts::Copy(TObject &c) const
{
  //
  // Copy function
  //

  IlcESDv0Cuts& target = (IlcESDv0Cuts &) c;
...
//_____________________________________________________________________________
Long64_t IlcESDv0Cuts::Merge(TCollection* list) {
  // Merge a list of IlcESDv0Cuts objects with this (needed for PROOF)
  ...
  while ((obj = iter->Next())) {
    ...
    IlcESDv0Cuts* entry = dynamic_cast<IlcESDv0Cuts*>(obj);
    if (entry == 0)
      continue;
...
void IlcESDv0Cuts::EnableNeededBranches(TTree* tree)
{
...
//____________________________________________________________________
Bool_t
IlcESDv0Cuts::IsSelected(TList* listObj) {
// Selection cuts
  if(listObj->GetSize()!=4) return kFALSE;
  IlcESDv0           *esdV0     = (IlcESDv0*)listObj->At(0);
  IlcESDtrack        *trackPos  = (IlcESDtrack*)listObj->At(1);
  IlcESDtrack        *trackNeg  = (IlcESDtrack*)listObj->At(2);
  const IlcESDVertex *esdVertex = (IlcESDVertex*)listObj->At(3);
  return AcceptV0(esdV0,trackPos,trackNeg,esdVertex);
}

//____________________________________________________________________
Bool_t
IlcESDv0Cuts::AcceptV0(IlcESDv0* const esdV0, IlcESDtrack* const trackPos, IlcESDtrack* const trackNeg, const IlcESDVertex* esdVertex) {
  // 
...
//____________________________________________________________________
TObjArray* IlcESDv0Cuts::GetAcceptedV0s(const IlcESD* esd)
{
  //
  ...
  //  const IlcESDVertex *spdVertex = esd->GetVertex();
  const IlcESDVertex *primaryVertex = esd->GetPrimaryVertex();
  Int_t    lIndexTrackPos       = 0, lIndexTrackNeg       = 0;

  // loop over esd v0s
  for (Int_t iV0 = 0; iV0 < esd->GetNumberOfV0s(); iV0++) {
    IlcESDv0* v0 = esd->GetV0(iV0);

    lIndexTrackPos = TMath::Abs(v0->GetPindex());
    lIndexTrackNeg = TMath::Abs(v0->GetNindex());
    IlcESDtrack *trackPos = esd->GetTrack(lIndexTrackPos);
    IlcESDtrack *trackNeg = esd->GetTrack(lIndexTrackNeg);
...
//____________________________________________________________________
Int_t IlcESDv0Cuts::CountAcceptedV0s(const IlcESD* esd)
{
  //
...
//____________________________________________________________________
TObjArray* IlcESDv0Cuts::GetAcceptedV0s(const IlcESDEvent* esd)
{
  //
  // returns an array of all v0s that pass the cuts
  //
...
//____________________________________________________________________
Int_t IlcESDv0Cuts::CountAcceptedV0s(const IlcESDEvent* esd)
{
  //
  // returns an the number of v0s that pass the cuts
  //
...
//____________________________________________________________________
 void IlcESDv0Cuts::DefineHistograms(Int_t color) {
   // 
...
//____________________________________________________________________
Bool_t IlcESDv0Cuts::LoadHistograms(const Char_t* dir)
{
  //
...
//____________________________________________________________________
void IlcESDv0Cuts::SaveHistograms(const Char_t* dir) {
  //
...
//____________________________________________________________________
void IlcESDv0Cuts::DrawHistograms()
{

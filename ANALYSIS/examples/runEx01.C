// run.C
//
// Template run macro for IlcBasicTask.cxx/.h with example layout of
...
void runEx01(
         ...
         const char *proofdataset = "/ilc/data/LHC10c_000120821_p1", // path to dataset on proof cluster, for proof analysis
         const char *proofcluster = "ilc-caf.cern.ch", // which proof cluster to use in proof mode
         ...
         )
{
    ...
    // add ilcroot indlude path
    gROOT->ProcessLine(Form(".include %s/include",gSystem->ExpandPathName("$ILC_ROOT")));
    gROOT->SetStyle("Plain");
        
    // analysis manager
    IlcAnalysisManager* mgr = new IlcAnalysisManager(taskname);
    
    // create the alien handler and attach it to the manager
    IlcAnalysisGrid *plugin = CreateAlienHandler(taskname, gridmode, proofcluster, proofdataset); 
    mgr->SetGridHandler(plugin);
    
    IlcVEventHandler* iH = new IlcESDInputHandler();
//    IlcAODInputHandler* iH = new IlcAODInputHandler();
//    iH->SetInactiveBranches("tracks. vertices. v0s. cascades. jets. caloClusters. fmdClusters. pmdClusters. dimuons. IlcAODZDC");
...
//    iH->SetCheckStatistics(kTRUE);
    ...
    // and see IlcBasicTask.cxx UserExec() function for details on this.

//    gROOT->LoadMacro("$ILC_ROOT/ANALYSIS/macros/AddTaskPhysicsSelection.C");
//    IlcPhysicsSelectionTask *physSelTask = AddTaskPhysicsSelection(bMCphyssel);
//    if(!physSelTask) { Printf("no physSelTask"); return; }
    ...
    // create task
    gROOT->LoadMacro("IlcAnalysisTaskEx01.cxx+g");
    IlcAnalysisTaskSE* task = new IlcAnalysisTaskEx01(taskname);
    task->SelectCollisionCandidates(IlcVEvent::kMB); // if physics selection performed in UserExec(), this line should be commented
    mgr->AddTask(task);
    ...
    IlcAnalysisDataContainer *coutput1 = mgr->CreateContainer("coutput1", TList::Class(), IlcAnalysisManager::kOutputContainer, outfilename);
...
//______________________________________________________________________________
IlcAnalysisGrid* CreateAlienHandler(const char *taskname, const char *gridmode, const char *proofcluster, const char *proofdataset)
{
    ...
    plugin->SetIlcROOTVersion("v4-21-25-AN-1");
...
//    plugin->SetCheckCopy(kFALSE);
    ...
    plugin->SetGridDataDir("/ilc/data/2010/LHC10b");
    // On real reconstructed data:
    // plugin->SetGridDataDir("/ilc/data/2009/LHC09d");
...
//    plugin->SetDataPattern("*ESDs/pass2/*ESDs.root"); // CHECK LATEST PASS OF DATA SET IN ALIENSH
    plugin->SetDataPattern("ESDs/pass2/AOD038/*IlcAOD.root"); // CHECK LATEST PASS OF DATA SET IN ALIENSH
...
//     plugin->AddRunNumber(138275);
    ...
    plugin->SetAnalysisSource("IlcAnalysisTaskEx01.cxx");

    // Declare all libraries (other than the default ones for the framework. These will be
    // loaded by the generated analysis macro. Add all extra files (task .cxx/.h) here.
    plugin->SetAdditionalLibs("IlcAnalysisTaskEx01.h IlcAnalysisTaskEx01.cxx");
    ...
    // May set the ilcroot mode. Check http://aaf.cern.ch/node/83 
    plugin->SetIlcRootMode("default"); // Loads AF libs by default

...
/* $Id: IlcAnalysisTaskEx01.cxx 53223 2011-11-28 09:40:24Z agheata $ */

/* IlcAnalysisTaskEx01.cxx
 *
...
ClassImp(IlcAnalysisTaskEx01)

//________________________________________________________________________
IlcAnalysisTaskEx01::IlcAnalysisTaskEx01() // All data members should be initialised here
   :IlcAnalysisTaskSE(),
    fOutput(0),
    fTrackCuts(0),
    fHistPt(0), 
...
//________________________________________________________________________
IlcAnalysisTaskEx01::IlcAnalysisTaskEx01(const char *name) // All data members should be initialised here
...
//________________________________________________________________________
IlcAnalysisTaskEx01::~IlcAnalysisTaskEx01()
{
...
//________________________________________________________________________
void IlcAnalysisTaskEx01::UserCreateOutputObjects()
{
    // Create histograms
    ...
    fTrackCuts = IlcESDtrackCuts::GetStandardITSTPCTrackCuts2010(kTRUE);
    // === Primary Track Selection ===
    //
    // The definition of a primary track is taken from the ILC Twiki
    // page https://twiki.cern.ch/twiki/bin/view/ILC/SelectionOfPrimaryTracksForPpDataAnalysis
    ...
    // IlcESDtrackCuts object, with kTRUE argument to choose primary tracks.
    //
...
//________________________________________________________________________
void IlcAnalysisTaskEx01::UserExec(Option_t *) 
{
    // Main loop
    ...
    */  
    ...
    if (!esd) {
        IlcError("Cannot get the ESD event");
        return;
    }  
//    IlcESDHeader* esdheader = (IlcESDHeader*)esd->GetHeader();
    ...
    //
    //  UInt_t mask = ((IlcInputEventHandler*)(IlcAnalysisManager::GetAnalysisManager()->GetInputEventHandler()))->IsEventSelected();   
    //
    // This can be tested to produce the following
    //
    //  Bool_t bMinBias = (mask == IlcVEvent::kMB) ? 1 : 0; // check if minimum bias trigger class fired
    //  Bool_t bHighMult = (mask == IlcVEvent::kHighMult) ? 1 : 0; // check if high multiplicity trigger class fired
    //
    ...
    for(Int_t i = 0; i < ntracks; i++) {
        IlcESDtrack* esdtrack = esd->GetTrack(i); // pointer to reconstructed to track          
        if(!esdtrack) { 
            IlcError(Form("ERROR: Could not retrieve esdtrack %d",i)); 
            continue; 
        }
...
//________________________________________________________________________
void IlcAnalysisTaskEx01::Terminate(Option_t *) 
{
    ...
    //IlcESDInputHandler *inputH = dynamic_cast<IlcESDInputHandler*>(mgr->GetInputEventHandler());
    ...
    TCanvas *c = new TCanvas("IlcAnalysisTaskEx01","P_{T} & #eta",10,10,1020,510);
    c->Divide(2,1);
    c->cd(1)->SetLogy();
    fHistPt->DrawCopy("E");
    c->cd(2);

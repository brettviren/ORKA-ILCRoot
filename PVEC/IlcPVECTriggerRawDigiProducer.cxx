 ...
 **************************************************************************/
//This class produces PVEC trigger digits of one event.
//Authors: Henrik Qvigstad, Boris Polishchuk.

#include "IlcPVECTriggerRawDigiProducer.h"
#include "IlcPVECTriggerRawReader.h"
#include "IlcPVECTRURawReader.h"
...
ClassImp(IlcPVECTriggerRawDigiProducer)

IlcPVECTriggerRawDigiProducer::IlcPVECTriggerRawDigiProducer()
...
IlcPVECTriggerRawDigiProducer::IlcPVECTriggerRawDigiProducer(IlcRawReader *rawReader)
  :fModules(kNMods, false),
   fSaturationThreshold(950),
   fParameters(0),
   fRawReader(rawReader),
   fRawStream(0),
   fTriggerReader(new IlcPVECTriggerRawReader)
{
  SetAnalyseModule(2);
  SetAnalyseModule(3);
  SetAnalyseModule(4);

  fRawStream = new IlcCaloRawStreamV3(rawReader,"PVEC");
}
...
void IlcPVECTriggerRawDigiProducer::ProcessEvent(TClonesArray* tdigits)
{
  ...
  for(unsigned int mod = 0; mod < fModules.size(); ++mod) {
	...
	for(int branch = 0; branch < kNBranches; ++branch) {
	  ...
	  IlcPVECTRURawReader* truReader = fTriggerReader->GetTRU(mod, TRURow, branch);
	  if( truReader->IsActive() ) {
		...
		if( triggered ){
		  ...
		  new((*tdigits)[iDigit]) IlcPVECTriggerRawDigit(mod,xIdx,zIdx,TRURow,branch,TSmax); 
		  iDigit++;
		}// end  "if triggered"
...
int IlcPVECTriggerRawDigiProducer::Get2x2Max(IlcPVECTriggerRawReader* reader, IlcPVECTriggerParameters* params, int mod, int xIdx, int zIdx)
{
  int max = 0;
  for(int timeBin = 0; timeBin < kNTRUTimeBins; ++timeBin) {
...
int IlcPVECTriggerRawDigiProducer::Get2x2Signal(IlcPVECTriggerRawReader* reader, IlcPVECTriggerParameters* parameters, int mod, int xIdx, int zIdx, int timeBin)
{
  ...
  if( reader->GetTRU(mod, TRURow, branch)->IsActive() ){
    ...
    if( parameters )
      ...
      return signal - IlcPVECTRURawReader::kDefaultSignalValue;
  }
  else
    return 0;
}

int IlcPVECTriggerRawDigiProducer::Get4x4Max(IlcPVECTriggerRawReader* reader, IlcPVECTriggerParameters* params, int mod, int TRURow, int branch, int xIdx, int zIdx)
{
  int max = 0;
  for(int timeBin = 0; timeBin < kNTRUTimeBins; ++timeBin) {
...
int IlcPVECTriggerRawDigiProducer::Get4x4Signal(IlcPVECTriggerRawReader* reader, IlcPVECTriggerParameters* params, int mod, int TRURow, int branch, int xIdx, int zIdx, int timeBin)
{
...
bool IlcPVECTriggerRawDigiProducer::Is2x2Active(IlcPVECTriggerRawReader* reader, int mod, int xIdx, int zIdx)
{
  const int TRURow = xIdx / kN2x2XPrTRURow;
  const int branch = zIdx / kN2x2ZPrBranch;
...
bool IlcPVECTriggerRawDigiProducer::Is2x2Active(IlcPVECTriggerRawReader* reader, int mod, int xIdx, int zIdx, int timeBin)
{
  const int TRURow = xIdx / kN2x2XPrTRURow;
  const int branch = zIdx / kN2x2ZPrBranch;

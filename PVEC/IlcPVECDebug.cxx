...
/* $Id: IlcPVECebug.cxx 30520 2009-01-02 23:54:05Z fca $ */
...
//____________________________________________________________________________
IlcPVECv1::IlcPVECv1(IlcPVECReconstructor * Reconstructioner, const char *name, const char *title):
  IlcPVECv0(name,title)
{
  // ctor : title is used to identify the layout
  ...
  IlcPVECGeometry::GetInstance(title, "") ; 

  if (GetGeometry()->IsInitialized() ) 
    Info("IlcPVECv1", "IlcPVEC %d : PVEC geometry intialized for %s", Version(), GetGeometry()->GetName() );
  else
    Info("IlcPVECv1", "IlcPVEC %d : PVEC geometry initialization failed !", Version() ) ;   

  // Defining the PVEC Reconstructioner
...
//____________________________________________________________________________
void IlcPVECv1::AddHit(Int_t shunt, Int_t primary, Int_t tracknumber, Int_t Id, Float_t * hits, Int_t trackpid, TLorentzVector p, Float_t * lpos)
{
  // Add a hit to the hit list.
  // A PVEC hit is the sum of all hits in a single crystal
  ...
  newHit = new IlcPVECHit(shunt, primary, tracknumber, Id, hits, trackpid, p, lpos) ;

  for ( hitCounter = fNhits-1 ; hitCounter >= 0 && !deja ; hitCounter-- ) {
    curHit = (IlcPVECHit*) (*fHits)[hitCounter] ;
    if( *curHit == *newHit ) {
...
void IlcPVECv1::Hits2SDigits()
{
  ...
  Int_t j ; 
  IlcPVECHit   * hit ;
  IlcPVECDigit * newdigit ;
  IlcPVECDigit * curdigit ;
  Bool_t deja = kFALSE ; 
  ...
  for (itrack=0; itrack<gIlc->GetNtrack(); itrack++){
    ...
    for ( i = 0 ; i < fHits->GetEntries() ; i++ ) {
      hit = (IlcPVECHit*)fHits->At(i) ;
    
      // Assign primary number only if contribution is significant
      if( hit->GetEnergy() > fDigitThreshold)
	newdigit = new IlcPVECDigit( hit->GetPrimary(), hit->GetId(), Digitize( hit->GetEnergy() ) ) ;
      else
	newdigit = new IlcPVECDigit( -1               , hit->GetId(), Digitize( hit->GetEnergy() ) ) ;
      deja =kFALSE ;

      for ( j = 0 ; j < fnSdigits ;  j++) { 
	curdigit = (IlcPVECDigit*) fSDigits->At(j) ;
	if ( *curdigit == *newdigit) {
	  *curdigit = *curdigit + *newdigit ; 
      ...
      if ( !deja ) {
	new((*fSDigits)[fnSdigits]) IlcPVECDigit(* newdigit) ;
	fnSdigits++ ;  
      }
  ...
  for (i = 0 ; i < fnSdigits ; i++) { 
    IlcPVECDigit * digit = (IlcPVECDigit *) fSDigits->At(i) ; 
...
void IlcPVECv1::SDigits2Digits()
{
  ...
  Int_t idCurSDigit = ((IlcPVECDigit *)fSDigits->At(0))->GetId() ;
  ...
  for(absID = 1; absID < GetGeometry()->GetNModules()*GetGeometry()->GetNPhi()*GetGeometry()->GetNZ(); absID++){
	...
	new((*fDigits)[fNdigits]) IlcPVECDigit( -1,absID,Digitize(noise) ) ;
	fNdigits++ ;  
      }
    }
    else{ //add noise and may be remove the true hit
      Float_t signal = noise + Calibrate(((IlcPVECDigit *)fSDigits->At(iCurSDigit))->GetAmp()) ;
      if( signal >fDigitThreshold ){
	IlcPVECDigit * digit = (IlcPVECDigit*) fSDigits->At(iCurSDigit) ;
	new((*fDigits)[fNdigits]) IlcPVECDigit( *digit ) ;
	((IlcPVECDigit *)fDigits->At(fNdigits))->SetAmp(Digitize(signal));
	fNdigits++ ;  
      }

      if(iCurSDigit < fSDigits->GetEntries()-1){
	iCurSDigit++ ;
	idCurSDigit = ((IlcPVECDigit*)fSDigits->At(iCurSDigit))->GetId() ;
      }
      else
  ...
  for(idigit = iCurSDigit; idigit < fSDigits->GetEntries() ; idigit++){  //loop over CPV/PPSD digits
    ...
    IlcPVECDigit * digit = (IlcPVECDigit *) fSDigits->At(idigit) ; 
  ...
  for (i = 0 ; i < fNdigits ; i++) { 
    IlcPVECDigit * digit = (IlcPVECDigit *) fDigits->At(i) ; 
    digit->SetIndexInList(i) ;     
  }
...
void IlcPVECv1::MakeBranch(Option_t* opt, char *file)
{ 
  // Called by IlcRun
...
void IlcPVECv1::Reconstruction(IlcPVECReconstructor * Reconstructioner)
{ 
...
//____________________________________________________________________________
void IlcPVECv1::ResetReconstruction() 
{ 
  // Deleting reconstructed objects
...
void IlcPVECv1::StepManager(void)
{
  ...
  if ( name == "IHEP" ) {       // ======> CPV is a IHEP's one
	...
	gMC->TrackCharge() != 0) {      
      ...
      TClonesArray *cpvDigits = new TClonesArray("IlcPVECCPVDigit",0);   // array of digits for current hit
	...
	IlcPVECCPVDigit  *cpvDigit1 = (IlcPVECCPVDigit*) cpvDigits->UncheckedAt(idigit);
	Float_t x1 = cpvDigit1->GetXpad() ;
	Float_t z1 = cpvDigit1->GetYpad() ;
	for (Int_t jdigit=idigit+1; jdigit<ndigits; jdigit++) {
	  IlcPVECCPVDigit  *cpvDigit2 = (IlcPVECCPVDigit*) cpvDigits->UncheckedAt(jdigit);
	...
	IlcPVECCPVDigit  *cpvDigit = (IlcPVECCPVDigit*) cpvDigits->UncheckedAt(idigit);
	relid[0] = moduleNumber + 1 ;                             // CPV (or PVEC) module number
	relid[1] =-1 ;                                            // means CPV
  ...
  if(gMC->CurrentVolID(copy) == gMC->VolId("PXTL") ) { //  We are inside a PBWO crystal
    ...
    if ( (xyze[3] != 0) || entered ) {  // Track is inside the crystal and deposits some energy or just entered 
      ...
      gMC->CurrentVolOffID(10, relid[0]) ; // get the PVEC module number ;
...
//____________________________________________________________________________
void IlcPVECv1::CPVDigitize (TLorentzVector p, Float_t *zxhit, Int_t moduleNumber, TClonesArray *cpvDigits)
{
  // ------------------------------------------------------------------------
  ...
  for (Int_t iter=0; iter<nIter; iter++) {
	...
	new(ldigits[cpvDigits->GetEntriesFast()]) IlcPVECCPVDigit(kxg,kzg,qpad);
      }
    }
  }
}
...
//____________________________________________________________________________
Float_t IlcPVECv1::CPVPadResponseFunction(Float_t qhit, Float_t zhit, Float_t xhit) {
  // ------------------------------------------------------------------------
...
//____________________________________________________________________________
Double_t IlcPVECv1::CPVCumulPadResponse(Double_t x, Double_t y) {
  // ------------------------------------------------------------------------
  // Cumulative pad response function

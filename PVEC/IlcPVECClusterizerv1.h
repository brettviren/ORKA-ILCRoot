#ifndef ILCPVECCLUSTERIZERV1_H
#define ILCPVECCLUSTERIZERV1_H
/* 2012-2013 Infrastructure for Large Collider Experiment.       *
 * All rights reserved.                                                   *
 * See cxx source for full Copyright notice                               */

/* $Id: IlcPVECClusterizerv1.h 43634 2010-09-17 09:17:08Z prsnko $ */

/* History of cvs commits:
 *
 * $Log: IlcPVECClusterizerv1.h,v $
...
//  PVECCpvRP (CPV RecPoints) and IlcPVECClusterizer
//
...
class IlcPVECClusterizerv1 : public IlcPVECClusterizer {
  
public:
  
  IlcPVECClusterizerv1() ;
  IlcPVECClusterizerv1(IlcPVECGeometry *geom);
  virtual ~IlcPVECClusterizerv1()  ;

  void    InitParameters() ;
  
  virtual Int_t   AreNeighbours(IlcPVECDigit * d1, IlcPVECDigit * d2)const ; 
...
protected:
  ...
  virtual Bool_t IsInEmc (IlcPVECDigit * digit)const ;     // Tells if id digit is in EMC
  virtual Bool_t IsInCpv (IlcPVECDigit * digit)const ;     // Tells if id digit is in CPV
...
private:
  IlcPVECClusterizerv1(const IlcPVECClusterizerv1 & clu) ;
  IlcPVECClusterizerv1 & operator = (const IlcPVECClusterizerv1 & obj);

  Bool_t  FindFit(IlcPVECEmcRecPoint * emcRP, IlcPVECDigit ** MaxAt, Float_t * maxAtEnergy, 
		  Int_t NPar, Float_t * FitParametres) const; //Used in UnfoldClusters, calls TMinuit
  void    Init() ;

  virtual void   MakeUnfolding() ;
  void           UnfoldCluster(IlcPVECEmcRecPoint * iniEmc,Int_t Nmax, 
		       IlcPVECDigit ** maxAt,Float_t * maxAtEnergy ) ; //Unfolds cluster using TMinuit package
  void           PrintRecPoints(Option_t * option) ;

private:

  Bool_t  fDefaultInit;              //! Says if the task was created by defaut ctor (only parameters are initialized)
  Int_t   fEmcCrystals ;             // number of EMC cristals in PVEC
  ...
  ClassDef(IlcPVECClusterizerv1,8)   // Clusterizer implementation version 1

};

#endif // IlcPVECCLUSTERIZERV1_H

...
// 
// Typical use case:
//     IlcPVECRawFitter *fitter=new IlcPVECRawFitter();
...
ClassImp(IlcPVECRawFitterv1)

//-----------------------------------------------------------------------------
IlcPVECRawFitterv1::IlcPVECRawFitterv1():
  IlcPVECRawFitterv0(),
...
//-----------------------------------------------------------------------------
IlcPVECRawFitterv1::IlcPVECRawFitterv1(const IlcPVECRawFitterv1 &PVECFitter ):
  IlcPVECRawFitterv0(PVECFitter),
  fSampleParamsLow(0x0),
  fSampleParamsHigh(0x0),
  fToFit(0x0)
{
  //Copy constructor.
  fToFit = new TList() ;
  fSampleParamsLow =new TArrayD(*(PVECFitter.fSampleParamsLow)) ;
  fSampleParamsHigh=new TArrayD(*(PVECFitter.fSampleParamsHigh)) ;
}

//-----------------------------------------------------------------------------
IlcPVECRawFitterv1& IlcPVECRawFitterv1::operator = (const IlcPVECRawFitterv1 &PVECFitter)
{
  //Assignment operator.
  if(this != &PVECFitter) {
    fToFit = new TList() ;
    if(fSampleParamsLow){
      fSampleParamsLow = PVECFitter.fSampleParamsLow ;
      fSampleParamsHigh= PVECFitter.fSampleParamsHigh ;
    }
    else{
      fSampleParamsLow =new TArrayD(*(PVECFitter.fSampleParamsLow)) ; 
      fSampleParamsHigh=new TArrayD(*(PVECFitter.fSampleParamsHigh)) ;
...
//-----------------------------------------------------------------------------
Bool_t IlcPVECRawFitterv1::Eval(const UShort_t *signal, Int_t sigStart, Int_t sigLength)
{
  ...
  gMinuit->SetFCN(IlcPVECRawFitterv1::UnfoldingChiSquare) ;  
...
//-----------------------------------------------------------------------------
Double_t IlcPVECRawFitterv1::Gamma2(Double_t dt,Double_t en,Double_t b,TArrayD * params){  //Function for fitting samples
  //parameters:
  //dt-time after start
  //en-amplutude
  //function parameters
...
void IlcPVECRawFitterv1::UnfoldingChiSquare(Int_t & /*nPar*/, Double_t * Grad, Double_t & fret, Double_t * x, Int_t iflag)
{

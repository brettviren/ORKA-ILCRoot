...
/* $Id: IlcPVECQADataMakerRec.cxx 57196 2012-06-15 08:08:57Z hristov $ */
...
#include "IlcLog.h"
#include "IlcPVECQADataMakerRec.h"
...
ClassImp(IlcPVECQADataMakerRec)
           
//____________________________________________________________________________ 
  IlcPVECQADataMakerRec::IlcPVECQADataMakerRec() : 
  IlcQADataMakerRec(IlcQAv1::GetDetName(IlcQAv1::kPVEC), "PVEC Quality Assurance Data Maker")
{
  // ctor
}

//____________________________________________________________________________ 
IlcPVECQADataMakerRec::IlcPVECQADataMakerRec(const IlcPVECQADataMakerRec& qadm) :
  IlcQADataMakerRec()
{
  //copy ctor 
...
IlcPVECQADataMakerRec& IlcPVECQADataMakerRec::operator = (const IlcPVECQADataMakerRec& qadm )
{
  // Equal operator.
  this->~IlcPVECQADataMakerRec();
  new(this) IlcPVECQADataMakerRec(qadm);
  return *this;
}
 
//____________________________________________________________________________ 
void IlcPVECQADataMakerRec::EndOfDetectorCycle(IlcQAv1::TASKINDEX_t task, TObjArray ** list)
{
...
//____________________________________________________________________________ 
void IlcPVECQADataMakerRec::InitESDs()
{
  ...
  TH1F * h1 = new TH1F("hESDPVECSpectrum",  "ESDs spectrum in PVEC; Energy [MeV];Counts"                ,  200, 0.,   20.) ;
  h1->Sumw2() ;
  Add2ESDsList(h1, kESDSpec, !expert, image)  ;

  TH1I * h2 = new TH1I("hESDPVECMul",       "ESDs multiplicity distribution in PVEC; # of clusters;Counts", 100, 0,   100 ) ; 
  h2->Sumw2() ;
  Add2ESDsList(h2, kESDNtot, !expert, image) ;
 
  TH1F * h3 = new TH1F("hESDPVECEtot",      "ESDs total energy;Energy [MeV];Counts"                     , 2000, 0,  200.) ; 
  h3->Sumw2() ;
  Add2ESDsList(h3, kESDEtot, !expert, image) ;  //Expert histo
 
  TH1F * h4 = new TH1F("hESDpid",           "ESDs PID distribution in PVEC;Particle Id;Counts"         , 100, 0.,    1.) ;
  h4->Sumw2() ;
...
//____________________________________________________________________________ 
void IlcPVECQADataMakerRec::InitDigits()
{
...
//____________________________________________________________________________ 
void IlcPVECQADataMakerRec::InitRecPoints()
{
  // create Reconstructed Points histograms in RecPoints subdir
  const Bool_t expert   = kTRUE ; 
  const Bool_t image    = kTRUE ; 
  
  TH2I * h0 = new TH2I("hRpPVECxyMod1","RecPoints Rows x Columns for PVEC module 1;Row #;Column #", 64, -72., 72., 56, -63., 63.) ;                             
  Add2RecPointsList(h0,kRPmod1, expert, !image) ;
  TH2I * h1 = new TH2I("hRpPVECxyMod2","RecPoints Rows x Columns for PVEC module 2;Row #;Column #", 64, -72., 72., 56, -63., 63.) ;                             
  Add2RecPointsList(h1,kRPmod2, expert, !image) ;
  TH2I * h2 = new TH2I("hRpPVECxyMod3","RecPoints Rows x Columns for PVEC module 3;Row #;Column #", 64, -72., 72., 56, -63., 63.) ;                             
  Add2RecPointsList(h2,kRPmod3, expert, !image) ;
  TH2I * h3 = new TH2I("hRpPVECxyMod4","RecPoints Rows x Columns for PVEC module 4;Row #;Column #", 64, -72., 72., 56, -63., 63.) ;                             
  Add2RecPointsList(h3,kRPmod4, expert, !image) ;
  TH2I * h4 = new TH2I("hRpPVECxyMod5","RecPoints Rows x Columns for PVEC module 5;Row #;Column #", 64, -72., 72., 56, -63., 63.) ;                             
  Add2RecPointsList(h4,kRPmod5, expert, !image) ;
 
  TH1F * h5 = new TH1F("hEmcPVECRecPointsSpectrum",  "EMC RecPoints spectrum in PVEC;Energy [MeV];Counts",   2000, 0., 20.) ; 
  h5->Sumw2() ;
  Add2RecPointsList(h5, kRPSpec, !expert, image)  ;

  TH1I * h6 = new TH1I("hEmcPVECRecPointsMul", "EMC RecPoints multiplicity distribution in PVEC;# of EMC Clusters;Entries", 100, 0,  100) ; 
  h6->Sumw2() ;
  Add2RecPointsList(h6, kRPNtot, !expert, image) ;

  TH1I * h7 = new TH1I("hEmcPVECRecPointsEtot", "EMC RecPoints Etot;Energy [MeV];Counts", 200, 0,  200.) ; 
  h7->Sumw2() ;
  Add2RecPointsList(h7, kRPEtot, !expert, image) ;

  TH1I * h8 = new TH1I("hCpvPVECRecPointsMul", "CPV RecPoints multiplicity distribution in PVEC;# of CPV clusters;Counts", 100, 0,  100) ; 
  h8->Sumw2() ;
...
//____________________________________________________________________________ 
void IlcPVECQADataMakerRec::InitRaws()
{
  ...
  TH1F * h12 = new TH1F("hLowPVECRawtime" , "Low Gain Time of raw hits in PVEC" , 500, -50., 200.) ;
  h12->SetXTitle("Time [samples]");
  h12->Sumw2() ;
  Add2RawsList(h12, kLGtime, expert, !image, !saveCorr) ;
  TH1F * h13 = new TH1F("hHighPVECRawtime", "High Gain Time of raw hits in PVEC", 500, -50., 200.) ;
  h13->SetXTitle("Time [samples]");
  h13->Sumw2() ;
  Add2RawsList(h13, kHGtime, expert, !image, !saveCorr) ;

  TH1F * h14 = new TH1F("hLowPVECRawEnergy" , "Low Gain Energy of raw hits in PVEC" , 512, 0., 1024.) ;
  h14->SetXTitle("Energy [ADC counts]");
  h14->Sumw2() ;
  Add2RawsList(h14, kSpecLG, !expert, image, !saveCorr) ;
  TH1F * h15 = new TH1F("hHighPVECRawEnergy", "High Gain Energy of raw hits in PVEC", 512, 0., 1024.) ;
  h15->SetXTitle("Energy [ADC counts]");
  h15->Sumw2() ;
  Add2RawsList(h15, kSpecHG, !expert, image, !saveCorr) ;

  TH1F * h16 = new TH1F("hLowNtot" , "Low Gain Total Number of raw hits in PVEC" , 500, 0., 5000.) ;
  h16->SetXTitle("Number of hits");
  h16->Sumw2() ;
  Add2RawsList(h16, kNtotLG, !expert, image, saveCorr) ;
  TH1F * h17 = new TH1F("hHighNtot", "High Gain Total Number of raw hits in PVEC", 500, 0., 5000.) ;
  ...
  TH1F * h18 = new TH1F("hLowEtot" , "Low Gain Total Energy of raw hits in PVEC" , 500, 0., 100000.) ;
  h18->SetXTitle("Energy [ADC counts]");
  h18->Sumw2() ;
  Add2RawsList(h18, kEtotLG, !expert, image, saveCorr) ;
  TH1F * h19 = new TH1F("hHighEtot", "High Gain Total Energy of raw hits in PVEC", 500, 0., 100000.) ;
...
//____________________________________________________________________________
void IlcPVECQADataMakerRec::MakeESDs(IlcESDEvent * esd)
{
...
//____________________________________________________________________________
void IlcPVECQADataMakerRec::MakeRaws(IlcRawReader* rawReader)
{
  //Fill prepared histograms with Raw digit properties

  rawReader->Reset() ;

  const TObjArray* maps = IlcPVECRecoParam::GetMappings();
  ...
  IlcCaloRawStreamV3 fRawStream(rawReader,"PVEC",mapping);

  IlcPVECRawFitterv0 * fitter ;
  if     (strcmp(GetRecoParam()->EMCFitterVersion(),"v1")==0)
    fitter=new IlcPVECRawFitterv1();
  else if(strcmp(GetRecoParam()->EMCFitterVersion(),"v2")==0)
    fitter=new IlcPVECRawFitterv2();
  else
    fitter=new IlcPVECRawFitterv0();
  Double_t lgEtot=0. ;
  Double_t hgEtot=0. ;
  Int_t    lgNtot=0 ;
  Int_t    hgNtot=0 ;
...
//____________________________________________________________________________
void IlcPVECQADataMakerRec::MakeDigits()
{
...
//____________________________________________________________________________
void IlcPVECQADataMakerRec::MakeDigits(TTree * digitTree)
{
...
//____________________________________________________________________________
void IlcPVECQADataMakerRec::MakeRecPoints(TTree * clustersTree)
{
  {
    ...
    Double_t eTot = 0. ; 
    while ( (rp = static_cast<IlcPVECEmcRecPoint *>(next())) ) {
      FillRecPointsData(kRPSpec, rp->GetEnergy()) ;
      Int_t mod = rp->GetPVECMod() ;
      TVector3 pos ;
...
//____________________________________________________________________________ 
void IlcPVECQADataMakerRec::StartOfDetectorCycle()
{

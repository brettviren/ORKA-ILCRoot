...
/* $Id: IlcPVECvFast.cxx 32955 2009-06-16 12:54:07Z fca $ */
...
#include "IlcPVECvFast.h"
#include "IlcRun.h"

ClassImp(IlcPVECvFast)

IlcPVECvFast::IlcPVECvFast() :
  fBigBoxX(0.),
  fBigBoxY(0.),
  fBigBoxZ(0.),
  fFastRecParticles(0),
...
IlcPVECvFast::IlcPVECvFast(const char *name, const char *title):
  IlcPVEC(name,title),
  fBigBoxX(0.),
  fBigBoxY(0.),
  fBigBoxZ(0.),
  fFastRecParticles(new IlcPVECFastRecParticle::FastRecParticlesList("IlcPVECFastRecParticle", 100)),
  fNRecParticles(0),
  fRan(0),
  fResPara1(0.030), // GeV
  fResPara2(0.00003),
...
//____________________________________________________________________________
void IlcPVECvFast::AddRecParticle(const IlcPVECFastRecParticle & rp)
{  
  // Add a virtually reconstructed particle to the list 

  new( (*fFastRecParticles)[fNRecParticles] ) IlcPVECFastRecParticle(rp) ;
  fNRecParticles++ ; 
}

//____________________________________________________________________________
void IlcPVECvFast::CreateGeometry()
{
  // Create the geometry for GEANT
  
  IlcPVECvFast *PVECtmp = (IlcPVECvFast*)gIlc->GetModule("PVEC") ;
  ...
  gMC->Gsvolu("PVEC", "BOX ", idtmed[798], bigbox, 3) ;
  
  // --- Position  PVEC mdules in ILC setup ---
  ...
  for( Int_t i = 1; i <= GetGeometry()->GetNModules(); i++ ) {
    ...
    Float_t angle = GetGeometry()->GetPVECAngle(i) ;
    ...
    gMC->Gspos("PVEC", i, "ILCM", xP1, yP1, 0.0, idrotm[i-1], "ONLY") ;
...
Float_t IlcPVECvFast::GetBigBox(Int_t index) const
{
  // Get the X, Y or Z dimension of the box describing a PVEC module
...
void IlcPVECvFast::MakeBranch(Option_t* opt)
{  
...
Double_t IlcPVECvFast::MakeEnergy(Double_t energy)
{  
...
TVector3 IlcPVECvFast::MakePosition(Double_t energy, TVector3 pos, Double_t theta, Double_t phi)
{
...
//____________________________________________________________________________
void IlcPVECvFast::MakeRecParticle(Int_t modid, TVector3 pos, IlcPVECFastRecParticle & rp)
{
  // Modify the primary particle properties according
  //  1. the response function of PVEC
  ...
  Double_t incidencephi   = ( 270 + GetGeometry()->GetPVECAngle(modid) ) * TMath::Pi() / 180. - rp.Phi() ;   
...
//____________________________________________________________________________
Int_t IlcPVECvFast::MakeType(IlcPVECFastRecParticle & rp )
{
  // Generate a particle type using the performance of the EMC+PPSD setup

  Int_t rv =   IlcPVECFastRecParticle::kUNDEFINED ;
  ...
  case 22:    // it's a photon              // NB: ALL VALUES SHOLD BE CHECKED !!!!
    ran = fRan.Rndm() ; 
    if( ran <= 0.9498 )
      rv =  IlcPVECFastRecParticle::kNEUTRALHAFAST ; 
    else
      rv =  IlcPVECFastRecParticle::kNEUTRALEMFAST ;     
    break ; 

  case 2112:  // it's a neutron
    ran = fRan.Rndm() ; 
    if ( ran <= 0.9998 )
      rv =  IlcPVECFastRecParticle::kNEUTRALHASLOW ; 
    else 
      rv = IlcPVECFastRecParticle::kNEUTRALEMSLOW ; 
    break ; 
  ...
  case 11:    // it's a electron
    ran = fRan.Rndm() ; 
    if ( ran <= 0.9996 )
      rv =  IlcPVECFastRecParticle::kCHARGEDEMFAST ; 
    else 
      rv =  IlcPVECFastRecParticle::kCHARGEDHAFAST ; 
    break; 
...
//___________________________________________________________________________
void IlcPVECvFast::ResetFastRecParticles()
{
...
//___________________________________________________________________________
void IlcPVECvFast::SetBigBox(Int_t index, Float_t value)
{
  // Set the size of the Box describing a PVEC module
...
//____________________________________________________________________________
Double_t IlcPVECvFast::SigmaE(Double_t energy)
{
...
//____________________________________________________________________________
Double_t IlcPVECvFast::SigmaP(Double_t energy, Double_t incidence)
{
...
//____________________________________________________________________________
void IlcPVECvFast::StepManager(void)
{
  // Only verifies if the particle reaches PVEC and stops the tracking 
  ...
  if(energy > mass){
    ...
    IlcPVECFastRecParticle rp(*part) ;

    // Adds the response of PVEC to the particle
  ...
  // stop the track as soon PVEC is reached

 ...
 **************************************************************************/
/* $Id: IlcPVECDApi0mip.cxx 52964 2011-11-17 12:12:19Z policheh $ */
...
#include <math.h>
#include "IlcLog.h"
#include "IlcPVECDApi0mip.h"
ClassImp(IlcPVECDApi0mip)
//----------------------------------------------------------------
IlcPVECDApi0mip::IlcPVECDApi0mip(int module,int iterid,const char* fopt) :
  ...
  TString shname="IlcPVECDApi0mip_mod%d_iter%d";
  snprintf(hname,shname.Length(),shname.Data(),module,iterid);
  SetName(hname);

  TString shtitle="PVEC MIP/pi0 Calibration DA for Module:%d Iteration:%d";
  ...
  TString sfname="IlcPVECDApi0mip_mod%d.root";
  snprintf(fname,sfname.Length(),sfname.Data(),module);
  fTFile = TFile::Open(fname,fopt);
}
//----------------------------------------------------------------
IlcPVECDApi0mip::IlcPVECDApi0mip(const IlcPVECDApi0mip& da):
  ...
  if( da.fCreateTree ){
    ...
    fTTree->Branch("IlcPVECDATreeEvent","IlcPVECDATreeEvent",&fEvent);
  } else {
    fCreateTree = false;
    fTTree = 0;
  }
...
//-------------------------------------------------------------------
IlcPVECDApi0mip& IlcPVECDApi0mip::operator= (const IlcPVECDApi0mip&)
{
...
//-------------------------------------------------------------------
IlcPVECDApi0mip::~IlcPVECDApi0mip(){
  // Destructor

  //std::cout<<" DEBUGDEBUG :: IlcPVECDApi0mip:: deconstructor "<<std::endl;
  if( fCreateHist ){
...
//-------------------------------------------------------------------
void IlcPVECDApi0mip::FillDigit(float adc,int row,int col){
  // Put new digit information in event
  //

  if( fEvent==0 ) fEvent = new IlcPVECDATreeEvent;
  float energy = adc * 0.005; // 5MeV/ch is fixed.
  if( energy > 0.020 ){  // Threshold at 20MeV.
    //std::cout<<" IlcPVECDApi0mip::AppendDigit() DEBUG:: (row,col)=("<<row<<","<<col<<") adc="<<adc<<" energy="<<energy<<std::endl;
    fEvent->Fill(energy,row,col);
  }
}
//-------------------------------------------------------------------
void IlcPVECDApi0mip::NewEvent(){
...
//----------------------------------------------------------------
bool IlcPVECDApi0mip::CreateTree(){
  if(! fTFile ) {
    return false;
  }
  fTFile->cd();
...
//----------------------------------------------------------------
bool IlcPVECDApi0mip::CreateHist(){
  ...
  if( fH1Time>0 ){
    std::cout<<" IlcPVECDApi0mip:Warning!! Output object already exist : "<<fH1Time->GetName()<<std::endl;
    return false;
  }
  ...
  snprintf(htitle,100,"PVEC pi0 mass vs Digit Id : Mod:%d Iter:%d",fMod,fIterId);
  fH2Pi0DigitId = new TH2F(hname,htitle,17920,0,17920,120,0,0.3);
      
  snprintf(hname,100,"h2_mipdigitid_mod%d_iter%d",fMod,fIterId);
  snprintf(htitle,100,"PVEC MIP vs Digit Id : Mod:%d Iter:%d",fMod,fIterId);
  ...
  snprintf(htitle,100,"PVEC pi0 mass vs pT vs Asym : Mod:%d Iter:%d",fMod,fIterId);
...
//-------------------------------------------------------------------
void IlcPVECDApi0mip::FillTree(IlcPVECDATreeEvent* event){
  // Fill all information (IlcPVECDATreeEvent) into TTree
  // if event==NULL, the obtained information (=fEvent) is used.

  if(! fCreateTree ) CreateTree();
  if( event == 0 ){
    if( fEvent == 0 ){
      fEvent = new IlcPVECDATreeEvent();
    }
...
//-------------------------------------------------------------------
void IlcPVECDApi0mip::FillHist(IlcPVECDATreeEvent* event){
  // Fill all information (IlcPVECDATreeEvent) into histograms
  // if event==NULL, the obtained information (=fEvent) is used.
  ...
  int ndigits;
  int ndigitsall = 0;
  IlcPVECDATreeDigit digit1, digit2;
  ...
  //std::cout<<" IlcPVECDApi0mip::FillHisto() DEBUG:: Number of clusters: "<<nclt1<<std::endl;
  while( nclt1-- ){
    IlcPVECDATreeCluster& clt1 = event->GetCluster(nclt1);
    ndigits = clt1.GetNDigits();
    while( ndigits-- ){
      IlcPVECDATreeDigit& digit = clt1.GetDigit(ndigits);
    ...
    if( clt1.GetEnergy() > 0.2 ){
      nclt2 = nclt1;
      while( nclt2-- ){
	IlcPVECDATreeCluster& clt2 = event->GetCluster(nclt2);
...
//-------------------------------------------------------------------
void IlcPVECDApi0mip::Print(Option_t *option) const
{
  // Print Out

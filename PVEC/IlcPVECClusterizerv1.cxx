...
/* $Id: IlcPVECClusterizerv1.cxx 51445 2011-09-06 18:02:04Z hristov $ */

/* History of cvs commits:
 *
 * $Log: IlcPVECClusterizerv1.cxx,v $
 ...
 * Bug fix for attempt to use IlcPVECEmcRecPoint after its deletion
 *
 ...
 * Mode protection against absence of calib.data from IlcPVECCalibData to IlcPVECClusterizerv1::GetCalibrationParameters()
 *
 * Revision 1.104  2007/04/27 16:55:53  kharlov
 * Calibration stops if PVEC CDB objects do not exist
 *
 ...
 * Update of the PVEC code needed in order to read and reconstruct the beam test raw data (i.e. without an existing gilc.root)
 *
 ...
 * 1. Adding a possibility to read and reconstruct an old rcu formatted raw data. This is controlled by an option of IlcReconstruction and IlcPVECReconstructor. 2. In case of raw data processing (without gilc.root) create the default IlcPVECGeometry object. Most likely this should be moved to the CDB
 *
 ...
 * Add fEnergy to IlcPVECDigit and operate with EMC amplitude in energy units (Yu.Kharlov)
 *
...
//  Results are stored in TreeR#, branches PVECEmcRP (EMC recPoints),
//  PVECCpvRP (CPV RecPoints) and IlcPVECClusterizer (Clusterizer with all 
...
//  standalone mode.
// Use Case:
//  root [0] IlcPVECClusterizerv1 * cl = new IlcPVECClusterizerv1(<pointer_to_PVEC_geometry_onject>)  
...
ClassImp(IlcPVECClusterizerv1)
  
//____________________________________________________________________________
IlcPVECClusterizerv1::IlcPVECClusterizerv1() :
  IlcPVECClusterizer(),
...
//____________________________________________________________________________
IlcPVECClusterizerv1::IlcPVECClusterizerv1(IlcPVECGeometry *geom) :
  IlcPVECClusterizer(geom),
  fDefaultInit(0),            fEmcCrystals(0),          fToUnfold(0),
  fWrite(0),                
  fNumberOfEmcClusters(0),    fNumberOfCpvClusters(0),
  fEmcClusteringThreshold(0), fCpvClusteringThreshold(0), 
...
//____________________________________________________________________________
void IlcPVECClusterizerv1::Digits2Clusters(Option_t *option)
{
  ...
  if(strstr(option,"tim")){
    ...
    IlcInfo(Form("took %f seconds for Clusterizing\n",
		 gBenchmark->GetCpuTime("PVECClusterizer"))); 
  }
...
//____________________________________________________________________________
Bool_t IlcPVECClusterizerv1::FindFit(IlcPVECEmcRecPoint * emcRP, IlcPVECDigit ** maxAt, Float_t * maxAtEnergy,
...
{ 
  ...
  gMinuit->SetFCN(IlcPVECClusterizerv1::UnfoldingChiSquare) ;  
  ...
  for(iDigit = 0; iDigit < nDigits; iDigit++){
    ...
    index++ ;   
    if(ierflg != 0){ 
      Warning("FindFit", "PVEC Unfolding unable to set initial value for fit procedure : x = %f\n", x ) ;
      return kFALSE;
    }
    gMinuit->mnparm(index, "z",  z, 0.1, 0, 0, ierflg) ;
    index++ ;   
    if(ierflg != 0){
       Warning("FindFit", "PVEC Unfolding unable to set initial value for fit procedure : z =%f\n", z ) ;
      return kFALSE;
    }
    gMinuit->mnparm(index, "Energy",  energy , 0.05*energy, 0., 4.*energy, ierflg) ;
    index++ ;   
    if(ierflg != 0){
      Warning("FindFit", "PVEC Unfolding unable to set initial value for fit procedure : energy = %f\n", energy ) ;      
      return kFALSE;
    }
  }
  ...
  if(ierflg == 4){  // Minimum not found   
    Warning("FindFit", "PVEC Unfolding fit not converged, cluster abandoned\n" );      
    return kFALSE ;
  }            
...
//____________________________________________________________________________
void IlcPVECClusterizerv1::Init()
{
  // Make all memory allocations which can not be done in default constructor.
  // Attach the Clusterizer task to the list of PVEC tasks
...
//____________________________________________________________________________
void IlcPVECClusterizerv1::InitParameters()
{

  fNumberOfCpvClusters     = 0 ; 
  fNumberOfEmcClusters     = 0 ; 

  const IlcPVECRecoParam* recoParam = IlcPVECReconstructor::GetRecoParam();
...
//____________________________________________________________________________
Int_t IlcPVECClusterizerv1::AreNeighbours(IlcPVECDigit * d1, IlcPVECDigit * d2)const
{
  ...
  if ( (relid1[0] == relid2[0]) && (relid1[1]==relid2[1]) ) { // inside the same PVEC module 
...
//____________________________________________________________________________
Bool_t IlcPVECClusterizerv1::CheckTimeGate(Float_t t1, Float_t amp1, Float_t t2, Float_t amp2)const{
...
//____________________________________________________________________________
Bool_t IlcPVECClusterizerv1::IsInEmc(IlcPVECDigit * digit) const
{
...
//____________________________________________________________________________
Bool_t IlcPVECClusterizerv1::IsInCpv(IlcPVECDigit * digit) const
{
...
//____________________________________________________________________________
void IlcPVECClusterizerv1::WriteRecPoints()
{
  ...
  Float_t emcMinE= IlcPVECReconstructor::GetRecoParam()->GetEMCMinE(); //Minimal digit energy
  TVector3 fakeVtx(0.,0.,0.) ;
  for(index = 0; index < nEmc; index++){
    IlcPVECEmcRecPoint * rp =
      static_cast<IlcPVECEmcRecPoint *>( fEMCRecPoints->At(index) );
    rp->Purify(emcMinE) ;
  ...
  for(index = 0; index < fEMCRecPoints->GetEntries(); index++){
    static_cast<IlcPVECEmcRecPoint *>( fEMCRecPoints->At(index) )->SetIndexInList(index) ;
  }
  ...
  for(index = 0; index < fCPVRecPoints->GetEntries(); index++){
    IlcPVECCpvRecPoint * rp = static_cast<IlcPVECCpvRecPoint *>( fCPVRecPoints->At(index) );
    rp->EvalAll(fDigitsArr) ;
  ...
  for(index = 0; index < fCPVRecPoints->GetEntries(); index++)
    static_cast<IlcPVECCpvRecPoint *>( fCPVRecPoints->At(index) )->SetIndexInList(index) ;
...
//____________________________________________________________________________
void IlcPVECClusterizerv1::MakeClusters()
{
  ...
  const Int_t maxNDigits = 3584; // There is no clusters larger than PVEC module ;)
  ...
  IlcPVECRecPoint * clu = 0 ; 
  for(Int_t i=0; i<nDigits; i++){
    if(fDigitsUsed[i])
      continue ;

    digit=static_cast<IlcPVECDigit*>(fDigitsArr->At(i)) ;
    ...
    if (( IsInEmc(digit) &&  Calibrate(digit->GetEnergy(),digit->GetId()) > fEmcClusteringThreshold ) || 
      ...
      if  ( IsInEmc(digit) ) {   
        ...
        fEMCRecPoints->AddAt(new  IlcPVECEmcRecPoint(""), fNumberOfEmcClusters) ;
        clu = static_cast<IlcPVECEmcRecPoint *>( fEMCRecPoints->At(fNumberOfEmcClusters) ) ; 
	...
	clu->AddDigit(*digit, Calibrate(digit->GetEnergy(),digit->GetId()),CalibrateT(digit->GetTime(),digit->GetId())) ;
        ...
        fCPVRecPoints->AddAt(new IlcPVECCpvRecPoint(""), fNumberOfCpvClusters) ;
        clu =  static_cast<IlcPVECCpvRecPoint *>( fCPVRecPoints->At(fNumberOfCpvClusters) ) ;  
      ...
      while (index < iDigitInCluster){ // scan over digits already in cluster 
        digit =  static_cast<IlcPVECDigit*>( fDigitsArr->At(clusterdigitslist[index]) )  ;      
        index++ ; 
	  ...
	  if (iDigitInCluster >= maxNDigits) {
	    ...
	    return;
          ...
          digitN = static_cast<IlcPVECDigit*>( fDigitsArr->At(j) ) ;
...
//____________________________________________________________________________
void IlcPVECClusterizerv1::MakeUnfolding()
{
  ...
  if(fNumberOfEmcClusters > 0){
    ...
    for(index = 0 ; index < numberofNotUnfolded ; index++){
      ...
      IlcPVECEmcRecPoint * emcRecPoint = static_cast<IlcPVECEmcRecPoint *>( fEMCRecPoints->At(index) ) ;
      if(emcRecPoint->GetPVECMod()> nModulesToUnfold)
        break ;
      
      Int_t nMultipl = emcRecPoint->GetMultiplicity() ; 
      IlcPVECDigit ** maxAt = new IlcPVECDigit*[nMultipl] ;
  ...
  if(fNumberOfCpvClusters > 0){
    ...
    for(index = 0 ; index < numberofCpvNotUnfolded ; index++){
      ...
      IlcPVECRecPoint * recPoint = static_cast<IlcPVECRecPoint *>( fCPVRecPoints->At(index) ) ;

      if(recPoint->GetPVECMod()> nModulesToUnfold)
        break ;
      
      IlcPVECEmcRecPoint * emcRecPoint = static_cast<IlcPVECEmcRecPoint*>(recPoint) ; 
...
//____________________________________________________________________________
Double_t  IlcPVECClusterizerv1::ShowerShape(Double_t x, Double_t z)
{ 
  // Shape of the shower (see PVEC TDR)
...
//____________________________________________________________________________
void  IlcPVECClusterizerv1::UnfoldCluster(IlcPVECEmcRecPoint * iniEmc, 
                                          Int_t nMax, 
                                          IlcPVECDigit ** maxAt, 
                                          Float_t * maxAtEnergy)
{ 
  ...
  Int_t iDigit ;
  for(iDigit = 0 ; iDigit < nDigits ; iDigit ++){
    digit = static_cast<IlcPVECDigit*>( fDigitsArr->At(emcDigits[iDigit] ) ) ;   
    fGeom->AbsToRelNumbering(digit->GetId(), relid) ;
...
//      efit[iDigit] += epar * ShowerShape(xDigit - xpar,zDigit - zpar,vIncid) ;
  ...
  while(iparam < nPar ){
    ...
    epar = fitparameters[iparam+2] ;
    iparam += 3 ;    
//    fGeom->GetIncidentVector(fVtx,iniEmc->GetPVECMod(),xpar,zpar,vIncid) ;

    IlcPVECEmcRecPoint * emcRP = 0 ;  
    ...
    if(iniEmc->IsEmc()){ //create new entries in fEMCRecPoints...
      ...
      (*fEMCRecPoints)[fNumberOfEmcClusters] = new IlcPVECEmcRecPoint("") ;
      emcRP = static_cast<IlcPVECEmcRecPoint *>( fEMCRecPoints->At(fNumberOfEmcClusters) ) ;
      fNumberOfEmcClusters++ ;
      ...
      (*fCPVRecPoints)[fNumberOfCpvClusters] = new IlcPVECCpvRecPoint("") ;
      emcRP = static_cast<IlcPVECEmcRecPoint *>( fCPVRecPoints->At(fNumberOfCpvClusters) ) ;
      fNumberOfCpvClusters++ ;
    }
...
void IlcPVECClusterizerv1::UnfoldingChiSquare(Int_t & nPar, Double_t * Grad, Double_t & fret, Double_t * x, Int_t iflag)
{
  ...
  IlcPVECEmcRecPoint * emcRP = static_cast<IlcPVECEmcRecPoint*>( toMinuit->At(0) )  ;
  TClonesArray * digits = static_cast<TClonesArray*>( toMinuit->At(1) )  ;
  // A bit buggy way to get an access to the geometry
  // To be revised!
  IlcPVECGeometry *geom = static_cast<IlcPVECGeometry *>(toMinuit->At(2));

//  TVector3 * vtx = static_cast<TVector3*>(toMinuit->At(3)) ;  //Vertex position
  
  //  IlcPVECEmcRecPoint * emcRP = static_cast<IlcPVECEmcRecPoint *>( gMinuit->GetObjectFit() ) ; // EmcRecPoint to fit
...
//  TVector3 vInc ;
  ...
  for( iDigit = 0 ; iDigit < nOdigits ; iDigit++) {
    ...
    digit = static_cast<IlcPVECDigit*>( digits->At( emcDigits[iDigit] ) ); 
    ...
    geom->RelPosInModule(relid, xDigit, zDigit) ;
     ...
     if(iflag == 2){  // calculate gradient
       ...
       while(iParam < nPar ){
         ...
         iParam++ ;          
//         fGeom->GetIncidentVector(*vtx,emcRP->GetPVECMod(),x[iParam-2],x[iParam-1],vInc) ;
...
//         fGeom->GetIncidentVector(*vtx,emcRP->GetPVECMod(),xpar,zpar,vInc) ;
...
//____________________________________________________________________________
void IlcPVECClusterizerv1::Print(const Option_t *)const
{
  ...
  if( strcmp(GetName(), "") !=0 ) {  
    ...
    message = " IlcPVECClusterizerv1 not initialized " ;
...
//____________________________________________________________________________
void IlcPVECClusterizerv1::PrintRecPoints(Option_t * option)
{
  // Prints list of RecPoints produced at the current pass of IlcPVECClusterizer
  ...
  if(strstr(option,"all")) {
    ...
    Int_t index ;
    for (index = 0 ; index < fEMCRecPoints->GetEntries() ; index++) {
      IlcPVECEmcRecPoint * rp = (IlcPVECEmcRecPoint * )fEMCRecPoints->At(index) ; 
      TVector3  locpos;  
      ...
      printf("\n%6d  %8.2f  %3d     %2d     %4.1f   %4.1f   %4.1f   %4f  %4f    %2d     : ", 
	      rp->GetIndexInList(), rp->GetEnergy(), rp->GetMultiplicity(), rp->GetPVECMod(), 
    ...
    for (index = 0 ; index < fCPVRecPoints->GetEntries() ; index++) {
      IlcPVECCpvRecPoint * rp = (IlcPVECCpvRecPoint * )fCPVRecPoints->At(index) ; 
      
      TVector3  locpos;  
      rp->GetLocalPosition(locpos);
      
      printf("\n%6d  %8.2f  %2d     %4.1f    %4.1f %4.1f \n", 
	     rp->GetIndexInList(), rp->GetEnergy(), rp->GetPVECMod(), 
...
//____________________________________________________________________________
void IlcPVECClusterizerv1::SetDistancesToBadChannels()
{
  ...
  for(Int_t iRP=0; iRP<fEMCRecPoints->GetEntries(); iRP++){
    rp = (IlcPVECEmcRecPoint*)fEMCRecPoints->At(iRP);
    //evaluate distance to border
    relid[0]=rp->GetPVECMod() ;
    relid[2]=1 ;
    relid[3]=1 ;
    ...
    for(Int_t iBad=0; iBad<fgCalibData->GetNumOfEmcBadChannels(); iBad++) {
      ...
      if(relid[0]!=rp->GetPVECMod()) //We can not evaluate global position directly since 
...
//==================================================================================
Float_t IlcPVECClusterizerv1::Calibrate(Float_t amp, Int_t absId) const{
...
//==================================================================================
Float_t IlcPVECClusterizerv1::CalibrateT(Float_t time, Int_t absId)const{

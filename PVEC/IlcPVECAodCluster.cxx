...
//  IlcAODCaloCluster extension for PVEC to recalculate cluster 
...
#include "IlcPVECAodCluster.h" 
#include "IlcPVECCalibData.h"
#include "IlcAODCaloCells.h"

ClassImp(IlcPVECAodCluster)

//____________________________________________________________________________
IlcPVECAodCluster::IlcPVECAodCluster() : 
  IlcAODCaloCluster(),fRecalibrated(0)
{
  // ctor
}
//____________________________________________________________________________
IlcPVECAodCluster::IlcPVECAodCluster(const IlcAODCaloCluster & clu) : 
...
//____________________________________________________________________________
void IlcPVECAodCluster::Recalibrate(IlcPVECCalibData * calibData,IlcAODCaloCells *phsCells){
  ...
  if(!PVECgeom)
    IlcFatal("IlcPVECGeometry was not contructed\n") ;
	
  Double32_t * cellsAmpFraction = GetCellsAmplitudeFraction(); 
	
  for(Int_t i=0; i < GetNCells(); i++){
    Int_t relId[4];
    PVECgeom->AbsToRelNumbering(GetCellAbsId(i),relId) ;
...
//____________________________________________________________________________
void  IlcPVECAodCluster::EvalAll(Float_t logWeight, TVector3 &vtx){
...
//____________________________________________________________________________
void IlcPVECAodCluster::EvalEnergy(){
...
//____________________________________________________________________________
void IlcPVECAodCluster::EnergyCorrection(){
  //apply nonlinearity correction same as in IlcPVECPIDv1.
  SetE(IlcPVECReconstructor::CorrectNonlinearity(E())) ;
}
//____________________________________________________________________________
void IlcPVECAodCluster::EvalPID(IlcPVECPIDv1 * /*pid*/){           
...
//____________________________________________________________________________
void IlcPVECAodCluster::EvalCoord(Float_t logWeight, TVector3 &vtx)
{
  // Calculates new center of gravity in the local PVEC-module coordinates 
  ...
  for(Int_t iDigit=0; iDigit < GetNCells(); iDigit++) {
    Float_t xi ;
    Float_t zi ;
    PVECgeom->AbsToRelNumbering(GetCellAbsId(iDigit), relid) ;
...
// Calculates the dispersion and second momenta
  ...
  TVector3 vInc ;
  PVECgeom->GetIncidentVector(vtx,PVECMod,xMean,zMean,vInc) ;
  ...
  TVector3 gps ;
  PVECgeom->Local2Global(PVECMod, xMean, zMean, gps) ;

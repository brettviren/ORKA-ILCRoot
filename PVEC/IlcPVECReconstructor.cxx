...
/* $Id: IlcPVECReconstructor.cxx 54863 2012-02-28 12:10:29Z hristov $ */
...
#include "IlcPVECTriggerRawDigit.h"
#include "IlcPVECTriggerRawDigiProducer.h"
#include "IlcPVECTriggerParameters.h"

ClassImp(IlcPVECReconstructor)

Bool_t IlcPVECReconstructor::fgDebug = kFALSE ; 
TClonesArray*     IlcPVECReconstructor::fgDigitsArray = 0;   // Array of PVEC digits
TObjArray*        IlcPVECReconstructor::fgEMCRecPoints = 0;   // Array of EMC rec.points
IlcPVECCalibData * IlcPVECReconstructor::fgCalibData  = 0 ;
TClonesArray*     IlcPVECReconstructor::fgTriggerDigits = 0;   // Array of PVEC trigger digits

//____________________________________________________________________________
IlcPVECReconstructor::IlcPVECReconstructor() :
  fGeom(NULL),fClusterizer(NULL),fTSM(NULL),fPID(NULL),fTmpDigLG(NULL)
{
  // ctor
  fGeom          = IlcPVECGeometry::GetInstance("ORKA","");
  fClusterizer   = new IlcPVECClusterizerv1      (fGeom);
  fTSM           = new IlcPVECTrackSegmentMakerv1(fGeom);
  fPID           = new IlcPVECPIDv1              (fGeom);
  fTmpDigLG      = new TClonesArray("IlcPVECDigit",100);
  fgDigitsArray  = new TClonesArray("IlcPVECDigit",100);
  fgEMCRecPoints = new TObjArray(100) ;
  if (!fgCalibData)
    fgCalibData = new IlcPVECCalibData(-1); //use IlcCDBManager's run number
  
  fgTriggerDigits  = new TClonesArray("IlcPVECTriggerRawDigit",100);
  
  IlcInfo(Form("PVEC bad channel map contains %d bad channel(s).\n",
...
//____________________________________________________________________________
void IlcPVECReconstructor::Reconstruct(TTree* digitsTree, TTree* clustersTree) const
{
...
//____________________________________________________________________________
void IlcPVECReconstructor::FillESD(TTree* digitsTree, TTree* clustersTree, 
				   IlcESDEvent* esd) const
{
  // This method produces PVEC rec-particles,
  ...
  TBranch *tbranch = digitsTree->GetBranch("TPVEC");
  if (tbranch) { 
    
    tbranch->SetAddress(&fgTriggerDigits);
    tbranch->GetEntry(0);
    
    IlcESDCaloTrigger* trgESD = esd->GetCaloTrigger("PVEC");
    
    if (trgESD) {
      trgESD->Allocate(fgTriggerDigits->GetEntriesFast());
      
      for (Int_t i = 0; i < fgTriggerDigits->GetEntriesFast(); i++) {
	IlcPVECTriggerRawDigit* tdig = (IlcPVECTriggerRawDigit*)fgTriggerDigits->At(i);
...
//   // Attention! PVEC modules in order to calculate AbsId need to be 1-5 not 0-4 as returns trigger.
...
//   //esd->SetPVECTriggerCells(triggerPosition);
//   esd->AddPVECTriggerPosition(triggerPosition);
//   esd->AddPVECTriggerAmplitudes(triggerAmplitudes);
  ...
  for (Int_t idig = 0 ; idig < nDigits ; idig++) {
    const IlcPVECDigit * dig = (const IlcPVECDigit*)fgDigitsArray->At(idig);
    if(dig->GetId() <= knEMC && 
  ...
  for (Int_t recpart = 0 ; recpart < nOfRecParticles ; recpart++) {
    IlcPVECRecParticle  *rp    = static_cast<IlcPVECRecParticle*>(recParticles->At(recpart));
    if (Debug()) 
      rp->Print();
    // Get track segment and EMC rec.point associated with this rec.particle
    IlcPVECTrackSegment *ts    = static_cast<IlcPVECTrackSegment *>(fTSM->GetTrackSegments()
								    ->At(rp->GetPVECTSIndex()));

    IlcPVECEmcRecPoint  *emcRP = static_cast<IlcPVECEmcRecPoint *>(fgEMCRecPoints->At(ts->GetEmcIndex()));
    ...
    for (Int_t iCell=0; iCell<cellMult; iCell++) {
      IlcPVECDigit *digit = static_cast<IlcPVECDigit *>(fgDigitsArray->At(digitsList[iCell]));
...
//      break ;
  ...
  //Store PVEC misalignment matrixes
  FillMisalMatrixes(esd) ;

}

//____________________________________________________________________________
IlcTracker* IlcPVECReconstructor::CreateTracker() const
{
  // creates the PVEC tracker
  return new IlcPVECTracker();
}

//____________________________________________________________________________
void  IlcPVECReconstructor::ConvertDigits(IlcRawReader* rawReader, TTree* digitsTree) const
{
  ...
  if      (strcmp(GetRecoParam()->EMCFitterVersion(),"v0")==0) 
    ...
    fitter=new IlcPVECRawFitterv3();
  else
    fitter=new IlcPVECRawFitterv4();
  ...
  Int_t bufsize = 32000;
  digitsTree->Branch("PVEC", &digits, bufsize);

  IlcPVECRawDigiProducer rdp(rawReader,mapping);
  ...
  TClonesArray *tdigits = new TClonesArray("IlcPVECTriggerRawDigit",1);
  tdigits->SetName("TDIGITS");
  digitsTree->Branch("TPVEC", &tdigits, bufsize);  

  rawReader->Reset();
  IlcPVECTriggerRawDigiProducer tdp(rawReader);
  
  IlcPVECTriggerParameters* parameters = (IlcPVECTriggerParameters*)IlcPVECRecoParam::GetTriggerParameters();
  ...
  if (IlcLog::GetGlobalDebugLevel() == 1) {
    ...
    for(Int_t iDigit=0; iDigit<digits->GetEntries(); iDigit++) {
      IlcPVECDigit* digit = (IlcPVECDigit*)digits->At(iDigit);
...
//==================================================================================
Float_t IlcPVECReconstructor::Calibrate(Float_t amp, Int_t absId)const{
...
//==================================================================================
Float_t IlcPVECReconstructor::CalibrateT(Float_t time, Int_t absId)const{
  // Calibrate EMC digit, i.e. multiply its Amp by a factor read from CDB
...
//==================================================================================
void IlcPVECReconstructor::FillMisalMatrixes(IlcESDEvent* esd)const{
  //Store PVEC matrixes in ESD Header
  ...
  TGeoHMatrix * m ;
  for(Int_t mod=0; mod<5; mod++){
    snprintf(path,255,"/ILCM_1/PVEC_%d",mod+1) ; //In Geometry modules numbered 1,2,.,5
...
//==================================================================================
Float_t IlcPVECReconstructor::CorrectNonlinearity(Float_t en){
...
void IlcPVECReconstructor::readTRUParameters(IlcPVECTriggerParameters* parameters) const
{
  //Read trigger parameters.

  TString path(gSystem->Getenv("ILC_ROOT"));
  path += "/PVEC/macros/Trigger/OCDB/";

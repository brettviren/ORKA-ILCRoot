 ...
 **************************************************************************/
/* $Id: IlcPVECDATreeEvent.cxx 53738 2011-12-20 16:49:27Z kharlov $ */
...
ClassImp(IlcPVECDATreeEvent)
//------------------------------------------------------------------------
IlcPVECDATreeEvent::IlcPVECDATreeEvent(const IlcPVECDATreeEvent& evt)
: fTime(0),
  fNDigits(0),
  fDigits(0),
  fNClusters(0),
  fClusters(0)
{
  ...
  if( fNClusters > 0 ){
    fClusters = new IlcPVECDATreeCluster[fNClusters];
...
//------------------------------------------------------------------------
IlcPVECDATreeEvent& IlcPVECDATreeEvent::operator=(const IlcPVECDATreeEvent& evt){
  // Copy Operator
...
//------------------------------------------------------------------------
bool IlcPVECDATreeEvent::Fill(float energy,int row,int col){
  // Fill new digit information

  IlcPVECDATreeDigit digit(energy,row,col);
  return Fill(digit);
}
//------------------------------------------------------------------------
bool IlcPVECDATreeEvent::Fill(IlcPVECDATreeDigit& digit){
  // Fill new digit information

  IlcPVECDATreeDigit* newDigits = new IlcPVECDATreeDigit[fNDigits+1];
  int ndigits = fNDigits;
...
//------------------------------------------------------------------------
bool IlcPVECDATreeEvent::Clusterize(IlcPVECDATreeDigit& digit){
  ...
  if( !status ){
    IlcPVECDATreeCluster* newClusters = new IlcPVECDATreeCluster[fNClusters+1];
...
//------------------------------------------------------------------------
bool IlcPVECDATreeEvent::ExecuteClustering(){
  ...
  while( nclusters0-- ){
    IlcPVECDATreeCluster& cluster0 = fClusters[nclusters0];
    int nclusters1 = nclusters0;
    while( nclusters1-- ){
      IlcPVECDATreeCluster& cluster1 = fClusters[nclusters1];
  ...
  if( nclustersnonzero > 0 ){
    IlcPVECDATreeCluster* newClusters = new IlcPVECDATreeCluster[nclustersnonzero];
    nclusters = fNClusters;
...
//------------------------------------------------------------------------
void IlcPVECDATreeEvent::Print(Option_t *opt) const
{
  // Print Out
  
  char* when = ctime(&fTime);
  std::cout<<" IlcPVECDATreeEvent:: fNDigits = "<<fNDigits<<" fNClusters="<<fNClusters<<" : "<<when;
  int ndigits = fNDigits;
  while( ndigits-- ){
    std::cout<<" =>["<<ndigits<<"] : ";
...
//------------------------------------------------------------------------
std::ostream& operator<<(std::ostream& out, const IlcPVECDATreeEvent& event){
  // Print Out

  char* when = ctime(&(event.fTime));
  out<<" IlcPVECDATreeEvent:: fNClusters="<<event.fNClusters<<" : "<<when;

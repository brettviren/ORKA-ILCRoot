#include "IlcPVECRcuDA1.h"
#include "TString.h"

ClassImp(IlcPVECRcuDA1)

//----------------------------------------------------------------
IlcPVECRcuDA1::IlcPVECRcuDA1(Int_t module, Int_t rcu) : TNamed(),
...

{
  ...
  // Root file name: PVEC_ModuleX_RCUY_Calib.root, where X - module number,
  // Y - RCU number. If no RCU specified (rcu<0), file name is simply
  // PVEC_ModuleX_Calib.root.
  
  char name[128]; TString sname;

  if(rcu<0) { 
    sname="PVEC_Module%d_Calib";
    snprintf(name,sname.Length(),sname.Data(),fMod);
  }
  else {
    sname="PVEC_Module%d_RCU%d_Calib";
...
//-------------------------------------------------------------------
IlcPVECRcuDA1::IlcPVECRcuDA1(Int_t module, Int_t rcu, TObjArray* oldTimeEnergy) :
...
//-------------------------------------------------------------------
void IlcPVECRcuDA1::FillHistograms(Float_t e[64][56][2], Float_t t[64][56][2]) 
{
...
//-------------------------------------------------------------------
void IlcPVECRcuDA1::UpdateHistoFile()
{
...
//-----------------------------------------------------------------------
void IlcPVECRcuDA1::SetWriteToFile(Bool_t write)
{
  if(!write) { 
    fWriteToFile = write;
    return;

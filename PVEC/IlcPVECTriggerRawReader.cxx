 ...
 *   please read documentation for IlcPVECTriggerRawReader::ReadFromStream .
 */
...
ClassImp(IlcPVECTriggerRawReader)


//________________________________________________________________
IlcPVECTriggerRawReader::IlcPVECTriggerRawReader()
  : TObject(),
    fTRUs()
{
  // default constructor.
...
//________________________________________________________________
IlcPVECTRURawReader* IlcPVECTriggerRawReader::GetTRU(Int_t mod, Int_t truRow, Int_t branch)
{
  // Get TRU Raw Reader.
  ...
  if( ! fTRUs[mod][truRow][branch] )
    fTRUs[mod][truRow][branch] = new IlcPVECTRURawReader();
  return fTRUs[mod][truRow][branch];
}
 

//________________________________________________________________
void IlcPVECTriggerRawReader::ReadFromStream(IlcCaloRawStreamV3* rawStream)
{
  // Give a IlcCaloRawStreamV3* to an instance of this class.
  // It will read from the stream. The stream is passed to 'IlcPVECTRURawReader's
  // which are accesible through 'IlcPVECTriggerRawReader::GetTRU'.
  ...
  while (rawStream->NextBunch()) {
    ...
    IlcPVECTRURawReader* reader = GetTRU(module, rcuRow, branch);
    if (reader) reader->ReadFromStream(rawStream);
  } // end while 
}

//________________________________________________________________
void IlcPVECTriggerRawReader::Reset()
{
  // Reset

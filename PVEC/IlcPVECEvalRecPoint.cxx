...
/* $Id: IlcPVECEvalRecPoint.cxx 47119 2011-02-04 15:35:48Z policheh $ */
...
ClassImp(IlcPVECEvalRecPoint)

IlcPVECEvalRecPoint::IlcPVECEvalRecPoint() :
  fIsEmc(kFALSE),
  fIsCpv(kTRUE),
  fParent(-333),
  fChi2Dof(-111),
...
IlcPVECEvalRecPoint::IlcPVECEvalRecPoint(Bool_t cpv, IlcPVECEvalRecPoint* parent) : 
  fIsEmc(kFALSE),
  fIsCpv(kFALSE),
  fParent(-333),
  fChi2Dof(-111),
...
IlcPVECEvalRecPoint::IlcPVECEvalRecPoint(Int_t i, Bool_t cpv) : 
  fIsEmc(kFALSE),
  fIsCpv(kFALSE),
  fParent(-333),
  fChi2Dof(-111),
...
{
  // ctor
  IlcPVECEmcRecPoint* rp=0;

  IlcPVECLoader* fLoader = IlcPVECLoader::GetPVECLoader(fEventFolderName);

  if(cpv) {
    rp = (IlcPVECCpvRecPoint*)fLoader->CpvRecPoints()->At(i);
    fIsEmc = kFALSE;
    fIsCpv = kTRUE;
  }
  else {
    rp = (IlcPVECEmcRecPoint*)fLoader->EmcRecPoints()->At(i);
    fIsEmc = kTRUE;
    fIsCpv = kFALSE;
  }
  ...
  for(Int_t iDigit=0; iDigit<nDigits; iDigit++) {
    IlcPVECDigit* digit = (IlcPVECDigit*)fLoader->Digits()->At( digits[iDigit] );
...
IlcPVECClusterizer* IlcPVECEvalRecPoint::GetClusterizer()
{
  // returns clusterizer task
  TFolder* ilcF = (TFolder*)gROOT->FindObjectAny("YSIlc");
  TFolder* wPoolF = (TFolder*)ilcF->FindObject("WhiteBoard/RecPoints/PVEC/SmP");
  IlcPVECClusterizer* clu = (IlcPVECClusterizer*)wPoolF->FindObject("PVEC:clu-v1");
  if(!clu) { 
...
Bool_t IlcPVECEvalRecPoint::TooClose(IlcPVECRecPoint* pt) const 
{
...
Bool_t IlcPVECEvalRecPoint::NeedToSplit() const 
{
  // rec.point needs to be split
  return kFALSE;
}

void IlcPVECEvalRecPoint::DeleteParent()
{
  // delete parent
  fParent=-333;
}

void IlcPVECEvalRecPoint::UpdateWorkingPool()
{
  // update pool of rec.points
  Int_t i; //loop variable
  
  for(i=0; i<this->InWorkingPool(); i++) {
     IlcPVECEvalRecPoint* parent = (IlcPVECEvalRecPoint*)GetFromWorkingPool(i);
     TObjArray children;
     Int_t nChild = parent->HasChild(children);
     for(Int_t iChild=0; iChild<nChild; iChild++)
       {
	 ((IlcPVECEvalRecPoint*)children.At(iChild))->DeleteParent();
       }
  ...
  for(i=0; i<this->InWorkingPool(); i++) {
    IlcPVECEvalRecPoint* weak = (IlcPVECEvalRecPoint*)GetFromWorkingPool(i);
    if (weak->KillWeakPoint()) delete weak;
  }

  for(i=0; i<this->InWorkingPool(); i++) {
    IlcPVECEvalRecPoint* close = (IlcPVECEvalRecPoint*)GetFromWorkingPool(i);
    close->MergeClosePoint();
  }

  for(i=0; i<this->InWorkingPool(); i++)
    ((IlcPVECEvalRecPoint*)IlcPVECEvalRecPoint::GetFromWorkingPool(i))->SetIndexInList(i);

}

Int_t IlcPVECEvalRecPoint::HasChild(TObjArray& children)
{
  // returns the number of children
  for(Int_t iChild=0; iChild<InWorkingPool(); iChild++)
    {
      IlcPVECEvalRecPoint* child = (IlcPVECEvalRecPoint*)GetFromWorkingPool(iChild);
...
void IlcPVECEvalRecPoint::Init()
{
  // initialization
  IlcPVECClusterizer* clusterizer = GetClusterizer();
  if(!clusterizer) {
    IlcFatal("Cannot get clusterizer") ;
  }

  TClonesArray* digits = IlcPVECLoader::GetPVECLoader(fEventFolderName)->Digits();
  Float_t logWeight=0;  
...
void IlcPVECEvalRecPoint::MakeJob()
{
  // Reconstruction algoritm implementation.

  IlcPVECRecManager* recMng = GetReconstructionManager();
  ...
  for(Int_t iChild=0; iChild<nChild; iChild++) {
    IlcPVECEvalRecPoint* child = (IlcPVECEvalRecPoint*)children.At(iChild);
...
void IlcPVECEvalRecPoint::InitTwoGam(Float_t* gamma1, Float_t* gamma2)
{
...
//    yy = XY(yy/e);
  ...
  for(iDigit = 0 ; iDigit < nDigits ; iDigit ++)
    {
      digit = (IlcPVECDigit*)fLoader->Digits()->At( digits[iDigit] ); 
      eDigit = energies[iDigit];
      PVECgeom->AbsToRelNumbering(digit->GetId(), relid) ;
      PVECgeom->RelPosInModule(relid, iy, ix);
...
void IlcPVECEvalRecPoint::TwoGam(Float_t* gamma1, Float_t* gamma2)
{
...
void IlcPVECEvalRecPoint::UnfoldTwoMergedPoints(Float_t* gamma1, Float_t* gamma2)
{
  ...
  for( Int_t iMax=0; iMax<nMax; iMax++)
	...
	gamma = gamma2;
      ...
      IlcPVECEvalRecPoint* newRP = new IlcPVECEvalRecPoint(IsCPV(),this);
      ...
      for( Int_t iDigit = 0 ; iDigit < nDigits ; iDigit ++)
	{
	  ...
	  PVECgeom->AbsToRelNumbering(digit->GetId(), relid) ;
	  Float_t ix,iz;
	  PVECgeom->RelPosInModule(relid, ix, iz);
...
void IlcPVECEvalRecPoint::EvaluatePosition() 
{
...
Bool_t IlcPVECEvalRecPoint::KillWeakPoint()
{
...
void IlcPVECEvalRecPoint::SplitMergedShowers() 
{
...
void IlcPVECEvalRecPoint::MergeClosePoint() 
{
  // merge rec.points if they are too close
  IlcPVECLoader* fLoader = IlcPVECLoader::GetPVECLoader(fEventFolderName);
//    IlcPVECDigitizer* digitizer = fLoader->Digitizer();
...
//    Float_t fSlope = digitizer->GetSlope();
  ...
  for (Int_t i=0;i<InWorkingPool(); i++)
	...
	{
	  IlcPVECRecPoint* rp = (IlcPVECRecPoint*)obj;
	  if(GetPVECMod() == rp->GetPVECMod())
	    {
	      if(TooClose(rp))
		{
		  ...
		  Float_t* energies = ((IlcPVECEmcRecPoint*)rp)->GetEnergiesList();
...
Int_t IlcPVECEvalRecPoint::UnfoldLocalMaxima() 
{
  ...
  for(iDigit=0; iDigit<nDigits; iDigit++)
    {
      ...
      IlcPVECDigit* digit = (IlcPVECDigit*)fLoader->Digits()->At( digitsList[iDigit] );
      PVECgeom->AbsToRelNumbering(digit->GetId(), relid) ;
      Float_t x,z;
      PVECgeom->RelPosInModule(relid, x, z);
      ...
      for(Int_t iMax=0; iMax<nMax; iMax++)
	{
	  IlcPVECDigit* digitMax = maxAt[iMax];
	  Float_t eMax = maxAtEnergy[iMax]; 
	  PVECgeom->AbsToRelNumbering(digitMax->GetId(), relid) ;
	  PVECgeom->RelPosInModule(relid, xMax, zMax);
	  Float_t dx = xMax - x;
	  Float_t dz = zMax - z;
	  Float_t amp,gx,gy;
	  GetReconstructionManager()->AG(eMax,dz,dx,amp,gx,gy); 
...
void IlcPVECEvalRecPoint::PrintPoint()
{
  // print rec.point to stdout
  IlcPVECCpvRecPoint::Print();
  ...
  IlcInfo(Form(message.Data(), Chi2Dof(), lpos.X(), lpos.Z(), GetPVECMod())) ;

}

IlcPVECRecManager* IlcPVECEvalRecPoint::GetReconstructionManager() const
{
  // returns reconstruction manager
  TFolder* ilcF = (TFolder*)gROOT->FindObjectAny("YSIlc");
  TFolder* wPoolF = (TFolder*)ilcF->FindObject("WhiteBoard/RecPoints/PVEC/SmP");
  IlcPVECRecManager* recMng = (IlcPVECRecManager*)wPoolF->FindObject("IlcPVECRecManager");
  if(!recMng) { 
...
IlcPVECRecPoint* IlcPVECEvalRecPoint::Parent()
{
  // returns a parent
  if(fParent<0) return NULL;
  else
    return (IlcPVECRecPoint*)GetFromWorkingPool(fParent);
//    return fParent;
}

Float_t IlcPVECEvalRecPoint::Chi2Dof() const 
{
  // returns a chi^2 per degree of freedom
  return fChi2Dof;
}

const TObject* IlcPVECEvalRecPoint::GetWorkingPool()
{
...
void IlcPVECEvalRecPoint::AddToWorkingPool(TObject* obj)
{
  // add a rec.point to a pool
  ((TObjArray*)GetWorkingPool())->Add(obj);
}

TObject* IlcPVECEvalRecPoint::GetFromWorkingPool(Int_t i)
{
...
Int_t IlcPVECEvalRecPoint::InWorkingPool()
{
  // return the number of rec.points in a pool
  return ((TObjArray*)GetWorkingPool())->GetEntries();
}

void IlcPVECEvalRecPoint::RemoveFromWorkingPool(TObject* obj)
{
...
void IlcPVECEvalRecPoint::PrintWorkingPool()
{

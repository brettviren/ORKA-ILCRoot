...
// IHEP CPV/PVEC reconstruction algorithm used.
...
//____________________________________________________________________________
IlcPVECIhepAnalyze::IlcPVECIhepAnalyze(Text_t * name) : 
  fRunLoader(0),
  fFileName(name)
{
...
//____________________________________________________________________________
void IlcPVECIhepAnalyze::AnalyzeCPV1(Int_t Nevents)
{
  //
  ...
  IlcPVECLoader* please = dynamic_cast<IlcPVECLoader*>(fRunLoader->GetLoader("PVECLoader"));
  if ( please == 0 )
   {
...
/******************************************************************/     
    ...
    if (! (branchCPVimpacts =treeH->GetBranch("PVECCpvImpacts")) ) {
      IlcWarning(Form("Couldn't find branch PVECCpvImpacts. Exit.")) ;
      return;
    }
 
    // Create and fill arrays of hits for each CPV module
      
    Int_t nOfModules = PVECgeom->GetNModules();
    TClonesArray **hitsPerModule = new TClonesArray *[nOfModules];
    Int_t iModule = 0; 	
    for (iModule=0; iModule < nOfModules; iModule++)
      hitsPerModule[iModule] = new TClonesArray("IlcPVECImpact",100);

    TClonesArray    *impacts;
    IlcPVECImpact   *impact;
    TLorentzVector   p;
    ...
    IlcPVECCpvRecPoint *cpvRecPoint ;
    Float_t xgen, ygen, zgen;
    while( ( cpvRecPoint = (IlcPVECCpvRecPoint *)nextRP() ) ) {
      
      Float_t chi2dof = ((IlcPVECEvalRecPoint*)cpvRecPoint)->Chi2Dof();
      hChi2->Fill(chi2dof);
      hEr->Fill(cpvRecPoint->GetEnergy());

      TVector3  locpos;
      cpvRecPoint->GetLocalPosition(locpos);
      Int_t PVECModule = cpvRecPoint->GetPVECMod();
      ...
      Int_t nCPVhits  = (hitsPerModule[PVECModule-1])->GetEntriesFast();
	...
	impact = (IlcPVECImpact*)(hitsPerModule[PVECModule-1])->UncheckedAt(ihit);
	xgen   = impact->X();
	zgen   = impact->Z();
	ygen   = impact->Y();
      	
	//Transform to the local ref.frame
	Float_t phig = PVECgeom->GetPVECAngle(PVECModule);
	Float_t phi = TMath::Pi()/180*phig;
	Float_t distanceIPtoCPV = PVECgeom->GetIPtoOuterCoverDistance() -
	  (PVECgeom->GetFTPosition(1)+
	   PVECgeom->GetFTPosition(2)+
	   PVECgeom->GetCPVTextoliteThickness())/2;
	Float_t xoL,yoL,zoL ;
...
void IlcPVECIhepAnalyze::AnalyzeEMC1(Int_t Nevents)
{
  //
  ...
  for ( Int_t ievent=0; ievent<Nevents; ievent++) {  
    ...
    if (! (branchEMCimpacts =treeH->GetBranch("PVECEmcImpacts")) ) {
      IlcWarning(Form(" Couldn't find branch PVECEmcImpacts. Exit."));
      return;
    }
    ...
    IlcPVECEmcRecPoint *emcRecPoint ;
    Float_t xgen, ygen, zgen;
    while( ( emcRecPoint = (IlcPVECEmcRecPoint *)nextRP() ) ) {
      
      Float_t chi2dof = ((IlcPVECEvalRecPoint*)emcRecPoint)->Chi2Dof();
      hChi2->Fill(chi2dof);
      
      TVector3  locpos;
      emcRecPoint->GetLocalPosition(locpos);
      Int_t PVECModule = emcRecPoint->GetPVECMod();
      Int_t rpMult     = emcRecPoint->GetMultiplicity();
      Int_t rpMultX, rpMultZ;
      ((IlcPVECEvalRecPoint*)emcRecPoint)->GetClusterLengths(rpMultX,rpMultZ);
      Float_t xrec  = locpos.X();
      Float_t zrec  = locpos.Z();
      Float_t dxmin = 1.e+10;
      Float_t dzmin = 1.e+10;
      ...
      Int_t nEMChits  = (hitsPerModule[PVECModule-1])->GetEntriesFast();
      Float_t locImpX=1e10,locImpZ=1e10;         // local coords of closest impact
      Float_t gImpX=1e10, gImpZ=1e10,gImpY=1e10; // global coords of closest impact
	...
	Float_t phig = PVECgeom->GetPVECAngle(PVECModule);
	Float_t phi = TMath::Pi()/180*phig;
	Float_t distanceIPtoEMC = PVECgeom->GetIPtoCrystalSurface();
	Float_t xoL,yoL,zoL ;
//  	xoL = xgen*TMath::Cos(phig)+ygen*TMath::Sin(phig) ;
...
//____________________________________________________________________________
void IlcPVECIhepAnalyze::AnalyzeCPV2(Int_t Nevents)
{
  ...
  for (Int_t nev=0; nev<Nevents; nev++) 
    { 
	 ...
	 IlcError(Form("Can not get TreeH"));
      ...
      if (! (branchCPVimpacts =treeH->GetBranch("PVECCpvImpacts")) )  return;
	...
	printf("CPV module %d has %d hits\n",iModule,nsum);
        ...
        IlcPVECImpact* genHit1;
        IlcPVECImpact* genHit2;
        Int_t irp1,irp2;
	for(irp1=0; irp1< nsum; irp1++)
	  {
	    genHit1 = (IlcPVECImpact*)((hitsPerModule[iModule])->At(irp1));
	    for(irp2 = irp1+1; irp2<nsum; irp2++)
	      {
		genHit2 = (IlcPVECImpact*)((hitsPerModule[iModule])->At(irp2));
...
//      		IlcInfo(Form("(dx dz dr): %f %f", dx, dz));
  ...
  //--------- Combinatoric distance between rec. hits in CPV
      ...
      if(nRecCPV)
	{
	  IlcPVECCpvRecPoint* recHit1;
	  IlcPVECCpvRecPoint* recHit2;
	  ...
	  while(TObject* obj1 = nextCPVrec1() )
	      ...
	      while (TObject* obj2 = nextCPVrec2())
		{
		  if(!obj2->IsEqual(obj1))
		    {
		      recHit1 = (IlcPVECCpvRecPoint*)obj1;
		      recHit2 = (IlcPVECCpvRecPoint*)obj2;
		      TVector3 locpos1;
		      TVector3 locpos2;
		      ...
		      Float_t dr = TMath::Sqrt(dx*dx + dz*dz);
		      if(recHit1->GetPVECMod() == recHit2->GetPVECMod())
			hDrijCPVr->Fill(dr);
		    }
		}
	    }	
...
void IlcPVECIhepAnalyze::CpvSingle(Int_t nevents)
{
  ...
  IlcPVECLoader* gime = dynamic_cast<IlcPVECLoader*>(fRunLoader->GetLoader("PVECLoader"));
  if ( gime == 0 )
   {
     IlcError(Form("Could not obtain the Loader object !"));
     return ;
  ...
  for(Int_t ievent=0; ievent<nevents; ievent++)
    {
      ...
      IlcPVECCpvRecPoint* pt = (IlcPVECCpvRecPoint*)(gime->CpvRecPoints())->At(0);
      if(pt) {
	TVector3 lpos;
	...
	hNrpX->Fill(rpMultX);
	hNrpZ->Fill(rpMultZ);
	hChi2->Fill(((IlcPVECEvalRecPoint*)pt)->Chi2Dof());
...
void IlcPVECIhepAnalyze::HitsCPV(Int_t nev)
{
...
//    IlcPVECLoader * please = IlcPVECLoader::GetInstance(GetFileName().Data(),"PVEC");
//    const IlcPVECGeometry *  fGeom  = please->PVECGeometry();

//    Int_t ntracks = gIlc->GetEvent(ievent);
//    Int_t nOfModules = fGeom->GetNModules();
//    IlcInfo(Form(" Tracks: "<<ntracks<<"  Modules: "<<nOfModules));

//    if (! (branchCPVimpacts =gIlc->TreeH()->GetBranch("PVECCpvImpacts")) )  return;
...
//  	    new(hits[hits.GetEntriesFast()]) IlcPVECImpact(*impact);
//  	    Info(Form("done."));
//  	  }
//        }
//      }
...
//    Info(Form(" PVEC event "<<ievent<<": "<<hits.GetEntries()<<" charged CPV hits."));

}


//  void IlcPVECIhepAnalyze::ChargedHitsCPV(TClonesArray* hits, Int_t ievent, Int_t iModule)
//  {
//    // Cumulative list of charged CPV hits in event ievent 
//    // in PVEC module iModule.

//    HitsCPV(hits,ievent,iModule);
//    TIter next(hits);

//    while(IlcPVECCPVHit* cpvhit = (IlcPVECCPVHit*)next())
//      {
...
//    Info(Form(" PVEC module "<<iModule<<": "<<hits->GetEntries()<<" charged CPV hits."));
//  }

Bool_t IlcPVECIhepAnalyze::IsCharged(Int_t pdgCode)
{
  // For HIJING

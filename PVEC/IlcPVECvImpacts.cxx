...
/* $Id: IlcPVECvImpacts.cxx 49869 2011-05-18 04:49:51Z hristov $ */
...
ClassImp(IlcPVECvImpacts)

//____________________________________________________________________________
IlcPVECvImpacts::IlcPVECvImpacts():
...
//____________________________________________________________________________
IlcPVECvImpacts::IlcPVECvImpacts(const char *name, const char *title):
  ...
  fNPPSDImpacts()
{
  ...
  //     This part inherits from IlcPVECv1
  //
  // We store impacts :
  //  - fEMCImpacts, fCPVImpacts which are
  //    TList of EMC and CPV modules respectively, each
  //    modules contains TClonesArray of IlcPVECImpacts

  Int_t nPVECModules = GetGeometry()->GetNModules();
  Int_t nCPVModules  = GetGeometry()->GetNModules();

  Int_t iPVECModule;
  TClonesArray * impacts;
  for (iPVECModule=0; iPVECModule<nPVECModules; iPVECModule++) {
    fEMCImpacts->Add(new TClonesArray("IlcPVECImpact",200)) ;
    fNEMCImpacts[iPVECModule] = 0;
    impacts = dynamic_cast<TClonesArray *>(fEMCImpacts->At(iPVECModule));
  }
  for (iPVECModule=0; iPVECModule<nCPVModules; iPVECModule++) {
    fCPVImpacts->Add(new TClonesArray("IlcPVECImpact",200)) ;
    fNCPVImpacts[iPVECModule] = 0;
    impacts = dynamic_cast<TClonesArray *>(fCPVImpacts->At(iPVECModule));
  }
...
//____________________________________________________________________________
void IlcPVECvImpacts::AddImpact(const char* det, Int_t shunt, Int_t primary, Int_t track, Int_t module,
			   Int_t pid, TLorentzVector p, Float_t *xyz)
{
  ...
  if (strcmp(det,"EMC ")==0) {
    ...
    IlcFatal(Form("Wrong PVEC configuration: det=%s",det));

  new((*impacts)[nImpacts]) IlcPVECImpact(shunt,primary,track,pid,p,xyz) ;

  IlcDebugClass(1,Form("Module %d %s: ",module,det));
  if (IlcLog::GetGlobalDebugLevel()>0)
    (static_cast<IlcPVECImpact*>((impacts->At(nImpacts))))->Print();
}

//____________________________________________________________________________
void IlcPVECvImpacts::MakeBranch(Option_t *opt)
{  
  // Create new branch in the current Hits Root Tree containing
  // a list of PVEC impacts (exact values of track coming to detector)

  IlcDetector::MakeBranch(opt);
  
  Int_t bufferSize = 32000 ;
  Int_t splitlevel = 0 ;
  fLoader->TreeH()->Branch("PVECEmcImpacts" , "TList", &fEMCImpacts , bufferSize, splitlevel);
  fLoader->TreeH()->Branch("PVECCpvImpacts" , "TList", &fCPVImpacts , bufferSize, splitlevel);
  
}

//____________________________________________________________________________
void IlcPVECvImpacts::ResetHits() 
{              

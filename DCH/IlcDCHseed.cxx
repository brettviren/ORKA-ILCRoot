...
{
  //
  for (Int_t i=0;i<kMaxLayer;i++) SetClusterIndex(i,-3);
  for (Int_t i=0;i<kMaxLayer;i++) fClusterPointer[i]=0;
  for (Int_t i=0;i<kMaxLayer;i++) fClusterChi2[i]=-1;
  ...
  fCircular(0)
{
  ...
  for (Int_t i=0;i<kMaxLayer;i++) {
	...
	fClusterPointer[i] = new IlcDCHcluster(*(s.fClusterPointer[i]));
    }else{
      fClusterPointer[i] = s.fClusterPointer[i];
    }
    fTrackPoints[i] = s.fTrackPoints[i];
  }
  for (Int_t i=0;i<kMaxLayer;i++) fClusterChi2[i]=s.fClusterChi2[i];
  for (Int_t i=0;i<kMaxLayer;i++) fIndex[i] = s.fIndex[i];
...
IlcDCHTrackerPoint * IlcDCHseed::GetTrackPoint(Int_t i)
{
  //
  // 
...
void IlcDCHseed::RebuildSeed()
{
  ...
  for (Int_t i=0;i<kMaxLayer;i++){
    IlcDCHcluster * cl0 = fClusterPointer[i];
    IlcDCHTrackPoint *trpoint = (IlcDCHTrackPoint*)fPoints->UncheckedAt(i);     
    if (cl0){
...
Bool_t IlcDCHseed::Update(const IlcCluster* c2, Double_t chi2, Int_t /*i*/,
			  double rwire,double anglewire,double tanstereo,bool withZ,int signImP){
  const IlcDCHcluster *c=dynamic_cast<const IlcDCHcluster*>(c2);
  if(!c) return kFALSE;
  ...
  if(IlcDebugLevelClass()>10){
	...
	<<" tr "<<sqrt(GetSigmaY2())<<" "<<sqrt(GetSigmaZ2())<<endl;
    Print("param");
...
Float_t IlcDCHseed::CookdEdx(Double_t low, Double_t up,Int_t i1, Int_t i2, Bool_t onlyused) {
  ...
  for (Int_t of =0; of<4; of++){    
	...
	IlcDCHTrackerPoint * point = GetTrackPoint(i);

	if (point==0) continue;
	IlcDCHcluster * cl = fClusterPointer[i];
	if (cl==0) continue;	

...
/* $Id: IlcDCHtrack.cxx,v 1.5 2009/06/30 11:17:54 cgatto Exp $ */
...
//   cc[10]*=w1; cc[11]*=w1; cc[12]*=w1; cc[13]*=w1; cc[14]*=w1;
  ...
  fIndex[0]=index;
  for (Int_t i=1; i<kMaxLayer;i++) fIndex[i]=-2;
  for (Int_t i=0; i<4;i++) fPoints[i]=0.;
  for (Int_t i=0; i<12;i++) fKinkPoint[i]=0.;
  for (Int_t i=0; i<3;i++) fKinkIndexes[i]=0;
  for (Int_t i=0; i<3;i++) fV0Indexes[i]=0;
...

{
  ...
  for (Int_t i=0; i<kMaxLayer; i++) fIndex[i]=t.fIndex[i];
...
void  IlcDCHtrack::UpdatePoints()
{
  ...
  for (Int_t i=0;i<kMaxLayer;i++){
    ...
    if (nall<range) nall++;
    if (last>=0){
      if (fIndex[last]>=0&& (fIndex[last]&ClusterIndex::NotAccept)==0) ngood--;
      if (fIndex[last]==-1) undeff--;
    }
    if (fIndex[i]>=0&& (fIndex[i]&ClusterIndex::NotAccept)==0)   ngood++;

...
/*
$Log: IlcDCHmcm.cxx,v $
Revision 1.2  2009/12/11 10:40:28  vitomeg
typo fixed
...
Revision 1.1  2006/12/04 16:55:23  ilcrootcvs
Added new files in order to support DCH

Revision 1.1  2006/11/30 17:23:33  giuter
add classes DCH

Revision 1.1.1.1  2006/11/09 16:31:41  vitomeg
Lite Version of IlcRoot to convert in IlcRoot

Revision 1.1.1.1  2006/09/04 14:57:01  vitomeg
initial revision

Revision 1.2  2006/04/05 12:45:40  hristov
Updated DCH trigger code. Now the trigger code can run:
...
//_____________________________________________________________________________
IlcDCHmcm::IlcDCHmcm(IlcDCHtrigParam *trigp, const Int_t id) 
{
  //
  // IlcDCHmcm constructor
  //
...
{
  //
  // IlcDCHmcm destructor
  //

}

//_____________________________________________________________________________
IlcDCHmcm &IlcDCHmcm::operator=(const IlcDCHmcm &m)
{
  //
  // assignment operator
  //

  if (this != &m) ((IlcDCHmcm &) m).Copy(*this); 
  return *this;

}

//_____________________________________________________________________________
void IlcDCHmcm::Copy(TObject &m) const
{
  //
  // copy function
  //

  ((IlcDCHmcm &) m).fTrigParam  = NULL;
  ((IlcDCHmcm &) m).fNtrk       = fNtrk;
  ((IlcDCHmcm &) m).fRobId      = fRobId;
  ((IlcDCHmcm &) m).fChaId      = fChaId;
  ((IlcDCHmcm &) m).fRow        = fRow;
  ((IlcDCHmcm &) m).fColFirst   = fColFirst;
  ((IlcDCHmcm &) m).fColLast    = fColLast;
  ((IlcDCHmcm &) m).fTime1      = fTime1;
  ((IlcDCHmcm &) m).fTime2      = fTime2;
  ((IlcDCHmcm &) m).fClusThr    = fClusThr;
  ((IlcDCHmcm &) m).fPadThr     = fPadThr;
  ((IlcDCHmcm &) m).fNtrkSeeds  = fNtrkSeeds;
  ((IlcDCHmcm &) m).fR1         = fR1;
  ((IlcDCHmcm &) m).fR2         = fR2;
  ((IlcDCHmcm &) m).fC1         = fC1;
  ((IlcDCHmcm &) m).fC2         = fC2;
  ((IlcDCHmcm &) m).fPedestal   = fPedestal;
  ((IlcDCHmcm &) m).fId         = fId;

  for (Int_t i = 0; i < kMaxTrackletsPerMCM; i++) {
    ((IlcDCHmcm &) m).fTrkIndex[i] = 0;
  }
  for (Int_t i = 0; i < kMcmCol; i++) {
    ((IlcDCHmcm &) m).fPadHits[i] = 0;
    for (Int_t j = 0; j < kMcmTBmax; j++) {
      ((IlcDCHmcm &) m).fADC[i][j] = 0.0;
      ((IlcDCHmcm &) m).fIsClus[i][j] = kFALSE;
    }
  }
  for (Int_t i = 0; i < kMaxTrackletsPerMCM; i++) {
    ((IlcDCHmcm &) m).fSeedCol[i] = -1;
  }

}

//_____________________________________________________________________________
void IlcDCHmcm::AddTrk(const Int_t id) 
{
  //
...
//_____________________________________________________________________________
Bool_t IlcDCHmcm::Run()
{
  //
  // Run MCM
  //

  if ( fTrigParam->GetDebugLevel() > 1 ) printf("IlcDCHmcm::Run MCM %d\n",Id());
...
//_____________________________________________________________________________
Int_t IlcDCHmcm::CreateSeeds()
{
  //
  // Make column seeds (from Falk Lesser, ex KIP)
  //

  if ( fTrigParam->GetDebugLevel() > 1 ) printf("IlcDCHmcm::CreateSeeds MCM %d \n",Id());
...
//_____________________________________________________________________________
void IlcDCHmcm::Sort(Int_t nel, Int_t *x1, Int_t *x2, Int_t dir)
{
  //
...
//_____________________________________________________________________________
void IlcDCHmcm::AddTimeBin(const Int_t iTime)
{
  //
  // Build column seeds
  //
...
//_____________________________________________________________________________
Bool_t IlcDCHmcm::IsCluster(Float_t amp[3]) const
{
  //
...
//_____________________________________________________________________________
void IlcDCHmcm::Filter(Int_t nexp, Int_t ftype)
{
  //
  // exponential filter
  //
...
//_____________________________________________________________________________
void IlcDCHmcm::DeConvExpA(Double_t *source, Double_t *target, Int_t n, Int_t nexp) 
{
  //
  ...
  // FilterOpt.C (ilcroot@pel:/homel/ilcroot/root/work/beamt/CERN02)
...
//_____________________________________________________________________________
void IlcDCHmcm::DeConvExpD(Double_t *source, Int_t *target, Int_t n, Int_t nexp) 
{
  //
...
//_____________________________________________________________________________
void IlcDCHmcm::DeConvExpMI(Double_t *source, Double_t *target, Int_t n) 
{
  //
...
void IlcDCHmcm::TailMakerSpline(Double_t *ampin, Double_t *ampout, Double_t lambda, Int_t n) 
{
  //
...
//______________________________________________________________________________
void IlcDCHmcm::TailCancelationMI(Double_t *ampin, Double_t *ampout, Double_t norm, Double_t lambda, Int_t n) 
{
  //
  // Special filter (M. Ivanov)
  //

...
/* $Id: IlcDCHseed.h,v 1.5 2009/06/30 11:17:54 cgatto Exp $ */
...
class IlcDCHseed : public IlcDCHtrack {
  ...
  // DCH specific propagation
     ...
     Bool_t Update(const IlcCluster* c2, Double_t chi2, Int_t i,
		   double rwire,double anglewire,double tanstereo,bool withZ,int signImP=0);

  // seeding operation
     Int_t Compare(const TObject *o) const;
     void Reset(Bool_t all = kTRUE);
     IlcDCHTrackerPoint * GetTrackPoint(Int_t i);
     IlcDCHcluster * GetClusterFast(Int_t ilayer){ return fClusterPointer[ilayer];}
     void SetClusterPointer(Int_t ilayer, IlcDCHcluster* cl) {fClusterPointer[ilayer]=cl;}
     ...
     IlcDCHcluster* GetClusterPointer(Int_t i) const {return fClusterPointer[i];}
     Float_t GetClusterChi2(Int_t i) const {return fClusterChi2[i];}
     void SetClusterChi2(Int_t i,float chi2) {fClusterChi2[i]=chi2;}
 ...
 private:
     ...
     IlcESDtrack * fEsd; //!
     IlcDCHcluster*   fClusterPointer[kMaxLayer];  // array of cluster pointers  - 
     Float_t  fClusterChi2[kMaxLayer];  // array of cluster chi2  - 
     ...
     IlcDCHTrackerPoint  fTrackPoints[kMaxLayer];  //track points - array track points

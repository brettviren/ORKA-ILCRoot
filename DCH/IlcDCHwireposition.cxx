...
IlcDCHwireposition::IlcDCHwireposition(Bool_t newFile, const char *WireDataFile ) 
{ 
...
//	fWireDataAll[i] = (IlcDCHwiredata*)fwiredata->Clone();
  ...
  Warning("IlcDCHwireposition",Form("Default Param instanced\n"));
...
void IlcDCHwireposition::FillData(){
  trwdata->Fill();
}
...
void IlcDCHwireposition::WirePosAtZ(Float_t z, Float_t *pos){
  ...
  if (TMath::Abs(z)>DCHEndcapZ) {
    IlcInfoClass(Form("z: %f exceeds maximum length: %f",z , DCHEndcapZ));

#ifndef ILCDCHMCMTRACKLET_H
#define ILCDCHMCMTRACKLET_H
...
class IlcDCHmcmTracklet : public TObject {
 ...
 public:
  ...
  IlcDCHmcmTracklet();
  IlcDCHmcmTracklet(Int_t det, Int_t row, Int_t n);
  virtual ~IlcDCHmcmTracklet();
  IlcDCHmcmTracklet &operator=(const IlcDCHmcmTracklet &t);
  virtual void Copy(TObject &t) const;

  void Reset();

  void AddCluster(Int_t icol, Int_t itb, Float_t *adc, Int_t *track);
  void MakeTrackletGraph(IlcDCHgeometry *geo = 0, Float_t field = 0);
 ...
 protected:
  ...
  Int_t   fDetector;                     //  DCH detector number (0 ... 539)
  ...
  ClassDef(IlcDCHmcmTracklet,2)          // Track segment for the DCH (Tracklet)

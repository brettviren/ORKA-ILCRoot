...
/* $Id: IlcDCHtracker.cxx,v 1.16 2009/12/11 10:40:28 vitomeg Exp $ */
...
Int_t IlcDCHtracker::UpdateTrack(IlcDCHseed * track, Int_t accept){
  ...
  track->SetClusterIndex(track->GetLayer(), dchindex);  
  ...
  track->SetClusterPointer(track->GetLayer(),c);  
  //
  ...
  if (angle2<1) //PH sometimes angle2 is very big. To be investigated...
  {
    ...
    IlcDCHTrackerPoint   &point =*(track->GetTrackPoint(track->GetLayer()));
    //
  ...
  Double_t chi2 = track->GetPredictedChi2(c,
					  lay.GetR(),lay.GetWirePhi(c->GetW()),lay.GetTStereoAngle(),
					  kUseZCoordinate);

  track->SetClusterChi2(track->GetLayer(),chi2);

  return track->Update(c,chi2,dchindex,
		       lay.GetR(),lay.GetWirePhi(c->GetW()),lay.GetTStereoAngle(),
		       kUseZCoordinate);
}
...
IlcDCHtracker::IlcDCHtracker(const IlcDCHParam *par): 
IlcTracker()
{
  //---------------------------------------------------------------------
  ...
  IlcInfo(Form("Number of layers in DCH = %i",nlayers));
  ...
  while(ibegin<ncl) {
    ...
    if(layer>=fSector.GetNLayers()){
      IlcError(Form("Layer=%i from cluster id=%i bigger than total numberr of layers=%i",
		    layer,((IlcDCHcluster*)fClusters->At(ibegin)),fSector.GetNLayers()));
      ibegin=iend;
      continue;
    }
...
//         cl->SetZLocal(0.);
      ...
      if(fDebug>=10) cout<<" ("<<dchlayer->GetCluster(i)->GetX()<<","
			 ...
			 <<TMath::ATan2(dchlayer->GetCluster(i)->GetY(),dchlayer->GetCluster(i)->GetX())<<","
...
Int_t IlcDCHtracker::FollowToNext(IlcDCHseed& t, Int_t nr) {
  ...
  IlcDCHcluster *cl=0;
  Int_t dchindex= t.GetClusterIndex(nr);
  bool cannotProp=false;
  //
  if (fIteration>0 && dchindex>=-1){  //if we have already clusters 
    if (dchindex>=0){     //
      cl = t.GetClusterPointer(nr);
      if ( (cl==0) ) cl = GetCluster(dchindex);
    }
	...
	if((dchindex&ClusterIndex::NotAccept)) return -1;
      ...
      if (fDebug>15) cout<<"Try Accept cluster layer "<<nr<<" yz "<<cl->GetY()<<" "<<cl->GetZ()<<endl;
      t.SetLayer(nr);
  ...
  //calculate 
  if (klayer) {
    //    cl = klayer.FindNearest2(y,z,roady,roadz,index);   
    IlcDCHcluster* clar[2]={0,0};
    UInt_t indar[2]={0,0};
    klayer.FindNearest2(y,z,roady,roadz,1,clar,indar);
    double chi2[2]={1e30,1e30};
    if(clar[0]){
      if(clar[1]) chi2[0]=t.GetPredictedChi2(clar[0],klayer.GetR(),klayer.GetWirePhi(clar[0]->GetW()),klayer.GetTStereoAngle(),kUseZCoordinate);
      cl=clar[0];
      index=indar[0];
    }
    if(clar[1]){
      chi2[1]=t.GetPredictedChi2(clar[1],klayer.GetR(),klayer.GetWirePhi(clar[1]->GetW()),klayer.GetTStereoAngle(),kUseZCoordinate);
      if(chi2[1]<chi2[0]){
	cl=clar[1];
	index=indar[1];
      }      
...
Int_t IlcDCHtracker::UpdateClusters(IlcDCHseed& t,  Int_t nr) {
  ...
  IlcDCHcluster *cl=0;
  Int_t index = t.GetClusterIndex(nr);    
  ...
  if (klayer.GetN()>0) {    
    //    cl = klayer.FindNearest2(y,z,roady,roadz,uindex);      
    IlcDCHcluster* clar[2]={0,0};
    UInt_t indar[2]={0,0};
    klayer.FindNearest2(y,z,roady,roadz,1,clar,indar);
    double chi2[2]={1e30,1e30};
...
Int_t IlcDCHtracker::FollowProlongation(IlcDCHseed& t, Int_t rf, Int_t step,int contin) {
  ...
  if(first<nlayers-1&&!contin){
    if((t.GetClusterIndex(first+1)&ClusterIndex::NotAccept)==0)
      first++;
  }
  ...
  for (Int_t nr= first; nr>=rf; nr-=step) {
	...
	if (index<0) continue;
	//
	IlcKink * kink = (IlcKink*)fEvent->GetKink(index-1);
	if (!kink){
	  printf("PROBLEM\n");
	}
	else{
...
Float_t IlcDCHtracker::OverlapFactor(IlcDCHseed * s1, IlcDCHseed * s2, Int_t &sum1, Int_t & sum2)
{
  ...
  // Not used anyware so don't check distance forr reducing combinatorics
  ...
  for (Int_t i=firstpoint-1;i<lastpoint+1;i++){
    if (s1->GetClusterIndex(i)>0) sum1++;
    if (s2->GetClusterIndex(i)>0) sum2++;
    if (s1->GetClusterIndex(i)==s2->GetClusterIndex(i) && s1->GetClusterIndex(i)>0) {
      sum++;
    }
  }
  if (sum<5) return 0;
...
void  IlcDCHtracker::SignShared(IlcDCHseed * s1, IlcDCHseed * s2)
{
  ...
  if (sumshared>4){
    ...
    for (Int_t i=firstpoint;i<lastpoint;i++){
      if ( (s1->GetClusterIndex(i))==(s2->GetClusterIndex(i)) && s1->GetClusterIndex(i)>=0) {
	IlcDCHTrackerPoint *p1  = s1->GetTrackPoint(i);
	IlcDCHTrackerPoint *p2  = s2->GetTrackPoint(i);; 
...
void IlcDCHtracker::RemoveUsed(TObjArray * arr, Float_t factor1,  Float_t factor2, Int_t removilcndex)
{
  ...
  for (Int_t i=0; i<nseed; i++) {
    ...
    for (Int_t i=0; i<nlayers; i++) {
      Int_t dchindex=pt->GetClusterIndex(i);
      if (dchindex<0 || dchindex&ClusterIndex::NotAccept ) continue;
      IlcDCHcluster *c= pt->GetClusterPointer(i);        
      if (!c) continue;
      c->Use(10);  
    }
...
//       if (chi2>2.5) chi2=2.5;
  ...
  if (IlcDebugLevelClass()>3){
    Info("SignClusters",Form("Number of signed tracks %i clusters: %i",nsigned,nsignedcl));
  }
}
 ...
 Float_t th2) const
{
  ...
  for (Int_t i=layer0; i<maxindex; i++){
    Int_t index = seed->GetClusterIndex(i);
...
Int_t IlcDCHtracker::RefitInward(IlcESDEvent *event)
{
  //
...
Int_t IlcDCHtracker::PropagateBack(IlcESDEvent *event)
{
  //
  // back propagation of ESD tracks
  //
...
void IlcDCHtracker::ReadSeeds(IlcESDEvent *event, Int_t direction)
{
  //
...
void IlcDCHtracker::MakeSeeds3Stereo(TObjArray * arr, Int_t i1, Int_t i2,  Float_t cuts[4]) {
  ...
  for (Int_t is=0; is < kr1; is++) {
	...
	int foundmiddle=-1;
	for(int i=foundmiddleprev+1;i<3;i++){
	  double phisamestereo=123.;
	  if(tangle[2]*krm[i]->GetTStereoAngle()>0) {
	    phisamestereo=phi[2];
	    i0=1;
	  }else if(tangle[0]*krm[i]->GetTStereoAngle()>0){
	    phisamestereo=phi[0];
	    i0=0;
	  }else{
	    if(fDebug>15) cout<<"Different stereo angles "<<tangle[0]<<" "<<tangle[2]<<" "<<krm[i]->GetTStereoAngle()<<endl;
	    continue;
	  }
	  if(fDebug>15) cout<<"on middle "<<phisamestereo<<" road "<<fRecPar->GetRoadY()+rwire[2]-rwire[0];
	  
	  kcm = krm[i]->FindNearest2(phisamestereo,0.,fRecPar->GetRoadY()+rwire[2]-rwire[0],1e6,dummy,selectSecond);
	  if ((!kcm) || (kcm->IsUsed(10))) {	  
	    if(fDebug>15) cout<<" not found on middles "<<imiddle+(i/2-(i%2))<<" "<<kcm<<endl;
	  }else{
	    if(fDebug>15) cout<<" trackid "<<kcm->GetLabel(0)<<" "<<kcm->GetLabel(1)<<endl;
	    foundmiddle=i;
	    break;
	  }
	}
	...
	c[2]=/*100.0**/fRecPar->GetSeedBeamSigma2();
	c[14]=1.e-4;
	
	phi1=(phi[0]+phi[2])/2.+((fabs(phi[2]-phi[0])>TMath::Pi()/2)?(TMath::Pi()):0.);
	double zz=TMath::Tan(phi1-phi[0])*rwire[0]/tangle[0];
	...
	for(int lr=0;lr<8;lr++){
	  double leftr[3]={(lr%2?1:-1),((lr/2)%2?1:-1),((lr/4)%2?1:-1)};
	  if(fDebug>15)
...
// 	     clseed[0]->GetLabel(0)!=clseed[2]->GetLabel(0)) continue;
	  ...
	  x[0]=y3;
	  x[1]=z3;
	  x[3]=(zz-z3)/TMath::Hypot(rwire[0],zz*tangle[0]);
	  ...
	  for(int kk=0;kk<10;kk++){
	    ...
	    track->IlcExternalTrackParam::PropagateTo(0.001,IlcTracker::GetBz(),false,true);
	    ...
	    for(int i=0;i<3;i++){
	      ...
	      if(!track->PropagateTo(TMath::Hypot(xyz[0],xyz[1]))&&i<2){propagateOK=false;break;}
	      ...
	      chi2+=track->GetPredictedChi2(clseed[i],rwire[i],phi[i],tangle[i],kUseZCoordinate);
	      if(!track->Update(clseed[i],0,0,
				rwire[i],phi[i],tangle[i],kUseZCoordinate,((dphi*leftr[i])<0?-1:1)))
		{propagateOK=false;break;}
	      if(fabs(track->GetTgl())>IlcDCHReconstructor::GetCtgRange()||
		 fabs(track->GetSignedPt())<kSmallPt){propagateOK=false;break;}
	      //    track->Print();
	    }
	    ...
	    if(kk>=2&&chi2<1e-5) break;
	    if(fabs(track->GetZ())>2*(kr1.GetMaxZ()+fRecPar->GetSafetyZ())) break;
	  }
       
	  if(!propagateOK) continue;
	  if(chi2>1.) continue;
	  if(fabs(track->GetZ())>kr1.GetMaxZ()+fRecPar->GetSafetyZ()) continue;
	  ...
	  if(krn[0]){
	    ...
	    for(int i=0;i<3;i++){
	      ...
	      if(fDebug>15) cout<<"on next "<<phiat<<" road "<<fRecPar->GetRoadY()<<" z= "<<xyz[2]<<" ";
	      const IlcDCHcluster *kcm=krn[i]->FindNearest2(phiat,xyz[2],fRecPar->GetRoadY(),1e6,dummy);
	      if ((!kcm) || (kcm->IsUsed(10))) {	  
		...
		if(fDebug>15) cout<<"trackid "<<kcm->GetLabel(0)<<" "<<kcm->GetLabel(1)<<" chi2 "<<chi2<<endl;
	  ...
	  track->SetClusterIndex(i1,index);
	  track->SetClusterPointer(i1,(IlcDCHcluster*)kcl1);
...
void IlcDCHtracker::MakeSeeds3(TObjArray * arr, Int_t i1, Int_t i2,  Float_t cuts[4]) {
  ...
  for (Int_t is=0; is < 2*kr1; is++) {
	...
	cout<<"z estimate "<<zzzz2<<" clZ2="<<z2<<" clZ1="<<z1
      ...
      if (TMath::Abs(zzzz2-z2)>fRecPar->GetRoadZ()) continue;       
      nin1++;              
      //	
	...
	cm = krm[i]->FindNearest2(TMath::ATan2(ym,xm[i])+phi1,zm,
				  fRecPar->GetRoadY(),
	...
	continue;
      ...
      x[1]=z3;
      for(int i=0;i<15;i++) c[i]=0.;
      c[0]=c[2]=/*25000*x[4]*x[4]+*/fRecPar->GetSeedBeamSigma2();
...
void IlcDCHtracker::MakeSeeds5(TObjArray * arr, Int_t i1, Int_t i2,  Float_t cuts[4]) {
  ...
  for (Int_t is=0; is < 2*kr1.GetN(); is++) {
	...
	zzzm = anglez*(rm-x1)+z1;
        ...
        kcm = krm[i]->FindNearest2(TMath::ATan2(yyym,rm)+phi1,zzzm,
				   erry,errz,index);
        if ((!kcm) || (kcm->IsUsed(10))) {          
          if(fDebug>15) 
	...
	3*sqrt(kcl3->GetSigmaZ2()+kcl1->GetSigmaZ2()+kcm->GetSigmaZ2());
      ...
      const IlcDCHcluster *kc1m = kr1m.FindNearest2(TMath::ATan2(yyym,r1m)+phi1,zzzm,erry,errz,index);
	...
	if (kc1m->IsUsed(10)) used++;
      ...
      const IlcDCHcluster *kc1p = kr1p.FindNearest2(TMath::ATan2(yyym,r1p)+phi1,zzzm,erry,errz,index);
	...
	continue;
      ...
      const IlcDCHcluster *kc3m = kr3m.FindNearest2(TMath::ATan2(yyym,xxxm)+phi1,zzzm,erry,errz,index);
	...
	if (kc3m->IsUsed(10)) used++;
      ...
      yyym = angley*(xxxm-x3)+y3;
      zzzm = anglez*(xxxm-x3)+z3;
      const IlcDCHcluster *kc3p = kr3p.FindNearest2(TMath::ATan2(yyym,xxxm)+phi1,zzzm,erry,errz,index);
      //
      if (kc3p){
	found++;
	...
	track->SetClusterIndex(i1-1,index);
	track->SetClusterPointer(i1-1,(IlcDCHcluster*)kcl1);
...
IlcDCHseed *IlcDCHtracker::MakeSeed(IlcDCHseed *track, Float_t r0, Float_t r1, Float_t r2)
{
  ...
  for (Int_t i=0;i<nlayers;i++){
	...
	if (cl!=0){
	  ...
	  cl2=cl;
	  lay2=i;
	  indexlast=track->GetClusterIndex(i);
	}
      }
    }
  }
...
IlcDCHseed *IlcDCHtracker::ReSeed(IlcDCHseed *track, Float_t r0, Float_t r1, Float_t r2)
{
  ...
  Int_t nclusters = 0;
  for (Int_t ilayer=0;ilayer<nlayers;ilayer++){
    if (track->GetClusterIndex(ilayer)>=0) nclusters++;
  }
  //
  Int_t ipos[3];
  ...
  Int_t index=-1;
  for (Int_t ilayer=0;ilayer<nlayers;ilayer++){    
    if (track->GetClusterIndex(ilayer)<0) continue;
    index++;
    for (Int_t ipoint=0;ipoint<3;ipoint++){
      if (index<=ipos[ipoint]) layer[ipoint] = ilayer;
    }        
  }
  //
  //Get cluster and sector position
  for (Int_t ipoint=0;ipoint<3;ipoint++){
    Int_t clindex = track->GetClusterIndex(layer[ipoint]);
...
IlcDCHseed *IlcDCHtracker::ReSeed(IlcDCHseed *track,Int_t r0, Bool_t forward)
{
  ...
  if (forward){
    for (Int_t ilayer=r0;ilayer<nlayers;ilayer++){
      if (track->GetClusterIndex(ilayer)>=0){
	layer[0] = ilayer;
	break;
      }
    }
  ...
  for (Int_t ipoint=0;ipoint<3;ipoint++){
    ...
    xyz[ipoint][0]  = GetRlayer(layer[ipoint]);
    IlcDCHTrackerPoint * point = track->GetTrackPoint(layer[ipoint]);    
    if (point&&ipoint<2){
      //
  ...
  seed->SetLastPoint(layer[2]);
  seed->SetFirstPoint(layer[2]);  
  for (Int_t i=layer[0];i<layer[2];i++){
...
void  IlcDCHtracker::FindKinks(TObjArray * array, IlcESDEvent *esd)
{
  //
  //  find kinks
  //
  ...
  for (Int_t i=0;i<nentries;i++){
    IlcDCHseed * track0 = (IlcDCHseed*)array->At(i);
    if (!track0) continue;
    if (track0->GetSignedPt()<1.4) continue;
...
void  IlcDCHtracker::FindV0s(TObjArray * array, IlcESD *esd)
{
  ...
  for (Int_t i =0;i<nentries;i++){
	...
	if ((vertex.GetRr()<fParam->GetInnerRadius()&&densa0+densa1<1.)&&(!isGamma)) continue;
      ...
      ncandidates++;
      {
	Int_t eventNr = esd->GetEventNumberInFile();
...
Int_t IlcDCHtracker::Clusters2Tracks (IlcESDEvent *esd)
{
  //
...
Int_t IlcDCHtracker::CookLabel(IlcKalmanTrack *tr, Float_t wrong,Int_t first, Int_t last) const {
  ...
  for(int i=0;i<kMaxLayer;i++) {
    ...
    IlcCluster *cl = t->GetClusterPointer(i);
    if(!cl) continue;
  ...
  if((noc!=numberofpoints)&&first==0&&last==kMaxLayer-1){
    ...
    int nn=0;
    for(int i=kMaxLayer-1;i>=0;i--) {
      Int_t cindex = t->GetClusterIndex(i);
      if (cindex<0||(cindex&ClusterIndex::NotAccept)) continue;
      cout<<nn<<"lay "<<i<<" "<<cindex<<" "<<t->GetClusterPointer(i)<<endl;
      nn++;
    }
...
Int_t  IlcDCHtracker::IlcDCHSector::GetLayerNumber(Double_t x,double z) const 
{
  ...
  while(nup>nlow){
    ...
    rmid=0.5*(fLayer[nmid].GetRAtZ(z)+fLayer[nmid+1].GetRAtZ(z));
    if(rmid<x) nlow=nmid+1;
    else nup=nmid;
  };
  return nlow;
}
...
void IlcDCHtracker::IlcDCHSector::Setup(const IlcDCHParam *par,IlcDCHwireposition *wirepos) {
  ...
  for(int i=0;i<nsuper;i++){
    for(int j=0;j<nrings;j++){
      ...
      //cout<<"Super layer "<<i+1<<" ring "<<j<<" Nwire "<<klay.GetNWires()<<" r "<<klay.GetR()<<" sa "<<klay.GetStereoAngle()<<" z "<<klay.GetMaxZ()<<endl;
...
IlcDCHcluster * IlcDCHtracker::IlcDCHLayer::FindNearest2(Double_t phi, Double_t z, Double_t roadphiR, Double_t roadz,UInt_t & index, int first) const 
{
  //-----------------------------------------------------------------------
  // Return the index of the nearest cluster in z y 
  //-----------------------------------------------------------------------
  ...
  for (Int_t i=iz1; i<iz2; i++) {
      IlcDCHcluster *c=(IlcDCHcluster*)(fClusters[i]);
      if (c->GetZ() > z+roadz) break;
      double wphir=fabs(Phi_mpi_pi(GetWirePhi(c->GetW(),z)-phi))*fR-fabs(c->GetImP());
...
void IlcDCHtracker::IlcDCHLayer::FindNearest2(Double_t phi, Double_t z, Double_t roadphiR, Double_t roadz,int first,

...
/* $Id: IlcDCHtracker.h,v 1.7 2009/06/30 11:17:54 cgatto Exp $ */
...
public:
  ...
  IlcDCHtracker(const IlcDCHParam *par); 
  virtual ~IlcDCHtracker();
  //
  void SetIteration(Int_t iteration){fIteration = iteration;}
  virtual Int_t Clusters2Tracks (IlcESDEvent *esd);
  virtual Int_t RefitInward (IlcESDEvent *esd);
...
// **************** Internal tracker class ********************** 
   ...
   public:
     ...
     IlcDCHcluster *  FindNearest2(Double_t phi, Double_t z, Double_t roadphiR, Double_t roadz, UInt_t & index, int first=0) const;
     
     void FindNearest2(Double_t phi, Double_t z, Double_t roadphiR, Double_t roadz,int first,IlcDCHcluster* clar[],UInt_t inar[]) const;
     ...
     Double_t GetWirePhi(int nw,double z=0)const {
       return (fFirstPhi+nw*fDeltaPhi)+
	 TMath::ATan(z/fR*fTStereoAngle);
     }
...
private:
  ...
  Int_t GetLayerFromId(ULong_t clid) const;
   ...
   void ReadSeeds(IlcESDEvent *event, Int_t direction);  //read seeds from the event
   ...
   //
   IlcESDEvent * fEvent;      // output with esd tracks

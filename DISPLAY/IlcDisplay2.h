...
#ifndef IlcDISPLAY2_H
#define IlcDISPLAY2_H
...
class IlcDisplay2 : public TObject{
...
public:
 ...
 IlcDisplay2(const TGWindow *p, UInt_t w, UInt_t h); //Constructor, load all the widgets
 virtual ~IlcDisplay2(void);
 ...
 IlcModuleInfo*                    GetModuleInfo() const {return fModuleInfo;};//
 ...
 void	       			LoadPoints(IlcDetector* detector, Int_t track);//Load the hits points
...
private:
 ...
 char                           *fIconsPath;//Icon Path (by default $ILC_ROOT/DISPLAY/icons/)
 ...
 IlcModuleInfo                     *fModuleInfo;//Pointer to the class which map module name and internal module ID, necessary because of the dynamic load of the available modules
	
 TEnv				*fIlcDisplay2rc;//ressources file (.ilcdisplay.rc)
 ...
 IlcSliderFrame			*fSliderFrame;//Frame which contains the rapidity and momentum sliders

 //Shutter Frame
 TGLayoutHints		        *fShutterFrameLayout; // Shutter Frame Layout
 IlcShutterFrame	        *fShutterFrame;// Shutter Frame

 //Display Frame
 TGLayoutHints	        	*fDisplayFrameLayout; // Display Frame Layout
 IlcDisplayFrame	        	*fDisplayFrame;// Display Frame

 //Info Frame
 TGLayoutHints	        	*fInfoFrameLayout; // Info Frame Layout
 IlcInfoFrame			*fInfoFrame; // Info Frame
 ...
 RQ_OBJECT("IlcDisplay2")

 ClassDef(IlcDisplay2,0);
};

R__EXTERN  IlcDisplay2* gIlcDisplay2;

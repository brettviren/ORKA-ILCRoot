...
/* $Id: IlcGenAmpt.h 54863 2012-02-28 12:10:29Z hristov $ */
...
class TClonesArray;
class TGraph;
class IlcGenHijingEventHeader;

class IlcGenAmpt : public IlcGenMC
{
  enum {kNoTrigger, kHardProcesses, kDirectPhotons};

  public:
    IlcGenAmpt();
    IlcGenAmpt(Int_t npart);
    virtual ~IlcGenAmpt();
  ...
  protected:
    ...
    IlcGenHijingEventHeader *fHeader;          // header
    Bool_t                   fDecay;           // decay "long-lived" particles
    Bool_t                   fRotating;        // Random reaction plane on/off
 

  private:
    IlcGenAmpt(const IlcGenAmpt &Ampt);
    IlcGenAmpt &  operator=(const IlcGenAmpt & rhs);
    ...
    ClassDef(IlcGenAmpt, 4) // IlcGenerator interface to Ampt
};
#endif

...
/* $Id: IlcGenAmpt.cxx 58196 2012-08-15 14:35:40Z hristov $ */
...
#define IlcGenAmptEventHeader IlcGenHijingEventHeader
...
IlcGenAmpt::IlcGenAmpt() 
  : IlcGenMC(),
    ...
    fStringB(0.9),
    fEventTime(0.),
    fHeader(new IlcGenAmptEventHeader("Ampt")),
    fDecay(kTRUE),
    fRotating(kFALSE)
{
  // Constructor
  fEnergyCMS = 2760.;
  IlcAmptRndm::SetAmptRandom(GetRandom());
}

IlcGenAmpt::IlcGenAmpt(Int_t npart)
  : IlcGenMC(npart),
    fDecayer(NULL),
    fFrame("CMS"),
    fMinImpactParam(0.),
...
void IlcGenAmpt::Generate()
{
  ...
  while(1) {
    ...
    if( fRotating ) {     
      TRandom *r=IlcAmptRndm::GetAmptRandom();
    ...
    if (decayer&&fDecay) {
      ...
      for( Int_t nLoop=0; nLoop!=2; ++nLoop) { // In order to produce more than one generation of decays: NumberOfNestedLoops set to 2
        ...
        if (np!=np2) {
          IlcError(Form("Something is fishy: %d %d\n", np,np2));
        }
      } // end of nLoop->NumberOfNestedLoops
    } else {
      if (fDecay)
        IlcError("No decayer found, but fDecay==kTRUE!");
    }
	...
	tof = time0 + iparticle->T();
        ...
        if (hasMother) {
          ...
          if(procID==999)
            procID = kPPrimary; // reseting to ILCROOT convention
          else
...
Bool_t IlcGenAmpt::IsThisAKnownParticle(TParticle *thisGuy)
{
...
void IlcGenAmpt::EvaluateCrossSections()
{
...
Bool_t IlcGenAmpt::DaughtersSelection(TParticle* iparticle)
{
...
Bool_t IlcGenAmpt::SelectFlavor(Int_t pid)
{
...
Bool_t IlcGenAmpt::Stable(TParticle* particle) const
{
...
Bool_t IlcGenAmpt::CheckTrigger()
{

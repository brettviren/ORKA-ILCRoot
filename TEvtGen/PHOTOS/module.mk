...
MODDIRS:=$(MODDIR)
MODDIRI:=$(MODDIR)
MODDIRO:=$(MODDIR)/tgt_$(ILC_TARGET)
MODDIRC:=$(MODDIR)/check
MODDIRZ:=$(MODDIR)/smell
PHOTOSDIRO:=PHOTOS/tgt_$(ILC_TARGET)
...
#ifneq (,$(findstring macosx,$(ILC_TARGET)))
#CSRCS+=lnblnk.c
#endif 
...
PACKFFLAGS := $(FFLAGS) -IPYTHIA6  -L$(ILC_ROOT)/lib/tgt_$(ILC_TARGET) -lpythia6
...
photosELIBSDEP:=$(patsubst %,lib/tgt_$(ILC_TARGET)/lib%.$(SOEXT),$(ELIBS))
...
ifndef ILCQUIET
		...
		$(MUTE)$(CXX) $(photosDEFINE) -c $(photosINC)  -I$(ILC_ROOT) $< -o $@ $(photosDCXXFLAGS)
...
$(photosCHV:.viol=.i): $(MODDIRC)/%.i: $(MODDIR)/%.cxx $(MODDIRO)/%.d
	...
	@cd $(dir $@) ; $(IRST_INSTALLDIR)/patch/patch4ilc.prl $(notdir $@)
...
ifndef ILCQUIET
		...
		$(MUTE)rm -rf PHOTOS/tgt_$(ILC_TARGET) 

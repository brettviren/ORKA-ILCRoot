 ...
 **************************************************************************/
///////////////////////////////////////////////////////////////////////////
//  Implementation of IlcDecayer using EvtGen package.                   //
//                                                                       //
...
ClassImp(IlcDecayerEvtGen)
//____________________________________________________________
IlcDecayerEvtGen::IlcDecayerEvtGen():
  fRandomEngine(0x0),
  fGenerator(0x0),
  fEvtstdhep(0x0),
  fDecayTablePath(0x0),
  ...
  fDecayTablePath = gSystem->ExpandPathName("$ILC_ROOT/TEvtGen/EvtGen/DECAY.DEC"); //default decay table
  fParticleTablePath = gSystem->ExpandPathName("$ILC_ROOT/TEvtGen/EvtGen/evt.pdl"); //particle table
  }
//_________________________________________________________________
IlcDecayerEvtGen::IlcDecayerEvtGen(const IlcDecayerEvtGen &decayer):
  IlcDecayer(decayer),
...
void IlcDecayerEvtGen::Init()
  {
  //Standard IlcDecayerEvtGen initializer:
  ...
  //
  if(fGenerator){
  IlcWarning(" IlcDecayerEvtGen already initialized!!!!\n");
  return;
  }
...
//____________________________________________________________
void IlcDecayerEvtGen::Decay(Int_t ipart, TLorentzVector *p)
  {
  //
  //Decay a particle
...
//____________________________________________________________
Int_t IlcDecayerEvtGen::ImportParticles(TClonesArray *particles)
  {
  //
  ...
  if(!partPDG)
    {
    IlcWarning("Particle code non known in TDatabasePDG - set pdg = 89");
  ...
  IlcDebug(1,Form("partnum = %d istat = %d primaMadre = %d ultimaMadre = %d primaF = %d ultimaF=%d x=%f y=%f z=%f t=%f e=%f px=%f \n",partnum,istat,jmotherfirst,jmotherlast,jdaugfirst,jdauglast,x,y,z,t,e,px));
...
void  IlcDecayerEvtGen::Copy(TObject &) const
  {
  //
  // Copy *this onto IlcDecayerEvtGen -- not implemented
  //
   Fatal("Copy","Not implemented!\n");
  }

void IlcDecayerEvtGen::ForceDecay()
  {
  //
  ...
  switch(decay)
    {
     ...
     SetDecayTablePath(gSystem->ExpandPathName("$ILC_ROOT/TEvtGen/EvtGen/DecayTable/BTOJPSITOELE.DEC"));
     break;
     case kBJpsi:
     SetDecayTablePath(gSystem->ExpandPathName("$ILC_ROOT/TEvtGen/EvtGen/DecayTable/BTOJPSI.DEC"));
     break;
     case kBJpsiDiMuon:
     SetDecayTablePath(gSystem->ExpandPathName("$ILC_ROOT/TEvtGen/EvtGen/DecayTable/BTOJPSITOMU.DEC"));
     break;
     case kBSemiElectronic:
     SetDecayTablePath(gSystem->ExpandPathName("$ILC_ROOT/TEvtGen/EvtGen/DecayTable/BTOELE.DEC"));
     break;  
     case kHardMuons:
     ...
     case kBeautyUpgrade:
     IlcWarning(Form("Warning: case %s not implemented for this class!",(Char_t)decay));
     break;
     }
     ReadDecayTable();
  }

Float_t IlcDecayerEvtGen::GetPartialBranchingRatio(Int_t)
  {
   // This method is dummy
   return  1.;
  }

Float_t IlcDecayerEvtGen::GetLifetime(Int_t kf)
  {    
  //
  ...
  IlcDebug(1,Form("lifetime is %f (mum) , particle id= %d",lifetime*1000,kf));
  return lifetime*kconv; //tau (sec)
  }

void IlcDecayerEvtGen::ReadDecayTable()
    {
...
////////////////////////////////////////////////////////////
Bool_t IlcDecayerEvtGen::SetDecayTablePath(Char_t *path)
  {
  // 
  ...
  {
   IlcWarning("Attention: This path not exist!\n");
   return kFALSE;
  }

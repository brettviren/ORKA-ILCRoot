...
ClassImp(IlcOADBFillingScheme)


IlcOADBFillingScheme::IlcOADBFillingScheme() : TNamed("IlcOADBFillingScheme", "OADB object storing filling scheme infos"), fFSName(""), fBXIds(0){
  // default ctor

  
}
IlcOADBFillingScheme::IlcOADBFillingScheme(char* name) : TNamed(name, "OADB object storing filling scheme infos"), fFSName(""), fBXIds(0){
  // ctor
  Init();
}

void IlcOADBFillingScheme::Init() {
  // initialize pointers
  fBXIds = new TMap();
  fFSName = "";
...
IlcOADBFillingScheme::~IlcOADBFillingScheme(){
  // dtor
...
// IlcOADBFillingScheme::IlcOADBFillingScheme(const IlcOADBFillingScheme& cont) {
//   // Copy ctor
//   IlcError("To be implemented");
// }

// IlcOADBFillingScheme& IlcOADBFillingScheme::operator=(const IlcOADBFillingScheme& cont) {
//   //Assignment operator
//   IlcError("To be implemented");
// }
...
const char *  IlcOADBFillingScheme::GetBXIDs(const char * beamSide) const 
{
  ...
  if( !strcmp(beamSide,"ACE") && !(TObjString*)fBXIds->GetValue(beamSide)) 
    IlcWarning("Beware, no correction for the BXIDs for ACE");
...
void IlcOADBFillingScheme::Browse(TBrowser *b)
{
   // Browse this object.
...
void IlcOADBFillingScheme::Print(Option_t* option) const {

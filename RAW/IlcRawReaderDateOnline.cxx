...
ClassImp(IlcRawReaderDateOnline)

Bool_t IlcRawReaderDateOnline::fgNoSleep = kTRUE;

IlcRawReaderDateOnline::IlcRawReaderDateOnline(
#ifdef ILC_DATE
				     const char* filename
#else
				     const char* /* filename */
...
// Initialize the DATE monitoring libs
  ...
  if (status!=0) {
    IlcFatal(Form("monitorSetDataSource() failed : %s",monitorDecodeError(status)));
  }

  /* declare monitoring program */
  status=monitorDeclareMp( __FILE__ );
  if (status!=0) {
    IlcFatal(Form("monitorDeclareMp() failed : %s",monitorDecodeError(status)));
  }
  ...
  if (!fTable) {
    ...
    IlcInfo("Custom monitoring table:");
    Int_t index = 0;
    while (fTable[index] != NULL) {
      IlcInfo(Form("%s %s %s %s",fTable[index],fTable[index+1],fTable[index+2],fTable[index+3]));
      index += 4;
    }
    monitorDeclareTableExtended(const_cast<char**>(fTable));
  }
  // install SIGUSR1 handler to allow clean end-of-events loop
  gSystem->AddSignalHandler(new IlcRawReaderDateIntHandler(this));

#else
  Fatal("IlcRawReaderDateOnline", "this class was compiled without DATE");
#endif
}

Bool_t IlcRawReaderDateOnline::NextEvent()
{
...
// from shared memory
  ...
  // Stop on SIGUSR1
  if (fStop) {
    IlcInfo("Raw-data reading stopped by SIGUSR1");
  ...
  if (IlcRawReaderDate::NextEvent()) return kTRUE;
  ...
  while (1) {
    ...
    if (status!=0) {
      IlcError(Form("monitorGetEventDynamic() failed : %s\n",monitorDecodeError(status)));
      Reset();
      fEvent = NULL;
      return kFALSE;
    }
    ...
    IlcInfo(Form("Run #%lu, event size: %lu, BC:0x%x, Orbit:0x%x, Period:0x%x",
...
void IlcRawReaderDateOnline::SelectEvents(Int_t type,
...
#endif
  IlcRawReader::SelectEvents(type,triggerMask,triggerExpr);
}

//______________________________________________________________________________
void IlcRawReaderDateOnline::Stop()
{

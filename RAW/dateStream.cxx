...
#define CDH_TRIGGER_INFORMATION_UNAVAILABLE_MASK (1<<CDH_TRIGGER_INFORMATION_UNAVAILABLE_BIT)
	gotIlcTrigger = (cdhRef->cdhStatusErrorBits & CDH_TRIGGER_INFORMATION_UNAVAILABLE_MASK) == 0;
	if ( gotIlcTrigger && workingMode == fixedTarget ) {
	  fprintf( stderr,
		   "%s: ILC trigger and fixed target mode are not compatible.\n\
...
L1TriggerMessage:0x%x ILCTrigger:%s\n",
		       myName,
		       cdh->cdhL1TriggerMessage,
		       gotIlcTrigger ? "yes" : "no" );
	      exit( 1 );
	    }
	  }
	}

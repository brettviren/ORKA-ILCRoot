...
ClassImp(IlcAltroRawStreamV3)


//_____________________________________________________________________________
IlcAltroRawStreamV3::IlcAltroRawStreamV3(IlcRawReader* rawReader) :
...
//_____________________________________________________________________________
IlcAltroRawStreamV3::~IlcAltroRawStreamV3()
{
// destructor
...
//_____________________________________________________________________________
IlcAltroRawStreamV3::IlcAltroRawStreamV3(const IlcAltroRawStreamV3& stream) :
  TObject(stream),
  ...
  fFormatVersion(0)
{
  ...
  if (stream.fOldStream)
    fOldStream = new IlcAltroRawStream(*stream.fOldStream);
}

//_____________________________________________________________________________
IlcAltroRawStreamV3& IlcAltroRawStreamV3::operator = (const IlcAltroRawStreamV3& stream)
{
  // assignment operator
  // ... 
  ...
  if (stream.fOldStream) {
    if (fOldStream) delete fOldStream;
    fOldStream = new IlcAltroRawStream(stream.fRawReader);
...
//_____________________________________________________________________________
void IlcAltroRawStreamV3::Reset()
{
...
//_____________________________________________________________________________
Bool_t IlcAltroRawStreamV3::NextDDL()
{
...
// two sources
  ...
  if (fFormatVersion < 2) {
    ...
    if (!fOldStream) {
      fOldStream = new IlcAltroRawStream(fRawReader);
      IlcInfo(Form("RCU firmware verion %d detected. Using IlcAltroRawStream to decode the data.",
		   fFormatVersion));
    }
...
//_____________________________________________________________________________
Bool_t IlcAltroRawStreamV3::NextChannel()
{
  ...
  for (Int_t iword = 0; iword < nwords; iword++) {
    ...
    if ((word >> 30) != 0) {
      ...
      IlcWarning(Form("Unexpected end of payload in altro channel payload! DDL=%03d, Address=0x%x, word=0x%x",
		      fDDLNumber,fHWAddress,word));
      fRawReader->AddMinorErrorLog(kAltroPayloadErr,Form("hw=0x%x",fHWAddress));
      if (IlcDebugLevel() > 0) HexDumpChannel();
      fCount = -1;
      fPosition--;
      return kFALSE;
    }
...
//_____________________________________________________________________________
Bool_t IlcAltroRawStreamV3::NextBunch()
{
  ...
  if (fBunchLength <= 2) {
    // Invalid bunch size
     IlcDebug(1,Form("Too short bunch length (%d) @ %d in Address=0x%x (DDL=%03d)!",
  ...
  if ((fBunchDataIndex + fBunchLength) > fCount) {
    ...
    IlcDebug(1,Form("Too long bunch detected @ %d in Address=0x%x (DDL=%03d) ! Expected <= %d 10-bit words, found %d !", fBunchDataIndex,
  ...
  if (fCheckAltroPayload) {
    if ((fStartTimeBin-fBunchLength+1) < 0) {
      IlcWarning(Form("Invalid start time-bin @ %d in Address=0x%x (DDL=%03d)! (%d-%d+1) < 0", fBunchDataIndex-1,
    ...
    if (fStartTimeBin >= prevTimeBin) {
      IlcWarning(Form("Invalid start time-bin @ %d in Address=0x%x (DDL=%03d)! (%d>=%d)", fBunchDataIndex-1,
...
//_____________________________________________________________________________
const UChar_t *IlcAltroRawStreamV3::GetChannelPayload() const
{
...
//_____________________________________________________________________________
UInt_t IlcAltroRawStreamV3::Get32bitWord(Int_t index) const
{
...
///_____________________________________________________________________________
Bool_t IlcAltroRawStreamV3::ReadRCUTrailer(UChar_t rcuVer)
{
  ...
  if ((word >> 30) != 3) {
    ...
    IlcError("Last RCU trailer word not found!");
    return kFALSE;
  }
  ...
  if (ver != rcuVer) {
    ...
    fRawReader->AddMajorErrorLog(kRCUVerErr,Form("%d!=%d",
						 ver,rcuVer));
    IlcDebug(1,Form("Wrong RCU firmware version detected: %d != %d",
		    ver,rcuVer));
  }
  ...
  for (; trailerSize > 0; trailerSize--) {
    ...
    if ((word >> 30) != 2) {
      ...
      IlcWarning("Missing RCU trailer identifier pattern!");
      continue;
    }
  ...
  if ((fRawReader->GetDataSize() - fRCUTrailerSize - 8) != (fPayloadSize*4)) {
    fRawReader->AddMajorErrorLog(kRCUTrailerSizeErr,Form("h=%d tr=%d rcu=%d bytes",
							 ...
							 fRCUTrailerSize+8,
							 fPayloadSize*4));
    IlcWarning(Form("Inconsistent raw data size ! Raw data size - %d bytes (from CDH), RCU trailer - %d bytes, raw data size (from RCU trailer) - %d bytes !",
		    fRawReader->GetDataSize(),
		    fRCUTrailerSize+8,
		    fPayloadSize*4));
  }
...
//_____________________________________________________________________________
Int_t IlcAltroRawStreamV3::GetBranch() const
{
...
//_____________________________________________________________________________
Int_t IlcAltroRawStreamV3::GetFEC() const
{
...
//_____________________________________________________________________________
Int_t IlcAltroRawStreamV3::GetAltro() const
{
...
//_____________________________________________________________________________
Int_t IlcAltroRawStreamV3::GetChannel() const
{
...
//_____________________________________________________________________________
Bool_t IlcAltroRawStreamV3::GetRCUTrailerData(UChar_t*& data) const
{
...
//_____________________________________________________________________________
void IlcAltroRawStreamV3::PrintRCUTrailer() const
{
...
//_____________________________________________________________________________
void IlcAltroRawStreamV3::SelectRawData(Int_t detId)
{
...
//_____________________________________________________________________________
void IlcAltroRawStreamV3::SelectRawData(const char *detName)
{
  // Select the raw data for specific
  // detector name
...
//_____________________________________________________________________________
Double_t IlcAltroRawStreamV3::GetTSample() const
{
...
//_____________________________________________________________________________
Double_t IlcAltroRawStreamV3::GetL1Phase() const
{
...
//_____________________________________________________________________________
void IlcAltroRawStreamV3::AddMappingErrorLog(const char *message)
{
...
//_____________________________________________________________________________
UChar_t *IlcAltroRawStreamV3::GetRCUPayloadInSOD() const
{
...
//_____________________________________________________________________________
Int_t IlcAltroRawStreamV3::GetRCUPayloadSizeInSOD() const
{
...
void IlcAltroRawStreamV3::HexDumpChannel() const
{

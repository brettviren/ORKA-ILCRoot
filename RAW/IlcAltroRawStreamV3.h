#ifndef ILCALTRORAWSTREAMV3_H
#define ILCALTRORAWSTREAMV3_H
...
class IlcRawReader;
class IlcAltroRawStream;

class IlcAltroRawStreamV3: public TObject {
  public :
    IlcAltroRawStreamV3(IlcRawReader* rawReader);
    virtual ~IlcAltroRawStreamV3();

    IlcAltroRawStreamV3& operator = (const IlcAltroRawStreamV3& stream);
    IlcAltroRawStreamV3(const IlcAltroRawStreamV3& stream);
  ...
  private:
    ...
    IlcAltroRawStream* fOldStream;  // streamer for old altro format

    Bool_t           fCheckAltroPayload; // check altro payload correctness or not?
    UChar_t          fFormatVersion;

    ClassDef(IlcAltroRawStreamV3, 0)  // base class for reading Altro raw digits
};

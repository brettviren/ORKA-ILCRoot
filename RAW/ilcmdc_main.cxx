// @(#)ilcmdc:$Name:  $:$Id: ilcmdc_main.cxx 26401 2008-06-04 16:34:44Z cvetan $
...
static void IlcMDCErrorHandler(int level, Bool_t abort, const char *location,
                               const char *msg)
{
...
static void SMI_handle_command()
{
   ...
   if (strcmp(action, "STOP") == 0) {
      if (IlcMDC::Instance()) IlcMDC::Instance()->SetStopLoop();
   }
...
int main(int argc, char **argv)
{
   ...
   IlcLog::SetHandleRootMessages(kFALSE);
   SetErrorHandler(IlcMDCErrorHandler);
   ...
   if (gSystem->Getenv("ILCMDC_RFIO")) 
     rfioFS = gSystem->Getenv("ILCMDC_RFIO");
   if (gSystem->Getenv("ILCMDC_CASTOR")) 
     castorFS = gSystem->Getenv("ILCMDC_CASTOR");
   if (gSystem->Getenv("ILCMDC_ROOTD")) 
     rootdFS = gSystem->Getenv("ILCMDC_ROOTD");
   ...
   IlcMDC::EWriteMode wmode = IlcMDC::kLOCAL;
   Int_t    filterMode = 0;
   ...
   if (argv[iarg][0] == '-') {
      ...
      wmode = IlcMDC::kCASTOR;
      compress = atoi(argv[iarg]+1);
      fs1 = castorFS;
   } else if (argv[iarg][0] == '@') {
      wmode = IlcMDC::kDEVNULL;
      compress = atoi(argv[iarg]+1);
   } else {
   ...
   IlcMDC mdcproc(compress, delFiles, IlcMDC::EFilterMode(filterMode), 

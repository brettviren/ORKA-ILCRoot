// @(#) $Id: IlcTagDB.cxx 28621 2008-09-09 12:22:01Z hristov $
...
IlcTagDB::IlcTagDB(IlcRawEventTag *eventTag, const char* fileName) :
  fTagDB(NULL),
  fTree(NULL),
  fEventTag(eventTag),
  fMaxSize(-1),
...
//______________________________________________________________________________
Bool_t IlcTagDB::Create(const char* fileName)
{
   // Create a new tag DB.
   ...
   fTagDB = new TFile(name, "RECREATE",
                      Form("ILC tag DB (%s)", IlcRawDB::GetIlcRootTag()), 1);
   if (fTagDB->IsZombie()) {
   ...
   fTree = new TTree("T", Form("ILC raw-data tag tree (%s)", IlcRawDB::GetIlcRootTag()));
...
//______________________________________________________________________________
Bool_t IlcTagDB::NextFile(const char* fileName)
{
...
//______________________________________________________________________________
void IlcTagDB::SetFS(const char* fs)
{
...
//______________________________________________________________________________
Float_t IlcTagDB::GetCompressionFactor() const
{
...
//______________________________________________________________________________
const char *IlcTagDB::GetFileName() const
{
   ...
   Int_t runNumber = -1;
   if (fEventTag) {
     IlcRawEventHeaderBase *header = fEventTag->GetHeader();

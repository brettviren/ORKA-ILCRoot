void IlcVertexerTracksTest(TString outname="IlcVertexerTracksTest.root",
...
//
  ...
  if (retval) {
    cerr<<"LoadgIlc returned error"<<endl;
    delete rl;
    return;
  }
  ...
  // -----------   Create vertexer --------------------------
  IlcESDVertex *initVertex = 0;
  if(useMeanVtx) {
    // open the mean vertex
    TFile *invtx = new TFile("IlcESDVertexMean.root");
    initVertex = (IlcESDVertex*)invtx->Get("vtxmean");
    invtx->Close();
    delete invtx;
  } else {
    Double_t pos[3]={0.5,0.5,0.}; 
    Double_t err[3]={3.,3.,5.};
    initVertex = new IlcESDVertex(pos,err);
  }
  IlcVertexerTracks *vertexer = new IlcVertexerTracks(IlcTracker::GetBz());
  ...
  if(gSystem->AccessPathName("IlcESDs_itstpc.root",kFileExists)) {
    printf("IlcESDs_itstpc.root not found!\n"); 
    return;
  }
  TFile *fin = TFile::Open("IlcESDs_itstpc.root");
  ...
  for(Int_t iev=0; iev<events; iev++) { //LOOP ON EVENTS
    ...
    Int_t npart = (Int_t)gIlc->GetEvent(iev);
    printf("particles  %d\n",npart);
    IlcHeader *header = (IlcHeader*)rl->GetHeader();
    IlcGenPythiaEventHeader *pyheader = (IlcGenPythiaEventHeader*)header->GenEventHeader();
    ...
    // get # of tracklets
    IlcMultiplicity *ilcmult = esd->GetMultiplicity();
    if(ilcmult) {
      ntracklets = ilcmult->GetNumberOfTracklets();
      for(Int_t l=0;l<ilcmult->GetNumberOfTracklets();l++) 
	if(ilcmult->GetDeltaPhi(l)<-9998.) ntracklets--;
    } else {
      ntracklets = 0;
    }
    ...
    // VERTEX    
    IlcESDVertex *vertex = 0;
    if(!tpconly) {
      vertex = (IlcESDVertex*)vertexer->FindPrimaryVertex(esd);
    } else {
	...
	IlcESDtrack *esdt = esd->GetTrack(i);
	IlcExternalTrackParam *tpcTrack =
	  (IlcExternalTrackParam *)esdt->GetTPCInnerParam();
	ok = kFALSE;
	if (tpcTrack)
	  ok = IlcTracker::
	...
	if (ok) {
      ...
      vertex = (IlcESDVertex*)vertexer->FindPrimaryVertex(&trkArrayOrig,idOrig);
      delete [] idOrig;
    }
    ...
    for(Int_t itrk=0; itrk<ntracks; itrk++) {
      IlcESDtrack *esdtrack = (IlcESDtrack*)esd->GetTrack(itrk);
      UInt_t status = esdtrack->GetStatus();
      // only tracks found also in ITS
      if(! (status&IlcESDtrack::kITSin) ) continue;      
      nitstracks++;
...
//------------------------------------------------------------------------
  ...
  if(gSystem->AccessPathName("IlcESDs.root",kFileExists)) {
    printf("IlcESDs.root not found!\n"); 
    return;
  }
  TFile *fin = TFile::Open("IlcESDs.root");
  IlcESDEvent *esd = new IlcESDEvent();
  TTree *esdTree = (TTree*)fin->Get("esdTree");
  if(!esdTree) return;
  esd->ReadFromTree(esdTree);
  ...
  // VERTEX    
  IlcESDVertex *vertex = (IlcESDVertex*)vertexer->FindPrimaryVertex(esd);
  nc = (Int_t)vertex->GetNContributors();
  ...
  for(Int_t itrk=0; itrk<ntracks; itrk++) {
    ...
    UInt_t status = esdtrack->GetStatus();
    // only tracks found also in ITS
    if(! (status&IlcESDtrack::kITSrefit) ) continue;      
    nitstracks++;
    Int_t npts = (Int_t)esdtrack->GetNcls(0);
    if(npts==6) {nitstracks6++;nitstracks5or6++;}
    if(npts==5) {nitstracks5++;nitstracks5or6++;}
...
//----------------------------------------------------------------------------
void PlotVtxRes(TString inname="IlcVertexerTracksTest.root",
...
void TestRmTracks(Int_t iev=0) {
  ...
  // example: do vertex without tracks 1 and 2 (IlcESDtrack::GetID())
  //
  ...
  IlcESDVertex *vertex1 = (IlcESDVertex*)vertexer->FindPrimaryVertex(esd);
  vertex1->PrintStatus();
  ...
  skipped[1]=-99;
  vertexer->SetSkipTracks(nskip,skipped);  
  IlcESDVertex *vertex2 = (IlcESDVertex*)vertexer->FindPrimaryVertex(esd);
  vertex2->PrintStatus();
  ...
  IlcESDtrack *esdTrack = 0;
  trksTree->Branch("tracks","IlcESDtrack",&esdTrack);
  IlcESDtrack *et = esd->GetTrack(1);
  esdTrack = new IlcESDtrack(*et);
  trksTree->Fill();
  delete esdTrack;
  IlcESDtrack *et = esd->GetTrack(2);
  esdTrack = new IlcESDtrack(*et);
  trksTree->Fill();
  delete esdTrack;
  IlcVertexerTracks vt(IlcTracker::GetBz());
  IlcESDVertex *vertex3 = vt.RemoveTracksFromVertex(vertex2,trksTree,diamondxy);
  vertex3->PrintStatus();
...
//----------------------------------------------------------------------------
void IlcComputeVtxMeanFromESD(TString file="IlcESDs.root",  
			      Int_t mincontr=20) {
  //-----------------------------------------------------------------------
  // Compute weighted mean and cov. matrix from a set of IlcESD
  //-----------------------------------------------------------------------
  ...
  for(Int_t i=1; i<100; i++) {
    ...
    for(Int_t iev=0; iev<events; iev++) { //LOOP ON EVENTS
      esdTree->GetEvent(iev);
      vertex = (IlcESDVertex*)esd->GetPrimaryVertex();
      if(!vertex) continue;
  ...
  covvtx[4] = covyz;
  covvtx[5] = 5.3;
  IlcESDVertex *vtxmean = new IlcESDVertex(vtx,covvtx,0.,(Int_t)sum);
  vtxmean->SetTitle("Vertex from weighted average");
  TFile *out = new TFile("IlcESDVertexMean.root","recreate");
...
//----------------------------------------------------------------------------
void IlcComputeVtxMeanFromTree(TString file="IlcESDVertices.root",  
			       Int_t mincontr=20) {
  //-----------------------------------------------------------------------
  // Compute weighted mean and cov. matrix from a set of IlcESDVertices
  //-----------------------------------------------------------------------
  gStyle->SetOptStat(0);
  //gStyle->SetOptFit(0);
  ...
  IlcESDVertex *vertex = 0;
  tree->SetBranchAddress("IlcESDVertex",&vertex);

#ifndef ILCGENERATOR_H
#define ILCGENERATOR_H
/* 2012-2013 Infrastructure for Large Collider Experiment.       *
 * All rights reserved.                                                   *
 * See cxx source for full Copyright notice                               */

/* $Id: IlcGenerator.h 55692 2012-04-11 09:03:15Z hristov $ */
...
class IlcGenerator : public TNamed, public IlcRndm
{

 public:

    IlcGenerator();
    IlcGenerator(Int_t npart);
    virtual ~IlcGenerator();
    virtual void Init();
    ...
    void SetVertexGenerator(IlcVertexGenerator* vertexGenerator)
    ...
    virtual void SetContainer(IlcGenerator* container) {fContainer = container;}
  ...
  // Stack 
    void SetStack (IlcStack *stack) {fStack = stack;}
    IlcStack* GetStack(){return fStack;}
  // Collision Geometry
    virtual Bool_t ProvidesCollisionGeometry() const {return kFALSE;}
    virtual Bool_t NeedsCollisionGeometry()    const {return kFALSE;}    
    virtual IlcCollisionGeometry* CollisionGeometry() const {return fCollisionGeometry;}
    virtual void SetCollisionGeometry(IlcCollisionGeometry* geom) {fCollisionGeometry = geom;}
 ...
 protected:
   ...
   //
    ...
    IlcVertexGenerator* fVertexGenerator;  //! Generator for the vertex
    ...
    IlcStack*   fStack;         //! Local pointer to stack
    IlcGenerator* fContainer;   //! Local pointer to container
    
    IlcCollisionGeometry* fCollisionGeometry; //!Collision geometry
    /*************************************************************************/
 ...
 private:
    IlcGenerator(const IlcGenerator &gen);
    IlcGenerator & operator=(const IlcGenerator &gen);

    ClassDef(IlcGenerator,5) // Base class for event generators
};

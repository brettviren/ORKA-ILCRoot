...
/* and IlcMillepede class by Javier                                                           */ 
...
ClassImp(IlcMillePede2)

Bool_t   IlcMillePede2::fgInvChol        = kTRUE;     // Invert global matrix with Cholesky solver
Bool_t   IlcMillePede2::fgWeightSigma    = kTRUE;     // weight local constraint by module statistics
Bool_t   IlcMillePede2::fgIsMatGloSparse = kFALSE;    // use faster dense matrix by default
Int_t    IlcMillePede2::fgMinResCondType = 1;         // Jacoby preconditioner by default
Double_t IlcMillePede2::fgMinResTol      = 1.e-11;    // default tolerance
Int_t    IlcMillePede2::fgMinResMaxIter  = 10000;     // default max number of iterations 
Int_t    IlcMillePede2::fgIterSol        = IlcMinResSolve::kSolMinRes; // default iterative solver
Int_t    IlcMillePede2::fgNKrylovV       = 240;        // default number of Krylov vectors to keep

//_____________________________________________________________________________________________
IlcMillePede2::IlcMillePede2() 
: fNLocPar(0),
  fNGloPar(0),
  fNGloSize(0),
//
...
//
  ...
  fFillValue(0),
  //
  fRecDataTreeName("IlcMillePedeRecords_Data"),
  fRecConsTreeName("IlcMillePedeRecords_Consaints"),
...
//_____________________________________________________________________________________________
IlcMillePede2::IlcMillePede2(const IlcMillePede2& src) : 
...
//_____________________________________________________________________________________________
Int_t IlcMillePede2::InitMille(int nGlo, int nLoc, int lNStdDev,double lResCut, double lResCutInit)
{
  // init all
  ...
  fNGloSize = fNGloPar;
  //
  if (fgIsMatGloSparse) {fMatCGlo = new IlcMatrixSparse(fNGloPar); fMatCGlo->SetSymmetric(kTRUE);}
  else                   fMatCGlo = new IlcSymMatrix(fNGloPar);
  //
  fFillIndex    = new Int_t[fNGloPar];
  fFillValue    = new Double_t[fNGloPar];
  //
  fMatCLoc      = new IlcSymMatrix(fNLocPar);
  fMatCGloLoc   = new IlcRectMatrix(fNGloPar,fNLocPar);
  //
...
//_____________________________________________________________________________________________
Bool_t IlcMillePede2::ImposeDataRecFile(const char* fname)
{
...
//_____________________________________________________________________________________________
Bool_t IlcMillePede2::ImposeConsRecFile(const char* fname)
{
...
//_____________________________________________________________________________________________
Bool_t IlcMillePede2::InitDataRecStorage(Bool_t read)
{
  // initialize the buffer for processed measurements records
  //
  if (fTreeData) {IlcInfo("Data Records File is already initialized"); return kFALSE;} 
  //
  if (!fRecord) fRecord = new IlcMillePedeRecord();
  //
  if (!read) { // write mode: cannot use chain
    fDataRecFile = TFile::Open(GetDataRecFName(),"recreate");
    if (!fDataRecFile) {IlcFatal(Form("Failed to initialize data records file %s",GetDataRecFName())); return kFALSE;}
    IlcInfo(Form("File %s used for derivatives records",GetDataRecFName()));
    fTreeData = new TTree(GetRecDataTreeName(),"Data Records for IlcMillePede2");
    fTreeData->Branch(GetRecDataBranchName(),"IlcMillePedeRecord",&fRecord,32000,99);
  }
  else { // use chain
    ...
    if (fDataRecFName.EndsWith(".root")) ch->AddFile(fDataRecFName);
      ...
      if (!inpf.good()) {IlcInfo(Form("Failed on input records list %s\n",fDataRecFName.Data())); return kFALSE;}
      //
      TString recfName;
    ...
    if (nent<1) { IlcInfo("Obtained chain is empty"); return kFALSE;}
    fTreeData = ch;
    fTreeData->SetBranchAddress(GetRecDataBranchName(),&fRecord);
    IlcInfo(Form("Found %lld derivatives records",nent));
  }
  fCurrRecDataID = -1;
...
//_____________________________________________________________________________________________
Bool_t IlcMillePede2::InitConsRecStorage(Bool_t read)
{
  // initialize the buffer for processed measurements records
  //
  if (fConsRecFile) {IlcInfo("Constraints Records File is already initialized"); return kFALSE;} 
  //
  if (!fRecord) fRecord = new IlcMillePedeRecord();
  //
  fConsRecFile = TFile::Open(GetConsRecFName(),read ? "":"recreate");
  if (!fConsRecFile) {IlcInfo(Form("Failed to initialize constraints records file %s",GetConsRecFName())); return kFALSE;}
  //
  IlcInfo(Form("File %s used for constraints records",GetConsRecFName()));
  if (read) {
    fTreeConstr = (TTree*)fConsRecFile->Get(GetRecConsTreeName());
    if (!fTreeConstr) {IlcInfo(Form("Did not find constraints records tree in %s",GetConsRecFName())); return kFALSE;}
    fTreeConstr->SetBranchAddress(GetRecConsBranchName(),&fRecord);
    IlcInfo(Form("Found %lld constraints records",fTreeConstr->GetEntries()));
    //
  }
  else {
    //
    fTreeConstr = new TTree(GetRecConsTreeName(),"Constraints Records for IlcMillePede2");
    fTreeConstr->Branch(GetRecConsBranchName(),"IlcMillePedeRecord",&fRecord,32000,99);
  }
  fCurrRecConstrID = -1;
  //
  return kTRUE;
...
//_____________________________________________________________________________________________
void IlcMillePede2::CloseDataRecStorage()
{
  // close records file
  if (fTreeData) {
...
//_____________________________________________________________________________________________
void IlcMillePede2::CloseConsRecStorage()
{
...
//_____________________________________________________________________________________________
Bool_t IlcMillePede2::ReadNextRecordData()
{
...
//_____________________________________________________________________________________________
Bool_t IlcMillePede2::ReadNextRecordConstraint()
{
...
//_____________________________________________________________________________________________
void IlcMillePede2::SetRecordWeight(double wgh)
{
  // assign weight
...
//_____________________________________________________________________________________________
void IlcMillePede2::SetRecordRun(Int_t run)
{
  // assign run
  if (fRecFileStatus<2) InitDataRecStorage(); // create a buffer to store the data
...
//_____________________________________________________________________________________________
void IlcMillePede2::SetLocalEquation(double *dergb, double *derlc, double lMeas, double lSigma)
{
...
//_____________________________________________________________________________________________
void IlcMillePede2::SetLocalEquation(int *indgb, double *dergb, int ngb, int *indlc,
...
//_____________________________________________________________________________________________
void IlcMillePede2::SetGlobalConstraint(const double *dergb, double val, double sigma)
{	
...
//_____________________________________________________________________________________________
void IlcMillePede2::SetGlobalConstraint(const int *indgb, const double *dergb, int ngb, double val,double sigma)
{	
  // Define a constraint equation.
  if (!fConsRecFile || !fConsRecFile->IsWritable()) InitConsRecStorage(); // create a buffer to store the data
  fRecord->Reset();
...
//_____________________________________________________________________________________________
Int_t IlcMillePede2::LocalFit(double *localParams)
{
  /*
  ...
  */
  ...
  int nPoints = 0;
  //
  IlcSymMatrix    &matCLoc    = *fMatCLoc;
  IlcMatrixSq     &matCGlo    = *fMatCGlo;
  IlcRectMatrix   &matCGloLoc = *fMatCGloLoc;
  //
  ...
  */
  ...
  if (!matCLoc.SolveChol(fVecBLoc,kTRUE)) {
    IlcInfo("Failed to solve locals by Cholesky, trying Gaussian Elimination");
    if (!matCLoc.SolveSpmInv(fVecBLoc,kTRUE)) { 
      IlcInfo("Failed to solve locals by Gaussian Elimination, skip...");
      matCLoc.Print("d");
...
//_____________________________________________________________________________________________
Int_t IlcMillePede2::GlobalFit(Double_t *par, Double_t *error, Double_t *pull)
{
  ...
  //
  sw.Stop();
  IlcInfo(Form("Global fit %s, CPU time: %.1f",res ? "Converged":"Failed",sw.CpuTime()));  
  if (!res) return 0;
  //
...
//_____________________________________________________________________________________________
Int_t IlcMillePede2::GlobalFitIteration()
{
  // perform global parameters fit once all the local equations have been fitted
  //
  IlcInfo(Form("Global Fit Iteration#%2d (Local Fit Chi^2 cut factor: %.2f)",fIter,fChi2CutFactor));
  //
  if (!fNGloPar || !fTreeData) {
    IlcInfo("No data was stored, stopping iteration");
    return 0;
  }
  TStopwatch sw,sws; 
  sw.Start();
  ...
  ndr = last - first;
  //
  IlcInfo(Form("Building the Global matrix from data records %ld : %ld",first,last));
  if (ndr<1) return 0;
  //
  ...
  */
  ...
  for (int i=0; i<fNGloConstraints; i++) {
    ...
    if (nSuppressed==csize) {
      //      IlcInfo(Form("Neglecting constraint %d of %d derivatives since no free parameters left",i,csize));
      //
  ...
  //
  IlcInfo(Form("Obtained %-7ld equations from %-7ld records (%-7ld rejected). Fixed %-4d globals",
  ...
  */
  ...
  //
  sw.Stop();
  IlcInfo(Form("Iteration#%2d %s. CPU time: %.1f",fIter,fGloSolveStatus==kFailed ? "Failed":"Converged",sw.CpuTime()));
...
//_____________________________________________________________________________________________
Int_t IlcMillePede2::SolveGlobalMatEq()
{
  //
  ...
  */
  ...
  if (!fgIsMatGloSparse) {
    ...
    if (fNLagrangeConstraints==0) { // pos-def systems are faster to solve by Cholesky
      if ( ((IlcSymMatrix*)fMatCGlo)->SolveChol(fVecBGlo, fgInvChol) ) return fgInvChol ? kInvert:kNoInversion;
      else IlcInfo("Solution of Global Dense System by Cholesky failed, trying Gaussian Elimiation");
    }
    //
    if (((IlcSymMatrix*)fMatCGlo)->SolveSpmInv(fVecBGlo, kTRUE)) return kInvert;
    else IlcInfo("Solution of Global Dense System by Gaussian Elimination failed, trying iterative methods");
  }
  // try to solve by minres
  TVectorD sol(fNGloSize);
  //
  IlcMinResSolve *slv = new IlcMinResSolve(fMatCGlo,fVecBGlo);
  if (!slv) return kFailed;
  //
  Bool_t res = kFALSE;
  if      (fgIterSol == IlcMinResSolve::kSolMinRes) 
    res =  slv->SolveMinRes(sol,fgMinResCondType,fgMinResMaxIter,fgMinResTol);
  else if (fgIterSol == IlcMinResSolve::kSolFGMRes) 
    res =  slv->SolveFGMRES(sol,fgMinResCondType,fgMinResMaxIter,fgMinResTol,fgNKrylovV);
  else 
    IlcInfo(Form("Undefined Iteritive Solver ID=%d, only %d are defined",fgIterSol,IlcMinResSolve::kNSolvers));
  //
  if (!res) {
    const char* faildump = "fgmr_failed.dat";
    int defout = dup(1);
    if (defout<0) {
      IlcInfo("Failed on dup");
      return kFailed;
    }
    ...
    else IlcInfo("Failed on file open for matrix dumping");
    close(defout);
    return kFailed;
  }
...
//_____________________________________________________________________________________________
Float_t IlcMillePede2::Chi2DoFLim(int nSig, int nDoF) const
{
...
//_____________________________________________________________________________________________
Int_t IlcMillePede2::SetIterations(double lChi2CutFac)
{
  // Number of iterations is calculated from lChi2CutFac 
  fChi2CutFactor = TMath::Max(1.0, lChi2CutFac);
  //
  IlcInfo(Form("Initial cut factor is %f",fChi2CutFactor));
  fIter = 1; // Initializes the iteration process
  return 1;
}

//_____________________________________________________________________________________________
Double_t IlcMillePede2::GetParError(int iPar) const
{
...
//_____________________________________________________________________________________________
Int_t IlcMillePede2::PrintGlobalParameters() const
{
  ...
  IlcInfo("");
  IlcInfo("   Result of fit for global parameters");
  IlcInfo("   ===================================");
  IlcInfo("    I       initial       final       differ        lastcor        error       gcor       Npnt");
  IlcInfo("----------------------------------------------------------------------------------------------");
  //
  for (int i=0; i<fNGloPar; i++) {
    ...
    if (fGloSolveStatus==kInvert && TMath::Abs( (dg=fMatCGlo->QueryDiag(i)) *fDiagCGlo[i]) > 0) {    
      ...
      IlcInfo(Form("%d\t %.6f\t %.6f\t %.6f\t %.6f\t %.6f\t %.6f\t %6d",
		   i,fInitPar[i],fInitPar[i]+fDeltaPar[i],fDeltaPar[i],fVecBGlo[i],lError,lGlobalCor,fProcPnt[i]));
    }
    else {    
      IlcInfo(Form("%d\t %.6f\t %.6f\t %.6f\t %.6f\t OFF\t OFF\t %6d",i,fInitPar[i],fInitPar[i]+fDeltaPar[i],
...
//_____________________________________________________________________________________________
Bool_t IlcMillePede2::IsRecordAcceptable() const
{
...
//_____________________________________________________________________________________________
void IlcMillePede2::SetRejRunList(const UInt_t *runs, Int_t nruns)
{
...
//_____________________________________________________________________________________________
void IlcMillePede2::SetAccRunList(const UInt_t *runs, Int_t nruns)
{

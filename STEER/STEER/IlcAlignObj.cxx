...
/* $Id: IlcAlignObj.cxx 50615 2011-07-16 23:19:19Z hristov $ */

//-----------------------------------------------------------------
//  Implementation of the alignment object class, holding the alignment
//  constants for a single volume, through the abstract class IlcAlignObj.
//  From it two derived concrete representation of alignment object class
//  (IlcAlignObjParams, IlcAlignObjMatrix) are derived in separate files.
//-----------------------------------------------------------------
...
ClassImp(IlcAlignObj)

//_____________________________________________________________________________
IlcAlignObj::IlcAlignObj():
  TObject(),
  fVolPath(),
  fVolUID(0)
{
...
//_____________________________________________________________________________
IlcAlignObj::IlcAlignObj(const char* symname, UShort_t voluid) :
  TObject(),
  fVolPath(symname),
  fVolUID(voluid)
{
...
//_____________________________________________________________________________
IlcAlignObj::IlcAlignObj(const char* symname, UShort_t voluid, Double_t* cmat) :
  TObject(),
  fVolPath(symname),
  fVolUID(voluid)
{
...
//_____________________________________________________________________________
IlcAlignObj::IlcAlignObj(const IlcAlignObj& theAlignObj) :
  TObject(theAlignObj),
...
//_____________________________________________________________________________
IlcAlignObj &IlcAlignObj::operator =(const IlcAlignObj& theAlignObj)
{
  // assignment operator
...
//_____________________________________________________________________________
void IlcAlignObj::SetVolUID(IlcGeomManager::ELayerID detId, Int_t modId)
{
  // From detector name and module number (according to detector numbering)
  // build fVolUID, unique numerical identity of that volume inside ILC
  ...
  //
  fVolUID = IlcGeomManager::LayerToVolUID(detId,modId);
}

//_____________________________________________________________________________
void IlcAlignObj::GetVolUID(IlcGeomManager::ELayerID &layerId, Int_t &modId) const
{
  // From the fVolUID, unique numerical identity of that volume inside ILC,
  ...
  //
  layerId = IlcGeomManager::VolUIDToLayer(fVolUID,modId);
}

//_____________________________________________________________________________
Bool_t IlcAlignObj::GetPars(Double_t tr[], Double_t angles[]) const
{
  GetTranslation(tr);
...
//_____________________________________________________________________________
Int_t IlcAlignObj::GetLevel() const
{
  ...
  //
  if(!gGeoManager){
    IlcWarning("gGeoManager doesn't exist or it is still open: unable to return meaningful level value.");
    return (-1);
  }
...
//_____________________________________________________________________________
Int_t IlcAlignObj::Compare(const TObject *obj) const
{
  ...
  Int_t level2 = ((IlcAlignObj *)obj)->GetLevel();
  if (level == level2)
    return 0;
  else
...
void IlcAlignObj::GetCovMatrix(Double_t *cmat) const
{
...
//______________________________________________________________________________
void IlcAlignObj::GetCovMatrix(TMatrixDSym& mcov) const
{
...
//______________________________________________________________________________
Bool_t IlcAlignObj::GetLocalCovMatrix(TMatrixDSym& lCov) const
{
...
//______________________________________________________________________________
Bool_t IlcAlignObj::GetLocalCovMatrix(Double_t *lCov) const
{
  // Calculates the covariance matrix (6x6) associated to the six parameters
  // defining the current alignment in the global coordinates system (and sets
  // in the internal data members) from the covariance matrix (6x6) for the six
...
//______________________________________________________________________________
Bool_t IlcAlignObj::GetJacobian(TMatrixD& mJ) const
{
  ...
  if (!gGeoManager || !gGeoManager->IsClosed()) {
    IlcError("Can't compute the global covariance matrix from the local one without an open geometry!");
    return kFALSE;
  }
  ...
  if(pne){
    ...
    IlcWarning(Form("The symbolic volume name %s does not correspond to a physical entry. Using it as volume path!",symname));
    node = (TGeoPhysicalNode*) gGeoManager->MakePhysicalNode(symname);
  }

  if (!node) {
    IlcError(Form("Volume name or path %s not valid!",symname));
    return kFALSE;
  }
...
//______________________________________________________________________________
Bool_t IlcAlignObj::SetFromLocalCov(TMatrixDSym& lCov)
{
  // Calculates the covariance matrix (6x6) associated to the six parameters
  // defining the current alignment in the global coordinates system (and sets
  // in the internal data members) from the covariance matrix (6x6) for the six
...
//______________________________________________________________________________
Bool_t IlcAlignObj::SetFromLocalCov(Double_t *lCov)
{
  // Calculates the covariance matrix (6x6) associated to the six parameters
  // defining the current alignment in the global coordinates system, and sets
...
//______________________________________________________________________________
void IlcAlignObj::SetCorrMatrix(Double_t *cmat)
{
...
//______________________________________________________________________________
void IlcAlignObj::SetCorrMatrix(TMatrixDSym& mcov)
{
...
//_____________________________________________________________________________
void IlcAlignObj::AnglesToMatrix(const Double_t *angles, Double_t *rot) const
{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::MatrixToAngles(const Double_t *rot, Double_t *angles) const
{
  ...
  if(TMath::Abs(rot[0])<1e-7 || TMath::Abs(rot[8])<1e-7) {
    IlcError("Failed to extract roll-pitch-yall angles!");
    return kFALSE;
  }
...
//______________________________________________________________________________
void IlcAlignObj::Transform(IlcTrackPoint &p, Bool_t copycov) const
{
  // The method transforms the space-point coordinates using the
  // transformation matrix provided by the IlcAlignObj
  ...
  if (fVolUID != p.GetVolumeID())
    IlcWarning(Form("Alignment object ID is not equal to the space-point ID (%d != %d)",fVolUID,p.GetVolumeID())); 
...
//_____________________________________________________________________________
void IlcAlignObj::Transform(IlcTrackPointArray &array) const
{
  // This method is used to transform all the track points
  // from the input IlcTrackPointArray
  // 
  IlcTrackPoint p;
...
//_____________________________________________________________________________
void IlcAlignObj::Print(Option_t *) const
{
  ...
  printf("Volume=%s\n",GetSymName());
  if (GetVolUID() != 0) {
    IlcGeomManager::ELayerID layerId;
    Int_t modId;
    GetVolUID(layerId,modId);
    printf("VolumeID=%d LayerID=%d ( %s ) ModuleID=%d\n", GetVolUID(),layerId,IlcGeomManager::LayerName(layerId),modId);
  }
...
//_____________________________________________________________________________
void IlcAlignObj::SetPars(Double_t x, Double_t y, Double_t z,
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::SetLocalPars(Double_t x, Double_t y, Double_t z,
				 Double_t psi, Double_t theta, Double_t phi)
{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::SetLocalTranslation(Double_t x, Double_t y, Double_t z)
{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::SetLocalTranslation(const TGeoMatrix& m)
{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::SetLocalRotation(Double_t psi, Double_t theta, Double_t phi)
{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::SetLocalRotation(const TGeoMatrix& m)
{
  // Set the global delta transformation by passing the matrix of
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::SetLocalMatrix(const TGeoMatrix& m)
{
  ...
  //
  if (!gGeoManager || !gGeoManager->IsClosed()) {
    IlcError("Can't set the local alignment object parameters! gGeoManager doesn't exist or it is still open!");
    return kFALSE;
  }
  ...
  if(pne)
  {
    ...
    if(pn){
      if (pn->IsAligned())
	IlcWarning(Form("Volume %s has been misaligned already!",symname));
      gprime = *pn->GetMatrix();
    }else{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::SetMatrix(const TGeoMatrix& m)
{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::GetLocalPars(Double_t transl[], Double_t angles[]) const
{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::GetLocalTranslation(Double_t* tr) const
{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::GetLocalAngles(Double_t* angles) const
{
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::GetLocalMatrix(TGeoHMatrix& m) const
{
  ...
  //
  if (!gGeoManager || !gGeoManager->IsClosed()) {
    IlcError("Can't get the local alignment object parameters! gGeoManager doesn't exist or it is still open!");
    return kFALSE;
  }
...
//_____________________________________________________________________________
Bool_t IlcAlignObj::ApplyToGeometry(Bool_t ovlpcheck)
{
  ...
  //
  if (!gGeoManager || !gGeoManager->IsClosed()) {
    IlcError("Can't apply the alignment object! gGeoManager doesn't exist or it is still open!");
    return kFALSE;
  }

  if (gGeoManager->IsLocked()){
    IlcError("Can't apply the alignment object! Geometry is locked!");
    return kFALSE;
  }
  ...
  if(pne){
    ...
    IlcDebug(1,Form("The symbolic volume name %s does not correspond to a physical entry. Using it as a volume path!",symname));
    path=symname;
    if (!gGeoManager->CheckPath(path)) {
      IlcDebug(1,Form("Volume path %s not valid!",path));
      return kFALSE;
    }
    if (gGeoManager->GetListOfPhysicalNodes()->FindObject(path)) {
      IlcError(Form("Volume %s has been misaligned already!",path));
      return kFALSE;
    }
  ...
  if (!node) {
    IlcError(Form("Volume path %s not valid!",path));
    return kFALSE;
  }
  ...
  *ginv = g->Inverse();
  *ginv *= gprime;
  IlcGeomManager::ELayerID layerId; // unique identity for layer in the alobj
  Int_t modId; // unique identity for volume inside layer in the alobj
  GetVolUID(layerId, modId);
  IlcDebug(2,Form("Aligning volume %s of detector layer %d with local ID %d",symname,layerId,modId));
  if(ovlpcheck){
  ...
  if(ovlpcheck)
  {
    ...
    if(nOvlp)
    {
      IlcInfo(Form("Misalignment of node %s generated the following overlaps/extrusions:",node->GetName()));

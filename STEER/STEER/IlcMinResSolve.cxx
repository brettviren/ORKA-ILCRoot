...
ClassImp(IlcMinResSolve)

//______________________________________________________
IlcMinResSolve::IlcMinResSolve() : 
...
//______________________________________________________
IlcMinResSolve::IlcMinResSolve(const IlcMinResSolve& src) : 
  TObject(src),
...
//______________________________________________________
IlcMinResSolve::IlcMinResSolve(const IlcMatrixSq *mat, const TVectorD* rhs) :
  fSize(mat->GetSize()),fPrecon(0),fMatrix((IlcMatrixSq*)mat),fRHS((double*)rhs->GetMatrixArray()),
  fPVecY(0),fPVecR1(0),fPVecR2(0),fPVecV(0),fPVecW(0),fPVecW1(0),fPVecW2(0),
  fPvv(0),fPvz(0),fPhh(0),
  fDiagLU(0),fMatL(0),fMatU(0),fMatBD(0)
{
...
//______________________________________________________
IlcMinResSolve::IlcMinResSolve(const IlcMatrixSq *mat, const double* rhs) :
  fSize(mat->GetSize()),fPrecon(0),fMatrix((IlcMatrixSq*)mat),fRHS((double*)rhs),
  fPVecY(0),fPVecR1(0),fPVecR2(0),fPVecV(0),fPVecW(0),fPVecW1(0),fPVecW2(0),
  fPvv(0),fPvz(0),fPhh(0),
  fDiagLU(0),fMatL(0),fMatU(0),fMatBD(0)
{
...
//______________________________________________________
IlcMinResSolve& IlcMinResSolve::operator=(const IlcMinResSolve& src)
{
  // assignment op.
  if (this != &src) {
    fSize    = src.fSize;
...
Int_t IlcMinResSolve::BuildPrecon(Int_t prec)
{
  ...
  if (fPrecon>=kPreconILU0 && fPrecon<=kPreconILU10) {
    if (fMatrix->InheritsFrom("IlcMatrixSparse")) return BuildPreconILUK(fPrecon-kPreconILU0);
...
Bool_t IlcMinResSolve::SolveFGMRES(TVectorD& VecSol,Int_t precon,int itnlim,double rtol,int nkrylov)
{
  // solve by fgmres
...
Bool_t IlcMinResSolve::SolveFGMRES(double* VecSol,Int_t precon,int itnlim,double rtol,int nkrylov)
{
  ...
  //
  IlcInfo(Form("Solution by FGMRes: Preconditioner#%d Max.iter.: %d Tol.: %e NKrylov: %d",
	       precon,itnlim,rtol,nkrylov));
  //
  int its = 0;
  if (nkrylov<10) {
    IlcInfo(Form("Changing N Krylov vectors from %d to %d",nkrylov,10));
    nkrylov = 10;
  }
  //
  if (precon>0) {
    if (precon>=kPreconsTot) {
      IlcInfo(Form("Unknown preconditioner identifier %d, ignore",precon));
    }
    else {
      if  (BuildPrecon(precon)<0) {
	ClearAux();
	IlcInfo("FGMRES failed to build the preconditioner");
	return kFALSE;
      }
    }
  }
  ...
  while(1) {
    ...
    if (beta <= eps1) {
      timer.Stop();
      IlcInfo(Form("FGMRES converged in %d iterations, CPU time: %.1f sec",its,timer.CpuTime()));
      break;   // success
    }
    //
    if (its >= itnlim) {
      timer.Stop();
      IlcInfo(Form("%d iterations limit exceeded, CPU time: %.1f sec",itnlim,timer.CpuTime()));
      status = kFALSE;
      break;
    }
  }
...
Bool_t IlcMinResSolve::SolveMinRes(TVectorD& VecSol,Int_t precon,int itnlim,double rtol)
{
  // solve by minres
...
//________________________________________________________________________________
Bool_t IlcMinResSolve::SolveMinRes(double *VecSol,Int_t precon,int itnlim,double rtol)
{
  /*
  ...
  */
  if (!fMatrix->IsSymmetric()) {
    IlcInfo("MinRes cannot solve asymmetric matrices, use FGMRes instead");
    return kFALSE;
  }
  //
  ClearAux();
  ...
  if (precon>0) {
    if (precon>=kPreconsTot) {
      ...
      if  (BuildPrecon(precon)<0) {
	ClearAux();
	IlcInfo("MinRes failed to build the preconditioner");
	return kFALSE;
      }
    }
  }
  IlcInfo(Form("Solution by MinRes: Preconditioner#%d Max.iter.: %d Tol.: %e",precon,itnlim,rtol));
  //
  // ------------------------ initialization  ---------------------->>>>
  ...
  //
  if (beta1 < 0) {
    IlcInfo(Form("Preconditioner is indefinite (init) (%e).",beta1));
    ClearAux();
    status = 7;
    return kFALSE;
  }
  //
  if (beta1 < eps) {
    IlcInfo(Form("RHS is zero or is the nullspace of the Preconditioner: Solution is {0}"));
    ClearAux();
    return kTRUE;
  }  
  //
  ...
  while(status==0) { //-----------------  Main iteration loop ---------------------->>>>
    ...
    //
    if (beta<0) {
      IlcInfo(Form("Preconditioner is indefinite (%e)",beta));
      status = 7;
      break;
    }
    // 
    ...
    */
    ...
    //
    IlcDebug(2,Form("#%5d |qnrm: %+.2e Anrm:%+.2e Cnd:%+.2e Rnrm:%+.2e Ynrm:%+.2e EpsR:%+.2e EpsX:%+.2e Beta1:%+.2e",
		    itn,qrnorm, normA,condA,rnorm,ynorm,epsr,epsx,beta1));

    if (status == 0) {
      if (itn    >= itnlim    ) {status = 5; IlcInfo(Form("%d iterations limit exceeded",itnlim));}
      if (condA  >= 0.1/eps   ) {status = 4; IlcInfo(Form("Matrix condition nymber %e exceeds limit %e",condA,0.1/eps));}
      if (epsx   >= beta1     ) {status = 3; IlcInfo(Form("Approximate convergence"));}
      if (qrnorm <= epsx      ) {status = 2; IlcInfo(Form("Converged within machine precision"));}
      if (qrnorm <= epsr      ) {status = 1; IlcInfo(Form("Converged"));}
    }
    //
  }  //-----------------  Main iteration loop ----------------------<<<
  //
  ...
  //
  timer.Stop();
  IlcInfo(Form("Exit from MinRes: CPU time: %.2f sec\n"
	       "Status    :  %2d\n"
	       "Iterations:  %4d\n"
	       "Norm      :  %+e\n"
	       "Condition :  %+e\n"
...
void IlcMinResSolve::ApplyPrecon(const TVectorD& vecRHS, TVectorD& vecOut) const
{
  // apply precond.
...
//______________________________________________________________
void IlcMinResSolve::ApplyPrecon(const double* vecRHS, double* vecOut) const
{
  ...
  //
    ...
    for(int i=0; i<fSize; i++) {     // Block L solve
      vecOut[i] = vecRHS[i];
      IlcVectorSparse &rowLi = *fMatL->GetRow(i);
    ...
    for(int i=fSize; i--; ) {    // Block -- U solve
      IlcVectorSparse &rowUi = *fMatU->GetRow(i);
...
Bool_t IlcMinResSolve::InitAuxMinRes()
{
...
//___________________________________________________________
Bool_t IlcMinResSolve::InitAuxFGMRES(int nkrylov)
{
...
//___________________________________________________________
Int_t  IlcMinResSolve::BuildPreconBD(Int_t hwidth)
{
  // build preconditioner
  IlcInfo(Form("Building Band-Diagonal preconditioner of half-width = %d",hwidth));
  fMatBD = new IlcSymBDMatrix( fMatrix->GetSize(), hwidth );
  //
  // fill the band-diagonal part of the matrix
  if (fMatrix->InheritsFrom("IlcMatrixSparse")) {
    for (int ir=fMatrix->GetSize();ir--;) {
      int jmin = TMath::Max(0,ir-hwidth);
      IlcVectorSparse& irow = *((IlcMatrixSparse*)fMatrix)->GetRow(ir);
...
//___________________________________________________________
Int_t  IlcMinResSolve::BuildPreconILUK(Int_t lofM)
{
  /*----------------------------------------------------------------------------
   * ILUK preconditioner
   ...
   *----------------------------------------------------------------------------*/
  //
  IlcInfo(Form("Building ILU%d preconditioner",lofM));
  //
  TStopwatch sw; sw.Start();
  fMatL = new IlcMatrixSparse(fSize);
  fMatU = new IlcMatrixSparse(fSize);
  fMatL->SetSymmetric(kFALSE);
  fMatU->SetSymmetric(kFALSE);
  fDiagLU = new Double_t[fSize];
  IlcMatrixSparse* matrix = (IlcMatrixSparse*)fMatrix;
  //
  ...
  for(int i=0; i<fSize; i++ ) {            // beginning of main loop
    if ( (i%int(0.1*fSize)) == 0) {
      IlcInfo(Form("BuildPrecon: row %d of %d",i,fSize));
      sw.Stop();
      sw.Print();
      sw.Start(kFALSE);
    }
    ...
    IlcVectorSparse& rowLi = *fMatL->GetRow(i);
    IlcVectorSparse& rowUi = *fMatU->GetRow(i);
    IlcVectorSparse& rowM  = *matrix->GetRow(i);
    //
    ...
    for(int j=rowM.GetNElems(); j--;) {  // L and D part 
      if (IlcMatrixSq::IsZero(rowM.GetElem(j))) continue;
	...
	if (IlcMatrixSq::IsZero(vl)) continue;
    ...
    for(int j=0; j<rowLi.GetNElems(); j++) {
      ...
      IlcVectorSparse& rowUj = *fMatU->GetRow(jrow);
    ...
    //
    if( IlcMatrixSq::IsZero(fDiagLU[i]) ) {
      IlcInfo(Form("Fatal error in ILIk: Zero diagonal found..."));
      delete[] jw;
      return -1;
    }
  ...
  //
  sw.Stop();
  IlcInfo(Form("ILU%d preconditioner OK, CPU time: %.1f sec",lofM,sw.CpuTime()));
  IlcInfo(Form("Densities: M %f L %f U %f",matrix->GetDensity(),fMatL->GetDensity(),fMatU->GetDensity()));
  //
  return 0;
}

//___________________________________________________________
Int_t  IlcMinResSolve::BuildPreconILUKDense(Int_t lofM)
{
  /*----------------------------------------------------------------------------
   * ILUK preconditioner
   * incomplete LU factorization with level of fill dropping
  ...
  //
  TStopwatch sw; sw.Start();
  IlcInfo(Form("Building ILU%d preconditioner for dense matrix",lofM));
  //
  fMatL = new IlcMatrixSparse(fSize);
  fMatU = new IlcMatrixSparse(fSize);
  fMatL->SetSymmetric(kFALSE);
  ...
  //
  sw.Stop();
  IlcInfo(Form("ILU%d dense preconditioner OK, CPU time: %.1f sec",lofM,sw.CpuTime()));
  //  IlcInfo(Form("Densities: M %f L %f U %f",fMatrix->GetDensity(),fMatL->GetDensity(),fMatU->GetDensity()));
  //
  return 0;
}

//___________________________________________________________
Int_t  IlcMinResSolve::PreconILUKsymb(Int_t lofM)
{
  /*----------------------------------------------------------------------------
   * ILUK preconditioner
   * incomplete LU factorization with level of fill dropping
  ...
  for(int i=0; i<fSize; i++) {
    int incl = 0;
    int incu = i; 
    IlcVectorSparse& row = *matrix->GetRow(i);
    //
    // assign lof = 0 for matrix elements
    for(int j=0;j<row.GetNElems(); j++) {
      int col = row.GetIndex(j);
      if (IlcMatrixSq::IsZero(row.GetElem(j))) continue;  // !!!! matrix is sparse but sometimes 0 appears 
      if (col<i) {                      // L-part
	jbuf[incl] = col;
	levls[incl] = 0;
	iw[col] = incl++;
	...
	if (IlcMatrixSq::IsZero(matrix->Query(col,i))) continue;    // Due to the symmetry  == matrix(i,col)
	jbuf[incu] = col;
	levls[incu] = 0;
	iw[col] = incu++;
      }
	...
	k = kmin; 
      ...
      IlcVectorSparse& rowU = *fMatU->GetRow(k);
...
//___________________________________________________________
Int_t  IlcMinResSolve::PreconILUKsymbDense(Int_t lofM)
{
  /*----------------------------------------------------------------------------
   * ILUK preconditioner
   * incomplete LU factorization with level of fill dropping
  ...
  for(int i=0; i<fSize; i++) {
    ...
    for(int j=0;j<fSize; j++) {
      if (IlcMatrixSq::IsZero(fMatrix->Query(i,j))) continue;
      if (j<i) {                      // L-part
	jbuf[incl] = j;
	levls[incl] = 0;
	iw[j] = incl++;

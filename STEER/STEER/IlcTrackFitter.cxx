...
ClassImp(IlcTrackFitter)

//_____________________________________________________________________________
IlcTrackFitter::IlcTrackFitter() :
  TObject(),
  fCov(0),
  fPoints(0),
  fPVolId(0),
...
//_____________________________________________________________________________
IlcTrackFitter::IlcTrackFitter(IlcTrackPointArray *array, Bool_t owner) :
  TObject(),
...
//_____________________________________________________________________________
IlcTrackFitter::IlcTrackFitter(const IlcTrackFitter &fitter):
  TObject(fitter),
...
//_____________________________________________________________________________
IlcTrackFitter &IlcTrackFitter::operator =(const IlcTrackFitter& fitter)
{
  // assignment operator
  //
...
void IlcTrackFitter::SetTrackPointArray(IlcTrackPointArray *array, Bool_t owner)
{
  // Load space points from array
  // By default we don't copy them but
  // just put the pointers to them
  if (!array) {
    IlcWarning("Invalid pointer to the space-points array !");
  ...
  if (owner) {
    fPoints = new IlcTrackPointArray(*array);
    fIsOwner = kTRUE;
  }
  else {
    fPoints = array;
...
Bool_t IlcTrackFitter::FindVolId(const TArrayI *array, UShort_t volid) const
{
...
Bool_t IlcTrackFitter::Fit(const TArrayI *volIds,const TArrayI *volIdsFit,
...
IlcGeomManager::ELayerID layerRangeMax)
{
  ...
  if (volIdsFit != 0x0) {
     ...
     for (Int_t i=0; i<npoints; i++) {
         ...
         if (id < IlcGeomManager::LayerToVolUID(layerRangeMin,0)) continue;
	 if (id > IlcGeomManager::LayerToVolUID(layerRangeMax,
  ...
  fPVolId = new IlcTrackPointArray(countPnt);
  fPTrack = new IlcTrackPointArray(countPnt);

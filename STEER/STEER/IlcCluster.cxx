...
//______________________________________________________________________________
IlcCluster::IlcCluster(UShort_t volId,
			       const Float_t *hit,
			       Float_t x,
			       Float_t sigyz,
			       const Int_t *lab):
...
//______________________________________________________________________________
IlcCluster::IlcCluster(const IlcCluster& cluster):
  TObject(cluster),
  fX(cluster.fX),
  fY(cluster.fY),
  fZ(cluster.fZ),
...
//______________________________________________________________________________
IlcCluster & IlcCluster::operator=(const IlcCluster& cluster)
{
  // Assignment operator
...
//______________________________________________________________________________
void IlcCluster::Print(Option_t* /*option*/) const
{
  // Print cluster information.
  
  printf("IlcCluster pos=(%.4f, %.4f, %.4f), s_y2=%f, s_z2=%f, s_yz=%f, vol=%hu\n",
...
//______________________________________________________________________________
Bool_t IlcCluster::GetGlobalXYZ(Float_t xyz[3]) const
{
  ...
  if (!gGeoManager || !gGeoManager->IsClosed()) {
    IlcError("Can't get the global coordinates! gGeoManager doesn't exist or it is still opened!");
    return kFALSE;
  }
...
//______________________________________________________________________________
Bool_t IlcCluster::GetGlobalCov(Float_t cov[6]) const
{
...
//______________________________________________________________________________
Bool_t IlcCluster::GetXRefPlane(Float_t &xref) const
{
...
//______________________________________________________________________________
Bool_t IlcCluster::Misalign()
{
  // ...
  // All the needed information is taken only
  // from TGeo.
  if (!gGeoManager || !gGeoManager->IsClosed()) {
    IlcError("Can't get the PN entry! gGeoManager doesn't exist or it is still opened!");
    return kFALSE;
  }

  if (fIsMisaligned) {
    IlcError("The cluster was already misaligned!");
    return kFALSE;
  }
...
//______________________________________________________________________________
TGeoHMatrix* IlcCluster::GetMatrix(Bool_t original) const
{
  ...
  if (!fIsMisaligned && (original == kFALSE)) {
    return IlcGeomManager::GetMatrix(fVolumeId);
  }
  else {
    return IlcGeomManager::GetOrigGlobalMatrix(fVolumeId);
  }
}

//______________________________________________________________________________
const TGeoHMatrix* IlcCluster::GetTracking2LocalMatrix() const
{
  // Get the matrix which is stored with the PN entries in TGeo.
  // The matrix makes the transformation from the tracking c.s. to
  // the local one.
  return IlcGeomManager::GetTracking2LocalMatrix(fVolumeId);
}

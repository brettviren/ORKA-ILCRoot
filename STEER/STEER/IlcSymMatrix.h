#ifndef ILCSYMMATRIX_H
#define ILCSYMMATRIX_H
/**********************************************************************************************/
...
class IlcSymMatrix : public IlcMatrixSq {
  //
 public:
  IlcSymMatrix();
  IlcSymMatrix(Int_t size);
  IlcSymMatrix(const IlcSymMatrix &mat);
  virtual ~IlcSymMatrix();
  //
  ...
  IlcSymMatrix& operator=(const IlcSymMatrix& src);
  IlcSymMatrix& operator+=(const IlcSymMatrix& src);
  Double_t      operator()(Int_t rown, Int_t coln)               const;
  Double_t&     operator()(Int_t rown, Int_t coln);
  //
  Double_t      DiagElem(Int_t r)                                const {return (*(const IlcSymMatrix*)this)(r,r);}
  ...
  //
  // ----------------------------- Choleski methods ----------------------------------------
  IlcSymMatrix*       DecomposeChol();                  //Obtain Cholesky decomposition L matrix 
  void                InvertChol(IlcSymMatrix* mchol);  //Invert using provided Choleski decomposition
 ...
 protected:
  ...
  //
  static IlcSymMatrix* fgBuffer;  // buffer for fast solution
  static Int_t         fgCopyCnt;  // matrix copy counter
  ClassDef(IlcSymMatrix,0) //Symmetric Matrix Class
};


//___________________________________________________________
inline Int_t IlcSymMatrix::GetIndex(Int_t row,Int_t col) const
{
...
//___________________________________________________________
inline Double_t IlcSymMatrix::operator()(Int_t row, Int_t col) const
{
  //
...
//___________________________________________________________
inline Double_t& IlcSymMatrix::operator()(Int_t row, Int_t col)
{
  if (row<col) Swap(row,col);
...
//___________________________________________________________
inline void IlcSymMatrix::MultiplyByVec(const TVectorD &vecIn, TVectorD &vecOut) const
{
  MultiplyByVec(vecIn.GetMatrixArray(), vecOut.GetMatrixArray());
}

//___________________________________________________________
inline void IlcSymMatrix::Scale(Double_t coeff)
{
  for (int i=fNrowIndex;i--;) for (int j=i;j--;) { double& el = operator()(i,j); if (el) el *= coeff;}
}

//___________________________________________________________
inline void IlcSymMatrix::AddToRow(Int_t r, Double_t *valc, Int_t *indc,Int_t n)
{

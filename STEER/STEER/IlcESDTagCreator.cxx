...
/* $Id: IlcESDTagCreator.cxx 53511 2011-12-10 22:30:41Z shahoian $ */

//-----------------------------------------------------------------
//           IlcESDTagCreator class
...
ClassImp(IlcESDTagCreator)


//______________________________________________________________________________
  IlcESDTagCreator::IlcESDTagCreator() :
    IlcTagCreator(),
    fChain(new TChain("esdTree")), fGUIDList(new TList()), 
    fMD5List(new TList()), fTURLList(new TList()), fBranches(""), 
    meminfo(new MemInfo_t) {
  //==============Default constructor for a IlcESDTagCreator================
}

//______________________________________________________________________________
IlcESDTagCreator::~IlcESDTagCreator() {
//================Default destructor for a IlcESDTagCreator===================
  delete fChain;
  delete fGUIDList;
  delete fMD5List;
  delete fTURLList;
...
//______________________________________________________________________________
Bool_t IlcESDTagCreator::ReadGridCollection(TGridResult *fresult) {
...
//     if(guid && !strlen(guid)) guid = 0;
  ...
  if (fChain->GetEntries() > 0) {
    IlcInfo(Form("ESD chain created......."));	
    IlcInfo(Form("Chain entries: %lld",fChain->GetEntries()));	
  } else {
    IlcWarning(Form("No ESD files found !"));
    return kFALSE;
  }
...
//______________________________________________________________________________
Bool_t IlcESDTagCreator::ReadLocalCollection(const char *localpath) {
  // Checks the different subdirs of the given local path and in the
  // case where it finds an IlcESDs.root file it creates the tags
  
  void *dira =  gSystem->OpenDirectory(localpath);
  Char_t fPath[512];
  const char * dirname = 0x0;
  const char * filename = 0x0;
  const char * pattern = "IlcESDs.root"; 
...
//______________________________________________________________________________
Bool_t IlcESDTagCreator::ReadCAFCollection(const char *filename) {
...
void IlcESDTagCreator::CreateTag(TChain* chain, const char *type) {
  ...
  //IlcInfo(Form("After the tag initialization - Memory used: %d MB",meminfo->fMemUsed));
  //Int_t tempmem = meminfo->fMemUsed;
  
  IlcInfo(Form("Creating the ESD tags......."));	
  ...
  //gSystem->GetMemInfo(meminfo);
  //IlcInfo(Form("After the esd initialization - Memory used: %d MB - Increase: %d MB",meminfo->fMemUsed,meminfo->fMemUsed - tempmem));
  ...
  if(fStorage == 0) {
    ...
    IlcInfo(Form("Writing tags to local file: %s",fileName.Data()));
  }
  else if(fStorage == 1) {
    ...
    IlcInfo(Form("Writing tags to grid file: %s",fileName.Data()));
  }
  ...
  TBranch * btag = ttag->Branch("IlcTAG", &tag);
  ...
  for(Int_t iEventNumber = 0; iEventNumber < chain->GetEntries(); iEventNumber++) {
    ...
    if(iRunNumber != iInitRunNumber) IlcFatal("Inconsistency of run numbers in the IlcESD - You are trying to merge different runs!!!");
    ...
    if (foldguid == fguid) {
      ...
      IlcFileTag *nftag = new IlcFileTag();
      nftag->SetGUID(fguid);
...
//       else fturl = url->GetFile();
	...
	IlcFatal("Adding a file which is already in the RunTag.");
...
//     }
    if(iEventNumber+1 == chain->GetEntries()) {
      //IlcInfo(Form("File: %s",fturl.Data()));
  ...
  //gSystem->GetMemInfo(meminfo);
  //IlcInfo(Form("After the event and track loop - Memory used: %d MB - Increase: %d MB",meminfo->fMemUsed,meminfo->fMemUsed - tempmem));
  //tempmem = meminfo->fMemUsed;

  //chain->Delete("");
  
  //gSystem->GetMemInfo(meminfo);
  //IlcInfo(Form("After the t->Delete - Memory used: %d MB - Increase: %d MB",meminfo->fMemUsed,meminfo->fMemUsed - tempmem));
  //tempmem = meminfo->fMemUsed;
  ...
  //gSystem->GetMemInfo(meminfo);
  //IlcInfo(Form("After the file closing - Memory used: %d MB - Increase: %d MB",meminfo->fMemUsed,meminfo->fMemUsed - tempmem));
  //tempmem = meminfo->fMemUsed;
  ...
  //gSystem->GetMemInfo(meminfo);
  //IlcInfo(Form("After the delete objects - Memory used: %d MB - Increase: %d MB",meminfo->fMemUsed,meminfo->fMemUsed - tempmem));
}

//_____________________________________________________________________________
void IlcESDTagCreator::CreateTag(TFile* file, const char *guid, const char *md5, const char *turl, Long64_t size, Int_t Counter) {
  //private method that creates tag files
  TString fguid = guid;
  TString fmd5 = md5;
  TString fturl = turl;
...
//   TRefArray tmp;
  ...
  for (Int_t iEventNumber = 0; iEventNumber < iNumberOfEvents; iEventNumber++) {
    ...
    iRunNumber = esd->GetRunNumber();
    if(iRunNumber != iInitRunNumber) IlcFatal("Inconsistency of run numbers in the IlcESD!!!");
...
//_____________________________________________________________________________
void IlcESDTagCreator::CreateTag(TFile* file, const char *filepath, Int_t Counter) {
  //private method that creates tag files
  /////////////
  //muon code//
  ////////////
...
//_____________________________________________________________________________
void IlcESDTagCreator::CreateESDTags(Int_t fFirstEvent, Int_t fLastEvent, IlcGRPObject *grpData, ULong_t * qa, Bool_t * es, Int_t qalength, Int_t eslength) {
  //GRP
...
//   TRefArray tmp;
  ...
  IlcInfo(Form("writing tags to file %s", fileName));
  IlcDebug(1, Form("writing tags to file %s", fileName));
...
//       else fturl = url->GetFile(); 
      ...
      //    evTag->SetGUID(fguid);
      ((IlcFileTag *) tag->GetFileTag(tag->GetNFiles()-1))->SetGUID(fguid);
      if(fStorage == 1) {
	((IlcFileTag *) tag->GetFileTag(tag->GetNFiles()-1))->SetMD5("");
	((IlcFileTag *) tag->GetFileTag(tag->GetNFiles()-1))->SetTURL(fturl);
	((IlcFileTag *) tag->GetFileTag(tag->GetNFiles()-1))->SetSize(0);
      }
      else {
	//      evTag->SetPath(fturl);
	((IlcFileTag *) tag->GetFileTag(tag->GetNFiles()-1))->SetPath(fturl);
	((IlcFileTag *) tag->GetFileTag(tag->GetNFiles()-1))->SetMD5("");
	((IlcFileTag *) tag->GetFileTag(tag->GetNFiles()-1))->SetSize(0);
      }
  ...
  Float_t l3Current = grpData->GetL3Current((IlcGRPObject::Stats)0);
  ...
  // Dipole
  Float_t diCurrent = grpData->GetDipoleCurrent((IlcGRPObject::Stats)0);
...
void IlcESDTagCreator::CreateESDTagsFullRun(TTree *chain, IlcGRPObject *grpData, ULong_t * qa, Bool_t * es, Int_t qalength, Int_t eslength)
{
  //GRP
  Float_t lhcLuminosity = 0.0;
  TString lhcState = "test";
...
//_____________________________________________________________________________
void IlcESDTagCreator::SwitchOffBranches() const {
  //
  // Switch of branches on user request
  ...
  for (Int_t i = 0; i < ntok; i++)  {
    ...
    IlcInfo(Form("Branch %s switched off \n", str.Data()));
  }
  delete tokens;
}

//_____________________________________________________________________________
void IlcESDTagCreator::FillEventTag(TTree *chain, IlcEventTag *evTag, Int_t iEventNumber, IlcESDEvent *esd)
{
...
//   else fturl = url->GetFile();
  ...
  for (Int_t iTrackNumber = 0; iTrackNumber < esd->GetNumberOfTracks(); iTrackNumber++) {
    ...
    Double_t rcc = 0.0;
    for(Int_t i = 0; i < IlcPID::kSPECIES; i++) rcc += prob[i]*partFrac[i];
    if(rcc == 0.0) continue;
    //Bayes' formula
    Double_t w[5];
    for(Int_t i = 0; i < IlcPID::kSPECIES; i++) w[i] = prob[i]*partFrac[i]/rcc;
  ...
  for (Int_t iTrack = 0; iTrack <  nMuonTracks;  iTrack++) {
    IlcESDMuonTrack* muonTrack = esd->GetMuonTrack(iTrack);
  ...
  //IlcInfo(Form("====================================="));
  //IlcInfo(Form("URL: %s - GUID: %s",fturl.Data(),fguid.Data()));
  //IlcInfo(Form("====================================="));
  
  //First physics data
  const IlcMultiplicity *spdMult = esd->GetMultiplicity();
  ...
  IlcESDVZERO *vzeroData = esd->GetVZEROData();
...
void IlcESDTagCreator::CreateESDRunTagSummary(TTree *chain)
{

...
/* $Id: IlcQAv1.h 31503 2009-03-16 11:01:16Z schutz $ */
...
class IlcQAv1 : public TNamed {
public:
  
  enum DETECTORINDEX_t {
    kNULLDET=-1, kITS, kTPC, kTRD, kTOF, kPHOS, kPVD, kPVBAR, kPVEC, kRSTACK, kHMPID, kEMCAL, kMUON, kFMD,
  ...
  IlcQAv1(); // beware singleton, not to be used
  IlcQAv1(const Int_t qalength, ULong_t * qa, const Int_t eslength, Bool_t * es) ;
  IlcQAv1(const ILCTASK_t tsk) ;
  IlcQAv1(const DETECTORINDEX_t det) ;
  IlcQAv1(const IlcQAv1& qa) ;   
  IlcQAv1& operator = (const IlcQAv1& qa) ;
  virtual ~IlcQAv1();
  
  static  IlcQAv1 *      Instance() ;
  static  IlcQAv1 *      Instance(const Int_t qalength, ULong_t * qa, const Int_t eslength, Bool_t * es) ;
  static  IlcQAv1 *      Instance(const DETECTORINDEX_t det) ;
  static  IlcQAv1 *      Instance(const ILCTASK_t tsk) ;
  static  IlcQAv1 *      Instance(const TASKINDEX_t tsk) ;
  Bool_t                 CheckFatal() const ;
  static void            Close() ; 
  static const char *    GetIlcTaskName(ILCTASK_t tsk) ;
  static  TH1 *          GetData(TObjArray** list, Int_t index, IlcRecoParam::EventSpecie_t eventSpecie) ; 
  ...
  QABIT_t                GetQAStatusBit(IlcRecoParam::EventSpecie_t es = IlcRecoParam::kDefault, DETECTORINDEX_t det=kNULLDET, ILCTASK_t tsk=kNULLTASK) const ; 
  ...
  Bool_t                 IsSet(DETECTORINDEX_t det, ILCTASK_t tsk, IlcRecoParam::EventSpecie_t es, QABIT_t bit) const ;
  Bool_t                 IsSet(DETECTORINDEX_t det, ILCTASK_t tsk, Int_t es, QABIT_t bit) const ;
  Bool_t                 IsSetAny(DETECTORINDEX_t det, ILCTASK_t tsk, IlcRecoParam::EventSpecie_t es) const ;
  Bool_t                 IsSetAny(DETECTORINDEX_t det, IlcRecoParam::EventSpecie_t es) const ;
  Long64_t               Merge(const TCollection * list) ; 
  static MODE_t          Mode(TASKINDEX_t task) ;
  void                   Set(QABIT_t bit, IlcRecoParam::EventSpecie_t es) ;
  void                   Set(QABIT_t bit, Int_t es) ;
  void                   SetEventSpecie(IlcRecoParam::EventSpecie_t es) 
  {if(es==IlcRecoParam::kDefault) return; Int_t ibit=0; while(es!=1<<ibit) ++ibit; fEventSpecies[ibit] = kTRUE ; }
  void                   UnSetEventSpecie(IlcRecoParam::EventSpecie_t es) 
  {if(es==IlcRecoParam::kDefault) return; Int_t ibit=0; while(es!=1<<ibit) ++ibit; fEventSpecies[ibit] = kFALSE ; }  
  static void            SetQADebug() { IlcLog::SetGlobalDebugLevel(GetQADebugLevel()); }
  static void            SetQAResultDirName(const char * name) ; 
  static void            SetQARefStorage(const char * name) ; 
  static void            SetQARefDataDirName(IlcRecoParam::EventSpecie_t es) { fgRefDataDirName = IlcRecoParam::GetEventSpecieName(es) ; }
  static void            SetQARefDataDirName(Int_t es) { fgRefDataDirName = IlcRecoParam::GetEventSpecieName(es) ; }
  void                   Show(DETECTORINDEX_t det = kNULLDET) const ;
  void                   ShowAll() const ;
  void                   ShowStatus(DETECTORINDEX_t det, ILCTASK_t tsk=kNULLTASK, IlcRecoParam::EventSpecie_t es=IlcRecoParam::kDefault) const ;
  void                   UnSet(QABIT_t bit, IlcRecoParam::EventSpecie_t es) ;
...
private:      
  ...
  Bool_t                CheckRange(IlcRecoParam::EventSpecie_t es) const ;
  const char *          GetBitName(QABIT_t bit) const ;
  ULong_t               GetStatus(DETECTORINDEX_t det, IlcRecoParam::EventSpecie_t es) const  { return fQA[det*fNEventSpecies+IlcRecoParam::AConvert(es)] ;}
  void                  Finish() const ;  
  ULong_t               Offset(ILCTASK_t tsk) const ;
  void                  ShowASCIIStatus(IlcRecoParam::EventSpecie_t es, DETECTORINDEX_t det, ILCTASK_t tsk, ULong_t status) const ; 
  void                  Reset(DETECTORINDEX_t det, ILCTASK_t tsk, IlcRecoParam::EventSpecie_t es) ;  

  void                  ResetStatus(DETECTORINDEX_t det) ; 
  void                  Set(DETECTORINDEX_t det) { fDet = det ;}
  void                  Set(ILCTASK_t tsk) { fTask = tsk ; IlcDebug(GetQADebugLevel(), Form("Ready to set QA status in %s", GetIlcTaskName(tsk) )) ; }
  void                  SetStatus(DETECTORINDEX_t det, IlcRecoParam::EventSpecie_t es, ULong_t status) { fQA[det*fNEventSpecies+IlcRecoParam::AConvert(es)] = status ; }
  void                  SetStatusBit(DETECTORINDEX_t det, ILCTASK_t tsk, IlcRecoParam::EventSpecie_t es, QABIT_t bit) ;
  void                  UnSetStatusBit(DETECTORINDEX_t det, ILCTASK_t tsk, IlcRecoParam::EventSpecie_t es, QABIT_t bit) ;
  
  static IlcQAv1 *       fgQA		                  ; // pointer to the instance of the singleton
  Int_t                  fNdet     	              ; // number of detectors
  Int_t                  fNEventSpecies           ; // number of Event Species (see IlcRecoParam)
  ...
  IlcRecoParam::EventSpecie_t fEventSpecie        ; //! the current event specie
 ...
 ClassDef(IlcQAv1,3)  //ILC Quality Assurance Object
};
#endif

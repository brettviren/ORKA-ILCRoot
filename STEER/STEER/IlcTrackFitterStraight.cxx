...
/* $Id: IlcTrackFitterStraight.cxx 50615 2011-07-16 23:19:19Z hristov $ */
...
ClassImp(IlcTrackFitterStraight)

IlcTrackFitterStraight::IlcTrackFitterStraight():
  IlcTrackFitter(),
  fAlpha(0.),
  fSumYY(0),
  fSumZZ(0),
...
IlcTrackFitterStraight::IlcTrackFitterStraight(IlcTrackPointArray *array, Bool_t owner):
...
IlcTrackFitterStraight::IlcTrackFitterStraight(const IlcTrackFitterStraight &fitter):
  IlcTrackFitter(fitter),
...
IlcTrackFitterStraight &IlcTrackFitterStraight::operator =(const IlcTrackFitterStraight& fitter)
{
  // assignment operator
  //
  if(this==&fitter) return *this;
  ((IlcTrackFitter *)this)->operator=(fitter);
...
void IlcTrackFitterStraight::Reset()
{
...
Bool_t IlcTrackFitterStraight::Fit(const TArrayI *volIds,const TArrayI *volIdsFit,
...
void IlcTrackFitterStraight::AddPoint(Float_t x, Float_t y, Float_t z, Float_t sy, Float_t sz)
{
...
Bool_t IlcTrackFitterStraight::Update(){
  //
  //  Track fitter update
  //
  //
...
Double_t IlcTrackFitterStraight::GetYat(Double_t x) const {
  if (!fConv) return 0.;
  return (fParams[0]+x*fParams[1]);
}

Double_t IlcTrackFitterStraight::GetDYat(Double_t x) const {
  if (!fConv) return 0.;
...
Double_t IlcTrackFitterStraight::GetZat(Double_t x) const {
  if (!fConv) return 0.;
  return (fParams[2]+x*fParams[3]);
}

Double_t IlcTrackFitterStraight::GetDZat(Double_t x) const {
  if (!fConv) return 0.;
  return fParams[3]+0.*x;
}

Bool_t IlcTrackFitterStraight::GetXYZat(Double_t r, Float_t *xyz) const {
...
Bool_t IlcTrackFitterStraight::GetPCA(const IlcTrackPoint &p, IlcTrackPoint &p2) const
{

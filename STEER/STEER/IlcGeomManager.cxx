 ...
 **************************************************************************/
//-------------------------------------------------------------------------
//   Implementation of IlcGeomManager, the geometry manager class 
...
ClassImp(IlcGeomManager)
  
Int_t IlcGeomManager::fgLayerSize[kLastLayer - kFirstLayer] = {
...
const char* IlcGeomManager::fgLayerName[kLastLayer - kFirstLayer] = {
...
TGeoPNEntry** IlcGeomManager::fgPNEntry[kLastLayer - kFirstLayer] = {
  0x0,0x0,
  0x0,0x0,
  0x0,0x0,
  0x0,0x0,
...
IlcAlignObj** IlcGeomManager::fgAlignObjs[kLastLayer - kFirstLayer] = {
  0x0,0x0,
  0x0,0x0,
  0x0,0x0,
  0x0,0x0,
...
const char* IlcGeomManager::fgkDetectorName[IlcGeomManager::fgkNDetectors] = {"GRP","TARGET", "DCH", "PVBAR", "PVEC", "RSTACK"};
Int_t IlcGeomManager::fgNalignable[fgkNDetectors] = {0, 0, 0, 0, 0, 0};

TGeoManager* IlcGeomManager::fgGeometry = 0x0;

//_____________________________________________________________________________
void IlcGeomManager::LoadGeometry(const char *geomFileName)
{
  // initialization
  ...
  if(fgGeometry->IsLocked()){
      IlcErrorClass("Cannot load a new geometry, the current one being locked. Setting internal geometry to null!!");
      fgGeometry = NULL;
      return;
  }
  ...
  if (geomFileName && (!gSystem->AccessPathName(geomFileName))) {
    ...
    IlcInfoClass(Form("From now on using geometry from custom geometry file \"%s\"",geomFileName));
  }
  ...
  if (!fgGeometry) {
    ...
    if(!entry) IlcFatalClass("Couldn't load geometry data from CDB!");

    entry->SetOwner(0);
    fgGeometry = (TGeoManager*) entry->GetObject();
    if (!fgGeometry) IlcFatalClass("Couldn't find TGeoManager in the specified CDB entry!");
    
    IlcInfoClass(Form("From now on using geometry from CDB base folder \"%s\"",
		      IlcCDBManager::Instance()->GetURI("GRP/Geometry/Data")));
  }
  ResetPNEntriesLUT();
  InitPNEntriesLUT();
  InitNalignable();
...
//_____________________________________________________________________________
void IlcGeomManager::SetGeometry(TGeoManager * const geom)
{
  // Load already active geometry
  if (!geom) IlcFatalClass("Pointer to the active geometry is 0x0!");
  ResetPNEntriesLUT();
  fgGeometry = geom;
  InitPNEntriesLUT();
  InitNalignable();
...
//_____________________________________________________________________________
Int_t IlcGeomManager::LayerSize(Int_t layerId)
{
  ...
  if (layerId < kFirstLayer || layerId >= kLastLayer) {
    IlcErrorClass(Form("Invalid layer index %d ! Layer range is (%d -> %d) !",layerId,kFirstLayer,kLastLayer));
    return 0;
  }
  else {
...
//_____________________________________________________________________________
const char* IlcGeomManager::LayerName(Int_t layerId)
{
...
//_____________________________________________________________________________
UShort_t IlcGeomManager::LayerToVolUID(ELayerID layerId, Int_t modId)
{
...
//_____________________________________________________________________________
UShort_t IlcGeomManager::LayerToVolUID(Int_t   layerId, Int_t modId)
{
  // From detector (layer) name and module number (according to detector
  // internal numbering) build the unique numerical identity of that volume
  // inside ILC
...
//_____________________________________________________________________________
UShort_t IlcGeomManager::LayerToVolUIDSafe(ELayerID layerId, Int_t modId)
{
  // From detector (layer) name and module number (according to detector
  // internal numbering) build the unique numerical identity of that volume
  // inside ILC
  ...
  if(modId < 0 || modId >= LayerSize(layerId)){
    IlcErrorClass(Form("Invalid volume id %d ! Range of valid ids for layer \"%s\" is [0, %d] !",modId,LayerName(layerId),LayerSize(layerId)-1));
    return 0;
  }
  return ((UShort_t(layerId) << 11) | UShort_t(modId));
}
...
//_____________________________________________________________________________
UShort_t IlcGeomManager::LayerToVolUIDSafe(Int_t layerId, Int_t modId)
{
  // From detector (layer) name and module number (according to detector
  // internal numbering) build the unique numerical identity of that volume
  // inside ILC
...
//_____________________________________________________________________________
IlcGeomManager::ELayerID IlcGeomManager::VolUIDToLayer(UShort_t voluid, Int_t &modId)
{
  // From voluid, unique numerical identity of that volume inside ILC,
...
//_____________________________________________________________________________
IlcGeomManager::ELayerID IlcGeomManager::VolUIDToLayer(UShort_t voluid)
{
  // From voluid, unique numerical identity of that volume inside ILC,
  // (voluid is 16 bits, first 5 reserved for layerID (32 possible values),
  // remaining 11 for module ID inside det (2048 possible values)), return
...
//_____________________________________________________________________________
IlcGeomManager::ELayerID IlcGeomManager::VolUIDToLayerSafe(UShort_t voluid, Int_t &modId)
{
  // From voluid, unique numerical identity of that volume inside ILC,
  // (voluid is 16 bits, first 5 reserved for layerID (32 possible values),
  ...
  if(layId != IlcGeomManager::kInvalidLayer){
  ...
  IlcErrorClass(Form("Invalid unique volume id: %d !",voluid));
  modId = -1;
...
//_____________________________________________________________________________
IlcGeomManager::ELayerID IlcGeomManager::VolUIDToLayerSafe(UShort_t voluid)
{
  // From voluid, unique numerical identity of that volume inside ILC,
  // (voluid is 16 bits, first 5 reserved for layerID (32 possible values),
  // remaining 11 for module ID inside det (2048 possible values)), returns
  ...
  IlcErrorClass(Form("Invalid layer id: %d !",(voluid >> 11)));
  return kInvalidLayer;

}

//_____________________________________________________________________________
Bool_t IlcGeomManager::GetFromGeometry(const char *symname, IlcAlignObj &alobj)
{
  ...
  if (!fgGeometry || !fgGeometry->IsClosed()) {
    IlcErrorClass("Can't get the alignment object! gGeoManager doesn't exist or it is still opened!");
    return kFALSE;
  }

  if (!fgGeometry->GetListOfPhysicalNodes()) {
    IlcErrorClass("Can't get the alignment object! gGeoManager doesn't contain any aligned nodes!");
    return kFALSE;
  }
  ...
  if(pne){
    ...
    IlcWarningClass(Form("The symbolic volume name %s does not correspond to a physical entry. Using it as a volume path!",symname));
    path = symname;
  }
  ...
  if (!node) {
    if (!fgGeometry->cd(path)) {
      IlcErrorClass(Form("%s not valid neither as symbolic volume name nor as volume path!",path));
      return kFALSE;
    }
    else {
      IlcWarningClass(Form("Volume (%s) has not been misaligned!",path));
      return kTRUE;
    }
  }
...
//_____________________________________________________________________________
void  IlcGeomManager::InitAlignObjFromGeometry()
{
  // Loop over all alignable volumes and extract
  // the corresponding alignment objects from
  // the TGeo geometry
  //
  for (Int_t iLayer = kFirstLayer; iLayer < IlcGeomManager::kLastLayer; iLayer++) {
    if (!fgAlignObjs[iLayer-kFirstLayer]) {
      fgAlignObjs[iLayer-kFirstLayer] = new IlcAlignObj*[LayerSize(iLayer)];
    }
    for (Int_t iModule = 0; iModule < LayerSize(iLayer); iModule++) {
      UShort_t volid = LayerToVolUID(iLayer,iModule);
      fgAlignObjs[iLayer-kFirstLayer][iModule] = new IlcAlignObjParams("",volid,0,0,0,0,0,0,kTRUE);
      const char *symname = SymName(volid);
      if (!GetFromGeometry(symname, *fgAlignObjs[iLayer-kFirstLayer][iModule]))
	IlcErrorClass(Form("Failed to extract the alignment object for the volume (ID=%d and path=%s) !",volid,symname));
    }
  }
...
//_____________________________________________________________________________
IlcAlignObj* IlcGeomManager::GetAlignObj(UShort_t voluid)
{
...
//_____________________________________________________________________________
IlcAlignObj* IlcGeomManager::GetAlignObj(ELayerID layerId, Int_t modId)
{
  // Returns pointer to alignment object given its layer and module ID
  //
  if(modId<0 || modId>=fgLayerSize[layerId-kFirstLayer]){
    IlcWarningClass(Form("Module number %d not in the valid range (0->%d) !",modId,fgLayerSize[layerId-kFirstLayer]-1));
    return NULL;
  }
...
//_____________________________________________________________________________
const char* IlcGeomManager::SymName(UShort_t voluid)
{
...
//_____________________________________________________________________________
const char* IlcGeomManager::SymName(ELayerID layerId, Int_t modId)
{
  // Returns the symbolic volume name given for a given layer
  // and module ID
  //
  if(!fgGeometry){
    IlcErrorClass("No geometry instance loaded yet!");
    return NULL;
  }
  if(modId<0 || modId>=fgLayerSize[layerId-kFirstLayer]){
    IlcWarningClass(Form("Module number %d not in the valid range (0->%d) !",modId,fgLayerSize[layerId-kFirstLayer]-1));
  ...
  if(!pne)
  {
    IlcWarningClass(Form("Module %d of layer %s is not activated!",modId,LayerName(layerId)));
    return NULL;
  }
  return pne->GetName();
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::CheckSymNamesLUT(const char* /*detsToBeChecked*/)
{
...
//  if(detsString.Contains("ALL")) detsString="ITS TPC TOF TRD HMPID PHOS EMCAL";
  ...
  TString detsString = "";
  if(fgGeometry->CheckPath("ILCM_1/ITSV_1")) detsString+="ITS ";
  if(fgGeometry->CheckPath("ILCM_1/TPC_M_1")) detsString+="TPC ";

  TString tofsm;
  TString baseTof("ILCM_1/B077_1/BSEGMO");
  ...
  TString trdsm;
  TString baseTrd("ILCM_1/B077_1/BSEGMO");
  ...
  if(fgGeometry->CheckPath("ILCM_1/Hmp0_0")) detsString+="HMPID ";
  
  TString phosMod, cpvMod;
  TString basePhos("ILCM_1/PHOS_");
  ...
  TString emcalSM;
  TString baseEmcalSM("ILCM_1/XEN1_1/SM");
  ...
  /*********************       ITS layers  ***********************/
    IlcDebugClass(2,"Checking consistency of symbolic names for ITS layers");
	...
	for(Int_t cStave =0; cStave<2; cStave++){
	  ...
	  for (Int_t cHS=0; cHS<2; cHS++) {
	    ...
	    for(Int_t cLad =0; cLad<2; cLad++){
	      ...
	      if(!pne)
	      {
		IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
		return kFALSE;
	      }
	      sname = pne->GetName();
	      if(symname.CompareTo(sname)) 
	      {
		IlcErrorClass(Form("Current loaded geometry differs in the definition of symbolic name for uid %d."
    ...
    IlcDebugClass(2,"Consistency check for ITS symbolic names finished successfully.");
  }
  ...
  if(detsString.Contains("TPC"))
  {
    ...
    IlcDebugClass(2,"Checking consistency of symbolic names for TPC layers");
    ...
    IlcDebugClass(2,"Consistency check for TPC symbolic names finished successfully.");
  }
  ...
  if(detsString.Contains("TOF"))
  {
    ...
    IlcDebugClass(2,"Checking consistency of symbolic names for TOF layers");
    modnum=0;
	...
	if(tofSMs[isect]) IlcDebugClass(3,Form("Consistency check for symnames of TOF supermodule %d.",isect));
    ...
    IlcDebugClass(2,"Consistency check for TOF symbolic names finished successfully.");
  } 
  ...
  if(detsString.Contains("HMPID"))
  {
    ...
    IlcDebugClass(2,"Checking consistency of symbolic names for HMPID layers");
    ...
    IlcDebugClass(2,"Consistency check for HMPID symbolic names finished successfully.");
  }
  ...
  if(detsString.Contains("TRD"))
  {
    ...
    IlcDebugClass(2,"Checking consistency of symbolic names for TRD layers");
    ...
    for(Int_t layer=0; layer<6; layer++){
      modnum=0;
      IlcDebugClass(3,Form("Consistency check for symnames of TRD layer %d.",layer));
    ...
    IlcDebugClass(2,"Consistency check for TRD symbolic names finished successfully.");
  }
  ...
  if(detsString.Contains("PHOS"))
  {
    ...
    IlcDebugClass(2,"Checking consistency of symbolic names for PHOS layers");
    ...
    IlcDebugClass(2,"Consistency check for PHOS symbolic names finished successfully.");
  }

if(detsString.Contains("PVD"))
{
  /*********************      PVD EMC layer   ***********************/
  
  IlcDebugClass(2,"Checking consistency of symbolic names for PVD layers");
  
  TString str = "PVD/Module";
  modnum=0;
  
  for (Int_t iModule=0; iModule < 5; iModule++) {
//     if(!PVDMods[iModule]) continue;
		symname = str;
    symname += (iModule+1);
    uid = LayerToVolUID(kPVD1,iModule);
    pne = fgGeometry->GetAlignableEntryByUID(uid);
    if(!pne)
    {
      IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
...
return kFALSE;
    ...
    if(!cpvActive) continue;
   symname += "/CPV";
   uid = LayerToVolUID(kPVD2,iModule);
   pne = fgGeometry->GetAlignableEntryByUID(uid);
   if(!pne)
   {
     IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
return kFALSE;
  ...
  IlcDebugClass(2,"Consistency check for PVD symbolic names finished successfully.");
}

if(detsString.Contains("PVBAR"))
{
  /*********************      PVBAR EMC layer   ***********************/
  
  IlcDebugClass(2,"Checking consistency of symbolic names for PVBAR layers");
  
  TString str = "PVBAR/Module";
  modnum=0;
  
  for (Int_t iModule=0; iModule < 5; iModule++) {
    //     if(!PVBARMods[iModule]) continue;
    symname = str;
    symname += (iModule+1);
    uid = LayerToVolUID(kPVBAR1,iModule);
    pne = fgGeometry->GetAlignableEntryByUID(uid);
    if(!pne)
    {
      IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
    ...
    if(!cpvActive) continue;
    symname += "/CPV";
    uid = LayerToVolUID(kPVBAR2,iModule);
    pne = fgGeometry->GetAlignableEntryByUID(uid);
    if(!pne)
    {
      IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
  ...
  IlcDebugClass(2,"Consistency check for PVBAR symbolic names finished successfully.");
}

if(detsString.Contains("PVEC"))
{
  /*********************      PVEC EMC layer   ***********************/
  
  IlcDebugClass(2,"Checking consistency of symbolic names for PVEC layers");
  
  TString str = "PVEC/Module";
  modnum=0;
  
  for (Int_t iModule=0; iModule < 5; iModule++) {
    //     if(!PVECMods[iModule]) continue;
    symname = str;
    symname += (iModule+1);
    uid = LayerToVolUID(kPVEC1,iModule);
    pne = fgGeometry->GetAlignableEntryByUID(uid);
    if(!pne)
    {
      IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
    ...
    if(!cpvActive) continue;
    symname += "/CPV";
    uid = LayerToVolUID(kPVEC2,iModule);
    pne = fgGeometry->GetAlignableEntryByUID(uid);
    if(!pne)
    {
      IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
  ...
  IlcDebugClass(2,"Consistency check for PVEC symbolic names finished successfully.");
}

if(detsString.Contains("RSTACK"))
{
  /*********************      RSTACK EMC layer   ***********************/
  
  IlcDebugClass(2,"Checking consistency of symbolic names for RSTACK layers");
  
  TString str = "RSTACK/Module";
  modnum=0;
  
  for (Int_t iModule=0; iModule < 5; iModule++) {
    //     if(!RSTACKMods[iModule]) continue;
    symname = str;
    symname += (iModule+1);
    uid = LayerToVolUID(kRSTACK1,iModule);
    pne = fgGeometry->GetAlignableEntryByUID(uid);
    if(!pne)
    {
      IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
    ...
    if(!cpvActive) continue;
    symname += "/CPV";
    uid = LayerToVolUID(kRSTACK2,iModule);
    pne = fgGeometry->GetAlignableEntryByUID(uid);
    if(!pne)
    {
      IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
  ...
  IlcDebugClass(2,"Consistency check for RSTACK symbolic names finished successfully.");
}

if(detsString.Contains("PFCALO"))
  {
    /*********************      PFCALO layer   ***********************/
    
    IlcDebugClass(2,"Checking consistency of symbolic names for PFCALO layers");
    
    TString str = "PFCALO/Module";
    modnum=0;
    ...
    for (Int_t iModule=0; iModule < 1; iModule++) {
      ...
      symname = str;
      symname += (iModule+1);
      uid = LayerToVolUID(kPFCALO,iModule);
      pne = fgGeometry->GetAlignableEntryByUID(uid);
      if(!pne)
      {
	IlcErrorClass(Form("In the currently loaded geometry there is no TGeoPNEntry with unique id %d",uid));
    ...
    IlcDebugClass(2,"Consistency check for PFCALO symbolic names finished successfully.");
  }
  ...
  if(detsString.Contains("EMCAL"))
  {
    ...
    IlcDebugClass(2,"Checking consistency of symbolic names for EMCAL layers");
    ...
    IlcDebugClass(2,"Consistency check for EMCAL symbolic names finished successfully.");
  }
...
//_____________________________________________________________________________
void IlcGeomManager::InitPNEntriesLUT()
{
  ...
  // IlcGeomManager instance and recreated if the geometry has changed
  //
  if(!fgGeometry) {
    IlcErrorClass("Impossible to initialize PNEntries LUT without an active geometry");
    return;
  }
...
TGeoHMatrix* IlcGeomManager::GetMatrix(TGeoPNEntry * const pne) 
{
  // Get the global transformation matrix for a given PNEntry
  // by quering the TGeoManager

  if (!fgGeometry || !fgGeometry->IsClosed()) {
    IlcErrorClass("Can't get the global matrix! gGeoManager doesn't exist or it is still opened!");
    return NULL;
  }
  ...
  if (!fgGeometry->cd(path)) {
    IlcErrorClass(Form("Volume path %s not valid!",path));
    return NULL;
  }
...
//______________________________________________________________________
TGeoHMatrix* IlcGeomManager::GetMatrix(Int_t index) 
{
...
//______________________________________________________________________
TGeoHMatrix* IlcGeomManager::GetMatrix(const char* symname) 
{
  // Get the global transformation matrix for a given alignable volume
  //  identified by its symbolic name 'symname' by quering the TGeoManager

  if (!fgGeometry || !fgGeometry->IsClosed()) {
    IlcErrorClass("No active geometry or geometry not yet closed!");
    return NULL;
  }
...
//______________________________________________________________________
Bool_t IlcGeomManager::GetTranslation(Int_t index, Double_t t[3]) 
{
...
//______________________________________________________________________
Bool_t IlcGeomManager::GetRotation(Int_t index, Double_t r[9]) 
{
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::GetDeltaForBranch(Int_t index, TGeoHMatrix &inclusiveD)
{
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::GetDeltaForBranch(IlcAlignObj& aao, TGeoHMatrix &inclusiveD)
{
  // The method sets the matrix passed as argument as the global delta
  ...
  if(!index){
    IlcErrorClass("Either the alignment object or its index are not valid");
    return kFALSE;
  }
...
//______________________________________________________________________
Bool_t IlcGeomManager::GetOrigGlobalMatrix(const char* symname, TGeoHMatrix &m) 
{
  ...
  if (!fgGeometry->GetListOfPhysicalNodes()) {
    IlcWarningClass("gGeoManager doesn't contain any aligned nodes!");
    if (!fgGeometry->cd(symname)) {
      IlcErrorClass(Form("Volume path %s not valid!",symname));
      return kFALSE;
    }
    else {
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::GetOrigGlobalMatrixFromPath(const char *path, TGeoHMatrix &m)
{
  ...
  if (!fgGeometry || !fgGeometry->IsClosed()) {
    IlcErrorClass("Can't get the original global matrix! gGeoManager doesn't exist or it is still opened!");
    return kFALSE;
  }
...
//_____________________________________________________________________________
TGeoHMatrix* IlcGeomManager::GetOrigGlobalMatrix(TGeoPNEntry * const pne)
{
...
//______________________________________________________________________
TGeoHMatrix* IlcGeomManager::GetOrigGlobalMatrix(Int_t index)
{
...
//______________________________________________________________________
Bool_t IlcGeomManager::GetOrigTranslation(Int_t index, Double_t t[3]) 
{
...
//______________________________________________________________________
Bool_t IlcGeomManager::GetOrigRotation(Int_t index, Double_t r[9]) 
{
...
//______________________________________________________________________
const TGeoHMatrix* IlcGeomManager::GetTracking2LocalMatrix(Int_t index)
{
  ...
  if (!m)
    IlcErrorClass(Form("TGeoPNEntry (%s) contains no tracking-to-local matrix !",pne->GetName()));

  return m;
}

//______________________________________________________________________
Bool_t IlcGeomManager::GetTrackingMatrix(Int_t index, TGeoHMatrix &m)
{
...
//_____________________________________________________________________________
TGeoPNEntry* IlcGeomManager::GetPNEntry(Int_t voluid) {
...
//_____________________________________________________________________________
TGeoPNEntry* IlcGeomManager::GetPNEntry(ELayerID layerId, Int_t modId)
{
...
//_____________________________________________________________________________
void IlcGeomManager::CheckOverlapsOverPNs(Double_t threshold)
{
  ...
  IlcInfoClass("********* Checking overlaps/extrusions over physical nodes only *********");
  ...
  for(Int_t pni=0; pni<pnList->GetEntriesFast(); pni++){
    ...
    if(!mvol->IsSelected()){
      IlcInfoClass(Form("Checking overlaps for volume %s",mvol->GetName()));
  ...
  IlcInfoClass(Form("Number of overlapping/extruding PNs: %d",overlaps->GetEntriesFast()));
  timer2.Stop();
  timer2.Print();
...
//_____________________________________________________________________________
Int_t IlcGeomManager::GetNalignable(const char* module)
{
...
//_____________________________________________________________________________
void IlcGeomManager::InitNalignable()
{
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::ApplyAlignObjsFromCDB(const char* AlignDetsList)
{
  // Calls AddAlignObjsFromCDBSingleDet for the detectors appearing in
  // the list passed as argument (called by IlcSimulation and
  // IlcReconstruction)
  ...
  while((str = (TObjString*) iter.Next())){
    ...
    IlcDebugClass(5,Form("Loading alignment objs for %s",det.Data()));
  ...
  if(!alObjsLoaded.IsNull()) IlcInfoClass(Form("Alignment objects loaded for: %s",
					       alObjsLoaded.Data()));
  if(!alObjsNotLoaded.IsNull())
      IlcFatalClass(Form("Could not load alignment objects from OCDB for: %s",
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::LoadAlignObjsFromCDBSingleDet(const char* detName, TObjArray& alignObjArray)
{
  ...
  IlcDebugClass(2, Form("Loading alignment objs for detector: %s",detName));
  
  IlcCDBEntry *entry;
  	
  IlcCDBPath path(detName,"Align","Data");
	
  entry=IlcCDBManager::Instance()->Get(path.GetPath());
  if(!entry){ 
  	IlcDebugClass(2,Form("Couldn't load alignment data for detector %s",detName));
	return kFALSE;
  }
  entry->SetOwner(1);
  ...
  IlcDebugClass(2,Form("Found %d alignment objects for %s",nAlObjs,detName));
  Int_t nAlVols = GetNalignable(detName);
  if(nAlObjs!=nAlVols) IlcWarningClass(Form("%d alignment objects loaded for %s, which has %d alignable volumes",nAlObjs,detName,GetNalignable(detName)));

  IlcAlignObj *alignObj=0;
  TIter iter(alignArray);
	
  // loop over align objects in detector
  while( ( alignObj=(IlcAlignObj *) iter.Next() ) ){
  	alignObjArray.Add(alignObj);
  }
  // delete entry --- Don't delete, it is cached!
	
  IlcDebugClass(2, Form("fIlcgnObjArray entries: %d",alignObjArray.GetEntries() ));
  return kTRUE;

}

//_____________________________________________________________________________
Bool_t IlcGeomManager::ApplyAlignObjsToGeom(TObjArray& alignObjArray, Bool_t ovlpcheck)
{
    // Read collection of alignment objects (IlcAlignObj derived) saved
    ...
    {
	IlcAlignObj* alobj = (IlcAlignObj*) alignObjArray.UncheckedAt(j);
	if(!alobj->ApplyToGeometry(ovlpcheck))
	{
	    flag = kFALSE;
	    IlcDebugClass(5,Form("Error applying alignment object for volume %s !",alobj->GetSymName()));
	}else{
	    IlcDebugClass(5,Form("Alignment object for volume %s applied successfully",alobj->GetSymName()));
	}
	...
	if(ovexlist->GetEntriesFast()){  
	    IlcErrorClass("The application of alignment objects to the geometry caused huge overlaps/extrusions!");
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::ApplyAlignObjsToGeom(const char* fileName, const char* clArrayName)
{
  // read collection of alignment objects (IlcAlignObj derived) saved
  ...
  inFile->Close();
  if (!alignObjArray) {
    IlcErrorClass(Form("Could not get array (%s) from file (%s) !",clArrayName,fileName));
    return kFALSE;
  }
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::ApplyAlignObjsToGeom(IlcCDBParam* param, IlcCDBId& Id)
{
  // read collection of alignment objects (IlcAlignObj derived) saved
  // in the TClonesArray ClArrayName in the IlcCDBEntry identified by
  // param (to get the IlcCDBStorage) and Id; apply the alignment objects
  // to the geometry
  //

  IlcCDBStorage* storage = IlcCDBManager::Instance()->GetStorage(param);
  IlcCDBEntry* entry = storage->Get(Id);
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::ApplyAlignObjsToGeom(const char* uri, const char* path, Int_t runnum, Int_t version, Int_t sversion)
{
  // read collection of alignment objects (IlcAlignObj derived) saved
  // in the TClonesArray ClArrayName in the IlcCDBEntry identified by
  // param (to get the IlcCDBStorage) and Id; apply the alignment objects
  ...
  IlcCDBParam* param = IlcCDBManager::Instance()->CreateParameter(uri);
  IlcCDBId id(path, runnum, runnum, version, sversion);
...
//_____________________________________________________________________________
Bool_t IlcGeomManager::ApplyAlignObjsToGeom(const char* detName, Int_t runnum, Int_t version, Int_t sversion)
{
  // read collection of alignment objects (IlcAlignObj derived) saved
  // in the TClonesArray ClArrayName in the IlcCDBEntry identified by
  // param (to get the IlcCDBStorage) and Id; apply the alignment objects
  ...
  IlcCDBPath path(detName,"Align","Data");
  IlcCDBEntry* entry = IlcCDBManager::Instance()->Get(path.GetPath(),runnum,version,sversion);
...
//_____________________________________________________________________________
void IlcGeomManager::ResetPNEntriesLUT()
{

...
/* $Id: IlcLego.cxx 52906 2011-11-16 07:13:43Z morsch $ */
...
//  Note that when calling gIlc->RunLego, an optional list
//  of arguments may be specified.
//
//Begin_Html
/*
<img src="picts/ilclego.gif">
*/
//End_Html
//
//////////////////////////////////////////////////////////////
...
//_______________________________________________________________________
IlcLego::IlcLego(const IlcLego &lego):
  TNamed(lego),
  fGener(0),
  fTotRadl(0),
  fTotAbso(0),
...
//_______________________________________________________________________
IlcLego::IlcLego(const char *title, Int_t ntheta, Float_t thetamin, 
  ...
  fStopped(0)
{
  ...
  //
   fGener = new IlcLegoGenerator(ntheta, thetamin, thetamax,
   ...
   fHistGcm2 = new TH2F("hgcm2","g/cm2 length map",        
			ntheta,thetamin,thetamax,nphi,phimin,phimax);
//
   fVolumesFwd     = new TClonesArray("IlcDebugVolume",1000);
   fVolumesBwd     = new TClonesArray("IlcDebugVolume",1000);
   fDebug          = IlcDebugLevel();
}

//_______________________________________________________________________
IlcLego::IlcLego(const char *title, IlcLegoGenerator* generator):
  TNamed("Lego Generator",title),
  fGener(0),
  fTotRadl(0),
  fTotAbso(0),
...
void IlcLego::BeginEvent()
{
  ...
  if (fDebug) {
    if (fErrorCondition) ToIlcDebug(1, DumpVolumes());
    fVolumesFwd->Delete();
    fVolumesBwd->Delete();
    fStepsForward    = 0;
    fStepsBackward   = 0;		  
    fErrorCondition  = 0;
    if (gIlc->GetMCApp()->GetCurrentTrackNumber() == 0) fStepBack = 0;
  }
}

//_______________________________________________________________________
void IlcLego::FinishEvent()
{
  //
...
void IlcLego::FinishRun()
{
  ...
  //
  if (fErrorCondition) ToIlcError(DumpVolumes());
}

//_______________________________________________________________________
void IlcLego::Copy(TObject&) const
{
  //
...
//_______________________________________________________________________
void IlcLego::StepManager() 
{
  //
  // called from IlcRun::Stepmanager from gustep.
...
//	    printf("\n steps fwd %d %s %d %f", fStepsForward, vol, fErrorCondition, step);	  
      ...
      //
	    ...
	    if (fStepsForward > 0) {
		IlcDebugVolume* tmp = dynamic_cast<IlcDebugVolume*>((*fVolumesFwd)[fStepsForward-1]);
		...
		IlcDebugVolume(vol,copy,step,pos[0], pos[1], pos[2], status);
...
//		       pos[2], TMath::Sqrt(pos[0] * pos[0] + pos[1] * pos[1]), step, a, z, radl, absl, gMC->CurrentVolName(), fTotRadl);
		if (fDebug) {
		    ...
		    gIlc->GetMCApp()->PushTrack(1, gIlc->GetMCApp()->GetCurrentTrackNumber(), 
...
//	     pos[2],  TMath::Sqrt(pos[0] * pos[0] + pos[1] * pos[1]), step, a, z, radl, absl, gMC->CurrentVolName(), fTotRadl);
	...
	if (fDebug) {
		...
		IlcDebugVolume* tmp = dynamic_cast<IlcDebugVolume*>((*fVolumesBwd)[fStepsBackward]);
		if (tmp && tmp->IsVEqual(vol, copy) && gMC->IsTrackEntering()) {
		    fStepsBackward += 2;
	    ...
	    new(lvols[fStepsBackward]) IlcDebugVolume(vol,copy,step,pos[0], pos[1], pos[2], status);
	    
	    IlcDebugVolume* tmp = dynamic_cast<IlcDebugVolume*>((*fVolumesFwd)[fStepsBackward]);
	    if (tmp && !(tmp->IsVEqual(vol, copy)) && (!fErrorCondition)) 
	    {
		IlcWarning(Form("Problem at (x,y,z): %d %f %f %f, volumes: %s %s step: %f\n", 
...
void IlcLego::DumpVolumes()
{
  ...
  for (Int_t i = fStepsForward-1; i>=0; i--)
    {
      IlcDebugVolume* tmp1 = dynamic_cast<IlcDebugVolume*>((*fVolumesFwd)[i]);
      IlcDebugVolume* tmp2 = dynamic_cast<IlcDebugVolume*>((*fVolumesBwd)[i]);
      if (tmp1)

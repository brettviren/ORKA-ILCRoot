...
ClassImp(IlcParamSolver)

//______________________________________________________________________________________
IlcParamSolver::IlcParamSolver()
...
//______________________________________________________________________________________
IlcParamSolver::IlcParamSolver(Int_t maxglo,Int_t locbuff)
...
//______________________________________________________________________________________
IlcParamSolver::IlcParamSolver(const IlcParamSolver& src)
...
//______________________________________________________________________________________
IlcParamSolver& IlcParamSolver::operator=(const IlcParamSolver& src)
{
  // assignment operator
...
//______________________________________________________________________________________
Bool_t IlcParamSolver::SolveGlobals(Bool_t obtainCov)
{
  // solve against global vars.
  if (fNPoints<fNGlobal/3) {
    IlcError(Form("Number of points: %d is not enough for %d globals",fNPoints,fNGlobal));
    return kFALSE;
  }
  //
  if (!TestBit(kBitGloSol)) {
    if (!fMatrix->SolveChol(fRHSGlo, fSolGlo, obtainCov)) {
      IlcError("Solution Failed");
      return kFALSE;
    }
    SetBit(kBitGloSol);
...
//______________________________________________________________________________________
Bool_t IlcParamSolver::SolveLocals()
{
  // solve for locals
  const double kTiny = 1e-16;
  if (TestBit(kBitLocSol)) return kTRUE;
  if (!TestBit(kBitGloSol)) {
    IlcError("Cannot solve for Locals before SolveGlobals is called");
    return kFALSE;
  }
...
//______________________________________________________________________________________
IlcSymMatrix* IlcParamSolver::GetCovMatrix()
{
  // obtain cov.mat
  if (!TestBit(kBitGloSol)) {
    IlcError("Cannot obtain Cov.Matrix before SolveGlobals is called");
    return 0;
  }
...
//______________________________________________________________________________________
Bool_t IlcParamSolver::Solve(Bool_t obtainCov)
{
  // solve all
...
//______________________________________________________________________________________
void IlcParamSolver::AddEquation(const Double_t* dGl,const Double_t *dLc,const Double_t *res, const Double_t *covI,Double_t sclErrI)
{
...
//______________________________________________________________________________________
void IlcParamSolver::AddConstraint(Int_t parID, Double_t val, Double_t err2inv)
{
  // add gassian constriant to parameter parID
  if (parID>=fNGlobal) {
    IlcError(Form("Attempt to constraint non-existing parameter %d",parID));
    return;
  }
  //
...
//______________________________________________________________________________________
void IlcParamSolver::Init(Int_t npini)
{
  // create storage assuming maximum npini measured points
  fMatrix = new IlcSymMatrix(fMaxGlobal);
...
//______________________________________________________________________________________
void IlcParamSolver::ExpandStorage(Int_t newSize)
{
...
//______________________________________________________________________________________
void IlcParamSolver::Clear(Option_t*)
{
  // reset all
  fNPoints = 0;
  fMatrix->Reset();
...
//______________________________________________________________________________________
void IlcParamSolver::Print(Option_t*) const
{
  // print itself
  IlcInfo(Form("Solver with %d globals for %d points",fNGlobal,fNPoints));
}

//______________________________________________________________________________________
void IlcParamSolver::SetNGlobal(Int_t n) 
{
  // set N global params
  if (n>fMaxGlobal) {
    IlcError(Form("Maximum number of globals was set to %d",fMaxGlobal));
    return;
  }
  fNGlobal = n;
  fMatrix->SetSizeUsed(fNGlobal);
...
//______________________________________________________________________________________
void IlcParamSolver::SetMaxGlobal(Int_t n) 
{
  // set limit on N glob.

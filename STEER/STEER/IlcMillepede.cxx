...
/* $Id: IlcMillepede.cxx 50615 2011-07-16 23:19:19Z hristov $ */
    ...
    fNLocalPar(0)
{
  ...
  IlcInfo("            * o o                   o      ");
  IlcInfo("              o o                   o      ");
  IlcInfo("   o ooooo  o o o  oo  ooo   oo   ooo  oo  ");
  IlcInfo("    o  o  o o o o o  o o  o o  o o  o o  o ");
  IlcInfo("    o  o  o o o o oooo o  o oooo o  o oooo ");
  IlcInfo("    o  o  o o o o o    ooo  o    o  o o    ");
  IlcInfo("    o  o  o o o o  oo  o     oo   ooo  oo  ++ starts");	   
  IlcInfo("                                           ");
...
//=============================================================================
Int_t IlcMillepede::InitMille(int nGlo, int nLoc, int lNStdDev,
			      double lResCut, double lResCutInit)
{
  ...
  IlcMillepede::SetNLocalEquations(0);     // Number of local fits (starts at 0)
  ...
  IlcDebug(1,Form("Number of global parameters   : %d", fNGlobalPar));
  IlcDebug(1,Form("Number of local parameters    : %d", fNLocalPar));
  IlcDebug(1,Form("Number of standard deviations : %d", fNStdDev));

  if (fNGlobalPar>fgkMaxGlobalPar || fNLocalPar>fgkMaxLocalPar) {
    IlcDebug(1,"Two many parameters !!!!!");
    return 0;
  }
  ...
  for (int j=0; j<fNGlobalPar; j++) {
    IlcMillepede::SetParSigma(j,0.0);
  }
  ...
  IlcDebug(1,"----------------------------------------------------");
  IlcDebug(1,"");
  IlcDebug(1,"    InitMille has been successfully called!");
  IlcDebug(1,"");
  IlcDebug(1,"-----------------------------------------------------");
  IlcDebug(1,"");
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::SetGlobalParameters(double *param)
{
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::SetGlobalParameter(int iPar, double param)
{
...
-----------------------------------------------------------
*/ 
Int_t IlcMillepede::SetParSigma(int iPar, double sigma)
{
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::SetNonLinear(int iPar)
{
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::SetIterations(double lChi2CutFac)
{
...
/*
-----------------------------------------------------------
  CONSTF: define a constraint equation in IlcMillepede
-----------------------------------------------------------
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::SetGlobalConstraint(double dercs[], double lLagMult)
{ 
  /// Define a constraint equation
  if (fNGlobalConstraints>=fgkMaxGloCsts) {  
    IlcInfo("Too many constraints !!!");
    return 0;
  }
  ...
  fNGlobalConstraints++ ;
  IlcInfo(Form("Number of constraints increased to %d",fNGlobalConstraints));
  return 1;
}
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::SetLocalEquation(double dergb[], double derlc[], double lMeas, double lSigma)
{	
  ...
  IlcDebug(2,Form("%d / %d",iLocFirst, iLocLast));
  ...
  IlcDebug(2,Form("%d / %d",iGlobFirst,iGlobLast));
  ...
  IlcDebug(2,Form("Out Equloc --  NST = %d",fNDerivLocEq));
  return 1; 	
}
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::LocalFit(int iFit, double localParams[], Bool_t bSingleFit)
{
  ...
  if (fIter < 2 && !bSingleFit) { // Do it only once 
    IlcDebug(1,Form("Store equation no: %d", iFit)); 
    ...
    IlcDebug(2,Form("FLocEqPlace size = %d",fLocEqPlace[iFit])); 
    IlcDebug(2,Form("FIndexAllEqs size   = %d",fNIndexAllEqs)); 
  }
...
// 	IlcDebug(1,Form("lMeas = %f", lMeas));
// 	IlcDebug(1,Form("lWeight = %f", lWeight));
...
// 	  IlcDebug(2,Form("fDeltaPar[%d] = %f", iIdx, fDeltaPar[iIdx]));        
// 	  IlcDebug(2,Form("Starting misalignment = %f",fInitPar[iIdx]));        
...
// 	IlcDebug(2,Form("lMeas after global stuff removal = %f", lMeas));
...
//
  ...
  Int_t nRank = IlcMillepede::SpmInv(fMatCLoc, fVecBLoc, fNLocalPar);
  // nRank is the number of nonzero diagonal elements 
     	
  IlcDebug(1,"");
  IlcDebug(1," __________________________________________________");
  IlcDebug(1,Form(" Printout of local fit  (FITLOC)  with rank= %d", nRank));
  IlcDebug(1," Result of local fit :      (index/parameter/error)");
  
  for (i=0; i<fNLocalPar; i++) {
    IlcDebug(1,Form("%d   /   %.6f   /   %.6f", i, fVecBLoc[i], TMath::Sqrt(fMatCLoc[i][i])));	
  }
...
// 	IlcDebug(2,"");
// 	IlcDebug(2,Form(". equation:  measured value %.6f +/- %.6f", lMeas, 1.0/TMath::Sqrt(lWeight)));
// 	IlcDebug(2,Form("Number of derivatives (global, local): %d, %d",nDerGlo,nDerLoc));
// 	IlcDebug(2,"Global derivatives are: (index/derivative/parvalue) ");
	
// 	for (i=iGloFirst; i<=iGloLast; i++) {
// 	  IlcDebug(2,Form("%d / %.6f / %.6f",fIndexLocEq[i],fDerivLocEq[i],fInitPar[fIndexLocEq[i]]));
//      } 

// 	IlcDebug(2,"Local derivatives are: (index/derivative) ");
	
// 	for (i=(ja+1); i<jb; i++) {IlcDebug(2,Form("%d / %.6f",fIndexLocEq[i], fDerivLocEq[i]));}	  
  ...
  IlcDebug(1,Form("Final chi square / degrees of freedom %.2f / %d",lChi2, nDoF));
  ...
  {
    lChi2Cut = IlcMillepede::Chi2DoFLim(fNStdDev, nDoF)*fChi2CutFactor;
    
    IlcDebug(1,Form("Reject if Chisq/Ndf = %.4f > %.4f",lRedChi2,lChi2Cut));
...
// 	  IlcDebug(2,Form("fVecBGlo[%d] = %.6f", j, fVecBGlo[j] ));
...
// 	    IlcDebug(2,Form("fMatCGlo[%d][%d] = %.6f",iIdx,jIdx,fMatCGlo[iIdx][jIdx]));
	  } 
...
// 	    IlcDebug(2,Form("fMatCGloLoc[%d][%d] = %.6f",iIdxIdx,kIdx,fMatCGloLoc[iIdxIdx][kIdx]));
	  } 
	}
	iMeas   = -1;
	iWeight =  0;
  ...
  IlcMillepede::SpAVAt(fMatCLoc, fMatCGloLoc, fMatCGloCorr, fNLocalPar, nGloInFit);
  IlcMillepede::SpAX(fMatCGloLoc, fVecBLoc, fVecBGloCorr, fNLocalPar, nGloInFit);
...
-----------------------------------------------------------
  ...
  error[]      = array containing the error on global 
                 parameters (estimated by IlcMillepede)

  pull[]        = array containing the corresponding pulls 

-----------------------------------------------------------
*/
Int_t IlcMillepede::GlobalFit(double par[], double error[], double pull[])
{
  ...
  IlcInfo("..... Making global fit .....");
  IlcInfo(Form("First Iteration with cut factor %.2f", fChi2CutFactor));

  nLocFitsTot = IlcMillepede::GetNLocalEquations();
	
  while (fIter < fMaxIter)  // Iteration for the final loop
  {

    nLocFits = IlcMillepede::GetNLocalEquations();
    IlcInfo(Form("...using %d local fits...",nLocFits));
...
//  Then we retrieve the different constraints: fixed parameter or global equation
    ...
    IlcDebug(1,Form("Number of constraint equations : %d", fNGlobalConstraints));
...
// 	  printf("%d, %d, %.6f  %.6f  %.6f\n",i,j,step[i],fMatCGlo[i][j],step[j]);
    ...
    IlcInfo(Form(" Final coeff is %.6f",lFinalCor));		
    IlcInfo(Form(" Final NDOFs = %d", fNGlobalPar));

    //  The final matrix inversion

    Int_t nRank = IlcMillepede::SpmInv(fMatCGlo, fVecBGlo, nVar);

    for (i=0; i<fNGlobalPar; i++) {    
      fDeltaPar[i] += fVecBGlo[i];    // Update global parameters values (for iterations)
      IlcDebug(1,Form("fDeltaPar[%d] = %.6f", i, fDeltaPar[i]));
      IlcDebug(1,Form("fMatCGlo[%d][%d] = %.6f", i, i, fMatCGlo[i][i]));
      IlcDebug(1,Form("err = %.6f", TMath::Sqrt(TMath::Abs(fMatCGlo[i][i]))));

      step[i] = fVecBGlo[i];

      if (fIter == 1) error[i] = fMatCGlo[i][i]; // Unfitted error
    }
    IlcInfo(Form("The rank defect of the symmetric %d by %d matrix is %d (bad if non 0)",
	    nVar, nVar, nVar-nGloFix-nRank));
		
    IlcInfo(Form("Total : %d local fits, %d rejected.", fNLocalFits, fNLocalFitsRejected));
    ...
    IlcInfo("");
    IlcInfo("----------------------------------------------");
    IlcInfo(Form("Iteration %d with cut factor %.2f", fIter, fChi2CutFactor));
    ...
    for (i=0; i<nLocFitsTot; i++) {
      ...
      IlcDebug(2,Form("Track %d : ",i));
      IlcDebug(2,Form("Starts at %d", iEqFirst));
      IlcDebug(2,Form("Ends at %d",iEqLast));
...
//  	Int_t sc = IlcMillepede::LocalFit(i,localPars,0);
// 	(sc) ? nLocFitsGood++ : fIndexAllEqs[iEqFirst] = -999;   
	IlcMillepede::LocalFit(i,localPars,0);
	nLocFitsGood++;
      }
    } // End of loop on fits

    IlcMillepede::SetNLocalEquations(nLocFitsGood);

  } // End of iteration loop
	
  IlcMillepede::PrintGlobalParameters(); // Print the final results
  ...
  IlcInfo("    o  o  o o o o oooo o  o oooo o  o oooo ");
  IlcInfo("    o  o  o o o o o    ooo  o    o  o o    ");
  IlcInfo("    o  o  o o o o  oo  o     oo   ooo  oo ++ ends.");
  IlcInfo("                       o                   ");	  
...
-----------------------------------------------------------
*/ 
Double_t IlcMillepede::GetParError(int iPar) const
{
...
-----------------------------------------------------------
*/
int IlcMillepede::SpmInv(double matV[][fgkMaxGloPC], double vecB[], int nGlo)
{
...
//
int IlcMillepede::SpmInv(double matV[][fgkMaxLocalPar], double vecB[], int nLoc)
{
  ///  Obtain solution of a system of linear equations with symmetric matrix 
  ///  and the inverse (using 'singular-value friendly' GAUSS pivot)
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::SpAVAt(double matV[][fgkMaxLocalPar], double matA[][fgkMaxLocalPar], double matW[][fgkMaxGlobalPar], int nLoc, int nGlo)
{
  ///  multiply symmetric N-by-N matrix from the left with general M-by-N
  ///  matrix and from the right with the transposed of the same general
  ///  matrix to form symmetric M-by-M matrix.
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::SpAX(double matA[][fgkMaxLocalPar], double vecX[], double vecY[], int nCol, int nRow)
{
  ///   multiply general M-by-N matrix A and N-vector X
...
-----------------------------------------------------------
*/
Int_t IlcMillepede::PrintGlobalParameters() const
{
  ///  Print the final results into the logfile
  double lError = 0.;
  double lGlobalCor =0.;
  ...
  IlcInfo("   ===================================");
  IlcInfo("    I       initial       final       differ        lastcor        error       gcor");
  IlcInfo("-----------------------------------------------------------------------------------");
  ...
  for (int i=0; i<fNGlobalPar; i++) {
    ...
    if (TMath::Abs(fMatCGlo[i][i]*fDiagCGlo[i]) > 0) {    
      ...
      IlcInfo(Form("%d\t %.6f\t %.6f\t %.6f\t %.6f\t %.6f\t %.6f",
		   i,fInitPar[i],fInitPar[i]+fDeltaPar[i],fDeltaPar[i],fVecBGlo[i],lError,lGlobalCor));
    }
    else {    
      IlcInfo(Form("%d\t %.6f\t %.6f\t %.6f\t %.6f\t OFF\t OFF",i,fInitPar[i],fInitPar[i]+fDeltaPar[i],fDeltaPar[i],fVecBGlo[i]));
    }
  }
  return 1;
}
...
----------------------------------------------------------------
*/
double IlcMillepede::Chi2DoFLim(int nSig, int nDoF)
{

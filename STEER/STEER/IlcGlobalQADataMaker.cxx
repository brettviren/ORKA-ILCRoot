...
ClassImp(IlcGlobalQADataMaker)
 
//____________________________________________________________________________ 
void IlcGlobalQADataMaker::EndOfDetectorCycle(IlcQAv1::TASKINDEX_t task, TObjArray ** list)
{
  //Detector specific actions at end of cycle
  // do the QA checking
  IlcQAChecker::Instance()->Run(IlcQAv1::kGLOBAL, task, list) ;  
}

//____________________________________________________________________________ 
void IlcGlobalQADataMaker::InitRaws()
{
...
//____________________________________________________________________________
void IlcGlobalQADataMaker::InitRecoParams() 
{
  // Get the recoparam form the OCDB 
  if (!fRecoParam) {
    TString name("GRP") ; 
    IlcDebug(IlcQAv1::GetQADebugLevel(), Form("Loading reconstruction parameter objects for detector %s", name.Data()));
    IlcCDBPath path(name.Data(),"Calib","RecoParam");
    IlcCDBEntry *entry=IlcCDBManager::Instance()->Get(path.GetPath());
    if(!entry) {
      fRecoParam = NULL ; 
      IlcDebug(IlcQAv1::GetQADebugLevel(), Form("Couldn't find RecoParam entry in OCDB for detector %s",name.Data()));
    }
    else {
      TObject * recoParamObj = entry->GetObject() ; 
      if ( strcmp(recoParamObj->ClassName(), "TObjArray") == 0 ) {
          // The detector has only one set of reco parameters
        IlcDebug(IlcQAv1::GetQADebugLevel(), Form("Array of reconstruction parameters found for detector %s",name.Data()));
        ...
        IlcDebug(IlcQAv1::GetQADebugLevel(), Form("Single set of reconstruction parameters found for detector %s",name.Data()));
        fRecoParam = static_cast<IlcDetectorRecoParam*>(recoParamObj) ;
        static_cast<IlcDetectorRecoParam*>(recoParamObj)->SetAsDefault();
      } else { 
        IlcError(Form("No valid RecoParam object found in the OCDB for detector %s",name.Data()));
      }
    }
  }
}
...
//____________________________________________________________________________ 
void IlcGlobalQADataMaker::InitRecPoints() {
  //------------------------------------------------------
  ...
  for (Int_t m=1; m<IlcGeomManager::kLastLayer; m++) {
    Int_t i=2*m-2;
...
//____________________________________________________________________________ 
void IlcGlobalQADataMaker::InitESDs() {
  //------------------------------------------------------
...
//____________________________________________________________________________
void IlcGlobalQADataMaker::MakeRaws(IlcRawReader* rawReader)
{
...
//____________________________________________________________________________ 
void IlcGlobalQADataMaker::MakeESDs(IlcESDEvent * event) {
  //-----------------------------------------------------------
  // This function fills the ESD QA histograms
  // as a part of global QA
  //-----------------------------------------------------------

  const IlcESDEvent *esd=event;
  ...
  for (Int_t i=0; i<ntrk; i++) {
    const IlcESDtrack *track=esd->GetTrack(i);

    // Cluster related QA
    if (track->IsOn(IlcESDtrack::kITSrefit)) {
    ...
    if (track->IsOn(IlcESDtrack::kTPCrefit)) {
    ...
    if (track->IsOn(IlcESDtrack::kTRDrefit)) {
	...
	FillESDsData(kTrk1,y);
	  ...
	  if (track->IsOn(IlcESDtrack::kITSrefit)) FillESDsData(kTrk3,p);
	  //if (track->IsOn(IlcESDtrack::kTOFout)) FillESDsData(kTrk4,p);
	  if (track->GetTOFsignal()>0) FillESDsData(kTrk4,p);
	}
      }
    }
    const IlcExternalTrackParam *tpcTrack=track->GetTPCInnerParam();
    const IlcExternalTrackParam *innTrack=track->GetInnerParam();
    if (tpcTrack)
    if (innTrack) {
       Float_t dz[2];
    ...
    if (p>1.0) {
      if (track->IsOn(IlcESDtrack::kITSrefit))
      if (track->IsOn(IlcESDtrack::kTPCrefit))
      if (track->IsOn(IlcESDtrack::kTOFout)) {
         Float_t dz[2];
         track->GetDZ(xv,yv,zv,esd->GetMagneticField(),dz);
         if (dz[1]<3.) {
            Double_t times[10];
    ...
    const IlcExternalTrackParam *par=track->GetInnerParam();
    if (par) {
  ...
  // Multiplicity related QA
  IlcESDVZERO     *mltV0 =esd->GetVZEROData();
  const IlcMultiplicity *mltITS=esd->GetMultiplicity();
  if (mltV0)
    if (mltITS) {
  ...
  for (Int_t i=0; i<nV0; i++) {
    Double_t mass;
    IlcESDv0 v0(*esd->GetV0(i));

    Int_t nidx=TMath::Abs(v0.GetNindex());
    IlcESDtrack *ntrack1=esd->GetTrack(nidx);
    if (!ntrack1->IsOn(IlcESDtrack::kTPCrefit)) continue;

    Int_t pidx=TMath::Abs(v0.GetPindex());
    IlcESDtrack *ptrack1=esd->GetTrack(pidx);
    if (!ptrack1->IsOn(IlcESDtrack::kTPCrefit)) continue;

#ifndef ILCRELILCGNERKALMAN_H
#define ILCRELILCGNERKALMAN_H
...
//     (see IlcRelAlignerKalman.cxx for details)
//
...
class IlcRelAlignerKalman : public TObject {

public:
    IlcRelAlignerKalman();
    virtual ~IlcRelAlignerKalman();
    IlcRelAlignerKalman& operator= (const IlcRelAlignerKalman& a );
    IlcRelAlignerKalman(const IlcRelAlignerKalman& a);

    //User methods:
    Bool_t AddCosmicEvent( const IlcESDEvent* pEvent );
    Bool_t AddTrackParams( const IlcExternalTrackParam* p1, const IlcExternalTrackParam* p2 );
    ...
    Bool_t Merge( const IlcRelAlignerKalman* al );
    Long64_t Merge( TCollection* list );

    //Expert methods:
    Bool_t AddESDevent( const IlcESDEvent* pEvent );
    Bool_t AddESDtrack( const IlcESDtrack* pTrack );
    void SetMagField( const Double_t f ) { fMagField=f; }
    Double_t GetMagField() const { return fMagField; }
    Bool_t FindCosmicTrackletNumbersInEvent( TArrayI& outITStracksTArr, TArrayI& outTPCtracksTArr, const IlcESDEvent* pEvent );
    Int_t FindMatchingTracks(TObjArray& arrITS, TObjArray& arrTPC, IlcESDEvent* pESD);
    Bool_t Update();
  ...
  Int_t CheckCovariance(); // check covariance matrix
    ...
    Bool_t SetTrackParams( const IlcExternalTrackParam* exparam1, const IlcExternalTrackParam* exparam2 );
    const IlcExternalTrackParam* GetTrackParams1() const {return fPTrackParam1;}
    const IlcExternalTrackParam* GetTrackParams2() const {return fPTrackParam2;}
    ...
    Bool_t CorrectTrack( IlcExternalTrackParam* tr, const TVectorD& misalignment ) const;
    Bool_t MisalignTrack( IlcExternalTrackParam* tr, const TVectorD& misalignment ) const;
...
protected:
    ...
    Bool_t IsOutlierSigma2Median( const IlcExternalTrackParam* pITS, const IlcExternalTrackParam* pTPC );
...
private:
    ...
    //Track parameters
    IlcExternalTrackParam* fPTrackParam1;   //!local track parameters
    IlcExternalTrackParam* fPTrackParam2;   //!local track parameters
    ...
    ClassDef(IlcRelAlignerKalman,4)     //IlcRelAlignerKalman class
};

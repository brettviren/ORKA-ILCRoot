...
class IlcLoader: public TNamed
 {
   public:
    IlcLoader();
    IlcLoader(const Char_t *detname,const Char_t *eventfoldername); //contructor with name of the top folder of the tree
    IlcLoader(const Char_t *detname,TFolder* eventfolder);

    virtual ~IlcLoader();//----------------- 
 
    virtual Int_t  GetEvent();//changes to root directory
                             //relies on the event number defined in gIlc
    ...
    // 1. Opens the file (accordingly to gIlc->GetEvNumber()/Nevents per File
    ...
    //   "update" - update file
    //   "recreate" 
    IlcDataLoader* GetHitsDataLoader()     const {return (IlcDataLoader*)fDataLoaders->At(kHits);}
    IlcDataLoader* GetSDigitsDataLoader()  const {return (IlcDataLoader*)fDataLoaders->At(kSDigits);}
    IlcDataLoader* GetDigitsDataLoader()   const {return (IlcDataLoader*)fDataLoaders->At(kDigits);}
    IlcDataLoader* GetRecPointsDataLoader()const {return (IlcDataLoader*)fDataLoaders->At(kRecPoints);}
    IlcDataLoader* GetTracksDataLoader()   const {return (IlcDataLoader*)fDataLoaders->At(kTracks);}
    IlcDataLoader* GetRecParticlesDataLoader()const {return (IlcDataLoader*)fDataLoaders->At(kRecParticles);}
 
    IlcDataLoader* GetDataLoader(const char* name);
    void           AddDataLoader(IlcDataLoader* dl);
    ...
    TFolder*       GetTopFolder();  //returns top ilcroot folder
  ...
  //void SetTopFolder(TString& str){}; //Sets root top folder for the run
    ...
    IlcRunLoader*  GetRunLoader();//gets the run-getter from event folder
 ...
 // Max events per file functionality
   ...
   private:
    ...
    IlcLoader(const IlcLoader&); //Not implemented
    IlcLoader& operator=(const IlcLoader&); //Not implemented

    ClassDef(IlcLoader,3)
 };
/******************************************************************/
/************************ I N L I N E S ***************************/
/******************************************************************/

inline TFolder* IlcLoader::GetDetectorDataFolder()
 {

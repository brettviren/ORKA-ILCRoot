#ifndef ILCSIMULATION_H
#define ILCSIMULATION_H
/* 2012-2013 Infrastructure for Large Collider Experiment.       *
 * All rights reserved.                                                   *
 * See cxx source for full Copyright notice                               */

/* $Id: IlcSimulation.h 55676 2012-04-10 19:18:54Z hristov $ */
...
class IlcLego;
class IlcMagF;
class IlcHLTSimulation;

class IlcSimulation: public TNamed {
public:
  IlcSimulation(const char* configFileName = "Config.C",
		const char* name = "IlcSimulation", 
		const char* title = "generation, simulation and digitization");
  virtual ~IlcSimulation();

  static IlcSimulation *Instance() {if(!fgInstance) fgInstance=new IlcSimulation(); return fgInstance;}
  ...
  Bool_t         MisalignGeometry(IlcRunLoader *runLoader = NULL);
  ...
  void SetCDBSnapshotMode(const char* snapshotFileName) {IlcCDBManager::Instance()->SetSnapshotMode(snapshotFileName);}
  ...
  virtual Bool_t RunLego(const char *setup="Config.C",Int_t nc1=60,Float_t c1min=2,Float_t c1max=178,
			 ...
			 Float_t rmax=430,Float_t zmax=10000, IlcLegoGenerator* gener=NULL, Int_t nev = -1);
  ...
  IlcLego* Lego() const {return fLego;}
  virtual  void  FinishRun();

  //Quality Assurance
  Int_t       GetDetIndex(const char * detector);
  void        SetQACycles(IlcQAv1::DETECTORINDEX_t det, const Int_t cycles) {  IlcQAManager::QAManager()->SetCycleLength(det, cycles) ; }
  Bool_t      RunQA() ;
  Bool_t      SetRunQA(TString detAndAction="ALL:ALL") ; 
  void        SetQAWriteExpert(IlcQAv1::DETECTORINDEX_t det) { IlcQAManager::QAManager()->SetWriteExpert(det) ; }  
...
private:
  ...
  IlcSimulation(const IlcSimulation&); // Not implemented
  IlcSimulation& operator = (const IlcSimulation&); // Not implemented

  void 		 InitCDB();
  void 		 InitRunNumber();
  void 		 SetCDBLock();
  Bool_t         SetRunNumberFromData();
  IlcRunLoader*  LoadRun(const char* mode = "UPDATE") const;
  Int_t          GetNSignalPerBkgrd(Int_t nEvents = 0) const;
  Bool_t         IsSelected(TString detName, TString& detectors) const;

  static IlcSimulation *fgInstance;    // Static pointer to object
  ...
  Int_t 	   fRun; 		                     //! Run number, will be passed to CDB and gIlc!!
  ...
  IlcLego       *fLego;                //! Pointer to ilcLego object if it exists
  // OCDB
...
//   #endif
  static const Int_t   fgkNDetectors = 5 ;             // number of detectors    // AU
  static const char *  fgkDetectorName[fgkNDetectors] ; // names of detectors
  TString              fQADetectors ;                   // list of detectors to be QA'ed 	
  TString              fQATasks ;                       // list of QA tasks to be performed	
  Bool_t               fRunQA ;                         // Runs the QA at the end of simulation
  IlcRecoParam::EventSpecie_t fEventSpecie ;            // type of event (see IlcRecoParam::EventSpecie_t)
  ...
  IlcHLTSimulation*    fpHLT;         //! The instance of HLT simulation

  Bool_t         fWriteGRPEntry;      // Write or not GRP entry corresponding to the settings in Config.C

  ClassDef(IlcSimulation, 12)  // class for running generation, simulation and digitization
};

...
ClassImp(IlcSurveyObj)


const TString IlcSurveyObj::fgkStorage = "alien://ilc.cern.ch";  
const TString IlcSurveyObj::fgkBaseFolder = "/ilc/data/Reference";
const TString IlcSurveyObj::fgkValidDetectors = "ACORDE,BABYFRAME,BACKFRAME,\
...
const TString IlcSurveyObj::fgkGRPDetectors = "BABYFRAME,BACKFRAME,L3 MAGNET,\
SPACEFRAME,MUON DIPOLE,MUON ABSORBERS,GRP";
const TString IlcSurveyObj::fgkMUONDetectors = "MUON,SUPERSTRUCTURE";

  
//_____________________________________________________________________________
IlcSurveyObj::IlcSurveyObj():
  TObject(),
  fTitle(""),
  fDate(""),
  fDetector(""),
...
//_____________________________________________________________________________
IlcSurveyObj::IlcSurveyObj(const IlcSurveyObj& surveyObj):
  TObject(),
...
//_____________________________________________________________________________
IlcSurveyObj& IlcSurveyObj::operator=(const IlcSurveyObj& surveyObj)
{
  // assignment operator
  if (this != &surveyObj) {
    fTitle = surveyObj.fTitle;
...
//_____________________________________________________________________________
void IlcSurveyObj::AddPoint(IlcSurveyPoint* point) {
...
//_____________________________________________________________________________
Bool_t IlcSurveyObj::Connect(const char *gridUrl, const char *user) {
  ...
  if (!gGrid || gridUrl != gGrid->GridUrl() ||
    ...
    if (gGrid) {
      IlcInfo(Form("gGrid = %p; GridUrl = %s; gGrid->GridUrl() = %s", 
		   gGrid, gridUrl, gGrid->GridUrl()));
      IlcInfo(Form("User = %s; gGrid->GetUser() = %s",
		   user, gGrid->GetUser()));
    }
...
Bool_t IlcSurveyObj::OpenFile(TString openString) {
  ...
  if ( file->ReadBuffer(buf, size) ) {
    IlcError("Error reading file contents to buffer!");
    return kFALSE;
  }
  IlcInfo(Form("%lld bytes read!\n", file->GetBytesRead()));
...
//_____________________________________________________________________________
Bool_t IlcSurveyObj::FillFromLocalFile(const Char_t* filename) {
...
//_____________________________________________________________________________
Bool_t IlcSurveyObj::IsValidDetector(TString detector) const {
...
//_____________________________________________________________________________
TString IlcSurveyObj::RealFolderName(TString detector) const {
...
//_____________________________________________________________________________
Bool_t IlcSurveyObj::Fill(TString detector, Int_t reportNumber,
                          TString username) {
...
Bool_t IlcSurveyObj::Fill(TString detector, Int_t reportNumber,
  ...
  if (!IsValidDetector(detector)) {
    IlcWarning(Form("Detector '%s' is not a valid detector/structure!", detector.Data()));
    return kFALSE;
  }
  ...
  if ((reportNumber < 1) || (reportVersion < -1) || (0 == reportVersion)) {
    IlcError("Invalid parameter values for IlcSurveyObj::Fill. (Report Number or Report Version)");
    return kFALSE;
  }
  ...
  if (1 == numberEntries) fileNamePath = res->GetFileNamePath(0);
	...
	IlcWarning("Inconsistency detected, year differs for reports with the same report number! Please inform the responsible and check the report against the one in DCDB.");
    }
...
//_____________________________________________________________________________
TString IlcSurveyObj::FileNamePathToDetector(TString filename) const {
  // Get the report number from the complete path in the format:
  // /ilc/data/Reference/HMPID/RawSurvey/2006/781282_v1.txt
...
///ilc/cern.ch/user/r/rsilva/TRD/RawSurvey/2007/.816582_v2.txt/v1.0

//_____________________________________________________________________________
Int_t IlcSurveyObj::FileNamePathToReportYear(TString filename) const {
...
//_____________________________________________________________________________
Int_t IlcSurveyObj::FileNamePathToReportNumber(TString filename) const {
  // Get the report number from the complete path in the format:
  // /ilc/data/Reference/HMPID/RawSurvey/2006/781282_v1.txt
  ...
  if (filename.Length() > fgkBaseFolder.Length()) {
    ...
    if ((ret.CountChar('/') > 3) || (ret.CountChar('.') > 1)) {
      IlcWarning("Error getting the Report Number from the filename path!");
      return -1;
    }
...
//_____________________________________________________________________________
Int_t IlcSurveyObj::FileNamePathToReportVersion(TString filename) const {
  ...
  if (filename.Length() > fgkBaseFolder.Length()) {
    ...
    ret.Remove(TString::kLeading, '/');
    if ((ret.CountChar('/') > 3) || (ret.CountChar('.') > 1)) {
      IlcWarning("Error getting the Report Version from the filename path!");
      return -1;
    }
...
//_____________________________________________________________________________
void IlcSurveyObj::ListValidDetectors() {
...
//_____________________________________________________________________________
TGridResult * IlcSurveyObj::QueryReports(TString detector, Int_t year,
					 Int_t reportNumber,
					 Int_t reportVersion) {
  ...
  if (detector.Length() > 0) {
    ...
    // Check if <detector> is valid
    if (!IsValidDetector(detector)) {
      IlcError(Form("Detector '%s' is not a valid detector/structure!",
		    detector.Data()));
      return 0;
    }
    // Some "detectors" don't have a folder of their own
  ...
  if (0 == fGridUser.Length()) {
    IlcError("To use this method it's necessary to call SetGridUser(...) in advance.");
    return 0;
...
//_____________________________________________________________________________
Int_t IlcSurveyObj::ListReports(TString detector, Int_t year, 
				Int_t reportNumber,
				Int_t reportVersion) {
  ...
  if (numberEntries > 0) {
    ...
    IlcInfo("No results found for the requested query.");
    delete res;
    return 0;
  }
}
...
//_____________________________________________________________________________
void IlcSurveyObj::SetGridUser(TString username){
...
//_____________________________________________________________________________
TString &IlcSurveyObj::Sanitize(TString str) {
  ...
  if (!str.IsAscii()) {
    IlcWarning("Warning: Non-ASCII characters!\n");
    str = "";
  }
...
//_____________________________________________________________________________
Bool_t IlcSurveyObj::ParseBuffer(const Char_t* buf) {
  // Parses a character buffer assuming the format defined with the TS/SU
  // http://ilcinfo/Offline/Activities/Alignment/SurveyInformation.html
  ...
  for (Int_t i = 0; i < nrLines; ++i) {
    ...
    if (currLine.BeginsWith(">") && !nextLine.BeginsWith(">")) {
      ...
      IlcDebug(2, Form(" -> field line: \"%s\"\n", currLine.Data()));
      IlcDebug(2, Form(" -> value line: \"%s\"\n", nextLine.Data()));
	...
	if (nextLine.BeginsWith("http://edms.cern.ch/document/", TString::kIgnoreCase) ||
	  ...
	  if (!nextLine.IsDigit()) {
	    IlcError("Survey text file sintax error! (incorrectly formatted Report URL)");
	    IlcError(Form("Wrong report number string: \"%s\"",nextLine.Data()));
	    lines->Delete();
	...
	if (!nextLine.IsDigit()) {
	  lines->Delete();
	  delete lines; lines = NULL;
	  IlcError("Survey text file sintax error! (incorrectly formatted Report Version)");
	  return kFALSE;
	}
	...
	if (!nextLine.IsDigit()) {
	  lines->Delete();
	  delete lines; lines = NULL;
	  IlcError("Survey text file sintax error! (incorrectly formatted Number of Columns)");
	  return kFALSE;
	}
	...
	if (colLine->GetEntries() != fNrColumns) {
	  IlcError("Survey text file sintax error! (Declared number of Columns doesn't match number of column names)");
	  colLine->Delete();
	  lines->Delete();
	  delete lines; lines = NULL;
	  return kFALSE;
	...
	for (Int_t j = 0; j < fNrColumns; ++j) {
	  ...
	  if (cn.BeginsWith("Point Name", TString::kIgnoreCase)) {
		...
		IlcWarning("Global precision will not be used for X axis");
	      }
	      if(!orderedValues[7]){
		orderedValues[7] = &value[j];
	        check[7] = kTRUE;
	      }else{
		IlcWarning("Global precision will not be used for Y axis");
	      }
	      if(!orderedValues[8]){
		orderedValues[8] = &value[j];
		check[8] = kTRUE;
	      }else{
		IlcWarning("Global precision will not be used for Z axis");
	      }
	    } else {
	      Bool_t orXYZ = kFALSE;
		...
		if(!prX){
		  ...
		  IlcError("Precision for X axis was already set!");
		  return kFALSE;
		}
	      }
		...
		if(!prY){
		  ...
		  IlcError("Precision for Y axis was already set!");
		  return kFALSE;
		}
	      }
		...
		if(!prZ){
		  ...
		  IlcError("Precision for Z axis was already set!");
		  return kFALSE;
		}
	      }
	      if(!orXYZ)
	      {
		IlcError("Survey text file sintax error: precision column name does not refer to any axis!");
		return kFALSE;
	      }
	    }
	...
	if(!check[0]){
	  IlcError("Missing mandatory column \"Point Name\"!");
	  return kFALSE;
	}
	if(!(check[1]&&check[2]&&check[3])){
	  IlcError("Missing one or more mandatory columns for coordinates \"X\",\"Y\",\"Z\"");
	  return kFALSE;
	}
	if(!check[4]){
	  IlcError("Missing mandatory column \"Point Type\"!");
	  return kFALSE;
	}
	if(!(check[6]&&check[7]&&check[8])){
	  IlcError("Missing one or more mandatory columns for precision along \"X\",\"Y\",\"Z\" axes");
	  return kFALSE;
	}
	...
	while ((nextLine.Length() > 0) && ('>' != nextLine[0])) {
	  ...
	  if (fNrColumns == nextLine.CountChar(',') + 1) dataLine = nextLine.Tokenize(',');
	    ...
	    IlcError("Survey text file syntax error! Error processing data line!");
	    lines->Delete();
	    delete lines; lines = NULL;
	    return kFALSE;
	  }
	  ...
	  if (dataLine->GetEntries() != fNrColumns) {
	    ...
	    IlcError("Survey text file sintax error! (Number of entries in line is different from number of Columns)");
	    dataLine->Delete();
	    lines->Delete();
	    delete lines; lines = NULL;
	    return kFALSE;
	  ...
	  dp = new IlcSurveyPoint(tmpname, tmpx, tmpy, tmpz, tmpprecX, tmpprecY, tmpprecZ, tmptype, tmptarg);
	  if(IlcLog::GetDebugLevel("","IlcSurveyObj")>1) dp->PrintPoint();
	  AddPoint(dp);
      ...
      IlcError("Survey text file sintax error!");
      lines->Delete();
      delete lines; lines = NULL;
      return kFALSE;
    }
...
//_____________________________________________________________________________
void IlcSurveyObj::Reset() {
  // Resets the IlcSurveyObj to a clean object.

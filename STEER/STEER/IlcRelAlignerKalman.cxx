...
//
//        Bool_t AddESDevent( IlcESDevent* pTrack );
//    
//    or add each individual track
//
//        AddESDtrack( IlcESDtrack* pTrack );
//
...
//    one IlcESDEvent. The method
//
//        Bool_t AddCosmicEvent( IlcESDEvent* pEvent );
//
...
ClassImp(IlcRelAlignerKalman)

//______________________________________________________________________________
IlcRelAlignerKalman::IlcRelAlignerKalman():
    TObject(),
    fPTrackParam1(new IlcExternalTrackParam()),
    fPTrackParam2(new IlcExternalTrackParam()),
    fMagField(0.),
...
//______________________________________________________________________________
IlcRelAlignerKalman::IlcRelAlignerKalman(const IlcRelAlignerKalman& a):
...
//______________________________________________________________________________
IlcRelAlignerKalman& IlcRelAlignerKalman::operator=(const IlcRelAlignerKalman& a)
{
  //assignment operator
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::AddESDevent( const IlcESDEvent* pEvent )
{
  ...
  for (Int_t i=0; i<pEvent->GetNumberOfTracks(); i++)
  {
    ...
    if ( ((track->GetStatus()&IlcESDtrack::kTPCin)>0)&&
         ((track->GetStatus()&IlcESDtrack::kVXDrefit)>0)&&
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::AddESDtrack( const IlcESDtrack* pTrack )
{
  //Adds a full track, returns true if results in a new estimate
  //  gets the inner TPC parameters from IlcESDTrack::GetInnerParam()
  //  gets the outer ITS parameters from IlcESDfriendTrack::GetITSout()

  const IlcExternalTrackParam* pconstparamsITS = pTrack->GetOuterParam();
  if (!pconstparamsITS) return kFALSE;
  const IlcExternalTrackParam* pconstparamsTPC = pTrack->GetInnerParam();
  if (!pconstparamsTPC) return kFALSE;
  
  //TPC part
  IlcExternalTrackParam paramsTPC = (*pconstparamsTPC);
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::AddTrackParams( const IlcExternalTrackParam* p1, const IlcExternalTrackParam* p2 )
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::AddCosmicEvent( const IlcESDEvent* pEvent )
{
  ...
  SetMagField( pEvent->GetMagneticField() );
  IlcESDtrack* ptrack=NULL;
  const IlcExternalTrackParam* pconstparams1;
  const IlcExternalTrackParam* pconstparams2;
  IlcExternalTrackParam params1;
  IlcExternalTrackParam params2;
...
//______________________________________________________________________________
void IlcRelAlignerKalman::SetPoint2Track( Bool_t set )
{
...
//______________________________________________________________________________
void IlcRelAlignerKalman::Print(Option_t*) const
{
...
//______________________________________________________________________________
void IlcRelAlignerKalman::PrintSystemMatrix()
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::SetTrackParams( const IlcExternalTrackParam* exparam1, const IlcExternalTrackParam* exparam2 )
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::Update()
{
  //perform the update
...
//______________________________________________________________________________
void IlcRelAlignerKalman::RotMat( TMatrixD &R, const TVectorD& angles )
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::PrepareMeasurement()
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::PrepareSystemMatrix()
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::PreparePrediction()
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::PredictMeasurement( TVectorD& pred, const TVectorD& state )
{
  ...
  if (fCorrectionMode)
  {
      IlcExternalTrackParam track(*fPTrackParam2); //make a copy track
  ...
  {
      IlcExternalTrackParam track(*fPTrackParam1); //make a copy track
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::UpdateEstimateKalman()
{
...
//  }
  ...
  {
    fNOutliers++;
    //printf("IlcRelAlignerKalman: outlier\n");
    return kFALSE;
  }
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::IsOutlier( const TVectorD& update, const TMatrixDSym& covmatrix )
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::IsOutlierSigma2Median( const IlcExternalTrackParam* pITS, 
                                                   const IlcExternalTrackParam* pTPC )
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::IsPositiveDefinite( const TMatrixD& mat ) const
{
...
//______________________________________________________________________________
void IlcRelAlignerKalman::TMatrixDSymFromTMatrixD( TMatrixDSym& matsym, const TMatrixD& mat )
{
...
//______________________________________________________________________________
void IlcRelAlignerKalman::Angles( TVectorD &angles, const TMatrixD &rotmat )
{
...
//______________________________________________________________________________
void IlcRelAlignerKalman::PrintCorrelationMatrix()
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::FindCosmicTrackletNumbersInEvent( TArrayI& outITSindexTArr, TArrayI& outTPCindexTArr, const IlcESDEvent* pEvent )
{
  ...
  for (Int_t itrack=0; itrack < ntracks; itrack++)
  {
    ...
    //check if track is VXD
    Int_t nClsITS = pTrack->GetNcls(0);
    Int_t nClsTPC = pTrack->GetNcls(1);
    if ( ((pTrack->GetStatus()&IlcESDtrack::kVXDout)>0)&&
         !((pTrack->GetStatus()&IlcESDtrack::kTPCin)>0)&&
    ...
    if ( ((pTrack->GetStatus()&IlcESDtrack::kTPCin)>0)&&
  ...
  for (Int_t itpc=0;itpc<nCandidateTPCtracks;itpc++)
  {
    ...
    {
      IlcESDtrack* itstrack = pEvent->GetTrack(goodtracksArr[tracksArrITS[iits]]);
      const IlcExternalTrackParam* parits = itstrack->GetOuterParam();
      IlcESDtrack* tpctrack = pEvent->GetTrack(goodtracksArr[candidateTPCtracksArr[itpc]]);
      const IlcExternalTrackParam* tmp = tpctrack->GetInnerParam();
      IlcExternalTrackParam partpc(*tmp);  //make a copy to avoid tampering with original params
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::CorrectTrack( IlcExternalTrackParam* tr, const TVectorD& misal ) const
{
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::MisalignTrack( IlcExternalTrackParam* tr, const TVectorD& misal ) const
{
  //implements the system model -
...
//______________________________________________________________________________
void IlcRelAlignerKalman::Reset()
{
...
//______________________________________________________________________________
void IlcRelAlignerKalman::ResetCovariance( const Double_t number )
{
...
//______________________________________________________________________________
void IlcRelAlignerKalman::ResetTPCparamsCovariance( const Double_t number )
{
  //Resets the covariance to the default if arg=0 or resets the off diagonals
  //to zero and releases the diagonals by factor arg.
...
//______________________________________________________________________________
Bool_t IlcRelAlignerKalman::Merge( const IlcRelAlignerKalman* al )
{
  //Merge two aligners
  ...
  {
    fNMergesFailed++;
    //printf("IlcRelAlignerKalman::Merge failed\n");
...
//______________________________________________________________________________
Long64_t IlcRelAlignerKalman::Merge( TCollection* list )
{
  //merge all aligners in the collection
  Long64_t numberOfMerges=0;
  IlcRelAlignerKalman* alignerFromList;
  if (!list) return 0;
  TIter next(list);
  while ( (alignerFromList = dynamic_cast<IlcRelAlignerKalman*>(next())) )
  {
...
//______________________________________________________________________________
Int_t IlcRelAlignerKalman::Compare(const TObject *obj) const
{
  if (this == obj) return 0;
  const IlcRelAlignerKalman* aobj = dynamic_cast<const IlcRelAlignerKalman*>(obj);
  if (!aobj) return 0;
...
Int_t IlcRelAlignerKalman::FindMatchingTracks(TObjArray& arrITS, TObjArray& arrTPC, IlcESDEvent* pESD)
{
  ...
  for (Int_t i=0; i<ntracks; i++)
  {
    ...
    IlcESDtrack* track1 = pESD->GetTrack(i);
    if (!track1) continue;

    if (track1->GetNcls(0) < fMinPointsVol1) continue;

    if (!( ( track1->IsOn(IlcESDtrack::kVXDrefit)) &&
           (!track1->IsOn(IlcESDtrack::kTPCin)) )) continue;

    const IlcESDfriendTrack* constfriendtrack1 = track1->GetFriendTrack();
    if (!constfriendtrack1) continue;
    IlcESDfriendTrack friendtrack1(*constfriendtrack1);
    
    if (!friendtrack1.GetITSOut()) continue;
    IlcExternalTrackParam params1(*(friendtrack1.GetITSOut()));
    ...
    for (Int_t j=0; j<ntracks; j++)
    {
      ...
      IlcESDtrack* track2 = pESD->GetTrack(j);
      if (!track2) continue;
      if (track1==track2) continue;
      //if ( ( ( track2->IsOn(IlcESDtrack::kVXDout)) &&
      //       (!track2->IsOn(IlcESDtrack::kTPCin)) )) continue; //all but ITS standalone
      ...
      IlcExternalTrackParam params2(*(track2->GetInnerParam()));
      ...
      {
        arrITS[iMatched] = new IlcExternalTrackParam(params1);
        arrTPC[iMatched] = new IlcExternalTrackParam(params2);
      }//else
    }//for j
  }//for i
  delete [] matchedArr;
...
//________________________________________________________________________
void IlcRelAlignerKalman::SetRejectOutliersSigma2Median(const Bool_t set )
{
...
Int_t IlcRelAlignerKalman::CheckCovariance(){
  //

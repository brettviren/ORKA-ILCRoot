...
ClassImp(IlcTrackFitterKalman)

const Double_t IlcTrackFitterKalman::fgkMaxChi2=33.;

IlcTrackFitterKalman::
IlcTrackFitterKalman(IlcTrackPointArray *array, Bool_t owner):
...
Bool_t IlcTrackFitterKalman::Begin(Int_t first, Int_t last) {
  //
...
Bool_t IlcTrackFitterKalman::
MakeSeed(const IlcTrackPoint *p0, const IlcTrackPoint *p1) {
  //
  ...
  if (dy==0.) { 
     IlcError("Seeds perpendicular to Y axis are not allowed !"); 
     return kFALSE; 
  }
  Double_t vx=dx/dy;
  Double_t vz=dz/dy;
...
Bool_t IlcTrackFitterKalman::AddPoint(const IlcTrackPoint *p)
{
  //
...
Double_t IlcTrackFitterKalman::GetPredictedChi2(const IlcTrackPoint *p) const {
  //
  ...
  IlcTrackPoint tp(txyz,tcov,p->GetVolumeID());
...
Bool_t IlcTrackFitterKalman::Propagate(const IlcTrackPoint *p) {
  //
...
Bool_t IlcTrackFitterKalman::Update(const IlcTrackPoint *p, Double_t chi2) {
  //
...
Bool_t IlcTrackFitterKalman::GetPCA(const IlcTrackPoint &p, IlcTrackPoint &i) const
{
  //
...
//_____________________________________________________________________________
void 
IlcTrackFitterKalman::SetSeed(const Double_t par[5], const Double_t cov[15]) {
  //

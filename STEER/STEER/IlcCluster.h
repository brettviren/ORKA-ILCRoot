...
class IlcCluster : public TObject {
 public:
  ...
  IlcCluster();
  IlcCluster(UShort_t volId, const Float_t *hit, Float_t x = 0, Float_t sigyz = 0, const Int_t *lab = NULL);
  IlcCluster(UShort_t volId,
		 Float_t x, Float_t y, Float_t z,
		 Float_t sy2, Float_t sz2, Float_t syz,
		 const Int_t *lab = NULL);
  IlcCluster(const IlcCluster& cluster);
  IlcCluster &operator=(const IlcCluster& cluster);
  virtual ~IlcCluster() {;}
 ...
 private:
  ...
  ClassDef(IlcCluster,3) // Barrel detectors cluster
};

...
//  value --> TObjArray of IlcDCSArray objects                               //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
//--------------------------------------------------------------------------
TMap* IlcLHCReader::ReadLHCDP(TString filename)
{
	//
  ...
  gSystem->ExpandPathName(filename);
	...
	if (!*file) {
		IlcError(Form("Error opening file (%s) !",filename.Data()));
		file->close();
		delete file;
		return NULL;
	}
	...
	while(strLine.ReadLine(*file)){
		nline++;
		IlcDebug(3,Form("line n. = %d",nline));
		...
		//
		IlcDebug(3,Form("Number of tokens = %d",ntokens));
		if (ntokens == 2 && !(((TObjString*)tokens->At(0))->String()).CompareTo("LHC_ENTRIES")){
			lhcEntries = (((TObjString*)tokens->At(1))->String()).Atoi();
			IlcInfo(Form("LHC entries = %d",lhcEntries));
			IlcDebug(3,Form("LHC entries = %d",lhcEntries));
			continue;
		}
		if (ntokens == 1 && !(((TObjString*)tokens->At(0))->String()).CompareTo("END_OF_DATA")){
			IlcDebug(2,"End of file reached");
			delete tokens;
			break;
		}
		if (ntokens < 4){  
			IlcInfo(Form("Wrong number of tokens --> # tokens = %d at line %d",ntokens,nline));
		...
		IlcDebug(2,Form("lhcDPindex = %d",lhcDPindex));
		TObjString* lhcDPname = (TObjString*)tokens->At(1);
		TString lhcDPtype = ((TObjString*)tokens->At(2))->String();
		IlcDebug(2,Form("lhcDPname = %s",(lhcDPname->String()).Data()));
		IlcDebug(2,Form("lhcDPtype = %s",lhcDPtype.Data()));
		...
		if (typeTokens->GetEntriesFast() < 2 ){  
			...
			IlcError(Form("The format does not match the expected one, skipping the current line for DP = %s", lhcDPtype.Data()));
			delete typeTokens;
			delete tokens;
			continue;
		}
		TString type = ((TObjString*)typeTokens->At(0))->String();
		IlcDebug(2,Form("type = %s",type.Data()));
      		Int_t nelements = (((TObjString*)typeTokens->At(1))->String()).Atoi();
		IlcDebug(2,Form("nelements = %i",nelements));
		Int_t nentries = (((TObjString*)tokens->At(3))->String()).Atoi();
		IlcDebug(2,Form("nentries = %i",nentries));
		...
		if (mapLHC->GetValue(lhcDPname)==0x0){
			...
			IlcDebug(2,Form("entry found! --> %p",array));
		}
		...
		for (Int_t ientry=0; ientry< nentries; ientry ++){
			...
			IlcDebug(2,Form("Timestamp in unix time = %f (s)",timestamp));
			if (fStartTime!=0 && fEndTime!=0 && (fStartTime > timestamp || fEndTime < timestamp)){
				...
				IlcError(Form("Timestamp for entry %d of DP %s not in [%d,%d]", ientry, lhcDPtype.Data(),fStartTime,fEndTime));
				continue;
			}
			if (type == "i"){
				...
				for (Int_t ielement=0; ielement<nelements; ielement++){
					...
					IlcDebug(2,Form("Value at index %d = %d",nfixed+ielement+ientry*nValuesPerEntry,value[ielement]));
				}
				IlcDCSArray* dcs = new IlcDCSArray(nelements,value,timestamp);
				array->Add(dcs);
				delete[] value;
			}
			else if (type == "b"){
				...
				for (Int_t ielement=0; ielement<nelements; ielement++){
					...
					IlcDebug(2,Form("Value at index %d = %d",nfixed+ielement+ientry*nValuesPerEntry,Int_t(value[ielement])));
				}
				IlcDCSArray* dcs = new IlcDCSArray(nelements,value,timestamp);
				array->Add(dcs);
				delete[] value;
				...
				for (Int_t ielement=0; ielement<nelements; ielement++){
					...
					IlcDebug(2,Form("Value at index %d = %f from string %s",nfixed+ielement+ientry*nValuesPerEntry,value[ielement],tempstr.Data()));
				} 
				IlcDCSArray* dcs = new IlcDCSArray(nelements,value,timestamp);
				array->Add(dcs);
				delete[] value;
				...
				for (Int_t ielement=0; ielement<nelements; ielement++){
				  TObjString* strobj = (new TObjString(((TObjString*)tokens->At(nfixed+ielement+ientry*nValuesPerEntry))->String()));
					IlcDebug(2,Form("Value at index %d = %s",nfixed+ielement+ientry*nValuesPerEntry,(strobj->String()).Data()));
					value->Add(strobj);
				}
				IlcDCSArray* dcs = new IlcDCSArray(nelements,value,timestamp);
				array->Add(dcs);
				...
				IlcError(Form("Non-expected type %s",type.Data()));
				delete typeTokens;
				delete tokens;	
				file->close();
				delete file;	
...
//--------------------------------------------------------------------------
TObjArray* IlcLHCReader::ReadSingleLHCDP(TString filename, TString alias)
{
	//
  ...
  gSystem->ExpandPathName(filename);
	...
	if (selection.Length() == 0) {
		IlcError(Form("Alias %s not fouond in LHC Data file, returning a null pointer",alias.Data()));
		return NULL;
	}
	...
	IlcDebug(3,Form("Number of tokenslines = %d",ntokenslines));

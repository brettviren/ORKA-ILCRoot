...
/* $Id: IlcTrackFitterRieman.cxx 50615 2011-07-16 23:19:19Z hristov $ */
...
//
// Internal usage of IlcRieman class for minimization
//
//////////////////////////////////////////////////////////////////////////////
...
ClassImp(IlcTrackFitterRieman)


IlcTrackFitterRieman::IlcTrackFitterRieman():
  IlcTrackFitter(),
  fBCorrection(kFALSE),
  fAlpha(0.),
  fNUsed(0),
  fConv(kFALSE),
  fMaxDelta(3),
  fRieman(new IlcRieman(10000)),  // allocate rieman
  fMinPointRadius(2.),
  fMaxPointRadius(500.),
...
IlcTrackFitterRieman::IlcTrackFitterRieman(IlcTrackPointArray *array, Bool_t owner):
  IlcTrackFitter(array,owner),
  fBCorrection(kFALSE),
  fAlpha(0.),
  fNUsed(0),
  fConv(kFALSE),
  ...
  fDebugStream(0)
{
  ...
  fCorrY[0] = fCorrY[1] = fCorrY[2] = fCorrY[3] = 0;
  fCorrZ[0] = fCorrZ[1] = fCorrZ[2] = fCorrZ[3] = 0;
  if (IlcLog::GetDebugLevel("","IlcTrackFitterRieman")) fDebugStream = new TTreeSRedirector("RiemanAlignDebug.root");
}

IlcTrackFitterRieman::IlcTrackFitterRieman(const IlcTrackFitterRieman &rieman):
  IlcTrackFitter(rieman),
  fBCorrection(rieman.fBCorrection),
  fAlpha(rieman.fAlpha),
  fNUsed(rieman.fNUsed),
  fConv(rieman.fConv),
  fMaxDelta(rieman.fMaxDelta),
  fRieman(new IlcRieman(*(rieman.fRieman))),
...
IlcTrackFitterRieman &IlcTrackFitterRieman::operator =(const IlcTrackFitterRieman& rieman)
{
  //
  // Assignment operator
  //
  if(this==&rieman) return *this;
  ((IlcTrackFitter *)this)->operator=(rieman);
...
void IlcTrackFitterRieman::Reset()
{
  // Reset the track parameters and
  // rieman sums
  //
  IlcTrackFitter::Reset();
  fRieman->Reset();
  fAlpha = 0.;
  fNUsed = 0;
  fConv =kFALSE;
...
Bool_t IlcTrackFitterRieman::Fit(const TArrayI *volIds,const TArrayI *volIdsFit,
			 ...
			 IlcGeomManager::ELayerID layerRangeMax)
{
  ...
  // As a result two IlcTrackPointArray's obects are filled.
  ...
  // found using the covariance matrix of the point
  // (assuming sigma(x)=0 at the reference coordinate system.
  Int_t debugLevel = IlcLog::GetDebugLevel("","IlcTrackFitterRieman");
  
  //  Float_t debugRatio = 1./(1.+debugLevel);
  Float_t debugRatio = debugLevel? 1.0/debugLevel : 1.0;

  if (!fPoints) {
    IlcError("Track points array not available! Exiting...");
    return kFALSE;
  }
  ...
  if (fPoints && volIdsFit && IlcLog::GetDebugLevel("","IlcTrackFitterRieman")>1&& gRandom->Rndm()<debugRatio){
...
//   fAlpha = TMath::ATan2(p.GetY(),p.GetX());
  ...
  for (Int_t ipoint = 0; ipoint < npoints; ipoint++)
	...
	if (iVolId < IlcGeomManager::LayerToVolUID(layerRangeMin,0) ||
	    iVolId > IlcGeomManager::LayerToVolUID(layerRangeMax,
						IlcGeomManager::LayerSize(layerRangeMax))) continue;
      }
      if (!isAlphaCalc) {
	fAlpha = p.GetAngle();
	isAlphaCalc = kTRUE;
  ...
  fPVolId = new IlcTrackPointArray(npVolId);
  fPTrack = new IlcTrackPointArray(npVolId);
  IlcTrackPoint p2;
  ...
  if (IlcLog::GetDebugLevel("","IlcTrackFitterRieman")>0 && gRandom->Rndm()<debugRatio){
    //
    // Debug Info
    //
    IlcTrackPointArray *lPVolId = new IlcTrackPointArray(npVolId);
    IlcTrackPointArray *lPTrack = new IlcTrackPointArray(npVolId);
    IlcTrackPointArray *lPTrackE = new IlcTrackPointArray(npVolId);
    IlcRieman * residual = fRieman->MakeResiduals();
    for (Int_t ipoint = 0; ipoint < npVolId; ipoint++){
      IlcTrackPoint p0, p0local;      
      IlcTrackPoint pFit, pFitlocal, pFitLocalE;      
    ...
    Int_t volId   = volIds->At(0);   
    Int_t modId   =0;
    Int_t layer   =  IlcGeomManager::VolUIDToLayer(volId,modId);
    Int_t volIdFit   = volIdsFit->At(0);   
    Int_t modIdFit   =0;
    Int_t layerFit   =  IlcGeomManager::VolUIDToLayer(volIdFit,modIdFit);
...
void IlcTrackFitterRieman::AddPoint(Float_t x, Float_t y, Float_t z, Float_t sy, Float_t sz)
{
  //
...
Bool_t IlcTrackFitterRieman::Update(){
  //
  // 
  //
  fRieman->Update();
...
//_____________________________________________________________________________
Bool_t IlcTrackFitterRieman::GetPCA(const IlcTrackPoint &p, IlcTrackPoint &p2) const
{
  //
  ...
  if ((r*r - (x-x0p)*(x-x0p))<0) {
    IlcWarning(Form("Track extrapolation failed ! (Track radius = %f, track circle x = %f, space-point x = %f, reference plane angle = %f\n",r,x0p,x,alphap));
    return kFALSE;
  }
  ...
  if (IlcLog::GetDebugLevel("","IlcTrackFitterRieman")>0 && gRandom->Rndm()<debugRatio){
    IlcTrackPoint lp0(p);
    IlcTrackPoint lp2(p2);
    IlcTrackPoint localp0(p);
    IlcTrackPoint localp2(p2);
...
Double_t IlcTrackFitterRieman::GetYat(Double_t x) const  {
  //
...
Double_t IlcTrackFitterRieman::GetZat(Double_t x) const  {
  //
...
Double_t IlcTrackFitterRieman::GetErrY2at(Double_t x) const {
  //
...
Double_t IlcTrackFitterRieman::GetErrZ2at(Double_t x) const {
  //
  // get estimate of extrapolation error
  //
...
void IlcTrackFitterRieman::SetParam(Int_t i, Double_t par) {
  if (i<0 || i>5) return;
  fParams[i]=par;

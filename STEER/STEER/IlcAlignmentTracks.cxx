...
//   IlcTrackFitter class) and minimization fo the
...
ClassImp(IlcAlignmentTracks)

//______________________________________________________________________________
IlcAlignmentTracks::IlcAlignmentTracks():
  fESDChain(0),
  fPointsFilename("IlcTrackPoints.root"),
  fPointsFile(0),
  fPointsTree(0),
  fLastIndex(0),
  fArrayIndex(0),
...
//______________________________________________________________________________
IlcAlignmentTracks::IlcAlignmentTracks(TChain *esdchain):
  fESDChain(esdchain),
  fPointsFilename("IlcTrackPoints.root"),
  fPointsFile(0),
  fPointsTree(0),
...
//______________________________________________________________________________
IlcAlignmentTracks::IlcAlignmentTracks(const char *esdfilename, const char *esdtreename):
...
//______________________________________________________________________________
void IlcAlignmentTracks::AddESD(TChain *esdchain)
{
...
//______________________________________________________________________________
void IlcAlignmentTracks::AddESD(const char *esdfilename, const char *esdtreename)
{
...
void IlcAlignmentTracks::ProcessESD(Bool_t onlyITS,
				    Int_t minITSpts,
				    Bool_t cuts,
				    ...
				    Float_t minSinTheta,Float_t maxSinTheta)
{
  ...
  if (fPointsFilename.IsNull()) {
    IlcWarning("Incorrect output filename!");
    return;
  }

  TFile *pointsFile = TFile::Open(fPointsFilename,"RECREATE");
  if (!pointsFile || !pointsFile->IsOpen()) {
    IlcWarning(Form("Can't open %s !",fPointsFilename.Data()));
    return;
  }

  TTree *pointsTree = new TTree("spTree", "Tree with track space point arrays");
  const IlcTrackPointArray *array = 0;
  IlcTrackPointArray *array2 = 0;
  if(onlyITS) {   // only ITS IlcTrackPoints 
    pointsTree->Branch("SP","IlcTrackPointArray", &array2);
  } else {
    pointsTree->Branch("SP","IlcTrackPointArray", &array);
  } 
  ...
  while (fESDChain->GetEntry(ievent++)) {
	...
	array2 = new IlcTrackPointArray(jpt);
	jpt=0;
	for(ipt=0; ipt<array->GetNPoints(); ipt++) {
	  array->GetPoint(point,ipt);
	  volId = point.GetVolumeID();
  ...
  if (!pointsTree->Write()) {
    IlcWarning("Can't write the tree with track point arrays!");
    return;
  }
...
void IlcAlignmentTracks::ProcessESDCosmics(Bool_t onlyITS,
			     	   ...
			     	   Float_t minSinTheta,Float_t maxSinTheta)
{
  ...
  while (fESDChain->GetEntry(ievent++)) {
    ...
    IlcESDtrack * track1 = esd->GetTrack(good1);
    IlcESDtrack * track2 = esd->GetTrack(good2);
...
//______________________________________________________________________________
void IlcAlignmentTracks::ProcessESD(TSelector *selector)
{
  IlcWarning(Form("ESD processing based on selector is not yet implemented (%p) !",selector));
}

//______________________________________________________________________________
void IlcAlignmentTracks::BuildIndex()
{
  ...
  //  IlcTrackPointArray* array = new IlcTrackPointArray;
  IlcTrackPointArray* array = 0;
  fPointsTree = (TTree*) fPointsFile->Get("spTree");
  if (!fPointsTree) {
    IlcWarning("No pointsTree found!");
    return;
  }
  ...
  for (Int_t iArray = 0; iArray < nArrays; iArray++)
	...
	if (!IlcGeomManager::SymName(volId)) {
	  IlcError(Form("The volume id %d has no default volume name !",
			volId));
	  continue;
	}
	Int_t modId;
	Int_t layerId = IlcGeomManager::VolUIDToLayer(volId,modId)
	              - IlcGeomManager::kFirstLayer;
...
//______________________________________________________________________________
void IlcAlignmentTracks::InitIndex()
{
  // Initialize the index arrays
  Int_t nLayers = IlcGeomManager::kLastLayer - IlcGeomManager::kFirstLayer;
  fLastIndex = new Int_t*[nLayers];
  fArrayIndex = new TArrayI**[nLayers];
  for (Int_t iLayer = 0; iLayer < (IlcGeomManager::kLastLayer - IlcGeomManager::kFirstLayer); iLayer++) {
    fLastIndex[iLayer] = new Int_t[IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer)];
    fArrayIndex[iLayer] = new TArrayI*[IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer)];
    for (Int_t iModule = 0; iModule < IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer); iModule++) {
...
//______________________________________________________________________________
void IlcAlignmentTracks::ResetIndex()
{
...
//______________________________________________________________________________
void IlcAlignmentTracks::DeleteIndex()
{
...
//______________________________________________________________________________
Bool_t IlcAlignmentTracks::ReadAlignObjs(const char *alignObjFileName, const char* arrayName)
{
  ...
  if (!fRealign || !fRealign->IsOpen()) {
    IlcError(Form("Could not open Align Obj File file %s !",alignObjFileName));
    return kFALSE;
  }  
  ...
  for(Int_t ivol=0;ivol<size;ivol++){
    IlcAlignObjParams *a=(IlcAlignObjParams*)clnarray->At(ivol);
    volid=a->GetVolUID();
    Int_t iModule;
    IlcGeomManager::ELayerID iLayer = IlcGeomManager::VolUIDToLayer(volid,iModule);
    if(iLayer<IlcGeomManager::kFirstLayer||iLayer>IlcGeomManager::kSSD2)continue;
    printf("Updating volume: %d ,layer: %d module: %d \n",volid,iLayer,iModule);
    *fIlcgnObjs[iLayer-IlcGeomManager::kFirstLayer][iModule] *= *a;
  }
...
//______________________________________________________________________________
void IlcAlignmentTracks::InitAlignObjs()
{
  // Initialize the alignment objects array
  Int_t nLayers = IlcGeomManager::kLastLayer - IlcGeomManager::kFirstLayer;
  fIlcgnObjs = new IlcAlignObj**[nLayers];
  for (Int_t iLayer = 0; iLayer < (IlcGeomManager::kLastLayer - IlcGeomManager::kFirstLayer); iLayer++) {
    fIlcgnObjs[iLayer] = new IlcAlignObj*[IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer)];
    for (Int_t iModule = 0; iModule < IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer); iModule++) {
      UShort_t volid = IlcGeomManager::LayerToVolUID(iLayer+ IlcGeomManager::kFirstLayer,iModule);
      fIlcgnObjs[iLayer][iModule] = new IlcAlignObjParams(IlcGeomManager::SymName(volid),volid,0,0,0,0,0,0,kTRUE);
    }
  }
}

//______________________________________________________________________________
void IlcAlignmentTracks::ResetAlignObjs()
{
  // Reset the alignment objects array
  for (Int_t iLayer = 0; iLayer < (IlcGeomManager::kLastLayer - IlcGeomManager::kFirstLayer); iLayer++) {
    for (Int_t iModule = 0; iModule < IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer); iModule++)
      fIlcgnObjs[iLayer][iModule]->SetPars(0,0,0,0,0,0);
  }
}

//______________________________________________________________________________
void IlcAlignmentTracks::DeleteAlignObjs()
{
  // Delete the alignment objects array
  for (Int_t iLayer = 0; iLayer < (IlcGeomManager::kLastLayer - IlcGeomManager::kFirstLayer); iLayer++) {
    for (Int_t iModule = 0; iModule < IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer); iModule++)
      if (fIlcgnObjs[iLayer][iModule])
	delete fIlcgnObjs[iLayer][iModule];
    delete [] fIlcgnObjs[iLayer];
  }
  delete [] fIlcgnObjs;
  fIlcgnObjs = 0;
}

Bool_t IlcAlignmentTracks::AlignDetector(IlcGeomManager::ELayerID firstLayer,
					 IlcGeomManager::ELayerID lastLayer,
					 IlcGeomManager::ELayerID layerRangeMin,
					 IlcGeomManager::ELayerID layerRangeMax,
					 Int_t iterations)
{
  ...
  for (Int_t iLayer = firstLayer; iLayer <= lastLayer; iLayer++)
    nModules += IlcGeomManager::LayerSize(iLayer);
  TArrayI volIds(nModules);

  Int_t modnum = 0;
  for (Int_t iLayer = firstLayer; iLayer <= lastLayer; iLayer++) {
    for (Int_t iModule = 0; iModule < IlcGeomManager::LayerSize(iLayer); iModule++) {
      UShort_t volId = IlcGeomManager::LayerToVolUID(iLayer,iModule);
...
//______________________________________________________________________________
Bool_t IlcAlignmentTracks::AlignLayer(IlcGeomManager::ELayerID layer,
				      IlcGeomManager::ELayerID layerRangeMin,
				      IlcGeomManager::ELayerID layerRangeMax,
				      Int_t iterations)
{
  ...
  // Tracks are fitted only within
  // the range defined by the user.
  Int_t nModules = IlcGeomManager::LayerSize(layer);
  TArrayI volIds(nModules);
  for (Int_t iModule = 0; iModule < nModules; iModule++) {
    UShort_t volId = IlcGeomManager::LayerToVolUID(layer,iModule);
...
//______________________________________________________________________________
Bool_t IlcAlignmentTracks::AlignVolume(UShort_t volId, UShort_t volIdFit,
				     Int_t iterations)
{
...
//______________________________________________________________________________
Bool_t IlcAlignmentTracks::AlignVolumes(const TArrayI *volids, const TArrayI *volidsfit,
				     IlcGeomManager::ELayerID layerRangeMin,
				     IlcGeomManager::ELayerID layerRangeMax,
				     Int_t iterations)
{
  ...
  if (nVolIds == 0) {
    IlcError("Volume IDs array is empty!");
    return kFALSE;
  }
  ...
  while (iterations > 0) {
    ...
    IlcTrackResiduals *minimizer = CreateMinimizer();
    minimizer->SetNTracks(nArrays);
    minimizer->InitAlignObj();
    IlcTrackFitter *fitter = CreateFitter();
    for (Int_t iArray = 0; iArray < nArrays; iArray++) {
      ...
      IlcTrackPointArray *pVolId,*pTrack;
    ...
    if (fDoUpdate) for (Int_t iVolId = 0; iVolId < nVolIds; iVolId++) {
      ...
      Int_t iModule;
      IlcGeomManager::ELayerID iLayer = IlcGeomManager::VolUIDToLayer(volid,iModule);
      IlcAlignObj *alignObj = fIlcgnObjs[iLayer-IlcGeomManager::kFirstLayer][iModule];      
...
//______________________________________________________________________________
Int_t IlcAlignmentTracks::LoadPoints(const TArrayI *volids, IlcTrackPointArray** &points,Int_t &pointsdim)
{
  ...
  if (!fPointsTree) {
    IlcError("Tree with the space point arrays not initialized!");
    points = 0;
    return 0;
  }
  ...
  for (Int_t iVolId = 0; iVolId < nVolIds; iVolId++) {
    ...
    if (fLastIndex[iLayer-IlcGeomManager::kFirstLayer][iModule] == 0) {
      IlcWarning(Form("There are no space-points belonging to the volume which is to be aligned (Volume ID =%d)!",volid));
      continue;
    }
    nArrays += fLastIndex[iLayer-IlcGeomManager::kFirstLayer][iModule];
  }

  if (nArrays == 0) {
    IlcError("There are no space-points belonging to all of the volumes which are to be aligned!");
    points = 0x0;
    return 0;
  }
  ...
  pointsdim=nArrays;
  points = new IlcTrackPointArray*[nArrays];
  ...
  for (Int_t iVolId = 0; iVolId < nVolIds; iVolId++) {
    ...
    Int_t nArraysId = fLastIndex[iLayer-IlcGeomManager::kFirstLayer][iModule];
    TArrayI *index = fArrayIndex[iLayer-IlcGeomManager::kFirstLayer][iModule];
    IlcTrackPoint p;
	...
	continue;
      ...
      if (!array) {
	IlcWarning("Wrong space point array index!");
	continue;
      }
      ...
      points[iArray] = new IlcTrackPointArray(nPoints);
      for (Int_t iPoint = 0; iPoint < nPoints; iPoint++) {
	array->GetPoint(p,iPoint);
	Int_t modnum;
	IlcGeomManager::ELayerID layer = IlcGeomManager::VolUIDToLayer(p.GetVolumeID(),modnum);
	// check if the layer id is valid
	if ((layer < IlcGeomManager::kFirstLayer) ||
	    (layer >= IlcGeomManager::kLastLayer)) {
	  IlcError(Form("Layer index is invalid: %d (%d -> %d) !",
			layer,IlcGeomManager::kFirstLayer,IlcGeomManager::kLastLayer-1));
	  continue;
	}
	if ((modnum >= IlcGeomManager::LayerSize(layer)) ||
	    (modnum < 0)) {
	  IlcError(Form("Module number inside layer %d is invalid: %d (0 -> %d)",
			layer,modnum,IlcGeomManager::LayerSize(layer)));
	  continue;
	}
	...
	// alignment job!
	if (fMisalignObjs) {
	  IlcAlignObj *misalignObj = fMisalignObjs[layer-IlcGeomManager::kFirstLayer][modnum];
	  if (misalignObj)
	...
	IlcAlignObj *alignObj = fIlcgnObjs[layer-IlcGeomManager::kFirstLayer][modnum];
...
//______________________________________________________________________________
void IlcAlignmentTracks::UnloadPoints(Int_t n, IlcTrackPointArray **points)
{
...
//______________________________________________________________________________
IlcTrackFitter *IlcAlignmentTracks::CreateFitter()
{
  // Check if the user has already supplied
  // a track fitter object.
  // If not, create a default one.
  if (!fTrackFitter)
    fTrackFitter = new IlcTrackFitterRieman;

  return fTrackFitter;
}

//______________________________________________________________________________
IlcTrackResiduals *IlcAlignmentTracks::CreateMinimizer()
{
  // Check if the user has already supplied
  // a track residuals minimizer object.
  // If not, create a default one.
  if (!fMinimizer)
    fMinimizer = new IlcTrackResidualsChi2;

  return fMinimizer;
}

//______________________________________________________________________________
Bool_t IlcAlignmentTracks::Misalign(const char *misalignObjFileName, const char* arrayName)
{
  // The method reads from a file a set of IlcAlignObj which are
  ...
  Int_t nLayers = IlcGeomManager::kLastLayer - IlcGeomManager::kFirstLayer;
  fMisalignObjs = new IlcAlignObj**[nLayers];
  for (Int_t iLayer = 0; iLayer < (IlcGeomManager::kLastLayer - IlcGeomManager::kFirstLayer); iLayer++) {
    fMisalignObjs[iLayer] = new IlcAlignObj*[IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer)];
    for (Int_t iModule = 0; iModule < IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer); iModule++)
  ...
  if (!inFile || !inFile->IsOpen()) {
    IlcError(Form("Could not open misalignment file %s !",misalignObjFileName));
    return kFALSE;
  }

  TClonesArray* array = ((TClonesArray*) inFile->Get(arrayName));
  if (!array) {
    IlcError(Form("Could not find misalignment array %s in the file %s !",arrayName,misalignObjFileName));
    inFile->Close();
    return kFALSE;
  }
  inFile->Close();
  ...
  IlcGeomManager::ELayerID layerId; // volume layer
  Int_t modId; // volume ID inside the layer
  for(Int_t i=0; i<nObjs; i++)
    {
      IlcAlignObj* alObj = (IlcAlignObj*)array->UncheckedAt(i);
      alObj->GetVolUID(layerId,modId);
      if(layerId<IlcGeomManager::kFirstLayer) {
	IlcWarning(Form("Alignment object is ignored: %s",alObj->GetSymName()));
	continue;
      }
      fMisalignObjs[layerId-IlcGeomManager::kFirstLayer][modId] = alObj;
    }
  return kTRUE;
}
...
void IlcAlignmentTracks::WriteRealignObjArray(TString outfilename,IlcGeomManager::ELayerID layerRangeMin,IlcGeomManager::ELayerID layerRangeMax){
  
  Int_t last=0;
  TClonesArray *clonesArray=new TClonesArray("IlcAlignObjParams",2200);
  TClonesArray &alo=*clonesArray;
  for (Int_t iLayer = layerRangeMin-IlcGeomManager::kFirstLayer; iLayer <= (layerRangeMax - IlcGeomManager::kFirstLayer);iLayer++) {
  
    for (Int_t iModule = 0; iModule < IlcGeomManager::LayerSize(iLayer + IlcGeomManager::kFirstLayer); iModule++) {
     
      IlcAlignObj *alignObj = fIlcgnObjs[iLayer][iModule]; 
      new(alo[last])IlcAlignObjParams(*alignObj);
      last++;
    }
  }

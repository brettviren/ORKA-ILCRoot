...
ClassImp(IlcSymMatrix)


IlcSymMatrix* IlcSymMatrix::fgBuffer = 0; 
Int_t         IlcSymMatrix::fgCopyCnt = 0; 
//___________________________________________________________
IlcSymMatrix::IlcSymMatrix() 
...
//___________________________________________________________
IlcSymMatrix::IlcSymMatrix(Int_t size)
  : IlcMatrixSq(),fElems(0),fElemsAdd(0)
{
...
//___________________________________________________________
IlcSymMatrix::IlcSymMatrix(const IlcSymMatrix &src) 
  : IlcMatrixSq(src),fElems(0),fElemsAdd(0)
{
  // copy constructor
...
//___________________________________________________________
IlcSymMatrix&  IlcSymMatrix::operator=(const IlcSymMatrix& src)
{
  // assignment operator
  if (this != &src) {
...
IlcSymMatrix& IlcSymMatrix::operator+=(const IlcSymMatrix& src)
{
  ...
  if (GetSizeUsed() != src.GetSizeUsed()) {
    IlcError("Matrix sizes are different");
    return *this;
  }
...
//___________________________________________________________
void IlcSymMatrix::Clear(Option_t*)
{
  // clear dynamic part
...
//___________________________________________________________
Float_t IlcSymMatrix::GetDensity() const
{
...
//___________________________________________________________
void IlcSymMatrix::Print(Option_t* option) const
{
  // print itself
...
//___________________________________________________________
void IlcSymMatrix::MultiplyByVec(const Double_t *vecIn,Double_t *vecOut) const
{
...
//___________________________________________________________
IlcSymMatrix* IlcSymMatrix::DecomposeChol() 
{
  ...
  if (!fgBuffer || fgBuffer->GetSizeUsed()!=GetSizeUsed()) {
    delete fgBuffer; 
    fgBuffer = new IlcSymMatrix(*this);
  }
  else (*fgBuffer) = *this;
  //
  IlcSymMatrix& mchol = *fgBuffer;
  //
  for (int i=0;i<GetSizeUsed();i++) {
	...
	if (sum <= 0.0) { // not positive-definite
	  IlcInfo(Form("The matrix is not positive definite [%e]\n"
...
//___________________________________________________________
Bool_t IlcSymMatrix::InvertChol() 
{
  // Invert matrix using Choleski decomposition
  //
  IlcSymMatrix* mchol = DecomposeChol();
  if (!mchol) {
    IlcInfo("Failed to invert the matrix");
    return kFALSE;
  }
  //
  InvertChol(mchol);
...
//___________________________________________________________
Bool_t IlcSymMatrix::SolveChol(Double_t *b, Bool_t invert) 
{
  // Adopted from Numerical Recipes in C, ch.2-9, http://www.nr.com
  ...
  Double_t sum;
  //
  IlcSymMatrix *pmchol = DecomposeChol();
  if (!pmchol) {
    IlcInfo("SolveChol failed");
    //    Print("l");
    return kFALSE;
  }
  IlcSymMatrix& mchol = *pmchol;
...
//___________________________________________________________
Bool_t IlcSymMatrix::SolveChol(TVectorD &b, Bool_t invert) 
{
...
//___________________________________________________________
Bool_t IlcSymMatrix::SolveChol(Double_t *brhs, Double_t *bsol,Bool_t invert) 
{
...
//___________________________________________________________
Bool_t IlcSymMatrix::SolveChol(const TVectorD &brhs, TVectorD &bsol,Bool_t invert) 
{
  bsol = brhs;
  return SolveChol(bsol,invert);
}
...
//___________________________________________________________
void IlcSymMatrix::AddRows(int nrows)
{
  // add empty rows
  if (nrows<1) return;
...
//___________________________________________________________
/*
void IlcSymMatrix::AddToRow(Int_t r, Double_t *valc,Int_t *indc,Int_t n)
{
  //   for (int i=n;i--;) {
...
//___________________________________________________________
Double_t* IlcSymMatrix::GetRow(Int_t r)
{
  // get pointer on the row
  if (r>=GetSize()) {
    int nn = GetSize();
    AddRows(r-GetSize()+1); 
    IlcDebug(2,Form("create %d of %d\n",r, nn));
...
//___________________________________________________________
int IlcSymMatrix::SolveSpmInv(double *vecB, Bool_t stabilize)
{

void cloneAODTreeAndAddObject(const char *newFileName = "IlcAOD_new.root", const char *orgFileName = "IlcAOD.root") {
  ...
  // a look at $ILC_ROOT/STEER/ReadAOD.C and add one line
  ...
  IlcAODEvent *evNew = new IlcAODEvent();
  evNew->ReadFromTree(newAodTree);
    
  // add new information to the list (we use IlcESDtracks as an example)

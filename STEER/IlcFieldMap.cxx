...
/* $Id: IlcFieldMap.cxx 30421 2008-12-16 15:42:04Z shahoian $ */
...
#include "IlcLog.h"
#include "IlcFieldMap.h"

ClassImp(IlcFieldMap)

//_______________________________________________________________________
IlcFieldMap::IlcFieldMap():
  fXbeg(0),
  fYbeg(0),
  fZbeg(0),
  fXend(0),
...
//_______________________________________________________________________
IlcFieldMap::IlcFieldMap(const char *name, const char *title):
  TNamed(name,title),
  fXbeg(0),
  fYbeg(0),
  fZbeg(0),
...
//_______________________________________________________________________
IlcFieldMap::IlcFieldMap(const IlcFieldMap &map):
  TNamed(map),
  fXbeg(0),
  fYbeg(0),
  fZbeg(0),
...
void IlcFieldMap::ReadField()
{
  ...
  Float_t bx, by, bz;
  char *fname = 0;
  IlcInfo(Form("Reading Magnetic Field Map %s from file %s",
...
//_______________________________________________________________________
void IlcFieldMap::Field(float *x, float *b) const
{
  //
...
//_______________________________________________________________________
void IlcFieldMap::Field(double *x, double *b) const
{
  //
  // Use simple interpolation to obtain field at point x
  //
...
//_______________________________________________________________________
void IlcFieldMap::Copy(TObject & /* magf */) const
{
  //
...
//_______________________________________________________________________
IlcFieldMap & IlcFieldMap::operator =(const IlcFieldMap &magf)
{
  magf.Copy(*this);
  return *this;
}
...
//_______________________________________________________________________
void IlcFieldMap::Streamer(TBuffer &R__b)
{
   // Stream an object of class IlcFieldMap.
    TVector* save = 0;
    
    if (R__b.IsReading()) {
	IlcFieldMap::Class()->ReadBuffer(R__b, this);
    } else {
	if (!fWriteEnable) {
	    save = fB;
	    fB = 0;
	}
	IlcFieldMap::Class()->WriteBuffer(R__b, this);

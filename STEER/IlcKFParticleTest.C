...
//---------------------------------------------------------------------------------
// The example of usage of IlcKFParticle & IlcKFVertex classes for V0 analysis
// .
// @author  S.Gorbunov, I.Kisel
// @version 1.0
// @since   13.05.07
// 
// The IlcKFParticleTest macro contains a toy V0 finder for ESD tracks.
...
void StartV0()
{
  ...
  canvas = new TCanvas();
  histoMass[0] = new TH1D("massAll","IlcKFParticle test", 500,0,3);
  ...
  TFile* file = new TFile("IlcKFParticleTest.root", "READ" );
  if( !old_file && file ){
    file->cd();
    for( Int_t i=0; i<3; i++ ){
...
void EndV0()
{
  //* End of gIlc -> store all histos in temporary file, clean memory

  TDirectory *curr = gDirectory;
  TFile* file = new TFile("IlcKFParticleTest.root", "RECREATE" );
  old_file = 0;
  file->cd();
...
public:
  TESDTrackInfo(){}
  IlcKFParticle fParticle; //* assigned KFParticle
...
void RunV0(  IlcESDEvent *event )
{
  ...
  for (Int_t iTr=0; iTr<nESDTracks; iTr++)
    {   
      ...
      IlcESDtrack *pTrack = event->GetTrack(iTr);    
      if( !pTrack  ) continue;
      if (pTrack->GetKinkIndex(0)>0) continue;
      if ( !( pTrack->GetStatus()&IlcESDtrack::kITSrefit ) ) continue;
      Int_t indi[12];
	...
	if( part ){
      ...
      info.fParticle = IlcKFParticle( *pTrack, PDG );
      info.fOK = 1;   
    }
  ...
  IlcKFVertex primVtx;  
  {
    const IlcKFParticle * vSelected[1000]; //* Selected particles for vertex fit
  ...
  for( Int_t iTr = 0; iTr<nESDTracks; iTr++ ){ //* first daughter
    ...
    for( Int_t jTr = iTr+1; jTr<nESDTracks; jTr++ ){  //* second daughter
      ...
      IlcKFParticle V0( info.fParticle, jnfo.fParticle );     
      ...
      IlcKFVertex primVtxCopy = primVtx;
...
Int_t IlcKFParticleTest(Int_t n1=0,Int_t n2=1000,char *dire="/d/ilc10/sma/my_v4-05-Release/pp/"){
  //* Main macro
  ...
  for (Int_t ifi=n1; ifi<=n2; ifi++) {
    ...
    sprintf(filename,"%s%s/gilc.root",dire,nstring);
    sprintf(esdfile,"%s%s/IlcESDs.root",dire,nstring);
    ...
    if ( !rl ) { 
      ::Error("IlcKFParticleTest.C","Can not open session !");
      continue;
    }
    if (rl->LoadgIlc()) {
      ::Error("IlcKFParticleTest.C","LoadgIlc returned error !");
      delete rl;
      continue;
    }
    if (rl->LoadHeader()) {
      ::Error("IlcKFParticleTest.C","LoadHeader returned error !");
      delete rl;
      continue;
    }
    rl->LoadKinematics();
    IlcTracker::SetFieldMap(gIlc->Field(),1);
    IlcKFParticle::SetField( IlcTracker::GetBz() );
    ...
    if (!ef || !ef->IsOpen()) {cerr<<"Error open IlcESDs.root !\n"; continue ;}

    //create event object
    IlcESD *event = new IlcESDEvent;

...
revision of IlcDCSValue: Removed wrapper classes, reduced storage size per value by factor 2
...
ClassImp(IlcDCSValue)

IlcDCSValue::IlcDCSValue() :
  TObject(),
  fType(kInvalid),
  fBool(kFALSE),
  fChar(0),
...
IlcDCSValue::IlcDCSValue(Bool_t value, UInt_t timeStamp) : 
  TObject(),
  fType(kBool),
  fBool(value),
  fChar(0),
...
IlcDCSValue::IlcDCSValue(Char_t value, UInt_t timeStamp) :
  TObject(),
  fType(kChar),
  fBool(kFALSE),
  fChar(value),
...
IlcDCSValue::IlcDCSValue(Int_t value, UInt_t timeStamp) :
  TObject(),
  fType(kInt),
  fBool(kFALSE),
  fChar(0),
...
IlcDCSValue::IlcDCSValue(UInt_t value, UInt_t timeStamp) :
  TObject(),
  fType(kUInt),
  fBool(kFALSE),
  fChar(0),
...
IlcDCSValue::IlcDCSValue(Float_t value, UInt_t timeStamp) :
  TObject(),
  fType(kFloat),
  fBool(kFALSE),
  fChar(0),
...
IlcDCSValue::IlcDCSValue(const IlcDCSValue& c) :
  TObject(c),
  fType(c.fType),
  fBool(c.fBool),
  fChar(c.fChar),
...
IlcDCSValue &IlcDCSValue::operator=(const IlcDCSValue &c)
{
  // assigment operator

  if (this != &c) 
    ((IlcDCSValue &) c).Copy(*this);

  return *this;
}

void IlcDCSValue::Copy(TObject& c) const
{
  // copy function

  IlcDCSValue& target = (IlcDCSValue &) c;
...
Int_t IlcDCSValue::GetSize() const
{
...
const Char_t* IlcDCSValue::ToString() const
{
  TString str;
...
void IlcDCSValue::Print(Option_t* /*opt*/) const
{
  printf("%s\n", ToString());
}

Bool_t IlcDCSValue::GetBool() const
{
  // return bool value
  if (fType!=kBool) IlcError(Form("invalid request, object is not of type kBool (%d) but %d", kBool, fType));
  return fBool;
}

Char_t IlcDCSValue::GetChar() const
{ 
  // return char value
  if (fType!=kChar) IlcError(Form("invalid request, object is not of type kChar (%d) but %d", kChar, fType));
  return fChar;
}

Int_t IlcDCSValue::GetInt() const
{
  // return int value
  if (fType!=kInt) IlcError(Form("invalid request, object is not of type kInt (%d) but %d", kInt, fType));
  return fInt;
}

UInt_t IlcDCSValue::GetUInt() const
{
  // return uint value
  if (fType!=kUInt) IlcError(Form("invalid request, object is not of type kUInt (%d) but %d", kUInt, fType));
  return fUInt;
}

Float_t IlcDCSValue::GetFloat() const
{
  // return float value
  if (fType!=kFloat) IlcError(Form("invalid request, object is not of type kFloat (%d) but %d", kFloat, fType));
  return fFloat;
}

...
IlcDCSSensor::IlcDCSSensor(const IlcDCSSensor& source) :
   TNamed(source),
   fId(source.fId),
...
{ 
   ...
   if (source.fFit) fFit = (IlcSplineFit*)source.fFit->Clone();
}

IlcDCSSensor& IlcDCSSensor::operator=(const IlcDCSSensor& source){
//
// assignment operator
//
  if (&source == this) return *this;
  new (this) IlcDCSSensor(source);

  return *this;
}

//_____________________________________________________________________________
Double_t IlcDCSSensor::GetValue(UInt_t timeSec)
{
 //
 ...
 //         same absolute time, use IlcDCSSensor::GetValue(TTimeStamp time)
 //         or IlcDCSSensorArray::GetValue (UInt_t timeSec, Int_t sensor)
...
//_____________________________________________________________________________
Double_t IlcDCSSensor::GetValue(TTimeStamp time) 
{
 // Get DCS value for actual sensor
...
Double_t IlcDCSSensor::Eval(const TTimeStamp& time, Bool_t& inside) const
{
  // 
  // Return DCS value at given time
  //  The value is calculated from the IlcSplineFit, if a fit is not available 
...
Double_t IlcDCSSensor::EvalGraph(const TTimeStamp& time, Bool_t& inside) const
{
  // 
...
//_____________________________________________________________________________
Double_t IlcDCSSensor::EvalGraph(const Double_t& timeHour) const 
{
  //
...
//_____________________________________________________________________________
TGraph* IlcDCSSensor::MakeGraph(Int_t nPoints, Bool_t debug) const
{
  //
...
TClonesArray * IlcDCSSensor::ReadTree(TTree* tree) {
  //
  ...
  TClonesArray * array = new TClonesArray("IlcDCSSensor",nentries);
   printf ("nentries = %d\n",nentries);

  for (Int_t isensor=0; isensor<nentries; isensor++){
    IlcDCSSensor * sens = new ((*array)[isensor])IlcDCSSensor;

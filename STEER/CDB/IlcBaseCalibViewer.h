#ifndef ILCBASECILCBVIEWER_H
#define ILCBASECILCBVIEWER_H
...
class IlcBaseCalibViewer : public TObject {
 public:
   IlcBaseCalibViewer();
   IlcBaseCalibViewer(const IlcBaseCalibViewer &c);
   IlcBaseCalibViewer(TTree* tree);
   IlcBaseCalibViewer(const Char_t* fileName, const Char_t* treeName = "tree");
   IlcBaseCalibViewer &operator = (const IlcBaseCalibViewer & param);
   virtual ~IlcBaseCalibViewer();
 ...
 protected:
   TTree* fTree;     // tree containing visualization data (e.g. written by IlcTPCCalPad::MakeTree(...)
   TFile* fFile;     // file that contains a calPads tree (e.g. written by IlcTPCCalPad::MakeTree(...)
   ...
   ClassDef(IlcBaseCalibViewer,1)    //  Base calibration viewer class
};

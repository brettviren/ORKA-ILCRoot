...
//-------------------------------------------------------------------------
//                Implementation of the IlcSplineFit class
...
ClassImp(IlcSplineFit)

TLinearFitter* IlcSplineFit::fitterStatic()
{
  static TLinearFitter* fit = new TLinearFitter(4,"pol3","");
  return fit;
}

IlcSplineFit::IlcSplineFit() :
  fBDump(kFALSE),
  fGraph (0),
  fNmin (0),
  fMinPoints(0),
...
IlcSplineFit::IlcSplineFit(const IlcSplineFit& source) :
  TObject(source),
...
IlcSplineFit& IlcSplineFit::operator=(const IlcSplineFit& source){
//
//  assignment operator
//
...
Double_t   IlcSplineFit::Eval(Double_t x, Int_t deriv) const{
  //
  // evaluate value at x
...
TGraph * IlcSplineFit::GenerGraph(Int_t npoints, Double_t fraction, Double_t s1, Double_t s2, Double_t s3, Int_t der){
  //
...
TGraph * IlcSplineFit::GenerNoise(TGraph * graph0, Double_t sigma0){
  //
  // add noise to graph
  //
...
TGraph * IlcSplineFit::MakeGraph(Double_t xmin, Double_t xmax, Int_t npoints, Int_t deriv) const {
  //
  ...
  if (npoints<=0) {
    if (deriv<=0) 
      ...
      IlcWarning("npoints == 0 et deriv == 2: unhandled condition");
      return 0;
    }
  }
...
TGraph * IlcSplineFit::MakeDiff(TGraph * graph0) const {
  //
...
TH1F * IlcSplineFit::MakeDiffHisto(TGraph * graph0) const {
  //
...
void IlcSplineFit::InitKnots(TGraph * graph, Int_t min, Int_t iter, Double_t maxDelta){
  //
  ...
  for (Int_t i=0; i<fN0; i++){
    ...
    if (iKnot>=fN) {
      printf("IlcSplineFit::InitKnots: Knot number > Max knot number\n");
      break;
    }
...
Int_t IlcSplineFit::OptimizeKnots(Int_t nIter){
  //
  //
  //
...
Bool_t   IlcSplineFit::RefitKnot(Int_t iKnot){
  //
  //
  //
  ...
  Double_t startX = fGraph->GetX()[fIndex[iKnot]]; 
  IlcSplineFit::fitterStatic()->ClearPoints();
...
//    IlcSplineFit::fitterStatic.AddPoint(&dxl,y,fSigma+TMath::Abs(y)*kEpsilon);
  }
  IlcSplineFit::fitterStatic()->AssignData(nPoints,1,xPoint,yPoint,ePoint);
  IlcSplineFit::fitterStatic()->Eval();
...
//  delete temporary arrays
  ...
  TMatrixD   * covar = (TMatrixD*)fCovars->At(iKnot);
  TVectorD   * param = (TVectorD*)fParams->At(iKnot);
  IlcSplineFit::fitterStatic()->GetParameters(*param);
  IlcSplineFit::fitterStatic()->GetCovarianceMatrix(*covar);
  return 0;
}


Float_t IlcSplineFit::CheckKnot(Int_t iKnot){
  //
  //
  //
...
void IlcSplineFit::SplineFit(Int_t nder){
  //
...
void IlcSplineFit::MakeKnots0(TGraph * graph, Double_t maxdelta, Int_t minpoints){
  //
...
void IlcSplineFit::MakeSmooth(TGraph * graph, Float_t ratio, Option_t * type){
  //
...
void IlcSplineFit::Update(TSpline3 *spline, Int_t nknots){
  //
  //
  //
...
void IlcSplineFit::Test(Int_t npoints, Int_t ntracks, Float_t snoise){  
  //
  // test function
  //
  ...
  for (Int_t i=0; i<ntracks; i++){
    graph0 = IlcSplineFit::GenerGraph(npoints,0.05,0,0,1,0);  
    graph1 = IlcSplineFit::GenerNoise(graph0,snoise);  

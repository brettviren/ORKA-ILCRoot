...
//__________________________________________________________________________________________
IlcCheb3D::IlcCheb3D(const IlcCheb3D& src) : 
  TNamed(src),
  fDimOut(src.fDimOut), 
  fPrec(src.fPrec), 
  fChebCalc(1), 
  ...
  fUsrMacro(0)
{
  ...
  for (int i=0;i<fDimOut;i++) {
    IlcCheb3DCalc* cbc = src.GetChebCalc(i);
    if (cbc) fChebCalc.AddAtAndExpand(new IlcCheb3DCalc(*cbc),i);
  }
}

//__________________________________________________________________________________________
IlcCheb3D::IlcCheb3D(const char* inpFile) : 
  fDimOut(0), 
  fPrec(0),  
  fChebCalc(1),
  fMaxCoefs(0),  
...
//__________________________________________________________________________________________
IlcCheb3D::IlcCheb3D(FILE* stream) : 
  fDimOut(0), 
  fPrec(0), 
  fChebCalc(1), 
  fMaxCoefs(0),
...
IlcCheb3D::IlcCheb3D(const char* funName, int DimOut, const Float_t  *bmin, const Float_t  *bmax, Int_t *npoints, Float_t prec) : 
  ...
  fUsrMacro(0)
{
  ...
  //
  if (DimOut<1) {Error("IlcCheb3D","Requested output dimension is %d\nStop\n",fDimOut); exit(1);}
  for (int i=3;i--;) {
    fBMin[i] = fBMax[i] = fBScale[i] = fBOffset[i] = 0;
    fNPoints[i] = 0;
    fGridOffs[i] = 0.;
...
//__________________________________________________________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
IlcCheb3D::IlcCheb3D(void (*ptr)(float*,float*), int DimOut, Float_t  *bmin,Float_t  *bmax, Int_t *npoints, Float_t prec) : 
  fDimOut(0), 
  fPrec(TMath::Max(1.E-12f,prec)), 
  fChebCalc(1), 
  fMaxCoefs(0), 
...
//__________________________________________________________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
IlcCheb3D::IlcCheb3D(void (*ptr)(float*,float*), int DimOut, Float_t  *bmin,Float_t  *bmax, Int_t *npX,Int_t *npY,Int_t *npZ, Float_t prec) : 
  fDimOut(0), 
  fPrec(TMath::Max(1.E-12f,prec)), 
  fChebCalc(1), 
  fMaxCoefs(0), 
...
//__________________________________________________________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
IlcCheb3D::IlcCheb3D(void (*ptr)(float*,float*), int DimOut, Float_t  *bmin,Float_t  *bmax, Float_t prec, Bool_t run) : 
  fDimOut(0), 
  fPrec(TMath::Max(1.E-12f,prec)), 
  fChebCalc(1), 
  fMaxCoefs(0), 
  ...
  fUsrMacro(0)
{
  ...
  // prec    : max allowed absolute difference between the user function and computed parameterization on the requested grid
  //
  if (DimOut!=3) {Error("IlcCheb3D","This constructor works only for 3D fits, %dD fit was requested\n",fDimOut); exit(1);}
  if (DimOut<1) {Error("IlcCheb3D","Requested output dimension is %d\nStop\n",fDimOut); exit(1);}
  for (int i=3;i--;) {
    fBMin[i] = fBMax[i] = fBScale[i] = fBOffset[i] = 0;
    fNPoints[i] = 0;
...
//__________________________________________________________________________________________
IlcCheb3D& IlcCheb3D::operator=(const IlcCheb3D& rhs)
{
  // assignment operator
  //
  if (this != &rhs) {
    ...
    for (int i=0;i<fDimOut;i++) {
      IlcCheb3DCalc* cbc = rhs.GetChebCalc(i);
      if (cbc) fChebCalc.AddAtAndExpand(new IlcCheb3DCalc(*cbc),i);
    }    
  }
  return *this;
...
//__________________________________________________________________________________________
void IlcCheb3D::Clear(const Option_t*)
{
...
//__________________________________________________________________________________________
void IlcCheb3D::Print(const Option_t* opt) const
{
  // print info
  //
...
//__________________________________________________________________________________________
void IlcCheb3D::PrepareBoundaries(const Float_t  *bmin, const Float_t  *bmax)
{
...
void (*gUsrFunIlcCheb3D) (float* ,float* );

void IlcCheb3D::EvalUsrFunction() 
{
  // call user supplied function
  if   (gUsrFunIlcCheb3D) gUsrFunIlcCheb3D(fArgsTmp,fResTmp);
  else fUsrMacro->Execute(); 
}

void IlcCheb3D::SetUsrFunction(const char* name)
{
...
//__________________________________________________________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
void IlcCheb3D::SetUsrFunction(void (*ptr)(float*,float*))
{
  ...
  fUsrMacro = 0;
  fUsrFunName = "";
  gUsrFunIlcCheb3D = ptr;
}
#endif

//__________________________________________________________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
void IlcCheb3D::EvalUsrFunction(const Float_t  *x, Float_t  *res) 
{
...
//__________________________________________________________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
Int_t IlcCheb3D::CalcChebCoefs(const Float_t  *funval,int np, Float_t  *outCoefs, Float_t  prec)
{
...
//__________________________________________________________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
void IlcCheb3D::DefineGrid(Int_t* npoints)
{
...
//__________________________________________________________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
Int_t IlcCheb3D::ChebFit(int dmOut)
{
  ...
  IlcCheb3DCalc* cheb =  GetChebCalc(dmOut);
  //
...
#ifdef _INC_CREATION_ILCCHEB3D_
void IlcCheb3D::SaveData(const char* outfile,Bool_t append) const
{
...
//_______________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
void IlcCheb3D::SaveData(FILE* stream) const
{
...
//__________________________________________________________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
void IlcCheb3D::InvertSign()
{
  // invert the sign of all parameterizations
  for (int i=fDimOut;i--;) {
    IlcCheb3DCalc* par =  GetChebCalc(i);
...
//_______________________________________________
void IlcCheb3D::LoadData(const char* inpFile)
{
...
//_______________________________________________
void IlcCheb3D::LoadData(FILE* stream)
{
  ...
  //
  IlcCheb3DCalc::ReadLine(buffs,stream); // N output dimensions
  fDimOut = buffs.Atoi(); 
  if (fDimOut<1) {Error("LoadData","Expected: '<number_of_output_dimensions>', found \"%s\"\nStop\n",buffs.Data());exit(1);}
  //
  SetDimOut(fDimOut);
  //
  IlcCheb3DCalc::ReadLine(buffs,stream); // Interpolation abs. precision
  fPrec = buffs.Atof();
...
//_______________________________________________
void IlcCheb3D::SetDimOut(const int d)
{
  ...
  fChebCalc.Delete();
  for (int i=0;i<d;i++) fChebCalc.AddAtAndExpand(new IlcCheb3DCalc(),i);
}

//_______________________________________________
void IlcCheb3D::ShiftBound(int id,float dif)
{
...
//_______________________________________________
#ifdef _INC_CREATION_ILCCHEB3D_
TH1* IlcCheb3D::TestRMS(int idim,int npoints,TH1* histo)
{
...
void IlcCheb3D::EstimateNPoints(float Prec, int gridBC[3][3],Int_t npd1,Int_t npd2,Int_t npd3)
{
...
/*
void IlcCheb3D::EstimateNPoints(float Prec, int gridBC[3][3])
{
  // Estimate number of points to generate a training data
  //
...
int* IlcCheb3D::GetNCNeeded(float xyz[3],int DimVar, float mn,float mx, float prec)
{
  ...
  for (curNP=3; curNP<kMaxPoint; curNP+=3) { 
    ...
    for (int ib=0;ib<3;ib++) {
      curNC[ib] = IlcCheb3D::CalcChebCoefs(&gridVal[ib*kMaxPoint], curNP, &coefs[ib*kMaxPoint],prec);
...
int* IlcCheb3D::GetNCNeeded(float xyz[3],int DimVar, float mn,float mx, float prec, Int_t npCheck)
{
  // estimate needed number of chebyshev coefs for given function description in DimVar dimension
  // The values for two other dimensions must be set beforehand
  //
  ...
  //
  for (int ib=0;ib<3;ib++) retNC[ib] = IlcCheb3D::CalcChebCoefs(&gridVal[ib*npCheck], npCheck, &coefs[ib*npCheck],prec);
  return retNC;
  //
}

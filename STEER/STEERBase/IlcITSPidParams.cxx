...
ClassImp(IlcITSPidParams)

//______________________________________________________________________
IlcITSPidParams::IlcITSPidParams(Bool_t isMC):
TNamed("default",""),
...
//______________________________________________________________________
IlcITSPidParams::IlcITSPidParams(Char_t * name, Bool_t isMC):
  TNamed(name,""),
  fSDDElecLandauWidth(0),
  fSDDElecGaussWidth(0),
  fSSDElecLandauWidth(0),
...
//______________________________________________________________________
IlcITSPidParams::~IlcITSPidParams(){
  //
...
//______________________________________________________________________
Double_t IlcITSPidParams::BetheBloch(Double_t mom, Double_t mass, const Double_t *p) const{
 //
...
//______________________________________________________________________
Double_t IlcITSPidParams::ExtrapolateWidth(Double_t mom, Double_t x1, Double_t y1,Double_t x2, Double_t y2) const{
  //
...
Double_t IlcITSPidParams::GetLandauGausNormPdgCode(Double_t dedx, Int_t pdgCode, Double_t mom, Int_t lay) const {
  // Computes Landau Gauss convolution for given particle specie and given momentum in a given ITS layer
  if(TMath::Abs(pdgCode)==11) return GetLandauGausNorm(dedx,IlcPID::kElectron,mom,lay);
  else if(TMath::Abs(pdgCode)==211) return GetLandauGausNorm(dedx,IlcPID::kPion,mom,lay);
  else if(TMath::Abs(pdgCode)==321) return GetLandauGausNorm(dedx,IlcPID::kKaon,mom,lay);
  else if(TMath::Abs(pdgCode)==2212) return GetLandauGausNorm(dedx,IlcPID::kProton,mom,lay);
  else return 0.;
}
//_______________________________________________________________________
Double_t IlcITSPidParams::GetLandauGausNorm(Double_t dedx, Int_t partType, Double_t mom, Int_t lay) const{
  // Computes Landau Gauss convolution for given particle specie and given momentum in a given ITS layer
  
  Double_t par[4];
  Bool_t isSet=kFALSE;
  if(partType==IlcPID::kElectron){
    if(lay==3 || lay==4){
  ...
  }else if(partType==IlcPID::kProton){
    if(lay==3 || lay==4){

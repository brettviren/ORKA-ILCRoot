...
ClassImp(IlcTRDPIDReference)

//____________________________________________________________
IlcTRDPIDReference::IlcTRDPIDReference():
TNamed(),
fRefContainer(NULL),
fMomentumBins()
{
...
//____________________________________________________________
IlcTRDPIDReference::IlcTRDPIDReference(const Char_t *name):
...
//____________________________________________________________
IlcTRDPIDReference::IlcTRDPIDReference(const IlcTRDPIDReference &ref):
		  TNamed(ref),
		  fRefContainer(NULL),
		  fMomentumBins(ref.fMomentumBins)
{
    ...
    // Copy constructor
    //
    fRefContainer = new TObjArray(fMomentumBins.GetSize() * IlcPID::kSPECIES);
...
//____________________________________________________________
IlcTRDPIDReference &IlcTRDPIDReference::operator=(const IlcTRDPIDReference &ref){
	//
	// Assginment operator
...
//____________________________________________________________
void IlcTRDPIDReference::SetNumberOfMomentumBins(Int_t nBins, Float_t *momenta){
	//
	...
	fRefContainer->Expand(nBins * IlcPID::kSPECIES);
	fMomentumBins.Set(nBins,momenta);
}

//____________________________________________________________
void IlcTRDPIDReference::AddReference(TObject *ref, IlcPID::EParticleType spec, Int_t pbin){
	//
	...
	//
	if(!fRefContainer){
		IlcError("Reference Container not initialized");
		return;
	}
	if(pbin > fMomentumBins.GetSize()){
		IlcError("Pbin overflow");
		return;
	}
	IlcDebug(1, Form("Adding object with address %p to position %d", ref, spec * fMomentumBins.GetSize() + pbin));
	fRefContainer->AddAt(ref->Clone(), spec * fMomentumBins.GetSize() + pbin);
}

//____________________________________________________________
TObject *IlcTRDPIDReference::GetUpperReference(IlcPID::EParticleType spec, Float_t p, Float_t &pUpper) const{
	//
	...
	for(Int_t ip = 0; ip < fMomentumBins.GetSize(); ip++){
		IlcDebug(10, Form("Bin %d, p = %.1f", ip, fMomentumBins[ip]));
		if(p < fMomentumBins[ip]){
			IlcDebug(10, "Bin found");
			bin = ip;
			break;
		}
	}
	IlcDebug(2, Form("p = %.1f, bin = %d\n", p, bin));
	if(bin >= 0) {
...
//____________________________________________________________
TObject *IlcTRDPIDReference::GetLowerReference(IlcPID::EParticleType spec, Float_t p, Float_t &pLower) const{
	//
...
//____________________________________________________________
void IlcTRDPIDReference::Print(const Option_t*) const{
	//

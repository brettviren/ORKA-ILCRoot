#ifndef ILCCHEB3DCALC_H
#define ILCCHEB3DCALC_H
...
//#define _INC_CREATION_ILCCHEB3D_
...
class IlcCheb3DCalc: public TNamed
{
 public:
  IlcCheb3DCalc();
  IlcCheb3DCalc(const IlcCheb3DCalc& src);
  IlcCheb3DCalc(FILE* stream);
  ~IlcCheb3DCalc()                                                           {Clear();}
  //
  IlcCheb3DCalc& operator=(const IlcCheb3DCalc& rhs);
 ...
 protected:
  ...
  //
  ClassDef(IlcCheb3DCalc,3)      // Class for interpolation of 3D->1 function by Chebyshev parametrization 
};

//__________________________________________________________________________________________
inline Float_t IlcCheb3DCalc::ChebEval1D(Float_t  x, const Float_t * array, int ncf ) 
{
...
//__________________________________________________________________________________________
inline Float_t IlcCheb3DCalc::Eval(const Float_t  *par) const 
{
...
//__________________________________________________________________________________________
inline Double_t IlcCheb3DCalc::Eval(const Double_t  *par) const 
{
  // evaluate Chebyshev parameterization for 3D function.
  // VERY IMPORTANT: par must contain the function arguments ALREADY MAPPED to [-1:1] interval
  if (!fNRows) return 0.;

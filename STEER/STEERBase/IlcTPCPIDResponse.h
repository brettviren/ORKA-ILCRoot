#ifndef ILCTPCPIDRESPONSE_H
#define ILCTPCPIDRESPONSE_H
...
class IlcTPCPIDResponse {
public:
  IlcTPCPIDResponse();
  IlcTPCPIDResponse(const Double_t *param);
  virtual ~IlcTPCPIDResponse() {}
  ...
  void SetResponseFunction(IlcPID::EParticleType type, TObject * const o) { fResponseFunctions.AddAt(o,(Int_t)type); }
  const TObject * GetResponseFunction(IlcPID::EParticleType type) { return fResponseFunctions.At((Int_t)type); }
  
  Double_t GetExpectedSignal(const Float_t mom,
                     IlcPID::EParticleType n=IlcPID::kKaon) const;
...
private:
  ...
  ClassDef(IlcTPCPIDResponse,3)   // TPC PID class
};

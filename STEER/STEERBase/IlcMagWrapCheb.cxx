...
ClassImp(IlcMagWrapCheb)

//__________________________________________________________________________________________
IlcMagWrapCheb::IlcMagWrapCheb() : 
...
//__________________________________________________________________________________________
IlcMagWrapCheb::IlcMagWrapCheb(const IlcMagWrapCheb& src) : 
  TNamed(src),
  fNParamsSol(0),fNZSegSol(0),fNPSegSol(0),fNRSegSol(0),
  fSegZSol(0),fSegPSol(0),fSegRSol(0),
  fBegSegPSol(0),fNSegPSol(0),fBegSegRSol(0),fNSegRSol(0),fSegIDSol(0),fMinZSol(1.e6),fMaxZSol(-1.e6),fParamsSol(0),fMaxRSol(0),
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::CopyFrom(const IlcMagWrapCheb& src) 
{ 
  // copy method
  Clear();
  SetName(src.GetName());
  ...
  if (src.fNParamsSol) {
    ...
    for (int i=0;i<fNParamsSol;i++) fParamsSol->AddAtAndExpand(new IlcCheb3D(*src.GetParamSol(i)),i);
  }
  //
  ...
  if (src.fNParamsTPC) {
    ...
    for (int i=0;i<fNParamsTPC;i++) fParamsTPC->AddAtAndExpand(new IlcCheb3D(*src.GetParamTPCInt(i)),i);
  }
  //
  ...
  if (src.fNParamsTPCRat) {
    ...
    for (int i=0;i<fNParamsTPCRat;i++) fParamsTPCRat->AddAtAndExpand(new IlcCheb3D(*src.GetParamTPCRatInt(i)),i);
  }
  //
  ...
  if (src.fNParamsDip) {
    ...
    for (int i=0;i<fNParamsDip;i++) fParamsDip->AddAtAndExpand(new IlcCheb3D(*src.GetParamDip(i)),i);
  }
  //
}

//__________________________________________________________________________________________
IlcMagWrapCheb& IlcMagWrapCheb::operator=(const IlcMagWrapCheb& rhs)
{
  // assignment
  if (this != &rhs) {  
    Clear();
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::Clear(const Option_t *)
{
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::Field(const Double_t *xyz, Double_t *b) const
{
  ...
  if (iddip<0) return;
  IlcCheb3D* par = GetParamDip(iddip);
...
//__________________________________________________________________________________________
Double_t IlcMagWrapCheb::GetBz(const Double_t *xyz) const
{
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::Print(Option_t *) const
{
  // print info
  printf("Ilc magnetic field parameterized by Chebyshev polynomials\n");
...
Int_t IlcMagWrapCheb::FindDipSegment(const Double_t *xyz) const 
{
...
//__________________________________________________________________________________________________
Int_t IlcMagWrapCheb::FindSolSegment(const Double_t *rpz) const 
{
  // find the segment containing point xyz. If it is outside find the closest segment 
...
//__________________________________________________________________________________________________
Int_t IlcMagWrapCheb::FindTPCSegment(const Double_t *rpz) const 
{
  // find the segment containing point xyz. If it is outside find the closest segment 
...
//__________________________________________________________________________________________________
Int_t IlcMagWrapCheb::FindTPCRatSegment(const Double_t *rpz) const 
{
  // find the segment containing point xyz. If it is outside find the closest segment 
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::GetTPCInt(const Double_t *xyz, Double_t *b) const
{
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::GetTPCRatInt(const Double_t *xyz, Double_t *b) const
{
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::FieldCylSol(const Double_t *rphiz, Double_t *b) const
{
...
//__________________________________________________________________________________________
Double_t IlcMagWrapCheb::FieldCylSolBz(const Double_t *rphiz) const
{
  // compute Solenoid field in Cylindircal coordinates
  // note: if the point is outside the volume get the field in closest parameterized point
  int id = FindSolSegment(rphiz);
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::GetTPCIntCyl(const Double_t *rphiz, Double_t *b) const
{
  ...
  if (id>=fNParamsTPC) {
    IlcError(Form("Wrong TPCParam segment %d",id));
    b[0] = b[1] = b[2] = 0;
    return;
  }
  IlcCheb3D* par = GetParamTPCInt(id);
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::GetTPCRatIntCyl(const Double_t *rphiz, Double_t *b) const
{
  ...
  if (id>=fNParamsTPCRat) {
    IlcError(Form("Wrong TPCRatParam segment %d",id));
    b[0] = b[1] = b[2] = 0;
    return;
  }
  IlcCheb3D* par = GetParamTPCRatInt(id);
  if (par->IsInside(rphiz)) {
    par->Eval(rphiz,b); 
    return;
  }
...
#ifdef  _INC_CREATION_ILCCHEB3D_
//_______________________________________________
void IlcMagWrapCheb::LoadData(const char* inpfile)
{
  ...
  //
  TString buffs;
  IlcCheb3DCalc::ReadLine(buffs,stream);
  ...
  IlcCheb3DCalc::ReadLine(buffs,stream); // nparam
  int nparSol = buffs.Atoi(); 
  //
  for (int ip=0;ip<nparSol;ip++) {
    IlcCheb3D* cheb = new IlcCheb3D();
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::BuildTableSol()
{
  // build lookup table
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::BuildTableDip()
{
  // build lookup table
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::BuildTableTPCInt()
{
  // build lookup table
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::BuildTableTPCRatInt()
{
  // build lookup table
...
//__________________________________________________________________________________________
IlcMagWrapCheb::IlcMagWrapCheb(const char* inputFile) : 
  fNParamsSol(0),fNZSegSol(0),fNPSegSol(0),fNRSegSol(0),
  fSegZSol(0),fSegPSol(0),fSegRSol(0),
  fBegSegPSol(0),fNSegPSol(0),fBegSegRSol(0),fNSegRSol(0),fSegIDSol(0),fMinZSol(1.e6),fMaxZSol(-1.e6),fParamsSol(0),fMaxRSol(0),
//
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::AddParamSol(const IlcCheb3D* param)
{
  ...
  fParamsSol->Add( (IlcCheb3D*)param );
  fNParamsSol++;
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::AddParamTPCInt(const IlcCheb3D* param)
{
  // adds new parameterization piece for TPCInt
  // NOTE: pieces must be added strictly in increasing R then increasing Z order
  //
  if (!fParamsTPC) fParamsTPC = new TObjArray();
  fParamsTPC->Add( (IlcCheb3D*)param);
  fNParamsTPC++;
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::AddParamTPCRatInt(const IlcCheb3D* param)
{
  // adds new parameterization piece for TPCRatInt
  // NOTE: pieces must be added strictly in increasing R then increasing Z order
  //
  if (!fParamsTPCRat) fParamsTPCRat = new TObjArray();
  fParamsTPCRat->Add( (IlcCheb3D*)param);
  fNParamsTPCRat++;
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::AddParamDip(const IlcCheb3D* param)
{
  // adds new parameterization piece for Dipole
  //
  if (!fParamsDip) fParamsDip = new TObjArray();
  fParamsDip->Add( (IlcCheb3D*)param);
  fNParamsDip++;
  //
}

//__________________________________________________________________________________________
void IlcMagWrapCheb::ResetTPCInt()
{
...
//__________________________________________________________________________________________
void IlcMagWrapCheb::ResetTPCRatInt()
{
...
void IlcMagWrapCheb::BuildTable(Int_t npar,TObjArray *parArr, Int_t &nZSeg, Int_t &nYSeg, Int_t &nXSeg,
				...
				Int_t **segID)
{
  ...
  for (int iz=0;iz<nZSeg;iz++) {
	...
	for (int ipar=0;ipar<npar;ipar++) {
	  IlcCheb3D* cheb = (IlcCheb3D*) parArr->At(ipar);
...
void IlcMagWrapCheb::BuildTableDip()
{
  ...
  for (int iz=0;iz<fNZSegDip;iz++) {
	...
	for (int ipar=0;ipar<fNParamsDip;ipar++) {
	  IlcCheb3D* cheb = (IlcCheb3D*) fParamsDip->At(ipar);
	  if (!cheb->IsInside(xyz)) continue;
...
void IlcMagWrapCheb::SaveData(const char* outfile) const
{
  ...
  //
  // Sol part    ---------------------------------------------------------
  fprintf(stream,"# Set of Chebyshev parameterizations for ILC magnetic field\nSTART %s\n",GetName());
...
Int_t IlcMagWrapCheb::SegmentDimension(float** seg,const TObjArray* par,int npar, int dim, 
				       float xmn,float xmx,float ymn,float ymx,float zmn,float zmx)
{
  ...
  for (int ip=0;ip<npar;ip++) {
    IlcCheb3D* cheb = (IlcCheb3D*) par->At(ip);

...
//
// Interface between the TVirtualMagField and IlcMagWrapCheb: wrapper to
...
class IlcMagWrapCheb;

class IlcMagF : public TVirtualMagField
{
 public:
  enum BMap_t      {k2kG, k5kG, k5kGUniform, kConst};
  ...
  //
  IlcMagF();
  IlcMagF(const char *name, const char* title,Double_t factorSol=1., Double_t factorDip=1., 
	  BMap_t maptype = k5kG, BeamType_t btype=kBeamTypepp, Double_t benergy=-1,	
	  Int_t integ=2, Double_t fmax=15,const char* path="$(ILC_ROOT)/data/maps/mfchebKGI_sym.root");
  IlcMagF(const IlcMagF& src);             
  IlcMagF& operator=(const IlcMagF& src);
  virtual ~IlcMagF();
  //
  ...
  //
  IlcMagWrapCheb* GetMeasuredMap()                              const {return fMeasuredMap;}
  //
  // former IlcMagF methods or their aliases
  ...
  Double_t     SolenoidField()                                  const {return 10.*fFactorSol*fSolenoid;}
  //
  ...
  static IlcMagF* CreateFieldMap(Float_t l3Current=-30000., Float_t diCurrent=-6000., 
				 Int_t convention=0, Bool_t uniform = kFALSE, 
				 Float_t beamenergy=7000, const Char_t* btype="pp",
				 const Char_t* path="$(ILC_ROOT)/data/maps/mfchebKGI_sym.root");
  //
 protected:
  ...
  //
 protected:
  IlcMagWrapCheb*  fMeasuredMap;     //! Measured part of the field map
  ...
  //   
  ClassDef(IlcMagF, 2)           // Class for all Ilc MagField wrapper for measured data + Tosca parameterization
};

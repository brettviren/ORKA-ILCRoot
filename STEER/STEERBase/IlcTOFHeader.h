#ifndef ILCTOFHEADER_H
#define ILCTOFHEADER_H
...
class IlcTOFHeader : public TObject {
 
 public:
 
  IlcTOFHeader();
  IlcTOFHeader(Float_t defEvTime, Float_t defResEvTime,
	       Int_t nDifPbins, Float_t *times, Float_t *res,
	       Int_t *nPbin, Float_t tofTimeRes, Float_t t0spread);
  IlcTOFHeader(const IlcTOFHeader &source);
  IlcTOFHeader &operator=(const IlcTOFHeader &source);
  virtual void Copy(TObject &obj) const;

  virtual ~IlcTOFHeader();
 ...
 private:
  ...
  ClassDef(IlcTOFHeader,2)  // Class for TOF event times and so on
};

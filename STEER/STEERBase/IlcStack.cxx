...
/* $Id: IlcStack.cxx 51413 2011-09-04 16:47:44Z morsch $ */
...
//_______________________________________________________________________
IlcStack::IlcStack(Int_t size, const char* /*evfoldname*/):
  fParticles("TParticle",1000),
  fParticleMap(size),
  fParticleFileMap(0),
  fParticleBuffer(0),
...
//_______________________________________________________________________
IlcStack::IlcStack(const IlcStack& st):
    TVirtualMCStack(st),
    fParticles("TParticle",1000),
...
//_______________________________________________________________________
void IlcStack::Copy(TObject&) const
{
...
void IlcStack::PushTrack(Int_t done, Int_t parent, Int_t pdg, const Float_t *pmom,
...
//	   mass,e,fNtrack,pdg,parent,done,vpos[0],vpos[1],vpos[2],pmom[0],pmom[1],pmom[2],kS);
	...
	IlcWarning(Form("Particle type %d not defined in PDG Database !", pdg));
	IlcWarning("Particle skipped !");
    }
}

//_____________________________________________________________________________
void IlcStack::PushTrack(Int_t done, Int_t parent, Int_t pdg,
...
{ 
  ...
  if(parent>=0) {
	  ...
	  IlcError(Form("Parent %d does not exist",parent));
      }
  } else { 
      //
...
//_____________________________________________________________________________
TParticle*  IlcStack::PopNextTrack(Int_t& itrack)
{
  //
...
//_____________________________________________________________________________
TParticle*  IlcStack::PopPrimaryForTracking(Int_t i)
{
  //
...
Bool_t IlcStack::KeepPhysics(const TParticle* part)
{
    //
...
//    Don't panic this is a probably a lego run
  ...
  for(Int_t i=0; i<fHgwmk+1; ++i) {
    if((part=GetParticleMapEntry(i))) {
      ...
      if (allFilled) IlcWarning(Form("Why != 0 part # %d?\n",i));
    }
    else 
    {
      // // printf("Why = 0 part # %d?\n",i); => We know.
...
void IlcStack::FlagTrack(Int_t track)
{
  //
...
//_____________________________________________________________________________
void IlcStack::KeepTrack(Int_t track)
{ 
  //
...
//_____________________________________________________________________________
void  IlcStack::Clean(Int_t size) 
{
  //
...
//_____________________________________________________________________________
void  IlcStack::Reset(Int_t size) 
{
  //
...
//_____________________________________________________________________________
void  IlcStack::ResetArrays(Int_t size) 
{
  //
  // Resets stack arrays
  //
...
//_____________________________________________________________________________
void IlcStack::SetHighWaterMark(Int_t)
{
  //
...
//_____________________________________________________________________________
TParticle* IlcStack::Particle(Int_t i)
{
  //
...
//_____________________________________________________________________________
TParticle* IlcStack::ParticleFromTreeK(Int_t id) const
{
// 
...
//_____________________________________________________________________________
Int_t IlcStack::TreeKEntry(Int_t id) const 
{
//
...
//_____________________________________________________________________________
Int_t IlcStack::GetCurrentParentTrackNumber() const
{
  //
  ...
  if (current) 
    ...
    IlcWarning("Current track not found in the stack");
    return -1;
  }  
}
 
//_____________________________________________________________________________
Int_t IlcStack::GetPrimary(Int_t id)
{
  //
...
//_____________________________________________________________________________
void IlcStack::DumpPart (Int_t i) const
{
  //
...
//_____________________________________________________________________________
void IlcStack::DumpLoadedStack() const
{
  //
...
//_____________________________________________________________________________
void  IlcStack::SetCurrentTrack(Int_t track)
{ 
  fCurrent = track; 
...
//_____________________________________________________________________________
TParticle* IlcStack::GetNextParticle()
{
  //
...
void IlcStack::ConnectTree(TTree* tree)
{
//
...
//
  ...
  IlcDebug(1, "Connecting TreeK");
  if (fTreeK == 0x0)
   {
    if (TreeK() == 0x0)
     {
      IlcFatal("Parameter is NULL");//we don't like such a jokes
      return;
     }
 ...
 //  Create a branch for particles   
  ...
  IlcDebug(2, Form("Tree name is %s",fTreeK->GetName()));
   
  if (fTreeK->GetDirectory())
   {
     IlcDebug(2, Form("and dir is %s",fTreeK->GetDirectory()->GetName()));
   }    
  else
    IlcWarning("DIR IS NOT SET !!!");
  
  TBranch *branch=fTreeK->GetBranch("Particles");
  if(branch == 0x0)
   {
    branch = fTreeK->Branch("Particles", &fParticleBuffer, 4000);
    IlcDebug(2, "Creating Branch in Tree");
   }  
  else
   {
    IlcDebug(2, "Branch Found in Tree");
    branch->SetAddress(&fParticleBuffer);
   }
  if (branch->GetDirectory())
   {
    IlcDebug(1, Form("Branch Dir Name is %s",branch->GetDirectory()->GetName()));
   } 
  else
    IlcWarning("Branch Dir is NOT SET");
}

//_____________________________________________________________________________

Bool_t IlcStack::GetEvent()
{
//
...
Bool_t IlcStack::IsStable(Int_t pdg) const
{
  //
...
//_____________________________________________________________________________
Bool_t IlcStack::IsPhysicalPrimary(Int_t index)
{
    //
...
Bool_t IlcStack::IsSecondaryFromWeakDecay(Int_t index) {
...
Bool_t IlcStack::IsSecondaryFromMaterial(Int_t index) {

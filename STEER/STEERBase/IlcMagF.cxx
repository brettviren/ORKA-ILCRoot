...
const Double_t IlcMagF::fgkSol2DipZ    =  -700.;  
const UShort_t IlcMagF::fgkPolarityConvention = IlcMagF::kConvLHC;
/*
 Explanation for polarity conventions: these are the mapping between the
 current signs and main field components in L3 (Bz) and Dipole (Bx) (in Ilc frame)
 ...
 static Int_t IlcMagF::GetPolarityConvention()) which do not lead to such field polarities.
...
//_______________________________________________________________________
IlcMagF::IlcMagF(const char *name, const char* title, Double_t factorSol, Double_t factorDip, 
  ...
  fParNames("","")
{
  ...
  SetTitle(title);
  if(integ<0 || integ > 2) {
    IlcWarning(Form("Invalid magnetic field flag: %5d; Helix tracking chosen instead",integ));
    fInteg = 2;
  }
  ...
  if (fBeamEnergy<=0 && fBeamType!=kNoBeamField) {
    ...
    IlcInfo("Maximim possible beam energy for requested beam is assumed");
  } 
  ...
  else if (fMapType == kConst) parname = "Const";
  else IlcFatal(Form("Unknown field identifier %d is requested\n",fMapType));
  //
...
//_______________________________________________________________________
IlcMagF::IlcMagF(const IlcMagF &src):
...
{
  if (src.fMeasuredMap) fMeasuredMap = new IlcMagWrapCheb(*src.fMeasuredMap);
}
...
//_______________________________________________________________________
Bool_t IlcMagF::LoadParameterization()
{
  if (fMeasuredMap) {
    IlcFatal(Form("Field data %s are already loaded from %s\n",GetParamName(),GetDataFileName()));
  }

  if(fMapType == kConst){
    IlcInfo("fMapType == kConst");
    return kTRUE;
  }
  //
  ...
  if (!file) {
    IlcFatal(Form("Failed to open magnetic field data file %s\n",fname));
  }
  //
  fMeasuredMap = dynamic_cast<IlcMagWrapCheb*>(file->Get(GetParamName()));
  if (!fMeasuredMap) {
    IlcFatal(Form("Did not find field %s in %s\n",GetParamName(),fname)); 
  }
  file->Close();
  delete file;
  return kTRUE;
...
//_______________________________________________________________________
void IlcMagF::Field(const Double_t *xyz, Double_t *b)
{
  ...
  else if(fMapType == kConst){
    b[0]=b[1]=0.;
    b[2]=10.*fFactorSol*fSolenoid;
    IlcDebug(2,Form("B=%5.2f,%5.2f,%5.2f -- x=%5.2f, y=%5.2f z=%5.2f",b[0],b[1],b[2],xyz[0],xyz[1],xyz[2]));
// // // //     if(fFactor!=1) {
// // // //       b[0]*=fFactor;
// // // //       b[1]*=fFactor;
// // // //       b[2]*=fFactor;
...
//_______________________________________________________________________
Double_t IlcMagF::GetBz(const Double_t *xyz) const
{
  // Method to calculate the field at point  xyz
  //
  if (fMeasuredMap && xyz[2]>fMeasuredMap->GetMinZ() && xyz[2]<fMeasuredMap->GetMaxZ()) {
  ...
  else return 10.*fFactorSol*fSolenoid;
}

//_______________________________________________________________________
IlcMagF& IlcMagF::operator=(const IlcMagF& src)
{
  if (this != &src) {
    if (src.fMeasuredMap) { 
      if (fMeasuredMap) delete fMeasuredMap;
      fMeasuredMap = new IlcMagWrapCheb(*src.fMeasuredMap);
    }
    SetName(src.GetName());
    fSolenoid    = src.fSolenoid;
    fBeamType    = src.fBeamType;
...
//_______________________________________________________________________
void IlcMagF::InitMachineField(BeamType_t btype, Double_t benergy)
{
...
//_______________________________________________________________________
void IlcMagF::MachineField(const Double_t *x, Double_t *b) const
{
  // ---- This is the ZDC part
  // Compansators for Ilc Muon Arm Dipole
...
//_______________________________________________________________________
void IlcMagF::GetTPCInt(const Double_t *xyz, Double_t *b) const
{
...
//_______________________________________________________________________
void IlcMagF::GetTPCRatInt(const Double_t *xyz, Double_t *b) const
{
...
//_______________________________________________________________________
void IlcMagF::GetTPCIntCyl(const Double_t *rphiz, Double_t *b) const
{
  // Method to calculate the integral_0^z of br,bt,bz 
...
//_______________________________________________________________________
void IlcMagF::GetTPCRatIntCyl(const Double_t *rphiz, Double_t *b) const
{
  // Method to calculate the integral_0^z of bx/bz,by/bz and (bx/bz)^2+(by/bz)^2
  // in cylindrical coordiates ( -pi<phi<pi convention )
  b[0]=b[1]=b[2]=0.0;
...
//_______________________________________________________________________
void IlcMagF::SetFactorSol(Float_t fc)
{
...
//_______________________________________________________________________
void IlcMagF::SetFactorDip(Float_t fc)
{
...
//_______________________________________________________________________
Double_t IlcMagF::GetFactorSol() const
{
...
//_______________________________________________________________________
Double_t IlcMagF::GetFactorDip() const
{
  // return the sign*scale of the current in the Dipole according to fgkPolarityConventionthe 
  switch (fgkPolarityConvention) {
...
IlcMagF* IlcMagF::CreateFieldMap(Float_t l3Cur, Float_t diCur, Int_t convention, Bool_t uniform,
				 Float_t beamenergy, const Char_t *beamtype, const Char_t *path) 
{
  ...
  if (TMath::Abs((sclDip=diCur/diNominalCurrent)-1.) > tolerance && !uniform) {
    if (diCur <= zero) sclDip = 0.; // some small current.. -> Dipole OFF
      ...
      IlcFatalGeneral("IlcMagF",Form("Wrong dipole current (%f A)!",diCur));
    }
  }
  //
  if (uniform) { 
    ...
    if      (TMath::Abs((sclL3=l3Cur/l3NominalCurrent1)-1.) < tolerance) map  = k5kG;
      ...
      IlcFatalGeneral("IlcMagF",Form("Wrong L3 current (%f A)!",l3Cur));
    }
  }
  //
  if (sclDip!=0 && map!=k5kGUniform) {
    if ( (l3Cur<=zero) || ((convention==kConvLHC && l3Pol!=diPol) || (convention==kConvDCS2008 && l3Pol==diPol)) ) { 
      IlcFatalGeneral("IlcMagF",Form("Wrong combination for L3/Dipole polarities (%c/%c) for convention %d",
  ...
  else IlcInfoGeneral("IlcMagF",Form("Assume no LHC magnet field for the beam type %s, ",beamtype));
  char ttl[80];
  ...
  //
  return new IlcMagF("MagneticFieldMap", ttl,sclL3,sclDip,map,btype,beamenergy,2,10.,path);
  //
}

//_____________________________________________________________________________
const char*  IlcMagF::GetBeamTypeText() const
{
...
//_____________________________________________________________________________
void IlcMagF::Print(Option_t *opt) const
{
  // print short or long info
  TString opts = opt; opts.ToLower();
  IlcInfo(Form("%s:%s",GetName(),GetTitle()));
  IlcInfo(Form("Solenoid (%+.2f) T", GetFactorSol()));
  
  IlcDebug(1,Form("Solenoid %f fFactorSol: %f fSolenoid: %f",SolenoidField(), fFactorSol, fSolenoid));


//   IlcInfo(Form("%s:%s",GetName(),GetTitle()));
//   IlcInfo(Form("Solenoid (%+.2f*)%.0f kG, Dipole %s (%+.2f) %s",
// 	       GetFactorSol(),(fMapType==k5kG||fMapType==k5kGUniform)?5.:2.,
// 	       fDipoleOFF ? "OFF":"ON",GetFactorDip(),fMapType==k5kGUniform?" |Constant Field!":""));
//   if (opts.Contains("a")) {
//     IlcInfo(Form("Machine B fields for %s beam (%.0f GeV): QGrad: %.4f Dipole: %.4f",
// 		 GetBeamTypeText(),
// 		 fBeamEnergy,fQuadGradient,fDipoleField));
//     IlcInfo(Form("Uses %s of %s",GetParamName(),GetDataFileName()));
//     IlcInfo(Form("Solenoid %f fFactorSol: %f fSolenoid: %f",SolenoidField(), fFactorSol, fSolenoid));
// 
//   }

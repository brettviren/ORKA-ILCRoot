...
ClassImp(IlcMathBase) // Class implementation to enable ROOT I/O
 
IlcMathBase::IlcMathBase() : TObject()
{
  //
  // Default constructor
  //
...
void IlcMathBase::EvaluateUni(Int_t nvectors, Double_t *data, Double_t &mean
                           , Double_t &sigma, Int_t hh)
{
  ...
  if (nvectors<2) {
    IlcErrorClass(Form("nvectors = %d, should be > 1",nvectors));
    return;
  }
  if (hh<2)
    hh=(nvectors+2)/2;
...
void IlcMathBase::EvaluateUniExternal(Int_t nvectors, Double_t *data, Double_t &mean, Double_t &sigma, Int_t hh,  Float_t externalfactor)
{
...
//_____________________________________________________________________________
Int_t IlcMathBase::Freq(Int_t n, const Int_t *inlist
...
//___IlcMathBase__________________________________________________________________________
void IlcMathBase::TruncatedMean(TH1F * his, TVectorD *param, Float_t down, Float_t up, Bool_t verbose){
  //
  //
  //
...
void IlcMathBase::LTM(TH1F * his, TVectorD *param , Float_t fraction,  Bool_t verbose){
  //
  // LTM
  //
  Int_t nbins    = his->GetNbinsX();
  ...
  IlcMathBase::EvaluateUni(npoints, data, mean,sigma,npoints2);
  delete [] data;
  if (verbose)  printf("Mean\t%f\t Sigma2\t%f\n", mean,sigma);if (param){
    (*param)[0] = his->GetMaximum();
    (*param)[1] = mean;
...
Double_t  IlcMathBase::FitGaus(TH1F* his, TVectorD *param, TMatrixD */*matrix*/, Float_t xmin, Float_t xmax, Bool_t verbose){
  //
...
Double_t  IlcMathBase::FitGaus(Float_t *arr, Int_t nBins, Float_t xMin, Float_t xMax, TVectorD *param, TMatrixD */*matrix*/, Bool_t verbose){
  //
  //  Fit histogram with gaussian function
  //  
  //  Prameters:
...
Float_t IlcMathBase::GetCOG(Short_t *arr, Int_t nBins, Float_t xMin, Float_t xMax, Float_t *rms, Float_t *sum)
{
    //
...
Double_t IlcMathBase::ErfcFast(Double_t x){
...
void IlcMathBase::TestGausFit(Int_t nhistos){
  //
  ...
  s.Start();
  //IlcMathBase gaus fit
  for (Int_t i=0; i<nhistos; i++){
    IlcMathBase::FitGaus(h1f[i]->GetArray()+1,h1f[i]->GetNbinsX(),h1f[i]->GetXaxis()->GetXmin(),h1f[i]->GetXaxis()->GetXmax(),par2[i],&dummy);
  }
...
TGraph2D * IlcMathBase::MakeStat2D(TH3 * his, Int_t delta0, Int_t delta1, Int_t type){
  //
  //
  //
  ...
  for (Int_t ix=0; ix<nbinx;ix++)
    for (Int_t iy=0; iy<nbiny;iy++){
      ...
      if (type==2 || type==3){
	TVectorD vec(3);
	IlcMathBase::LTM((TH1F*)projection,&vec,0.7);
...
TGraph * IlcMathBase::MakeStat1D(TH3 * his, Int_t delta1, Int_t type){
  //
  //
  //
  // delta - number of bins to integrate
...
Double_t IlcMathBase::TruncatedGaus(Double_t mean, Double_t sigma, Double_t cutat)
{
...
Double_t IlcMathBase::TruncatedGaus(Double_t mean, Double_t sigma, Double_t leftCut, Double_t rightCut)
{
...
Double_t IlcMathBase::BetheBlochAleph(Double_t bg,
         Double_t kp1,
         Double_t kp2,
         Double_t kp3,
         Double_t kp4,
  ...
  return IlcExternalTrackParam::BetheBlochAleph(bg,kp1,kp2,kp3,kp4,kp5);
}

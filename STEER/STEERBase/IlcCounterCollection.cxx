...
/* $Id: IlcCounterCollection.cxx 54089 2012-01-23 19:15:13Z hristov $ */

//-----------------------------------------------------------------------------
/// \class IlcCounterCollection
/// 
...
ClassImp(IlcCounterCollection)

//-----------------------------------------------------------------------
IlcCounterCollection::IlcCounterCollection(const char* name) :
TNamed(name,name),
...
//-----------------------------------------------------------------------
IlcCounterCollection::~IlcCounterCollection()
{
  /// Destructor
  delete fRubrics;
  delete fRubricsSize;
...
//-----------------------------------------------------------------------
void IlcCounterCollection::Clear(Option_t*)
{
  /// Clear counters
  fRubrics->Clear();
  fRubricsSize->Reset();
...
//-----------------------------------------------------------------------
void IlcCounterCollection::AddRubric(TString name, TString listOfKeyWords)
{
  ...
  if (fRubrics->Contains(name.Data())) {
    IlcError(Form("rubric named %s already exist",name.Data()));
    return;
  }
...
//-----------------------------------------------------------------------
void IlcCounterCollection::AddRubric(TString name, Int_t maxNKeyWords)
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::Init(Bool_t weightedCounters)
{
...
//-----------------------------------------------------------------------
Int_t IlcCounterCollection::GetNActiveBins(Int_t dim)
{
...
//-----------------------------------------------------------------------
Bool_t IlcCounterCollection::ContainsAny(Int_t dim)
{
...
//-----------------------------------------------------------------------
const Int_t* IlcCounterCollection::FindBins(const TString& externalKey, Bool_t allocate, Int_t& nEmptySlots)
{
  ...
  while ((pair = static_cast<TObjString*>(next()))) {
    ...
    if (bins[dim] >= 0) {
      IlcWarning("key already given for that rubric --> ignored");
      delete rubricKeyPair;
      continue;
    }
...
//-----------------------------------------------------------------------
Int_t IlcCounterCollection::FindDim(const TString& rubricName) const
{
  ...
  if (!rubric) {
    IlcError(Form("invalid rubric: %s",rubricName.Data()));
    return -1;
  }
...
//-----------------------------------------------------------------------
Int_t IlcCounterCollection::FindBin(Int_t dim, const TString& keyWord, Bool_t allocate)
{
  ...
  if (bin<0) IlcError(Form("invalid key word: %s:%s",axis->GetName(),keyWord.Data()));
  
  return bin;
}

//-----------------------------------------------------------------------
Short_t** IlcCounterCollection::DecodeSelection(const TString& selections, const TObjArray& displayedRubrics)
{
...
//-----------------------------------------------------------------------
Bool_t IlcCounterCollection::Select(Bool_t include, const TString& rubric, const TString& keywords,
				    Bool_t displayed, Short_t* selectBins[])
{
  ...
  if (selectBins[dim]) {
    IlcWarning(Form("selection already made for rubric %s --> ignored",rubric.Data()));
    return kTRUE;
  }
  ...
  if (keys->GetEntriesFast() == 0) {
    IlcError(Form("no key word specified for rubric %s",rubric.Data()));
    delete keys;
    return kFALSE;
  }
...
//-----------------------------------------------------------------------
void IlcCounterCollection::CleanListOfStrings(TObjArray* list)
{
  ...
  for (Int_t i = 0; i < nEntries; i++) {
	...
	IlcWarning(Form("multiple-occurence of string \"%s\" --> removed",entry2->String().Data()));
	list->RemoveAt(j);
      }
    }
  }
...
//-----------------------------------------------------------------------
void IlcCounterCollection::Count(TString externalKey, Int_t value)
{
  /// Add "value" to the counter referenced by "externalKey".
  /// The externalKey format must be rubric:keyWord/rubric:keyWord/rubric:keyWord/...
  if (value > 0) CountAsDouble(externalKey, (Double_t)value);
  else if (value < 0) IlcError("cannot count negative values");
}

//-----------------------------------------------------------------------
void IlcCounterCollection::Count(TString externalKey, Double_t value)
{
  /// Add "value" to the counter referenced by "externalKey".
  /// The externalKey format must be rubric:keyWord/rubric:keyWord/rubric:keyWord/...
  if (fWeightedCounters) CountAsDouble(externalKey, value);
  else IlcError("non-weighted counters can only be filled with intergers");
}

//-----------------------------------------------------------------------
void IlcCounterCollection::CountAsDouble(TString externalKey, Double_t value)
{
  /// Add "value" to the counter referenced by "externalKey".
  /// The externalKey format must be rubric:keyWord/rubric:keyWord/rubric:keyWord/...
  
  if (!fCounters) {
    IlcError("counters are not initialized");
    return;
  }
...
//-----------------------------------------------------------------------
void IlcCounterCollection::Print(const Option_t* opt) const
{
  /// Print every individual counters if opt=="" or call "Print(opt, "")".
  
  if (strcmp(opt,"")) {
    const_cast<IlcCounterCollection*>(this)->Print(opt, "");
    return;
  }
...
//-----------------------------------------------------------------------
TString IlcCounterCollection::GetKeyWords(TString rubric) const
{
...
//-----------------------------------------------------------------------
Double_t IlcCounterCollection::GetSum(TString selections, Bool_t* longCounters)
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::PrintKeyWords() const
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::PrintValue(TString selections)
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::Print(TString rubrics, TString selections, Bool_t removeEmpty)
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::PrintSum(TString selections)
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::PrintList(const TH1D* hist, Bool_t removeEmpty, Bool_t longCounters) const
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::PrintArray(const TH2D* hist, Bool_t removeEmpty, Bool_t longCounters) const
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::PrintListOfArrays(const THnSparse* hist, Bool_t removeEmpty, Bool_t longCounters) const
{
...
//-----------------------------------------------------------------------
Int_t IlcCounterCollection::GetMaxLabelSize(THashList* labels) const
{
...
//-----------------------------------------------------------------------
TH1D* IlcCounterCollection::Get(TString rubric, TString selections)
{
...
//-----------------------------------------------------------------------
TH2D* IlcCounterCollection::Get(TString rubric1, TString rubric2, TString selections)
{
...
//-----------------------------------------------------------------------
TH1D* IlcCounterCollection::Draw(TString rubric, TString selections)
{
...
//-----------------------------------------------------------------------
TH2D* IlcCounterCollection::Draw(TString rubric1, TString rubric2, TString selections)
{
...
//-----------------------------------------------------------------------
TObject* IlcCounterCollection::Projection(const TObjArray& data, const TString& selections, Bool_t& longCounters)
{
...
//-----------------------------------------------------------------------
Int_t* IlcCounterCollection::CheckConsistency(const IlcCounterCollection* c)
{
  ...
  if (c->fWeightedCounters != fWeightedCounters) IlcWarning("merging non-weighted with weigthed counters");
  
  // check if the number of rubrics is the same
  Int_t nRubrics = fRubrics->GetSize();
  if (c->fRubrics->GetSize() != nRubrics) {
    IlcError("both counters do not contain the same number of rubrics");
    return 0x0;
  }
  ...
  while ((rubric1 = nextRubric())) {
    ...
    if (!rubric2) {
      IlcError(Form("the other counter does not contain the rubric %s", rubric1->GetName()));
      delete[] otherDims;
      return 0x0;
    }
	...
	IlcError("that rubric does not contain the same number of authorized key words in both counters");
	delete[] otherDims;
	return 0x0;
      }
	...
	if (!keyWords2->FindObject(keyWord->String().Data())) {
	  IlcError(Form("rubric %s does not contain the key word %s in the other counter", rubric1->GetName(), keyWord->String().Data()));
	  delete[] otherDims;
	  return 0x0;
	}
      ...
      if (keyWords1) {
	IlcError(Form("rubric %s of the other counter does not contain a list of authorized key words while this does", rubric1->GetName()));
      } else {
	IlcError(Form("rubric %s of this counter does not contain a list of authorized key words while the other does", rubric1->GetName()));
      }
      delete[] otherDims;
      return 0x0;
...
//-----------------------------------------------------------------------
void IlcCounterCollection::Add(const IlcCounterCollection* counter)
{
  /// Add the given IlcCounterCollections to this. They must have the
  ...
  for (Long64_t i = 0; i < counter->fCounters->GetNbins(); i++) {
    ...
    for (Int_t dim = 0; dim < nRubrics; dim++) {
      ...
      if (thisBins[dim] < 0) {
	IlcError("this counter is full, unable to add that key word");
	ok = kFALSE;
	break;
      }
    }
...
//-----------------------------------------------------------------------
Long64_t IlcCounterCollection::Merge(TCollection* list)
{
  /// Merge this with a list of IlcCounterCollections. All IlcCounterCollections provided
  /// must have the same rubrics with the same list of authorized key words if any.
  ...
  while ((obj = next())) {
    ...
    // check that "obj" is an object of the class IlcCounterCollection
    const IlcCounterCollection* counter = dynamic_cast<const IlcCounterCollection*>(obj);
    if (!counter) {
      IlcFatal(Form("object named \"%s\" is a %s instead of an IlcCounterCollection!", obj->GetName(), obj->ClassName()));
      continue;
    }
...
//-----------------------------------------------------------------------
void IlcCounterCollection::Sort(Option_t* opt, Bool_t asInt)
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::SortRubric(TString rubric, Bool_t asInt)
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::Sort(const Bool_t* rubricsToSort, Bool_t asInt)
{
...
//-----------------------------------------------------------------------
THashList* IlcCounterCollection::SortAsInt(const THashList* labels)
{
...
//-----------------------------------------------------------------------
void IlcCounterCollection::ConvertToTHnSparseL(THnSparse* &h)
{

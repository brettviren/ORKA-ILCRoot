...
Double_t tpcBethe(Double_t *x, Double_t *par) {
  ...
  Double_t bg=p/m;
  return IlcTPCpidESD::Bethe(bg);  
}


Int_t IlcPIDComparison(const Char_t *dir=".") { 
   gBenchmark->Start("IlcPIDComparison");

   ::Info("IlcPIDComparison.C","Doing comparison...");
   ...
   IlcRunLoader *rl = IlcRunLoader::Open(fname);
   if (rl == 0x0) {
      ::Error("IlcPIDComparison.C","Can not open session !");
      return 1;
   }
   if (rl->LoadgIlc()) {
      ::Error("IlcPIDComparison.C","LoadgIlc returned error !");
      delete rl;
      return 1;
   }
   if (rl->LoadHeader()) {
      ::Error("IlcPIDComparison.C","LoadHeader returned error !");
      delete rl;
      return 1;
   }
   rl->LoadKinematics();
   ...
   if (!ef || !ef->IsOpen()) {
      ::Error("IlcPIDComparison.C","Can't IlcESDs.root !"); 
      delete rl;
      return 1;
   }
   ...
   if (!tree) {
      ::Error("IlcPIDComparison.C", "no ESD tree found");
      delete rl;
      return 1;
   }
   ...
   while (tree->GetEvent(e)) {
      ...
      while (ntrk--) {
        IlcESDtrack *t=event->GetTrack(ntrk);
	...
	//*** Some track quality cuts ****
        ...
        if (!t->IsOn(IlcESDtrack::kITSpid)) continue;
        if (!t->IsOn(IlcESDtrack::kTPCpid)) continue;
        //if (!t->IsOn(IlcESDtrack::kTRDpid)) continue;
        if (!t->IsOn(IlcESDtrack::kTOFpid)) continue;
        {
           nsel++;
	   ...
	   tpcHist->Fill(p,dedx,1);
           ...
           Double_t w[10];
           w[0]=pid.GetProbability(IlcPID::kElectron);
           w[1]=pid.GetProbability(IlcPID::kMuon);
           w[2]=pid.GetProbability(IlcPID::kPion);
           w[3]=pid.GetProbability(IlcPID::kKaon);
           w[4]=pid.GetProbability(IlcPID::kProton);
   ...
   if (!fpi) {
      ...
      fpi->SetLineColor(2);
      fpi->SetParameter(0,IlcPID::ParticleMass(IlcPID::kPion));
   }
   fpi->Draw("same");
   ...
   if (!fka) {
      ...
      fka->SetLineColor(3);
      fka->SetParameter(0,IlcPID::ParticleMass(IlcPID::kKaon));
   }
   fka->Draw("same");
   ...
   if (!fpr) {
      ...
      fpr->SetLineColor(4);
      fpr->SetParameter(0,IlcPID::ParticleMass(IlcPID::kProton));
   }
   fpr->Draw("same");
   ...
   ef->Close();
   TFile fc("IlcPIDComparison.root","RECREATE");
   c1->Write();
   fc.Close();

   gBenchmark->Stop("IlcPIDComparison");
   gBenchmark->Show("IlcPIDComparison");

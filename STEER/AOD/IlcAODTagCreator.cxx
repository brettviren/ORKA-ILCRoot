...
/* $Id: IlcAODTagCreator.cxx 50615 2011-07-16 23:19:19Z hristov $ */

//-----------------------------------------------------------------
//           IlcAODTagCreator class
...
ClassImp(IlcAODTagCreator)


//______________________________________________________________________________
  IlcAODTagCreator::IlcAODTagCreator() :
    IlcTagCreator(), 
    fChain(0), 
    fAODEvent(0), 
    fTreeT(0), 
    ...
    fRunTagEsd(0)  
{
  //==============Default constructor for a IlcAODTagCreator================
}

//______________________________________________________________________________
IlcAODTagCreator::~IlcAODTagCreator() {
//================Default destructor for a IlcAODTagCreator===================
  delete fChain;
  delete fAODEvent;
}

//______________________________________________________________________________
Bool_t IlcAODTagCreator::ReadGridCollection(TGridResult *fresult) {
  ...
  IlcInfo(Form("AOD chain created......."));	
...
//______________________________________________________________________________
Bool_t IlcAODTagCreator::ReadLocalCollection(const char *localpath, const char* pattern) {
  // Checks the different subdirs of the given local path and in the
  // case where it finds an IlcAOD.root file it creates the tags
...
//______________________________________________________________________________
Bool_t IlcAODTagCreator::ReadCAFCollection(const char *filename) {
...
void IlcAODTagCreator::CreateAODTags(Int_t fFirstEvent, Int_t fLastEvent, TList */*grpList*/) {
  // Creates tag files for AODs

    IlcInfo(Form("Creating the AOD tags......."));	
    
    TFile *file = TFile::Open("IlcAOD.root");
    if (!file || !file->IsOpen()) {
    ...
    TBranch * btag = fTreeT->Branch("IlcTAG", &fRunTag);
...
void IlcAODTagCreator::CreateTag(TChain* chain, const char *type) {
    ...
    IlcInfo(Form("Chained tag files: %lld",fTreeTEsd->GetEntries()));
    ...
    fRunTagEsd = new IlcRunTag();
    fTreeTEsd->SetBranchAddress("IlcTAG",&fRunTagEsd);
...
void IlcAODTagCreator::CreateTags(const char* /*type*/)
{
...
void IlcAODTagCreator::FillEventTag(IlcAODEvent* aod, IlcEventTag* evTag)
{
// 
// Fill the event tag information	
    ...
    // Primary Vertex
    IlcAODVertex *pVertex = fAODEvent->GetPrimaryVertex();
    if (pVertex) {
	...
	IlcAODVertex *vertex = fAODEvent->GetVertex(nVtx);
	...
	IlcAODTrack *track = fAODEvent->GetTrack(nTr);

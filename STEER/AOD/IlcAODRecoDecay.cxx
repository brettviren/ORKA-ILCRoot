...
ClassImp(IlcAODRecoDecay)

//--------------------------------------------------------------------------
IlcAODRecoDecay::IlcAODRecoDecay() :
  IlcVTrack(),
  fSecondaryVtx(0x0),
...
//--------------------------------------------------------------------------
IlcAODRecoDecay::IlcAODRecoDecay(IlcAODVertex *vtx2,Int_t nprongs,
				 Short_t charge,
...
{
  //
  // Constructor with IlcAODVertex for decay vertex
  //
...
{
  //
  // Constructor with IlcAODVertex for decay vertex and without prongs momenta
  //
...
//--------------------------------------------------------------------------
IlcAODRecoDecay::IlcAODRecoDecay(const IlcAODRecoDecay &source) :
  IlcVTrack(source),
...
//--------------------------------------------------------------------------
IlcAODRecoDecay &IlcAODRecoDecay::operator=(const IlcAODRecoDecay &source)
{
  //
  // assignment operator
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::Alpha() const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::DecayLength2(Double_t point[3]) const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::DecayLengthXY(Double_t point[3]) const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::CosPointingAngle(Double_t point[3]) const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::CosPointingAngleXY(Double_t point[3]) const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::CosThetaStar(Int_t ip,UInt_t pdgvtx,UInt_t pdgprong0,UInt_t pdgprong1) const 
{
  //
...
//---------------------------------------------------------------------------
Double_t IlcAODRecoDecay::Ct(UInt_t pdg,Double_t point[3]) const
{
  //
...
//---------------------------------------------------------------------------
Double_t IlcAODRecoDecay::E2(UInt_t pdg) const 
{
  //
  // Energy
  //
...
//---------------------------------------------------------------------------
Double_t IlcAODRecoDecay::E2Prong(Int_t ip,UInt_t pdg) const 
{
  //
...
//--------------------------------------------------------------------------
Bool_t IlcAODRecoDecay::GetCovarianceXYZPxPyPz(Double_t cv[21]) const {
  //
  ...
  if(!GetNDaughters()) {
    IlcError("No daughters available");
    return kFALSE;
  }

  Double_t v[6];
  IlcAODVertex *secv=GetSecondaryVtx();
  if(!secv) {
    IlcError("Vertex covariance matrix not available");
    return kFALSE;
  }
  ...
  for(Int_t i=1; i<GetNDaughters(); i++) {
    IlcVTrack *daugh = (IlcVTrack*)GetDaughter(i);
    Double_t dcov[21];
    if(!daugh->GetCovarianceXYZPxPyPz(dcov)) error=kTRUE;
    for(j=0;j<21;j++) p[j] += dcov[j];
  }
  if(error) {
    IlcError("No covariance for at least one daughter");
    return kFALSE;
  }
...
//----------------------------------------------------------------------------
UChar_t  IlcAODRecoDecay::GetITSClusterMap() const {
  //
...
//--------------------------------------------------------------------------
ULong_t IlcAODRecoDecay::GetStatus() const {
  // 
  ...
  IlcVTrack *daugh0 = (IlcVTrack*)GetDaughter(0);
...
//--------------------------------------------------------------------------
Bool_t IlcAODRecoDecay::PropagateToDCA(const IlcVVertex* vtx,Double_t b,Double_t maxd,Double_t dz[2],Double_t covar[3]) 
{
  ...
  IlcWarning("The IlcAODRecoDecay momentum is not updated to the DCA");

  Bool_t retval=kTRUE;
  if(Charge()==0) {
    // convert to IlcNeutralTrackParam
    IlcNeutralTrackParam ntp(this);  
...
//--------------------------------------------------------------------------
Double_t IlcAODRecoDecay::ImpParXY(Double_t point[3]) const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::InvMass2(Int_t npdg,UInt_t *pdg) const 
{
  //
...
//----------------------------------------------------------------------------
Bool_t IlcAODRecoDecay::PassInvMassCut(Int_t pdgMom,Int_t npdgDg,UInt_t *pdgDg,Double_t cut) const {
  //
  // Apply mass cut
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::InvMass2Prongs(Int_t ip1,Int_t ip2,
...
//----------------------------------------------------------------------------
Int_t IlcAODRecoDecay::MatchToMC(Int_t pdgabs, TClonesArray *mcArray,
				 Int_t ndgCk, const Int_t *pdgDg) const
{
  ...
  if(ndg>10) {
    IlcError("Only decays with <10 daughters supported");
    return -1;
  }
  if(ndgCk>0 && ndgCk!=ndg) {
    IlcError("Wrong number of daughter PDGs passed");
    return -1;
  }
  ...
  for(Int_t i=0; i<ndg; i++) {
    IlcAODTrack *trk = (IlcAODTrack*)GetDaughter(i);
				 ...
				 Int_t ndgCk, const Int_t *pdgDg) const
{
  ...
  IlcAODMCParticle *part=0;
  IlcAODMCParticle *mother=0;
  ...
  for(i=0; i<ndg; i++) {
    ...
    part = (IlcAODMCParticle*)mcArray->At(lab);
    if(!part) { 
    ...
    while(mother->GetMother()>=0) {
      ...
      mother = (IlcAODMCParticle*)mcArray->At(labMother);
      if(!mother) {
  ...
  IlcAODMCParticle* p1=(IlcAODMCParticle*)(mcArray->At(mother->GetDaughter(1)));
  IlcAODMCParticle* p0=(IlcAODMCParticle*)(mcArray->At(mother->GetDaughter(0)));
...
//---------------------------------------------------------------------------
void IlcAODRecoDecay::Print(Option_t* /*option*/) const 
{
  //
  // Print some information
  //
  printf("IlcAODRecoDecay with %d prongs\n",GetNProngs());
  printf("Secondary Vertex: (%f, %f, %f)\n",GetSecVtxX(),GetSecVtxY(),GetSecVtxZ());

  return;
}
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::ProngsRelAngle(Int_t ip1,Int_t ip2) const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::QlProng(Int_t ip) const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::QtProng(Int_t ip) const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::QlProngFlightLine(Int_t ip,Double_t point[3]) const 
{
  //
...
//----------------------------------------------------------------------------
Double_t IlcAODRecoDecay::QtProngFlightLine(Int_t ip,Double_t point[3]) const 
{
  //

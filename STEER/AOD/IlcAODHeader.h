#ifndef ILCAODHEADER_H
#define ILCAODHEADER_H
/* 2012-2013 Infrastructure for Large Collider Experiment.       *
 * All rights reserved.                                                   *
 * See cxx source for full Copyright notice                               */

/* $Id: IlcAODHeader.h 58331 2012-08-28 10:38:13Z hristov $ */
...
class IlcAODHeader : public IlcVHeader {

 public :
  IlcAODHeader();
 
  IlcAODHeader(Int_t nRun, UShort_t nBunchX, UInt_t nOrbit, UInt_t nPeriod, const Char_t *title="");
  IlcAODHeader(Int_t nRun, 
	       UShort_t nBunchX,
	       UInt_t nOrbit,
	       UInt_t nPeriod,
	       Int_t refMult,
  ...
  virtual ~IlcAODHeader();
  IlcAODHeader(const IlcAODHeader& evt); 
  IlcAODHeader& operator=(const IlcAODHeader& evt);
  ...
  IlcCentrality* GetCentralityP()  const { return fCentralityP; }
  IlcEventplane* GetEventplaneP()  const { return fEventplaneP; }
  ...
  void SetCentrality(const IlcCentrality* cent);
  void SetEventplane(IlcEventplane* eventplane);
 ...
 private :
  ...
  IlcCentrality* fCentralityP;      // Pointer to full centrality information
  IlcEventplane* fEventplaneP;	    // Pointer to full event plane information
  ...
  ClassDef(IlcAODHeader, 18);
};
inline
void IlcAODHeader::SetCentrality(const IlcCentrality* cent)      { 
    if(cent){
	if(fCentralityP)*fCentralityP = *cent;
	else fCentralityP = new IlcCentrality(*cent);
...
inline
void IlcAODHeader::SetEventplane(IlcEventplane* eventplane)      { 
    if(eventplane){
	if(fEventplaneP)*fEventplaneP = *eventplane;
	else fEventplaneP = new IlcEventplane(*eventplane);
...
inline
void IlcAODHeader::SetITSClusters(Int_t ilay, UInt_t nclus)
{

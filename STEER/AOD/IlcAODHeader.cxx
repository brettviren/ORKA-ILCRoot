...
/* $Id: IlcAODHeader.cxx 58331 2012-08-28 10:38:13Z hristov $ */
...
ClassImp(IlcAODHeader)

//______________________________________________________________________________
IlcAODHeader::IlcAODHeader() : 
  IlcVHeader(),
  fMagneticField(-999.),
...
//______________________________________________________________________________
IlcAODHeader::IlcAODHeader(Int_t nRun, 
			   UShort_t nBunchX,
			   UInt_t nOrbit,
			   UInt_t nPeriod,
			   const Char_t *title) :
...
//______________________________________________________________________________
IlcAODHeader::IlcAODHeader(const IlcAODHeader& hdr) :
  IlcVHeader(hdr),
  ...
  fCentralityP(new IlcCentrality(*hdr.fCentralityP)),
  fEventplaneP(new IlcEventplane(*hdr.fEventplaneP))
{
  // Copy constructor.
...
//______________________________________________________________________________
IlcAODHeader& IlcAODHeader::operator=(const IlcAODHeader& hdr)
{
  // Assignment operator
  if(this!=&hdr) {
    
     IlcVHeader::operator=(hdr);
    ...
    if(hdr.fEventplaneP){
      ...
      else fEventplaneP = new IlcEventplane(*hdr.fEventplaneP);
    }

    if(hdr.fCentralityP){
      if(fCentralityP)*fCentralityP = *hdr.fCentralityP;
      else fCentralityP = new IlcCentrality(*hdr.fCentralityP);
    }
...
//______________________________________________________________________________
void IlcAODHeader::SetQTheta(Double_t *QTheta, UInt_t size) 
{
  if (QTheta && size>0) {
...
//______________________________________________________________________________
Double_t IlcAODHeader::GetQTheta(UInt_t i) const
{
...
//______________________________________________________________________________
void IlcAODHeader::RemoveQTheta()
{
  delete[] fQTheta;
  fQTheta = 0x0;
  fNQTheta = 0;
...
void IlcAODHeader::Clear(Option_t* /*opt*/)
{
// Clear memory
  RemoveQTheta();
  if (fCentralityP){
...
//______________________________________________________________________________
void IlcAODHeader::Print(Option_t* /*option*/) const 
{

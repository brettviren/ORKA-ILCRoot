#ifndef ILCAODVERTEX_H
#define ILCAODVERTEX_H
/* 2012-2013 Infrastructure for Large Collider Experiment.       *
 * All rights reserved.                                                   *
 * See cxx source for full Copyright notice                               */

/* $Id: IlcAODVertex.h 51831 2011-09-29 16:57:48Z morsch $ */
...
class IlcAODVertex : public IlcVVertex {
 ...
 public :
  ...
  IlcAODVertex();
  IlcAODVertex(const Double_t *position, 
	       ...
	       Char_t vtype=kUndef,
	       Int_t nprong = 0);
  IlcAODVertex(const Float_t *position, 
  ...
  virtual ~IlcAODVertex();
  IlcAODVertex(const IlcAODVertex& vtx); 
  IlcAODVertex& operator=(const IlcAODVertex& vtx);

  virtual IlcAODVertex* CloneWithoutRefs() const;
  ...
  template <class T> void SetCovMatrix(const T *covMatrix) {
    if(!fCovMatrix) fCovMatrix=new IlcAODRedCov<3>();
  ...
  template <class T, class P> void     PhiAndThetaToVertex(IlcAODVertex *vtx, P &phi, T &theta) const;
  Double_t  Distance2ToVertex(const IlcAODVertex *vtx) const;
  Double_t  DistanceToVertex(IlcAODVertex *vtx) const 
     {return TMath::Sqrt(Distance2ToVertex(vtx));}
  Double_t  DistanceXY2ToVertex(const IlcAODVertex *vtx) const;
  Double_t  DistanceXYToVertex(IlcAODVertex *vtx) const 
     {return TMath::Sqrt(DistanceXY2ToVertex(vtx));}
  Double_t  Error2DistanceToVertex(IlcAODVertex *vtx) const;
  Double_t  ErrorDistanceToVertex(IlcAODVertex *vtx) const
     {return TMath::Sqrt(Error2DistanceToVertex(vtx));}
  Double_t  Error2DistanceXYToVertex(IlcAODVertex *vtx) const;
  Double_t  ErrorDistanceXYToVertex(IlcAODVertex *vtx) const
...
private:
 ...
 private:
  ...
  IlcAODRedCov<3> *fCovMatrix;    // vertex covariance matrix; values of and below the diagonal
  ...
  ClassDef(IlcAODVertex, 8);
};

inline  Int_t IlcAODVertex::GetNDaughters() const
{
    if (!fProngs) {
...
inline TObject* IlcAODVertex::GetDaughter(Int_t i)
{
    if (!fProngs) {
	...
	if (i < fNprong) {
	    ...
	    IlcWarning("Daughter index out of range !\n");
	    return 0;
	}
    }
}

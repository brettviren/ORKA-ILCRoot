...
Bool_t CheckESD(const char* gIlcFileName = "gilc.root", 
		const char* esdFileName = "IlcESDs.root")
{
// check the content of the ESD
  ...
  // open run loader and load gIlc, kinematics and header
  IlcRunLoader* runLoader = IlcRunLoader::Open(gIlcFileName);
  if (!runLoader) {
  ...
  if (!gIlc) {
    Error("CheckESD", "no gilc object found");
    return kFALSE;
  }
  ...
  // PID
  Int_t partCode[IlcPID::kSPECIES] = 
    {kElectron, kMuonMinus, kPiPlus, kKPlus, kProton};
  const char* partName[IlcPID::kSPECIES+1] = 
    {"electron", "muon", "pion", "kaon", "proton", "other"};
  Double_t partFrac[IlcPID::kSPECIES] = 
    {0.01, 0.01, 0.85, 0.10, 0.05};
  Int_t identified[IlcPID::kSPECIES+1][IlcPID::kSPECIES];
  for (Int_t iGen = 0; iGen < IlcPID::kSPECIES+1; iGen++) {
    for (Int_t iRec = 0; iRec < IlcPID::kSPECIES; iRec++) {
  ...
  for (Int_t iEvent = 0; iEvent < runLoader->GetNumberOfEvents(); iEvent++) {
    ...
    for (Int_t iTrack = 0; iTrack < esd->GetNumberOfTracks(); iTrack++) {
      ...
      if ((track->GetStatus() & IlcESDtrack::kITSrefit) == 0) continue;
      ...
      // PID
      if ((track->GetStatus() & IlcESDtrack::kESDpid) == 0) continue;
      Int_t iGen = 5;
	...
	if (probability[i] > pMax) {
      ...
      // dE/dx and TOF
      Double_t time[IlcPID::kSPECIES];
      track->GetIntegratedTimes(time);
      if (iGen == iRec) {
	hDEdxRight->Fill(particle->P(), track->GetTPCsignal());
        if ((track->GetStatus() & IlcESDtrack::kTOFpid) != 0) {
    ...
    for (Int_t iCascade = 0; iCascade < esd->GetNumberOfCascades(); 
	 iCascade++) {
      IlcESDcascade* cascade = esd->GetCascade(iCascade);
      Double_t v0q;
	...
	IlcESDCaloCluster * clust = esd->GetCaloCluster(iCluster);

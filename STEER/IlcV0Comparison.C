 ...
 *   TPC/IlcTPCComparison.C and ITS/IlcITSComparisonV2.C macros.            *
 *                                                                          *
...
Int_t IlcV0Comparison(Int_t code=310, const Char_t *dir=".") { 
   //Lambda=3122, LambdaBar=-3122
   gBenchmark->Start("IlcV0Comparison");

   ::Info("IlcV0Comparison.C","Doing comparison...");
   ...
   ::Info("IlcV0Comparison.C","Marking good V0s (will take a while)...");
     if (GoodV0s(dir)) {
        ::Error("IlcV0Comparison.C","Can't generate the reference file !");
        return 1;
     }
   }
   refFile=TFile::Open(fname,"old");
   if (!refFile || !refFile->IsOpen()) {
     ::Error("IlcV0Comparison.C","Can't open the reference file !");
     return 1;
   }   
  
   TTree *v0Tree=(TTree*)refFile->Get("v0Tree");
   if (!v0Tree) {
     ::Error("IlcV0Comparison.C","Can't get the reference tree !");
     return 2;
   }
   TBranch *pbranch=v0Tree->GetBranch("positive");
   if (!pbranch) {
     ::Error("IlcV0Comparison.C","Can't get the positive daughter branch !");
     return 3;
   }
   ...
   if (!nbranch) {
     ::Error("IlcV0Comparison.C","Can't get the negative daughter branch !");
     return 4;
   }
   ...
   if ((!ef)||(!ef->IsOpen())) {
      sprintf(fname,"%s/IlcESDv0.root",dir);
      ef=TFile::Open(fname);
      if ((!ef)||(!ef->IsOpen())) {
         ::Error("IlcV0Comparison.C","Can't open IlcESDv0.root !");
         return 5;
      }
   }
   ...
   if (!esdTree) {
      ::Error("IlcV0Comparison.C", "no ESD tree found");
      return 6;
   }
   ...
   while (esdTree->GetEvent(e)) {
      ...
      for (i=0; i<nentr; i++) {
          ...
          IlcTrackReference *nref=0, *pref=0;
          Int_t j;
          for (j=0; j<ngood; j++) {
   ...
   TFile fc("IlcV0Comparison.root","RECREATE");
   c1->Write();
   c2->Write();
   fc.Close();

   gBenchmark->Stop("IlcV0Comparison");
   gBenchmark->Show("IlcV0Comparison");
...
Int_t GoodV0s(const Char_t *dir) {
   ...
   TClonesArray dum("IlcTrackReference",1000), *itsRefs=&dum;
	...
	//cerr<<np<<'\r';
	 ...
	 if (part) {
	   TParticlePDG * partPDG = part->GetPDG();
         ...
         new((*nrefs)[nv]) IlcTrackReference(*nref);
         new((*prefs)[nv]) IlcTrackReference(*pref);

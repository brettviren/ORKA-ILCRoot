...
/* $Id: IlcTrackerBase.cxx 38069 2009-12-24 16:56:18Z belikov $ */

//-------------------------------------------------------------------------
//               Implementation of the IlcTrackerBase class
//                that is the base for the IlcTracker class    
  ...
  fSigmaZ(0.010)
{
  ...
  if (!TGeoGlobalMagField::Instance()->GetField())
    IlcWarning("Field map is not set.");
}

//__________________________________________________________________________
IlcTrackerBase::IlcTrackerBase(const IlcTrackerBase &atr):
  TObject(atr),
  fX(atr.fX),
  fY(atr.fY),
  fZ(atr.fZ),
...
//__________________________________________________________________________
Double_t IlcTrackerBase::GetBz(const Double_t *r) {
  //------------------------------------------------------------------
...
//__________________________________________________________________________
void IlcTrackerBase::GetBxByBz(const Double_t r[3], Double_t b[3]) {
  //------------------------------------------------------------------
...
Double_t IlcTrackerBase::MeanMaterialBudget(const Double_t *start, const Double_t *end, Double_t *mparam)
{
// // // //   // 
...
// // // //   if (!startnode) {
// // // //     IlcErrorClass(Form("start point out of geometry: x %f, y %f, z %f",
...
// // // //       IlcErrorClass("Cannot cross boundary\n");
...
Bool_t 
IlcTrackerBase::PropagateTrackTo(IlcExternalTrackParam *track, Double_t xToGo, 
  ...
  while ( (xToGo-xpos)*dir > kEpsilon){
    ...
    if (rotateTo){
      ...
      if (!track->IlcExternalTrackParam::Rotate(alphan)) return kFALSE;
    }
    xpos = track->GetX();
...
Bool_t 
IlcTrackerBase::PropagateTrackToBxByBz(IlcExternalTrackParam *track,
...
Double_t IlcTrackerBase::GetTrackPredictedChi2(IlcExternalTrackParam *track,
...
Double_t IlcTrackerBase::MakeC(Double_t x1,Double_t y1,
...
Double_t IlcTrackerBase::MakeSnp(Double_t x1,Double_t y1,
                   Double_t x2,Double_t y2,
                   Double_t x3,Double_t y3)
{
  //-----------------------------------------------------------------
...
Double_t IlcTrackerBase::MakeTgl(Double_t x1,Double_t y1,
                   Double_t x2,Double_t y2,
...
IlcExternalTrackParam * IlcTrackerBase::MakeSeed( IlcTrackPoint &point0, IlcTrackPoint &point1, IlcTrackPoint &point2){
  //
  // Make Seed  - IlcExternalTrackParam from input 3 points   
  ...
  //
  IlcTrackPoint p0r = point0.Rotate(alpha);
  IlcTrackPoint p1r = point1.Rotate(alpha);
  IlcTrackPoint p2r = point2.Rotate(alpha);
  xyz0[0]=p0r.GetX();
  xyz0[1]=p0r.GetY();
  xyz0[2]=p0r.GetZ();
  xyz1[0]=p1r.GetX();
  ...
  IlcExternalTrackParam * trackParam = new IlcExternalTrackParam(xyz0[0],alpha,param, covar);
  if (0) {
    ...
    if (TMath::Abs(y1-xyz1[1])> TMath::Sqrt(p1r.GetCov()[3]*5)){
      IlcWarningClass("Seeding problem y1\n");
    }
    if (TMath::Abs(y2-xyz2[1])> TMath::Sqrt(p2r.GetCov()[3]*5)){
      IlcWarningClass("Seeding problem y2\n");
    }
    if (TMath::Abs(z1-xyz1[2])> TMath::Sqrt(p1r.GetCov()[5]*5)){
      IlcWarningClass("Seeding problem z1\n");
    }
  }
  return trackParam;  
} 

Double_t  IlcTrackerBase::FitTrack(IlcExternalTrackParam * trackParam, IlcTrackPointArray *pointArray, Double_t mass, Double_t maxStep){
  //
  ...
  // rotate method of IlcExternalTrackParam should be revisited
...
void IlcTrackerBase::UpdateTrack(IlcExternalTrackParam &track1, const IlcExternalTrackParam &track2){
  //

#ifndef ILCVERTEXERTRACKS_H
#define ILCVERTEXERTRACKS_H
...
class IlcVertexerTracks : public TObject {
 ...
 public:
  ...
  IlcVertexerTracks(); 
  IlcVertexerTracks(Double_t fieldkG); 
  virtual ~IlcVertexerTracks();

  IlcESDVertex* FindPrimaryVertex(const IlcVEvent *vEvent);
  IlcESDVertex* FindPrimaryVertex(const TObjArray *trkArrayOrig,UShort_t *idOrig);
  IlcESDVertex* VertexForSelectedTracks(const TObjArray *trkArray,UShort_t *id,
					Bool_t optUseFitter=kTRUE,
					Bool_t optPropagate=kTRUE,
					Bool_t optUseDiamondConstraint=kFALSE);
  IlcESDVertex* VertexForSelectedESDTracks(TObjArray *trkArray,
					Bool_t optUseFitter=kTRUE,
					Bool_t optPropagate=kTRUE,
					Bool_t optUseDiamondConstraint=kFALSE);
  IlcESDVertex* RemoveTracksFromVertex(IlcESDVertex *inVtx,
				       const TObjArray *trkArray,UShort_t *id,
				       const Float_t *diamondxy) const; 
  IlcESDVertex* RemoveConstraintFromVertex(IlcESDVertex *inVtx,
					   Float_t *diamondxyz,
  ...
  void  SetOnlyFitter() { if(!fConstraint) IlcFatal("Set constraint first!"); 
  ...
  static IlcESDVertex TrackletVertexFinder(const TClonesArray *lines, Int_t optUseWeights=0);
  static IlcESDVertex TrackletVertexFinder(IlcStrLine **lines, const Int_t knacc, Int_t optUseWeights=0);
  void     SetFieldkG(Double_t field=-999.) { fFieldkG=field; return; }
  Double_t GetFieldkG() const { 
    if(fFieldkG<-99.) IlcFatal("Field value not set");
    return fFieldkG; } 
  ...
  //
  IlcESDVertex* GetCurrentVertex() const {return (IlcESDVertex*)fCurrentVertex;}
  TObjArray*    GetVerticesArray() const {return (TObjArray*)fMVVertices;}   // RS to be removed
  void          AnalyzePileUp(IlcESDEvent* esdEv);
 ...
 protected:
  ...
  Int_t    PrepareTracks(const TObjArray &trkArrayOrig,const UShort_t *idOrig,
			 Int_t optImpParCut);
  Bool_t   PropagateTrackTo(IlcExternalTrackParam *track,
			    Double_t xToGo);
  Bool_t   TrackToPoint(IlcExternalTrackParam *t,
  ...
  IlcESDVertex fVert;         // vertex after vertex finder
  IlcESDVertex *fCurrentVertex;  // ESD vertex after fitter
  ...
  UShort_t  *fIdSel;          // IDs of the tracks (IlcESDtrack::GetID())
  ...
  //
 private:
  IlcVertexerTracks(const IlcVertexerTracks & source);
  IlcVertexerTracks & operator=(const IlcVertexerTracks & source);

  ClassDef(IlcVertexerTracks,16) // 3D Vertexing with tracks 
};

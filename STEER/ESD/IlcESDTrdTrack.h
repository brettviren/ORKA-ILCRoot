#ifndef ILCESDTRDTRACK_H
#define ILCESDTRDTRACK_H
...
class IlcESDTrdTrack : public TObject {

 public:

  IlcESDTrdTrack();
  virtual ~IlcESDTrdTrack() {};
  IlcESDTrdTrack(const IlcESDTrdTrack& track);
  IlcESDTrdTrack& operator=(const IlcESDTrdTrack& track);
  ...
  IlcESDTrdTracklet* GetTracklet(Int_t idx) const
    { return (GetLayerMask() & (1<<idx)) ? (IlcESDTrdTracklet*) ((fTrackletRefs[idx]).GetObject()) : 0x0; }
  ...
  void AddTrackletReference(IlcESDTrdTracklet* trkl, Int_t layer) { fTrackletRefs[layer] = trkl; }

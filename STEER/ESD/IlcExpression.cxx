...
/* $Id: IlcExpression.cxx 50636 2011-07-18 11:27:31Z hristov $ */
...
ClassImp( IlcExpression )

//______________________________________________________________________________
IlcExpression::IlcExpression( TString exp ) :
  TObject(),
  fVname(""),
  fArg1(0x0),
  fArg2(0x0),
  fOperator(0)
{
   ...
   Int_t i = -1;
   IlcExpression* e = Expression( *tokens, i );
   // Copy !!!
...
//______________________________________________________________________________
IlcExpression& IlcExpression::operator=(const IlcExpression& e)
{
   // IlcExpression assignment operator.
...
//______________________________________________________________________________
IlcExpression::IlcExpression( int op, IlcExpression* a, IlcExpression* b ) :
  TObject(),
  fVname(""),
  fArg1(a),
  fArg2(b),
...
//______________________________________________________________________________
IlcExpression::IlcExpression( int op, IlcExpression* a ) :
  TObject(),
  fVname(""),
  fArg1(0),
  fArg2(a),
  ...
  if ( ( fArg2 == 0 && fVname.IsNull() ) ||
   ...
   switch (fOperator) {
      ...
      case 0 :
	{
          ...
          if( dd == NULL ) {
             IlcError( fVname + " is undefined" );
             return 0;
          }
          return ((IlcTriggerInput*)dd)->GetValue();
	}

      default:
          IlcError( "Illegal operator in expression!");
...
//______________________________________________________________________________
TString IlcExpression::Unparse() const
{
...
//______________________________________________________________________________
TObjArray* IlcExpression::Tokenize( TString str ) const
{
...
//______________________________________________________________________________
IlcExpression* IlcExpression::Element( TObjArray &st, Int_t &i )
{
   // create an element
   
   IlcExpression* result = 0;
   ...
   switch( ttok ) {
      case 'w' :
          result = new IlcVariableExpression( token );
          break;
      case '(' :
          ...
          if( token[0] != ')' ) {
       //       i--; // push back
              IlcErrorGeneral( "IlcExpression::Element", "Mismatched parenthesis." );
              delete result;
              result = new IlcExpression;
          }
          break;
      default:
          i--; // push back
          IlcErrorGeneral( "IlcExpression::Element", Form("Unexpected symbol on input. %s", token.Data()) );
          result = new IlcExpression;
   }
   return result;
}
...
//______________________________________________________________________________
IlcExpression* IlcExpression::Primary( TObjArray &st, Int_t &i )
{
   // create a primary
   ...
   switch (token[0]) {
       case '!' :
           return new IlcExpression( kOpNOT, Primary( st, i ) );
       default:
           i--; // push back
           return Element( st, i );
   }
...
//______________________________________________________________________________
IlcExpression* IlcExpression::Expression( TObjArray &st,Int_t &i )
{
...
//   cout <<"i "<<i<< "Primary " << result->Unparse() << endl;
   while (! done) {
      ...
      switch (token[0]) {
         case '&' :
             result = new IlcExpression( kOpAND, result, Primary( st, i ) );
//   cout <<"i "<<i<< " Expression AND " << result->Unparse() << endl;
             break;
         case '|' :
             result = new IlcExpression( kOpOR, result, Primary( st, i ) );
//   cout <<"i "<<i<< " Expression OR " << result->Unparse() << endl;
   ...
   if( stack == 0 && !token.IsNull() && token[0] == ')' ) {
      IlcErrorGeneral( "IlcExpression::Expression", "To many closing parenthesis." );
      delete result;
      result = new IlcExpression;
   } else 
   if( stack == 0 && i< nt-1 ) {
      IlcErrorGeneral( "IlcExpression::Expression", Form( "Unexpected symbol on input. %s", token.Data() ) );
      delete result;
      result = new IlcExpression;
   }
   return result;
...
ClassImp( IlcVariableExpression )

//______________________________________________________________________________
Bool_t IlcVariableExpression::Value( const TObjArray& pgm )
{
   // return the value

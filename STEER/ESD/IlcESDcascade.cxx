...
/* $Id: IlcESDcascade.cxx 50615 2011-07-16 23:19:19Z hristov $ */
...
ClassImp(IlcESDcascade)

IlcESDcascade::IlcESDcascade() : 
  IlcESDv0(),
...
IlcESDcascade::IlcESDcascade(const IlcESDcascade& cas) :
  IlcESDv0(cas),
...
IlcESDcascade::IlcESDcascade(const IlcESDv0 &v,
			     const IlcExternalTrackParam &t, Int_t i) : 
  IlcESDv0(v),
  fEffMassXi(TDatabasePDG::Instance()->GetParticle(kXiMinus)->Mass()),
  fChi2Xi(1024),
  fDcaXiDaughters(1024),
...
IlcESDcascade& IlcESDcascade::operator=(const IlcESDcascade& cas)
{
  //--------------------------------------------------------------------
...
void IlcESDcascade::Copy(TObject &obj) const {
  ...
  if(this==&obj)return;
  IlcESDcascade *robj = dynamic_cast<IlcESDcascade*>(&obj);
  if(!robj)return; // not an IlcESDcascade
  *robj = *this;
}
...
// Start with IlcVParticle functions
Double_t IlcESDcascade::E() const {
  //--------------------------------------------------------------------
...
Double_t IlcESDcascade::Y() const {
  //--------------------------------------------------------------------
  // This gives the energy assuming the ChangeMassHypothesis was called
  //--------------------------------------------------------------------
  return Y(fPdgCodeXi);
...
// Then extend IlcVParticle functions
Double_t IlcESDcascade::E(Int_t pdg) const {
  //--------------------------------------------------------------------
...
Double_t IlcESDcascade::Y(Int_t pdg) const {
  //--------------------------------------------------------------------
...
Double_t IlcESDcascade::RapXi() const {
  //--------------------------------------------------------------------
...
Double_t IlcESDcascade::RapOmega() const {
  //--------------------------------------------------------------------
...
Double_t IlcESDcascade::AlphaXi() const {
  //--------------------------------------------------------------------
...
Double_t IlcESDcascade::PtArmXi() const {
  //--------------------------------------------------------------------
...
Double_t IlcESDcascade::ChangeMassHypothesis(Double_t &v0q, Int_t code) {
  //--------------------------------------------------------------------
  ...
  if (Charge()*code<0)
    ...
    IlcWarning("Chosen PDG code does not match the sign of the bachelor... Corrected !!");
    fPdgCodeXi = -code;
  }
  ...
  case kOmegaPlusBar: 
       ...
       break;
  default:
       IlcError("Invalide PDG code !  Assuming a Xi particle...");
       if (Charge()<0) {
	 fPdgCodeXi=kXiMinus;
       }
       else {
...
void 
IlcESDcascade::GetPxPyPz(Double_t &px, Double_t &py, Double_t &pz) const {
  //--------------------------------------------------------------------
...
void IlcESDcascade::GetXYZcascade(Double_t &x, Double_t &y, Double_t &z) const {
  //--------------------------------------------------------------------
...
Double_t IlcESDcascade::GetDcascade(Double_t x0, Double_t y0, Double_t z0) const {
  //--------------------------------------------------------------------
...
Double_t IlcESDcascade::GetCascadeCosineOfPointingAngle(Double_t refPointX, Double_t refPointY, Double_t refPointZ) const {
...
void IlcESDcascade::GetPosCovXi(Double_t cov[6]) const {

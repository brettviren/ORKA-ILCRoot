...
/* $Id: IlcFMDMap.cxx 51728 2011-09-25 13:14:50Z hristov $ */
...
//
#include "IlcFMDMap.h"		// ILCFMDMAP_H
#include "IlcLog.h"
//#include <TClass.h>
//#include <TBuffer.h>
#include <TFile.h>
...
//____________________________________________________________________
IlcFMDMap::IlcFMDMap(UShort_t maxDet, 
		     UShort_t maxRing, 
		     UShort_t maxSec, 
		     UShort_t maxStr)
...
//____________________________________________________________________
IlcFMDMap::IlcFMDMap(const IlcFMDMap& other)
  : TObject(other), 
...
//____________________________________________________________________
void
IlcFMDMap::CheckNeedUShort(TFile* file) 
{
  if (!file) return;
  TObject* o = file->GetStreamerInfoList()->FindObject("IlcFMDMap");
  if (!o) return;
...
//____________________________________________________________________
void
IlcFMDMap::Index2CoordsOld(Int_t     idx, 
			   UShort_t& det, 
			   Char_t&   ring, 
			   UShort_t& sec, 
			   UShort_t& str) const
...
//____________________________________________________________________
void
IlcFMDMap::Index2Coords(Int_t     idx, 
			UShort_t& det, 
			Char_t&   ring, 
			UShort_t& sec, 
			UShort_t& str) const
...
//____________________________________________________________________
void
IlcFMDMap::CalcCoords(Int_t     idx, 
		      UShort_t& det, 
		      Char_t&   ring, 
		      UShort_t& sec, 
		      UShort_t& str) const
...
//____________________________________________________________________
Int_t 
IlcFMDMap::Coords2IndexOld(UShort_t det, Char_t ring, UShort_t sec, 
			   UShort_t str) const
{
...
//____________________________________________________________________
Int_t 
IlcFMDMap::Coords2Index(UShort_t det, Char_t ring, UShort_t sec, 
			UShort_t str) const
{
  // Check that the index supplied is OK.   Returns true index, or -1
  // on error. 
...
//____________________________________________________________________
Int_t 
IlcFMDMap::CheckIndex(UShort_t det, Char_t ring, UShort_t sec, 
		      UShort_t str) const
{
  // Check that the index supplied is OK.   Returns true index, or -1
  // on error. 
...
//____________________________________________________________________
Int_t 
IlcFMDMap::CalcIndex(UShort_t det, Char_t ring, UShort_t sec, UShort_t str) const
{
  ...
  if (idx < 0) {
    UShort_t ringi = (ring == 'I' ||  ring == 'i' ? 0 : 
		      (ring == 'O' || ring == 'o' ? 1 : 2));
    IlcFatal(Form("Index FMD%d%c[%2d,%3d] out of bounds, "
...
#define INCOMP_OP(self, other, OP) do {					\
  IlcWarning("Incompatible sized IlcFMDMap");				\
  ...
  for (UShort_t d = 1; d <= maxDet; d++) {				\
    ...
    for (UShort_t q = 0; q < nRng; q++) {                               \
	  ...
	  if (idx1 < 0 || idx2 < 0) {					\
	    IlcWarning("Index out of bounds");				\
	    continue;							\
	  }								\
	  if (self->IsFloat())						\
...
IlcFMDMap&
IlcFMDMap::operator*=(const IlcFMDMap& other)
{
...
//__________________________________________________________
Bool_t
IlcFMDMap::ForEach(ForOne& algo) const
{
  // Assignment operator 
  Bool_t ret = kTRUE;
...
//__________________________________________________________
void
IlcFMDMap::Print(Option_t* option) const
{
...
//===================================================================
IlcFMDMap::Printer::Printer(const char* format)
  : fFormat(format), fOldD(0), fOldR('-'), fOldS(1024) 
{}

//___________________________________________________________________
IlcFMDMap::Printer::Printer(const Printer& p) 
  : IlcFMDMap::ForOne(p),
    fFormat(p.fFormat), 
    fOldD(p.fOldD), 
    fOldR(p.fOldR), 
...
//___________________________________________________________________
void
IlcFMDMap::Printer::PrintHeadings(UShort_t d, Char_t r, UShort_t s, UShort_t t) 
{
  if (d != fOldD) { 
    fOldD = d;
    fOldR = '-';
...
//___________________________________________________________________
Bool_t
IlcFMDMap::Printer::operator()(UShort_t d, Char_t r, UShort_t s, UShort_t t, 
			       Float_t m)
{
...
#if 0
//___________________________________________________________________
void IlcFMDMap::Streamer(TBuffer &R__b)
{
  // Stream an object of class IlcFMDMap.
  ...
  if (R__b.IsReading()) {
    ...
    IlcFMDMap::Class()->ReadBuffer(R__b, this, version, R__s, R__c);
    if (version == 2) SetBit(kNeedUShort);
  } else {
    IlcFMDMap::Class()->WriteBuffer(R__b, this);
  }
}
#endif

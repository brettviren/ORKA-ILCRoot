...
/* $Id: IlcESDUtils.cxx 54304 2012-02-01 10:37:13Z shahoian $ */
...
Float_t IlcESDUtils::GetCorrV0(const IlcESDEvent* esd, Float_t &v0CorrResc, Float_t *v0multChCorr, Float_t *v0multChCorrResc)
{
  ...
  for(Int_t i = 0; i < 64; ++i) {
    if (esdV0->TestBit(IlcESDVZERO::kCorrectedForSaturation)) {
...
//______________________________________________________________________________
Float_t IlcESDUtils::GetCorrSPD2(Float_t spd2raw,Float_t zv)
{
...
//______________________________________________________________________________
Bool_t  IlcESDUtils::RefitESDVertexTracks(IlcESDEvent* esdEv, Int_t algo)
{
  ...
  if (currRun!=esdEv->GetRunNumber() && kVtxConstr) { // update diamond for this run
    ...
    IlcESDVertex initVertex(pos,cov,1.,1);
  ...
  ((IlcESDVertex*)esdEv->GetPrimaryVertexTracks())->SetNContributors(-1);
  //
  IlcESDVertex *pvtx=vtFinder->FindPrimaryVertex(esdEv);
  if (pvtx && pvtx->GetStatus()) {
    esdEv->SetPrimaryVertexTracks(pvtx);
    for (Int_t i=esdEv->GetNumberOfTracks(); i--;) {
      IlcESDtrack *t = esdEv->GetTrack(i);

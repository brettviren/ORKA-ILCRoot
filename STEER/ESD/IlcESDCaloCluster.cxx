...
/* $Id: IlcESDCaloCluster.cxx 50615 2011-07-16 23:19:19Z hristov $ */
/* $Log $ */
...
ClassImp(IlcESDCaloCluster)

//_______________________________________________________________________
IlcESDCaloCluster::IlcESDCaloCluster() : 
  IlcVCluster(),
  fTracksMatched(0x0),
  fLabels(0x0),
  fNCells(0),
  ...
  fClusterType(kUndef), fTOF(0.)
{
  ...
  for(Int_t i=0; i<IlcPID::kSPECIESN; i++) fPID[i] = 0.;
}

//_______________________________________________________________________
IlcESDCaloCluster::IlcESDCaloCluster(const IlcESDCaloCluster& clus) : 
  IlcVCluster(clus),
  ...
  fTOF(clus.fTOF)
{
  ...
  for(Int_t i=0; i<IlcPID::kSPECIESN; i++) fPID[i] = clus.fPID[i];
...
//_______________________________________________________________________
IlcESDCaloCluster &IlcESDCaloCluster::operator=(const IlcESDCaloCluster& source)
{
  // assignment operator

  if(&source == this) return *this;
  IlcVCluster::operator=(source);
  ...
  for(Int_t i=0; i<IlcPID::kSPECIESN; i++) fPID[i] = source.fPID[i];
  fID = source.fID;
...
//_______________________________________________________________________
void IlcESDCaloCluster::Copy(TObject &obj) const {
  ...
  if(this==&obj)return;
  IlcESDCaloCluster *robj = dynamic_cast<IlcESDCaloCluster*>(&obj);
  if(!robj)return; // not an IlcESDCluster
  *robj = *this;
...
//_______________________________________________________________________
void IlcESDCaloCluster::Clear(const Option_t*){ 
  //
  // This is destructor according Coding Conventions 
  //
  if(fTracksMatched)delete fTracksMatched;fTracksMatched = 0;
...
//_______________________________________________________________________
void IlcESDCaloCluster::SetPID(const Float_t *p) {
  // Sets the probability of each particle type
  // Copied from IlcESDtrack SetPIDValues
...
//_______________________________________________________________________
void IlcESDCaloCluster::GetMomentum(TLorentzVector& p, Double_t *vertex ) {
...
//_______________________________________________________________________
void  IlcESDCaloCluster::SetCellsAbsId(UShort_t *array)
{
...
//_______________________________________________________________________
void  IlcESDCaloCluster::SetCellsAmplitudeFraction(Double32_t *array)
{
...
void IlcESDCaloCluster::SetPosition(Float_t *x) 
{
  // set the position

//----------------------------------------------------------------------------
// Implementation of the IlcKFVertex class
// .
...
ClassImp(IlcKFVertex)


IlcKFVertex::IlcKFVertex( const IlcVVertex &vertex ): fIsConstrained(0)
{
  // Constructor from ILC VVertex
...
/*
void     IlcKFVertex::Print(Option_t* ) const
{  
  cout<<"IlcKFVertex position:    "<<GetX()<<" "<<GetY()<<" "<<GetZ()<<endl;
  cout<<"IlcKFVertex cov. matrix: "<<GetCovariance(0)<<endl;
  cout<<"                         "<<GetCovariance(1)<<" "<<GetCovariance(2)<<endl;
  cout<<"                         "<<GetCovariance(3)<<" "<<GetCovariance(4)<<" "<<GetCovariance(5)<<endl;
}
  */

void IlcKFVertex::SetBeamConstraint( Double_t x, Double_t y, Double_t z, 
...
void IlcKFVertex::SetBeamConstraintOff()
{
  fIsConstrained = 0;
}

void IlcKFVertex::ConstructPrimaryVertex( const IlcKFParticle *vDaughters[], 
					  ...
					  Double_t ChiCut  )
{
  ...
  while( nRest>2 )
	...
	const IlcKFParticle &p = *(vDaughters[it]);
	IlcKFVertex tmp = *this - p;
  ...
  if( nRest>=2 ){// final refit     
    ...
    int nDaughtersNew=0;
    const IlcKFParticle **vDaughtersNew=new const IlcKFParticle *[NDaughters];

...
/* $Id: IlcESD.h 52237 2011-10-20 19:26:08Z hristov $ */
...
class IlcESD : public TObject {
public:
  IlcESD();
  IlcESD(const IlcESD&);  
  virtual ~IlcESD(); 

  void SetESDfriend(const IlcESDfriend *f);
  ...
  IlcESDtrack *GetTrack(Int_t i) const {
    return (IlcESDtrack *)fTracks.UncheckedAt(i);
  }
  IlcESDHLTtrack *GetHLTConfMapTrack(Int_t i) const {
    return (IlcESDHLTtrack *)fHLTConfMapTracks.UncheckedAt(i);
  }
  IlcESDHLTtrack *GetHLTHoughTrack(Int_t i) const {
    return (IlcESDHLTtrack *)fHLTHoughTracks.UncheckedAt(i);
  }
  IlcESDMuonTrack *GetMuonTrack(Int_t i) const {
    return (IlcESDMuonTrack *)fMuonTracks.UncheckedAt(i);
  }
  IlcESDPmdTrack *GetPmdTrack(Int_t i) const {
    return (IlcESDPmdTrack *)fPmdTracks.UncheckedAt(i);
  }
  IlcESDTrdTrack *GetTrdTrack(Int_t i) const {
    return (IlcESDTrdTrack *)fTrdTracks.UncheckedAt(i);
  }
  ...
  Int_t  AddTrack(const IlcESDtrack *t) {
    IlcESDtrack * track = new(fTracks[fTracks.GetEntriesFast()]) IlcESDtrack(*t);track->SetID(fTracks.GetEntriesFast()-1);return  track->GetID();
  }
  void AddHLTConfMapTrack(const IlcESDHLTtrack *t) {
    new(fHLTConfMapTracks[fHLTConfMapTracks.GetEntriesFast()]) IlcESDHLTtrack(*t);
  }
  void AddHLTHoughTrack(const IlcESDHLTtrack *t) {
    new(fHLTHoughTracks[fHLTHoughTracks.GetEntriesFast()]) IlcESDHLTtrack(*t);
  }
  void AddMuonTrack(const IlcESDMuonTrack *t) {
    new(fMuonTracks[fMuonTracks.GetEntriesFast()]) IlcESDMuonTrack(*t);
  }
  void AddPmdTrack(const IlcESDPmdTrack *t) {
    new(fPmdTracks[fPmdTracks.GetEntriesFast()]) IlcESDPmdTrack(*t);
  }
  void AddTrdTrack(const IlcESDTrdTrack *t) {
    new(fTrdTracks[fTrdTracks.GetEntriesFast()]) IlcESDTrdTrack(*t);
  }

  IlcESDv0 *GetV0(Int_t i) const {
    return (IlcESDv0 *)fV0s.UncheckedAt(i);
  }
  Int_t AddV0(const IlcESDv0 *v);

  IlcESDcascade *GetCascade(Int_t i) const {
    return (IlcESDcascade *)fCascades.UncheckedAt(i);
  }
  void AddCascade(const IlcESDcascade *c) {
    new(fCascades[fCascades.GetEntriesFast()]) IlcESDcascade(*c);
  }

  IlcESDkink *GetKink(Int_t i) const {
    return (IlcESDkink *)fKinks.UncheckedAt(i);
  }
  Int_t AddKink(const IlcESDkink *c) {
    IlcESDkink * kink = new(fKinks[fKinks.GetEntriesFast()]) IlcESDkink(*c);
    kink->SetID(fKinks.GetEntriesFast());
    return fKinks.GetEntriesFast()-1;
  }

  IlcESDCaloCluster *GetCaloCluster(Int_t i) const {
    return (IlcESDCaloCluster *)fCaloClusters.UncheckedAt(i);
  }
  Int_t AddCaloCluster(const IlcESDCaloCluster *c) {
    IlcESDCaloCluster *clus = new(fCaloClusters[fCaloClusters.GetEntriesFast()]) IlcESDCaloCluster(*c);
  ...
  void SetVertex(const IlcESDVertex *vertex) {
     new (&fSPDVertex) IlcESDVertex(*vertex);
  }
  const IlcESDVertex *GetVertex() const {return &fSPDVertex;}

  void SetMultiplicity(const IlcMultiplicity *mul) {
     new (&fSPDMult) IlcMultiplicity(*mul);
  }
  const IlcMultiplicity *GetMultiplicity() const {return &fSPDMult;}

  void SetPrimaryVertex(const IlcESDVertex *vertex) {
     new (&fPrimaryVertex) IlcESDVertex(*vertex);
  }
  const IlcESDVertex *GetPrimaryVertex() const {return &fPrimaryVertex;}
  ...
  void SetFMDData(IlcESDFMD * obj) { fESDFMD = new IlcESDFMD(*obj); }
  IlcESDFMD *GetFMDData(){ return fESDFMD; }
   
  void SetVZEROData(IlcESDVZERO * obj) { fESDVZERO = new IlcESDVZERO(*obj); }
  IlcESDVZERO *GetVZEROData(){ return fESDVZERO; }
  #ifdef MFT_UPGRADE
  //void SetMFTData(IlcESDMFT * obj) { fESDMFT = new IlcESDMFT(*obj); }
 //IlcESDMFT *GetMFTData(){ return fESDMFT; }
  #endif
  void SetACORDEData(IlcESDACORDE * obj){ fESDACORDE = new IlcESDACORDE(*obj); } 
  IlcESDACORDE *GetACORDEDAta(){ return fESDACORDE; }
  IlcRawDataErrorLog *GetErrorLog(Int_t i) const {
    return (IlcRawDataErrorLog *)fErrorLogs.UncheckedAt(i);
  }
  void  AddRawDataErrorLog(const IlcRawDataErrorLog *log) {
    new(fErrorLogs[fErrorLogs.GetEntriesFast()]) IlcRawDataErrorLog(*log);
  }
  Int_t GetNumberOfErrorLogs()   const {return fErrorLogs.GetEntriesFast();}

protected:
  IlcESD &operator=(const IlcESD& source);
  ...
  Float_t      fMagneticField;   // Solenoid Magnetic Field in kG : for compatibility with IlcMagF
  ...
  IlcESDACORDE *fESDACORDE; // ACORDE ESD object containing bit pattern
  #ifdef MFT_UPGRADE
  //IlcESDMFT *fESDMFT; // MFT object containing rough multiplicity
  #endif
  TClonesArray fErrorLogs;        // Raw-data reading error messages

  ClassDef(IlcESD,23)  //ESD class 
};
#endif 

...
ClassImp(IlcESDVertex)

//--------------------------------------------------------------------------
IlcESDVertex::IlcESDVertex() :
  IlcVertex(),
  fCovXX(0.005*0.005),
  fCovXY(0),
  fCovYY(0.005*0.005),
...
//--------------------------------------------------------------------------
IlcESDVertex::IlcESDVertex(Double_t positionZ,Double_t sigmaZ,
...
//------------------------------------------------------------------------- 
IlcESDVertex::IlcESDVertex(Double_t position[3],Double_t covmatrix[6],
			   Double_t chi2,Int_t nContributors,
			   const Char_t *vtxName) :
  IlcVertex(position,0.,nContributors),
  fCovXX(covmatrix[0]),
  fCovXY(covmatrix[1]),
  fCovYY(covmatrix[2]),
  fCovXZ(covmatrix[3]),
...
//--------------------------------------------------------------------------
IlcESDVertex::IlcESDVertex(Double_t position[3],Double_t sigma[3],
			   const Char_t *vtxName) :
  IlcVertex(position,0.,0),
...
//--------------------------------------------------------------------------
IlcESDVertex::IlcESDVertex(const IlcESDVertex &source):
  IlcVertex(source),
...
//--------------------------------------------------------------------------
IlcESDVertex &IlcESDVertex::operator=(const IlcESDVertex &source){
  //
  // assignment operator
  //
  if(&source != this){
    IlcVertex::operator=(source);
...
//--------------------------------------------------------------------------
void IlcESDVertex::Copy(TObject &obj) const {
  ...
  if(this==&obj)return;
  IlcESDVertex *robj = dynamic_cast<IlcESDVertex*>(&obj);
  if(!robj)return; // not an IlcESDVertex
  *robj = *this;

}
//--------------------------------------------------------------------------
void IlcESDVertex::SetToZero() {
  //
...
//--------------------------------------------------------------------------
void IlcESDVertex::GetSigmaXYZ(Double_t sigma[3]) const {
  //
...
//--------------------------------------------------------------------------
void IlcESDVertex::GetCovMatrix(Double_t covmatrix[6]) const {
  //
...
//--------------------------------------------------------------------------
void IlcESDVertex::SetCovarianceMatrix(const Double_t *covmatrix) {
  //
  // Return covariance matrix of the vertex
  //
  fCovXX = covmatrix[0];
...
//--------------------------------------------------------------------------
void IlcESDVertex::GetSNR(Double_t snr[3]) const {
  //
  // Return S/N ratios
  //
...
//--------------------------------------------------------------------------
void IlcESDVertex::Print(Option_t* /*option*/) const {
  //
  ...
  //
  if (fCovXX<0 || fCovYY<0 || fCovZZ<0) {IlcError("Attention: negative diagonal element");}
  //
  return;
}
...
Double_t IlcESDVertex::GetWDist(const IlcESDVertex* v) const
{
  ...
  vVb.InvertFast();
  if (!vVb.IsValid()) {IlcError("Singular Matrix"); return dist;}

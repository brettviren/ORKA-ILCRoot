...
class IlcVertex : public IlcVVertex {
 
 public:
 
  IlcVertex();
  IlcVertex(Double_t position[3],Double_t dispersion,
		Int_t nContributors);
  IlcVertex(const IlcVertex &source);
  IlcVertex &operator=(const IlcVertex &source);
  virtual ~IlcVertex();
  ...
  virtual void     PrintIndices() const { if(fNIndices>0) 
    for(Int_t i=0;i<fNIndices;i++) printf("IlcVertex uses track %d\n",fIndices[i]); return; }
 ...
 protected:
  ...
  ClassDef(IlcVertex,4)  // Class for Primary Vertex
};

...
ClassImp(IlcCascadeVertexer)

//A set of loose cuts
Double_t 
  IlcCascadeVertexer::fgChi2max=33.;   //maximal allowed chi2 
Double_t 
  IlcCascadeVertexer::fgDV0min=0.01;   //min V0 impact parameter
Double_t 
  IlcCascadeVertexer::fgMassWin=0.008; //"window" around the Lambda mass
Double_t 
  IlcCascadeVertexer::fgDBachMin=0.01; //min bachelor impact parameter
Double_t 
  IlcCascadeVertexer::fgDCAmax=2.0;    //max DCA between the V0 and the track 
Double_t 
  IlcCascadeVertexer::fgCPAmin=0.98; //min cosine of the cascade pointing angle
Double_t 
  IlcCascadeVertexer::fgRmin=0.2;      //min radius of the fiducial volume
Double_t 
...
Int_t IlcCascadeVertexer::V0sTracks2CascadeVertices(IlcESDEvent *event) {
  ...
  //--------------------------------------------------------------------
   ...
   for (i=0; i<nentr; i++) {
       ...
       if ((status&IlcESDtrack::kITSrefit)==0)
   ...
   for (i=0; i<nV0; i++) { //loop on V0s
      ...
      IlcESDv0 *v=(IlcESDv0*)vtcs.UncheckedAt(i);
      IlcESDv0 v0(*v);
	 ...
	 IlcESDtrack *btrk=event->GetTrack(bidx);
         if (btrk->GetSign()>0) continue;  // bachelor's charge 
          
    	 IlcESDv0 *pv0=&v0;
         IlcExternalTrackParam bt(*btrk), *pbt=&bt;

         Double_t dca=PropagateToDCA(pv0,pbt,b);
         if (dca > fDCAmax) continue;

         IlcESDcascade cascade(*pv0,*pbt,bidx);//constucts a cascade candidate
...
Double_t IlcCascadeVertexer::Det(Double_t a00, Double_t a01, Double_t a10, Double_t a11) const {
  //--------------------------------------------------------------------
...
Double_t IlcCascadeVertexer::Det(Double_t a00,Double_t a01,Double_t a02,
...
Double_t IlcCascadeVertexer::PropagateToDCA(IlcESDv0 *v, IlcExternalTrackParam *t, Double_t b) {
  //--------------------------------------------------------------------

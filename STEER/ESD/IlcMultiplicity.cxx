...
ClassImp(IlcMultiplicity)

//______________________________________________________________________
IlcMultiplicity::IlcMultiplicity():
  TObject(),
  fNtracks(0),
  fNsingle(0),
  fNsingleSPD2(0),
...
//______________________________________________________________________
IlcMultiplicity::IlcMultiplicity(Int_t ntr, Float_t *th,  Float_t *ph, Float_t *dth, Float_t *dph, Int_t *labels, Int_t* labelsL2, Int_t ns, Float_t *ts, Float_t *ps, Int_t *labelss, Short_t nfcL1, Short_t nfcL2, const TBits & fFastOr):
  TObject(),
  fNtracks(ntr),
  fNsingle(ns),
  fNsingleSPD2(0),
...
//______________________________________________________________________
IlcMultiplicity::IlcMultiplicity(Int_t ntr, Int_t ns, Short_t nfcL1, Short_t nfcL2, const TBits & fFastOr) :
  TObject(),
  fNtracks(ntr),
  fNsingle(ns),
  fNsingleSPD2(0),
...
//______________________________________________________________________
IlcMultiplicity::IlcMultiplicity(const IlcMultiplicity& m):
  TObject(m),
  fNtracks(m.fNtracks),
  fNsingle(m.fNsingle),
...
//______________________________________________________________________
IlcMultiplicity &IlcMultiplicity::operator=(const IlcMultiplicity& m){
  // assignment operator
...
void IlcMultiplicity::Copy(TObject &obj) const {
  ...
  if(this==&obj)return;
  IlcMultiplicity *robj = dynamic_cast<IlcMultiplicity*>(&obj);
  if(!robj)return; // not an IlcMultiplicity
  *robj = *this;
...
//______________________________________________________________________
void IlcMultiplicity::Duplicate(const IlcMultiplicity& m){
  ...
  if(fNtracks>0){
    ...
    for (int i=2;i--;) for (int j=2;j--;) if (m.fTCl2Tracks[i][j]) fTCl2Tracks[i][j] = new IlcRefArray(*m.fTCl2Tracks[i][j]);
  }
  else {
    fTh = 0;
    fPhi = 0;
  ...
  if(fNsingle>0){
    ...
    for (int i=2;i--;) if (m.fSCl2Tracks[i]) fSCl2Tracks[i] = new IlcRefArray(*m.fSCl2Tracks[i]);
  }
  else {
    fThsingle = 0;
    fPhisingle = 0;
...
//______________________________________________________________________
void IlcMultiplicity::Clear(Option_t*)
{
  // reset all
  TObject::Clear();
  if(fTh)delete [] fTh;fTh = 0;
...
//______________________________________________________________________
void IlcMultiplicity::SetLabel(Int_t i, Int_t layer, Int_t label)
{
    if(i>=0 && i<fNtracks) {
	if (layer == 0) {
	    fLabels[i] = label;
...
//______________________________________________________________________
void IlcMultiplicity::SetLabelSingle(Int_t i, Int_t label) 
{
    if(i>=0 && i<fNsingle) {
      if (fLabelssingle) {
...
//______________________________________________________________________
UInt_t IlcMultiplicity::GetNumberOfITSClusters(Int_t layMin, Int_t layMax) const {

  if(layMax < layMin) {
    IlcError("layer min > layer max");
    return 0;
  }
  if(layMin < 0) {
    IlcError("layer min < 0");
    return 0;
  }
  if(layMax < 0) {
    IlcError("layer max > 0");
    return 0;
  }
...
//______________________________________________________________________
void IlcMultiplicity::SetTrackletData(Int_t id, const Float_t* tlet, UInt_t trSPD1, UInt_t trSPD2)
{
  // fill tracklet data
  if (id>=fNtracks) {IlcError(Form("Number of declared tracklets %d < %d",fNtracks,id)); return;}
  fTh[id]      = tlet[0];
  fPhi[id]     = tlet[1];
  fDeltPhi[id] = tlet[2];
  fDeltTh[id]  = tlet[3];
...
//______________________________________________________________________
void IlcMultiplicity::SetSingleClusterData(Int_t id, const Float_t* scl, UInt_t tr)
{
  // fill single cluster data
  if (id>=fNsingle) {IlcError(Form("Number of declared singles %d < %d",fNsingle,id)); return;}
  fThsingle[id]  = scl[0];
  fPhisingle[id] = scl[1];
...
//______________________________________________________________________
Bool_t IlcMultiplicity::FreeClustersTracklet(Int_t i, Int_t mode) const
{
...
//______________________________________________________________________
Bool_t IlcMultiplicity::GetTrackletTrackIDs(Int_t i, Int_t mode, Int_t &spd1, Int_t &spd2) const
{
...
//______________________________________________________________________
Int_t IlcMultiplicity::GetTrackletTrackIDsLay(Int_t lr,Int_t i, Int_t mode, UInt_t* refs, UInt_t maxRef) const
{
...
//______________________________________________________________________
Int_t IlcMultiplicity::GetSingleClusterTrackIDs(Int_t i, Int_t mode, UInt_t* refs, UInt_t maxRef) const
{
...
//______________________________________________________________________
Bool_t IlcMultiplicity::FreeSingleCluster(Int_t i, Int_t mode) const
{
...
//______________________________________________________________________
Bool_t IlcMultiplicity::GetSingleClusterTrackID(Int_t i, Int_t mode, Int_t &tr) const
{
...
//______________________________________________________________________
void IlcMultiplicity::CompactBits()
{
...
//______________________________________________________________________
void IlcMultiplicity::Print(Option_t *opt) const
{
  // print
...
Int_t IlcMultiplicity::GetLabelSingleLr(Int_t i, Int_t lr) const 
{
  if (lr==1) {
...
Float_t IlcMultiplicity::GetPhiAll(int icl, int lr) const
{
...
Float_t IlcMultiplicity::GetThetaAll(int icl, int lr) const
{
...
Int_t IlcMultiplicity::GetLabelAll(int icl, int lr) const
{

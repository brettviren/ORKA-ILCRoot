...
ClassImp(IlcVertexerTracks)


//----------------------------------------------------------------------------
IlcVertexerTracks::IlcVertexerTracks():
TObject(),
fVert(),
fCurrentVertex(0),
fMode(0),
...
//----------------------------------------------------------------------------
IlcVertexerTracks::IlcVertexerTracks(Double_t fieldkG):
TObject(),
fVert(),
fCurrentVertex(0),
fMode(0),
...
//----------------------------------------------------------------------------
IlcESDVertex* IlcVertexerTracks::FindPrimaryVertex(const IlcVEvent *vEvent)
{
//
...
//
  ...
  Bool_t inputAOD = ((evtype=="IlcAODEvent") ? kTRUE : kFALSE);

  if(inputAOD && fMode==1) {
    printf("Error : IlcVertexerTracks: no TPC-only vertex from AOD\n"); 
    TooFewTracks(); 
  ...
  for(Int_t i=0; i<nTrks; i++) {
    ...
    if(!fITSpureSA && (track->GetStatus()&IlcESDtrack::kITSpureSA)) continue;
    // or use only pure ITS SA tracks
    if(fITSpureSA && !(track->GetStatus()&IlcESDtrack::kITSpureSA)) continue;

    // kITSrefit
    if(fMode==0 && fITSrefit && !(track->GetStatus()&IlcESDtrack::kITSrefit)) continue;

    if(!inputAOD) {  // ESD
      IlcESDtrack* esdt = (IlcESDtrack*)track;
	...
	t = new IlcExternalTrackParam(x,esdt->GetAlpha(),p,cov);
      } else if(fMode==1) { // TPC mode
	t = (IlcExternalTrackParam*)esdt->GetTPCInnerParam();
	if(!t) continue;
  ...
  if(!inputAOD) AnalyzePileUp((IlcESDEvent*)vEvent);
  ...
  if(!inputAOD && fCurrentVertex) {
    ...
    for(Int_t ind=0; ind<nIndices; ind++) {
      IlcESDtrack *esdt = (IlcESDtrack*)vEvent->GetTrack(indices[ind]);
      esdt->SetVertexID(-1);
    }
  }
...
//----------------------------------------------------------------------------
IlcESDVertex* IlcVertexerTracks::FindPrimaryVertex(const TObjArray *trkArrayOrig,
						   UShort_t *idOrig)
{
//
// Primary vertex using the IlcExternalTrackParam's in the TObjArray.
// idOrig must contain the track IDs from IlcESDtrack::GetID()
// (Two iterations: 
//  1st with 5*fNSigma*sigma cut w.r.t. to initial vertex
//      + cut on sqrt(d0d0+z0z0) if fConstraint=kFALSE  
//  2nd with fNSigma*sigma cut w.r.t. to vertex found in 1st iteration) 
//
  ...
  IlcDebug(1,Form("Initial number of tracks: %d",nTrksOrig));
  if(nTrksOrig<fMinTracks) {
    IlcDebug(1,"TooFewTracks");
    TooFewTracks();
    return fCurrentVertex;
  } 
  ...
  if(!fConstraint) {
    ...
    default: {IlcFatal(Form("Wrong seeder algorithm %d",fAlgoIter0));} break;  
    }
    fDCAcut = cutsave;
    if(fVert.GetNContributors()>0) {
      ...
      IlcDebug(1,Form("No mean vertex: VertexFinder gives (%f, %f, %f)",fNominalPos[0],fNominalPos[1],fNominalPos[2]));
    } else {
      fNominalPos[0] = 0.;
      fNominalPos[1] = 0.;
      fNominalPos[2] = 0.;
      IlcDebug(1,"No mean vertex and VertexFinder failed");
    }
  }
  ...
  for(Int_t iter=1; iter<=2; iter++) {
    ...
    IlcDebug(1,Form("N tracks selected in iteration %d: %d",iter,nTrksSel));
	...
	case kMultiVertexer             : FindVerticesMV(); multiMode = kTRUE; break;
        default: {IlcFatal(Form("Wrong vertexer algorithm %d",fAlgo));} break;  
	}
      }
      IlcDebug(1," Vertex finding completed");
    }
  ...
  if (!multiMode || fMVVertices->GetEntries()==0) { // in multi-vertex mode this is already done for found vertices
    ...
    //
    IlcDebug(1,Form("xyz: %f %f %f; nc %d",fCurrentVertex->GetXv(),fCurrentVertex->GetYv(),fCurrentVertex->GetZv(),fCurrentVertex->GetNContributors()));
  }
  // clean up
...
//------------------------------------------------------------------------
Double_t IlcVertexerTracks::GetDeterminant3X3(Double_t matr[][3])
{
  //
...
//-------------------------------------------------------------------------
void IlcVertexerTracks::GetStrLinDerivMatrix(const Double_t *p0,const Double_t *p1,Double_t (*m)[3],Double_t *d)
{
  //
...
//--------------------------------------------------------------------------  
void IlcVertexerTracks::GetStrLinDerivMatrix(const Double_t *p0,const Double_t *p1,const Double_t *sigmasq,Double_t (*m)[3],Double_t *d)
{
  //
...
//--------------------------------------------------------------------------   
Double_t IlcVertexerTracks::GetStrLinMinDist(const Double_t *p0,const Double_t *p1,const Double_t *x0)
{
  //
  Double_t x12=p0[0]-p1[0];
  Double_t y12=p0[1]-p1[1];
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::OneTrackVertFinder() 
{
  // find vertex for events with 1 track, using DCA to nominal beam axis
  IlcDebug(1,Form("Number of prepared tracks =%d - Call OneTrackVertFinder",fTrkArraySel.GetEntries()));
  IlcExternalTrackParam *track1;
  track1 = (IlcExternalTrackParam*)fTrkArraySel.At(0);
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::HelixVertexFinder() 
{
  ...
  Int_t ncombi = 0;
  IlcExternalTrackParam *track1;
  IlcExternalTrackParam *track2;
  Double_t distCA;
  Double_t x;
  Double_t alpha, cs, sn;
  Double_t crosspoint[3];
  for(Int_t i=0; i<nacc; i++){
    track1 = (IlcExternalTrackParam*)fTrkArraySel.At(i);
    

    for(Int_t j=i+1; j<nacc; j++){
      track2 = (IlcExternalTrackParam*)fTrkArraySel.At(j);
...
//----------------------------------------------------------------------------
Int_t IlcVertexerTracks::PrepareTracks(const TObjArray &trkArrayOrig,
...
//
  ...
  IlcESDVertex *initVertex = new IlcESDVertex(fNominalPos,fNominalCov,1,1);

  if(!fTrkArraySel.IsEmpty()) fTrkArraySel.Delete();

  IlcExternalTrackParam *track=0;

  // loop on tracks
  for(Int_t i=0; i<nTrksOrig; i++) {
    IlcExternalTrackParam *trackOrig=(IlcExternalTrackParam*)trkArrayOrig.At(i);
    if(trackOrig->Charge()!=0) { // normal tracks
      track = new IlcExternalTrackParam(*(IlcExternalTrackParam*)trkArrayOrig.At(i));
    } else { // neutral tracks (from a V0)
      track = new IlcNeutralTrackParam(*(IlcNeutralTrackParam*)trkArrayOrig.At(i));
    }
    track->SetUniqueID(trackOrig->GetUniqueID());
    // tgl cut
    if(TMath::Abs(track->GetTgl())>fMaxTgl) {
      IlcDebug(1,Form(" rejecting track with tgl = %f",track->GetTgl()));
    ...
    if(optImpParCut<2 || fOnlyFitter) { // optImpParCut==1 or 0
      ...
      IlcDebug(1,Form("normdistx %f  %f    %f",fCurrentVertex->GetXv(),fNominalPos[0],TMath::Sqrt(sigmaCurr[0]*sigmaCurr[0]+fNominalCov[0])));
      IlcDebug(1,Form("normdisty %f  %f    %f",fCurrentVertex->GetYv(),fNominalPos[1],TMath::Sqrt(sigmaCurr[1]*sigmaCurr[1]+fNominalCov[2])));
      IlcDebug(1,Form("sigmaCurr %f %f    %f",sigmaCurr[0],sigmaCurr[1],TMath::Sqrt(fNominalCov[0])+TMath::Sqrt(fNominalCov[2])));
    ...
    IlcDebug(1,Form("trk %d; id %d; |d0| = %f;  d0 cut = %f; |z0| = %f; |d0|oplus|z0| = %f; d0z0 cut = %f",i,(Int_t)idOrig[i],TMath::Abs(d0z0[0]),maxd0rphi,TMath::Abs(d0z0[1]),TMath::Sqrt(d0z0[0]*d0z0[0]+d0z0[1]*d0z0[1]),fMaxd0z0));
...
//----------------------------------------------------------------------------
Bool_t IlcVertexerTracks::PropagateTrackTo(IlcExternalTrackParam *track,
					   Double_t xToGo) {
  //----------------------------------------------------------------
  // COPIED from IlcTracker
  //
...
//---------------------------------------------------------------------------
IlcESDVertex* IlcVertexerTracks::RemoveTracksFromVertex(IlcESDVertex *inVtx,
...
//
  ...
  for(Int_t i=0;i<ntrks;i++) {
    track = (IlcExternalTrackParam*)trkArray->At(i);
  ...
  IlcESDVertex *outVtx = new IlcESDVertex(position,cov,chi2,nUsedTrks+1); // the +1 is for the constraint
...
//---------------------------------------------------------------------------
IlcESDVertex* IlcVertexerTracks::RemoveConstraintFromVertex(IlcESDVertex *inVtx,
						     Float_t *diamondxyz,
...
//
  ...
  // store data in the vertex object
  IlcESDVertex *outVtx = new IlcESDVertex(position,cov,chi2,inVtx->GetNContributors()-1);
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::SetCuts(Double_t *cuts, Int_t ncuts) 
{
//
//  Cut values
//
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::SetITSMode(Double_t dcacut,
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::SetTPCMode(Double_t dcacut,
				   Double_t dcacutIter0,
				   Double_t maxd0z0,
				   Int_t minCls,
				   Int_t mintrks,
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::SetSkipTracks(Int_t n,const Int_t *skipped) 
{
//
// Mark the tracks not to be used in the vertex reconstruction.
// Tracks are identified by IlcESDtrack::GetID()
//
...
//---------------------------------------------------------------------------
void  IlcVertexerTracks::SetVtxStart(IlcESDVertex *vtx) 
{ 
//
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::StrLinVertexFinderMinDist(Int_t optUseWeights)
{
  IlcExternalTrackParam *track1;
  const Int_t knacc = (Int_t)fTrkArraySel.GetEntriesFast();
  IlcStrLine **linarray = new IlcStrLine* [knacc];
  for(Int_t i=0; i<knacc; i++){
    ...
    if(!TrackToPoint(track1,ri,wWi)) {
      ...
      IlcDebug(1,"WARNING: TrackToPoint failed");
    }
    Double_t wmat[9];
    Int_t iel=0;
    ...
    linarray[i] = new IlcStrLine(pos,sigmasq,wmat,dir);
  }
...
//---------------------------------------------------------------------------
IlcESDVertex IlcVertexerTracks::TrackletVertexFinder(const TClonesArray *lines, Int_t optUseWeights)
{
  // Calculate the point at minimum distance to prepared tracks (TClonesArray)
  const Int_t knacc = (Int_t)lines->GetEntriesFast();
  IlcStrLine** lines2 = new IlcStrLine* [knacc];
  for(Int_t i=0; i<knacc; i++){
    lines2[i]= (IlcStrLine*)lines->At(i);
  }
  IlcESDVertex vert = TrackletVertexFinder(lines2,knacc,optUseWeights); 
  delete [] lines2;
  return vert;
}

//---------------------------------------------------------------------------
IlcESDVertex IlcVertexerTracks::TrackletVertexFinder(IlcStrLine **lines, const Int_t knacc, Int_t optUseWeights)
{
  // Calculate the point at minimum distance to prepared tracks (array of IlcStrLine) 
  ...
  IlcESDVertex theVert(initPos,covmatrix,99999.,knacc);
...
//---------------------------------------------------------------------------
Bool_t IlcVertexerTracks::TrackToPoint(IlcExternalTrackParam *t,
				       TMatrixD &ri,TMatrixD &wWi,
				       Bool_t uUi3by3) const 
{
//
// Extract from the IlcExternalTrackParam the global coordinates ri and covariance matrix
  ...
  */
  ...
  if(!uUi3by3) {
    ...
    */
    ...
    if(fVert.GetNContributors()<1) IlcFatal("Vertex from finder is empty");
    ...
    IlcDebug(1,Form("=====> sqrtUi00 cm  %f",TMath::Sqrt(uUi(0,0))));
    uUi(0,1) = 0.;
    uUi(0,2) = 0.;
    uUi(1,0) = 0.;
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::TooFewTracks() 
{
//
...
//
  ...
  fCurrentVertex = new IlcESDVertex(pos,err);
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::VertexFinder(Int_t optUseWeights) 
{
  ...
  for(Int_t i=0; i<nacc; i++){
   ...
   //    IlcStrLine *line1 = new IlcStrLine();
   //    track1->ApproximateHelixWithLine(mindist,GetFieldkG(),line1);
    ...
    for(Int_t j=i+1; j<nacc; j++){
      ...
      IlcStrLine *line2 = new IlcStrLine(pos,dir); 
    //      IlcStrLine *line2 = new IlcStrLine();
...
//---------------------------------------------------------------------------
void IlcVertexerTracks::VertexFitter(Bool_t vfit, Bool_t chiCalc,Int_t useWeights) 
{
//
...
//
  ...
  IlcDebug(1,Form("--- VertexFitter(): start (%d,%d,%d)",vfit,chiCalc,useWeights));
  IlcDebug(1,Form(" Number of tracks in array: %d\n",nTrksSel));
  IlcDebug(1,Form(" Minimum # tracks required in fit: %d\n",fMinTracks));
  IlcDebug(1,Form(" Vertex position after finder: %f,%f,%f\n",initPos[0],initPos[1],initPos[2]));
  if(useConstraint) IlcDebug(1,Form(" This vertex will be used in fit: (%f+-%f,%f+-%f)\n",fNominalPos[0],TMath::Sqrt(fNominalCov[0]),fNominalPos[1],TMath::Sqrt(fNominalCov[2]))); 
  ...
  for(step=0; step<2; step++) {
    ...
    IlcDebug(2,Form("Step%d: inipos: %+f %+f %+f MinTr: %d, Sig2:%.2f)",step,initPos[0],initPos[1],initPos[2],fMinTracks,fMVSigma2));
    //
    ...
    for(k=0; k<nTrksSel; k++) {
      ...
      t = (IlcExternalTrackParam*)fTrkArraySel.At(k);
	...
	if (currBC!=IlcVTrack::kTOFBCNA && tBC!=IlcVTrack::kTOFBCNA && tBC!=currBC) continue;  // track does not match to current BCid
      }
      alpha = t->GetAlpha();
	...
	if (step==1) ((IlcESDtrack*)t)->SetBit(kBitUsed, wgh>0);
	...
	fMVWE2  += wgh*chi2iw;
      ...
      // add to total chi2
      if (fSelectOnTOFBunchCrossing && tBC!=IlcVTrack::kTOFBCNA && currBC<0) currBC = tBC;
      //
      chi2 += chi2i;
      TMatrixD wWiri(wWi,TMatrixD::kMult,ri); 
      sumWiri += wWiri;
  ...
  // store data in the vertex object
  if(fCurrentVertex) { delete fCurrentVertex; fCurrentVertex=0; }
  fCurrentVertex = new IlcESDVertex(position,covmatrix,chi2,nTrksUsed);
  fCurrentVertex->SetBC(currBC);
  fVert = *fCurrentVertex;  // RS
  IlcDebug(1," Vertex after fit:");
  IlcDebug(1,Form("xyz: %f %f %f; nc %d",fCurrentVertex->GetXv(),fCurrentVertex->GetYv(),fCurrentVertex->GetZv(),fCurrentVertex->GetNContributors()));
  IlcDebug(1,"--- VertexFitter(): finish\n");
 

  return;
}
//----------------------------------------------------------------------------
IlcESDVertex* IlcVertexerTracks::VertexForSelectedTracks(const TObjArray *trkArray,
							 UShort_t *id,
							 Bool_t optUseFitter,
							 Bool_t optPropagate,
...
{
//
// Return vertex from tracks (IlcExternalTrackParam) in array
//
  fCurrentVertex = 0;
  ...
  if(fCurrentVertex->GetNContributors()>0) {
    ...
    for(Int_t jj=0; jj<(Int_t)fTrkArraySel.GetEntriesFast(); jj++) {
      indices[jj] = fIdSel[jj];
      t = (IlcExternalTrackParam*)fTrkArraySel.At(jj);
	...
	if(TMath::Sqrt(fCurrentVertex->GetXv()*fCurrentVertex->GetXv()+fCurrentVertex->GetYv()*fCurrentVertex->GetYv())<3.) {
	  ...
	  IlcDebug(1,Form("Track %d propagated to found vertex",jj));
	} else {
	  IlcWarning("Found vertex outside beam pipe!");
	}
      }
    }
...
//----------------------------------------------------------------------------
IlcESDVertex* IlcVertexerTracks::VertexForSelectedESDTracks(TObjArray *trkArray,
							     Bool_t optUseFitter,
							    Bool_t optPropagate,
							    Bool_t optUseDiamondConstraint)
...
//
  ...
  IlcESDtrack *esdt = 0;
  for(Int_t i=0; i<nTrks; i++){
    esdt = (IlcESDtrack*)trkArray->At(i);
...
Bool_t IlcVertexerTracks::FindNextVertexMV()
{
  ...
  do {    
    fVert.SetBC(IlcVTrack::kTOFBCNA);
    VertexFitter(kTRUE,kFALSE,1);
    if (fVert.GetNContributors()<fMinTracks) {
      IlcDebug(3,Form("Failed in iteration %d: No Contributirs",iter)); 
      break;
    } // failed
    if (fMVWSum>0) fMVSigma2 = TMath::Max(fMVWE2/fMVWSum,kSigLimit);
    else {
      IlcDebug(3,Form("Failed %d, no weithgs",iter)); 
      iter = kMaxIter+1; 
      break;
    } // failed
    //
    ...
    IlcDebug(3,Form("It#%2d Vtx: %+f %+f %+f Dst:%f Sig: %f [%.2f/%.2f] SigRed:%f",iter,pos[0],pos[1],pos[2],TMath::Sqrt(dst2),fMVSigma2,fMVWE2,fMVWSum,sigRed));
    if ( (++iter<kMaxIter) && (sigRed<0 || sigRed<fMVMinSig2Red) && fMVSigma2>fMVMaxSigma2) {
      ...
      IlcDebug(3,Form("Pushed sigma2 to %f",fMVSigma2));
    }
  ...
  if (fVert.GetNContributors()<0 || iter>kMaxIter || fMVSigma2>fMVMaxSigma2) {
    ...
    for (int itr=0;itr<ntrk;itr++) {
      IlcExternalTrackParam* t = (IlcExternalTrackParam*)fTrkArraySel[itr];
    ...
    IlcDebug(3,Form("Added new vertex #%d NCont:%d XYZ: %f %f %f",nindices,nv,fCurrentVertex->GetX(),fCurrentVertex->GetY(),fCurrentVertex->GetZ()));
...
//______________________________________________________
void IlcVertexerTracks::FindVerticesMV()
{
  ...
  int izStart=0;
  IlcDebug(2,Form("%d seeds between %f and %f",nz,zmn+dz/2,zmx+dz/2));
  //
  ...
  while (runMore) {
    ...
    IlcDebug(3,Form("Wide search #%d Z= %f Sigma2=%f",cntWide,fNominalPos[2],fMVSig2Ini));
    if (FindNextVertexMV()) { // are there tracks left to consider?
      IlcESDVertex* vtLast = (IlcESDVertex*)fMVVertices->Last();
    ...
    for (;izStart<nz;izStart++) {
      ...
      IlcDebug(3,Form("Seed %d: Z= %f Sigma2=%f",izStart,zSeed,fMVSig2Ini));
      fVert.SetNContributors(-1);
      fVert.SetXYZ(fNominalPos);
      fVert.SetZv(zSeed);
      if (FindNextVertexMV()) { // are there tracks left to consider?
  ...
  IlcDebug(2,Form("Number of found vertices: %d",nvFound));
  if (nvFound<1) TooFewTracks();
  if (IlcLog::GetGlobalDebugLevel()>0) fMVVertices->Print();
  //
}

//______________________________________________________
void IlcVertexerTracks::AnalyzePileUp(IlcESDEvent* esdEv)
{
  ...
  for (int iv=0;iv<nFND;iv++) {
    ...
    for (int jv=iv+1;jv<nFND;jv++) {
      IlcESDVertex* fndJ = (IlcESDVertex*)fMVVertices->At(indCont[jv]);
    ...
    for (int jv0=0;jv0<ncomp;jv0++) {
      ...
      IlcESDVertex* fndJ = (IlcESDVertex*)fMVVertices->At(jv);
	...
	if (fndJ->GetBC()!=IlcVTrack::kTOFBCNA && fndI->GetBC()==IlcVTrack::kTOFBCNA) fndI->SetBC(fndJ->GetBC());
	//
	// leave the vertex only if both vertices have definit but different BC's, then this is not splitting.
	if ( (fndJ->GetBC()==fndI->GetBC()) || (fndJ->GetBC()==IlcVTrack::kTOFBCNA)) fndJ->SetNContributors(-fndJ->GetNContributors());
      }
    }
  }
  //
  ...
  for (int iv0=0;iv0<nFND;iv0++) {
    int iv = indCont[iv0];
    IlcESDVertex* fndI = (IlcESDVertex*)fMVVertices->At(iv);
    if (!fndI) continue;
    if (fndI->GetStatus()<1) {fMVVertices->RemoveAt(iv); delete fndI; continue;}
    if (primBC0<0  && fndI->GetBC()==0) primBC0 = iv;
    if (primNoBC<0 && fndI->GetBC()==IlcVTrack::kTOFBCNA)  primNoBC = iv;
  }
  //
  if (primBC0>=0) fCurrentVertex = (IlcESDVertex*)fMVVertices->At(primBC0);
  if (!fCurrentVertex && primNoBC>=0) fCurrentVertex = (IlcESDVertex*)fMVVertices->At(primNoBC);
  if (fCurrentVertex) fMVVertices->Remove(fCurrentVertex);
  else {  // all vertices have BC>0, no primary vertex
    fCurrentVertex = new IlcESDVertex();
    fCurrentVertex->SetNContributors(-3);
    fCurrentVertex->SetBC(IlcVTrack::kTOFBCNA);
  }
...
//______________________________________________________
void IlcVertexerTracks::FindAllVertices(Int_t nTrksOrig, 
  ...
  // sort points along Z
  IlcDebug(1,Form("Sort points along Z, used tracks %d",nTrksOrig));
  ...
  // Compute vertices
  IlcDebug(1,Form("Number of found clusters %d",nClusters+1));
  ...
  for(Int_t iClu=0; iClu<=nClusters; iClu++){
    ...
    cluTrackArr.Clear();
    IlcDebug(1,Form(" Vertex #%d tracks %d first tr %d  last track %d",iClu,nCluTracks,firstTr[iClu],lastTr[iClu]));
    Int_t nSelTr=0;
    for(Int_t iTr=firstTr[iClu]; iTr<=lastTr[iClu]; iTr++){
      IlcExternalTrackParam* t=(IlcExternalTrackParam*)trkArrayOrig->At(posOrig[iTr]);
      if(TMath::Abs(t->GetZ()-zTr[iTr])>0.00001){
	IlcError(Form("Clu %d Track %d zTrack=%f  zVec=%f\n",iClu,iTr,t->GetZ(),zTr[iTr]));
      }
      cluTrackArr.AddAt(t,nSelTr);
      idTrkClu[nSelTr]=idOrig[iTr];
      IlcDebug(1,Form("   Add track %d: id %d, z=%f",iTr,idOrig[iTr],zTr[iTr]));
      nSelTr++;
    }
    IlcESDVertex* vert=FindPrimaryVertex(&cluTrackArr,idTrkClu);
    IlcDebug(1,Form("Found vertex in z=%f with %d contributors",vert->GetZv(),
  ...
  IlcDebug(1,Form("Number of found vertices %d (%d)",nFoundVertices,fMVVertices->GetEntriesFast()));
  // if(maxPos>=0 && maxContr>0){
  //   IlcESDVertex* vtxMax=(IlcESDVertex*)fMVVertices->At(maxPos);
  //   if(fCurrentVertex) delete fCurrentVertex; 
  //   fCurrentVertex=new IlcESDVertex(*vtxMax);
  // }

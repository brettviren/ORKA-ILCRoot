#ifndef ILCESDCALOCLUSTER_H
#define ILCESDCALOCLUSTER_H
/* 2012-2013 Infrastructure for Large Collider Experiment.       *
 * All rights reserved.                                                   *
 * See cxx source for full Copyright notice                               */
/* $Id: IlcESDCaloCluster.h 50615 2011-07-16 23:19:19Z hristov $ */
/* $Log $ */
...
class IlcESDCaloCluster : public IlcVCluster 
{
  
 public:
  
  IlcESDCaloCluster();
  IlcESDCaloCluster(const IlcESDCaloCluster& clus);
  IlcESDCaloCluster & operator=(const IlcESDCaloCluster& source);
  virtual ~IlcESDCaloCluster();
  ...
  void SetPositionAt(Float_t pos, Int_t ipos) {if(ipos>=0 && ipos<3) fGlobalPos[ipos] = pos ; 
    else IlcInfo(Form("Bad index for position array, i = %d\n",ipos));}
  ...
  //for(Int_t i=0; i<IlcPID::kSPECIESN; ++i) pid[i]=fPID[i];}
  void SetPID  (const Float_t *pid) ;
  void SetPIDAt(Float_t p, Int_t i) {if(i>=0 && i<IlcPID::kSPECIESN) fPID[i] = p ; 
    else IlcInfo(Form("Bad index for PID array, i = %d \n",i));}
 ...
 protected:
  ...
  Double32_t   fPID[IlcPID::kSPECIESN]; //[0,1,8]"detector response  probabilities" (for the PID)
  ...
  ClassDef(IlcESDCaloCluster,11)  //ESDCaloCluster 

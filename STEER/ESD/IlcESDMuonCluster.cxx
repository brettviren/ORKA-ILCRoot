...
// $Id: IlcESDMuonCluster.cxx 55446 2012-03-30 16:00:07Z hristov $
...
/// \cond CLASSIMP
ClassImp(IlcESDMuonCluster)
/// \endcond

//_____________________________________________________________________________
IlcESDMuonCluster::IlcESDMuonCluster()
: TObject(),
  fCharge(0.),
  fChi2(0.),
  fPads(0x0),
...
//_____________________________________________________________________________
IlcESDMuonCluster::IlcESDMuonCluster (const IlcESDMuonCluster& cluster)
: TObject(cluster),
  ...
  fLabel(cluster.fLabel)
{
  ...
  if (cluster.fPads) {
    fPads = new TClonesArray("IlcESDMuonPad",cluster.fPads->GetEntriesFast());
    IlcESDMuonPad *pad = (IlcESDMuonPad*) cluster.fPads->First();
    while (pad) {
      new ((*fPads)[fPads->GetEntriesFast()]) IlcESDMuonPad(*pad);
      pad = (IlcESDMuonPad*) cluster.fPads->After(pad);
    }
  }
...
//_____________________________________________________________________________
IlcESDMuonCluster& IlcESDMuonCluster::operator=(const IlcESDMuonCluster& cluster)
{
  /// Equal operator
...
//__________________________________________________________________________
void IlcESDMuonCluster::Clear(Option_t* opt)
{
  /// Clear arrays
  if (opt && opt[0] == 'C') {
...
//_____________________________________________________________________________
void IlcESDMuonCluster::AddPadId(UInt_t padId)
{
...
//_____________________________________________________________________________
void IlcESDMuonCluster::SetPadsId(Int_t nPads, const UInt_t *padsId)
{
...
//_____________________________________________________________________________
void IlcESDMuonCluster::MovePadsToESD(IlcESDEvent &esd)
{
  /// move the pads to the new ESD structure
  if (!fPads) return;
  for (Int_t i = 0; i < fPads->GetEntriesFast(); i++) {
    IlcESDMuonPad *pad = static_cast<IlcESDMuonPad*>(fPads->UncheckedAt(i));
    IlcESDMuonPad *newPad = esd.NewMuonPad();
    *newPad = *pad;
...
//_____________________________________________________________________________
void IlcESDMuonCluster::Print(Option_t */*option*/) const
{

...
/* $Id: IlcESDv0.h 54081 2012-01-23 15:46:03Z shahoian $ */
...
class IlcESDV0Params;

class IlcESDv0 : public IlcVParticle {
public:
  IlcESDv0();
  IlcESDv0(const IlcExternalTrackParam &t1, Int_t i1,
           const IlcExternalTrackParam &t2, Int_t i2);

  IlcESDv0(const IlcESDv0& v0);
  virtual ~IlcESDv0();
  IlcESDv0& operator=(const IlcESDv0& v0);
...
// Start with IlcVParticle functions
  ...
  // Then extend the IlcVParticle functions
  ...
  const IlcExternalTrackParam *GetParamP() const {return &fParamP;}
  const IlcExternalTrackParam *GetParamN() const {return &fParamN;}


  void SetP(const IlcExternalTrackParam & paramp);
  void SetM(const IlcExternalTrackParam & paramd);
  ...
  //
  //
  static const IlcESDV0Params & GetParameterization(){return fgkParams;}
  void SetParamP(const IlcExternalTrackParam & paramP) {fParamP = paramP;}
  void SetParamN(const IlcExternalTrackParam & paramN) {fParamN = paramN;}
  ...
  //
protected:
  IlcExternalTrackParam fParamN;  // external parameters of negative particle
  IlcExternalTrackParam fParamP;  // external parameters of positive particle
  ...
  Double32_t   fDistNorm;   //normilczed  DCA
  ...
  static const IlcESDV0Params fgkParams;  //! resolution and likelihood parameterization  

private:

  ClassDef(IlcESDv0,5)      // ESD V0 vertex
};

inline 
void IlcESDv0::GetNPxPyPz(Double_t &px, Double_t &py, Double_t &pz) const {
px=fNmom[0]; py=fNmom[1]; pz=fNmom[2];
}

inline 
void IlcESDv0::GetPPxPyPz(Double_t &px, Double_t &py, Double_t &pz) const {
px=fPmom[0]; py=fPmom[1]; pz=fPmom[2];
}

inline
void IlcESDv0::SetDcaV0Daughters(Double_t rDcaV0Daughters){
  fDcaV0Daughters=rDcaV0Daughters;
}

inline 
void IlcESDv0::SetIndex(Int_t i, Int_t ind) {
  if(i==0)
    fNidx=ind;
  else
    fPidx=ind;

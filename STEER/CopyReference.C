...
void CopyReference () {
  ...
  // connect to gIlc object in input file
  const char *inFileName = "gilc.root";
  TFile *inFile = new TFile(inFileName, "READ");
  
  gIlc = (IlcRun*)inFile->Get("gIlc");
  gIlc->GetEvent(0);
  ...
  for (Int_t j=0; j<nDet; j++) {
	 ...
	 refArray[j] = new TClonesArray("IlcTrackReference", 100);
  ...
  for (Int_t i=0; i<nTracks; i++) {
	 ...
	 for (Int_t j=0; j<nDet; j++) {
		...
		Int_t lab, start;
		IlcTrackReference *ref = 0;
		refBranch[j]->GetEvent(i);
		Int_t nObj = refArray[j]->GetEntries();
		
		if (!nObj) continue;
		
		lab = ((IlcTrackReference*)refArray[j]->At(0))->GetTrack();
		start = 0;
		
		for (Int_t e=0; e<nObj-1; e++) {
		  
		  ref = (IlcTrackReference*)refArray[j]->At(e+1);
  ...
  for(Int_t j=0; j<nDet; j++) {
	 outArray[j] = new TClonesArray("IlcTrackReference", 200);
	 ...
	 for(Int_t j=0; j<nDet; j++) {
		...
		if (lutTrack[i*nDet+j] != -1) {
		  ...
		  for(Int_t k = lutStart[i*nDet+j], en = 0; k<lutStop[i*nDet+j]; k++ ) {		  
			 IlcTrackReference *ref = (IlcTrackReference*)refArray[j]->At(k);
			 new( (*(outArray[j]))[en++] ) IlcTrackReference(*ref);
		  }
		}
	 }

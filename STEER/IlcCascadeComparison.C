...
Int_t IlcCascadeComparison(Int_t code=3312, const Char_t *dir=".") {
  //code= 3312; //kXiMinus 
  //code=-3312; //kXiPlusBar
  //code= 3334; //kOmegaMinus
  //code=-3334; //kOmegaPlusBar
   gBenchmark->Start("IlcCascadeComparison");

   ::Info("IlcCascadeComparison.C","Doing comparison...");
   ...
   ::Info("IlcCascadeComparison.C","Marking good cascades (will take a while)...");
     if (GoodCascades(dir)) {
        ::Error("IlcCascadesComparison.C","Can't generate the reference file !");
        return 1;
     }
   }
   refFile=TFile::Open(fname,"old");
   if (!refFile || !refFile->IsOpen()) {
     ::Error("IlcCascadeComparison.C","Can't open the reference file !");
     return 1;
   }   
  
   TTree *csTree=(TTree*)refFile->Get("csTree");
   if (!csTree) {
     ::Error("IlcCascadeComparison.C","Can't get the reference tree !");
     return 2;
   }
   TBranch *pbranch=csTree->GetBranch("positive");
   if (!pbranch) {
     ::Error("IlcCascadeComparison.C","Can't get the positive daughter branch !");
     return 3;
   }
   TClonesArray dummy("IlcTrackReference",1000), *prefs=&dummy;
   pbranch->SetAddress(&prefs);

   TBranch *nbranch=csTree->GetBranch("negative");
   if (!nbranch) {
     ::Error("IlcCascadeComparison.C","Can't get the negative daughter branch !");
     return 4;
   }
   TClonesArray dumm("IlcTrackReference",1000), *nrefs=&dumm;
   nbranch->SetAddress(&nrefs);

   TBranch *bbranch=csTree->GetBranch("bachelor");
   if (!nbranch) {
     ::Error("IlcCascadeComparison.C","Can't get the bachelor branch !");
     return 4;
   }
   TClonesArray dum("IlcTrackReference",1000), *brefs=&dum;
   ...
   if ((!ef)||(!ef->IsOpen())) {
      sprintf(fname,"%s/IlcESDcascade.root",dir);
      ef=TFile::Open(fname);
      if ((!ef)||(!ef->IsOpen())) {
         ::Error("IlcCascadeComparison.C","Can't open IlcESDcascade.root !");
         return 5;
      }
   }
   ...
   if (!esdTree) {
      ::Error("IlcCascadeComparison.C", "no ESD tree found");
      return 6;
   }
   ...
   while (esdTree->GetEvent(e)) {
      ...
      Int_t i;
      for (i=0; i<nentr; i++) {
          IlcESDcascade *cascade=event->GetCascade(i);

          Int_t nidx=TMath::Abs(cascade->GetNindex());
          Int_t pidx=TMath::Abs(cascade->GetPindex());
          Int_t bidx=TMath::Abs(cascade->GetBindex());

          IlcESDtrack *ntrack=event->GetTrack(nidx);
          IlcESDtrack *ptrack=event->GetTrack(pidx);
          IlcESDtrack *btrack=event->GetTrack(bidx);
          ...
          IlcTrackReference *nref=0, *pref=0, *bref=0;
          Int_t j;
          for (j=0; j<ngood; j++) {
              bref=(IlcTrackReference*)brefs->UncheckedAt(j);
              nref=(IlcTrackReference*)nrefs->UncheckedAt(j);
              pref=(IlcTrackReference*)prefs->UncheckedAt(j);
              if (bref->Label() == blab)
              if (nref->Label() == nlab)
      ...
      for (i=0; i<ngood; i++) {
         IlcTrackReference *bref=(IlcTrackReference*)brefs->UncheckedAt(i);
         IlcTrackReference *nref=(IlcTrackReference*)nrefs->UncheckedAt(i);
         IlcTrackReference *pref=(IlcTrackReference*)prefs->UncheckedAt(i);
   ...
   TFile fc("IlcCascadeComparison.root","RECREATE");
   c1->Write();
   c2->Write();
   fc.Close();

   gBenchmark->Stop("IlcCascadeComparison");
   gBenchmark->Show("IlcCascadeComparison");
...
Int_t GoodCascades(const Char_t *dir) {
   ...
   TClonesArray dm("IlcTrackReference",1000), *itsRefs=&dm;
   ...
   TClonesArray dummy("IlcTrackReference",1000), *nrefs=&dummy;
   TClonesArray dumm("IlcTrackReference",1000), *prefs=&dumm;
   TClonesArray dum("IlcTrackReference",1000), *brefs=&dum;
   ...
   for (Int_t e=0; e<nev; e++) {
	 ...
	 //cerr<<np<<'\r';
         ...
         for (i=0; i<nk; i++) {
	     bref=(IlcTrackReference*)itsRefs->UncheckedAt(i);
         ...
         for (i=0; i<nk; i++) {
	     nref=(IlcTrackReference*)itsRefs->UncheckedAt(i);
             if (nref->Label()==nlab) break;
         }
         if (i==nk) continue;
         for (i=0; i<nk; i++) {
	     pref=(IlcTrackReference*)itsRefs->UncheckedAt(i);
             if (pref->Label()==plab) break;
         }
         if (i==nk) continue;
         ...
         new((*nrefs)[nc]) IlcTrackReference(*nref);
         new((*prefs)[nc]) IlcTrackReference(*pref);
         new((*brefs)[nc]) IlcTrackReference(*bref);

#ifndef ILCCFGRIDSPARSE_H
#define ILCCFGRIDSPARSE_H
//--------------------------------------------------------------------//
//                                                                    //
// IlcCFGridSparse.cxx Class                                          //
...
class IlcCFGridSparse : public IlcCFFrame
{
 public:
  IlcCFGridSparse();
  IlcCFGridSparse(const Char_t* name, const Char_t* title);
  IlcCFGridSparse(const Char_t* name, const Char_t* title, Int_t nVarIn, const Int_t* nBinIn);
  IlcCFGridSparse(const IlcCFGridSparse& c);
  virtual ~IlcCFGridSparse();
  IlcCFGridSparse& operator=(const IlcCFGridSparse& c);
  virtual void Copy(TObject& c) const;

  // IlcCFFrame functions
  ...
  virtual IlcCFGridSparse* MakeSlice(Int_t nVars, const Int_t* vars,
  ...
  //basic operations
  virtual void     SumW2();
  virtual void     Add(const IlcCFGridSparse* aGrid, Double_t c=1.);
  virtual void     Add(const IlcCFGridSparse* aGrid1 ,const IlcCFGridSparse* aGrid2, Double_t c1=1.,Double_t c2=1.);
  virtual void     Multiply(const IlcCFGridSparse* aGrid, Double_t c=1.);
  virtual void     Multiply(const IlcCFGridSparse* aGrid1,const IlcCFGridSparse* aGrid2, Double_t c1=1.,Double_t c2=1.);
  virtual void     Divide(const IlcCFGridSparse* aGrid, Double_t c=1.);
  virtual void     Divide(const IlcCFGridSparse* aGrid1, const IlcCFGridSparse* aGrid2, Double_t c1=1., Double_t c2=1.,Option_t *option=0);
  ...
  /*  FUNCTIONS TO REMOVE   */
  virtual IlcCFGridSparse* Project(Int_t nVars, const Int_t* vars, const Double_t* varMin, const Double_t* varMax, Bool_t useBins=0) const 
 ...
 protected:
  ...
  ClassDef(IlcCFGridSparse,3);
};


//inline functions :

inline Long_t IlcCFGridSparse::GetNBinsTotal() const {
  Long_t n=1;
...
inline void IlcCFGridSparse::PrintNBins() const
{
  //
  // printing the array containing the # of bins  
  //
  for (Int_t i=0;i<GetNVar();i++) {
    IlcInfo(Form("bins in axis %i are: %i",i,fData->GetAxis(i)->GetNbins()));
  }
} 

inline void IlcCFGridSparse::PrintBinLimits() const
{
  //
  // printing the bin limits for each variable  
  //
  for (Int_t iVar=0; iVar<GetNVar(); iVar++) {
    IlcInfo(Form("variable %d :",iVar));
    const Int_t nBins = GetNBins(iVar) ;
    Double_t *array = new Double_t[nBins+1];
    GetBinLimits(iVar,array);
    for (Int_t iBin=0; iBin<nBins; iBin++) {
      IlcInfo(Form("    bin limit index %i is: %e",iBin,array[iBin]));
    }
    delete [] array ;
  }
}

inline void IlcCFGridSparse::GetBinLimits(Int_t ivar, Double_t * array) const {
...
inline Int_t* IlcCFGridSparse::GetNBins() const {
...
inline Double_t* IlcCFGridSparse::GetBinLimits(Int_t ivar) const {
  Double_t * binLimits = new Double_t[GetNBins(ivar)+1] ;
  GetBinLimits(ivar,binLimits);
  return binLimits;
}

inline Int_t IlcCFGridSparse::GetVar(const Char_t* title) const {
  TString str(title);
  for (Int_t iVar=0; iVar<GetNVar(); iVar++) {
    if (!str.CompareTo(GetVarTitle(iVar))) return iVar;
  }
  IlcError("Variable not found");
  return -1;
}

inline void IlcCFGridSparse::GetProjectionName (TString& s, Int_t var0, Int_t var1, Int_t var2) const {
...
inline void IlcCFGridSparse::GetProjectionTitle(TString& s, Int_t var0, Int_t var1, Int_t var2) const {

...
void testCFContainers(){
  ...
  // simple example macros for usage of a N-dim container (IlcCFContainer)
  // handling a set of grids to accumulate data at different 
  // selection steps, & derive efficiency (this is stored in IlcCFEffGrid) 
  ...
  IlcCFContainer *cont = new IlcCFContainer("cont","example of  container",nstep,nvar,iBin);
...
// Read the  container from file
   ...
   IlcCFContainer *data = (IlcCFContainer*) (file->Get("cont"));
...
//   cmc->Print("data.gif");
  ...
  IlcCFEffGrid *eff = new IlcCFEffGrid("eff"," The efficiency",*data);
  ...
  IlcCFDataGrid *corrdata = new IlcCFDataGrid("corrdata","corrected data",*data);

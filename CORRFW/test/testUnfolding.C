...
void testUnfolding() {
  ...
  // get the essential
  IlcCFDataGrid *measured    = (IlcCFDataGrid*) GetMeasuredSpectrum();
  IlcCFDataGrid *generated   = (IlcCFDataGrid*) GetGeneratedSpectrum();
  IlcCFEffGrid  *efficiency  = (IlcCFEffGrid*)  GetEfficiency();
  THnSparse     *response    = (THnSparse*)     GetResponseMatrix();
  
  // create a guessed "a priori" distribution using binning of MC
  THnSparse* guessed = CreateGuessed(((IlcCFGridSparse*)generated->GetData())->GetGrid()) ;
  //----
  ...
  IlcCFUnfolding unfolding("unfolding","",2,response,efficiency->GetGrid(),((IlcCFGridSparse*)measured->GetData())->GetGrid()/*,guessed*/);
...
TObject* GetMeasuredSpectrum() {
  ...
  IlcCFContainer* c = (IlcCFContainer*)f->Get("container");
  IlcCFDataGrid* data = new IlcCFDataGrid("data","",*c);
  data->SetMeasured(1);
  return data;
}
...
TObject* GetEfficiency() {
  TFile * f = TFile::Open("test/output.root","read");
  IlcCFContainer* c = (IlcCFContainer*)f->Get("container");
  IlcCFEffGrid* eff = new IlcCFEffGrid("eff","",*c);

#ifndef ILCCFCONTAINER_H
#define ILCCFCONTAINER_H

/* $Id: IlcCFContainer.h 45914 2010-12-09 11:18:45Z rvernet $ */
...
class IlcCFContainer : public IlcCFFrame
{
 public:
  IlcCFContainer();
  IlcCFContainer(const Char_t* name, const Char_t* title,const Int_t nSelStep, const Int_t nVarIn, const Int_t* nBinIn);
  IlcCFContainer(const IlcCFContainer& c);
  IlcCFContainer& operator=(const IlcCFContainer& corr);
  virtual void Copy(TObject& c) const;

  virtual ~IlcCFContainer();
  ...
  //basic operations
  virtual void     Add(const IlcCFContainer* aContainerToAdd, Double_t c=1.);
  virtual Long64_t Merge(TCollection* list);

  virtual TH1* Project (Int_t istep, Int_t ivar1, Int_t ivar2=-1 ,Int_t ivar3=-1) const;
  virtual IlcCFContainer* MakeSlice(Int_t nVars, const Int_t* vars, const Double_t* varMin=0x0, const Double_t* varMax=0x0, Bool_t useBins=0) const ;
  virtual IlcCFContainer* MakeSlice(Int_t nStep, const Int_t* steps, 
  ...
  virtual void  SetGrid(Int_t step, IlcCFGridSparse* grid) {if (fGrid[step]) delete fGrid[step]; fGrid[step]=grid;}
  virtual IlcCFGridSparse * GetGrid(Int_t istep) const {return fGrid[istep];};
 ...
 private:
  ...
  IlcCFGridSparse **fGrid;//[fNStep]
  
  ClassDef(IlcCFContainer,5);
};

inline void IlcCFContainer::SetBinLimits(Int_t ivar, const Double_t* array) {
  for (Int_t iStep=0; iStep<GetNStep(); iStep++) {
    fGrid[iStep]->SetBinLimits(ivar,array);
  }
}

inline void IlcCFContainer::SetBinLimits(Int_t ivar, Double_t min, Double_t max) {
  for (Int_t iStep=0; iStep<GetNStep(); iStep++) {
    fGrid[iStep]->SetBinLimits(ivar,min,max);
  }
}

inline void IlcCFContainer::SetVarTitle(Int_t ivar, const Char_t* title) {
  for (Int_t iStep=0; iStep<fNStep; iStep++) {
    GetAxis(ivar,iStep)->SetTitle(title);
  }
}

inline void IlcCFContainer::SetStepTitle(Int_t istep, const Char_t* title) {
  fGrid[istep]->SetTitle(title);
}

inline Int_t IlcCFContainer::GetStep(const Char_t* title) const {
  TString str(title);
  for (Int_t iStep=0; iStep<fNStep; iStep++) {
    if (!str.CompareTo(GetStepTitle(iStep))) return iStep;
  }
  IlcError("Step not found");
  return -1;
}

inline Int_t IlcCFContainer::GetVar(const Char_t* title) const {
  return fGrid[0]->GetVar(title);
}

inline void IlcCFContainer::SetBinLabel(Int_t iVar, Int_t iBin, const Char_t* label) {
  for (Int_t iStep=0; iStep<GetNStep(); iStep++) GetAxis(iVar,iStep)->SetBinLabel(iBin,label);
}

inline void  IlcCFContainer::Scale(Double_t factor) const {
  Double_t fact[2] = {factor,0} ;
  for (Int_t iStep=0; iStep<fNStep; iStep++) fGrid[iStep]->Scale(fact);
}

inline void IlcCFContainer::SetBinContent(Int_t* bin, Int_t step, Double_t value) {
...
inline void IlcCFContainer::SetBinError(Int_t* bin, Int_t step, Double_t value) {

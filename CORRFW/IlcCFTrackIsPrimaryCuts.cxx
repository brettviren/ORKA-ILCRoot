...
// The class IlcCFTrackIsPrimaryCut is designed to select reconstructed tracks
...
// IlcAnalysisCuts and is a part of the Correction Framework.
...
// added in the Analysis Framwork's class IlcAnalysisFilter and applied by
// the filter via a loop.
//
// author: I. Kraus (Ingrid.Kraus@cern.ch)
// idea taken form
// IlcESDtrackCuts writte by Jan Fiete Grosse-Oetringhaus and
// IlcRsnDaughterCut class written by A. Pulvirenti.
...
ClassImp(IlcCFTrackIsPrimaryCuts)

//__________________________________________________________________________________
IlcCFTrackIsPrimaryCuts::IlcCFTrackIsPrimaryCuts() :
  IlcCFCutBase(),
  fEvt(0x0),
  fUseSPDvertex(0),
  fUseTPCvertex(0),
...
//__________________________________________________________________________________
IlcCFTrackIsPrimaryCuts::IlcCFTrackIsPrimaryCuts(Char_t* name, Char_t* title) :
...
//__________________________________________________________________________________
IlcCFTrackIsPrimaryCuts::IlcCFTrackIsPrimaryCuts(const IlcCFTrackIsPrimaryCuts& c) :
  IlcCFCutBase(c),
  fEvt(c.fEvt),
  ...
  fhBinLimSigmaDcaZ(c.fhBinLimSigmaDcaZ)
{
  ...
  // copy constructor
  //
  ((IlcCFTrackIsPrimaryCuts &) c).Copy(*this);
}
//__________________________________________________________________________________
IlcCFTrackIsPrimaryCuts& IlcCFTrackIsPrimaryCuts::operator=(const IlcCFTrackIsPrimaryCuts& c)
{
  //
  // Assignment operator
  //
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::Initialise()
{
  //
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::Copy(TObject &c) const
{
  //
  // Copy function
  //
  IlcCFTrackIsPrimaryCuts& target = (IlcCFTrackIsPrimaryCuts &) c;
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::SetRecEventInfo(const TObject* evt) {
  //
  // Sets pointer to event information (IlcESDEvent or IlcAODEvent)
  //
  if (!evt) {
    IlcError("Pointer to IlcVEvent !");
    return;
  }
  TString className(evt->ClassName());
  if (! (className.CompareTo("IlcESDEvent")==0 || className.CompareTo("IlcAODEvent")==0)) {
    IlcError("argument must point to an IlcESDEvent or IlcAODEvent !");
    return ;
  }
  fEvt = (IlcVEvent*) evt;
}
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::UseSPDvertex(Bool_t b) {
  fUseSPDvertex = b;
  if(fUseTPCvertex && fUseSPDvertex) {
	fUseSPDvertex = kFALSE;
	IlcError("SPD and TPC vertex chosen. TPC vertex is preferred.");
  }
}
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::UseTPCvertex(Bool_t b) {
  fUseTPCvertex = b;
  if(fUseTPCvertex) fUseSPDvertex = kFALSE;
}
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::GetDCA(IlcESDtrack* esdTrack)
{
  if (!esdTrack) return;
  ...
  if( fUseSPDvertex) {
	if (!fEvt) return;
	IlcESDEvent * evt = 0x0 ; 
	evt = dynamic_cast<IlcESDEvent*>(fEvt);
	if (!evt) {
	  IlcError("event not found");
	  return;
	}
	const IlcESDVertex *vtx = evt->GetVertex();
  ...
  if(!fUseSPDvertex && !fUseTPCvertex)
	fDCA[5] = IlcESDtrackCuts::GetSigmaToVertex(esdTrack);
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::GetDCA(IlcAODTrack* aodTrack)
{
  if (!aodTrack) return;
  ...
  if (p[0]==-999. || p[1]==-999. ||  p[2]==-999.) {
    IlcError("dca info not available !");
  ...
  IlcAODEvent * evt = 0x0;
  evt = dynamic_cast<IlcAODEvent*>(fEvt);
  if (!evt) return;

  // primary vertex is the "best": tracks, SPD or TPC vertex
  IlcAODVertex * primaryVertex = evt->GetVertex(0);
  ...
  if (!fAbsDCAToVertex) {
	IlcError("resolution of impact parameter not available, use absolute dca cut instead !");
	if (fDCA[2] > 0) fDCA[0] = fDCA[0]/fDCA[2]; // normalised impact parameter xy
	if (fDCA[3] > 0) fDCA[1] = fDCA[1]/fDCA[3]; // normalised impact parameter z
  }
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::SelectionBitMap(TObject* obj)
{
  //
  ...
  if (!obj) return;
  if (!obj->InheritsFrom("IlcVParticle")) {
    IlcError("object must derived from IlcVParticle !");
    return;
  }
  
  IlcESDtrack * esdTrack = dynamic_cast<IlcESDtrack*>(obj);
  IlcAODTrack * aodTrack = dynamic_cast<IlcAODTrack*>(obj);

  if (!(esdTrack || aodTrack)) {
    IlcError("object must be an ESDtrack or an AODtrack !");
    return;
  }
  ...
  if (esdTrack) isESDTrack = strcmp(obj->ClassName(),"IlcESDtrack") == 0 ? kTRUE : kFALSE ;
  if (aodTrack) isAODTrack = strcmp(obj->ClassName(),"IlcAODTrack") == 0 ? kTRUE : kFALSE ;
  ...
  if (isAODTrack) {
    if (fAODType==IlcAODTrack::kUndef || fAODType == aodTrack->GetType()) {
      fBitmap->SetBitNumber(iCutBit,kTRUE);
    }
  }
...
//__________________________________________________________________________________
Bool_t IlcCFTrackIsPrimaryCuts::IsSelected(TObject* obj) {
  //
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::SetHistogramBins(Int_t index, Int_t nbins, Double_t *bins)
{
  //
  // variable bin size
  //
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::SetHistogramBins(Int_t index, Int_t nbins, Double_t xmin, Double_t xmax)
{
  //
  // fixed bin size
  //
...
//__________________________________________________________________________________
 void IlcCFTrackIsPrimaryCuts::DefineHistograms() {
  //
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::FillHistograms(TObject* obj, Bool_t f)
{
  //
  ...
  if (!obj) return;
  Bool_t isESDTrack = strcmp(obj->ClassName(),"IlcESDtrack") == 0 ? kTRUE : kFALSE ;
  Bool_t isAODTrack = strcmp(obj->ClassName(),"IlcAODTrack") == 0 ? kTRUE : kFALSE ;

  IlcESDtrack * esdTrack = 0x0 ;
  IlcAODTrack * aodTrack = 0x0 ; 
  if (isESDTrack) esdTrack = dynamic_cast<IlcESDtrack*>(obj);
  if (isAODTrack) aodTrack = dynamic_cast<IlcAODTrack*>(obj);
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::SaveHistograms(const Char_t* dir) {
  //
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::DrawHistograms()
{
  //
...
//__________________________________________________________________________________
void IlcCFTrackIsPrimaryCuts::AddQAHistograms(TList *qaList) {
  //

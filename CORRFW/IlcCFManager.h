#ifndef ILCCFMANAGER_H
#define ILCCFMANAGER_H
/**************************************************************************
...
class IlcCFManager : public TNamed 
{
 public :
  IlcCFManager() ;
  IlcCFManager(Char_t* name, Char_t* title) ;
  IlcCFManager(const IlcCFManager& c) ;
  IlcCFManager& operator=(const IlcCFManager& c) ;
  virtual ~IlcCFManager();
  ...
  virtual void SetEventContainer(IlcCFContainer* c) {
    fEvtContainer=c; 
    SetNStepEvent(c->GetNStep());
    IlcWarning(Form("Please dont forget to set the cut list (event empty) for the %d event-selection step requested",fNStepEvt));
  }
  
  //pass the pointer to the correction container
  virtual void SetParticleContainer(IlcCFContainer* c) {
    fPartContainer=c; 
    SetNStepParticle(c->GetNStep());
    IlcWarning(Form("Please dont forget to set the cut list (even empty) for the %d particle-selection step requested",fNStepPart));
  }
  ...
  virtual IlcCFContainer* GetEventContainer() const {return fEvtContainer;} ; 

  // pointer to the Particle-level correction container
  virtual IlcCFContainer* GetParticleContainer() const {return fPartContainer;} ; 
...
//pointer to the particle-level cut list for particle selection step isel
  ...
  virtual void SetEventInfo(TObject*) const {IlcError("DEPRECATED !! -> use SetMCEventInfo or SetRecEventInfo instead");}
 ...
 private:
  ...
  //the correction grid
  IlcCFContainer *fEvtContainer; //ptr to Evt-Level correction container
  //the correction grid
  IlcCFContainer *fPartContainer; //ptr to Particle-level correction container

...
// an IlcVEvent, but cuts have a true meaning only for IlcESD(AOD)Event 
// type objects.
...
#include "IlcLog.h"
#include "IlcVEvent.h"
#include "IlcCFEventClassCuts.h"
ClassImp(IlcCFEventClassCuts) 
//____________________________________________________________________
IlcCFEventClassCuts::IlcCFEventClassCuts() : 
  IlcCFCutBase(),
  fTriggerType(0),
  fTriggerAND(kFALSE),
...
//____________________________________________________________________
IlcCFEventClassCuts::IlcCFEventClassCuts(Char_t* name, Char_t* title) : 
...
IlcCFEventClassCuts::IlcCFEventClassCuts(const IlcCFEventClassCuts& c) : 
  IlcCFCutBase(c),
...
//_____________________________________________________________________________
IlcCFEventClassCuts& IlcCFEventClassCuts::operator=(const IlcCFEventClassCuts& c){
  //
  // Assignment operator
  //
  if (this != &c) {
...
//_____________________________________________________________________________
void IlcCFEventClassCuts::Initialise()
{
  //
  //initialization
  //
...
//____________________________________________________________________
Bool_t IlcCFEventClassCuts::IsSelected(TObject* obj) {
  //
...
//____________________________________________________________________
void IlcCFEventClassCuts::SetHistogramBins(Int_t index, Int_t nbins, Double_t *bins)
{
  //
  //setting x-axis bin limits of QA histogram fhQA[index] 
  // 
  for(Int_t i=0;i<kNStepQA;i++){
    if(!fhQA[index][i]){IlcWarning("non-existing histogram!");
    return;
    }
...
//____________________________________________________________________
void IlcCFEventClassCuts::SetHistogramBins(Int_t index, Int_t nbins, Double_t xmin, Double_t xmax)
{
  //
...
//____________________________________________________________________
void IlcCFEventClassCuts::SelectionBitMap(TObject* obj) {
  //
...
//_____________________________________________________________________________
Bool_t IlcCFEventClassCuts::IsTriggered(IlcVEvent* ev, TriggerType trigger) {
  //
  //look at the Trigger mask in current event
  TBits *triggerBitMap=new TBits(0);
...
//_____________________________________________________________________________
void IlcCFEventClassCuts::TriggerBitMap(IlcVEvent* ev, TBits *bitmapT ) {
  //
...
//_____________________________________________________________________________
 void IlcCFEventClassCuts::DefineHistograms() {
  //
  ...
  if(!fIsQAOn) {
    IlcInfo(Form("No QA histos requested, Please first set the QA flag on!"));
    return;
  }  
...
//_____________________________________________________________________________
void IlcCFEventClassCuts::FillHistograms(TObject* obj, Bool_t afterCuts)
{
  //
...
//_____________________________________________________________________________
void IlcCFEventClassCuts::AddQAHistograms(TList *qaList) {
  //

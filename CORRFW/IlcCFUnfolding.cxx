...
// found in IlcCFUnfolding::CalculateCorrelatedErrors()                //
...
ClassImp(IlcCFUnfolding)

//______________________________________________________________

IlcCFUnfolding::IlcCFUnfolding() :
  TNamed(),
  fResponseOrig(0x0),
  fPriorOrig(0x0),
  fEfficiencyOrig(0x0),
...
IlcCFUnfolding::IlcCFUnfolding(const Char_t* name, const Char_t* title, const Int_t nVar, 
  ...
  fRandomSeed(randomSeed)
{
  ...
  if (!prior) CreateFlatPrior(); // if no prior distribution declared, simply use a flat distribution
    ...
    if (fPrior->GetNdimensions() != fNVariables) 
      IlcFatal(Form("The prior matrix should have %d dimensions, and it has actually %d",fNVariables,fPrior->GetNdimensions()));
  }

  if (fEfficiency->GetNdimensions() != fNVariables) 
    IlcFatal(Form("The efficiency matrix should have %d dimensions, and it has actually %d",fNVariables,fEfficiency->GetNdimensions()));
  if (fMeasured->GetNdimensions() != fNVariables) 
    IlcFatal(Form("The measured matrix should have %d dimensions, and it has actually %d",fNVariables,fMeasured->GetNdimensions()));
  if (fResponse->GetNdimensions() != 2*fNVariables) 
    IlcFatal(Form("The response matrix should have %d dimensions, and it has actually %d",2*fNVariables,fResponse->GetNdimensions()));
  

  for (Int_t iVar=0; iVar<fNVariables; iVar++) {
    IlcInfo(Form("prior      matrix has %d bins in dimension %d",fPrior     ->GetAxis(iVar)->GetNbins(),iVar));
    IlcInfo(Form("efficiency matrix has %d bins in dimension %d",fEfficiency->GetAxis(iVar)->GetNbins(),iVar));
    IlcInfo(Form("measured   matrix has %d bins in dimension %d",fMeasured  ->GetAxis(iVar)->GetNbins(),iVar));
  }
...
void IlcCFUnfolding::CreateEstMeasured() {
  //
...
void IlcCFUnfolding::CreateInvResponse() {
  //
...
void IlcCFUnfolding::Unfold() {
  ...
  for (iIterBayes=0; iIterBayes<fMaxNumIterations; iIterBayes++) { // bayes iterations
    ...
    IlcDebug(0,Form("convergence at iteration %d is %e",iIterBayes,convergence));

    if (fMaxConvergence>0. && convergence<fMaxConvergence && fNCalcCorrErrors == 0) {
      fNRandomIterations = iIterBayes;
      IlcDebug(0,Form("convergence is met at iteration %d",iIterBayes));
      break;
    }

    if (fUseSmoothing) {
      if (Smooth()) {
	IlcError("Couldn't smooth the unfolded spectrum!!");
  ...
  //
  //for (Long_t iBin=0; iBin<fUnfoldedFinal->GetNbins(); iBin++) IlcDebug(2,Form("%e\n",fUnfoldedFinal->GetBinError(iBin)));
  //
...
void IlcCFUnfolding::CreateUnfolded() {
  //
...
void IlcCFUnfolding::CalculateCorrelatedErrors() {
  ...
  for (Long_t iBin=0; iBin<fUnfoldedFinal->GetNbins(); iBin++) {
    ...
    //IlcDebug(2,Form("filling error %e\n",sigma));
...
//______________________________________________________________
void IlcCFUnfolding::CreateRandomizedDist() {
  //
...
//______________________________________________________________
void IlcCFUnfolding::FillDeltaUnfoldedProfile() {
  //
...
void IlcCFUnfolding::GetCoordinates() {
  //
...
void IlcCFUnfolding::CreateConditional() {
  //
...
Int_t IlcCFUnfolding::GetDOF() {
  ...
  IlcDebug(0,Form("Number of degrees of freedom = %d",nDOF));
  return nDOF;
}

//______________________________________________________________

Double_t IlcCFUnfolding::GetChi2() {
  //
...
Double_t IlcCFUnfolding::GetConvergence() {
  //
  ...
  for (Long_t iBin=0; iBin < fPrior->GetNbins(); iBin++) {
    ...
    if (priorValue > 0.)
      ...
      IlcWarning(Form("priorValue = %f. Adding 0 to convergence criterion.",priorValue)); 
  }
  return convergence;
}
...
void IlcCFUnfolding::SetMaxConvergencePerDOF(Double_t val) {
  //
  ...
  IlcInfo(Form("MaxConvergence = %e. Number of degrees of freedom = %d",fMaxConvergence,nDOF));
}

//______________________________________________________________

Short_t IlcCFUnfolding::Smooth() {
  //
  ...
  if (fSmoothFunction) {
    IlcDebug(2,Form("Smoothing spectrum with fit function %p",fSmoothFunction));
...
Short_t IlcCFUnfolding::SmoothUsingNeighbours(THnSparse* hist) {
  //
...
Short_t IlcCFUnfolding::SmoothUsingFunction() {
  //
  // Fits the unfolded spectrum using the function fSmoothFunction
  //

  IlcDebug(0,Form("Smooth function is a %s with option \"%s\" and has %d parameters : ",fSmoothFunction->ClassName(),fSmoothOption,fSmoothFunction->GetNpar()));

  for (Int_t iPar=0; iPar<fSmoothFunction->GetNpar(); iPar++) IlcDebug(0,Form("par[%d]=%e",iPar,fSmoothFunction->GetParameter(iPar)));
  ...
  default: IlcFatal(Form("Cannot handle such fit in %d dimensions",fNVariables)) ; return 1;
  }

  if (fitResult != 0) {
    IlcWarning(Form("Fit failed with status %d, stopping the loop",fitResult));
    return 1;
  }
...
void IlcCFUnfolding::CreateFlatPrior() {
  //
  // Creates a flat prior distribution
  // 

  IlcInfo("Creating a flat a priori distribution");

...
// class IlcCFParticleGenCuts implementation
// Using this class a user may define selections relative to 
// MC particle (IlcMCParticle) using generation-level information.
////////////////////////////////////////////////////////////////////////////
...
ClassImp(IlcCFParticleGenCuts)

//______________________________
IlcCFParticleGenCuts::IlcCFParticleGenCuts() : 
  IlcCFCutBase(),
  fIsAODMC(0),
  fMCInfo(0x0),
  fRequireIsCharged(0),
...
//______________________________
IlcCFParticleGenCuts::IlcCFParticleGenCuts(const Char_t* name, const Char_t* title) : 
...
//______________________________
IlcCFParticleGenCuts::IlcCFParticleGenCuts(const IlcCFParticleGenCuts& c) : 
  IlcCFCutBase(c),
  fIsAODMC(c.fIsAODMC),
  fMCInfo(c.fMCInfo),
...
//______________________________
IlcCFParticleGenCuts& IlcCFParticleGenCuts::operator=(const IlcCFParticleGenCuts& c)
{
  //
  // Assignment operator
  //
...
//______________________________
Bool_t IlcCFParticleGenCuts::IsSelected(TObject* obj) {
  //
  ...
  if (!fIsAODMC) SelectionBitMap((IlcMCParticle*)   obj);
  else           SelectionBitMap((IlcAODMCParticle*)obj);
...
void IlcCFParticleGenCuts::SelectionBitMap(IlcMCParticle* mcPart)
{
  //
  ...
  IlcStack*  stack = ((IlcMCEvent*)fMCInfo)->Stack();
...
//__________________________________________________________________________________
void IlcCFParticleGenCuts::SelectionBitMap(IlcAODMCParticle* mcPart)
{
  //
  // test if the track passes the single cuts
  // and store the information in a bitmap
  ...
  IlcAODEvent* aod = dynamic_cast<IlcAODEvent*>(fMCInfo);
  
  if (!aod) {
    IlcError("AOD event casting failed");
    return;
  }
  
  TClonesArray* mcArray = dynamic_cast<TClonesArray*>(aod->FindListObject(IlcAODMCParticle::StdBranchName()));
  if (!mcArray) {
    IlcError("array casting failed");
    return;
  }
  IlcAODMCParticle* daughter = 0x0 ;

  if (mcPart->GetDaughter(0)>0) {
    daughter = dynamic_cast<IlcAODMCParticle*>(mcArray->At(mcPart->GetDaughter(0)));
    if (!daughter) {
      IlcError("daughter casting failed");
      return;
    }
  ...
  if ( fDecayChannel ) {
    ...
    if (goodDecay) {
      ...
      // first try
      for (Int_t iDaughter = 0 ; iDaughter<nDaughters; iDaughter++) {
	IlcAODMCParticle* daug = dynamic_cast<IlcAODMCParticle*>(mcArray->At(mcPart->GetDaughter(0)+iDaughter));
	if (!daug) {
	  IlcError("daughter: casting failed");
	  continue;
	}
	...
	goodDecay = kTRUE ;
	for (Int_t iDaughter = 0 ; iDaughter<nDaughters; iDaughter++) {
	  IlcAODMCParticle* daug = dynamic_cast<IlcAODMCParticle*>(mcArray->At(mcPart->GetDaughter(1)-iDaughter));
	  if (!daug) {IlcFatal(""); return;}
	  if (daug->GetPdgCode() != fDecayChannel->DaughterPdgCode(iDaughter)) {goodDecay = kFALSE; break;}
	}
      }
    }
...
//__________________________________________________________________________________
void IlcCFParticleGenCuts::FillHistograms(TObject* /*obj*/, Bool_t afterCuts)
{
  //
...
//__________________________________________________________________________________
void IlcCFParticleGenCuts::AddQAHistograms(TList *qaList) {
  //
...
//__________________________________________________________________________________
void IlcCFParticleGenCuts::DefineHistograms() {
  //
...
//______________________________
Bool_t IlcCFParticleGenCuts::IsCharged(IlcVParticle *mcPart) {
  //
...
//______________________________
Bool_t IlcCFParticleGenCuts::IsPrimary(IlcMCParticle *mcPart) {
  //
...
//______________________________
Bool_t IlcCFParticleGenCuts::IsPrimary(IlcAODMCParticle *mcPart) {
  //
  //check if particle is primary (standard definition)
  //
...
//______________________________
Bool_t IlcCFParticleGenCuts::IsPrimaryCharged(IlcVParticle *mcPart) {
  //
  //check if a charged particle is primary (standard definition)
  //

  if (!fIsAODMC) {
    if (!IsPrimary((IlcMCParticle*)mcPart) || !IsCharged(mcPart)) return kFALSE ;
  }
  else {
    if (!IsPrimary((IlcAODMCParticle*)mcPart) || !IsCharged(mcPart)) return kFALSE ;
  }
  return kTRUE;
}
//______________________________
Bool_t IlcCFParticleGenCuts::IsA(IlcMCParticle *mcPart, Int_t pdg, Bool_t abs) {
  //
...
//______________________________
Bool_t IlcCFParticleGenCuts::IsA(IlcAODMCParticle *mcPart, Int_t pdg, Bool_t abs) {
  //
  //Check on the pdg code of the MC particle. if abs=kTRUE then check on the 
  //absolute value. 
  //
...
//______________________________
void IlcCFParticleGenCuts::SetMCEventInfo(const TObject* mcEvent) {
  //
  // Sets pointer to MC event information (IlcMCEvent)
  //

  if (!mcEvent) {
    IlcError("Pointer to MC Event is null !");
    return;
  }
  
  TString className(mcEvent->ClassName());
  if (className.CompareTo("IlcMCEvent") != 0 && className.CompareTo("IlcAODEvent") != 0) {
    IlcError("argument must point to an IlcMCEvent or an IlcAODEvent !");
    return ;
  }

  fMCInfo = (IlcVEvent*)mcEvent ;
}

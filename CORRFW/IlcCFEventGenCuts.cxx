...
#include "IlcCFEventGenCuts.h"

ClassImp(IlcCFEventGenCuts) 
//____________________________________________________________________
IlcCFEventGenCuts::IlcCFEventGenCuts() : 
  IlcCFCutBase(),
  fMBProcType(-1),
  fNTracksMin(-1),
  fNTracksMax(100000),
...
//____________________________________________________________________
IlcCFEventGenCuts::IlcCFEventGenCuts(Char_t* name, Char_t* title) : 
...
//____________________________________________________________________
IlcCFEventGenCuts::IlcCFEventGenCuts(const IlcCFEventGenCuts& c) : 
  IlcCFCutBase(c),
...
//____________________________________________________________________
IlcCFEventGenCuts& IlcCFEventGenCuts::operator=(const IlcCFEventGenCuts& c)
{
  //
  // Assignment operator
  //
...
//____________________________________________________________________
Bool_t IlcCFEventGenCuts::IsSelected(TObject* obj) {
  //
...
//____________________________________________________________________
void IlcCFEventGenCuts::SelectionBitMap(TObject* obj){
  //
  //cut on the MB process type, the number of charged and neutral 
  //tracks and on the event vertex. So far specific to IlcMCEvents
  //

  //Check if the requested cuts are passed and return a bitmap
  for(Int_t j=0;j<kNCuts;j++)fBitMap->SetBitNumber(j,kFALSE);
  IlcMCEvent* ev = dynamic_cast<IlcMCEvent *>(obj);
  if ( !ev ) return;
  IlcGenEventHeader*genHeader = ev->GenEventHeader();  
  ...
  if(fMBProcType>-1){
    ...
    if(process==-1){
      IlcInfo(Form(" not a pythia event, not checking on the process type"));
    }else{
...
Bool_t IlcCFEventGenCuts::IsMBProcType(IlcMCEvent *ev, PrType iproc){
  //
...
Int_t IlcCFEventGenCuts::ProcType(IlcGenEventHeader *genHeader) {
  ...
  if (!pythiaGenHeader) {
    ...
    for (Int_t i=0; i<headerList->GetEntries(); i++) {
      pythiaGenHeader = dynamic_cast<IlcGenPythiaEventHeader*>(headerList->At(i));
      if (pythiaGenHeader)
        break;
    }

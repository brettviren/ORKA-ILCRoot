...
ClassImp(IlcGenCosmicsParam)

//-----------------------------------------------------------------------------
IlcGenCosmicsParam::IlcGenCosmicsParam():
IlcGenerator(),
fParamMI(kFALSE),
fParamACORDE(kFALSE),
fParamDataTPC(kTRUE),
...
//-----------------------------------------------------------------------------
void IlcGenCosmicsParam::Generate()
{
  //
  // Generate one muon
  //
  ...
  while (npart<fNpart) {
    ...
    if(fParamACORDE) { // extracted from IlcGenACORDE events
	...
	if(fParamMI) { // parametrization by M.Ivanov of LEP cosmics data
	  ...
	  IlcFatal("Parametrization not set: use SetParamDataTPC, SetParamMI, or SetParamACORDE");
	}
...
//-----------------------------------------------------------------------------
void IlcGenCosmicsParam::Init()
{
  // 
  // Initialisation, check consistency of selected ranges
  //
  if(TestBit(kPtRange)) 
    IlcFatal("You cannot set the pt range for this generator! Only momentum range");
  Double_t pmin=8.; // fParamACORDE
  if(fParamDataTPC) pmin=0.5;
  if(fPMin<pmin) { 
    fPMin=pmin; 
    if(TestBit(kMomentumRange)) 
      IlcWarning(Form("Minimum momentum cannot be < %f GeV/c",pmin)); 
  }
  if(fMaxAngleWRTVertical<0.) 
    IlcFatal("You must use SetMaxAngleWRTVertical() instead of SetThetaRange(), SetPhiRange()");

  printf("************ IlcGenCosmicsParam ****************\n");
...
//-----------------------------------------------------------------------------
Bool_t IlcGenCosmicsParam::IntersectCylinder(Float_t r,Float_t z,Int_t pdg,
...
//-----------------------------------------------------------------------------
Bool_t IlcGenCosmicsParam::IntersectACORDE(Int_t pdg,
					   Float_t o[3],Float_t p[3]) const
{
  //
...
//-----------------------------------------------------------------------------
Bool_t IlcGenCosmicsParam::IntersectBottomScintillator(Int_t pdg,
					   Float_t o[3],Float_t p[3]) const
{
  //
  // Intersection between muon and ACORDE (very rough)

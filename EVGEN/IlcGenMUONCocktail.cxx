...
/* $Id: IlcGenMUONCocktail.cxx 51126 2011-08-17 13:37:49Z cvetan $ */

//
// Classe to create the MUON coktail for physics in the Ilc muon spectrometer
...
ClassImp(IlcGenMUONCocktail)
  
  
//________________________________________________________________________
IlcGenMUONCocktail::IlcGenMUONCocktail()
    :IlcGenCocktail(),
     fFastGlauber (0x0),
     fTotalRate(0),  
     fMuonMultiplicity(1),
...
void IlcGenMUONCocktail::Init()
{
  ...
  fFastGlauber = IlcFastGlauber::Instance();
  ...
  IlcInfo(Form("<b>=%4.2f, <Ncoll>=%5.1f and and <Npart>=%5.1f",b, fNumberOfCollisions, fNumberOfParticipants));
  ...
  IlcInfo(Form("Shadowing for charmonium and beautonium production are %4.2f and %4.2f respectively",charmoniumshadowing,beautoniumshadowing));
  IlcInfo(Form("Shadowing for charm and beauty production are %4.2f and %4.2f respectively",charmshadowing,beautyshadowing));
  ...
  IlcGenParam * genjpsi = new IlcGenParam(1, IlcGenMUONlib::kJpsi, "CDF scaled", "Jpsi");
  ...
  IlcInfo(Form("Jpsi production cross-section in pp with shadowing %5.3g barns",sigmajpsi));
  IlcInfo(Form("Jpsi production probability per collisions in acceptance via the muonic channel %5.3g",ratiojpsi));
  // Generation in the kinematical limits of IlcGenMUONCocktail
 ...
 // Using CFD scaled distribution (see http://clrwww.in2p3.fr/DIMUON2004/talks/sgrigoryan.pdf )
  IlcGenParam * genpsiP = new IlcGenParam(1, IlcGenMUONlib::kPsiP, "CDF scaled", "PsiP");
  ...
  IlcInfo(Form("Psi prime production cross-section in pp with shadowing %5.3g barns",sigmapsiP));
  IlcInfo(Form("Psi prime production probability per collisions in acceptance via the muonic channel %5.3g",ratiopsiP));
  // Generation in the kinematical limits of IlcGenMUONCocktail
  ...
  // Using CFD scaled distribution (see http://clrwww.in2p3.fr/DIMUON2004/talks/sgrigoryan.pdf )
  IlcGenParam * genupsilon = new IlcGenParam(1, IlcGenMUONlib::kUpsilon, "CDF scaled", "Upsilon");  
  ...
  IlcInfo(Form("Upsilon 1S production cross-section in pp with shadowing %5.3g barns",sigmaupsilon));
  IlcInfo(Form("Upsilon 1S production probability per collisions in acceptance via the muonic channel %5.3g",ratioupsilon));
  ...
  // Using CFD scaled distribution (see http://clrwww.in2p3.fr/DIMUON2004/talks/sgrigoryan.pdf )
  IlcGenParam * genupsilonP = new IlcGenParam(1, IlcGenMUONlib::kUpsilonP, "CDF Scaled", "UpsilonP");  
  ...
  IlcInfo(Form("Upsilon 2S production cross-section in pp with shadowing %5.3g barns",sigmaupsilonP));
  IlcInfo(Form("Upsilon 2S production probability per collisions in acceptance via the muonic channel %5.3g",ratioupsilonP));
  ...
  // Using CFD scaled distribution (see http://clrwww.in2p3.fr/DIMUON2004/talks/sgrigoryan.pdf )
  IlcGenParam * genupsilonPP = new IlcGenParam(1, IlcGenMUONlib::kUpsilonPP, "CDF Scaled", "UpsilonPP");  
  ...
  IlcInfo(Form("Upsilon 3S production cross-section in pp with shadowing %5.3g barns",sigmaupsilonPP));
  IlcInfo(Form("Upsilon 3S production probability per collisions in acceptance via the muonic channel %5.3g",ratioupsilonPP));
...
// Generating non-correlated Charm Physics 
  IlcGenParam * gencharm = new IlcGenParam(1, IlcGenMUONlib::kCharm, "pp", "Charm");  
  ...
  IlcInfo(Form("Charm production cross-section in pp with shadowing %5.3g barns",sigmacharm));
  IlcInfo(Form("Charm production probability per collisions in acceptance via the semi-muonic channel %5.3g",ratiocharm));
...
// Generating non-correlated Beauty Physics 
  IlcGenParam * genbeauty = new IlcGenParam(1, IlcGenMUONlib::kBeauty, "pp", "Beauty");  
  ...
  IlcInfo(Form("Beauty production cross-section in pp with shadowing %5.3g barns",sigmabeauty));
  IlcInfo(Form("Beauty production probability per collisions in acceptance via the semi-muonic channel %5.3g",ratiobeauty));
  ...
  if(fHadronicMuons) { 
    ...
    IlcGenParam * genpion = new IlcGenParam(1, IlcGenMUONlib::kPion, "default", "Pion");  
    ...
    IlcInfo(Form("Pseudo-Pion production cross-section in pp with shadowing %5.3g barns",sigmapion));
    IlcInfo(Form("Pion production probability per collisions in acceptance via the muonic channel %5.3g",ratiopion));
    ...
    IlcGenParam * genkaon = new IlcGenParam(1, IlcGenMUONlib::kKaon, "default", "Kaon");  
    ...
    Double_t sigmakaon     = 2.40e-4;   // Valerie presentation Clermont-16-jan-2004 and Ilc-int-2002-06 
    ...
    IlcInfo(Form("Pseudo-kaon production cross-section in pp with shadowing %5.3g barns",sigmakaon));
    IlcInfo(Form("Kaon production probability per collisions in acceptance via the muonic channel %5.3g",ratiokaon));
...
//_________________________________________________________________________
void IlcGenMUONCocktail::Generate()
{
  //
// Generate event 
    TIter next(fEntries);
...
//  Generate the vertex position used by all generators    
    ...
    while(!primordialTrigger) {
		...
		while((entry = (IlcGenCocktailEntry*)next())) {
			...
			if ( (npart = gRandom->Poisson(entry->Rate())) >0 ) {
				...
				//  { IlcInfo(Form("This generator %s is finally not generated. This is option for hadronic muons.",gen->GetName() ) ); }
				// else {
		...
		for(iPart=0; iPart<partArray->GetEntriesFast(); iPart++){      
			...
			if ( (TMath::Abs(gIlc->GetMCApp()->Particle(iPart)->GetPdgCode())==13)  &&
	     		(gIlc->GetMCApp()->Particle(iPart)->Theta()*180./TMath::Pi()>fMuonThetaMinCut) &&
	     		(gIlc->GetMCApp()->Particle(iPart)->Theta()*180./TMath::Pi()<fMuonThetaMaxCut) &&
	     		(gIlc->GetMCApp()->Particle(iPart)->Pt()>fMuonPtCut)                             ) { 
	  				gIlc->GetMCApp()->Particle(iPart)->SetProductionVertex(fVertex.At(0), fVertex.At(1), fVertex.At(2), fTime);   
	  				GoodMuons.AddLast(gIlc->GetMCApp()->Particle(iPart));
					numberOfMuons++;
			}			
		}
    ...
    IlcDebug(5,Form("Generated Events are %d and Succeeded Events are %d",fNGenerated,fNSucceded));
}

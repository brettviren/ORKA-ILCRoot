...
/* $Id: IlcGenMUONCocktailpp.cxx 57853 2012-07-17 08:51:13Z hristov $ */
...
// July 07:added heavy quark production from IlcGenCorrHF and heavy quark 
...
// usage: see example of Config in $ILC_ROOT/prod/LHC09a10/Config.C
//------------------------
// 04/2010:
// - CMS energy passed via parameter
// i.e. gener->SetCMSEnergy(IlcGenMUONCocktailpp::kCMS07TeV) in Config.C 
...
// in the Config.C with one IlcGenMUONCocktailpp per bin. These generators
// included in a IlcGenCocktail together with an event generator (e.g. Hijing)
// providing the underlying event and collision centrality. The bin number n
// passed via IlcGenMUONCocktailpp::SetCentralityBin(n).
...
// IlcGenMUONCocktailpp::SetCMSEnergy(int CMSEnergyCode), CMSEnergy codes are
// defined in IlcGenMUONCocktailpp.h.
...
IlcGenMUONCocktailpp* MuonCocktail() {
  IlcGenMUONCocktailpp *mc = new IlcGenMUONCocktailpp();
  ....................................
  return mc;
}
*/
...
ClassImp(IlcGenMUONCocktailpp)  
  
//________________________________________________________________________
IlcGenMUONCocktailpp::IlcGenMUONCocktailpp()
    :IlcGenCocktail(),
     fDecayer(0),
...
// -ccbar & bbbar: NLO pQCD computations - http://www-ilc.gsi.de/ana/MNR/results.html
...
//_________________________________________________________________________
void IlcGenMUONCocktailpp::SetCMSEnergy(CMSEnergyCode cmsEnergy) 
{
...
//_________________________________________________________________________
void IlcGenMUONCocktailpp::SetResPolarization(Double_t JpsiPol, Double_t PsiPPol, Double_t UpsPol,
...
// setter for resonances polarization   
   if (strcmp(PolFrame,"kColSop")==0){
       ...
       IlcInfo(Form("The polarization frame is not valid"));
       IlcInfo(Form("No polarization will be set"));
       fJpsiPol=0.;
       fPsiPPol=0.;
       fUpsPol=0.;
       fUpsPPol=0.;
...
//_________________________________________________________________________
void IlcGenMUONCocktailpp::CreateCocktail()
{
...
// (only in case of use of IlcDecayerPolarized)
...
// Cross sections scaled to manage the statistics
    ...
    IlcInfo(Form("the parametrised resonances uses the decay mode %d",fDecayModeResonance));

// Create and add resonances to the generator
    IlcGenParam * genjpsi=0;
    IlcGenParam * genchic1=0;
    IlcGenParam * genchic2=0;
    IlcGenParam * genpsiP=0;
    IlcGenParam * genupsilon=0;
    IlcGenParam * genupsilonP=0;
    IlcGenParam * genupsilonPP=0;
	...
	IlcError("Initialisation failed, wrong cmsEnergy");
	return;
    }
    genjpsi = new IlcGenParam(1, IlcGenMUONlib::kJpsi, tname, "Jpsi");
    genchic1 = new IlcGenParam(1, IlcGenMUONlib::kChic1, tname, "Chic1");
    genchic2 = new IlcGenParam(1, IlcGenMUONlib::kChic2,  tname, "Chic2");
    genpsiP   = new IlcGenParam(1, IlcGenMUONlib::kPsiP,   tname, "PsiP");
    genupsilon = new IlcGenParam(1, IlcGenMUONlib::kUpsilon, tname, "Upsilon");
    genupsilonP = new IlcGenParam(1, IlcGenMUONlib::kUpsilonP, tname, "UpsilonP");
    genupsilonPP = new IlcGenParam(1, IlcGenMUONlib::kUpsilonPP, tname, "UpsilonPP");
...
//------------------------------------------------------------------
// Generator of charm
    IlcGenCorrHF *gencharm = new IlcGenCorrHF(1, 4, cmsEnergy);
    ...
    gencharm->Init();
    if (!fSigmaSilent) {
      IlcInfo(Form("c-cbar prod. cross-section in pp %5.3g b",sigmaccbar));
      IlcInfo(Form("c-cbar prod. probability per collision in acceptance %5.3g",ratioccbar));
    }
    AddGenerator(gencharm,"CorrHFCharm",ratioccbar);
//------------------------------------------------------------------
// Generator of beauty
    IlcGenCorrHF *genbeauty = new IlcGenCorrHF(1, 5, cmsEnergy);
    ...
    genbeauty->Init();
    if (!fSigmaSilent) {
      IlcInfo(Form("b-bbar prod. cross-section in pp  %5.3g b",sigmabbbar));
      IlcInfo(Form("b-bbar prod. probability per collision in acceptance %5.3g",ratiobbbar));
    }
...
//-------------------------------------------------------------------
void IlcGenMUONCocktailpp::AddReso2Generator(Char_t* nameReso, 
					     IlcGenParam* const genReso,
					     Double_t sigmaReso,
					     Double_t polReso)
{
...
// kinematics limit of the MUONCocktail
    ...
    if (!fSigmaSilent) {
      IlcInfo(Form("%s prod. cross-section in pp %5.3g b",nameReso,sigmaReso));
      IlcInfo(Form("%s prod. probability per collision in acceptance %5.3g",nameReso,ratioReso));
    }
// second step: generation in selected kinematical range
    ...
    genReso->Init(); // generation in selected kinematical range
     ...
     if(TMath::Abs(polReso) > 1.e-30){
      IlcInfo(Form("******Setting polarized decayer for %s''",nameReso));
      if(fPolFrame==0) {
        decReso = new IlcDecayerPolarized(polReso,IlcDecayerPolarized::kColSop,IlcDecayerPolarized::kMuon);
        IlcInfo(Form("******Reference frame: %s, alpha: %f","Collins-Soper",polReso));
	  }
      if(fPolFrame==1) {
        decReso = new IlcDecayerPolarized(polReso,IlcDecayerPolarized::kHelicity,IlcDecayerPolarized::kMuon);
        IlcInfo(Form("******Reference frame: %s, alpha: %f","Helicity",polReso));
	  }
      if (decReso) {
...
//-------------------------------------------------------------------
void IlcGenMUONCocktailpp::Init()
{
    // Initialisation
    TIter next(fEntries);
...
//_________________________________________________________________________
void IlcGenMUONCocktailpp::Generate()
{
// Generate event 
    TIter next(fEntries);
    ...
    fHeader = new IlcGenCocktailEventHeader("MUON Cocktail Header");
...
//     IlcInfo(Form("Generated Events are %d and Succeeded Events are %d",fNGenerated,fNSucceded));

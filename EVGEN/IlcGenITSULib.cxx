...
ClassImp(IlcGenITSULib)

 typedef Double_t (*GenFunc)   (const Double_t*,  const Double_t*);
 typedef Int_t    (*GenFuncIp) (TRandom *);


Double_t IlcGenITSULib::PtLcDist( const Double_t *x, const Double_t *)
{
 //     pt-distribution
...
Double_t IlcGenITSULib::PtLbDist( const Double_t *x, const Double_t *)
{
...
GenFunc IlcGenITSULib::GetPt(Int_t iPID, const char * sForm) const
{
 // Return pointer to Pt parameterisation
 IlcDebug(1,Form("PID: %i, form: %s \n",iPID,sForm));	
 TString type(sForm);
 GenFunc func;
 ...
 if(type=="FLAT") {
  ...
  switch(TMath::Abs(iPID)) {
   ...
   default : IlcError(Form("Unknown particle type: %i, Pt dist is 0",iPID));      func=0;
  } 
 }else {
  IlcError(Form("Unknown Pt distribution %s. Pt distribution is set to 0 ",sForm));
  func=0;
 }
...
GenFuncIp IlcGenITSULib::GetIp(Int_t iPID, const char *sForm) const
{
 IlcDebug(1,Form(" %i - %s",iPID,sForm));
 // Return pointer to particle type parameterisation
 GenFuncIp id;

 if(TMath::Abs(iPID) != kLc && TMath::Abs(iPID) != kLb && TMath::Abs(iPID) != kXi_c && TMath::Abs(iPID) != kB && TMath::Abs(iPID)!=kDplus && TMath::Abs(iPID)!=kDs) {
  IlcError(Form("Unknown PID: %i, form: %s, return 0",iPID,sForm));   //////////	
  id = 0;
 } else {
  switch (iPID){
   ...
   default  : IlcFatal(Form("Unknown particle type: %i",iPID));  id=0;
  }

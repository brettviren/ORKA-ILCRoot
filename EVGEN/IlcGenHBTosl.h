#ifndef ILCGENHBTOSL_H
#define ILCGENHBTOSL_H
...
class IlcGenHBTosl: public IlcGenerator
{
 public:
   IlcGenHBTosl();
   IlcGenHBTosl(Int_t n,Int_t pid = 211);
   IlcGenHBTosl(const IlcGenHBTosl& hbt);
   virtual ~IlcGenHBTosl();

   void      Init();
   void      Generate();
   
   void      SetGenerator(IlcGenerator* gen){fGenerator = gen;}
 ...
 protected:
   ...
   void Mix(IlcStack* stack, TH3D* numerator, TH3D* numerator2);
   ...
   void SetTrack(TParticle* p, Int_t& ntr, IlcStack* stack) const ;
   
   IlcStack* RotateStack();
   void SwapGeneratingHistograms();
   void     TestCoarseSignal();
   
   Bool_t CheckParticle(TParticle* p, TParticle* aupair,IlcStack* stack);
   void Copy(TObject&) const;
   IlcGenHBTosl & operator=(const IlcGenHBTosl & rhs);
 private:
  ...
  IlcGenerator* fGenerator;//input generator

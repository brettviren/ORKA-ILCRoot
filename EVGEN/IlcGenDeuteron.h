#ifndef ILCGENDEUTERON_H
#define ILCGENDEUTERON_H
...
class IlcGenDeuteron: public IlcGenerator
{

 public:

	IlcGenDeuteron(Int_t sign=1, Double_t pmax=0.3, Double_t rmax=2.1, Int_t cluster=0 );
	virtual ~IlcGenDeuteron();
 ...
 private:
	...
	IlcGenDeuteron(const IlcGenDeuteron &other);
	IlcGenDeuteron& operator=(const IlcGenDeuteron &other);
 ...
 private:
	...
	IlcStack* fCurStack;  //! current event stack
	Int_t fNtrk;          //! number of the stored track
	
	ClassDef(IlcGenDeuteron,1)
};

#endif // ILCGENDEUTERON_H

...
ClassImp(IlcGenMUONLMR)


IlcGenMUONLMR::IlcGenMUONLMR () : IlcGenMC(), fNMuMin(2), fGenSingleProc(-1), fCosTheta(0), fRhoLineShape(0), fHMultMu(0), fHNProc(0) { 
  //
  // default constructor 
  //
  // initialize pt and y distributions according to a fit to 
  // Pythia simulation at sqrt(s) = 7 TeV
  printf ("Using IlcGenMUONLMR as generator\n");
  ...
  Double_t ptparam[7][3] = { {1,0.427,2.52}, // pions from Pythia
			     ...
			     {1,1.2,2.5},    // rho+omega from ILC muon  
			     {1,1.2,2.5},    // rho+omega from ILC muon  
			     {1,1.03,2.5},   // phi from ILC muon  
  ...
  for (Int_t i=0; i<fgkNpart; i++) { 
    ...
    fPt[i] = new TF1(fptname[i],IlcGenMUONLMR::PtDistr,0,20,3);
    fPt[i]->SetParameters(ptparam[i][0], ptparam[i][1], ptparam[i][2]);  
    fY[i] = new TF1(fyname[i],IlcGenMUONLMR::YDistr,-10,10,3);
...
void IlcGenMUONLMR::FinishRun(){ 
  ...
  for (Int_t ibin=1; ibin <= nbins; ibin++) printf ("ibin = %d nEvProc = %g\n",
    ...
    TFile *fout = new TFile("IlcGenMUONLMR_histos.root","recreate"); 
  fHMultMu->Write(); 
  fHNProc->Write(); 
  fout->Close(); 
}
...
Double_t IlcGenMUONLMR::YDistr(const Double_t *px, const Double_t *par){ 
...
Double_t IlcGenMUONLMR::PtDistr(const Double_t *px, const Double_t *par){
...
//       printf ("Multiplicity for process %d is %d\n",iproc,mult[iproc]); 
  ...
  IlcGenEventHeader* header = new IlcGenEventHeader("LMR");
...
void IlcGenMUONLMR::Decay2Body(const TParticle *mother){ 
...
void IlcGenMUONLMR::DecayPiK(TParticle *mother, Bool_t &hasDecayed){ 
  ...
  if (TMath::Abs(mother->GetPdgCode())== 211) {
      ...
      IlcWarning("Mother particle is not a pion or kaon \n");
  }
...
void IlcGenMUONLMR::DalitzDecay(const TParticle *mother){
  //
  ...
  if (TMath::Abs(mother->GetPdgCode())== 221) {
    ...
    // eta'
      ...
      IlcWarning("Mother particle is not a eta, eta' or omega \n");
  }
  ...
  if (psquare<0) {
    printf("Error in IlcGenMUONLMR::DalitzDecay: sqrt of psquare = %f put to 0\n",psquare); 
    psquare = 0;
  }
  ...
  if (psquare3<0) {
    printf("Error in IlcGenMUONLMR::DalitzDecay: sqrt of psquare3 = %f put to 0\n",psquare3); 
    psquare3 = 0;
  }
...
Double_t IlcGenMUONLMR::FormFactor(Double_t q2, Int_t decay){ 
...
Double_t IlcGenMUONLMR::RhoLineShapeNew(const Double_t *x, const Double_t* /*para*/){

...
/* $Id: IlcGenHalo.cxx 54888 2012-02-29 19:49:22Z hristov $ */
...
IlcGenHalo::IlcGenHalo(Int_t npart)
    :IlcGenerator(npart),
     fFile(0),
     fFileName(0),
     fSide(1),
...
//    Read file with gas pressure values
    char *name = 0;
    if (fRunPeriod < 5) {
	name = gSystem->ExpandPathName("$(ILC_ROOT)/LHC/gasPressure.dat" );
	fGPASize = 21;
	...
	name = gSystem->ExpandPathName("$(ILC_ROOT)/LHC/pressure_2003_startup.dat");
	fGPASize = 18853;
	fG1 = new Float_t[fGPASize];
	fZ1 = new Float_t[fGPASize];
    } else if (fRunPeriod ==6) {
	name = gSystem->ExpandPathName("$(ILC_ROOT)/LHC/pressure_2003_conditioned.dat");
	fGPASize = 12719;
	fG1 = new Float_t[fGPASize];
	fZ1 = new Float_t[fGPASize];
    } else {
...
//
  ...
  SetHighWaterMark(nt);
  IlcGenEventHeader* header = new IlcGenEventHeader("HALO");
...
Float_t IlcGenHalo::GasPressureWeight(Float_t zPrimary)
{
//
...
Int_t IlcGenHalo::ReadNextParticle()
{

...
/* $Id: IlcGenGeVSim.cxx 51126 2011-08-17 13:37:49Z cvetan $ */
...
ClassImp(IlcGenGeVSim)

//////////////////////////////////////////////////////////////////////////////////

IlcGenGeVSim::IlcGenGeVSim() : 
  IlcGenerator(-1),
  fModel(0),
  fPsi(0),
  fIsMultTotal(kTRUE),
...
IlcGenGeVSim::IlcGenGeVSim(Float_t psi, Bool_t isMultTotal) 
    : IlcGenerator(-1),
      fModel(0),
      fPsi(psi),
      ...
      fPartTypes(0) 
 {
  ...
  if (psi < 0 || psi > 360 ) 
    Error ("IlcGenGeVSim", "Reaction plane angle ( %13.3f )out of range [0..360]", psi);
...
Bool_t IlcGenGeVSim::CheckPtYPhi(Float_t pt, Float_t y, Float_t phi) const {
  //
...
Bool_t IlcGenGeVSim::CheckAcceptance(Float_t p[3]) {
  //
  // private function used by Generate()
  //
...
void IlcGenGeVSim::AddParticleType(IlcGeVSimParticle *part) {
  //
  // Adds new type of particles.
  // 
  // Parameters are defeined in IlcGeVSimParticle object
...
void IlcGenGeVSim::SetMultTotal(Bool_t isTotal) {
  //
  //
  //
...
Float_t IlcGenGeVSim::FindScaler(Int_t paramId, Int_t pdg) {
  //
  // private function
  ...
  for (i=0; i<2; i++) {
    for (j=0; j<2; j++) {
      ...
      if (form != 0) {
	if (j == 0) scaler *= form->Eval(gIlc->GetEvNumber()); 
	if (j == 1) {
	  form->SetParameter(0, gIlc->GetEvNumber());
...
void IlcGenGeVSim::DetermineReactionPlane() {
  //
  // private function used by Generate()
  //
  ...
  if (form) fPsi = form->Eval(gIlc->GetEvNumber()) * TMath::Pi() / 180;
...
Float_t IlcGenGeVSim::GetdNdYToTotal() {
  //
...
void IlcGenGeVSim::SetFormula(Int_t pdg) {
  //
  // Private function used by Generate() 
  //
...
void IlcGenGeVSim:: AdjustFormula() {
  //
  // Private Function
...
void IlcGenGeVSim::GetRandomPtY(Double_t &pt, Double_t &y) {
  //
  // Private function used by Generate()
  //
...
void IlcGenGeVSim::Generate() {
  //
  // Standard IlcGenerator function
  ...
  for (nType = 0; nType < fPartTypes->GetEntries(); nType++) {
    ...
    partType = (IlcGeVSimParticle *)fPartTypes->At(nType);
  ...
  IlcGenGeVSimEventHeader *header = new IlcGenGeVSimEventHeader("GeVSim header");

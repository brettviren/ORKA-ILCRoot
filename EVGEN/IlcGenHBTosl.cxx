...
/* $Id: IlcGenHBTosl.cxx 46822 2011-01-29 17:58:12Z morsch $ */
...
/***********************************************************/
ClassImp(IlcGenHBTosl)

IlcGenHBTosl::IlcGenHBTosl():
 IlcGenerator(),
...
IlcGenHBTosl::IlcGenHBTosl(Int_t n, Int_t pid):
 IlcGenerator(n),
 fQCoarseBackground(0x0),
 fQCoarseSignal(0x0),
 fQSignal(0x0),
...
IlcGenHBTosl::IlcGenHBTosl(const IlcGenHBTosl & hbt):
 IlcGenerator(-1),
 fQCoarseBackground(0x0),
 fQCoarseSignal(0x0),
 fQSignal(0x0),
...
void IlcGenHBTosl::Init()
{
  //Initializes generator
  if (fGenerator == 0x0)
   { 
   
     IlcGenHIJINGpara* bkggen = new IlcGenHIJINGpara(fNpart*4);
     fGenerator = bkggen;

/*     
     IlcGenGeVSim * gevsim = new IlcGenGeVSim(0.0);
     IlcGeVSimParticle* kplus = new IlcGeVSimParticle(fPID,1,fNpart, 0.17, 0.9);
...
/*   
    ...
    IlcMevSimConfig *c = new IlcMevSimConfig(1);
    c->SetRectPlane(1);                                 // reaction plane control, model 4
    c->SetGrid(80,80);

    IlcGenMevSim *mevsim = new IlcGenMevSim(c);
    ...
    IlcMevSimParticle *kplus = new IlcMevSimParticle(kKPlus, fNpart, 0, 0.25, 0.0, 2, 0.15, 0.0, 0.0 );
...
void IlcGenHBTosl::Generate()
{
 ...
 IlcStack* stack = RotateStack();

 IlcStack* genstack = fGenerator->GetStack();
 if (genstack == 0x0)
  {
   genstack = new IlcStack(fNpart);
...
void IlcGenHBTosl::GetOneD(TParticle* first, TParticle* second,Double_t qinv)
{
...
//                px, py, pz, e, vx, vy, vz, tof);
  ...
  IlcDebug(1,Form("Randomized qinv = %f, obtained = %f",qinv,GetQInv(first,second)));

}
/***********************************************************/

Int_t IlcGenHBTosl::GetThreeD(TParticle* first,TParticle* second, Double_t qout, Double_t qside, Double_t qlong)
{
//deprecated method that caclulates momenta of the second particle 
...
void IlcGenHBTosl::StartSignalPass1()
{
...
void IlcGenHBTosl::FillCoarseSignal()
{
...
void IlcGenHBTosl::Mix(TList* eventbuffer,TH3D* denominator,TH3D* denominator2)
{
  //Fills denominators
  ...
  IlcStack* firstevent;
  IlcStack* secondevent = 0x0;
  
  while(( firstevent=(IlcStack*)next() ))
   {
    if (secondevent == 0x0) 
     {
...
void IlcGenHBTosl::Mix(IlcStack* stack, TH3D* numerator, TH3D* numerator2)
{
...
Double_t IlcGenHBTosl::GetQInv(TParticle* f, TParticle* s)
{
//calculates qinv 
// cout<<f->Px()<<"   "<<s->Px()<<endl;
...
void  IlcGenHBTosl::GetQOutQSideQLong(TParticle* f, TParticle* s,Double_t& out, Double_t& side, Double_t& lon)
{
...
Double_t IlcGenHBTosl::Scale(TH3D* num, TH3D* den)
{
 ...
 //quatience of num and den to 1 at tail
  ...
  if(!num)
   {
     IlcError("No numerator");
     return 0.0;
   }
  if(!den)
   {
     IlcError("No denominator");
     return 0.0;
   }

  if(fNBinsToScale < 1)
   {
    IlcError("Number of bins for scaling is smaller than 1");
    return 0.0;
   }
  ...
  if (fNBinsToScaleX > nbinsX) 
   {
    IlcError("Number of X bins for scaling is bigger thnan number of bins in histograms");
    return 0.0;
   }
  ...
  if (fNBinsToScaleY > nbinsY) 
   {
    IlcError("Number of Y bins for scaling is bigger thnan number of bins in histograms");
    return 0.0;
   }
  ...
  if (fNBinsToScaleZ > nbinsZ) 
   {
    IlcError("Number of Z bins for scaling is bigger thnan number of bins in histograms");
    return 0.0;
   }

  IlcDebug(1,"No errors detected");
  ...
  IlcDebug(1,Form("numsum=%f densum=%f fNBinsToScaleX=%d fNBinsToScaleY=%d fNBinsToScaleZ=%d",
          numsum,densum,fNBinsToScaleX,fNBinsToScaleY,fNBinsToScaleZ));
  
  if (numsum == 0) return 0.0;
  Double_t ret = densum/numsum;

  IlcDebug(1,Form("returning %f",ret));
  return ret;
   
}
/***********************************************************/

void IlcGenHBTosl::TestCoarseSignal()
{
...
void IlcGenHBTosl::SetTrack(TParticle* p, Int_t& ntr) 
{
//Shortcut to PushTrack(bla,bla,bla,bla.............)
   ...
   Double_t poly = pol.Y();
   Double_t polz = pol.Z();
   TMCProcess mech = IlcGenCocktailAfterBurner::IntToMCProcess(p->GetUniqueID());
   Float_t weight = p->GetWeight();

   IlcGenerator::PushTrack(fTrackIt, -1, pdg, px, py, pz, e, vx, vy, vz, tof,polx, poly, polz, mech, ntr, weight);
}
/***********************************************************/

void IlcGenHBTosl::SetTrack(TParticle* p, Int_t& ntr, IlcStack* stack) const 
{
//Shortcut to SetTrack(bla,bla,bla,bla.............)
   if (p->P() == 0.0)
    {
      Error("SetTrack(TParticle*,Int_t&,IlcStack*)","Particle has zero momentum");
      return;
    }
...
void IlcGenHBTosl::Rotate(TVector3& relvector, TVector3& vector)
{
  ...
  if (IlcDebugLevel()>5)
   {
    ...
    IlcInfo(Form("Rotation test: px %f %f",first.x(),test.x()));
    IlcInfo(Form("Rotation test: py %f %f",first.y(),test.y()));
    IlcInfo(Form("Rotation test: pz %f %f",first.z(),test.z()));
   }
}
/***********************************************************/

Double_t IlcGenHBTosl::Rotate(Double_t x,Double_t y,Double_t z)
{
//Rotates vector to base where only x - coordinate is no-zero, and returns that 
  ...
  if (IlcDebugLevel()>5)
   {
     IlcInfo(Form("Xpr = %f  Ypr = %f",xprime,yprime));
     IlcInfo(Form("Calc sin = %f, and %f",sinphi,TMath::Sin(a1)));
     IlcInfo(Form("Calc cos = %f, and %f",cosphi,TMath::Cos(a1)));
   }
  ...
  IlcInfo(Form("Calculated rot %f, modulus %f",xbis,TMath::Sqrt(x*x+y*y+z*z)));
  return xbis;
}
/***********************************************************/

IlcStack* IlcGenHBTosl::RotateStack()
{ 
//swaps to next stack last goes to first and is reseted
 ...
 if ( fStackBuffer->GetSize() >= fBufferSize )
  {
    stack = (IlcStack*)fStackBuffer->Remove(fStackBuffer->Last());
  }
 else
  {
    stack = new IlcStack(fNpart);
  }
...
Double_t IlcGenHBTosl::GetQInvCorrTheorValue(Double_t qinv) const
{
//Function (deprecated)
...
Double_t IlcGenHBTosl::GetQOutQSideQLongCorrTheorValue(Double_t& out, Double_t& side, Double_t& lon) const
{
...
Bool_t IlcGenHBTosl::CheckParticle(TParticle* p, TParticle* aupair ,IlcStack* stack)
{
...
void IlcGenHBTosl::SwapGeneratingHistograms()
{
...
IlcGenHBTosl& IlcGenHBTosl::operator=(const  IlcGenHBTosl& rhs)
{
// Assignment operator
    rhs.Copy(*this);
    return *this;
...
void IlcGenHBTosl::Copy(TObject&) const
{
    //
    // Copy 
    //

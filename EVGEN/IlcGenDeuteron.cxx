...
ClassImp(IlcGenDeuteron)

IlcGenDeuteron::IlcGenDeuteron(Int_t sign, Double_t pmax, Double_t rmax, Int_t cluster)
 :fSign(1)
 ,fPmax(pmax)
 ,fRmax(rmax)
 ,fSpinProb(0.75)
...
void IlcGenDeuteron::Generate()
{
//
// Look for coalescence of (anti)nucleons in the nucleon source
// provided by the generator cocktail
//
	IlcInfo(Form("sign: %d, Pmax: %g GeV/c, Rmax: %g fm, cluster: %d",fSign, fPmax, fRmax, fClusterType));
	if(fModel!=kNone) IlcInfo(Form("model: %d, time: %g fm/c", fModel, fTimeLength));
	
	IlcGenCocktailAfterBurner* gener = (IlcGenCocktailAfterBurner*)gIlc->GetMCApp()->Generator();
	...
	if(fModel != kNone && gener->FirstGenerator()->Generator()->ProvidesCollisionGeometry())
	{
		...
		if(ap != at) IlcWarning("projectile and target have different size");
...
Double_t IlcGenDeuteron::GetCoalescenceProbability(const TParticle* nucleon1, const TParticle* nucleon2) const
{
//
...
void IlcGenDeuteron::FirstPartner(const TList* protons, TList* neutrons)
{
//
...
void IlcGenDeuteron::WeightMatrix(const TList* protons, const TList* neutrons)
{
//
...
void IlcGenDeuteron::PushDeuteron(TParticle* parent1, TParticle* parent2)
{
//
...
void IlcGenDeuteron::FixProductionVertex(TParticle* i)
{
//

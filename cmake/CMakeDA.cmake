#--------------------------------------------------------------------
# CMakeDA.cmake file for IlcRoot Build System
#
...
list(APPEND mod "-I${ILC_ROOT}/${h_module}")
endforeach(oldetect ${ONLINEDETECTORS})
list(APPEND mod "-I${ILC_ROOT}/include" "-I${ILC_ROOT}/STEER" "-I${ILC_ROOT}/ANALYSIS" "-I${ILC_ROOT}/RAW" "-I${ILC_ROOT}/STEER/STEER" "-I${ILC_ROOT}/STEER/CDB" "-I${ROOTSYS}/include" "-I${ILC_ROOT}/STEER/STEERBase" "-I${ILC_ROOT}/STEER/AOD" "-I${ILC_ROOT}/STEER/ESD" "-I${ILC_ROOT}/MUON/mapping" "-I$ENV{AMORE}/include/amore")

# ----------Common stuff-------------------

file(GLOB_RECURSE _dafiles $ENV{ILC_ROOT}/*da.cxx)
set (DAINSTALL "$ENV{ILC_INSTALL}/DA")
if(DAINSTALL STREQUAL "/DA") 
  set(DAINSTALL "$ENV{ILC_ROOT}/DA")
...
if(DAQDALIB_PATH)
  ...
  set(DAQDADIR "$ENV{ILC}/daqDAlib")
endif(DAQDALIB_PATH)
set(DAQDALIB "${DAQDADIR}/libdaqDA.a")

include_directories(${DAQDADIR} ${ILC_ROOT}/RAW ${ILC_ROOT}/include ${ILC_ROOT}/STEER)
...
foreach(detector ${ONLINEDETECTORS} )
 ...
 list(APPEND ILCROOTALIBS RAWDatabase-static RAWDatarec-static RAWDatasim-static STEERBase-static STEER-static CDB-static ESD-static STAT-static AOD-static ANALYSIS-static ANALYSISilc-static )

  expand(ILCROOTALIBS2 "\${${DAMODULE}ALIBS}")
  expand(DAINCDIRS "\${${DAMODULE}INC}")
  list(APPEND ILCROOTALIBS ${ILCROOTALIBS2})
...
#Get detector algorithms for this detector
  ...
  foreach(dafile ${_dafiles})
	...
	string(REGEX MATCH "$ENV{ILC_ROOT}/${DAMODULE}/${DAMODULE}${SUBDAMODULE}" match ${dafile})
#Found a valid target name
	if(match)
          string(REGEX REPLACE "$ENV{ILC_ROOT}/${DAMODULE}/${DAMODULE}${SUBDAMODULE}(.*)da\\.cxx" "\\1" DANAME ${dafile})
#Check for default DA 
	...
	add_dependencies(${BASIC_RPM} ${DATARGETNAME}-rpm) 
	 set(DATARGETDIR "${DAINSTALL}/${DAMODULE}/tgt_$ENV{ILC_TARGET}")
	...
	# DAVERSION
	  execute_process(COMMAND svn info $ENV{ILC_ROOT}/${DASRC} OUTPUT_VARIABLE _daversion OUTPUT_STRIP_TRAILING_WHITESPACE)
	...
	# DAILCROOTRELEASE
	  string(REGEX REPLACE ".*URL: .*/(.+)/${DASRC}.*$" "\\1" DAILCROOTRELEASE ${_daversion})
          string (REPLACE "-" "." DAILCROOTRELEASE "${DAILCROOTRELEASE}")
	  ...
	  file(READ "$ENV{ILC_ROOT}/${DASRC}" _dasrc)
...
COMMAND @echo "DAQDADIR=${ILC}/daqDAlib" >> ${DAMAKEFILE}
...
COMMAND cp -a ${ILC_ROOT}/${DASRC} junk/${DAARC} 
...
COMMAND @echo "Release: ${DAILCROOTRELEASE}.${DAREVISION}" >> ${DASPECFILE}
COMMAND @echo "License: CERN Ilc DAQ/Offine" >> ${DASPECFILE}
COMMAND @echo "Source: %{name}-%{version}.src.tar.gz" >> ${DASPECFILE}
COMMAND @echo "Group: Applications/Ilc" >> ${DASPECFILE}
...
COMMAND @echo "Build requires: daqDAlib, date, IlcRoot ${DAILCROOTRELEASE},ROOT ${DAROOTRELEASE}." >> ${DASPECFILE}
...
COMMAND echo "${DFLAGS}"
COMMAND g++ -c -D${CMAKE_SYSTEM_NAME} -DDATE_SYS=${CMAKE_SYSTEM_NAME} -Dlong32="int" -Dlong64="long long" -DdatePointer="long" -I/date/rorc -I/date/runControl -I/date/readList -I/date/eventBuilder -I/date/banksManager -I/date/bufferManager -I/date/db -I/date/commonDefs -I/date/monitoring -I/date/infoLogger -I/date/logbook -I${DAQDADIR} -I${ILC_ROOT}/RAW -I${CMAKE_BINARY_DIR}/include -I$ENV{ROOTSYS}/include ${mod} ${date_head} ${ILC_ROOT}/${DASRC} -o ${DAOBJ}
#COMMAND g++ -c ${DATEFLAGS} -I${DAQDADIR} -I${ILC_ROOT}/RAW -I${CMAKE_BINARY_DIR}/include -I$ENV{ROOTSYS}/include ${mod} ${date_head} ${ILC_ROOT}/${DASRC} -o ${DAOBJ}

WORKING_DIRECTORY ${ILC_ROOT} 
)

...
ClassImp(IlcKMeansClustering)

Double_t IlcKMeansClustering::fBeta = 10.;

 
Int_t IlcKMeansClustering::SoftKMeans(Int_t k, Int_t n, const Double_t* x, const Double_t* y, Double_t* mx, Double_t* my , Double_t* rk )
{
    //
...
Int_t IlcKMeansClustering::SoftKMeans2(Int_t k, Int_t n, Double_t* x, Double_t* y, Double_t* mx, Double_t* my , Double_t* sigma2, Double_t* rk )
{
    //
    // The soft K-means algorithm
    //
...
Int_t IlcKMeansClustering::SoftKMeans3(Int_t k, Int_t n, Double_t* x, Double_t* y, Double_t* mx, Double_t* my , 
...
Double_t IlcKMeansClustering::d(Double_t mx, Double_t my, Double_t x, Double_t y)
{
    //
    // Distance definition 
...
void IlcKMeansClustering::OptimalInit(Int_t k, Int_t n, const Double_t* x, const Double_t* y, Double_t* mx, Double_t* my)
{
  //  
...
ClassImp(IlcKMeansResult)


    
IlcKMeansResult::IlcKMeansResult(Int_t k):
    TObject(),
    fK(k),
    fMx    (new Double_t[k]),
    fMy    (new Double_t[k]),
...
IlcKMeansResult::IlcKMeansResult(const IlcKMeansResult &res):
  TObject(res),
  fK(res.GetK()),
...
IlcKMeansResult& IlcKMeansResult::operator=(const IlcKMeansResult& res)
{
  //
  // Assignment operator
  if (this != &res) {
...
IlcKMeansResult::~IlcKMeansResult()
{
// Destructor
    delete[] fMx;
    delete[] fMy;
...
void IlcKMeansResult::Sort(Int_t n, const Double_t* x, const Double_t* y)
{
  // Build target array and sort
  for (Int_t i = 0; i < fK; i++)
    {
      ...
      for (Int_t j = 0; j < n; j++)
	{
	  if (2. * IlcKMeansClustering::d(fMx[i], fMy[i], x[j], y[j])  <  2.28 * fSigma2[i]) nc++;
	}
...
void IlcKMeansResult::CopyResults(const IlcKMeansResult* res)
{
  fK = res->GetK();
  for (Int_t i = 0; i <fK; i++) {
    fMx[i]     = (res->GetMx())    [i];

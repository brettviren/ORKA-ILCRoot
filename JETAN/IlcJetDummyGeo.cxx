...
ClassImp(IlcJetDummyGeo)

IlcJetDummyGeo::IlcJetDummyGeo():
  fArm1EtaMin(-0.7), 
  fArm1EtaMax(+0.7), 
  fArm1PhiMin(80.0), 
  fArm1PhiMax(200.0),
...
IlcJetDummyGeo::IlcJetDummyGeo(const IlcJetDummyGeo& geom):
  TObject(geom),  
...
//------------------------------------------------------------------------------------
void IlcJetDummyGeo::EtaPhiFromIndex(Int_t absId, Float_t& eta, Float_t& phi) const
{
...
//------------------------------------------------------------------------------------
void IlcJetDummyGeo::GetGlobal(const Double_t *loc, Double_t *glob, Int_t ind) const
{
  // Figure out the global numbering of a given supermodule from the
  // local numbering
  // Ilc numbering - Jun 03,2006
...
//------------------------------------------------------------------------------------
void IlcJetDummyGeo::GetGlobal(Int_t absId , Double_t glob[3]) const
{ 
  // Ilc numbering scheme - Jun 03, 2006
...
//------------------------------------------------------------------------------------
void IlcJetDummyGeo::GetGlobal(Int_t absId , TVector3 &vglob) const
{ 
  // Ilc numbering scheme - Jun 03, 2006
...
//------------------------------------------------------------------------------------
Bool_t IlcJetDummyGeo::RelPosCellInSModule(Int_t absId, Double_t loc[3]) const
{
  // Ilc numbering scheme - Jun 03, 2006
...
//------------------------------------------------------------------------------------
Bool_t IlcJetDummyGeo::RelPosCellInSModule(Int_t absId, Double_t &xr, Double_t &yr, Double_t &zr) const
{
  // Look to see what the relative position inside a given cell is
  // for a recpoint.
  // Ilc numbering scheme - Jun 08, 2006
  // In:
...
//------------------------------------------------------------------------------------
Bool_t  IlcJetDummyGeo::CheckAbsCellId(Int_t absId) const
{ 
...
//------------------------------------------------------------------------------------
Bool_t IlcJetDummyGeo::GetCellIndex(Int_t absId,Int_t &nSupMod,Int_t &nModule,Int_t &nIphi,Int_t &nIeta) const
{ 
  // 21-sep-04; 19-oct-05;
  // May 31, 2006; ILC numbering scheme:
  // 
  // In:
  // absId   - cell is as in Geant,     0<= absId   < fNCells;
  // Out:
...
//------------------------------------------------------------------------------------
void IlcJetDummyGeo::GetCellPhiEtaIndexInSModule(Int_t nSupMod, Int_t nModule, Int_t nIphi, Int_t nIeta, Int_t &iphi, Int_t &ieta) const
{ 
  // 
  // Added nSupMod; Nov 25, 05
  // Ilc numbering scheme  - Jun 01,2006 
  // IN:
  // nSupMod - super module(SM) number, 0<= nSupMod < fNumberOfSuperModules;
  // nModule  - module number in SM,     0<= nModule  < fNCellsInSupMod/fNCellsInSupMod or(/2) for tow last SM (10th and 11th);
  // nIphi   - cell number in phi driection inside module; 0<= nIphi < fNPHIdiv; 
...
//------------------------------------------------------------------------------------
void IlcJetDummyGeo::GetModulePhiEtaIndexInSModule(Int_t nSupMod, Int_t nModule,  Int_t &iphim, Int_t &ietam) const
{ 
...
//------------------------------------------------------------------------------------
Bool_t IlcJetDummyGeo::GetAbsCellIdFromEtaPhi(Double_t eta, Double_t phi, Int_t &absId) const
{
  // Nov 17,2006
...
//------------------------------------------------------------------------------------
Bool_t IlcJetDummyGeo::SuperModuleNumberFromEtaPhi(Double_t eta, Double_t phi, Int_t &nSupMod) const
{ 
...
//------------------------------------------------------------------------------------
Int_t  IlcJetDummyGeo::GetAbsCellIdFromCellIndexes(Int_t nSupMod, Int_t iphi, Int_t ieta) const
{
...
//------------------------------------------------------------------------------------
void  IlcJetDummyGeo::GetModuleIndexesFromCellIndexesInSModule(Int_t nSupMod, Int_t iphi, Int_t ieta, 
...
//------------------------------------------------------------------------------------
Int_t IlcJetDummyGeo::GetAbsCellId(Int_t nSupMod, Int_t nModule, Int_t nIphi, Int_t nIeta) const
{ 
  // 27-aug-04; 
  // corr. 21-sep-04; 
...
//------------------------------------------------------------------------------------
Bool_t IlcJetDummyGeo::GetPhiBoundariesOfSMGap(Int_t nPhiSec, Double_t &phiMin, Double_t &phiMax) const
{
...
//------------------------------------------------------------------------------------
void IlcJetDummyGeo::CreateListOfTrd1Modules()
{
  ...
  IlcJetDummyShishKebabTrd1Module *mod=0, *mTmp=0; // current module
  if(fShishKebabTrd1Modules == 0) {
    ...
    for(Int_t iz=0; iz< GetNZ(); iz++) { 
      if(iz==0) { 
        mod  = new IlcJetDummyShishKebabTrd1Module(TMath::Pi()/2.,this);
      } else {
        mTmp  = new IlcJetDummyShishKebabTrd1Module(*mod);
        mod   = mTmp;
      }
  ...
  mod = (IlcJetDummyShishKebabTrd1Module*)fShishKebabTrd1Modules->At(fShishKebabTrd1Modules->GetSize()-1);
  ...
  // Feb 20,2006;
  // Jun 01, 2006 - ILC numbering scheme
  ...
  if(fDebug>1) IlcInfo(Form(" Cells grid in eta directions : size %i\n", fCentersOfCellsEtaDir.GetSize()));
  for(Int_t it=0; it<fNZ; it++) {
    IlcJetDummyShishKebabTrd1Module *trd1 = GetShishKebabModule(it);
    nModule = fNPhi*it;
    for(Int_t ic=0; ic<fNETAdiv; ic++) {
      ...
      for(Int_t iphi=0; iphi<fCentersOfCellsPhiDir.GetSize(); iphi++) {
        ...
        eta   = IlcJetDummyShishKebabTrd1Module::ThetaToEta(theta);
  ...
  if(fDebug>1){
    for(Int_t i=0; i<fCentersOfCellsEtaDir.GetSize(); i++) {
      IlcInfo(Form(" ind %2.2i : z %8.3f : x %8.3f", i+1, 
...
//------------------------------------------------------------------------------------
IlcJetDummyShishKebabTrd1Module* IlcJetDummyGeo::GetShishKebabModule(Int_t neta)
{
  ...
  static IlcJetDummyShishKebabTrd1Module* trd1=0;
  if(fShishKebabTrd1Modules && neta>=0 && neta<fShishKebabTrd1Modules->GetSize()) {
    trd1 = (IlcJetDummyShishKebabTrd1Module*)fShishKebabTrd1Modules->At(neta);
  } else trd1 = 0;
  return trd1;
}

//------------------------------------------------------------------------------------
void  IlcJetDummyGeo::GetTransformationForSM()
{

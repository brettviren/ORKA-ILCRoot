...
//
// **February 2011
// implemented  standard geometry (IlcEMCALGeometry) (was IlcJetDummyGeo implented separately in ESDReader and AODReader
// local2master matrices are now get from $ILC_ROOT/OADB/PWG4/JetReconstruction/EMCALlocal2master.root
...
// ** 2011
// - IlcJetESD/AODReader classes removed from JETAN. Reader now ESD/AOD independent. It uses VEvent in the IlcJetFill* classes.
...
#include "IlcJetFillCalTrkTrackMC.h"
#include "IlcJetCalTrk.h"

ClassImp(IlcJetReader)

////////////////////////////////////////////////////////////////////////


IlcJetReader::IlcJetReader():
  fCalTrkEvent(0x0),
  fFillEvent(0x0),
  fReaderHeader(0),
  fFillEventwTrks(0x0), 
...
void IlcJetReader::InitTasks()
{
  ...
  fCalTrkEvent = new IlcJetCalTrkEvent(fOpt,kIsKine,kIsHighMult);
...
//-----------------------------------------------------------------------
Bool_t IlcJetReader::ProcessEvent()
{
  // Process one event
...
//-----------------------------------------------------------------------
void IlcJetReader::SetInputEvent(const TObject* esd, const TObject* aod, const IlcMCEvent* mc)
{
  // set input event pointers
  if( fReaderHeader->GetDataType().Contains("AOD") && aod) {fVEvent = (IlcAODEvent*) aod;}
  else if( fReaderHeader->GetDataType().Contains("ESD") && esd) {fVEvent = (IlcESDEvent*) esd;}
  else if ( fReaderHeader->GetDataType().Contains("MC") || fReaderHeader->GetDataType().Contains("MC2")) { fMCEvent = (IlcMCEvent*) mc;}
  else {printf("No input event ! ");}

}

//-----------------------------------------------------------------------
Bool_t IlcJetReader::CreateTasks()
{
  // For reader task initialization

  fDebug = fReaderHeader->GetDebug();

  fFillEvent = new IlcJetFillCalTrkEvent();
  if (fOpt>0) {
    // Tracks
    if(fOpt%2==!0 && fOpt!=0){
      fFillEventwTrks = new IlcJetFillCalTrkTrack();
      fFillEventwTrks->SetReaderHeader(fReaderHeader);
    }
  }
  else { // MC/Kine cases
    fFillEventwTrks = new IlcJetFillCalTrkTrackMC();
    fFillEventwTrks->SetReaderHeader(fReaderHeader);
  }
...
//-----------------------------------------------------------------------
void IlcJetReader::WriteRHeaderToFile() const
{
  // write reader header
  IlcJetReaderHeader *rh = GetReaderHeader();
  rh->Write();

}

//-----------------------------------------------------------------------
void IlcJetReader::WriteReaderHeader()
{
  // Write the Headers

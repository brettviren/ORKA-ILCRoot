...
ClassImp(IlcJetFillCalTrkTrackMC)

/////////////////////////////////////////////////////////////////////////

IlcJetFillCalTrkTrackMC::IlcJetFillCalTrkTrackMC(): 
...
//-----------------------------------------------------------------------
IlcJetFillCalTrkTrackMC::IlcJetFillCalTrkTrackMC(IlcVEvent* evt): 
  IlcJetFillCalTrkEvent(),
  fHadCorr(0),
  fApplyMIPCorrection(kTRUE),
  fVEvt(evt),
...
//-----------------------------------------------------------------------
IlcJetFillCalTrkTrackMC::IlcJetFillCalTrkTrackMC(const IlcJetFillCalTrkTrackMC &det): 
...
//-----------------------------------------------------------------------
IlcJetFillCalTrkTrackMC& IlcJetFillCalTrkTrackMC::operator=(const IlcJetFillCalTrkTrackMC& other)
{
  // Assignment
  if (this != &other) { 
...
//-----------------------------------------------------------------------
void IlcJetFillCalTrkTrackMC::Exec(Option_t* const /*option*/)
{
  // Main method.
  // Fill IlcJetFillCalTrkTrackMC the with the charged particle information
  ...
  if (type == kTrackKineAll || type == kTrackKineCharged){
    ...
    if(!fVEvt) return; 
    TClonesArray *mcArray = dynamic_cast<TClonesArray*>(fVEvt->FindListObject(IlcAODMCParticle::StdBranchName()));
    if(!mcArray) {
    ...
    for(int it = 0;it < mcArray->GetEntriesFast();++it){
      cflag=sflag=0;
      IlcAODMCParticle *part = (IlcAODMCParticle*)(mcArray->At(it));
...
//-----------------------------------------------------------------------
Bool_t IlcJetFillCalTrkTrackMC::FillKine()
{
  // Fill event
  Int_t goodTrack = 0;
  ...
  for (Int_t it = 0; it < nt; it++) {
    ...
    IlcVParticle* part = fMCEvent->GetTrack(it);
...
//-----------------------------------------------------------------------
Float_t IlcJetFillCalTrkTrackMC::SmearMomentum(Int_t ind, Float_t p)
{
...
//-----------------------------------------------------------------------
Bool_t IlcJetFillCalTrkTrackMC::Efficiency(Float_t p, Float_t /*eta*/, Float_t phi)
{

...
#include "IlcAnalysisTaskKMeans.h"
#include "IlcTrackReference.h"
#include "IlcStack.h"
#include "IlcHeader.h"
#include "IlcKMeansClustering.h"



ClassImp(IlcAnalysisTaskKMeans)

IlcAnalysisTaskKMeans::IlcAnalysisTaskKMeans() 
    : IlcAnalysisTaskSE()
    ,fK(0)
    ,fNMin(0)
    ,fHists(0)
...
IlcAnalysisTaskKMeans::IlcAnalysisTaskKMeans(const char *name) 
...
IlcAnalysisTaskKMeans::IlcAnalysisTaskKMeans(const IlcAnalysisTaskKMeans &res)
: IlcAnalysisTaskSE(res) 
      ,fK(0)
      ,fNMin(0)
      ,fHists(0)
      ,fH1CEta(0)
...
IlcAnalysisTaskKMeans& IlcAnalysisTaskKMeans::operator=(const IlcAnalysisTaskKMeans& /*trclass*/)
{
...
//________________________________________________________________________
void IlcAnalysisTaskKMeans::UserCreateOutputObjects()
{
  // Create histograms
  // Called once
    fHists   = new TList();
    ...
    //
    for (Int_t i = 0; i < 10; i++) {
      fA[i] = new IlcKMeansResult(i+1);
      fB[i] = new IlcKMeansResult(i+1);
    }
}

//________________________________________________________________________
void IlcAnalysisTaskKMeans::UserExec(Option_t *) 
{
  // Main loop
  ...
  for (Int_t iTracks = 0; iTracks < fInputEvent->GetNumberOfTracks(); iTracks++) {
      IlcVParticle* track = fInputEvent->GetTrack(iTracks);
      if ((fCuts->AcceptTrack((IlcESDtrack*)track))) 
      {
	const IlcExternalTrackParam * tpcT = ((IlcESDtrack*) track)->GetTPCInnerParam();
	if (!tpcT) continue;
  ...
  //
  Double_t rk0[10];
  IlcKMeansResult* res = 0;
  IlcKMeansResult best(10);
  Float_t   rmaxG = -1.;
  ...
  for (Int_t k = 0; k < 20; k++) {
    ...
    for (Int_t i = 0; i < fK; i++) {
      res = fA[i];
      IlcKMeansClustering::SoftKMeans2(i+1, ic, phi, eta, res->GetMx(),res->GetMy(), res->GetSigma2(), res->GetRk());
      res->Sort(ic, phi, eta);
  ...
  for (Int_t k = 0; k < 20; k++) {
    ...
    for (Int_t i = 0; i < fK; i++) {
      res = fB[i];
      IlcKMeansClustering::SoftKMeans2(i+1, ic, phiR, etaR, res->GetMx(),res->GetMy(), res->GetSigma2(), res->GetRk());
...
Double_t IlcAnalysisTaskKMeans::DeltaPhi(Double_t phi1, Double_t phi2)
{
...
Double_t IlcAnalysisTaskKMeans::DeltaR(Double_t phi1, Double_t eta1, Double_t phi2, Double_t eta2)
{
...
//________________________________________________________________________
void IlcAnalysisTaskKMeans::Terminate(Option_t *) 
{
}  

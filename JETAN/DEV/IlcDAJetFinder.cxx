...
#include "IlcDAJetFinder.h"

ClassImp(IlcDAJetFinder)

///////////////////////////////////////////////////////////////////////

IlcDAJetFinder::IlcDAJetFinder():
  IlcJetFinder(),
  fAlpha(1.01),
  fDelta(1e-8),
  fAvDist(1e-6),
...
//-----------------------------------------------------------------------------------
void IlcDAJetFinder::InitDetAnn(Double_t &dEtSum,Double_t **xData,TVectorD *vPx,TVectorD *vPy,TMatrixD *mPyx,TMatrixD *mY)
{
  //Initialise the variables used by the algorithm
  fBeta=0.1;
  fNclustMax = ((IlcDAJetHeader*)fHeader)->GetFixedCl() ?
    ((IlcDAJetHeader*)fHeader)->GetNclustMax() :
    TMath::Max((Int_t)TMath::Sqrt(fNin),5);
  Float_t etaEff = ((IlcDAJetHeader*)fHeader)->GetEtaEff();

  fNin=0;
  for (Int_t iTr=0; iTr<GetCalTrkEvent()->GetNCalTrkTracks(); iTr++) if (GetCalTrkEvent()->GetCalTrkTrack(iTr)->GetCutFlag()==1) fNin++;

  fNeff = ((IlcDAJetHeader*)fHeader)->GetNeff();
  ...
  for (Int_t iTr=0; iTr<GetCalTrkEvent()->GetNCalTrkTracks(); iTr++){
    IlcJetCalTrkTrack* ctT = GetCalTrkEvent()->GetCalTrkTrack(iTr);
...
//-----------------------------------------------------------------------------------
void IlcDAJetFinder::DoubleClusters(Int_t nc,Int_t &nk, TVectorD *vPy, TMatrixD *mY) const
{
...
//-----------------------------------------------------------------------------------
void IlcDAJetFinder::Annealing(Int_t nk,Double_t **xData,  const TVectorD *vPx,  TVectorD *vPy,  TMatrixD *mPyx,  TMatrixD *mY)
{
  ...
  Double_t df[2]={((IlcDAJetHeader*)fHeader)->GetFiducialEtaMax(),pi};
  TVectorD vPart(2);
...
//-----------------------------------------------------------------------------------
void IlcDAJetFinder::NumCl(Int_t &nc,Int_t &nk,TVectorD *vPy, TMatrixD *mPyx,TMatrixD *mY)
{
  // Number of clusters
...
//-----------------------------------------------------------------------------------
void IlcDAJetFinder::ReduceClusters(Int_t **iSame,Int_t nc,Int_t &ncout,Int_t **cont,Int_t *nSameOut) const
{
  // Reduction step
...
//-----------------------------------------------------------------------------------
void IlcDAJetFinder::EndDetAnn(Int_t &nk,Double_t **xData,Int_t *xx,Double_t etx,const TVectorD *vPx,TVectorD *vPy,TMatrixD *mPyx,TMatrixD *mY)
{
...
//-----------------------------------------------------------------------------------
void IlcDAJetFinder::StoreJets(Int_t nk, Double_t **xData, const Int_t *xx, const TMatrixD *mY)
{
  //evaluate significant clusters properties
  const Double_t pi=TMath::Pi();
  Float_t dFidEtaMax = ((IlcDAJetHeader*)fHeader)->GetFiducialEtaMax();
  Float_t dFidEtaMin = ((IlcDAJetHeader*)fHeader)->GetFiducialEtaMin();
  ...
  if (((IlcDAJetHeader*)fHeader)->GetSelJets()){
    for (Int_t iClust=0; iClust<nk; iClust++){
	...
	etDensMed/=extSurf;
	etNoBg[iClust]=(*mY)(3,iClust)-etDensMed*surf[iClust];
	if (etNoBg[iClust]<((IlcDAJetHeader*)fHeader)->GetEtMin()){
	  isJet[iClust]=kFALSE;
	  iClust=-1;
	}
      }

...
//
// **February 2011
// implemented standard geometry (IlcEMCALGeometry) instead of dummy one (IlcJetDummyGeo)
// moved geometry definition in IlcJetReader
...
#include "IlcJetAODFillUnitArrayTracks.h"
...
ClassImp(IlcJetAODReader)

IlcJetAODReader::IlcJetAODReader():
    IlcJetReader(),
    fAOD(0x0),
    fRef(new TRefArray),
    fDebug(0),
...
void IlcJetAODReader::OpenInputFiles()
{
...
//   // Add files matching patters to the chain
   ...
   const char *name = 0x0;
   int naod = ((IlcJetAODReaderHeader*) fReaderHeader)->GetNaod();
   int a = 0;
  ...
  printf("\n IlcJetAODReader: Total number of events in chain= %d \n",nMax);
...
void IlcJetAODReader::ConnectTree(TTree* tree, TObject* /*data*/) {
    // Connect the tree
    // For AOD reader it's needed only to set the number of events
     fChain = (TChain*)      tree;
     
     Int_t nMax = fChain->GetEntries(); 
     printf("\n IlcJetAODReader: Total number of events in chain= %5d \n", nMax);
     // set number of events in header
     if (fReaderHeader->GetLastEvent() == -1)
	 fReaderHeader->SetLastEvent(nMax);
     else {
...
Bool_t IlcJetAODReader::AcceptAODMCParticle(IlcAODMCParticle *mcP,Short_t flag){
  ...
  if(flag==IlcJetAODReaderHeader::kAllMC)return kTRUE;
  if(flag==IlcJetAODReaderHeader::kChargedMC){
...
Bool_t IlcJetAODReader::FillMomentumArrayMC(){
  ...
  TClonesArray *mcArray = (TClonesArray*)fAOD->FindListObject(IlcAODMCParticle::StdBranchName());
  if(!mcArray){
  ...
  Short_t readerFlag = ((IlcJetAODReaderHeader*)fReaderHeader)->GetReadAODMC();


  for (Int_t it = 0; it < nMC; it++) {
    IlcAODMCParticle *track = (IlcAODMCParticle*)mcArray->At(it);
...
Bool_t IlcJetAODReader::FillMomentumArray()
{
  ...
  Float_t etaMin = fReaderHeader->GetFiducialEtaMin();
  Float_t etaMax = fReaderHeader->GetFiducialEtaMax();  
  UInt_t  filterMask =  ((IlcJetAODReaderHeader*)fReaderHeader)->GetTestFilterMask();

  // ----- number of tracks -----
  Int_t nTracksStd    = 0;
  Short_t mcReaderFlag = ((IlcJetAODReaderHeader*)fReaderHeader)->GetReadAODMC();
  TClonesArray *mcArray = 0x0;
  // check if we have to read from MC branch
  if (((IlcJetAODReaderHeader*)fReaderHeader)->GetReadStdBranch()) {
    if(mcReaderFlag) {
      mcArray = (TClonesArray*)fAOD->FindListObject(IlcAODMCParticle::StdBranchName());
      if(!mcArray){
	Printf("%s:%d No MC particle branch found",(char*)__FILE__,__LINE__);
      }
  ...
  if (((IlcJetAODReaderHeader*)fReaderHeader)->GetReadNonStdBranch()) {
    nonStdTracks =
      (TClonesArray*) fAOD->FindListObject(((IlcJetAODReaderHeader*)fReaderHeader)->GetNonStdBranch());
    if (nonStdTracks)
  ...
  if (mcArray) {
    for (Int_t iTrack = 0; iTrack < nTracksStd; iTrack++) {
      IlcAODMCParticle *track = (IlcAODMCParticle*)mcArray->At(iTrack);
      if(!track->IsPhysicalPrimary())continue;
    ...
    for (Int_t iTrack = 0; iTrack < nTracksStd; iTrack++) {
      IlcAODTrack *track = fAOD->GetTrack(iTrack);
  ...
  for (Int_t iTrack = 0; iTrack < nTracksNonStd; iTrack++) {
    IlcVParticle *track = dynamic_cast<IlcVParticle*> ((*nonStdTracks)[iTrack]);
    if (!track)
      continue;
...
void IlcJetAODReader::SetApplyMIPCorrection(Bool_t val)
{
  //
...
//__________________________________________________________
void IlcJetAODReader::SetApplyFractionHadronicCorrection(Bool_t val)
{
  //
...
//__________________________________________________________
void IlcJetAODReader::SetFractionHadronicCorrection(Double_t val)
{
  //
...
//____________________________________________________________________________
void IlcJetAODReader::CreateTasks(TChain* tree)
{
  //
  ...
  fFillUAFromTracks = new IlcJetAODFillUnitArrayTracks();
  ...
  fFillUAFromEMCalDigits = new IlcJetAODFillUnitArrayEMCalDigits();
...
//____________________________________________________________________________
Bool_t IlcJetAODReader::ExecTasks(Bool_t procid, TRefArray* refArray)
{
  //
  // Main function
...
//____________________________________________________________________________
void IlcJetAODReader::InitParameters()
{
  // Initialise parameters
  fOpt = fReaderHeader->GetDetector();
  fNumUnits       = 0;//fGeom->GetEMCGeometry()->GetNCells();      // Number of cells in EMCAL
  if(fDebug>1) printf("\n EMCal parameters initiated ! \n");
}

//____________________________________________________________________________
void IlcJetAODReader::InitUnitArray()
{
      ...
      for(Int_t nBin = 1; nBin < nElements+1; nBin++)
        {
          ...
          new ((*fUnitArray)[nBin-1]) IlcJetUnitArray(nBin-1,0,eta,phi,0.,deltaEta,deltaPhi,kTpc,kOutJet,kPtSmaller,kPtSmaller,kBad,0.,-1);
        }
    }
...
//           fGeom->GetEMCGeometry()->GetPhiBoundariesOfSMGap(0,phimin0,phimax0);
          fGrid0 = new IlcJetGrid(0,95,phimin0,phimax0,-0.7,0.7); // 0.015 x 0.015
...
//           fGeom->GetEMCGeometry()->GetPhiBoundariesOfSMGap(1,phimin1,phimax1);
          fGrid1 = new IlcJetGrid(0,95,phimin1,phimax1,-0.7,0.7); // 0.015 x 0.015
...
//           fGeom->GetEMCGeometry()->GetPhiBoundariesOfSMGap(2,phimin2,phimax2);
          fGrid2 = new IlcJetGrid(0,95,phimin2,phimax2,-0.7,0.7); // 0.015 x 0.015
...
//           fGeom->GetEMCGeometry()->GetPhiBoundariesOfSMGap(3,phimin3,phimax3);
          fGrid3 = new IlcJetGrid(0,95,phimin3,phimax3,-0.7,0.7); // 0.015 x 0.015
...
//           fGeom->GetEMCGeometry()->GetPhiBoundariesOfSMGap(4,phimin4,phimax4);
          fGrid4 = new IlcJetGrid(0,95,phimin4,phimax4,-0.7,0.7); // 0.015 x 0.015
...
// 	      fGeom->EtaPhiFromIndex(nBin, eta, phi); // From EMCal geometry 
	      ...
	      new ((*fUnitArray)[nBin]) IlcJetUnitArray(nBin,0,eta,phi,0.,deltaEta,deltaPhi,kTpc,kOutJet,kPtSmaller,kPtSmaller,kBad,0.,-1);
	    } 
	  else {

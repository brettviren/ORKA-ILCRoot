...
#include "IlcAnalysisTaskJetBackgroundSubtract.h"
...
ClassImp(IlcAnalysisTaskJetBackgroundSubtract)

IlcAnalysisTaskJetBackgroundSubtract::~IlcAnalysisTaskJetBackgroundSubtract(){
  // 
  // destructor
  //
  delete fJBArray;
...
IlcAnalysisTaskJetBackgroundSubtract::IlcAnalysisTaskJetBackgroundSubtract(const char* name):
...
Bool_t IlcAnalysisTaskJetBackgroundSubtract::Notify()
{
  //
  // exec with every new file
  //
  fAODIn = dynamic_cast<IlcAODEvent*>(InputEvent());
...
void IlcAnalysisTaskJetBackgroundSubtract::UserCreateOutputObjects()
{
  ...
  if(fBackgroundBranch.Length()==0)
    IlcError(Form("%s:%d No BackgroundBranch defined",(char*)__FILE__,__LINE__));
  if((fJBArray?fJBArray->GetEntries():0)==0)
    IlcError(Form("%s:%d No Jet Branches defined defined",(char*)__FILE__,__LINE__));
}

void IlcAnalysisTaskJetBackgroundSubtract::Init()
{
  //
  // Initialization
  //
...
void IlcAnalysisTaskJetBackgroundSubtract::UserExec(Option_t */*option*/)
{
  //
  ...
  bkgClusterName.ReplaceAll(Form("%s_",IlcAODJetEventBackground::StdBranchName()),"");
  TString bkgClusterRCName(Form("%s%s",bkgClusterName.Data(),"RandomCone")); 

  if(!evBkg&&!bkgClusters&&!bkgClustersRC&&fAODOut){
    evBkg = (IlcAODJetEventBackground*)(fAODOut->FindListObject(fBackgroundBranch.Data()));
  ...
  if(!evBkg&&!bkgClusters&&!bkgClustersRC&&fAODExtension){
    evBkg = (IlcAODJetEventBackground*)(fAODExtension->GetAOD()->FindListObject(fBackgroundBranch.Data()));
  ...
  if(!evBkg&&!bkgClusters&&!bkgClustersRC&&fAODIn){
    evBkg = (IlcAODJetEventBackground*)(fAODIn->FindListObject(fBackgroundBranch.Data()));
  ...
  // LOOP over all jet branches and subtract the background
   ...
   for(int iJB = 0;iJB<fInJetArrayList->GetEntries();iJB++){
    ...
    for(int i = 0;i < jarray->GetEntriesFast();i++){
      IlcAODJet *jet = (IlcAODJet*)jarray->At(i);
      IlcAODJet tmpNewJet(*jet);
      Bool_t bAdd = false;
      Float_t ptSub = 0;
	...
	tmpNewJet.SetPtSubtracted(ptSub,0);
	 ...
	 tmpNewJet.SetPtSubtracted(ptSub,0);
      ...
      if(bAdd){
        IlcAODJet *newJet = new ((*jarrayOut)[nOut++]) IlcAODJet(tmpNewJet);
	...
	for(Int_t ir=0;ir<refs->GetEntriesFast();ir++){
	  IlcVParticle *vp = dynamic_cast<IlcVParticle*>(refs->At(ir));
...
void IlcAnalysisTaskJetBackgroundSubtract::Terminate(Option_t */*option*/)
{
  // Terminate analysis
  //
...
Bool_t IlcAnalysisTaskJetBackgroundSubtract::RescaleJetMomentum(IlcAODJet *jet,Float_t pT){
...
Bool_t IlcAnalysisTaskJetBackgroundSubtract::RescaleJet4vector(IlcAODJet *jet,TLorentzVector backgroundv){
...
Double_t IlcAnalysisTaskJetBackgroundSubtract::RecalcRho(TClonesArray* bkgClusters,Double_t meanarea){
  ...
  if(jarray->GetEntries()>=2){ 
    IlcAODJet *first = (IlcAODJet*)(jarray->At(0)); 
    IlcAODJet *second= (IlcAODJet*)(jarray->At(1)); 
    for(Int_t k=0;k<bkgClusters->GetEntriesFast();k++){
      IlcAODJet *clus = (IlcAODJet*)(bkgClusters->At(k));
...
Double_t IlcAnalysisTaskJetBackgroundSubtract::RhoRC(TClonesArray* bkgClustersRC){
  ...
  //
	...
	if(!jarray)return rho;
	   ...
	   IlcAODJet *clus = (IlcAODJet*)(bkgClustersRC->At(k));
	   if(TMath::Abs(clus->Eta())>0.5) continue;
	   Double_t distance1=(first->Eta()-clus->Eta())*(first->Eta()-clus->Eta())+
	     (first->Phi()-clus->Phi())*(first->Phi()-clus->Phi());
	   Double_t distance2= (second->Eta()-clus->Eta())*(second->Eta()-clus->Eta())+
...
void IlcAnalysisTaskJetBackgroundSubtract::ResetOutJets(){
  //
...
void IlcAnalysisTaskJetBackgroundSubtract::PrintAODContents(){
...
Int_t IlcAnalysisTaskJetBackgroundSubtract::MultFromJetRefs(TClonesArray* jets){
  //
  ...
  for(int ij = 0;ij < jets->GetEntries();++ij){
    IlcAODJet* jet = (IlcAODJet*)jets->At(ij);
    if(!jet)continue;
    TRefArray *refs = jet->GetRefTracks();
    if(!refs)continue;
...
void  IlcAnalysisTaskJetBackgroundSubtract::AddJetBranch(const char* c){

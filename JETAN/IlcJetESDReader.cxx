...
//____________________________________________________________________________
void IlcJetESDReader::OpenInputFiles()
{
  // chain for the ESDs
  ...
  // get directory and pattern name from the header
   ...
   const char *name = 0x0;
   int nesd = ((IlcJetESDReaderHeader*) fReaderHeader)->GetNesd();
   int a = 0;
   while ((name = gSystem->GetDirEntry(dir))){
	 ...
	 printf("Adding %s\n",name);
	 fChain->AddFile(Form("%s/%s/IlcESDs.root",dirName,name));
	 a++;
       }
   }
  ...
  printf("\n IlcJetESDReader: Total number of events in chain= %d \n",nMax);
...
void IlcJetESDReader::SetApplyMIPCorrection(Bool_t val)
{
  //
...
//__________________________________________________________
void IlcJetESDReader::SetApplyFractionHadronicCorrection(Bool_t val)
{
  //
...
//__________________________________________________________
void IlcJetESDReader::SetFractionHadronicCorrection(Double_t val)
{
  //
...
//____________________________________________________________________________
void IlcJetESDReader::SetInputEvent(const TObject* esd, const TObject* /*aod*/, const TObject* /*mc*/) {
    // Connect the tree
     fESD   = (IlcESDEvent*) esd;
}

//____________________________________________________________________________
Bool_t IlcJetESDReader::FillMomentumArray()
{
  // Fill momentum array
  ...
  for (Int_t it = 0; it < nt; it++) {
      IlcESDtrack *track = fESD->GetTrack(it);
...
//____________________________________________________________________________
void IlcJetESDReader::CreateTasks(TChain* tree)
{
  //
  ...
  fFillUAFromTracks = new IlcJetESDFillUnitArrayTracks(); 
  ...
  fFillUAFromEMCalDigits = new IlcJetESDFillUnitArrayEMCalDigits();
...
//____________________________________________________________________________
Bool_t IlcJetESDReader::ExecTasks(Bool_t procid, TRefArray* refArray)
{
  //
...
//____________________________________________________________________________  
void IlcJetESDReader::InitParameters()
{
...
//____________________________________________________________________________
void IlcJetESDReader::InitUnitArray()
{
  //
  ...
  if(fTpcGrid->GetGridType()==0)
      ...
      for(Int_t nBin = 1; nBin < nElements+1; nBin++)
	{
	  ...
	  new ((*fUnitArray)[nBin-1]) IlcJetUnitArray(nBin-1,0,eta,phi,0.,Deta,Dphi,kTpc,kOutJet,kPtSmaller,kPtSmaller,kBad,0.,-1);
	}
    }
...
// 	      new ((*fUnitArray)[nBin]) IlcJetUnitArray(nBin,0,eta,phi,0.,Deta,Dphi,kTpc,kOutJet,kPtSmaller,kPtSmaller,kBad,0.,-1);
// 	    } 
// 	  else {
...
// 		      Deta = fGrid0->GetDeta(); 
// 		      Dphi = fGrid0->GetDphi(); 
// 		      new ((*fUnitArray)[nBin]) IlcJetUnitArray(nBin,0,eta,phi0,0.,Deta,Dphi,kTpc,kOutJet,kPtSmaller,kPtSmaller,kBad,0.,-1);
// 		    }

...
#include "IlcAnalysisTaskJetCluster.h"
...
ClassImp(IlcAnalysisTaskJetCluster)

IlcAnalysisTaskJetCluster::~IlcAnalysisTaskJetCluster(){
  //
  // Destructor
  //
...
IlcAnalysisTaskJetCluster::IlcAnalysisTaskJetCluster(const char* name):
...
Bool_t IlcAnalysisTaskJetCluster::Notify()
{
  //
...
void IlcAnalysisTaskJetCluster::UserCreateOutputObjects()
{
  ...
  if(fNonStdBranch.Length()!=0)
	...
	if(!AODEvent()->FindListObject(Form("%s_%s",IlcAODJetEventBackground::StdBranchName(),fNonStdBranch.Data()))){
	  fAODJetBackgroundOut = new IlcAODJetEventBackground();
	  fAODJetBackgroundOut->SetName(Form("%s_%s",IlcAODJetEventBackground::StdBranchName(),fNonStdBranch.Data()));
	  if(fUseDiceEfficiency || fUseTrMomentumSmearing)
	    fAODJetBackgroundOut->SetName(Form("%s_%sDetector%d%d",IlcAODJetEventBackground::StdBranchName(),fNonStdBranch.Data(),fUseTrMomentumSmearing,fUseDiceEfficiency));

	  AddAODBranch("IlcAODJetEventBackground",&fAODJetBackgroundOut,fNonStdFile.Data());  
	}
      }
	...
	if(fJetTypes&kRC){
	  if(!AODEvent()->FindListObject(cName.Data())){
	    fTCARandomConesOut = new TClonesArray("IlcAODJet", 0);
	...
	if(fJetTypes&kRCRan){
	  ...
	  if(!AODEvent()->FindListObject(cName.Data())){
	    fTCARandomConesOutRan = new TClonesArray("IlcAODJet", 0);
...
void IlcAnalysisTaskJetCluster::Init()
{
  //
  // Initialization
  //
...
void IlcAnalysisTaskJetCluster::UserExec(Option_t */*option*/)
{
  ...
  IlcAODJetEventBackground* externalBackground = 0;
  if(!externalBackground&&fBackgroundBranch.Length()){
    externalBackground =  (IlcAODJetEventBackground*)(AODEvent()->FindListObject(fBackgroundBranch.Data()));
    if((!externalBackground)&&fAODExtension)externalBackground = (IlcAODJetEventBackground*)(fAODExtension->GetAOD()->FindListObject(fBackgroundBranch.Data()));
  ...
  if(fUseAODTrackInput){    
    fAOD = dynamic_cast<IlcAODEvent*>(InputEvent());
    if(!fAOD){
    ...
    if(fDebug>0){
      fESD = dynamic_cast<IlcESDEvent*> (InputEvent());
    }
  }
  ...
  Bool_t physicsSelection = true;// handled by the framework(fInputHandler->IsEventSelected()&IlcVEvent::kMB)==IlcVEvent::kMB;

  Float_t cent = 0;
  Float_t zVtx  = 0;
  Int_t cenClass = -1;
  if(fAOD){
    const IlcAODVertex *vtxAOD = fAOD->GetPrimaryVertex();
  ...
  // we simply fetch the tracks/mc particles as a list of IlcVParticles
  ...
  IlcAODJet vTmpRan(1,0,0,1);
  for(int i = 0; i < recParticles.GetEntries(); i++){
    IlcVParticle *vp = (IlcVParticle*)recParticles.At(i);
 ...
 // loop over all jets an fill information, first on is the leading jet
  ...
  if(inclusiveJets.size()>0){
    ...
    recIter->Reset();
    IlcVParticle *tmpRecTrack = 0;
    while((tmpRecTrack = (IlcVParticle*)(recIter->Next()))){
    ...
    for(int j = 0; j < nRec;j++){
      IlcAODJet tmpRec (sortedJets[j].px(), sortedJets[j].py(), sortedJets[j].pz(), sortedJets[j].E());
      aodOutJet = 0;
      nAodOutTracks = 0;
      Float_t tmpPt = tmpRec.Pt();  
      
      if(tmpPt>fJetOutputMinPt&&fTCAJetsOut){// cut on the non-background subtracted...
	aodOutJet =  new ((*fTCAJetsOut)[nAodOutJets++]) IlcAODJet(tmpRec);
	...
	IlcVParticle *part = (IlcVParticle*)recParticles.At(constituents[ic].user_index());
	if(!part) continue;
	...
	if(aodOutJet){
	  ...
	  if(part->Pt()>fMaxTrackPtInJet)aodOutJet->SetTrigger(IlcAODJet::kHighTrackPtTriggered);
	}
   ...
   if(fNRandomCones>0&&fTCARandomConesOut){       
     ...
     for(int ir = 0;ir < fNRandomCones;ir++){
       ...
       IlcAODJet tmpRecC (pXC,pYC,pZC, pC); 
       bool skip = false;
       for(int jj = 0; jj < TMath::Min(nRec,fNSkipLeadingCone);jj++){// test for overlap with leading jets
	 IlcAODJet jet (sortedJets[jj].px(), sortedJets[jj].py(), sortedJets[jj].pz(), sortedJets[jj].E());
	 ...
	 IlcAODJet *iicone = (IlcAODJet*)fTCARandomConesOut->At(iic);
	 if(iicone){
       ...
       if(fTCARandomConesOut)new ((*fTCARandomConesOut)[nCone++]) IlcAODJet(tmpRecC);
       if(fTCARandomConesOutRan)new ((*fTCARandomConesOutRan)[nConeRan++]) IlcAODJet(tmpRecC);
     ...
     IlcAODJet vTmpRanR(1,0,0,1);
     for(int i = 0; i < recParticles.GetEntries(); i++){
       IlcVParticle *vp = (IlcVParticle*)recParticles.At(i);
       if(fTCARandomConesOut){
	 for(int ir = 0;ir < fNRandomCones;ir++){
	   IlcAODJet *jC = (IlcAODJet*)fTCARandomConesOut->At(ir);  
	   if(jC&&jC->DeltaR(vp)<fRparam){
	     if(vp->Pt()>fMaxTrackPtInJet)jC->SetTrigger(IlcAODJet::kHighTrackPtTriggered);
	 ...
	 if(fTCARandomConesOutRan){
	   for(int ir = 0;ir < fTCARandomConesOutRan->GetEntriesFast();ir++){
	     IlcAODJet *jC = (IlcAODJet*)fTCARandomConesOutRan->At(ir);  
	     if(jC&&jC->DeltaR(&vTmpRanR)<fRparam){
	       if(vTmpRanR.Pt()>fMaxTrackPtInJet)jC->SetTrigger(IlcAODJet::kHighTrackPtTriggered);
	 ...
	 IlcAODJet *rC = (IlcAODJet*)fTCARandomConesOut->At(ir);
	 if(rC){
     ...
     if(fTCARandomConesOutRan){
       for(int ir = 0;ir < fTCARandomConesOutRan->GetEntriesFast();ir++){
	 IlcAODJet* rC = (IlcAODJet*)fTCARandomConesOutRan->At(ir);
	 // same wit random
	 if(rC){
	   Double_t etaC = rC->Eta();
	   Double_t phiC = rC->Phi();
  ...
  if(nTrackOver>0){
   TIterator *recIter = recParticles.MakeIterator();
   IlcVParticle *tmpRec = (IlcVParticle*)(recIter->Next());  
   ...
   for(int i = 1;i <= fh2NRecTracksPt->GetNbinsX();i++){
     ...
     while(pt<ptCut&&tmpRec){
       nTrackOver--;
       tmpRec = (IlcVParticle*)(recIter->Next()); 
       if(tmpRec){
	 pt = tmpRec->Pt();
       }
     }
   ...
   recIter->Reset();
   IlcVParticle *leading = (IlcVParticle*)recParticles.At(0);
   Float_t phi = leading->Phi();
   if(phi<0)phi+=TMath::Pi()*2.;    
   Float_t eta = leading->Eta();
   pt  = leading->Pt();
   while((tmpRec = (IlcVParticle*)(recIter->Next()))){
     Float_t tmpPt = tmpRec->Pt();
     Float_t tmpEta = tmpRec->Eta();
 ...
 if(inclusiveJetsRan.size()>0){
   IlcAODJet leadingJet (sortedJetsRan[0].px(), sortedJetsRan[0].py(), sortedJetsRan[0].pz(), sortedJetsRan[0].E());
   Float_t pt = leadingJet.Pt();
   ...
   for(int i = 1;i <= fh2NRecJetsPtRan->GetNbinsX();i++){
    ...
    Int_t nAodOutJetsRan = 0;
     IlcAODJet *aodOutJetRan = 0;
    for(int j = 0; j < nRecRan;j++){
      IlcAODJet tmpRec (sortedJetsRan[j].px(), sortedJetsRan[j].py(), sortedJetsRan[j].pz(), sortedJetsRan[j].E());
      Float_t tmpPt = tmpRec.Pt();
     ...
     if(tmpPt>fJetOutputMinPt&&fTCAJetsOutRan){
       aodOutJetRan =  new ((*fTCAJetsOutRan)[nAodOutJetsRan++]) IlcAODJet(tmpRec);
   ...
   */
   ...
   if(fTCAJetsOut){
     for(int i = 0;i < fTCAJetsOut->GetEntriesFast();i++){
       IlcAODJet *jet = (IlcAODJet*)fTCAJetsOut->At(i);
...
void IlcAnalysisTaskJetCluster::Terminate(Option_t */*option*/)
{
  //
  // Terminate analysis
  //
...
Int_t  IlcAnalysisTaskJetCluster::GetListOfTracks(TList *list,Int_t type){
  ...
  if(type==kTrackAOD || type==kTrackAODextra || type==kTrackAODextraonly){
    if(type!=kTrackAODextraonly) {
      IlcAODEvent *aod = 0;
      if(fUseAODTrackInput)aod = dynamic_cast<IlcAODEvent*>(InputEvent());
      else aod = AODEvent();
      if(!aod){
	...
	IlcAODTrack *tr = aod->GetTrack(it);
	Bool_t bGood = false;
    ...
    for(int it = 0;it < mcEvent->GetNumberOfTracks();++it){
      ...
      IlcMCParticle* part = (IlcMCParticle*)mcEvent->GetTrack(it);
    ...
    IlcAODEvent *aod = 0;
    if(fUseAODMCInput)aod = dynamic_cast<IlcAODEvent*>(InputEvent());
    else aod = AODEvent();
    if(!aod)return iCount;
    TClonesArray *tca = dynamic_cast<TClonesArray*>(aod->FindListObject(IlcAODMCParticle::StdBranchName()));
    if(!tca)return iCount;
    for(int it = 0;it < tca->GetEntriesFast();++it){
      IlcAODMCParticle *part = (IlcAODMCParticle*)(tca->At(it));
...
void IlcAnalysisTaskJetCluster::SetMomentumResolutionHybrid(TProfile *p1, TProfile *p2, TProfile *p3) {
...
void IlcAnalysisTaskJetCluster:: SetEfficiencyHybrid(TH1 *h1, TH1 *h2, TH1 *h3) {
  //
...
Double_t IlcAnalysisTaskJetCluster::GetMomentumSmearing(Int_t cat, Double_t pt) {
...
void IlcAnalysisTaskJetCluster::FitMomentumResolution() {
  //
...
/*
Int_t IlcAnalysisTaskJetCluster::AddParticlesFastJet(TList &particles,vector<fastjet::PseudoJet> &inputParticles){
  for(int i = 0; i < particles.GetEntries(); i++){
    IlcVParticle *vp = (IlcVParticle*)particles.At(i);
    // Carefull energy is not well determined in real data, should not matter for p_T scheme?

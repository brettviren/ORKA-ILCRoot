...
#include "IlcSISConeJetFinder.h"
#include "IlcSISConeJetHeader.h"
...
ClassImp(IlcSISConeJetFinder)


//____________________________________________________________________________

IlcSISConeJetFinder::IlcSISConeJetFinder():
IlcJetFinder()
{
  // Constructor
}
...
void IlcSISConeJetFinder::FindJets()
{

  // Pick up siscone header
  IlcSISConeJetHeader *header      = (IlcSISConeJetHeader*)fHeader;
  IlcJetReaderHeader *readerheader = (IlcJetReaderHeader*)fReader->GetReaderHeader();
...
//       if (clustSeq.is_pure_ghost(corrJets[j]) == 1) continue;
	...
	cout<<"e = "<<corrJets[j].E()<<endl;
      ...
      IlcAODJet aodjet (corrJets[j].px(), corrJets[j].py(), corrJets[j].pz(), corrJets[j].E());
...
//******************************** NO BG SUBTRACTION
  ...
  if (bgMode == 0)// No BG subtraction
  {
	...
	cout<<"e = "<<jets[k].E()<<endl;
      ...
      IlcAODJet aodjet (jets[k].px(), jets[k].py(), jets[k].pz(), jets[k].E());
...
Float_t  IlcSISConeJetFinder::EtaToTheta(Float_t arg)
{
...
void IlcSISConeJetFinder::InitTask(TChain *tree)
{
...
void IlcSISConeJetFinder::WriteJHeaderToFile() const
{
  fHeader->Write();
}

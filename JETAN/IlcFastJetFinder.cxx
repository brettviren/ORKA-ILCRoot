...
ClassImp(IlcFastJetFinder)


//____________________________________________________________________________

IlcFastJetFinder::IlcFastJetFinder():
  IlcJetFinder(),
  fInputFJ(new IlcFastJetInput()),
  fJetBkg(new  IlcJetBkg())
{
  // Constructor
}
...
void IlcFastJetFinder::FindJets()
{
  ...
  if(debug)cout<<"----------in IlcFastJetFinder::FindJets() ------------------"<<endl;
  ...
  for (size_t j = 0; j < jets.size(); j++) { // loop for jets
	  ...
	  IlcAODJet aodjet (jets[j].px(), jets[j].py(), jets[j].pz(), jets[j].E());
	  ...
	  for (int i=0; i < nCon; i++)
	  {
		  ...
		  if(fromAod)
		  {
			  if(fReader->GetReaderHeader()->GetDetector()==0)
			  {
				  ...
				  {
					  if(iref==ind[i]){
						  IlcAODTrack * track = (IlcAODTrack*)refs->At(iref);
				  ...
				  {
					  IlcJetUnitArray *uArray = (IlcJetUnitArray*)fUnit->At(ii);
					  if(uArray->GetUnitEnergy()>0.){
						  ...
						  if(ipart==ind[i]){
							  ...
							  for(int ji = 0; ji < tracksInCell; ji++){
								  IlcAODTrack * track = (IlcAODTrack*)trackArray->At(ji);
								  aodjet.AddTrack(track);
							  }
...
//____________________________________________________________________________
void IlcFastJetFinder::RunTest(const char* datafile)
...
void IlcFastJetFinder::WriteJHeaderToFile() const
{
  fHeader->Write();
}
...
Float_t  IlcFastJetFinder::EtaToTheta(Float_t arg)
{
...
void IlcFastJetFinder::InitTask(TChain *tree)
{
...
Bool_t IlcFastJetFinder::ProcessEvent()
{
  //
  // Process one event
  // from meomntum array
...
Bool_t IlcFastJetFinder::ProcessEvent2()
{
  //
  // Process one event

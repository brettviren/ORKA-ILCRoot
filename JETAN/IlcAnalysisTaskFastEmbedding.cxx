...
#include "IlcAnalysisTaskFastEmbedding.h"
...
ClassImp(IlcAnalysisTaskFastEmbedding)

//__________________________________________________________________________
IlcAnalysisTaskFastEmbedding::IlcAnalysisTaskFastEmbedding()
: IlcAnalysisTaskSE()
,fESD(0)
,fAODout(0)
,fAODevent(0)
...
,fAODPathArray(0)
,fAODEntriesArray(0)
,fAODPath("IlcAOD.root")
,fAODEntries(-1)
,fAODEntriesSum(0)
,fAODEntriesMax(0)
,fOfflineTrgMask(IlcVEvent::kAny)
,fMinContribVtx(1)
,fVtxZMin(-8.)
,fVtxZMax(8.)
,fEvtClassMin(0)
...
//__________________________________________________________________________
IlcAnalysisTaskFastEmbedding::IlcAnalysisTaskFastEmbedding(const char *name)
...
//__________________________________________________________________________
IlcAnalysisTaskFastEmbedding::IlcAnalysisTaskFastEmbedding(const IlcAnalysisTaskFastEmbedding &copy)
: IlcAnalysisTaskSE()
,fESD(copy.fESD)
...
//__________________________________________________________________________
IlcAnalysisTaskFastEmbedding& IlcAnalysisTaskFastEmbedding::operator=(const IlcAnalysisTaskFastEmbedding& o)
{
   // assignment

   if(this!=&o){
      IlcAnalysisTaskSE::operator=(o);
      fESD               = o.fESD;
      fAODout            = o.fAODout;
...
//__________________________________________________________________________
void IlcAnalysisTaskFastEmbedding::UserCreateOutputObjects()
{
   // create output objects
   if(fDebug > 1) Printf("IlcAnalysisTaskFastEmbedding::UserCreateOutputObjects()");
   IlcLog::SetClassDebugLevel("IlcAnalysisTaskFastEmbedding", IlcLog::kInfo);
   ...
   IlcInfo(Form("TRandom3 seed: %d", rndm->GetSeed()));
   ...
   fAODout = AODEvent();
   if(!fAODout){
      IlcError("Output AOD not found.");
      PostData(1, fHistList);
      return;
   }
   if(!fAODout->FindListObject(fTrackBranch.Data()) && strlen(fTrackBranch.Data())){
      IlcInfo(Form("Add AOD branch %s", fTrackBranch.Data()));
      TClonesArray *tracks = new TClonesArray("IlcAODTrack",0);
   ...
   if(fAODevent && fAODevent->FindListObject("mcparticles") && strlen(fMCparticlesBranch.Data())){
      IlcInfo(Form("Add AOD branch %s", fMCparticlesBranch.Data()));
      TClonesArray *mcparticles = new TClonesArray("IlcAODMCParticle",0);
...
//__________________________________________________________________________
void IlcAnalysisTaskFastEmbedding::Init()
{
   // Initialization
   if(fDebug > 1) Printf("IlcAnalysisTaskFastEmbedding::Init()");

}

//__________________________________________________________________________
Bool_t IlcAnalysisTaskFastEmbedding::UserNotify()
{
   // User defined Notify(), called once
   if(fDebug > 1) Printf("IlcAnalysisTaskFastEmbedding::UserNotify()");

   // get total nb of events in tree (of this subjob)
   IlcInputEventHandler* inputHandler = (IlcInputEventHandler*)((IlcAnalysisManager::GetAnalysisManager())->GetInputEventHandler());
   fInputEntries = inputHandler->GetTree()->GetEntriesFast();
   IlcInfo(Form("Total nb. of events: %d", fInputEntries));
...
//__________________________________________________________________________
void IlcAnalysisTaskFastEmbedding::UserExec(Option_t *)
{
   // Analysis, called once per event
   if(fDebug > 1) Printf("IlcAnalysisTaskFastEmbedding::UserExec()");

   if(!fAODout){
      IlcError("Need output AOD, but is not connected."); 
      PostData(1, fHistList);
      return;
   }
   ...
   // physics selection
   IlcInputEventHandler* inputHandler = (IlcInputEventHandler*)
   ...
   // vertex selection
   IlcAODVertex* primVtx = fAODout->GetPrimaryVertex();
   ...
   Int_t eventClass = IlcAnalysisHelperJetTasks::EventClass();
   ...
   */
   ...
   if(!tracks){
      IlcError("Extra track branch not found in output.");
      PostData(1, fHistList);
      return;
   }
   tracks->Delete();
   ...
   if(fEmbedMode==kAODFull || fEmbedMode==kAODJetTracks || fEmbedMode==kAODJet4Mom){
      if(!fAODevent){
         IlcError("Need input AOD, but is not connected."); 
         PostData(1, fHistList);
         return;
      }
      ...
      while(!useEntry){  // protection need, if no event fulfills requierment
         ...
         if(fCountEvents>=fNevents){ 
            if(!fAODPathArray){
               IlcDebug(IlcLog::kDebug, "No list of AODs, keep current AOD.");
            } 
            else {
               IlcDebug(IlcLog::kDebug, "Select new AOD file ...");
               ...
               if(fAODEntries!=fNevents){
                  IlcError("File with list of AODs and file with nb. of entries inconsistent");
                  PostData(1, fHistList);
                  return;
               }
            }
         ...
         if(mcHeader){
            ...
            IlcDebug(IlcLog::kDebug,Form("pt hard %.2f", fPtHard));
         }
         else{
            IlcDebug(IlcLog::kDebug,"No mcHeader");
         }
         ...
         if(fEvtSelecMode==kEventsJetPt){
            ...
            for(Int_t ij=0; ij<nJets; ++ij){
               IlcAODJet *jet = dynamic_cast<IlcAODJet*>(fAODJets->At(ij));
               if(!jet) continue;
      ...
      IlcDebug(IlcLog::kDebug,Form("Use entry %d from extra AOD.", fAODEntry));
      ...
      if(mcpartIN){
         ...
         if(!mcpartOUT){
            IlcError("Extra MC particles branch not found in output.");
            PostData(1, fHistList);
            return;
         }
         mcpartOUT->Delete();
      } else {
         IlcInfo("No extra MC particles found.");
      }
      ...
      if(fEmbedMode==kAODFull || fEmbedMode==kAODJetTracks){ // take all tracks or jet tracks
         ...
         IlcAODJet *leadJet = 0x0; // used for jet tracks only
         ...
         for(Int_t it=0; it<nTracks; ++it){
            IlcAODTrack *tmpTr = 0x0;
            if(fEmbedMode==kAODFull)      tmpTr = fAODevent->GetTrack(it);
            if(fEmbedMode==kAODJetTracks) tmpTr = dynamic_cast<IlcAODTrack*>(leadJet->GetRefTracks()->At(it));
            if(!tmpTr) continue;

            tmpTr->SetStatus(IlcESDtrack::kEmbedded);

            new ((*tracks)[nAODtracks++]) IlcAODTrack(*tmpTr); 
         ...
         if(mcpartIN){
            ...
            for(Int_t ip=0; ip<nMCpart; ++ip){
               IlcAODMCParticle *tmpPart = (IlcAODMCParticle*) mcpartIN->At(ip);
               ...
               new((*mcpartOUT)[nAODmcpart++]) IlcAODMCParticle(*tmpPart);
      ...
      if(fEmbedMode==kAODJet4Mom){
         ...
         for(Int_t ij=0; ij<nJets; ++ij){
            IlcAODJet *jet = dynamic_cast<IlcAODJet*>(fAODJets->At(ij));
            if(!jet) continue;
            IlcAODTrack *tmpTr = (IlcAODTrack*)jet;
            tmpTr->SetFlags(IlcESDtrack::kEmbedded);
   ...
   if(fEmbedMode==kToyTracks){
      ...
      for(Int_t i=0; i<nT; ++i){
         ...
         IlcAODTrack *tmpTr = new IlcAODTrack( -999,   // id
         -999,   // label
         mom,    // momentum[3]
         kFALSE, // cartesian
         xyz,    // position
...
//__________________________________________________________________________
void IlcAnalysisTaskFastEmbedding::Terminate(Option_t *)
{
   // terminate
   if(fDebug > 1) Printf("IlcAnalysisTaskFastEmbedding::Terminate()");
...
//__________________________________________________________________________
Int_t IlcAnalysisTaskFastEmbedding::GetJobID()
{
   ...
   if(env && strlen(env)){
      id= atoi(env);
      IlcInfo(Form("Job index %d", id));
   }
   else{
      IlcInfo("Job index not found. Okay if running locally.");
   }
...
//__________________________________________________________________________
Int_t IlcAnalysisTaskFastEmbedding::SelectAODfile()
{
   ...
   fAODEntry = (Int_t)(rndm->Uniform(fAODEntries));
      ...
      IlcWarning("Number of entries in extra AODs not set!");
      n = rndm->Integer(nFiles);
      fAODEntry = 0;
   }

   IlcInfo(Form("Select AOD file %d", n));
   TObjString *objStr = (TObjString*) fAODPathArray->At(n);
   if(!objStr){
      IlcError("Could not get path of aod file.");
      return -1;
   }
...
//__________________________________________________________________________
Int_t IlcAnalysisTaskFastEmbedding::OpenAODfile(Int_t trial)
{
   ...
   if(!fAODfile){
      ...
      if(fAODPathArray){
         if(trial<=3){ 
            IlcError(Form("Could not open AOD file %s (trial %d), try again ...", fAODPath.Data(), trial));
            fFileId = OpenAODfile(trial+1);
         } else {
            IlcError(Form("Could not open AOD file %s, give up ...",fAODPath.Data()));
            return -1;
         }
      } else {
         IlcError(Form("Could not open AOD file %s.", fAODPath.Data()));
      }
   ...
   */
   ...
   mcHeader = (IlcAODMCHeader*)fAODevent->FindListObject(IlcAODMCHeader::StdBranchName());
   ...
   if(!fAODJets){
      IlcError("Could not find jets in AOD. Check jet branch when indicated.");
      return -1;
   }

   TFile *curfile = fAODtree->GetCurrentFile();
   if (!curfile) {
      IlcError("No current file.");
      return -1;
   }
   ...
   if(fFileId>=0){
      IlcInfo(Form("Read successfully AOD event from file %d",fFileId));
   }
...
Float_t IlcAnalysisTaskFastEmbedding::GetPtHard(Bool_t bSet, Float_t newValue){
...
//____________________________________________________________________________
Int_t IlcAnalysisTaskFastEmbedding::GetPtHardBin(Double_t ptHard){
...
//____________________________________________________________________________
Bool_t IlcAnalysisTaskFastEmbedding::PythiaInfoFromFile(const char* currFile,Float_t &fXsec,Float_t &fTrials){
   //
   // get the cross section and the trails either from pyxsec.root or from pysec_hists.root
   // This should provide the path to the AOD/ESD file
   // (taken from PWG4/JetTasks/IlcAnalysisHelperJetTasks) 
   ...
   if(!fxsec){
      ...
      if(!fxsec){
         ...
         IlcDebug(IlcLog::kDebug,Form("Neither pyxsec.root nor pyxsec_hists.root found in %s",file.Data()));
         return kFALSE;
      }
      else{

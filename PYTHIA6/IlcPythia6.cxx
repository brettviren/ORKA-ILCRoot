...
/* $Id: IlcPythia.cxx,v 1.40 2007/10/09 08:43:24 morsch Exp $ */

#include "IlcPythia6.h"
#include "IlcStack.h"
...
IlcPythia6* IlcPythia6::fgIlcPythia=NULL;

IlcPythia6::IlcPythia6():
    TPythia6(),
    IlcPythiaBase(),
    fProcess(kPyMb),
    fEcms(0.),
...
IlcPythia6::IlcPythia6(const IlcPythia6& pythia):
    TPythia6(),
    IlcPythiaBase(),
    fProcess(kPyMb),
    fEcms(0.),
...
void IlcPythia6::ProcInit(Process_t process, Float_t energy, StrucFunc_t strucfunc)
{
...
//...Switch off decay of pi0, K0S, Lambda, Sigma+-, Xi0-, Omega-.
    ...
    SetMSTP(52,2);
    SetMSTP(51,IlcStructFuncType::PDFsetIndex(strucfunc));
...
//   
	...
	SetMSUB(94,1);             // double diffraction
	SetMSUB(95,1);	           // low pt production
	SetMSTP(51,IlcStructFuncType::PDFsetIndex(kCTEQ6ll));
	SetMSTP(52,2);
	SetMSTP(68,1);
	SetMSTP(70,2);
...
Int_t IlcPythia6::CheckedLuComp(Int_t kf)
{
...
void IlcPythia6::SetNuclei(Int_t a1, Int_t a2)
{
...
IlcPythia6* IlcPythia6::Instance()
{ 
// Set random number generator 
    if (fgIlcPythia) {
	return fgIlcPythia;
    } else {
	fgIlcPythia = new IlcPythia6();
	return fgIlcPythia;
    }
}

void IlcPythia6::PrintParticles()
{ 
...
void  IlcPythia6::ResetDecayTable()
{
...
void  IlcPythia6::Pyjoin(Int_t& npart, Int_t *ipart)
{
...
void  IlcPythia6::Pyshowq(Int_t ip1, Int_t ip2, Double_t qmax)
{
...
void  IlcPythia6::Pyclus(Int_t& njet)
{
...
void  IlcPythia6::Pycell(Int_t& njet)
{
...
void IlcPythia6::GetJet(Int_t i, Float_t& px, Float_t& py, Float_t& pz, Float_t& e)
{
    // Get jet number i
    Int_t n = GetN();
...
void  IlcPythia6::Pyshow(Int_t ip1, Int_t ip2, Double_t qmax)
{
...
void IlcPythia6::Pyrobo(Int_t imi, Int_t ima, Double_t the, Double_t phi, Double_t bex, Double_t bey, Double_t bez)
{
...
void IlcPythia6::InitQuenching(Float_t cMin, Float_t cMax, Float_t k, Int_t iECMethod, Float_t zmax, Int_t ngmax)
{
// Initializes 
...
//     
    ...
    fGlauber = IlcFastGlauber::Instance();
    fGlauber->Init(2);
    fGlauber->SetCentralityClass(cMin, cMax); 

    fQuenchingWeights = new IlcQuenchingWeights();
    fQuenchingWeights->InitMult();
    fQuenchingWeights->SetK(k);
    fQuenchingWeights->SetECMethod(IlcQuenchingWeights::kECMethod(iECMethod));
    fNGmax = ngmax;
    fZmax  = zmax;
...
/*    
		...
		Double_t r       = IlcPythiaRndm::GetPythiaRandom()->Rndm();
		Double_t jtKick  = 0.3 * TMath::Sqrt(-TMath::Log(r));
		if (ish == 2)   jtKick  = wjtKick[i] * TMath::Sqrt(-TMath::Log(r)) / TMath::Sqrt(2.);
		Double_t phiKick = 2. * TMath::Pi() * IlcPythiaRndm::GetPythiaRandom()->Rndm();
...
void IlcPythia6::Pyquen(Double_t a, Int_t ibf, Double_t b)
{
...
void IlcPythia6::SetPyquenParameters(Double_t t0, Double_t tau0, Int_t nf, Int_t iengl, Int_t iangl)
{
...
void  IlcPythia6::LoadEvent(IlcStack* stack, Int_t flag, Int_t reHadr)
{
//
...
void IlcPythia6::Pyevnw()
{
    // New multiple interaction scenario
    pyevnw();
}

void IlcPythia6::GetQuenchingParameters(Double_t& xp, Double_t& yp, Double_t z[4])
{
...
void IlcPythia6::ConfigHeavyFlavor()
{
    //
...
void IlcPythia6::AtlasTuning()
{
    //
    // Configuration for the ATLAS tuning
	SetMSTP(51,IlcStructFuncType::PDFsetIndex(kCTEQ5L));
	SetMSTP(81,1);             // Multiple Interactions ON
	SetMSTP(82,4);             // Double Gaussian Model
...
void IlcPythia6::AtlasTuningMC09()
{
    //
    // Configuration for the ATLAS tuning
...
void IlcPythia6::SetPtHardRange(Float_t ptmin, Float_t ptmax)
{
...
void IlcPythia6::SetYHardRange(Float_t ymin, Float_t ymax)
{
...
void IlcPythia6::SetFragmentation(Int_t flag)
{
    // Switch fragmentation on/off
    SetMSTP(111, flag);
}

void IlcPythia6::SetInitialAndFinalStateRadiation(Int_t flag1, Int_t flag2)
{
...
void IlcPythia6::SetIntrinsicKt(Float_t kt)
{
...
void IlcPythia6::SetPycellParameters(Float_t etamax, Int_t neta, Int_t nphi,
...
void IlcPythia6::ModifiedSplitting()
{
...
void IlcPythia6::SwitchHadronisationOff()
{
    // Switch off hadronisarion
    SetMSTJ(1, 0);
}

void IlcPythia6::SwitchHadronisationOn()
{
...
void IlcPythia6::GetXandQ(Float_t& x1, Float_t& x2, Float_t& q)
{
...
Float_t IlcPythia6::GetXSection()
{
    // Get the total cross-section
    return (GetPARI(1));
}

Float_t IlcPythia6::GetPtHard()
{
    // Get the pT hard for this event
    return GetVINT(47);
}

Int_t IlcPythia6::ProcessCode()
{
    // Get the subprocess code
    return GetMSTI(1);
}

void IlcPythia6::PrintStatistics()
{
    // End of run statistics
    Pystat(1);
}

void IlcPythia6::EventListing()
{
    // End of run statistics
    Pylist(2);
}

IlcPythia6& IlcPythia6::operator=(const  IlcPythia6& rhs)
{
// Assignment operator
    rhs.Copy(*this);
    return *this;
 ...
 void IlcPythia6::Copy(TObject&) const
{
    //
    // Copy 
    //

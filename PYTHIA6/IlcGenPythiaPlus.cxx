...
ClassImp(IlcGenPythiaPlus)


IlcGenPythiaPlus::IlcGenPythiaPlus():
    IlcGenMC(),
    fPythia(0),
    fProcess(kPyCharm),          
    fStrucFunc(kCTEQ5L), 
...
IlcGenPythiaPlus::IlcGenPythiaPlus(IlcPythiaBase* pythia)
    :IlcGenMC(-1),
     fPythia(pythia),
     fProcess(kPyCharm),          
     fStrucFunc(kCTEQ5L), 
...
void IlcGenPythiaPlus::SetInteractionRate(Float_t rate,Float_t timewindow)
{
...
void IlcGenPythiaPlus::GeneratePileup()
{
...
void IlcGenPythiaPlus::SetPycellParameters(Float_t etamax, Int_t neta, Int_t nphi,
...
void IlcGenPythiaPlus::SetEventListRange(Int_t eventFirst, Int_t eventLast)
{
...
//  pt - kick
    ...
    if (fReadFromFile) {
	fRL  =  IlcRunLoader::Open(fFileName, "Partons");
	fRL->LoadKinematics();
	fRL->LoadHeader();
    } else {
	fRL = 0x0;
...
void IlcGenPythiaPlus::Generate()
{
// Generate one event
...
Int_t  IlcGenPythiaPlus::GenerateMB()
{
//
...
// store track information
		...
		if (fStack) {
		    particle = fStack->Particle(nt);
  		    ...
  		    particle = IlcRunLoader::Instance()->Stack()->Particle(nt);
		}
...
void IlcGenPythiaPlus::FinishRun()
{
...
void IlcGenPythiaPlus::AdjustWeights() const
{
...
void IlcGenPythiaPlus::SetNuclei(Int_t a1, Int_t a2)
{
...
void IlcGenPythiaPlus::MakeHeader()
{
//
...
// 
  if (gIlc) {
	...
	gIlc->GetEvNumber()<=fDebugEventLast) fPythia->EventListing();
  }
...
//
// Event type  
    ((IlcGenPythiaEventHeader*) fHeader)->SetProcessType(fPythia->ProcessCode());
//
// Number of trials
...
//
// Store pt^hard 
    ((IlcGenPythiaEventHeader*) fHeader)->SetPtHard(fPythia->GetPtHard());
//
//  Pass header
//
    AddHeader(fHeader);
...
Bool_t IlcGenPythiaPlus::CheckTrigger(const TParticle* jet1, const TParticle* jet2)
{
...
Bool_t IlcGenPythiaPlus::CheckKinematicsOnChild(){
//
...
void IlcGenPythiaPlus::RecJetsUA1(Int_t& njets, Float_t jets [4][50])
{
//
...
void  IlcGenPythiaPlus::GetJets(Int_t& nJets, Int_t& nJetsTrig, Float_t jets[4][10])
{
//
...
void IlcGenPythiaPlus::GetSubEventTime()
{
...
Bool_t IlcGenPythiaPlus::IsInEMCAL(Float_t phi, Float_t eta) const
{
...
Bool_t IlcGenPythiaPlus::IsInPHOS(Float_t phi, Float_t eta) const
{
...
void IlcGenPythiaPlus::RotatePhi(Int_t iphcand, Bool_t& okdd)
{
...
#ifdef never
void IlcGenPythiaPlus::Streamer(TBuffer &R__b)
{
   // Stream an object of class IlcGenPythia.

   if (R__b.IsReading()) {
      Version_t R__v = R__b.ReadVersion(); if (R__v) { }
      IlcGenerator::Streamer(R__b);
      R__b >> (Int_t&)fProcess;
      ...
      R__b >> fXsection;
//      (IlcPythia::Instance())->Streamer(R__b);
      R__b >> fPtHardMin;
      R__b >> fPtHardMax;
//      if (fDecayer) fDecayer->Streamer(R__b);
   } else {
      R__b.WriteVersion(IlcGenPythiaPlus::IsA());
      IlcGenerator::Streamer(R__b);
      R__b << (Int_t)fProcess;
      R__b << (Int_t)fStrucFunc;
      R__b << (Int_t)fForceDecay;

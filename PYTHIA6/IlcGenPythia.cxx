...
/* $Id: IlcGenPythia.cxx 58467 2012-09-05 15:56:33Z hristov $ */
...
ClassImp(IlcGenPythia)


IlcGenPythia::IlcGenPythia():
    IlcGenMC(),
    fProcess(kPyCharm),          
    fItune(-1),
    fStrucFunc(kCTEQ5L), 
...
IlcGenPythia::IlcGenPythia(Int_t npart)
    :IlcGenMC(npart),
     fProcess(kPyCharm),          
     fItune(-1),
     fStrucFunc(kCTEQ5L), 
...
void IlcGenPythia::SetInteractionRate(Float_t rate,Float_t timewindow)
{
...
void IlcGenPythia::GeneratePileup()
{
// Generate sub events time for pileup
    ...
    while(1)
      {
	eventtime += (IlcPythiaRndm::GetPythiaRandom())->Exp(1./fInteractionRate);
...
void IlcGenPythia::SetPycellParameters(Float_t etamax, Int_t neta, Int_t nphi,
...
void IlcGenPythia::SetEventListRange(Int_t eventFirst, Int_t eventLast)
{
...
void IlcGenPythia::Init()
{
// Initialisation
    
    SetMC(IlcPythia::Instance());
    fPythia=(IlcPythia*) fMCEvGen;
...
//  pt - kick
    ...
    if (fReadFromFile) {
	fRL  =  IlcRunLoader::Open(fkFileName, "Partons");
	fRL->LoadKinematics();
	fRL->LoadHeader();
    } else {
	fRL = 0x0;
 ...
 //
    ...
    //  Forward Paramters to the IlcPythia object
...
// Switch off Heavy Flavors on request  
	...
	((IlcDecayerPythia*) fDecayer)->HeavyFlavourOff();
    }
...
//  
	...
	IlcFastGlauber* glauber = IlcFastGlauber::Instance();
	glauber->Init(2);
...
void IlcGenPythia::Generate()
{
// Generate one event
    if (!fPythia) fPythia=(IlcPythia*) fMCEvGen;
    fDecayer->ForceDecay();
...
//
	...
	if (fQuench ==4){
	    ...
	    IlcFastGlauber::Instance()->GetRandomBHard(bimp);
...
//	
	if (!fReadFromFile) {
	    ...
	    printf("Loading Event %d\n",IlcRunLoader::Instance()->GetEventNumber());
	    fRL->GetEvent(IlcRunLoader::Instance()->GetEventNumber());
	    fPythia->SetN(0);
...
//  converts from mm/c to s
  ...
  if ((fFragPhotonInCalo || fPi0InCalo || fHadronInCalo) && 
    ...
    for (i=0; i< np; i++) {
      ...
      if(ok && iparticle->Pt() > fTriggerParticleMinPt)
      {
          ...
          {
            ok =kTRUE;
            IlcDebug(1,Form("Selected trigger pdg %d, status %d, pt %2.2f, eta %2.2f, phi %2.2f\n",pdg,status,iparticle->Pt(), eta, phi));
            break;
          }
      }
      else ok = kFALSE;
  ...
  // Select beauty jets with electron in EMCAL
	...
	return 0;
      ...
      IlcDebug(5,Form("Found an electron jet (pt,eta,phi) = (%f,%f,%f)",pt,eta,phi));
    }
...
void IlcGenPythia::AdjustWeights() const
{
// Adjust the weights after generation of all events
//
    if (gIlc) {
	TParticle *part;
	Int_t ntrack=gIlc->GetMCApp()->GetNtrack();
	for (Int_t i=0; i<ntrack; i++) {
	    part= gIlc->GetMCApp()->Particle(i);
	    part->SetWeight(part->GetWeight()*fKineBias);
	}
    }
}
    
void IlcGenPythia::SetNuclei(Int_t a1, Int_t a2, Int_t pdfset)
{
...
// 
  if (gIlc) {
	...
	gIlc->GetEvNumber()<=fDebugEventLast) fPythia->Pylist(2);
  }

// Builds the event header, to be called after each event
    if (fHeader) delete fHeader;
    fHeader = new IlcGenPythiaEventHeader("Pythia");
//
// Event type  
    if(fProcDiff>0){
      //      if(fProcDiff == 92 || fProcDiff == 93) printf("\n\n\n\n\n");
      //      printf("fPythia->GetMSTI(1) = %d   fProcDiff = %d\n",fPythia->GetMSTI(1), fProcDiff);
    ((IlcGenPythiaEventHeader*) fHeader)->SetProcessType(fProcDiff);
    }
    else 
    ((IlcGenPythiaEventHeader*) fHeader)->SetProcessType(fPythia->GetMSTI(1));
//
// Number of trials
    ((IlcGenPythiaEventHeader*) fHeader)->SetTrials(fTrials);
//
// Event Vertex 
...
// Jets that have triggered
	...
	for (Int_t i = 0; i < ntrig; i++) {
	    ((IlcGenPythiaEventHeader*) fHeader)->AddJet(jets[0][i], jets[1][i], jets[2][i], 
							jets[3][i]);
	}
    }
//
...
//
    ...
    if (fRL) {
	IlcGenPythiaEventHeader* exHeader = (IlcGenPythiaEventHeader*) (fRL->GetHeader()->GenEventHeader());
	for (Int_t i = 0; i < exHeader->NTriggerJets(); i++)
	{
	    ...
	    ((IlcGenPythiaEventHeader*) fHeader)->AddUQJet(uqJet[0], uqJet[1], uqJet[2], uqJet[3]);
	}
    }
//
...
//
	...
	if (fQuench == 1) {
	    ...
	    Double_t xy[2];
	    Double_t i0i1[2];
	    IlcFastGlauber::Instance()->GetSavedXY(xy);
	    IlcFastGlauber::Instance()->GetSavedI0I1(i0i1);
	    xp = xy[0];
	    yp = xy[1];
	    ((IlcGenPythiaEventHeader*) fHeader)->SetImpactParameter(fImpact);
	}
	
	    ((IlcGenPythiaEventHeader*) fHeader)->SetXYJet(xp, yp);
	    ((IlcGenPythiaEventHeader*) fHeader)->SetZQuench(z);
    }
//
// Store pt^hard 
    ((IlcGenPythiaEventHeader*) fHeader)->SetPtHard(fPythia->GetVINT(47));
//
//  Pass header
//
    AddHeader(fHeader);
...
Bool_t IlcGenPythia::CheckTrigger(const TParticle* jet1, const TParticle* jet2)
{
...
Bool_t IlcGenPythia::CheckKinematicsOnChild(){
//
...
void  IlcGenPythia::LoadEvent(IlcStack* stack, Int_t flag, Int_t reHadr)
{
  //
...
void  IlcGenPythia::LoadEvent(const TObjArray* stack, Int_t flag, Int_t reHadr)
{
  //
  // Load event into Pythia Common Block
  //
...
void IlcGenPythia::RecJetsUA1(Int_t& njets, Float_t jets [4][50])
{
//
...
void  IlcGenPythia::GetJets(Int_t& nJets, Int_t& nJetsTrig, Float_t jets[4][10])
{
//
...
void IlcGenPythia::GetSubEventTime()
{
...
Bool_t IlcGenPythia::IsInEMCAL(Float_t phi, Float_t eta) const
{
...
Bool_t IlcGenPythia::IsInPHOS(Float_t phi, Float_t eta) const
{
...
void IlcGenPythia::RotatePhi(Int_t iphcand, Bool_t& okdd)
{
  ...
  Double_t phiPHOS = (IlcPythiaRndm::GetPythiaRandom())->Uniform(phiPHOSmin,phiPHOSmax);
...
Bool_t IlcGenPythia::CheckDiffraction()
{
  ...
  const Int_t pdgStable[20] = {
	...
	if (y > y2)
	{
	 ...
	 if(y1 > y2) 
	   ...
	   iPart = iPart1;
	   if((IlcPythiaRndm::GetPythiaRandom())->Uniform(0.,1.)>0.5) iPart = iPart2;
	 }
       }
     else if (pdg1 == 2212)
       iPart = iPart1;
     ...
     if(proc==1 && (IlcPythiaRndm::GetPythiaRandom())->Uniform(0.,1.) > wDD) return kFALSE;
     if(proc==2 && (IlcPythiaRndm::GetPythiaRandom())->Uniform(0.,1.) > wND) return kFALSE;
    ...
    if(wSD<0)  IlcError("wSD<0 ! \n");

    if((IlcPythiaRndm::GetPythiaRandom())->Uniform(0.,1.)> wSD) return kFALSE;
...
Bool_t IlcGenPythia::GetWeightsDiffraction(Double_t M, Double_t &Mmin, Double_t &Mmax, 
...
Bool_t IlcGenPythia::IsFromHeavyFlavor(Int_t ipart)
{

...
/* $Id: IlcPIPEv0.cxx 49869 2011-05-18 04:49:51Z hristov $ */
...
//_____________________________________________________________________________
IlcPIPEv0::IlcPIPEv0(const char *name, const char *title)
    : IlcPIPE(name,title),
      fPipeMaterial(kBe)   
{
// Constructor
...
//End_Html
    ...
    IlcDebugClass(1,"Create PIPEv0 geometry");
    ...
    Int_t   idrotm[2099];  
    IlcMatrix(idrotm[2001],90.,240.,  0.,  0., 90.,150.);
    IlcMatrix(idrotm[2002],90.,  0.,  0.,  0., 90.,270.);
    IlcMatrix(idrotm[2003],90.,120.,  0.,  0., 90., 30.);
    IlcMatrix(idrotm[2004],90.,315., 90., 45.,  0.,  0.);
    IlcMatrix(idrotm[2005],90.,270., 90.,  0.,  0.,  0.);
    IlcMatrix(idrotm[2006],90.,225., 90.,315.,  0.,  0.);
    IlcMatrix(idrotm[2007],90.,180., 90.,270.,  0.,  0.);
    IlcMatrix(idrotm[2008],90.,135., 90.,225.,  0.,  0.);
    IlcMatrix(idrotm[2009],90., 90., 90.,180.,  0.,  0.);
    IlcMatrix(idrotm[2010],90., 45., 90.,135.,  0.,  0.);
    idrotm[2011] = 0;
    IlcMatrix(idrotm[2012],90.,180., 90., 90.,180.,  0.);
    IlcMatrix(idrotm[2013],90.,  0., 90., 90.,180.,  0.);
//
//  Bellow
//
...
//    gMC->Gspos("QBT1", 1, "QBPM", 0., 0.,  zpos, 0, "ONLY");
    ...
    gMC->Gspos("QA28", 1, "ILCM", 0.0, 0.0, zpos, 0, "ONLY"); 
...
// --- Place the PIPE ghost volume (QBPM) in its mother volume (ILCM)
//    by rotating it to 180 deg. and make it invisible
// 
    gMC->Gspos("QBPM",1,"ILCM", 0, 0, 0, 0, "ONLY");
...
//___________________________________________
void IlcPIPEv0::CreateMaterials()
{
  //
  // Define materials for beam pipe
  //

  IlcDebugClass(1,"Create PIPEv0 materials");
  ...
  IlcMedium(35, "AIR_HIFG",35, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Vacuum 

...
/* $Id: IlcFRAMEv2.cxx 54863 2012-02-28 12:10:29Z hristov $ */
...
//_____________________________________________________________________________
IlcFRAMEv2::IlcFRAMEv2(const char *name, const char *title)
    : IlcFRAME(name,title), 
      fHoles(0)
{
// Constructor
...
void IlcFRAMEv2::CreateGeometry()
{
//Begin_Html
/*
...
//End_Html
  ...
  IlcMatrix(idrotm[2070],  90.0,   0.0,  90.0, 270.0,   0.0,   0.0);  
//
  IlcMatrix(idrotm[2083], 170.0,   0.0,  90.0,  90.0,  80.0,   0.0);
  IlcMatrix(idrotm[2084], 170.0, 180.0,  90.0,  90.0,  80.0, 180.0);
  IlcMatrix(idrotm[2085],  90.0, 180.0,  90.0,  90.0,   0.0,   0.0);
//  
  IlcMatrix(idrotm[2086],  90.0,   0.0, 90.0,   90.,   0.0,   0.0);
  IlcMatrix(idrotm[2087],  90.0, 180.0, 90.0,  270.,   0.0,   0.0);
  IlcMatrix(idrotm[2088],  90.0,  90.0, 90.0,  180.,   0.0,   0.0);
  IlcMatrix(idrotm[2089],  90.0,  90.0, 90.0,    0.,   0.0,   0.0);
//
  IlcMatrix(idrotm[2090],  90.0,   0.0,   0.0,    0.,   90.0, 90.0);
  IlcMatrix(idrotm[2091],   0.0,   0.0,  90.0,   90.,   90.0,  0.0);
//
// Matrices have been imported from Euclid. Some simplification
// seems possible
//

  IlcMatrix(idrotm[2003],   0.0, 0.0, 90.0, 130.0, 90.0,  40.0);
  IlcMatrix(idrotm[2004], 180.0, 0.0, 90.0, 130.0, 90.0,  40.0);
  IlcMatrix(idrotm[2005], 180.0, 0.0, 90.0, 150.0, 90.0, 240.0);
  IlcMatrix(idrotm[2006],   0.0, 0.0, 90.0, 150.0, 90.0, 240.0);
  IlcMatrix(idrotm[2007],   0.0, 0.0, 90.0, 170.0, 90.0,  80.0);
  IlcMatrix(idrotm[2008], 180.0, 0.0, 90.0, 190.0, 90.0, 280.0);
  IlcMatrix(idrotm[2009], 180.0, 0.0, 90.0, 170.0, 90.0,  80.0);
  IlcMatrix(idrotm[2010],   0.0, 0.0, 90.0, 190.0, 90.0, 280.0);
  IlcMatrix(idrotm[2011],   0.0, 0.0, 90.0, 350.0, 90.0, 260.0);
  IlcMatrix(idrotm[2012], 180.0, 0.0, 90.0, 350.0, 90.0, 260.0);
  IlcMatrix(idrotm[2013], 180.0, 0.0, 90.0,  10.0, 90.0, 100.0);
  IlcMatrix(idrotm[2014],   0.0, 0.0, 90.0,  10.0, 90.0, 100.0);
  IlcMatrix(idrotm[2015],   0.0, 0.0, 90.0,  30.0, 90.0, 300.0);
  IlcMatrix(idrotm[2016], 180.0, 0.0, 90.0,  30.0, 90.0, 300.0);
  IlcMatrix(idrotm[2017], 180.0, 0.0, 90.0,  50.0, 90.0, 140.0);
  IlcMatrix(idrotm[2018],   0.0, 0.0, 90.0,  50.0, 90.0, 140.0);

  IlcMatrix(idrotm[2019], 180.0, 0.0, 90.0, 130.0, 90.0, 220.0);
  IlcMatrix(idrotm[2020], 180.0, 0.0, 90.0,  50.0, 90.0, 320.0);
  IlcMatrix(idrotm[2021], 180.0, 0.0, 90.0, 150.0, 90.0,  60.0);
  IlcMatrix(idrotm[2022], 180.0, 0.0, 90.0,  30.0, 90.0, 120.0);
  IlcMatrix(idrotm[2023], 180.0, 0.0, 90.0, 170.0, 90.0, 260.0);
  IlcMatrix(idrotm[2024], 180.0, 0.0, 90.0, 190.0, 90.0, 100.0);
  IlcMatrix(idrotm[2025], 180.0, 0.0, 90.0, 350.0, 90.0,  80.0);
  IlcMatrix(idrotm[2026], 180.0, 0.0, 90.0,  10.0, 90.0, 280.0);
  
  IlcMatrix(idrotm[2027],   0.0, 0.0, 90.0,  50.0, 90.0, 320.0);
  IlcMatrix(idrotm[2028],   0.0, 0.0, 90.0, 150.0, 90.0,  60.0); 
  IlcMatrix(idrotm[2029],   0.0, 0.0, 90.0,  30.0, 90.0, 120.0);
  IlcMatrix(idrotm[2030],   0.0, 0.0, 90.0,  10.0, 90.0, 280.0);
  IlcMatrix(idrotm[2031],   0.0, 0.0, 90.0, 170.0, 90.0, 260.0);
  IlcMatrix(idrotm[2032],   0.0, 0.0, 90.0, 190.0, 90.0, 100.0);
  IlcMatrix(idrotm[2033],   0.0, 0.0, 90.0, 350.0, 90.0,  80.0);
...
//
  ...
  gMC->Gspos("B077", 1, "ILCM", 0., 0., 0., 0, "ONLY");
//
...
//
  ...
  IlcMatrix(idrotm[2001], -theta+180, 0.0, 90.0, 90.0, 90.-theta, 0.0); 
  ...
  IlcMatrix(idrotm[2002], -theta, 0.0, 90.0, 90.0, 270.-theta, 0.0);
...
//
  ...
  for (i = 0; i < 18; i++) {
      ...
      IlcMatrix(idrotm[2034+i],  90.0, phi1, 0., 0., 90., phi2);  
...
//
      ...
      phi1 = i*20.+10;
      phi2 = 270+phi1;
      IlcMatrix(idrotm[2052+i],  90.0, phi1, 90., phi2, 0., 0.);  
...
//
  ...
  gMC->Gspos("BRS1", 1, "ILCM", -430.-3.,    -190., 0., 0, "ONLY");
  gMC->Gspos("BRS1", 2, "ILCM",  430.+3.,    -190., 0., 0, "ONLY");
  ...
  gMC->Gspos("BRS4", 1, "ILCM",  430.+3.,    -190.+55./2.+rbox[1],  224., 0, "ONLY");
  gMC->Gspos("BRS4", 2, "ILCM",  430.+3.,    -190.+55./2.+rbox[1], -224., 0, "ONLY");
//  gMC->Gspos("BRS4", 3, "ILCM", -430.+3,    -180.+55./2.+rbox[1],  224., 0, "ONLY");
//  gMC->Gspos("BRS4", 4, "ILCM", -430.+3,    -180.+55./2.+rbox[1], -224., 0, "ONLY");
  ...
  for (i = 0; i < 18; i++)
  {
      ...
      IlcMatrix(idrotm[2090+i],  90.0, phi0,  90.0, phi0 + 270., 0., 0.);
  ...
  gMC->Gspos("BFMO", i + 19, "ILCM", 0, 0, - 376. - kBFMdz/2. - 0.5 , 0, "ONLY");       
...
//
  ...
  IlcMatrix(idrotm[2092],  90.0,  90.0,   0.0,   0.0,   90.0,  0.0);
 ...
 //  Inner
  ...
  gMC->Gspos("BBMO", 1, "ILCM", 0., 0., + 376. + kBBMdz / 2. + 0.5, 0, "ONLY");  


}

//___________________________________________
void IlcFRAMEv2::AddAlignableVolumes() const
{
  // Add the 18 spaceframe sectors as alignable volumes
  TString basesymname("FRAME/Sector");
  TString basevolpath("ILCM_1/B077_1/BSEGMO");
  TString symname;
  TString volpath;
  ...
  for(Int_t sec=0; sec<18; sec++)
  {
	...
	IlcFatal(Form("Alignable entry %s not created. Volume path %s not valid",
	      symname.Data(),volpath.Data()));
  }
}

//___________________________________________
void IlcFRAMEv2::CreateMaterials()
{
  ...
  IlcMixture(65, "STAINLESS STEEL$", asteel, zsteel, 7.88, 4, wsteel);
  IlcMixture(5, "AIR$      ", aAir, zAir, dAir,4, wAir);
  IlcMaterial(9, "ALU      ", 26.98, 13., 2.7, 8.9, 37.2);

  IlcMedium(65, "Stainless Steel", 65, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  IlcMedium( 5, "Air", 5, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  IlcMedium( 9, "Aluminum", 9, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);

}

//_____________________________________________________________________________
void IlcFRAMEv2::Init()
{
  //
...
Int_t IlcFRAMEv2::IsVersion() const 
{
...
void IlcFRAMEv2::StepManager()
{
//
// Stepmanager of IlcFRAMEv2.cxx
...
//
  ...
  //
  AddTrackReference(gIlc->GetMCApp()->GetCurrentTrackNumber(), IlcTrackReference::kFRAME);
}

  

void IlcFRAMEv2::MakeHeatScreen(const char* name, Float_t dyP, Int_t rot1, Int_t rot2)
{
    // Heat screen panel
    //
    Int_t *idtmed = fIdtmed->GetArray()-1999;
...
void IlcFRAMEv2::WebFrame(const char* name, Float_t dHz, Float_t theta0, Float_t phi0)
{
    //

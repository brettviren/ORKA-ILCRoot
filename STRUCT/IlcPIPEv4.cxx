...
//_____________________________________________________________________________
IlcPIPEv4::IlcPIPEv4(const char *name, const char *title)
  : IlcPIPE(name,title),
    fRmax(1.98),            // outer radius of Be beam pipe
    fBe(0.08),              // width of Be beam pipe
    fZ1(50.),               // beginning of beam pipe z location (A side)
...
//_____________________________________________________________________________
IlcPIPEv4::IlcPIPEv4(const char *name, const char *title, const Float_t rmax, const Float_t width)
  : IlcPIPE(name,title),
...
//_____________________________________________________________________________
IlcPIPEv4::IlcPIPEv4(const char *name, const char *title, const Float_t rmax, const Float_t width, const Float_t z2)
  : IlcPIPE(name,title),
    fRmax(rmax),             // outer radius of Be beam pipe 
    fBe(width),              // width of Be beam pipe 
    fZ1(50.),                // beginning of beam pipe z location (A side)
...
void IlcPIPEv4::CreateGeometry()
{
  IlcDebug(1,"Create PIPEv4 geometry");
  //
  ...
  // Top volume
  TGeoVolume* top    = gGeoManager->GetVolume("ILCM");
  //
  //
  ////////////////////////////////////////////////////////////////////////////////     
  //                                                                            //
  ...
  //
  //
  //  The ILC central beam-pipe according to drawing         LHCVC2C_0001 
 ...
 */
  ...
  //  ILCFWDA_0025
  //    
  //  Plaque de Centrage  ILCFWDA_0019
...
//___________________________________________
void IlcPIPEv4::CreateMaterials()
{
  //
  // Define materials for beam pipe
  //

  IlcDebugClass(1,"Create PIPEv4 materials");
  ...
  IlcMaterial(2,  "SILICON$",28.09 , 14.00 , 2.33 , 9.36 , 45.); 

  //
  //     Berillium 
  IlcMaterial(5, "BERILLIUM$", 9.01, 4., 1.848, 35.3, 36.7);
  //
  //     Carbon 
  IlcMaterial(6,  "CARBON$   ", 12.01, 6., 2.265, 18.8, 49.9);
  //
  //     Aluminum 
  IlcMaterial(9,  "ALUMINIUM$", 26.98, 13., 2.7, 8.9, 37.2);
  //
  //     Copper 
  IlcMaterial(10, "COPPER", 63.55, 29, 8.96, 1.43, 85.6/8.96);
  //
  //     Air 
  IlcMixture(15, "AIR$      ", aAir, zAir, dAir, 4, wAir);
  IlcMixture(35, "AIR_HIGH$ ", aAir, zAir, dAir, 4, wAir);
  //
  //     Vacuum 
  IlcMixture(16, "VACUUM$ ", aAir, zAir, dAir1, 4, wAir);
  //
  //     stainless Steel 
  IlcMixture(19, "STAINLESS STEEL$", asteel, zsteel, 7.88, 4, wsteel);
  //
  //     reduced density steel to approximate pump getter material
  IlcMixture(20, "GETTER$", asteel, zsteel, 1.00, 4, wsteel);
  //     Al-Be alloy
  //     
  IlcMixture(21, "AlBe$", aAlBe, zAlBe, 2.07, 2, wAlBe); 
  //     Polyamid
  //   
  IlcMixture(22, "PA$", aPA, zPA, 1.14, -4, wPA);
  //
  //     Kapton
  IlcMixture(23, "KAPTON", aKapton, zKapton, dKapton, 4, wKapton);
  // Anticorodal 
  IlcMixture(24, "ANTICORODAL", aaco, zaco, 2.66, 3, waco);
  ...
  //    NEG
  IlcMixture(25, "NEG COATING", aNEG, zNEG, dNEG, -3, wNEG);
  
  //---------------------------------
  //  Aluminium AA5083 for MFT
  IlcMixture(63, "ALUMINIUM5083$",aALU5083,zALU5083, 2.66 ,4,wALU5083); // from aubertduval.fr
  // Aluminium AA2219 for MFT
  IlcMixture(64, "ALUMINIUM2219$",aALU2219,zALU2219, 2.84 ,6,wALU2219); // from aubertduval.fr
  //---------------------------------
  ...
  //
  // Silicon for ITS UPGRADE
  IlcMedium(2,  "SILICON",   2, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);


  //    Beryllium 
  
  IlcMedium(5, "BE",       5, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);

  //    Carbon 
  IlcMedium(6, "C",        6, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Aluminum 
  IlcMedium(9, "ALU",      9, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //    Copper 
  IlcMedium(10, "CU",      10, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Air 
  IlcMedium(15, "AIR",     15, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  IlcMedium(35, "AIR_HIGH",35, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Vacuum 
  IlcMedium(16, "VACUUM", 16, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Steel 
  IlcMedium(19, "INOX",   19, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Getter 
  IlcMedium(20, "GETTER", 20, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //   AlBe - Aloy 
  IlcMedium(21, "AlBe"  , 21, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //   Polyamid
  IlcMedium(22, "PA"  ,   22, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //   Antocorodal
  IlcMedium(24, "ANTICORODAL",   24, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //    Insulation Powder 
  ...
  //
  //   KAPTON
  IlcMedium(23, "KAPTON", 23, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);

  //
  //   NEG
  IlcMedium(25, "NEG COATING", 25, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);

  //----------------- for the MFT ----------------------
  IlcMedium(63,"AA5083", 63, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  IlcMedium(64,"AA2219", 64, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //----------------------------------------------------
...
TGeoPcon* IlcPIPEv4::MakeMotherFromTemplate(TGeoPcon* shape, Int_t imin, Int_t imax, Float_t r0, Int_t nz)
{
  //
...
TGeoPcon* IlcPIPEv4::MakeInsulationFromTemplate(TGeoPcon* shape)
{
  //
...
TGeoVolume* IlcPIPEv4::MakeBellow(const char* ext, Int_t nc, Float_t rMin, Float_t rMax, Float_t dU, Float_t rPlie, Float_t dPlie)
{

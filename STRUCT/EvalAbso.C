...
// Material numbers
    ...
    IlcABSO *pABSO  = (IlcABSO*) gIlc->GetModule("ABSO");
...
//
	...
	f0=f1=f2=0;
	for (i=0; i< nl[j]; i++) {
	    pABSO->IlcGetMaterial(mat[j][i], name, a, z, dens, radl, absl);
...
//
	...
	for (i=0; i< nl[0]; i++) {
	    Float_t a, z, dens, radl, absl;
	    pABSO->IlcGetMaterial(mat[0][i], name, a, z, dens, radl, absl);
...
// radative losses
    for (i=0; i< nl[1]; i++) {
	Float_t a, z, dens, radl, absl;
	pABSO->IlcGetMaterial(mat[1][i], name, a, z, dens, radl, absl);
	for (Int_t j=0; j<100; j++) {

...
//  IlcGenTherminator.cxx created by Adam Kisiel                           //
//                                                                         //
////////////////////////////////////////////////////////////////////////////
...
/* RHIC
  ...
  fHydjetParams.fIanglu=0;
*/
  strncpy(fParticleFilename, Form("%s/TUHKMgen/UHKM/particles.data", gSystem->Getenv("ILC_ROOT")), 255);
  strncpy(fDecayFilename, Form("%s/TUHKMgen/UHKM/tabledecay.txt", gSystem->Getenv("ILC_ROOT")), 255);
...
//_______________________________________
IlcGenUHKM::IlcGenUHKM(Int_t npart)
  :IlcGenMC(npart),
   fTrials(0),
   fUHKMgen(0),
   fHydjetParams(),
...
void IlcGenUHKM::SetAllParametersRHIC() 
{
...
void IlcGenUHKM::SetAllParametersLHC()
{
...
void IlcGenUHKM::Generate()
{
  ...
  IlcGenEventHeader* header = new IlcGenHijingEventHeader("UHKM");
  Double_t b       = 0.;
  Double_t npart   = 0; 
  Double_t nbin    = 0;
  ...
  ((IlcGenHijingEventHeader*) header)->SetImpactParameter(b);
  ((IlcGenHijingEventHeader*) header)->SetTotalEnergy(0.0);
  ((IlcGenHijingEventHeader*) header)->SetHardScatters(0);
  ((IlcGenHijingEventHeader*) header)->SetParticipants(Int_t(npart), 0);
  ((IlcGenHijingEventHeader*) header)->SetCollisions(Int_t(nbin), 0, 0, 0);
  ((IlcGenHijingEventHeader*) header)->SetSpectators(0, 0, 0, 0);
  ((IlcGenHijingEventHeader*) header)->SetReactionPlaneAngle(0);//evrot);
...
void IlcGenUHKM::Copy(TObject &) const
{
  Fatal("Copy","Not implemented!\n");
}

void IlcGenUHKM::SetAllParameters() {
  ...
  //  cout << "IlcGenUHKM::Init() no. stable flagged particles = " << fStableFlagged << endl;
  for(Int_t i=0; i<fStableFlagged; i++) {
    //    cout << "IlcGenUHKM::Init() flag no. " << i
...
cout<<" --------Flags------ "<<endl;
 ...
 cout<<" FlagWeakDecay= "<<fHydjetParams.fWeakDecay<<endl;
  ...
  //  cout << "IlcGenUHKM::SetAllParameters() OUT" << endl;
}

// add the additional PDG codes from UHKM(SHARE table) to ROOT's table
void IlcGenUHKM::CheckPDGTable() {

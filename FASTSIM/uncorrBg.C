...
//
//
    IlcFastGlauber*  glauber = new IlcFastGlauber();
    glauber->Init(1);
...
//
    ...
    Float_t scaleD0 = 1.28 * etar * ptUp / 1.35; // scaled by 1.35 to match ILC-INT-2002-6
    
//
//
//  Fast response
//    
    IlcFastMuonTriggerEff  *trigeff = new IlcFastMuonTriggerEff();
    IlcFastMuonTrackingAcc *acc     = new IlcFastMuonTrackingAcc();
    IlcFastMuonTrackingEff *eff     = new IlcFastMuonTrackingEff();
    IlcFastMuonTrackingRes *res     = new IlcFastMuonTrackingRes();
    acc->SetBackground(1.);
    eff->SetBackground(1.);
    res->SetBackground(1.);  
...
//  To be improved
//
    IlcFastDetector* tracker = new IlcFastDetector("Tracker", "Muon Tracker");
    tracker->AddResponse(acc);
    tracker->AddResponse(eff);
    tracker->AddResponse(res);

    IlcFastDetector* trigger = new IlcFastDetector("Trigger", "Muon Trigger");
    trigger->AddResponse(trigeff);

    IlcFastDetector* spectro = new IlcFastDetector("Spectro", "Muon Spectrometer");
...
//  pi/K -> mu
//
    TFile* f = new TFile("$(ILC_ROOT)/FASTSIM/data/pikmu.root");

#ifndef ILCQUENCHINGWEIGHTS_H
#define ILCQUENCHINGWEIGHTS_H
/* 2012-2013 Infrastructure for Large Collider Experiment.       *
 * All rights reserved.                                                   *
 * See cxx source for full Copyright notice                               */

/* $Id: IlcQuenchingWeights.h 48085 2011-03-05 18:37:52Z morsch $ */
...
class IlcQuenchingWeights : public TObject {
 public:
  enum kECMethod {kDefault=0,kReweight=1,kReweightCont=2};

  IlcQuenchingWeights();
  IlcQuenchingWeights(const IlcQuenchingWeights& a);
  IlcQuenchingWeights& operator=(const IlcQuenchingWeights& a)
      {a.Copy(*this); return(*this);}
  virtual ~IlcQuenchingWeights();
  void Init();
  void Reset();
  ...
  Int_t InitMult(const Char_t *contall="$(ILC_ROOT)/FASTSIM/data/cont_mult.all",
                 const Char_t *discall="$(ILC_ROOT)/FASTSIM/data/disc_mult.all"); 

  //single hard scattering approximation
  Int_t InitSingleHard(const Char_t *contall="$(ILC_ROOT)/FASTSIM/data/cont_lin.all",
                       const Char_t *discall="$(ILC_ROOT)/FASTSIM/data/disc_lin.all"); 
 ...
 protected:
  ...
  ClassDef(IlcQuenchingWeights,1)    // Base class for Quenching Weights
};

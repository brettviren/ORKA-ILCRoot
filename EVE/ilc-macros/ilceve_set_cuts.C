...
namespace
{
   ...
   const char *gMacroSaveAsTypes[] = {"IlcEve settings", "*.ilceve", 0, 0};
}
...
SetCutsWindow::SetCutsWindow() : TGMainFrame(gClient->GetRoot(), 10, 10, kHorizontalFrame)
{
  ...
  TGPictureButton* b1 = new TGPictureButton(hframe, gClient->GetPicture("$ILC_ROOT/EVE/ilc-data/ORKA.xpm"));
...
/*
   ...
   else
   {
      ...
      gOverlayButton3D = new TGLOverlayButton(glv,  "ILC run 123456 event 123", 0, 0, multiView->Get3DView()->GetEveFrame()->GetWidth(), 30);
      gOverlayButton3D->SetAlphaValues(0.5, 0.5);

      glv = (dynamic_cast<TEveViewer*>(gEve->GetViewers()->FindChild("RPhi View")))->GetGLViewer();
      gOverlayButtonRPhi = new TGLOverlayButton(glv,  "ILC run 123456 event 123", 0, 0, multiView->GetRPhiView()->GetEveFrame()->GetWidth(), 30);
      gOverlayButtonRPhi->SetAlphaValues(0.5, 0.5);

      glv = (dynamic_cast<TEveViewer*>(gEve->GetViewers()->FindChild("RhoZ View")))->GetGLViewer();
      gOverlayButtonRhoZ = new TGLOverlayButton(glv,  "ILC run 123456 event 123", 0, 0, multiView->GetRhoZView()->GetEveFrame()->GetWidth(), 30);
...
void SetCutsWindow::DrawHistos()
{
   ...
   {
         nHistos1++;
         histPt = new TH1D("Pt\nSingle Event", "IlcEve Pt histogram", 100, 0.0, 5.0);
   }

   if(gDrawHistograms[1])
   {
         nHistos1++;
         histEta = new TH1D("#eta\nSingle Event", "IlcEve #eta histogram", 100, -1.5, 1.5);
   }

   if(gDrawHistograms[2])
   {
         nHistos1++;
         histPhi = new TH1D("#phi\nSingle Event", "IlcEve #phi histogram", 100, 0.0, 2*TMath::Pi());
   }

   if(gDrawHistograms[3])
   {
         nHistos2++;
         histPhiPt = new TH2D("#phi-Pt\nSingle Event", "IlcEve #phi-Pt histogram", 100, 0.0, 2*TMath::Pi(), 100, 0.0, 5.0);
   }

   if(gDrawHistograms[4])
   {
         nHistos2++;
         histPtY = new TH2D("Pt-Y\nSingle Event", "IlcEve Pt-y histogram", 100, 0.0, 5.0, 100, -1.5, 1.5);
   }

   if(gDrawHistograms[5])
   {
         nHistos2++;
         histEtaPhi = new TH2D("#eta-#phi\nSingle Event", "IlcEve #eta-#phi histogram", 100, -1.5, 1.5, 100, 0.0, 2*TMath::Pi());
   }
   ...
   if(esd->GetNumberOfTracks())
   {
      ...
      for(Int_t j = 0; j < esd->GetNumberOfTracks(); j++)
      {
         ...
         IlcESDtrack* track = esd->GetTrack(j);
   ...
   {
      case 1:
         pad1 = new TCanvas("IlcEve 1D histograms - Single Event","IlcEve 1D histograms - Single Event",800,600);
         if(gDrawHistograms[0])
            histPt->Draw();
         if(gDrawHistograms[1])
            histEta->Draw();
      ...
      case 2:
         pad1 = new TCanvas("IlcEve 1D histograms - Single Event","IlcEve 1D histograms - Single Event",1200,500);
         pad1->Divide(2,1);
         if(!gDrawHistograms[0])
         {
            pad1->cd(1);
   ...
   {
      case 1:
         pad2 = new TCanvas("IlcEve 2D histograms - Single Event","IlcEve 2D histograms - Single Event",800,600);
         if(gDrawHistograms[3])
            histPhiPt->Draw();
         if(gDrawHistograms[4])
            histPtY->Draw();
      ...
      case 2:
         pad2 = new TCanvas("IlcEve 2D histograms - Single Event","IlcEve 2D histograms - Single Event",1200,500);
         pad2->Divide(2,1);
         if(!gDrawHistograms[3])
         {
            pad2->cd(1);
...
//   TH1D* histMult = new TH1D("Multiplicity\n", "IlcEve Multiplicity histogram", 1000, 0.0, 1000.0);
//   TH1D* histNcls = new TH1D("Number of ITS clusters\n", "IlcEve Multiplicity histogram", 10000, 0.0, 10000.0);

   if(gDrawHistograms[6])
   {
         nHistos1++;
         histPt = new TH1D("Pt\n", "IlcEve Pt histogram", 1000, 0.0, 1000.0);
   }

   if(gDrawHistograms[7])
   {
         nHistos1++;
         histEta = new TH1D("#eta\nAll Events", "IlcEve #eta histogram", 100, -1.5, 1.5);
   }

   if(gDrawHistograms[8])
   {
         nHistos1++;
         histPhi = new TH1D("#phi\nAll Events", "IlcEve #phi histogram", 100, 0.0, 2*TMath::Pi());
   }

   if(gDrawHistograms[9])
   {
         nHistos2++;
         histPhiPt = new TH2D("#phi-Pt\nAll Events", "IlcEve #phi-Pt histogram", 100, 0.0, 2*TMath::Pi(), 100, 0.0, 5.0);
   }

   if(gDrawHistograms[10])
   {
         nHistos2++;
         histPtY = new TH2D("Pt-Y\nAll Events", "IlcEve Pt-y histogram", 100, 0.0, 5.0, 100, -1.5, 1.5);
   }

   if(gDrawHistograms[11])
   {
         nHistos2++;
         histEtaPhi = new TH2D("#eta-#phi\nAll Events", "IlcEve #eta-#phi histogram", 100, 1.5, 1.5, 100, 0.0, 2*TMath::Pi());
   }

   Int_t nEvents = IlcEveEventManager::GetMaster()->GetMaxEventId();
...
*/
   ...
   {
      case 1:
         pad1 = new TCanvas("IlcEve 1D histograms - All Events","IlcEve 1D histograms - All Events",800,600);
         if(gDrawHistograms[6])
            histPt->Draw();
         if(gDrawHistograms[7])
            histEta->Draw();
      ...
      case 2:
         pad1 = new TCanvas("IlcEve 1D histograms - All Events","IlcEve 1D histograms - All Events",1200,500);
         pad1->Divide(2,1);
         if(!gDrawHistograms[6])
         {
            pad1->cd(1);
   ...
   {
      case 1:
         pad2 = new TCanvas("IlcEve 2D histograms - All Events","IlcEve 2D histograms - All Events",800,600);
         if(gDrawHistograms[9])
            histPt->Draw();
         if(gDrawHistograms[10])
            histEta->Draw();
      ...
      case 2:
         pad2 = new TCanvas("IlcEve 2D histograms - All Events","IlcEve 2D histograms - All Events",1200,500);
         pad2->Divide(2,1);
         if(!gDrawHistograms[9])
         {
            pad2->cd(1);
...
/*
   pad3 = new TCanvas("IlcEve 1D histograms - All Events","IlcEve 1D histograms - All Events",1200,500);
   pad3->Divide(2,1);
   pad3->cd(1);
   histMult->Draw();
   pad3->cd(2);
   ...
   if(gDrawTracks->IsOn() || gDrawV0s->IsOn() || gDrawCascades->IsOn() || gDrawKinks->IsOn())
   {
      ...
      for(k = i; k != j; k++)
      {
         ...
         for(l = m; l != n; l++)
         {
            ...
            IlcEveTrack *track = dynamic_cast<IlcEveTrack*>((TEveElement*) *l);
...
/*
      ...
      for(k = i; k != j; k++)
      {
         ...
         IlcEveV0 *v0 = dynamic_cast<IlcEveV0*>((TEveElement*) *k);
      ...
      for(k = i; k != j; k++)
      {
         ...
         IlcEveCascade *cascade = dynamic_cast<IlcEveCascade*>((TEveElement*) *k);
      ...
      for(k = i; k != j; k++)
      {
         ...
         IlcEveKink *kink = dynamic_cast<IlcEveKink*>((TEveElement*) *k);
...
/*
  myfile <<"//Macro with display settings generated automatically by IlcEve\n\n";

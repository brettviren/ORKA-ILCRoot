// $Id: kine_tracks.C 55699 2012-04-11 10:01:56Z hristov $
...
void    set_track_color(IlcEveTrack* t, Bool_t pdg_col);
...
void kine_track_propagator_setup(TEveTrackPropagator* trkProp)
{
  IlcMagF* fld = IlcEveEventManager::AssertMagField();

  if (g_kine_tracks_true_field)
  {
    trkProp->SetMagFieldObj(new IlcEveMagField(fld));
  }
  else
  {

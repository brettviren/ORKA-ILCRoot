...
# include "ilcroot/EVE/EveBase/IlcEveEventManager.h"
# include <iostream>
#endif
...
public:
  ...
  Display(const char* file="./")
    : fCb(0), 
      ...
      fHasRaw(kFALSE),
      fHasESD(kFALSE),
      fAutoRotateOn(kFALSE)
  {
    IlcEveEventManager::SetCdbUri(TString("local://$ILC_ROOT/OCDB"));
    LoadMacros();
    Setup(file, false);
    SetupSelect();
    SetupControl();
    gROOT->Macro(Form("AutoRotate.C(%d)",1));
    g_rotate_timer->Stop();
  ...
  void Setup(const char* file, bool refresh)
  {
    ...
    if (IlcEveEventManager::GetMaster()) delete IlcEveEventManager::GetMaster();
    TString eventName("Event"); // CINT has trouble with direct "Event".
    /* IlcEveEventManager::GetMaster() =*/
    new IlcEveEventManager(eventName, dirName, 0);
...
//     fAutoRotateOn = kFALSE;
  ...
  {
    Prepare();
    Int_t event = IlcEveEventManager::GetMaster()->GetEventId();
  ...
  Refresh()
  {
    ...
    RefreshOne(fEnableAutoRotate, fAutoRotateOn, "AutoRotate");
  ...
  void ToggleTracks() { Toggle(fEnableTracks, "MCTracks"); }
  //____________________________________________________________________
  ...
  ToggleAutoRotate()
  {
    Toggle(fEnableAutoRotate, "AutoRotate");
    Prepare();
    std::cout << "AutoRotate Camera... " << std::flush;
    if(fEnableAutoRotate) g_rotate_timer->Start();
    else g_rotate_timer->Stop();
    std::cout << "done" << std::endl;
    Refresh();
  }
  //____________________________________________________________________
...
protected:
  ...
  Bool_t fEnableAutoRotate; // Whether enable AutoRotate
  ...
  Bool_t fAutoRotateOn;   // Whether AutoRotate is on
  TList  fEnabled;
...
Display* display(const char* file="")
{
  ...
  IlcGeomManager::LoadGeometry(Form("%s/EVE/ilc-data/simple_geo.root", gSystem->Getenv("ILC_ROOT")));

// $Id: IlcEveJetPlane.cxx 31373 2009-03-09 11:00:26Z mtadel $
...
ClassImp(IlcEveJetPlane)

Bool_t IlcEveJetPlane::fgOneMomentumXYZ      = kFALSE;
Bool_t IlcEveJetPlane::fgOneMomentumPhiTheta = kFALSE;
Bool_t IlcEveJetPlane::fgOneEta              = kFALSE;
Bool_t IlcEveJetPlane::fgOneE                = kFALSE;
Bool_t IlcEveJetPlane::fgOneChgMass          = kFALSE;


IlcEveJetPlane::IlcEveJetPlane(Int_t iev) :
  TEveElementList(Form("IlcEveJetPlane %i",iev), Form("%i",iev)),
...
void IlcEveJetPlane::AddJet(IlcAODJet* jet)
{
...
void IlcEveJetPlane::AddTrack(IlcAODTrack* track)
{
...
void IlcEveJetPlane::CreateArrows()
{
  ...
  {
    std::vector<IlcAODTrack>::iterator k = fTracks.begin();
    std::vector<IlcAODJet>::iterator   j = fJets.begin();
  ...
  if ( ! fSelConnected)
  {
    gEve->GetSelection()->Connect("SelectionAdded(TEveElement*)",
       "IlcEveJetPlane", this, "SelectionAdded(TEveElement*)");
    fSelConnected = kTRUE;
  }
}
...
void IlcEveJetPlane::SelectionAdded(TEveElement* el)
{
  ...
  if (fOneSelection)
  {
    if (HasChild(el))
    {
      ...
      IlcAODTrack *k = dynamic_cast<IlcAODTrack*>(src);
      IlcAODJet   *j = dynamic_cast<IlcAODJet*>  (src);
  ...
  if (fTwoSelection && sel->NumChildren() == 2)
  {
	...
	IlcVParticle *p    = dynamic_cast<IlcVParticle*>(src);
...
void IlcEveJetPlane::ComputeBBox()
{
...
void IlcEveJetPlane::Paint(Option_t* /*option*/)
{
  // Paint the object.
  ...
  if (reqSections == TBuffer3D::kNone) {
    // printf("IlcEveJetPlane::Paint viewer was happy with Core buff3d.\n");
    return;
  }
}
